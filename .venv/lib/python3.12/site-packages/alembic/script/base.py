...
try:
    if compat.py39:
        ...
        from backports.zoneinfo import (  # type: ignore[import-not-found,no-redef] # noqa: E501; type: ignore[no-redef] # noqa: E501
            ZoneInfo,
...
class ScriptDirectory:
    ...
    def __init__(
        ...
        version_locations: Optional[Sequence[Union[str, os.PathLike[str]]]] = None,
        sourceless: bool = False,
        output_encoding: str = "utf-8",
        timezone: Optional[str] = None,
        hooks: list[PostWriteHookConfig] = [],
        recursive_version_locations: bool = False,
        messaging_opts: MessagingOptions = cast("MessagingOptions", util.EMPTY_DICT),
    ) -> None:
    ...
    def from_config(cls, config: Config) -> ScriptDirectory:
        ...
        return ScriptDirectory(
            ...
            output_encoding=config.get_alembic_option("output_encoding", "utf-8"),
    ...
    def _generate_template(self, src: Path, dest: Path, **kw: Any) -> None:
        with util.status(f"Generating {dest.absolute()}", **self.messaging_opts):
    ...
    def _ensure_directory(self, path: Path) -> None:
        path = path.absolute()
        if not path.exists():
            with util.status(f"Creating directory {path}", **self.messaging_opts):
                os.makedirs(path)
    ...
    def _generate_create_date(self) -> datetime.datetime:
        if self.timezone is not None:
            ...
            create_date = datetime.datetime.now(tz=datetime.timezone.utc).astimezone(
                tzinfo
            )
        else:
...
class Script(revision.Revision):
    ...
    @classmethod
    def _list_py_dir(cls, scriptdir: ScriptDirectory, path: Path) -> List[Path]:
        paths = []
        for root, dirs, files in compat.path_walk(path, top_down=True):
            ...
            if scriptdir.sourceless:
                ...
                if py_cache_path.exists():
                    ...
                    names = {Path(filename).name.split(".")[0] for filename in files}
                    paths.extend(
                        py_cache_path / pyc
