...
from . import coroutines, events, futures, tasks
...
class BaseEventLoop(events.AbstractEventLoop):
    def __init__(self):
        ...
        self._debug = not sys.flags.ignore_environment and bool(
            os.environ.get("PYTHONASYNCIODEBUG")
        )
                        ...
                        for _, _, _, _, laddr in laddr_infos:
                            try:
                                ...
                                exc = OSError(
                                    ...
                                    "{!r}: {}".format(laddr, exc.strerror.lower()),
                                )
    ...
    def _create_connection_transport(
        self, sock, protocol_factory, ssl, server_hostname
    ):
    ...
    def create_server(
        ...
        *,
        ...
        if host is not None or port is not None:
            ...
            infos = yield from self.getaddrinfo(
                host, port, family=family, type=socket.SOCK_STREAM, proto=0, flags=flags
            )
            if not infos:
                raise OSError("getaddrinfo() returned empty list")
        ...
        **kwargs
        ...
        for arg in popen_args:
            if not isinstance(arg, (str, bytes)):
                raise TypeError(
                    "program arguments must be "
                    "a bytes or text string, not %s" % type(arg).__name__
                )
        protocol = protocol_factory()
        if self._debug:
            # don't log parameters: they may contain sensitive information
    ...
    def _run_once(self):
        ...
        if self._debug and timeout != 0:
            ...
            if timeout is None:
                ...
                logger.log(
                    level, "poll %.3f ms took %.3f ms: timeout", timeout * 1e3, dt * 1e3
                )
        else:
            event_list = self._selector.select(timeout)
