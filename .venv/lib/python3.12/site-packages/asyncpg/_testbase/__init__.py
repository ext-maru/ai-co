        ...
        try:
            ...
            if elapsed > delta:
                raise AssertionError(
                    ...
                    "than the expected maximum of {:0.3f}s".format(elapsed, delta)
                )
        ...
        for key in sorted(context):
            ...
            if key == "source_traceback":
                ...
                try:
                    ...
                    value = "Exception in __repr__ {!r}; " "value type: {!r}".format(
                        ex, type(value)
                    )
            lines.append("[{}]: {}\n\n".format(key, value))
...
class ClusterTestCase(TestCase):
    ...
    def setup_cluster(cls):
        ...
        if cls.cluster.get_status() != "running":
            cls.cluster.start(port="dynamic", server_settings=cls.get_server_settings())
...
class HotStandbyTestCase(ClusterTestCase):
    ...
    def setup_cluster(cls):
        ...
        cls.start_cluster(
            cls.master_cluster,
            server_settings={"max_wal_senders": 10, "wal_level": "hot_standby"},
        )
        ...
        try:
            ...
            cls.standby_cluster = cls.new_cluster(
                ...
                cluster_kwargs={"master": conn_spec, "replication_user": "replication"},
            )
            cls.start_cluster(
    ...
    @classmethod
    def get_connection_spec(cls, kwargs={}):
        primary_spec = cls.get_cluster_connection_spec(cls.master_cluster, kwargs)
        standby_spec = cls.get_cluster_connection_spec(cls.standby_cluster, kwargs)
        return {
    ...
    def connect_standby(cls, **kwargs):
        conn_spec = cls.get_cluster_connection_spec(cls.standby_cluster, kwargs)
        return pg_connection.connect(**conn_spec, loop=cls.loop)
