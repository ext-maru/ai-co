...
class Cluster:
    def __init__(self, data_dir, *, pg_config_path=None):
        ...
        self._pg_bin_dir = os.environ.get("PGINSTALLATION") or os.environ.get("PGBIN")
        self._pg_ctl = None
    ...
    def get_status(self):
        ...
        if (
            ...
            if not r:
                raise ClusterError(
                    "could not parse pg_ctl status output: {}".format(stdout.decode())
                )
    ...
    def init(self, **settings):
        ...
        if self.get_status() != "not-initialized":
            raise ClusterError(
                "cluster in {!r} has already been initialized".format(self._data_dir)
            )
        ...
        if settings:
            settings_args = ["--{}={}".format(k, v) for k, v in settings.items()]
    ...
    def stop(self, wait=60):
        process = subprocess.run(
            [self._pg_ctl, "stop", "-D", self._data_dir, "-t", str(wait), "-m", "fast"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            cwd=self._data_dir,
        )
    ...
    def reset_wal(self, *, oid=None, xid=None):
        ...
        if status == "not-initialized":
            raise ClusterError("cannot modify WAL status: cluster is not initialized")
    ...
    def reset_hba(self):
        ...
        status = self.get_status()
        if status == "not-initialized":
            raise ClusterError("cannot modify HBA records: cluster is not initialized")
    ...
    def add_hba_entry(
        ...
        *,
        ...
        if type != "local":
            if address is None:
                raise ValueError("{!r} entry requires a valid address".format(type))
            else:
        ...
        if auth_options is not None:
            record += " " + " ".join("{}={}".format(k, v) for k, v in auth_options)
    ...
    def trust_local_replication_by(self, user):
        if _system != "Windows":
            self.add_hba_entry(
                type="local", database="replication", user=user, auth_method="trust"
            )
        self.add_hba_entry(
            type="host",
            address="127.0.0.1/32",
    ...
    def _find_pg_config(self, pg_config_path):
        if pg_config_path is None:
            pg_install = os.environ.get("PGINSTALLATION") or os.environ.get("PGBIN")
            if pg_install:
                pg_config_path = platform_exe(os.path.join(pg_install, "pg_config"))
            else:
                pathenv = os.environ.get("PATH").split(os.pathsep)
                for path in pathenv:
                    pg_config_path = platform_exe(os.path.join(path, "pg_config"))
        ...
        if not os.path.isfile(pg_config_path):
            raise ClusterError("{!r} is not an executable".format(pg_config_path))
    ...
    def _get_pg_version(self):
        ...
        if not version_string.startswith(prefix):
            raise ClusterError(
                "could not determine server version from {!r}".format(version_string)
            )
