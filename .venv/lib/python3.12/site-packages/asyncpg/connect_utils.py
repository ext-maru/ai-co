    ...
    for i, hostspec in enumerate(hostspecs):
        if hostspec[0] == "/":
            ...
            if m:
                ...
                raise exceptions.ClientConfigurationError(
                    "invalid IPv6 address in the connection URI: {!r}".format(hostspec)
                )
        else:
            # IPv4 address
...
def _parse_connect_dsn_and_args(
    *,
    ...
    if direct_tls is not None:
        sslneg = SSLNegotiation.direct if direct_tls else SSLNegotiation.postgres
    else:
        ...
        if sslnegotiation is not None:
            try:
                ...
                modes = ", ".join(m.name.replace("_", "-") for m in SSLNegotiation)
                raise exceptions.ClientConfigurationError(
    ...
    if command_timeout is not None:
        try:
            ...
            raise ValueError(
                ...
                "expected greater than 0 float (got {!r})".format(command_timeout)
            ) from None
        ...
        if data == b"S":
            ...
            self.on_data.set_exception(
                ConnectionError(
                    ...
                    "rejected SSL upgrade".format(host=self.host, port=self.port)
                )
            )
...
async def _create_ssl_connection(
    ...
    tr, pr = await loop.create_connection(
        lambda: TLSUpgradeProto(loop, host, port, ssl_context, ssl_is_advisory),
        host,
        port,
    )
    ...
    if hasattr(loop, "start_tls"):
        if do_ssl_upgrade:
            try:
                new_tr = await loop.start_tls(tr, pr, ssl_context, server_hostname=host)
            except (Exception, asyncio.CancelledError):
                tr.close()
                raise
        else:
        ...
        conn_factory = functools.partial(loop.create_connection, protocol_factory)
...
async def _connect_addr(*, addr, loop, params, config, connection_class, record_class):
...
async def __connect_addr(
    ...
    if isinstance(addr, str):
        ...
        connector = loop.create_connection(proto_factory, *addr, ssl=params.ssl)
