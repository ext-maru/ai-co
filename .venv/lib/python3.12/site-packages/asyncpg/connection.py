...
class Connection(metaclass=ConnectionMeta):
    ...
    def __init__(
        ...
        self._stmt_cache = _StatementCache(
            ...
            on_remove=functools.partial(_weak_maybe_gc_stmt, weakref.ref(self)),
    ...
    async def _get_statement(
        ...
        *,
        ...
        if use_cache:
            statement = self._stmt_cache.get((query, record_class, ignore_custom_codec))
        ...
        if need_reprepare or (not statement.name and not self._stmt_cache_enabled):
        ...
        if use_cache:
            self._stmt_cache.put((query, record_class, ignore_custom_codec), statement)
    ...
    def cursor(self, query, *args, prefetch=None, timeout=None, record_class=None):
    ...
    async def fetchmany(self, query, args, *, timeout: float = None, record_class=None):
    ...
    async def set_type_codec(
        ...
        if introspection.is_scalar_type(typeinfo):
            ...
            kind = "composite"
            full_typeinfos, _ = await self._introspect_types((typeinfo["oid"],), 10)
        else:
        ...
        settings.add_python_codec(
            oid, typename, schema, full_typeinfos, kind, encoder, decoder, format
        )
        ...
        if listeners:
            ...
            w = exceptions.InterfaceWarning(
                ...
                "{type} listener{s}".format(
                    conn=self, c=count, type=listener_type, s="s" if count > 1 else ""
                )
            )
...
class _StatementCache:
    __slots__ = ("_loop", "_entries", "_max_size", "_on_remove", "_max_lifetime")
...
class _Callback(typing.NamedTuple):
    ...
    def from_callable(cls, cb: typing.Callable[..., None]) -> "_Callback":
        if inspect.iscoroutinefunction(cb):
            ...
            raise exceptions.InterfaceError(
                "expected a callable or an `async def` function," "got {!r}".format(cb)
            )
...
def _check_record_class(record_class):
    if record_class is protocol.Record:
        pass
    elif isinstance(record_class, type) and issubclass(record_class, protocol.Record):
        if (
