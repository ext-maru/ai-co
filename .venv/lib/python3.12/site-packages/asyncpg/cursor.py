...
from . import connresource, exceptions
...
class CursorFactory(connresource.ConnectionResource):
    ...
    def __init__(self, connection, query, state, args, prefetch, timeout, record_class):
...
class BaseCursor(connresource.ConnectionResource):
    ...
    def _check_ready(self):
        if self._state is None:
            raise exceptions.InterfaceError("cursor: no associated prepared statement")

        if self._state.closed:
            raise exceptions.InterfaceError("cursor: the prepared statement is closed")
    ...
    async def _bind_exec(self, n, timeout):
        ...
        if self._portal_name:
            raise exceptions.InterfaceError("cursor already has an open portal")
    ...
    async def _exec(self, n, timeout):
        ...
        if not self._portal_name:
            raise exceptions.InterfaceError("cursor does not have an open portal")
    ...
    def __repr__(self):
        ...
        return '<{}.{} "{!s:.30}" {}{:#x}>'.format(
            mod, self.__class__.__name__, self._state.query, " ".join(attrs), id(self)
        )
...
class CursorIterator(BaseCursor):
    ...
    def __init__(self, connection, query, state, args, record_class, prefetch, timeout):
