        ...
        def call_con_method(self, *args, **kwargs):
            ...
            if self._con is None:
                raise exceptions.InterfaceError(
                    ...
                    "connection has been released back to the pool".format(meth_name)
                )
...
class PoolConnectionProxy(
    ...
    def __init__(self, holder: "PoolConnectionHolder", con: connection.Connection):
        self._con = con
        self._holder = holder
        con._set_proxy(self)
...
class PoolConnectionHolder:
    ...
    async def acquire(self) -> PoolConnectionProxy:
        if self._con is None or self._con.is_closed():
            ...
            self._pool._loop.create_task(self._con.close(timeout=self._timeout))
            self._con = None
            await self.connect()
...
class Pool:
    ...
    def __init__(
        ...
        **connect_kwargs,
        ...
        if min_size < 0:
            raise ValueError("min_size is expected to be greater or equal to zero")
    ...
    async def _async__init__(self):
        ...
        if self._initializing:
            raise exceptions.InterfaceError("pool is being initialized in another task")
        if self._closed:
