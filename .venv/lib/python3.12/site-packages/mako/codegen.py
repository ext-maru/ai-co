...
from mako import ast, exceptions, filters, parsetree, util
...
class _GenerateRenderMethod:
    ...
    def write_toplevel(self):
        ...
        if self.compiler.future_imports:
            self.printer.writeline(
                "from __future__ import %s" % (", ".join(self.compiler.future_imports),)
            )
        ...
        self.printer.writeline(
            "_exports = %r" % [n.name for n in main_identifiers.topleveldefs.values()]
        )
            ...
            if ident in comp_idents:
                ...
                if getattr(self.compiler, "has_ns_imports", False):
                    if self.compiler.strict_undefined:
                        self.printer.writelines(
                            "%s = _import_ns.get(%r, UNDEFINED)" % (ident, ident),
                        ...
                        self.printer.writeline(
                            "%s = _import_ns.get"
                            "(%r, context.get(%r, UNDEFINED))" % (ident, ident, ident)
                        )
                else:
                    if self.compiler.strict_undefined:
                        self.printer.writelines(
    ...
    def visitTextTag(self, node):
        filtered = len(node.filter_args.args) > 0
        if filtered:
            self.printer.writelines("__M_writer = context._push_writer()", "try:")
        for n in node.nodes:
            n.accept_visitor(self)
        if filtered:
            self.printer.writelines(
    ...
    def visitCode(self, node):
        if not node.ismodule:
            self.printer.write_indented_block(node.text, starting_lineno=node.lineno)
    ...
    def __init__(self, compiler, node=None, parent=None, nested=False):
        ...
        if illegal_names:
            raise exceptions.NameConflictError(
                "Reserved words declared in template: %s" % ", ".join(illegal_names)
            )
