        ...
        if isinstance(callee.expr, RefExpr):
            ...
            if value_type is not None:
                ...
                if mypy.checker.is_valid_inferred_type(key_type, self.chk.options):
                    ...
                    new_type = self.chk.named_generic_type(
    ...
    def check_argument_count(
        ...
        for i, kind in enumerate(callee.arg_kinds):
            ...
            if kind.is_required() and not mapped_args and not is_unexpected_arg_error:
                ...
                kind.is_named()
                and mapped_args
                and actual_kinds[mapped_args[0]]
                ...
                if not mapped_args and callee.special_sig != "partial":
                    ...
                    if (
                        actual_kinds[mapped_args[0]] == nodes.ARG_STAR
                        and paramspec_entries > 1
                    ):
                        self.msg.fail(
                            "ParamSpec.args should only be passed once", context
                        )
                        ok = False
                    if (
                        actual_kinds[mapped_args[0]] == nodes.ARG_STAR2
                        and paramspec_entries > 1
                    ):
                        self.msg.fail(
                            "ParamSpec.kwargs should only be passed once", context
                        )
                        ok = False
        return ok
        ...
        for name, method, obj, arg in variants:
            with self.msg.filter_errors(save_filtered_errors=True) as local_errors:
                result = self.check_method_call(
                    name, obj, method, [arg], [ARG_POS], context
                )
