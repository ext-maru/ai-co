...
def analyze_instance_member_access(
    ...
    if method and not isinstance(method, Decorator):
        ...
        if not method.is_static:
            if (
                isinstance(method, (FuncDef, OverloadedFuncDef))
                and method.is_trivial_self
            ):
...
def analyze_member_var_access(
    ...
    if isinstance(v, Var):
        ...
        return analyze_var(
            name, v, itype, mx, implicit=implicit, is_trivial_self=is_trivial_self
        )
...
def analyze_descriptor_access(descriptor_type: Type, mx: MemberContext) -> Type:
    ...
    mx.chk.warn_deprecated_overload_item(
        dunder_get,
        mx.context,
        target=inferred_dunder_get_type,
        selftype=descriptor_type,
    )
    ...
    if typ:
        ...
        if var.is_initialized_in_class and (not is_instance_var(var) or mx.is_operator):
            ...
            if isinstance(typ, FunctionLike) and not typ.is_type_obj():
                ...
                deco_mx = mx.copy_modified(
                    original_type=typ, self_type=typ, is_lvalue=False
                )
                call_type = get_proper_type(
                    _analyze_member_access("__call__", typ, deco_mx)
                )
            else:
                call_type = typ
        ...
        if call_type and not var.is_staticmethod:
            bound_items = []
            for ct in (
                call_type.items if isinstance(call_type, UnionType) else [call_type]
            ):
                p_ct = get_proper_type(ct)
                if isinstance(p_ct, FunctionLike) and (
                    not p_ct.bound() or var.is_property
                ):
                    item = expand_and_bind_callable(
                        p_ct, var, itype, name, mx, is_trivial_self
                    )
                else:
...
def expand_and_bind_callable(
    ...
    if is_trivial_self:
        ...
        typ = check_self_arg(
            typ, mx.self_type, var.is_classmethod, mx.context, name, mx.msg
        )
...
def analyze_class_attribute_access(
    ...
    *,
    ...
    if t:
        ...
        ) or (isinstance(node.node, SYMBOL_FUNCBASE_TYPES) and node.node.is_static)
        t = get_proper_type(t)
        is_trivial_self = False
        if isinstance(node.node, Decorator):
            # Use fast path if there are trivial decorators like @classmethod or @property
            is_trivial_self = (
                node.node.func.is_trivial_self and not node.node.decorators
            )
