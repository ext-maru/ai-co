...
class FindModuleCache:
            ...
            if should_cache:
                if (
                    not (
                        fast_path
                        or (
                            self.options is not None and self.options.fast_module_lookup
                        )
                    )
    ...
    def _can_find_module_in_parent_dir(self, id: str) -> bool:
        ...
        while any(is_init_file(file) for file in os.listdir(working_dir)):
            ...
            if not isinstance(
                parent_search._find_module(id, False)[0], ModuleNotFoundReason
            ):
                return True
        return False
    ...
    if os.path.isfile(gitignore):
        ...
        try:
            return parent_gitignores + [
                (dir, PathSpec.from_lines("gitwildmatch", lines))
            ]
