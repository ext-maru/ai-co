    ...
    for cls in ctx.cls.info.mro[:-1]:
        for name in _ORDERING_METHODS:
            if name in cls.names and name not in comparison_methods:
                ...
                if isinstance(node, SYMBOL_FUNCBASE_TYPES) and isinstance(
...
def partial_call_callback(ctx: mypy.plugin.MethodContext) -> Type:
    ...
    # ensure *args: P.args
    args_passed = any(
        part.flavor == ParamSpecFlavor.ARGS for part in passed_paramspec_parts
    )
    if not args_bound and not args_passed:
        ctx.api.expr_checker.msg.too_few_arguments(
            partial_type, ctx.context, actual_arg_names
        )
    ...
    kwargs_passed = any(
        part.flavor == ParamSpecFlavor.KWARGS for part in passed_paramspec_parts
    )
