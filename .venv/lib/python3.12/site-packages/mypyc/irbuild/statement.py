...
def transform_assignment_stmt(builder: IRBuilder, stmt: AssignmentStmt) -> None:
    ...
    if (
        ...
        for lvalue_item, rvalue_item in zip(first_lvalue.items, borrows):
            ...
            builder.assign(
                builder.get_assignment_target(lvalue_item), rvalue_item, line
            )
        builder.flush_keep_alives()
        return
...
def transform_type_alias_stmt(builder: IRBuilder, s: TypeAliasStmt) -> None:
    ...
    alias = builder.py_call(
        type_alias_type, args, line, arg_names=arg_names, arg_kinds=arg_kinds
    )
    ...
    builder.builder.primitive_op(
        set_type_alias_compute_function_op, [alias, compute_fn], line
    )
