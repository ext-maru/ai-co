...
def insert_spills(ir: FuncIR, env: ClassIR) -> None:
    ...
    entry_live = {
        op for op in entry_live if not (isinstance(op, Register) and op.is_arg)
    }
    ...
    for block in blocks:
        ...
        for i, op in enumerate(ops):
            ...
            if isinstance(op, DecRef) and op.src in spill_locs:
                ...
                if (
                    op.src not in live.after[block, i]
                    and env_reg in live.after[block, i]
                ):
                    # Skip the DecRef but null out the spilled location
                    null = LoadErrorValue(op.src.type)
                    block.ops.extend(
                        [null, SetAttr(env_reg, spill_locs[op.src], null, op.line)]
                    )
                continue
