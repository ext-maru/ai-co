...
def tilbert(x, h, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return tilbert(tmp.real, h, period, _cache) + 1j * tilbert(
            tmp.imag, h, period, _cache
        )
...
def itilbert(x, h, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return itilbert(tmp.real, h, period, _cache) + 1j * itilbert(
            tmp.imag, h, period, _cache
        )
    if period is not None:
        h = h * 2 * pi / period
...
def shift(x, a, period=None, _cache=_cache):
    ...
    if omega is None:
        ...
        omega_real = convolve.init_convolution_kernel(
            n, kernel_real, d=0, zero_nyquist=0
        )
        omega_imag = convolve.init_convolution_kernel(
            n, kernel_imag, d=1, zero_nyquist=0
        )
