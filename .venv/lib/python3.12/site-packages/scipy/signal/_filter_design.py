...
def lp2bp_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    z_bp = xp.concat(
        (z_lp + xp.sqrt(z_lp**2 - wo**2), z_lp - xp.sqrt(z_lp**2 - wo**2))
    )
    p_bp = xp.concat(
        (p_lp + xp.sqrt(p_lp**2 - wo**2), p_lp - xp.sqrt(p_lp**2 - wo**2))
    )
...
def lp2bs_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    # Duplicate poles and zeros and shift from baseband to +wo and -wo
    z_bs = xp.concat(
        (z_hp + xp.sqrt(z_hp**2 - wo**2), z_hp - xp.sqrt(z_hp**2 - wo**2))
    )
    p_bs = xp.concat(
        (p_hp + xp.sqrt(p_hp**2 - wo**2), p_hp - xp.sqrt(p_hp**2 - wo**2))
    )
