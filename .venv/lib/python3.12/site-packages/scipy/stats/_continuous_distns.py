...
class beta_gen(rv_continuous):
    ...
    def _entropy(self, a, b):
        ...
        def asymptotic_ab_large(a, b):
            ...
            t1 = (
                110 / sum_ab + 20 * sum_ab**-2.0 + sum_ab**-3.0 - 2 * sum_ab**-4.0
            )
...
class genextreme_gen(rv_continuous):
    ...
    def _stats(self, c):
        ...
        gam2k = xpx.apply_where(
            abs(c) >= 1e-7, c, gam2k_f, fill_value=np.pi**2.0 / 6.0
        )
        eps = 1e-14
    ...
    def _entropy(self, a, c):
        ...
        def asymptotic(a, c):
            ...
            return (
                ...
                + (a**-1.0) / 6
                - (a**-3.0) / 90
                + (np.log(a) - (a**-1.0) / 2 - (a**-2.0) / 12 + (a**-4.0) / 120)
                / c
            )
...
class skewnorm_gen(rv_continuous):
    ...
    def _munp(self, order, a):
        if order % 2:
            ...
            return (
                delta * self._skewnorm_odd_moments[order](delta**2) * _SQRT_2_OVER_PI
            )
        else:
...
class rel_breitwigner_gen(rv_continuous):
    ...
    def _munp(self, n, rho):
        ...
        if n == 2:
            # C = pi * k / (4 * rho)
            C = (
                np.sqrt((1 + 1 / rho**2) / (2 * (1 + np.sqrt(1 + 1 / rho**2))))
                * rho
            )
