...
# ToDoリストを読み込み
with open(
    "/home/aicompany/ai_co/ai_todo/ai_self_growth_20250703_111927.json", "r"
) as f:
    ...
    try:
        ...
        if task["type"] == "bash":
            result = subprocess.run(
                task["content"], shell=True, capture_output=True, text=True
            )
            ...
            result = subprocess.run(
                [sys.executable, str(temp_file)], capture_output=True, text=True
            )
            success = result.returncode == 0
            output = result.stdout + result.stderr
            temp_file.unlink()
            ...
            result = subprocess.run(
                ["ai-send"] + cmd_parts, capture_output=True, text=True
            )
            success = result.returncode == 0
            output = result.stdout + result.stderr
        ...
        if success:
            ...
            if "ModuleNotFoundError" in output:
                ...
                learning_points.append(
                    {"type": "auto_fix", "action": f"pip install {module_name}"}
                )
...
failed_tasks = sum(
    1 for log in execution_log if log.get("status") in ["failed", "error"]
)
...
# 結果を保存
result_file = (
    Path("/home/aicompany/ai_co/ai_todo")
    / f"{todo_data['name']}_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
)
...
if learning_points:
    knowledge_file = (
        Path("/home/aicompany/ai_co/knowledge_base/ai_learning")
        / f"learning_{datetime.now().strftime('%Y%m%d')}.jsonl"
    )
