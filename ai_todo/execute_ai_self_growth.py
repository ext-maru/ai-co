...
def execute_task_1():
    ...
    # 最近のログファイルからエラーを抽出
    if log_dir.exists():
        for log_file in sorted(log_dir.glob("*.log"), key=lambda x: x.stat().st_mtime)[
            -10:
        ]:
            try:
...
def execute_task_2():
    ...
    # エラーパターンの結果を使用
    kb_path = Path(
        "/home/aicompany/ai_co/knowledge_base/ai_learning/error_patterns.jsonl"
    )
    error_data = {}
    if kb_path.exists():
    ...
    # 提案を保存
    proposals_file = Path(
        "/home/aicompany/ai_co/ai_todo/system_optimization_proposals.json"
    )
...
def execute_task_3():
    ...
    report = {
        ...
        "completed_tasks": ["ワーカー状態分析", "エラーパターン学習", "最適化提案生成", "自己診断レポート作成"],
    ...
    with open(
        report_dir / f"self_diagnosis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        "w",
    ) as f:
...
def main():
    ...
    tasks = [execute_task_0, execute_task_1, execute_task_2, execute_task_3]
