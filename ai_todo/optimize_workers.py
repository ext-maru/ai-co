    ...
    if candidates:
        ...
        for worker in candidates:
            try:
                ...
                # 重要プロセスをスキップ
                if any(critical in cmdline for critical in ['claude', 'dashboard_server.py']):
                    print(f"   ⚠️  保護: {worker.name} (PID: {worker.pid}) - 重要プロセス")
                    continue
                
                # 重複workerのみ終了
                if 'worker' in cmdline and ('slack_polling' in cmdline or 'error_intelligence' in cmdline):
