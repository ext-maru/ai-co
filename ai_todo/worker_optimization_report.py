...
def generate_detailed_report():
    ...
    for worker in workers:
        try:
            ...
            worker_info = {
                ...
                "uptime_hours": (datetime.now() - worker.start_time).total_seconds() / 3600,
                "category": "unknown"
            }
            ...
            if worker_info["category"] == "error_intelligence" and len([w for w in workers if 'error_intelligence_worker' in (' '.join(psutil.Process(w.pid).cmdline()) if psutil.Process(w.pid).cmdline() else '')]) > 2:
                is_termination_candidate = True
            ...
            # 終了コマンド生成（管理者用）
            if is_termination_candidate and worker_info["category"] not in ["dashboard", "claude_cli"]:
    ...
    error_workers = [w for w in report["worker_details"] if w["category"] == "error_intelligence"]
    if len(error_workers) > 2:
        recommendations.append(f"{len(error_workers)}個のerror_intelligence_workerが稼働中。2個まで削減推奨")
    
    # アイドルワーカー
    idle_workers = [w for w in report["worker_details"] if w["cpu_percent"] < 0.5 and w["uptime_hours"] > 1]
    ...
    # レポート出力
    output_file = "/home/aicompany/ai_co/ai_todo/worker_optimization_detailed_report.json"
    ...
    for category, stats in category_stats.items():
        print(f"   {category}: {stats['count']}個 (平均CPU: {stats['total_cpu']/stats['count']:.1f}%, 平均メモリ: {stats['total_memory']/stats['count']:.1f}MB)")
