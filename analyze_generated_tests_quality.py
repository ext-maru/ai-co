...
class GeneratedTestsQualityAnalysis:
    ...
    def __init__(self):
        ...
        logger.info(f"Initialized quality analysis for {len(self.generated_test_files)} generated test files")
    ...
    async def analyze_all_generated_tests(self) -> Dict[str, Any]:
            ...
            try:
                ...
                file_result = {
                    ...
                    'elder_council_review': {
                        ...
                        'improvement_suggestions_count': len(elder_review.improvement_suggestions)
                    },
                    'quality_metrics': {
                        'overall_score': metrics_analysis['quality_scores']['overall_quality_score'],
                        'quality_grade': metrics_analysis['quality_scores']['quality_grade'],
                        'coverage_effectiveness': metrics_analysis['quality_scores']['coverage_effectiveness'],
                        'test_complexity': metrics_analysis['quality_scores']['test_complexity'],
                        'pattern_compliance': metrics_analysis['quality_scores']['pattern_compliance'],
                        'documentation_quality': metrics_analysis['quality_scores']['documentation_quality'],
                        'edge_case_coverage': metrics_analysis['quality_scores']['edge_case_coverage']
                    },
                    'test_method_count': metrics_analysis['detailed_metrics'].get('test_method_count', 0),
                    'key_insights': metrics_analysis['quality_insights'][:3],
                    'top_recommendations': metrics_analysis['improvement_recommendations'][:3]
                }
                
                individual_results.append(file_result)
                total_quality_score += elder_review.quality_metrics.overall_quality_score
                quality_distribution[metrics_analysis['quality_scores']['quality_grade']] += 1
        ...
        avg_quality_score = total_quality_score / len(successful_analyses) if successful_analyses else 0.0
        ...
        analysis_results['overall_quality_assessment'] = {
            ...
            'elder_council_assessment': self._assess_elder_council_view(successful_analyses),
            'total_test_methods': sum(r['test_method_count'] for r in successful_analyses),
            'coverage_achievement_support': self._assess_coverage_support(avg_quality_score, quality_distribution)
        }
        
        # Pattern compliance analysis
        analysis_results['pattern_compliance_analysis'] = await self._analyze_pattern_compliance(successful_analyses)
        
        # Elder Council recommendations
        analysis_results['elder_council_recommendations'] = self._generate_elder_recommendations(successful_analyses, avg_quality_score)
        
        # 4 Sages insights summary
        analysis_results['four_sages_insights'] = await self._get_four_sages_insights(successful_analyses)
        ...
        logger.info(f"Completed analysis: {avg_quality_score:.2f} average quality score")
    ...
    def _assess_elder_council_view(self, results: List[Dict]) -> str:
        ...
        approved = len([r for r in results if r['elder_council_review']['approval_status'] == 'approved'])
        approved_with_rec = len([r for r in results if r['elder_council_review']['approval_status'] == 'approved_with_recommendations'])
        needs_improvement = len([r for r in results if r['elder_council_review']['approval_status'] == 'needs_improvement'])
        rejected = len([r for r in results if r['elder_council_review']['approval_status'] == 'rejected'])
        ...
        if approval_rate >= 0.8:
            ...
            return "Elder Council approves most generated tests with some recommendations"
    ...
    def _assess_coverage_support(self, avg_score: float, distribution: Dict[str, int]) -> str:
        ...
        if avg_score >= 0.85 and excellence_rate >= 0.7:
            return "Strongly supports 66.7% coverage achievement with high-quality tests"
            ...
            return "Limited support for coverage achievement, quality improvements needed"
    ...
    async def _analyze_pattern_compliance(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        avg_pattern_compliance = sum(pattern_scores) / len(pattern_scores) if pattern_scores else 0
        avg_coverage_effectiveness = sum(coverage_scores) / len(coverage_scores) if coverage_scores else 0
        avg_test_complexity = sum(complexity_scores) / len(complexity_scores) if complexity_scores else 0
        ...
        return {
            ...
            'pattern_application_success_rate': high_pattern_compliance / len(pattern_scores) if pattern_scores else 0,
            'assessment': self._assess_pattern_success(avg_pattern_compliance, high_pattern_compliance, len(results)),
            'proven_patterns_detected': self._identify_proven_patterns_used(results)
        }
    
    def _assess_pattern_success(self, avg_compliance: float, high_compliance_count: int, total_count: int) -> str:
    ...
    def _identify_proven_patterns_used(self, results: List[Dict]) -> List[str]:
        ...
        high_quality_files = [r for r in results if r['quality_metrics']['overall_score'] >= 0.8]
        ...
        if any(r['quality_metrics']['pattern_compliance'] >= 0.85 for r in results):
            patterns_detected.append("Parametrized testing patterns from high-coverage sources")
        
        return patterns_detected
    
    def _generate_elder_recommendations(self, results: List[Dict], avg_score: float) -> Dict[str, Any]:
        ...
        if avg_score >= 0.8:
            recommendations['strategic_assessment'] = "Generated tests demonstrate strong quality and support coverage achievement goals"
        elif avg_score >= 0.7:
            recommendations['strategic_assessment'] = "Generated tests show good foundation with opportunities for excellence"
        else:
            recommendations['strategic_assessment'] = "Generated tests require significant quality improvements before deployment"
        
        # Immediate actions
        low_quality_files = [r for r in results if r['quality_metrics']['overall_score'] < 0.7]
        if low_quality_files:
            recommendations['immediate_actions'].append(f"Review and improve {len(low_quality_files)} low-quality generated test files")
        ...
        for issue, count in top_issues:
            if issue == 'documentation':
                recommendations['quality_improvements'].append("Enhance test documentation and method naming clarity")
            elif issue == 'edge_cases':
                recommendations['quality_improvements'].append("Expand edge case and error condition testing")
            elif issue == 'patterns':
                recommendations['quality_improvements'].append("Improve compliance with proven testing patterns")
    ...
    async def _get_four_sages_insights(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        if not results:
            return {'error': 'No successful analyses available for 4 Sages consultation'}
        ...
        try:
            ...
            request = QualityLearningRequest(
                ...
                improvement_goals=["Optimize generated test quality", "Improve pattern application"],
                context={'source': 'generated_tests_analysis', 'sample_count': len(results)},
                priority='medium'
            )
            ...
            return {
                ...
                'sage_recommendations': sage_consultation.get('recommended_actions', []),
                'expected_improvement': sage_consultation.get('expected_quality_improvement', 0.0),
                'focus_areas': sage_consultation.get('quality_consensus', {}).get('focus_areas', []),
    ...
    def _assess_coverage_impact(self, overall_assessment: Dict, file_count: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'coverage_achievement_alignment': self._assess_coverage_alignment(overall_assessment),
    ...
    def _assess_quality_sustainability(self, avg_score: float) -> str:
        ...
        if avg_score >= 0.8:
            return "High - Generated tests sustainable for long-term coverage maintenance"
        elif avg_score >= 0.7:
    ...
    def _assess_coverage_alignment(self, assessment: Dict) -> str:
        ...
        if 'Strongly supports' in support:
            ...
            return "Partial alignment - Generated tests partially support coverage goals"
        else:
            return "Poor alignment - Generated tests do not effectively support coverage achievement"
    
    async def save_analysis_report(self, analysis: Dict[str, Any], output_file: str = None):
    ...
    def print_summary_report(self, analysis: Dict[str, Any]):
        ...
        print(f"\nPattern Compliance Analysis:")
        print(f"  Average Pattern Compliance: {patterns['average_pattern_compliance']:.2f}")
        print(f"  Pattern Success Rate: {patterns['pattern_application_success_rate']:.1%}")
