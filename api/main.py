...
# 資料生成
@app.post("/api/projects/{project_id}/documentation", response_model=DocumentationResponse)
...
async def find_similar_projects(
    project_id: str,
    limit: int = Query(5, ge=1, le=20, description="取得件数制限")
):
    """
    類似プロジェクト検索
...
async def get_statistics():
    ...
    try:
        ...
        return {
            ...
            "most_used_tech": max(tech_counts.items(), key=lambda x: x[1]) if tech_counts else None
        }
