...
class TestExecutionMonitor:
    ...
    def run_tests_with_monitoring(
        self, test_files: List[Path] = None
    ) -> Dict[str, Any]:
        """テスト実行を監視し、失敗を自動修復"""
        results = {
            "total_tests": 0,
...
class PersistentTestKnight:
    ...
    def ensure_tests_work(self, test_files: List[Path] = None) -> Dict[str, Any]:
        """テストが動くまで継続実行"""
        self.logger.info(
            "🛡️ Persistent Test Knight deployed - Tests will work or we die trying!"
        )
    ...
    def _is_success(self, results: Dict[str, Any]) -> bool:
        ...
        return (
            ...
            and results["error_tests"] == 0
            and results["coverage_percent"] > 0.5  # 最低0.5%のカバレッジ
        )
    ...
    def _perform_repairs(self, results: Dict[str, Any]):
        ...
        if import_fixer.exists():
            try:
                subprocess.run(
                    ["python3", str(import_fixer)], cwd=str(self.project_root)
                )
...
class EnhancedCoverageKnightsBrigade:
    ...
    def _generate_additional_tests(
        self, current_results: Dict[str, Any], target_coverage: float
    ) -> Dict[str, Any]:
        """追加のテストを生成"""
