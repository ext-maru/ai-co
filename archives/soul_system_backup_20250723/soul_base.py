...
class BaseSoul(ABC):
    ...
    async def _main_loop(self):
        """メインイベントループ"""
        logger.info(f"Soul {self.soul_name} entering main loop")
    ...
    async def _periodic_tasks(self):
        """定期実行タスク"""
        # メトリクス更新
        self.metrics["uptime_seconds"] = self.context.get_runtime()
    ...
    async def request_shutdown(self):
        """シャットダウンリクエスト"""
        logger.info(f"Shutdown requested for soul: {self.soul_name}")
        self._running = False
