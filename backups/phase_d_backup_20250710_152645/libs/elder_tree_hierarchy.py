...
class ElderTreeHierarchy:
    ...
    def _add_council_members_to_sage(self, sage_node: ElderTreeNode, sage_type: SageType):
        """賢者配下に評議会メンバーを追加"""
        # 賢者タイプに応じた評議会メンバー配置
        council_mapping = {
            SageType.KNOWLEDGE: [CouncilRole.GRAND_SAGE_OF_KNOWLEDGE, CouncilRole.KEEPER_OF_ANCIENT_WISDOM],
        ...
        for role in council_roles:
            ...
            council_node = ElderTreeNode(ElderRank.COUNCIL_MEMBER, council_id, role.value)
    ...
    def _add_servants_to_council(self, council_node: ElderTreeNode, council_role: CouncilRole):
        """評議会メンバー配下にサーバント追加"""
        # 評議会役職に応じたサーバント配置
        servant_mapping = {
        ...
        for i, servant_type in enumerate(servant_types):
            ...
            servant_node = ElderTreeNode(ElderRank.SERVANT, servant_id, f"{servant_type.value} #{i}")
        ...
        try:
            ...
            if message.recipient_id:
                ...
                if sender.rank.value > recipient.rank.value:  # 上位への報告
                    ...
                    logger.warning(f"⚠️ Unauthorized message from {sender.name} to {recipient.name}")
                    return False
    ...
    def visualize_hierarchy(self) -> str:
        """階層構造の可視化"""
        def _build_tree_string(node: ElderTreeNode, prefix: str = "", is_last: bool = True) -> str:
            # 絵文字マッピング
            emoji_map = {
            ...
            for i, child in enumerate(children):
                ...
                result += _build_tree_string(child, prefix + extension, i == len(children) - 1)
