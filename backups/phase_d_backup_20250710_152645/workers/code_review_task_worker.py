...
class CodeReviewTaskWorker(AsyncBaseWorkerV2):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger.info(f"CodeReviewTaskWorker initialized as Elder Tree code analysis specialist")
    ...
    async def _analyze_code_with_elders(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """コード解析実行"""
    ...
    async def _re_analyze_improved_code_with_elders(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """改善されたコードの再解析"""
        payload = message["payload"]
    ...
    async def _report_analysis_patterns_to_knowledge_sage(self, result: Dict[str, Any]):
        ...
        try:
            ...
            for category in ['syntax_issues', 'logic_issues', 'performance_issues', 'security_issues']:
            ...
            self.logger.error(f"Failed to report analysis patterns to Knowledge Sage: {e}")
    
    async def _report_error_to_incident_sage(self, error: Exception, message: Dict[str, Any]):
    ...
    def get_elder_analysis_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'analysis_stats': {
                ...
                'average_issues_per_analysis': self.issues_found / max(1, self.analyses_performed)
            },
