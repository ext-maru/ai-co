...
try:
    ...
    from libs.elder_tree_hierarchy import (
        ...
        ElderTreeNode, ElderDecision, ServantType
    )
...
class CommandExecutorWorker(BaseWorker):
    ...
    def _initialize_elder_systems(self):
        ...
        if ELDER_TREE_AVAILABLE:
            try:
                ...
                self.logger.info("🧙‍♂️ Four Sages Integration initialized for command execution")
                
                # エルダー評議会召集システム初期化
                self.elder_council = ElderCouncilSummoner()
                self.logger.info("🏛️ Elder Council Summoner initialized for critical decisions")
            ...
            self.logger.info("Running in standalone mode without Elder Tree integration")
        ...
        if self.elder_tree:
            try:
                ...
                servant_info = {
                    ...
                    'capabilities': ['safe_execution', 'pattern_learning', 'result_reporting']
                }
                ...
                self.logger.info(f"🤖 Registered as Command Execution Servant under Task Sage")
    ...
    def _notify_elder_high_priority_task(self, command_id: str, command: str, description: str):
        """高優先度タスクをエルダーに通知"""
        ...
        try:
            ...
            self.logger.info(f"🏛️ Notified Task Sage of high priority command: {command_id}")
    ...
    def _execute_command(self, command_id: str, command: str, description: str) -> dict:
        ...
        try:
            ...
            if optimized_command and optimized_command != command:
                self.logger.info(f"📋 Task Sage optimized command: {command} -> {optimized_command}")
            ...
            if not self._is_safe_command(command):
                ...
                escalation_result = self._escalate_to_incident_sage(command_id, command, description)
            ...
            # 実行結果をナレッジ賢者に報告
            self._report_to_knowledge_sage(command_id, command, status, result, duration)
    ...
    def _escalate_to_incident_sage(self, command_id: str, command: str, description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return result or {'approved': False, 'reason': 'No response from Incident Sage'}
            
        except Exception as e:
            self.logger.error(f"Failed to escalate to Incident Sage: {e}")
            return {'approved': False, 'reason': f'Escalation failed: {str(e)}'}
    
    def _report_to_knowledge_sage(self, command_id: str, command: str, status: str, 
    ...
    def _analyze_with_rag_sage(self, command: str, status: str, result: subprocess.CompletedProcess):
        """RAG賢者でコマンドパターンを分析"""
        if not self.four_sages:
            return
        ...
        try:
            ...
            if patterns and patterns.get('similar_patterns'):
                ...
                self.logger.debug(f"🔍 RAG Sage identified {len(patterns['similar_patterns'])} similar patterns")
    ...
    def cleanup(self):
        ...
        try:
            ...
            # 最終統計をログ出力
            self.logger.info(f"📊 Final execution statistics: {json.dumps(self.execution_stats, indent=2)}")
            
            # 実行中のコマンドがあれば待機
            if self.running_dir.exists():
                running_files = list(self.running_dir.glob("*.json"))
                if running_files:
                    self.logger.info(f"Waiting for {len(running_files)} running commands to complete...")
                    time.sleep(5)  # 簡易的な待機
    ...
    def initialize(self) -> None:
        """ワーカーの初期化処理"""
        self.logger.info(f"🚀 {self.__class__.__name__} initializing with Elder Tree integration...")
    ...
    def handle_error(self, error: Exception, context: Dict[str, Any] = None, severity: ErrorSeverity = ErrorSeverity.MEDIUM):
        """エラーハンドリング（Elder報告付き）"""
        error_info = {
            'error_type': type(error).__name__,
            'error_message': str(error),
    ...
    def validate_config(self) -> bool:
        """設定の検証"""
        required_dirs = [self.base_dir, self.pending_dir, self.running_dir, 
                        self.completed_dir, self.logs_dir]
