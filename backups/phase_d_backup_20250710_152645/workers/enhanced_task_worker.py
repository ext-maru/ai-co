#!/usr/bin/env python3
"""
Elders Guild Enhanced TaskWorker with Prompt Template Support
...
class EnhancedTaskWorker(BaseWorker, PromptTemplateMixin):
    ...
    def process_message(self, ch, method, properties, body):
        ...
        try:
            ...
            if task_type in ['code_generation', 'system_task', 'complex_analysis']:
                task_advice = asyncio.run(self.consult_task_sage({
                    'id': task_id,
                    'type': task_type,
                    'prompt': user_prompt,
                    'priority': priority,
                ...
                if task_advice.get('available') and task_advice.get('recommended_approach'):
                    self.logger.info(f"{EMOJI['sage']} タスク賢者推奨アプローチ: {task_advice['recommended_approach']}")
            
            # テンプレート選択
            template_name = self._select_template(task_type, user_prompt)
            
            # 🌳 Elder Tree Integration: RAG賢者によるプロンプト強化
            enhanced_prompt = asyncio.run(self.enhance_prompt_with_rag_sage(
                user_prompt, 
                {
                    'task_id': task_id,
                    'task_type': task_type,
            ...
            generated_prompt = self.generate_prompt(
                ...
                variables={
                    ...
                    'sage_advice': task_advice.get('recommended_approach', '') if task_advice else ''
                },
                include_rag=True
            )
            ...
            result = self._execute_claude(task_id, generated_prompt, task_context={
                'task_data': task,
                'sage_advice': task_advice
            })
            ...
            if result['success']:
                ...
                asyncio.run(self.report_task_execution_to_knowledge_sage(
                    task_id, task, result
                ))
            else:
                # 失敗時の処理
                ...
                asyncio.run(self.escalate_critical_failure_to_incident_sage(
                    task_id, error_data
                ))
    ...
    def _execute_claude(self, task_id: str, prompt: str, task_context: dict = None):
        ...
        if task_context and task_context.get('sage_advice'):
            ...
            if sage_advice.get('available') and sage_advice.get('recommended_approach'):
                ...
                elder_recommendations += f"- Recommended Approach: {sage_advice['recommended_approach']}\n"
                ...
                if sage_advice.get('execution_strategy'):
                    elder_recommendations += f"- Execution Strategy: {sage_advice['execution_strategy']}\n"
                
                if sage_advice.get('risk_mitigation'):
                    elder_recommendations += f"- Risk Mitigation: {sage_advice['risk_mitigation']}\n"
        ...
        self.logger.info(f"{EMOJI['robot']} Executing Claude with Elder-enhanced prompt")
    ...
    async def consult_task_sage(self, task_data: dict) -> dict:
        ...
        try:
            ...
            consultation_request = {
                ...
                'resource_requirements': self._estimate_resource_requirements(task_data),
            ...
            task_advice = {
                ...
                'resource_optimization': self._extract_resource_optimization(sage_response),
                ...
                'consultation_confidence': sage_response.get('consensus_reached', False)
            }
            ...
            self.logger.info(f"{EMOJI['sage']} タスク賢者からのアドバイス受領: {task_advice['recommended_approach']}")
    ...
    async def report_task_execution_to_knowledge_sage(self, task_id: str, task_data: dict, result: dict) -> bool:
        """タスク実行結果をナレッジ賢者に報告"""
        try:
            if not self.four_sages:
                return False
            ...
            execution_analysis = {
                ...
                'knowledge_patterns': self._identify_knowledge_patterns(task_data, result),
                'worker_id': self.worker_id,
                'timestamp': datetime.now().isoformat()
            }
    ...
    async def escalate_critical_failure_to_incident_sage(self, task_id: str, error_data: dict) -> dict:
        ...
        try:
            ...
            escalation_result = {
                ...
                'incident_id': incident_analysis['incident_id'],
                'sage_response': incident_response,
                'auto_recovery_recommended': incident_response.get('learning_outcome', {}).get('auto_recovery', False),
                'manual_intervention_required': incident_analysis['severity'] in ['critical', 'high']
            }
            
            self.logger.error(f"{EMOJI['sage']} インシデント賢者への重要エスカレーション: {incident_analysis['incident_id']}")
    ...
    async def enhance_prompt_with_rag_sage(self, user_prompt: str, task_context: dict) -> str:
        """RAG賢者を使用してプロンプトを強化"""
        try:
            if not self.four_sages:
            ...
            enhanced_prompt = self._build_enhanced_prompt(
                user_prompt,
                rag_response.get('learning_outcome', {}),
                task_context
            )
            ...
            if self.elder_tree:
                await self._report_to_elder_tree(
                    ...
                    'rag_sage',
                    {
                        ...
                        'enhancement_quality': rag_response.get('consensus_reached', False),
                        'worker_id': self.worker_id
                    }
                )
            
            self.logger.info(f"{EMOJI['sage']} RAG賢者によるプロンプト強化完了: {len(user_prompt)} -> {len(enhanced_prompt)} characters")
    ...
    async def _report_to_elder_tree(self, report_type: str, sage_type: str, content: dict) -> bool:
        """エルダーツリーへの報告"""
        try:
    ...
    async def _escalate_to_elder_council(self, incident_data: dict) -> bool:
        ...
        try:
            ...
            self.logger.critical(f"🏛️ エルダー評議会への緊急エスカレーション: {incident_data['incident_id']}")
    ...
    def _assess_task_complexity(self, task_data: dict) -> str:
        ...
        # 複雑度キーワード
        complex_keywords = ['architecture', 'integration', 'migration', 'optimization', 'complex', 'advanced']
    ...
    def _estimate_resource_requirements(self, task_data: dict) -> dict:
        """タスクのリソース要求を推定"""
        prompt = task_data.get('prompt', '').lower()
        
        # CPU要求度
        cpu_intensive = any(word in prompt for word in ['compile', 'build', 'analyze', 'process'])
        
        # メモリ要求度
        memory_intensive = any(word in prompt for word in ['large', 'massive', 'dataset', 'database'])
        
        # ネットワーク要求度
        network_intensive = any(word in prompt for word in ['api', 'download', 'upload', 'web'])
    ...
    def _estimate_task_duration(self, task_data: dict) -> str:
        """タスクの推定実行時間"""
        complexity = self._assess_task_complexity(task_data)
        prompt_length = len(task_data.get('prompt', ''))
    ...
    def _extract_task_approach(self, sage_response: dict) -> str:
        """タスクアプローチの抽出"""
        learning_outcome = sage_response.get('learning_outcome', {})
        if isinstance(learning_outcome, dict):
            return learning_outcome.get('recommended_approach', 'Standard iterative approach')
        return 'Standard iterative approach'
    
    def _extract_resource_optimization(self, sage_response: dict) -> dict:
        """リソース最適化の抽出"""
        learning_outcome = sage_response.get('learning_outcome', {})
        if isinstance(learning_outcome, dict):
    ...
    def _extract_execution_strategy(self, sage_response: dict) -> str:
        """実行戦略の抽出"""
        learning_outcome = sage_response.get('learning_outcome', {})
        if isinstance(learning_outcome, dict):
    ...
    def _extract_risk_mitigation(self, sage_response: dict) -> list:
        """リスク軽減策の抽出"""
        learning_outcome = sage_response.get('learning_outcome', {})
        if isinstance(learning_outcome, dict):
    ...
    def _assess_output_quality(self, result: dict) -> float:
        """出力品質の評価"""
    ...
    def _extract_techniques_used(self, result: dict) -> list:
        """使用されたテクニックの抽出"""
        ...
        if 'template' in output:
            techniques.append('Template-based generation')
        if 'rag' in output:
    ...
    def _extract_challenges(self, result: dict) -> list:
        """遭遇した課題の抽出"""
        output = result.get('output', '').lower()
    ...
    def _extract_solutions(self, result: dict) -> list:
        """適用された解決策の抽出"""
        output = result.get('output', '').lower()
    ...
    def _assess_error_severity(self, error_data: dict) -> str:
        """エラーの重要度を評価"""
        ...
        if any(word in error_message for word in ['critical', 'fatal', 'system']):
            ...
            return 'high'
        elif any(word in error_message for word in ['permission', 'access', 'connection']):
            return 'medium'
        else:
            return 'low'
    
    def _assess_impact_scope(self, error_data: dict) -> str:
        """エラーの影響範囲を評価"""
        error_message = error_data.get('error', '').lower()
    ...
    def _build_enhanced_prompt(self, original_prompt: str, rag_outcome: dict, task_context: dict) -> str:
        """RAG知識を使用してプロンプトを強化"""
        ...
        if self.elder_tree:
            try:
                elder_tree_details = {
                    ...
                    'hierarchy_levels': ['Grand Elder', 'Claude Elder', 'Sages', 'Council', 'Servants'],
