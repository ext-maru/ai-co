...
class CommandExecutorWatchdog:
    ...
    def run(self):
        ...
        self.logger.info(f"{EMOJI['start']} Command Executor Watchdog started as Elder Tree guardian")
    ...
    def restart_executor(self):
        ...
        if self.restart_count >= self.max_restarts:
            error_msg = f"Maximum restart attempts ({self.max_restarts}) reached. Giving up."
        ...
        self.logger.info(f"Attempting to restart Command Executor (attempt {self.restart_count})")
...
"""
            ...
            if self.check_executor_running():
                ...
                self.logger.error("Command Executor failed to start after restart attempt")
                return False
    ...
    def _escalate_critical_failure_to_claude_elder(self):
        ...
        try:
            ...
            self.logger.error(f"Failed to escalate critical failure to Claude Elder: {e}")
    ...
    def get_elder_watchdog_status(self):
        ...
        status = {
            ...
            'uptime': (datetime.now() - self.created_at).total_seconds(),
            'status': 'healthy' if self.running and self.elder_systems_initialized else 'degraded'
        }
