...
class IntelligentPMWorker(AsyncBaseWorkerV2):
    ...
    def _initialize_elder_systems(self):
        ...
        try:
            ...
            self.logger.info(f"🌳 Elder Tree Integration initialized for {self.elder_id}")
            self.logger.info(f"📜 Four Sages initialization: {init_result['integration_status']}")
            ...
            self.logger.warning("⚠️ Intelligent PM Worker operating without Elder guidance")
    ...
    async def process_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if project_context.requires_elder_guidance and self.elder_integration_enabled:
            ...
            return {
                ...
                'elder_consultations': project_context.sage_consultations if project_context else {},
                'elder_recommendations': project_context.elder_recommendations if project_context else []
            }
    ...
    async def _select_ai_command(self, analysis: Dict[str, Any], project_context: Optional[ProjectContext] = None) -> Dict[str, Any]:
...
Respond in {language}."""
    ...
    async def _evaluate_and_decide_next_action(self, task_id: str, analysis: Dict[str, Any], 
                                              command_result: Dict[str, Any], is_slack_task: bool,
    ...
    async def _create_project_context(self, task_id: str, analysis: Dict[str, Any]) -> ProjectContext:
        """プロジェクトコンテキストの作成"""
        # プロジェクトタイプ判定
        user_intent = analysis['user_intent']
        project_type_map = {
        ...
        requires_elder_guidance = (
            complexity in ['complex', 'critical'] or
    ...
    async def _consult_elders(self, project_context: ProjectContext, analysis: Dict[str, Any]):
        ...
        try:
            ...
            if project_context.urgency == 'critical':
                ...
                if incident_sage_result['status'] == 'success':
                    project_context.sage_consultations['incident_sage'] = incident_sage_result
            ...
            if (project_context.complexity == 'critical' and 
                project_context.urgency == 'critical'):
                ...
                council_result = await self.elder_council_summoner.summon_council(council_trigger)
                
                if council_result and council_result.get('decision'):
                    project_context.sage_consultations['elder_council'] = council_result
                    project_context.elder_recommendations.insert(0,
    ...
    async def _report_to_knowledge_sage(self, task_id: str, analysis: Dict[str, Any], 
        ...
        try:
            knowledge_entry = {
                ...
                'insights': {
                    ...
                    'response_quality': 'complete' if analysis['response_analysis'].get('appears_complete') else 'partial',
        ...
        try:
            ...
            if result['status'] == 'success':
                ...
                if result.get('escalation_required'):
                    ...
                    await self.elder_council_summoner.escalate_to_claude_elder(council_trigger)
