...
class KnowledgeSchedulerWorker(BaseWorker):
    ...
    def __init__(self):
        ...
        logger.info(
            f"KnowledgeSchedulerWorker initialized as Elder Tree knowledge coordination specialist"
        )
        ...
        try:
            ...
            if schedule_type == "daily":
                schedule.every().day.at(schedule_time).do(
                    self._execute_knowledge_update, schedule_id, knowledge_source
                )
    ...
    def _perform_immediate_update(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self._execute_knowledge_update(
                f"immediate_{datetime.now().timestamp()}", knowledge_source
            )
    ...
    def _execute_knowledge_update(
        self, schedule_id: str, knowledge_source: Dict[str, Any]
    ):
        ...
        try:
            ...
            if self.elder_systems_initialized:
                self._report_update_start_to_knowledge_sage(
                    schedule_id, knowledge_source
                )
            ...
            if self.rag_integration and knowledge_source:
                ...
                if source_type == "file":
                    ...
                    if file_path and Path(file_path).exists():
                        ...
                        if self.four_sages:
                            self.four_sages.store_knowledge(
                                "scheduled_updates",
                                {
                                    "file": file_path,
                                    "timestamp": datetime.now().isoformat(),
            ...
            if self.elder_systems_initialized:
                self._report_update_completion_to_knowledge_sage(
                    schedule_id, update_result
                )
    ...
    def _query_schedule(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if schedule_id:
                ...
                if schedule_info:
                    return {"status": "success", "schedule": schedule_info}
                else:
                    return {
                        "status": "error",
    ...
    def _report_success_to_knowledge_sage(
    ...
    def _report_failure_to_incident_sage(
        self, task: Dict[str, Any], result: Dict[str, Any]
    ):
    ...
    def _report_update_start_to_knowledge_sage(
        self, schedule_id: str, knowledge_source: Dict[str, Any]
    ):
    ...
    def _report_update_completion_to_knowledge_sage(
    ...
    def _report_update_failure_to_incident_sage(
        self, schedule_id: str, error: Exception
    ):
        ...
        status = {
            ...
            "scheduler_stats": {
                ...
                "active_schedules": len(
                    [s for s in self.schedules.values() if s.get("status") == "active"]
                ),
