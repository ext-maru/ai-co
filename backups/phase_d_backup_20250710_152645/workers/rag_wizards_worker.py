...
class RAGWizardsWorker(BaseWorker):
    ...
    def __init__(self, worker_id="rag_wizards"):
        """初期化"""
        super().__init__(worker_type="rag_wizards", worker_id=worker_id)
    ...
    async def _process_knowledge_gap(self, message: Dict) -> Dict:
        ...
        # ウィザードで処理
        enrichment_result = await self.wizards_orchestrator.manual_trigger_learning(
            gap.topic
        )

        return {
            "gap_id": gap.gap_id,
            "enrichment_result": enrichment_result.__dict__
        ...
        if self.wizards_orchestrator:
            ...
            for wizard in self.wizards_orchestrator.hunter_wizards:
                wizard_states.append(
                    {"id": wizard.wizard_id, "state": wizard.state.value}
                )
        ...
        if self.rag_integration:
            try:
                unified_results = await self.rag_integration.search_unified(
                    query=query, limit=10, threshold=self.rag_config.search_threshold
                )
            except Exception as e:
    ...
    async def _get_wizard_insights(self, query: str) -> Dict:
        ...
        try:
            ...
            for gap_id, gap in detector.detected_gaps.items():
                if (
                    query.lower() in gap.topic.lower()
                    or query.lower() in gap.description.lower()
                ):
                    related_gaps.append(
                        {
                            "gap_id": gap.gap_id,
    ...
    async def _store_wizard_knowledge(
        self, topic: str, content: str, metadata: Dict = None
    ):
...
async def main():
    ...
    try:
        ...
        # ステータスレポートを生成
        status_message = {"task_id": "test_002", "task_type": "status_report"}
