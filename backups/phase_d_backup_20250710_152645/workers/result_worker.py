...
try:
    ...
    from libs.elder_tree_hierarchy import (
        ...
        ElderDecision, ElderTreeHierarchy
    )
...
class ResultWorkerV2(BaseWorker):
    ...
    def __init__(self):
        ...
        if ELDER_INTEGRATION_AVAILABLE:
            ...
            self.logger.warning("Elder integration libraries not available - running in legacy mode")
    ...
    def _verify_elder_connections(self):
        ...
        try:
            ...
            if self.four_sages:
                ...
                sages_health = getattr(self.four_sages, 'get_sages_status', lambda: True)()
                if sages_health:
            ...
            if self.elder_council:
                # Test basic functionality
                council_health = getattr(self.elder_council, 'get_council_status', lambda: True)()
                if council_health:
            ...
            self.elder_integration_status['initialization_errors'].append(f"Verification error: {e}")
    ...
    async def report_to_knowledge_sage(self, task_result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"📚 Knowledge Sage: Learning data submitted for task {task_result.get('task_id', 'unknown')}")
        ...
        try:
            ...
            message = ElderMessage(
                ...
                content={
                    ...
                    "timestamp": datetime.now().isoformat(),
                    "severity": "high" if "critical" in str(task_result.get('error', '')).lower() else "medium"
                },
                priority="high"
            )
            ...
            if success:
                ...
                self.logger.warning(f"🚨 Incident Sage: Task failure escalated {task_result.get('task_id', 'unknown')}")
    ...
    async def consult_rag_sage(self, task_result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """RAG Sageにエラー分析を依頼"""
        if not self.four_sages or not self.elder_tree:
            return None
        ...
        try:
            ...
            if success:
                ...
                if hasattr(self.four_sages, 'rag_sage_analyze_error'):
                    ...
                    self.stats['sage_consultations'] += 1
                    self.logger.info(f"🔍 RAG Sage: Error analysis completed for {task_result.get('task_id', 'unknown')}")
                    return analysis
    ...
    async def request_elder_council(self, pattern_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"🏛️ Elder Council: Pattern analysis requested - {pattern_data.get('pattern_type', 'unknown')}")
            ...
            # Failure case - Escalate to Incident Sage
                ...
                if self._is_critical_failure_pattern(task_result):
                    pattern_data = {
                        ...
                        'action_items': ['Investigate root cause', 'Implement preventive measures']
                    }
                    await self.request_elder_council(pattern_data)
    ...
    def _should_notify_council(self, task_result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if task_result.get('files_created') and len(task_result['files_created']) > 5:
                return True
    ...
    def _analyze_result_pattern(self, task_result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """結果パターンの分析"""
        try:
            pattern_data = {
    ...
    def _get_elder_insights(self, task_result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """エルダーからの洞察を取得"""
        try:
            insights = {
            ...
            if task_result.get('status') == 'completed':
                insights['recommendations'].append("✅ Success data submitted to Knowledge Sage for learning")
                
                if self._should_notify_council(task_result):
                    insights['escalation_info']['council_notification'] = "High-impact task flagged for Elder Council review"
                    
                # Performance insights
                duration = task_result.get('duration', 0)
                if duration > 30:
                    insights['recommendations'].append("⚠️ Long-running task detected - consider optimization")
                elif duration < 1:
                    insights['recommendations'].append("⚡ Fast execution - excellent performance")
                    
                # File creation insights
                files_count = len(task_result.get('files_created', []))
                if files_count > 5:
                    insights['recommendations'].append(f"📁 High file output ({files_count} files) - review organization")
                    
            # Failure case insights
            elif task_result.get('status') == 'failed' or task_result.get('error'):
                insights['recommendations'].append("🚨 Task failure escalated to Incident Sage")
                insights['escalation_info']['incident_sage'] = "Automated failure analysis initiated"
                
                if self._is_critical_failure_pattern(task_result):
                    insights['escalation_info']['elder_council'] = "Critical failure pattern - Council summoned"
                    
                # RAG analysis insight
                if task_result.get('rag_analysis'):
                    insights['learning_status']['rag_analysis'] = "Error pattern analysis completed"
                    
            # Elder Tree status
            if self.elder_tree:
                insights['elder_tree_status'] = f"Connected ({len(self.elder_tree.nodes)} nodes)"
            else:
    ...
    def process_message(self, ch, method, properties, body):
        ...
        try:
            ...
            if self.config.get('slack.enabled', False):
                # Elder insights を含める
                elder_insights = self._get_elder_insights(body) if ELDER_INTEGRATION_AVAILABLE else None
    ...
    def _format_elder_insights(self, elder_insights: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if elder_insights.get('elder_status'):
                ...
                parts.append(f"  - Four Sages: {'✅' if status.get('four_sages') else '❌'}")
                parts.append(f"  - Elder Council: {'✅' if status.get('elder_council') else '❌'}")
                parts.append(f"  - Elder Tree: {'✅' if status.get('elder_tree') else '❌'}")
    ...
    def _get_elder_stats_summary(self) -> List[str]:
        ...
        try:
            ...
            if self.elder_integration_status:
                active_systems = sum(1 for status in self.elder_integration_status.values() if status is True)
        ...
        try:
            ...
            if self.four_sages:
                report['system_health']['four_sages'] = {
                    'status': 'active',
                    'available_methods': [
                        method for method in dir(self.four_sages) 
            ...
            if self.elder_council:
                report['system_health']['elder_council'] = {
                    'status': 'active',
                    'available_methods': [
                        method for method in dir(self.elder_council) 
                        if not method.startswith('_')
                    ]
                }
            else:
