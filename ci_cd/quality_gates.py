...
class QualityGateSystem:
    ...
    def check_test_gate(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not passed:
            result['reason'] = f"テスト成功率が不足: {success_rate:.1f}% < {self.test_success_threshold}%"
    ...
    def check_security_gate(self, security_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not passed:
            ...
            if not score_passed:
                reasons.append(f"セキュリティスコアが不足: {security_score} < {self.security_threshold}")
    ...
    def generate_quality_report(
        ...
        """
        recommendations = self._generate_recommendations(evaluation_result['gate_results'])
    ...
    def _extract_deployment_status(self, evaluation_result: Dict[str, Any]) -> Dict[str, Any]:
        """デプロイメント状態を抽出"""
        return {
    ...
    def analyze_quality_trends(
        ...
        # カバレッジトレンド
        coverage_values = [item['coverage'] for item in historical_data] + [current_metrics['coverage']]
        coverage_trend = calculate_trend(coverage_values)
        
        # テスト成功率トレンド
        test_values = [item['test_success_rate'] for item in historical_data] + [current_metrics['test_success_rate']]
