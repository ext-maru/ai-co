    ...
    def send_to_claude(self, message: str, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                error_msg = result.stderr.strip() or result.stdout.strip() or "Claude CLIエラー"
                self.logger.error(f"Claude CLI error (exit {result.returncode}): {error_msg}")
                
                if "Invalid API key" in error_msg or "authentication" in error_msg.lower():
                    # API認証エラーの場合はAPI Directにフォールバック
                    if self.api_direct:
                        self.logger.info("Falling back to API Direct due to CLI auth issues")
