...
from libs.a2a_communication import (
    create_a2a_client, AGENT_REGISTRY, MessageType, MessagePriority
)
...
class A2ACommand(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        test_parser.add_argument('--source', default='knowledge_sage', 
                               help='Source agent ID')
        test_parser.add_argument('--target', default='task_sage',
                               help='Target agent ID')
        test_parser.add_argument('--message', default='test query',
        ...
        demo_parser.add_argument('--scenario', choices=['collaboration', 'optimization'],
                               default='collaboration', help='Demo scenario')
        
        # optimization サブコマンド
        opt_parser = subparsers.add_parser('optimize', help='Elder-Servant optimization')
        opt_parser.add_argument('--enable', action='store_true',
                              help='Enable optimization')
        opt_parser.add_argument('--disable', action='store_true',
                              help='Disable optimization')
        opt_parser.add_argument('--metrics', action='store_true',
                              help='Show optimization metrics')
        ...
        try:
            ...
            for agent_id in AGENT_REGISTRY.keys():
                try:
                    ...
                    self.print_info(f"   Type: {AGENT_REGISTRY[agent_id].agent_type.value}")
                    self.print_info(f"   Capabilities: {', '.join(AGENT_REGISTRY[agent_id].capabilities)}")
                    self.print_info(f"   Messages sent: {metrics['messages_sent']}")
                    self.print_info(f"   Messages received: {metrics['messages_received']}")
        ...
        try:
            ...
            for agent_id, client in clients.items():
                try:
                    ...
                    sent_delta = final_metrics['messages_sent'] - initial['messages_sent']
                    received_delta = final_metrics['messages_received'] - initial['messages_received']
                    error_delta = final_metrics['errors_count'] - initial['errors_count']
            ...
            self.print_info(f"   Messages/sec: {(total_sent + total_received) / args.duration:.2f}")
