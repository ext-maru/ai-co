...
class AIBackupCommand(BaseCommand):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            'action',
            choices=['create', 'restore', 'list', 'verify', 'cleanup', 'schedule', 
                    'config', 'database', 'cloud', 'elders', 'monitor'],
            help='実行するアクション'
        )
        ...
        parser.add_argument(
            '--base-backup',
            type=str,
            help='増分バックアップのベースファイル'
        )
        parser.add_argument(
            '--since',
            type=str,
            help='変更日時以降のファイル（YYYY-MM-DD形式）'
        )
        ...
        try:
            ...
            if args.action == 'create':
                ...
                return CommandResult(
                    success=False,
                    message=f"無効なアクション: {args.action}"
                )
    ...
    def _handle_create_backup(self, args) -> CommandResult:
        ...
        if args.type == 'full':
            ...
            return CommandResult(
                success=False,
                message=f"無効なバックアップタイプ: {args.type}"
            )
    ...
    def _handle_restore_backup(self, args) -> CommandResult:
        ...
        if not args.backup_file:
            return CommandResult(
                success=False,
                message="復元するバックアップファイルを指定してください（--backup-file）"
            )
        ...
        if result['success']:
            ...
            return CommandResult(
                success=False,
                message=f"復元失敗: {result.get('error', 'Unknown error')}"
            )
    ...
    def _handle_verify_backup(self, args) -> CommandResult:
        ...
        if not args.backup_file:
            return CommandResult(
                success=False,
                message="検証するバックアップファイルを指定してください（--backup-file）"
            )
        ...
        if result['success']:
            ...
            overall_status = "検証成功" if result['integrity_check'] and result['content_check'] else "検証失敗"
            ...
            return CommandResult(
                success=False,
                message=f"検証エラー: {result.get('error', 'Unknown error')}"
            )
    ...
    def _setup_scheduled_backup(self, args) -> Dict[str, Any]:
        ...
        try:
            ...
            # 次回実行時刻計算
            try:
                next_run = croniter.croniter(cron_expr, datetime.now()).get_next(datetime)
            except:
                next_run = None
    ...
    def _handle_backup_config(self, args) -> CommandResult:
        ...
        if result['success']:
            ...
            return CommandResult(
                success=False,
                message=f"設定管理失敗: {result.get('error', 'Unknown error')}"
            )
    ...
    def _backup_databases(self, args) -> Dict[str, Any]:
        """データベースバックアップ実行"""
        try:
            output_dir = Path(args.output_dir) if args.output_dir else self.backup_dir / "databases"
    ...
    def _upload_to_cloud(self, args) -> Dict[str, Any]:
        ...
        try:
            ...
            cloud_filename = f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{backup_file.name}"
            ...
            if args.provider == 's3':
                ...
                cloud_url = f"https://{args.bucket}.blob.core.windows.net/{cloud_filename}"
            else:
                return {
                    'success': False,
                    'error': f'サポートされていないプロバイダー: {args.provider}'
                }
    ...
    def _backup_elders_data(self, args) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.elders:
                ...
                if args.include_learning_sessions:
                    ...
                    elders_data['learning_sessions'] = {
                        'total_sessions': analytics.get('learning_session_analytics', {}).get('total_sessions', 0),
            ...
            # バックアップファイル作成
            backup_file = Path(args.output) if args.output else self.backup_dir / f"elders_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tar.gz"
            backup_file.parent.mkdir(parents=True, exist_ok=True)
            ...
            return {
                ...
                'learning_sessions': elders_data.get('learning_sessions', {}).get('total_sessions', 0),
    ...
    def _handle_backup_monitoring(self, args) -> CommandResult:
        ...
        if result['success']:
            ...
            return CommandResult(
                success=False,
                message=f"監視エラー: {result.get('error', 'Unknown error')}"
            )
    ...
    def _monitor_backup_health(self, args) -> Dict[str, Any]:
        ...
        try:
            ...
            if backups:
                ...
                hours_since_backup = (datetime.now() - latest_backup['created']).total_seconds() / 3600
            else:
            ...
            return {
                'success': True,
                'last_backup_age': hours_since_backup if hours_since_backup != float('inf') else None,
    ...
    def _collect_backup_files(self, args) -> List[Path]:
        ...
        # 知識ベース追加
        if getattr(args, 'include_knowledge', False) or self.config.get('include_knowledge', True):
            base_dirs.append(self.project_root / "knowledge_base")
        
        # データベース追加
        if getattr(args, 'include_databases', False) or self.config.get('include_databases', True):
            base_dirs.append(self.project_root / "data")
        
        # ログファイル追加
        if getattr(args, 'include_logs', False) and not getattr(args, 'exclude_logs', False):
