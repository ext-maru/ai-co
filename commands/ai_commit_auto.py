...
class AutoCommitCLI:
    ...
    def display_analysis_report(
        self, context: Dict, recommended_layer: DevelopmentLayer, reasons: List[str]
    ):
        """詳細分析レポートを表示"""
        ...
        if context["message_analysis"]["keywords_found"]:
            print(
                f"🔍 検出キーワード: {', '.join(context['message_analysis']['keywords_found'])}"
            )
...
🤖 自動プロトコル選択ルール:
  ...
  👑 Grand: 大規模変更 (20ファイル以上、高複雑度)
    ...
    parser.add_argument("--emergency", action="store_true", help="緊急対応フラグ")

    parser.add_argument("--high-priority", action="store_true", help="高優先度フラグ")

    parser.add_argument("--files", nargs="+", help="特定ファイルのみコミット")

    parser.add_argument("--analyze", action="store_true", help="分析のみ実行（実際のコミットは行わない）")

    parser.add_argument(
        "--force-layer", choices=["lightning", "council", "grand"], help="プロトコルを強制指定"
    )
