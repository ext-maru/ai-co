...
class CouncilCommitCLI:
    ...
    def analyze_complexity(self, files: list) -> float:
        ...
        for file in files:
            # 重要ファイルの判定
            if any(critical in file.lower() for critical in 
                   ['config', 'core', 'libs', 'worker', 'elder']):
    ...
    def create_enhanced_commit_message(self, base_message: str, context: dict, 
                                     sage_results: List[SageConsultationResult]) -> str:
        """強化されたコミットメッセージ生成"""
        ...
        # 平均リスクスコア
        avg_risk = sum(r.risk_score for r in sage_results) / len(sage_results) if sage_results else 0
        if avg_risk > 0.5:
    ...
    def display_sage_consultation_details(self, sage_results: List[SageConsultationResult]):
        """賢者相談の詳細表示"""
        print("\n🧙‍♂️ 4賢者評議会結果:")
        print("=" * 50)
        ...
        for result in sage_results:
            ...
            risk_level = "🟢 低" if result.risk_score < 0.3 else "🟡 中" if result.risk_score < 0.7 else "🔴 高"
    ...
    async def execute_council_commit(self, message: str, args) -> bool:
        ...
        try:
            ...
            # 2. 自動ステージング（必要に応じて）
            if not git_changes["staged"] and (git_changes["unstaged"] or git_changes["untracked"]):
                print("🔄 変更を自動ステージング...")
                if args.files:
            ...
            # 5. 4賢者評議会開始
            print("🧙‍♂️ 4賢者評議会招集中...")
            sage_results = await self.lightning_system.harmony_engine.council_consultation(context)
            ...
            # 8. 強化コミットメッセージ
            enhanced_message = self.create_enhanced_commit_message(message, context, sage_results)
            
            # 9. Council実行
            print("🏛️ Council実行中...")
            success = await self.lightning_system.execute_council_commit(enhanced_message, context)
...
🏛️ Council Protocol適用条件:
  ...
  - 4賢者の過半数承認が必要
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="実際のコミットを行わず、評議会シミュレーションのみ"
    )
