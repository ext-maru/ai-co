...
class GrandCommitCLI:
    ...
    def analyze_change_impact(self, files: List[str]) -> Dict:
        ...
        for file in files:
            ...
            if any(
                keyword in file.lower()
                for keyword in ["core/", "libs/", "config", "architecture"]
            ):
                impact["architecture_impact"] = True

            # 破壊的変更の可能性
            if any(
                keyword in file.lower() for keyword in ["api/", "interface", "protocol"]
            ):
    ...
    def display_change_analysis(self, git_changes: Dict, impact: Dict):
        ...
        print(
            f"  📦 影響モジュール: {', '.join(impact['affected_modules']) if impact['affected_modules'] else 'なし'}"
        )
    ...
    async def report_to_elder_council(
        self, context: Dict, sage_results: List, elapsed_time: float
    ):
        """エルダー評議会への報告"""
        print("\n📜 エルダー評議会への報告書作成中...")
...
👑 Grand Protocol 使用基準:
  ...
  • 重要な意思決定を伴う変更
    ...
    parser.add_argument(
        "--skip-hooks", action="store_true", help="pre-commit hooks をスキップ（非推奨）"
    )
