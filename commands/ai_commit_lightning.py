...
class LightningCommitCLI:
    ...
    async def execute_lightning_commit(self, message: str, args) -> bool:
        ...
        try:
            ...
            # 緊急度判定
            urgency = CommitUrgency.EMERGENCY if args.emergency else \
                     CommitUrgency.HIGH if args.high_priority or complexity < 0.3 else \
                     CommitUrgency.NORMAL
...
⚡ Lightning Protocol適用条件:
  ...
  - システム影響: 限定的
    ...
    if args.dry_run:
        ...
        context = {
            "urgency": CommitUrgency.EMERGENCY if args.emergency else CommitUrgency.NORMAL,
