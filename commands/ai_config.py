...
class AIConfigCommand(BaseCommand):
    ...
    def setup_arguments(self):
        ...
        self.parser.add_argument(
            '--get', '-g',
            help='特定の設定値を取得（例: slack.ENABLE_SLACK）'
        )
    ...
    def _show_config_file(self, filename: str) -> CommandResult:
        ...
        if not config_path.exists():
            return CommandResult(
                success=False,
                message=f"設定ファイルが見つかりません: {filename}"
            )
            ...
            with open(conf_file) as f:
                for line in f:
                    ...
                    if line and not line.startswith('#') and '=' in line:
                        ...
                        # 機密情報はマスク
                        if any(secret in key.upper() for secret in ['TOKEN', 'PASSWORD', 'SECRET', 'KEY']):
    ...
    def _get_config_value(self, key: str) -> CommandResult:
        ...
        if not config_path.exists():
            return CommandResult(
                success=False,
                message=f"設定ファイルが見つかりません: {filename}.conf"
            )
            ...
            for line in f:
                ...
                if line and not line.startswith('#') and '=' in line:
                    ...
                    if k.strip() == config_key:
                        return CommandResult(
                            success=True,
                            message=v.strip().strip('"')
                        )
        
        return CommandResult(
            success=False,
            message=f"設定項目が見つかりません: {config_key}"
        )
