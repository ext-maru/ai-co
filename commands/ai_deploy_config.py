...
class DeployConfigCommand(BaseCommand):
    ...
    def _create_parser(self) -> argparse.ArgumentParser:
        ...
        show_parser.add_argument(
            "--environment", "-e", default="development", help="環境名"
        )
        ...
        method_parser.add_argument(
            "method", choices=["github_actions", "ssh", "hybrid"], help="デプロイ方法"
        )
        ...
        # sages-optimize コマンド
        sages_optimize_parser = subparsers.add_parser(
            "sages-optimize", help="4賢者による最適化"
        )
        sages_optimize_parser.add_argument("project", help="プロジェクト名")

        # sages-recommend コマンド
        sages_recommend_parser = subparsers.add_parser(
            "sages-recommend", help="4賢者による推奨"
        )
    ...
    def _set_deployment_method(
        self, project: str, environment: str, method: str
    ) -> int:
        """デプロイ方法設定"""
        try:
