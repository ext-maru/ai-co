...
class AIDockerCommand(BaseCommand):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser):
        ...
        create_parser.add_argument('--ports', '-p', nargs='*', help='ポートマッピング (例: 8080:80)')
    ...
    def _create_container(self, args) -> CommandResult:
        ...
        if response.status_code == 200:
            ...
            return CommandResult(
                success=False,
                message=f"コンテナ作成失敗: {response.text}"
            )
    ...
    def _list_containers(self, args) -> CommandResult:
        ...
        if response.status_code == 200:
            ...
            lines.append("-" * 60)
            for c in containers:
                lines.append(f"{c['id']:<11} {c['name']:<23} {c['status']:<11} {c['image']}")
            ...
            return CommandResult(
                success=False,
                message=f"一覧取得失敗: {response.text}"
            )
    ...
    def _execute_action(self, args) -> CommandResult:
        ...
        if args.subcommand == 'remove':
            response = requests.delete(f"{self.api_base_url}/containers/{args.container_id}")
        else:
            response = requests.post(
                f"{self.api_base_url}/containers/{args.container_id}/actions",
                json=data
            )
        ...
        if response.status_code == 200:
            ...
            return CommandResult(
                success=False,
                message=f"アクション実行失敗: {response.text}"
            )
    ...
    def _get_logs(self, args) -> CommandResult:
        ...
        if response.status_code == 200:
            ...
            return CommandResult(
                success=True,
                message=result.get('logs', 'ログがありません'),
                data=result
            )
        else:
            return CommandResult(
                success=False,
                message=f"ログ取得失敗: {response.text}"
            )
    
    def _get_stats(self, args) -> CommandResult:
        """統計情報取得"""
        response = requests.get(f"{self.api_base_url}/containers/{args.container_id}/stats")
        ...
        if response.status_code == 200:
            ...
            return CommandResult(
                success=False,
                message=f"統計情報取得失敗: {response.text}"
            )
    ...
    def _get_sages_status(self) -> CommandResult:
        ...
        if response.status_code == 200:
            ...
            return CommandResult(
                success=False,
                message=f"ステータス取得失敗: {response.text}"
            )
