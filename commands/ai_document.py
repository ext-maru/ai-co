...
class AutoPMOrchestrator:
    ...
    async def generate_documentation(
        self, project_path: str, options: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        プロジェクト全体のドキュメント生成
        Claude PMの役割を自動実行
        ...
        try:
            ...
            project_info = self._build_project_info(
                project_path, code_files, task_result, pm_result
            )
            ...
            doc_request = {
                ...
                "payload": {
                    ...
                    "project_description": options.get(
    ...
    def _discover_code_files(self, project_path: str) -> list:
        ...
        for file_path in project_path.rglob("*"):
            if file_path.is_file() and file_path.suffix in code_extensions:
                ...
                if not any(
                    part.startswith(".")
                    or part in ["venv", "node_modules", "__pycache__"]
    ...
    def _build_project_info(
        self, project_path: str, code_files: list, task_result: Dict, pm_result: Dict
    ) -> Dict:
        """プロジェクト情報の構築"""
        # 簡易的な関数・クラス抽出
        functions = []
        ...
        project_info = {
            ...
            "quality_report": {
                "quality_score": pm_result["payload"].get(
                    "final_quality_score",
...
使用例:
  ...
  %(prog)s --version
        ...
        generate_parser.add_argument(
            "--format", default="readme,api", help="生成形式 (readme,api,architecture)"
        )
        ...
        parser.add_argument(
            "--version",
            action="version",
            version="Elders Guild Documentation System v1.0",
        )
