...
from libs.self_evolution_system import SelfEvolutionSystem, GrandElderInterface
...
class AIGrandElderCommand:
        ...
        for category, proposals in proposals_summary['by_category'].items():
            if proposals:
                ...
                for proposal in proposals:
                    ...
                    print(f"     価値: {proposal['business_value']:.1%}, 複雑度: {proposal['complexity']:.1%}")
                    
                    while True:
                        decision = input(f"     決定 (approved/rejected/deferred): ").strip().lower()
                        if decision in ['approved', 'rejected', 'deferred']:
                            decisions[proposal['id']] = decision
                            break
                        else:
                            print("     ❌ 無効な選択です。approved/rejected/deferred のいずれかを入力してください")
    ...
    async def process_decisions(self, decisions_json):
        ...
        rejected_count = sum(1 for decision in decisions.values() if decision == 'rejected')
        deferred_count = sum(1 for decision in decisions.values() if decision == 'deferred')
...
def main():
    ...
    parser.add_argument('--future-vision', action='store_true',
                       help='未来ビジョンを要請')
    parser.add_argument('--review-proposals', action='store_true',
                       help='企画審査を要請')
    parser.add_argument('--decisions', type=str,
                       help='決定事項をJSON形式で入力')
    parser.add_argument('--consultation-log', action='store_true',
                       help='相談履歴を表示')
    parser.add_argument('--limit', type=int, default=10,
                       help='履歴表示件数 (default: 10)')
