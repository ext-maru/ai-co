...
class HealthCommand(BaseCommand):
    ...
    def setup_arguments(self):
        ...
        self.parser.add_argument("--fix", action="store_true", help="問題を自動修復する")
    ...
    def check_workers(self):
        ...
        for worker_type, info in worker_info.items():
            ...
            info["healthy"] = (
                info["actual"] >= info["expected"] if info["expected"] > 0 else True
            )
        ...
        for log_file, max_size_mb in log_files.items():
            ...
            if log_path.exists():
                ...
                log_info[log_file] = {
                    ...
                    "status": "ok"
                    if size_mb < max_size_mb and recent_errors < 10
                    else "warning",
                }
            else:
                log_info[log_file] = {"exists": False, "healthy": True, "status": "ok"}
    ...
    def calculate_overall_health(self, health_data):
        """全体的なヘルススコア計算"""
        scores = {"critical": 0, "warning": 0, "ok": 0, "total": 0}
        ...
        # スコア計算（100点満点）
        if scores["total"] > 0:
            health_score = (scores["ok"] * 100 + scores["warning"] * 50) / scores[
                "total"
            ]
        else:
            health_score = 0
        ...
        for queue, info in health_data["queues"].items():
            if info.get("messages", -1) >= 0:
                if info["status"] != "ok":
                    ...
                    self.print(
                        f"{icon} {queue}: {info['messages']} メッセージ",
                        color="yellow" if info["status"] == "warning" else "red",
                    )
    ...
    def auto_fix_issues(self, health_data):
        ...
        critical_queues = [
            q
            for q, info in health_data["queues"].items()
