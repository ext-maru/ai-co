...
class AILogsCommand(BaseCommand):
    ...
    def setup_arguments(self):
        ...
        self.parser.add_argument(
            "--tail", "-t", type=int, default=50, help="表示する行数（デフォルト: 50）"
        )
        self.parser.add_argument(
            "--follow", "-f", action="store_true", help="リアルタイム表示（tail -f）"
        )
        self.parser.add_argument(
            "--list", "-l", action="store_true", help="ログファイル一覧を表示"
        )
        self.parser.add_argument("--grep", "-g", help="検索文字列でフィルタ")
        self.parser.add_argument(
            "--since", help="指定時刻以降のログを表示（例: '10m', '1h', '2023-01-01'）"
        )
    ...
    def _tail_file(
        self, log_file: Path, lines: int, grep: str = None, since: str = None
    ):
        """ファイルの末尾を表示"""
        try:
            # tail コマンドを使用
