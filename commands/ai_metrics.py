...
class AIMetricsCommand(BaseCommand):
    """メトリクス表示コマンド"""
...
"""
        ...
        if stats['worker_stats']:
            ...
            for worker, count in sorted(stats['worker_stats'].items(), 
        ...
        if stats['type_stats']:
            ...
            total = stats['total_tasks']
            for task_type, count in sorted(stats['type_stats'].items(),
                                         key=lambda x: x[1], reverse=True):
                percentage = (count / total * 100) if total > 0 else 0
