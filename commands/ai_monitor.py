...
class AIMonitorCommand(BaseCommand):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser):
        ...
        dashboard_parser.add_argument('--interval', '-i', type=int, default=2, help='更新間隔（秒）')
        dashboard_parser.add_argument('--sages', '-s', action='store_true', help='4賢者詳細表示')
        
        # status サブコマンド
        status_parser = subparsers.add_parser('status', help='現在のステータス表示')
        status_parser.add_argument('--format', '-f', choices=['text', 'json'], default='text', help='出力形式')
        ...
        sages_parser.add_argument('--sage', choices=['knowledge', 'task', 'incident', 'rag'], help='特定賢者監視')
    ...
    def _show_alerts(self, args) -> CommandResult:
        ...
        for alert in recent_alerts[-args.last:]:
            ...
            lines.append(f"{timestamp} {level_icon} {alert['message']} ({alert['source']})")
    ...
    def _run_health_check(self) -> CommandResult:
        ...
        for component, result in health_results.items():
            ...
            lines.append(f"{status_icon} {component.title()}: {result.get('message', 'OK')}")
    ...
    def _collect_metrics(self, args) -> CommandResult:
        ...
        if args.export:
            ...
            return CommandResult(
                success=True,
                message=f"メトリクスを {args.export} に出力しました",
                data=metrics
            )
        else:
            return CommandResult(
    ...
    def _get_all_sages_data(self) -> Dict[str, Any]:
        ...
        knowledge_count = sum(1 for _ in kb_path.rglob("*.json*")) if kb_path.exists() else 0
    ...
    def _check_workers_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "details": [f"{name}: {'稼働中' if running else '停止'}" 
    ...
    def _check_queues_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "details": [f"{name}: {count}件" 
                       for name, count in queue_metrics["queues"].items()]
        }
    
    def _check_sages_health(self) -> Dict[str, Any]:
        """4賢者ヘルスチェック"""
        sages_data = self._get_all_sages_data()
        active_sages = sum(1 for data in sages_data.values() 
                          if data.get('status') == 'active')
        
        return {
            "healthy": active_sages == 4,
            "message": f"{active_sages}/4 賢者が稼働中",
            "details": [f"{data['name']}: {data['status']}" 
                       for data in sages_data.values()]
        }
...
"""
    ...
    def _get_queue_status(self) -> Panel:
        ...
        return Panel(table, title=f"📬 キュー状態 {status_icon} {status_text}", border_style="yellow")
    ...
    def _get_sages_status(self) -> Panel:
        ...
        active_count = sum(1 for data in sages_data.values() if data['status'] == 'active')
