...
class AIRagCommand(BaseCommand):
    ...
    def __init__(self):
        super().__init__(
            name="ai-rag",
            description="RAG（検索拡張生成）管理 - 情報検索と最適解探索",
            version="2.0.0"
        )
    ...
    def add_arguments(self, parser: argparse.ArgumentParser):
        ...
        search_parser.add_argument('--threshold', '-t', type=float, default=0.7, help='類似度閾値')
        search_parser.add_argument('--format', '-f', choices=['json', 'text'], default='text', help='出力形式')
        ...
        enhance_parser.add_argument('--model', '-m', default='claude-sonnet-4-20250514', help='使用モデル')
        
        # summary サブコマンド
        summary_parser = subparsers.add_parser('summary', help='要約生成')
        summary_parser.add_argument('text', help='要約対象のテキスト')
        summary_parser.add_argument('--length', '-l', type=int, default=100, help='要約文字数')
        ...
        optimize_parser.add_argument('--rebuild-index', action='store_true', help='インデックス再構築')
        
        # migrate サブコマンド
        migrate_parser = subparsers.add_parser('migrate', help='魔法書システムへの移行')
        migrate_parser.add_argument('--dry-run', action='store_true', help='実行せずに移行計画を表示')
        ...
        unified_parser.add_argument('action', choices=['status', 'init', 'cleanup'], help='実行する操作')
    ...
    def _analyze_context(self, args) -> CommandResult:
        """コンテキスト分析"""
        analysis = self.enhanced_rag.analyze_context(
            args.context,
            depth=args.depth
        )
    ...
    def _generate_summary(self, args) -> CommandResult:
        ...
        summary = self.rag_manager.generate_summary(
            prompt="要約生成タスク",
            response=text
        )
    ...
    def _optimize_search(self, args) -> CommandResult:
        ...
        if args.rebuild_index:
            ...
            return CommandResult(
                success=True,
                message="最適化オプションを指定してください (--rebuild-index)"
            )
        ...
        try:
            ...
            if migration_report['errors']:
                ...
                if len(migration_report['errors']) > 5:
                    lines.append(f"  ... および{len(migration_report['errors']) - 5}件の追加エラー")
    ...
    def _unified_system_management(self, args) -> CommandResult:
        ...
        try:
            ...
            if args.action == 'init':
                ...
                lines.append(f"統合システム: {'✅ アクティブ' if status['integration_active'] else '❌ 非アクティブ'}")
                lines.append(f"魔法書システム: {'✅ 準備完了' if status['grimoire_system_ready'] else '❌ 未準備'}")
                lines.append(f"レガシーRAG: {'✅ 利用可能' if status['legacy_rag_available'] else '❌ 利用不可'}")
                lines.append(f"知識管理: {'✅ 利用可能' if status['knowledge_manager_available'] else '❌ 利用不可'}")
                ...
                if any(migration_stats.values()):
                    ...
                    lines.append(f"  移行済み: {migration_stats.get('migrated_entries', 0)}")
                    lines.append(f"  失敗: {migration_stats.get('failed_entries', 0)}")
                    lines.append(f"  進化適用: {migration_stats.get('evolution_applied', 0)}")
                ...
                return CommandResult(
                    success=True,
                    message="✅ 統合RAGシステムのクリーンアップが完了しました"
                )
            
            else:
                return CommandResult(
                    success=False,
                    message=f"不明なアクション: {args.action}"
                )
                
        except Exception as e:
            return CommandResult(
                success=False,
                message=f"統合システム管理エラー: {str(e)}"
            )
