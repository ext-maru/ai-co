...
class AIReportCommand(BaseCommand):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser):
        ...
        parser.add_argument("--period", "-p", type=int, default=7, help="レポート対象期間（日数）")
        parser.add_argument(
            "--schedule",
            "-s",
        ...
        parser.add_argument("--template", type=str, help="カスタムテンプレートファイル")
        parser.add_argument(
            "--visualize", "-v", action="store_true", help="ビジュアライゼーションを含める"
        )
        parser.add_argument("--compare-with", type=int, help="比較対象期間（日数）")
        ...
        try:
            ...
            if args.type == "overview":
                ...
                return CommandResult(
                    success=False, message=f"エラー: 不明なレポートタイプ '{args.type}'"
                )
    ...
    def _generate_sages_report(self, args) -> Dict[str, Any]:
        ...
        collaboration_status = (
            self.four_sages.monitor_sage_collaboration() if self.four_sages else {}
        )
    ...
    def _generate_custom_report(self, args) -> Dict[str, Any]:
        ...
        rendered = template.render(
            title="カスタムレポート", period=args.period, generated_at=datetime.now(), **metrics
        )
...
"""
        ...
        return html_template.format(
            ...
            metrics_json=json.dumps(
                report.get("metrics", {}), indent=2, ensure_ascii=False
            ),
        )
    ...
    def _collect_performance_metrics(self, period_days: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "resource_utilization": {"cpu": 0.45, "memory": 0.62, "disk": 0.38},
        }
    ...
    def _collect_comparison_data(
        self, current_period: int, previous_period: int
    ) -> Dict[str, Any]:
        """比較データ収集"""
