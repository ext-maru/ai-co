...
class AIWorkerCommCommand(BaseCommand):
    ...
    def setup_arguments(self):
        ...
        self.parser.add_argument(
            "action", choices=["status", "send", "log", "monitor"], help="実行するアクション"
        )
        self.parser.add_argument("--worker", "-w", help="対象ワーカー名")
        self.parser.add_argument("--message", "-m", help="送信メッセージ（send時）")
        self.parser.add_argument(
    ...
    def _show_status(self, args) -> CommandResultprint(f"📡 ワーカー間通信状態")print(f"{'}")
    """信状態表示"""
    ...
    def _show_logs(self, args) -> CommandResultprint(f"📜 ワーカー間通信ログ")print(f"{'}")
    """信ログ表示"""

        # デモ用のログエントリ
        logs = [
            {
                "time": "2025-01-05 10:30:15",
                "from": "pm_worker",
                "to": "task_worker",
                "type": "task_assignment",
            },
            {
                "time": "2025-01-05 10:30:45",
                "from": "task_worker",
                "to": "result_worker",
                "type": "result_submission",
            },
            {
                "time": "2025-01-05 10:31:20",
                "from": "error_intelligence_worker",
                "to": "pm_worker",
                "type": "error_report",
            },
        ]
    ...
    def _monitor_communication(self, args) -> CommandResultprint(f"👁️  ワーカー間通信モニター")print(f"{'}")
    """信監視"""
        print(f"")
