...
class PMEnhancedWorker:
    ...
    def connect(self):
        ...
        try:
            ...
            self.channel.queue_declare(
                queue="pm_feedback_queue",
                durable=True,
                arguments={"x-max-priority": 10},
            )
            self.channel.queue_declare(
                queue="pm_retry_queue", durable=True, arguments={"x-max-priority": 10}
            )
                ...
                if os.environ.get("AI_AUTO_GIT_DISABLED", "false").lower() == "true":
                    ...
                    if new_files:
                        ...
                        if self.feedback_enabled:
                            feedback_result = self.feedback_loop.process_task_result(
                                task_id, result_data
                            )

                            pm_approved = feedback_result.get("pm_approved", False)
                            retry_required = feedback_result.get(
    ...
    def _execute_git_flow(self, task_id: str, result_data: dict, new_files: list):
        ...
        try:
            ...
            if test_passed:
                ...
                if self.git_flow.commit_changes(
                    commit_message, new_files, use_best_practices=True
                ):
                    ...
                    if self.git_flow.create_pull_request(
                        branch_name,
                        f"feat: {git_result_data['summary']}",
                        f"Auto-generated: {git_result_data['summary']}",
                    ):
        ...
        try:
            ...
            for py_file in python_files:
                ...
                if file_path.parts[0] in ["workers", "libs"]:
                    test_file = (
                        PROJECT_DIR / "tests" / "unit" / f"test_{file_path.name}"
                    )
            ...
            if all_passed:
                ...
                for i, result in enumerate(test_results):
                    if not result.get("success", False):
                        self.logger.error(
                            f"失敗テスト {i+1}: {result.get('errors', 'Unknown error')}"
                        )
                    ...
                    for ext in extensions:
                        ...
                        for file_path in files:
                            ...
                            if file_path.stat().st_mtime > recent_threshold:
                                ...
                                # __pycache__やvenvは除外
                                if not any(
                                    part in str(relative_path)
                                    for part in ["__pycache__", "venv", ".git"]
                                ):
    ...
    def process_pm_task(self, ch, method, properties, body):
        ...
        try:
            ...
            task_id = task.get(
                "task_id", f"pm_unknown_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            if command == "toggle_feedback":
                ...
                self.logger.info(
                    f"PMフィードバック機能: {'有効' if self.feedback_enabled else '無効'}"
                )
    ...
    def _send_success_notification(
        self, task_id: str, branch_name: str, files: list, test_passed: bool
    ):
        """成功通知"""
        try:
            if not self.slack:
    ...
    def start(self):
        ...
        self.logger.info(
            f"📊 Git状態 - ブランチ: {git_status.get('current_branch', 'unknown')}"
        )
        self.logger.info(f"🧪 テスト実行: {'有効' if self.test_before_commit else '無効'}")
        ...
        self.channel.basic_consume(
            queue="pm_task_queue", on_message_callback=self.process_pm_task
        )
