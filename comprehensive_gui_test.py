...
class GUITestRunner:
    ...
    def run_selenium_tests(self):
        ...
        try:
            ...
            results = run_gui_tests(
                base_url=self.server.server_url,
                headless=True
            )
            
            # Process results
            if results["status"] == "completed":
                logger.info(f"✅ Selenium Tests: {results['passed']}/{results['total_tests']} passed")
                ...
                logger.error(f"❌ Selenium Tests failed: {results.get('message', 'Unknown error')}")
                self.test_results.append({
                    "framework": "Selenium",
                    "status": "failed",
    ...
    def run_playwright_tests(self):
        ...
        try:
            ...
            if not PLAYWRIGHT_AVAILABLE:
                logger.warning("⚠️  Playwright not available, skipping Playwright tests")
                self.test_results.append({
            ...
            # Process results
            if results["status"] == "completed":
                logger.info(f"✅ Playwright Tests: {results['passed']}/{results['total_tests']} passed")
                self.test_results.append({
                    "framework": "Playwright",
                    "status": "success",
                    "passed": results["passed"],
                ...
                logger.error(f"❌ Playwright Tests failed: {results.get('message', 'Unknown error')}")
                self.test_results.append({
                    "framework": "Playwright",
                    "status": "failed",
                    "error": results.get("message", "Unknown error")
    ...
    def run_api_tests(self):
        ...
        for endpoint in api_endpoints:
            try:
                response = requests.get(f"{self.server.server_url}{endpoint}", timeout=5)
    ...
    def generate_report(self):
        ...
        successful_frameworks = sum(1 for r in self.test_results if r["status"] == "success")
        
        logger.info(f"📊 Overall Summary:")
        logger.info(f"   Test Frameworks: {successful_frameworks}/{total_frameworks} successful")
        ...
        for result in self.test_results:
            ...
            if status == "success":
                ...
                passed = result.get("passed", 0)
                total = result.get("total", 0)
                logger.info(f"   ⚠️  {framework}: {passed}/{total} tests passed (partial)")
            else:
        ...
        playwright_available = any(r["framework"] == "Playwright" and r["status"] == "success" for r in self.test_results)
        if not playwright_available:
            logger.info(f"   📦 Install Playwright browsers: playwright install chromium")
        
        api_issues = any(r["framework"] == "API" and r["status"] != "success" for r in self.test_results)
        if api_issues:
            logger.info(f"   🔧 Check API endpoints and server configuration")
        
        logger.info(f"   🧪 Run tests regularly during development")
        logger.info(f"   📈 Consider adding more specific UI tests for dashboard features")
