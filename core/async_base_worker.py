...
class AsyncBaseWorker(ABC):
    ...
    def __init__(
        ...
        self.logger = structlog.get_logger(
            ...
            worker_id=f"{worker_name}_{int(time.time())}"
        )
    ...
    def _setup_metrics(self):
        ...
        self.processing_time = Histogram(
            f'{self.worker_name}_processing_seconds',
            'Message processing time'
        )
        
        self.active_tasks = Gauge(
            f'{self.worker_name}_active_tasks',
            'Currently active tasks'
        )
        
        self.queue_size = Gauge(
            f'{self.worker_name}_queue_size',
            'Current queue size',
            ['queue']
        )
    ...
    async def _send_to_output_queues(self, result: Dict[str, Any]):
        ...
        for queue_name in self.output_queues:
            try:
                ...
                self.logger.debug("Result sent to output queue", 
                                queue=queue_name)
                
            except Exception as e:
                self.logger.error("Failed to send to output queue",
                                queue=queue_name,
                                error=str(e))
