...
from lightweight_logger import get_logger, LoggingMixin
...
class AsyncBaseWorkerV2(ABC, LoggingMixin):
    ...
    def _setup_metrics(self):
        """メトリクスの初期化"""
        self.processed_messages = self.metrics.counter('processed_total', 'Total processed messages')
        self.processing_time = self.metrics.histogram('processing_seconds', 'Message processing time')
    ...
    async def _handle_message_simulation(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            task_logger.info("Processing message", 
            ...
            task_logger.info("Message processed successfully", 
                           duration=duration)
    ...
    async def _health_check_loop(self):
        ...
        while self.running:
            try:
                ...
                # サーキットブレーカーの状態
                self.health_status['details']['circuit_breaker_state'] = self.circuit_breaker.state
