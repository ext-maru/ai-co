...
class BaseWorker(ABC, ErrorHandlerMixin):
    ...
    def _declare_queues(self):
        ...
        if self.channel:
            ...
            # 入力キュー
            self.channel.queue_declare(queue=self.input_queue, durable=True, arguments=queue_args)
            # 出力キュー
    ...
    def stop(self):
        ...
        try:
            ...
            if self.channel:
                ...
                try:
                    if hasattr(self.channel, 'is_closed') and not self.channel.is_closed:
                        self.channel.close()
                except Exception as e:
                    # ファイルディスクリプタエラーを抑制
                    if "Bad file descriptor" not in str(e):
            ...
            if self.connection:
                try:
                    if hasattr(self.connection, 'is_closed') and not self.connection.is_closed:
