...
class BaseWorker(OriginalBaseWorker):
    ...
    def __init__(self, worker_type: str, worker_id: str = None):
        ...
        # 起動メッセージを日本語化
        self.logger.info(
            msg("worker_started", worker_type=self.worker_type, pid=os.getpid())
        )
    ...
    def log_task_start(self, task_id: str, task_type: str = None):
        ...
        self.logger.info(
            msg(
                "task_started", task_id=task_id, task_type=task_type or self.worker_type
            )
        )
    ...
    def log_task_complete(self, task_id: str, duration: float = 0, files: int = 0):
        """タスク完了ログ（日本語）"""
        self.logger.info(
            msg("task_completed", task_id=task_id, duration=duration, files=files)
        )
