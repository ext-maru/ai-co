...
class SlackConfig:
    ...
    def from_file(cls, config_path: Path) -> 'SlackConfig':
        ...
        return cls(
            ...
            polling_channel_id=config.get('SLACK_POLLING_CHANNEL_ID', json_config.get('polling_channel_id', '')),
            polling_interval=int(config.get('SLACK_POLLING_INTERVAL', json_config.get('polling_interval', 20))),
            polling_enabled=config.get('SLACK_POLLING_ENABLED', 'false').lower() == 'true',
            require_mention=config.get('SLACK_REQUIRE_MENTION', 'true').lower() == 'true',
            ...
            # 通知設定
            mention_on_critical=config.get('SLACK_MENTION_ON_CRITICAL', 'true').lower() == 'true',
            mention_users=config.get('SLACK_MENTION_USERS', json_config.get('notification_settings', {}).get('mention_users', ['@channel'])[0]),
            rate_limit_max=int(config.get('SLACK_RATE_LIMIT_MAX', json_config.get('notification_settings', {}).get('rate_limit', {}).get('max_messages_per_minute', 10))),
            rate_limit_cooldown=int(config.get('SLACK_RATE_LIMIT_COOLDOWN', json_config.get('notification_settings', {}).get('rate_limit', {}).get('cooldown_seconds', 60))),
            
            # キーワード
            critical_keywords=json_config.get('critical_keywords', [
                "CRITICAL", "FATAL", "ERROR", "Exception", "Traceback"
            ])
        )
