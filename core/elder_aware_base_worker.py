...
from libs.unified_auth_provider import (
    UnifiedAuthProvider, ElderRole, SageType, User, AuthSession,
    elder_auth_required, sage_auth_required
)
...
class ElderAuditLogger:
    ...
    def log_elder_action(self, context: ElderTaskContext, action: str, result: Any):
        ...
        event = {
            ...
            "sage_type": context.user.sage_type.value if context.user.sage_type else None,
        ...
        self.logger.info(f"Elder Action: {context.user.username} ({context.user.elder_role.value}) - {action}")
    
    def log_security_event(self, context: ElderTaskContext, event_type: str, details: Dict):
        """セキュリティイベントログ"""
        security_event = {
            "timestamp": datetime.now().isoformat(),
        ...
        self.logger.warning(f"Security Event: {event_type} - User: {context.user.username}")
    
    def _calculate_risk_level(self, event_type: str, context: ElderTaskContext) -> str:
        """リスクレベル算出"""
        high_risk_events = ["permission_denied", "unauthorized_access", "privilege_escalation"]
...
class ElderAuthIntegration:
    ...
    def validate_elder_task_execution(self, user: User, required_role: ElderRole, 
                                    required_sage: Optional[SageType] = None) -> bool:
        """Elder階層タスク実行権限検証"""
        # Elder階層チェック
        if not self.auth_provider.check_elder_permission(user, required_role):
            self.audit_logger.log_security_event(
                ElderTaskContext(user, None, "unknown", WorkerExecutionMode.SERVANT_MODE, 
                               ElderTaskPriority.LOW, []),
                "permission_denied",
                {"required_role": required_role.value, "user_role": user.elder_role.value}
            )
            return False
        
        # 賢者権限チェック（必要な場合）
        if required_sage and not self.auth_provider.check_sage_permission(user, required_sage):
            self.audit_logger.log_security_event(
                ElderTaskContext(user, None, "unknown", WorkerExecutionMode.SAGE_MODE,
                               ElderTaskPriority.MEDIUM, []),
                "sage_permission_denied", 
                {"required_sage": required_sage.value, "user_sage": user.sage_type.value if user.sage_type else None}
            )
            return False
    ...
    def get_permitted_actions(self, user: User) -> List[str]:
        ...
        if user.elder_role == ElderRole.GRAND_ELDER:
            actions.extend([
                "system_admin", "user_management", "emergency_override",
                "council_decisions", "global_settings"
            ])
        elif user.elder_role == ElderRole.CLAUDE_ELDER:
            actions.extend([
                "development_management", "sage_coordination", 
                "system_monitoring", "deployment_control"
            ])
        elif user.elder_role == ElderRole.SAGE:
            if user.sage_type == SageType.KNOWLEDGE:
                ...
                actions.extend(["incident_response", "emergency_action", "security_control"])
...
class ElderAwareBaseWorker(BaseWorker):
    ...
    def __init__(self, auth_provider: Optional[UnifiedAuthProvider] = None,
        ...
        logger.info(f"ElderAwareBaseWorker initialized - Required: {required_elder_role.value}")
    ...
    @classmethod
    def with_elder_requirements(cls, elder_role: ElderRole, 
    ...
    def create_elder_context(self, user: User, session: AuthSession, 
                           task_id: str, priority: ElderTaskPriority = ElderTaskPriority.LOW) -> ElderTaskContext:
        """Elder実行コンテキスト作成"""
    ...
    async def execute_with_elder_context(self, context: ElderTaskContext, 
                                       task_func: Callable, *args, **kwargs) -> ElderTaskResult:
        ...
        try:
            ...
            if not self.elder_integration.validate_elder_task_execution(
                ...
                raise PermissionError(f"Insufficient Elder permissions: required {self.required_elder_role.value}")
            ...
            if context.execution_mode == WorkerExecutionMode.GRAND_ELDER:
                result = await self._execute_grand_elder_mode(context, task_func, *args, **kwargs)
            elif context.execution_mode == WorkerExecutionMode.CLAUDE_ELDER:
                result = await self._execute_claude_elder_mode(context, task_func, *args, **kwargs)
            elif context.execution_mode == WorkerExecutionMode.SAGE_MODE:
                result = await self._execute_sage_mode(context, task_func, *args, **kwargs)
            else:
                result = await self._execute_servant_mode(context, task_func, *args, **kwargs)
            ...
            if context.audit_enabled:
                ...
                self.audit_logger.log_security_event(context, "task_execution_error", {"error": str(e)})
    ...
    async def _perform_security_checks(self, context: ElderTaskContext):
        ...
        # 異常行動検知
        if context.priority == ElderTaskPriority.CRITICAL and context.user.elder_role != ElderRole.GRAND_ELDER:
            self.audit_logger.log_security_event(context, "privilege_escalation_attempt", {
    ...
    async def _execute_grand_elder_mode(self, context: ElderTaskContext, 
    ...
    async def _execute_claude_elder_mode(self, context: ElderTaskContext,
                                       task_func: Callable, *args, **kwargs):
        """🤖 クロードエルダーモード実行"""
    ...
    async def _execute_sage_mode(self, context: ElderTaskContext,
                                task_func: Callable, *args, **kwargs):
        """🧙‍♂️ 賢者モード実行"""
        sage_type = context.user.sage_type.value if context.user.sage_type else "unknown"
    ...
    async def _execute_servant_mode(self, context: ElderTaskContext,
                                   task_func: Callable, *args, **kwargs):
        """🧝‍♂️ サーバントモード実行"""
    ...
    def decorator(method):
        ...
        async def wrapper(self, context: ElderTaskContext, *args, **kwargs):
            if not isinstance(self, ElderAwareBaseWorker):
                raise TypeError("elder_worker_required can only be used with ElderAwareBaseWorker")
            
            # 権限チェック
            if not self.elder_integration.validate_elder_task_execution(
                context.user, elder_role, sage_type
            ):
                raise PermissionError(f"Insufficient Elder permissions: required {elder_role.value}")
...
# ファクトリー関数
def create_elder_worker(worker_class, elder_role: ElderRole, 
                       sage_type: Optional[SageType] = None,
                       auth_provider: Optional[UnifiedAuthProvider] = None):
    """Elder階層ワーカー作成ファクトリー"""
