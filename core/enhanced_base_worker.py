...
from core.retry_decorator import RetryableWorker, RetryPresets, retry


class EnhancedBaseWorker(
    OriginalBaseWorker, MonitoringMixin, RetryableWorker, DeadLetterQueueMixin
):
    """機能拡張版BaseWorker"""
    ...
    def __init__(
        ...
        """
        # 基底クラスの初期化
        OriginalBaseWorker.__init__(
            self, worker_type, worker_id, input_queue, output_queue
        )
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.info(
                f"🚀 Enhanced {self.worker_type} worker {self.worker_id} started"
            )
            self.logger.info(
