...
class ErrorHandlerMixin:
    ...
    def handle_error(
        ...
        # エラーインテリジェンスワーカーへ送信
        if self.error_intelligence_enabled and severity in [
    ...
    def _categorize_error(self, error: Exception) -> str:
        ...
        if any(
            keyword in error_msg
            for keyword in ["api", "rate limit", "unauthorized", "forbidden"]
        ):
        ...
        if any(
            keyword in error_msg
            for keyword in ["json", "decode", "parse", "validation"]
        ):
        ...
        if any(
            ...
            for keyword in ["OSError", "SystemError", "RuntimeError"]
        ):
        ...
        try:
            if hasattr(self, "channel") and self.channel:
                message = {
                    ...
                    "request_auto_fix": error_info["severity"]
                    == ErrorSeverity.CRITICAL,
                }
