...
class EnhancedTaskWorker(BaseWorker):
    ...
    def process_message(self, ch, method, properties, body) -> None:
        ...
        if result['success']:
            evolution_result = self._handle_evolution(
                result['output'], task_id, prompt
            )
        
        # 結果保存
        self._save_result(
            task_output_dir, task_id, prompt, 
            result['output'], rag_applied, evolution_result
        )
        
        # RAG履歴保存
        self._save_to_rag(
            task_id, prompt, result['output'], 
            result['status'], task_type
        )
    ...
    def _build_enhanced_prompt(self, prompt: str) -> str:
        """RAG強化プロンプト構築"""
        try:
            enhanced = self.rag.build_context_with_github(
                prompt, include_code=True
            )
...
"""
    ...
    def _find_recent_files(self, directory: str, 
                          max_age_seconds: int = 30) -> List[str]:
        """最近生成されたファイルを検索"""
    ...
    def _handle_evolution(self, output: str, task_id: str, 
    ...
    def _save_result(self, output_dir: Path, task_id: str, 
                    prompt: str, output: str, rag_applied: bool,
    ...
    def _save_to_rag(self, task_id: str, prompt: str, 
                    response: str, status: str, task_type: str) -> None:
        """RAG履歴保存"""
        try:
    ...
    def _send_slack_notification(self, task_id: str, prompt: str,
                               response: str, status: str, 
                               task_type: str, rag_applied: bool,
                               evolution_result: Optional[Dict]) -> None:
        """Slack通知送信"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Enhanced Task Worker with base class'
    )
    parser.add_argument(
        '--worker-id',
        default=None,
