...
class ImprovedBaseWorker(ABC):
    ...
    def __init__(self, worker_type: str, worker_id: Optional[str] = None):
        ...
        # プロフェッショナルな初期化ログ
        self.logger.info(f"{self.__class__.__name__} initialized (ID: {self.worker_id}, PID: {os.getpid()})")
        ...
        for attempt in range(retry_count):
            try:
                ...
                if attempt < retry_count - 1:
                    ...
                    self.logger.error("RabbitMQ connection failed - max retries reached")
                    return False
    ...
    def _declare_queues(self):
        ...
        if self.channel:
            ...
            self.logger.debug(f"Queue declared: {self.input_queue}, {self.output_queue}")
    ...
    def _message_wrapper(self, ch, method, properties, body):
        ...
        try:
            ...
            self.logger.info(f"Message processed: {task_id} (duration: {duration:.3f}s, total: {self.processed_count})")
            
        except Exception as e:
            self.error_count += 1
            duration = time.time() - start_time
            self.logger.error(f"Message processing failed: {task_id} - {type(e).__name__}: {str(e)} (duration: {duration:.3f}s, errors: {self.error_count})")
    ...
    def _handle_shutdown(self, signum, frame):
        ...
        if self.current_task:
            self.logger.info(f"Waiting for current task to complete: {self.current_task}")
    ...
    def start(self):
        ...
        try:
            ...
            self.channel.basic_consume(
                ...
                on_message_callback=self._message_wrapper
            )
    ...
    def stop(self):
        ...
        if runtime > 0:
            ...
            self.logger.info(f"Final metrics: processed={self.processed_count}, errors={self.error_count}, runtime={runtime:.1f}s, rate={rate:.2f}/s")
