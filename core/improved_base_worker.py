...
class ImprovedBaseWorker(ABC):
    ...
    def _message_wrapper(self, ch, method, properties, body):
        ...
        try:
            ...
            duration = time.time() - start_time
            self.logger.error(
                f"Message processing failed: {task_id} - {type(e).__name__}: {str(e)} (duration: {duration:.3f}s, errors: {self.error_count})"
            )
    ...
    def stop(self):
        ...
        if runtime > 0:
            ...
            self.logger.info(
                f"Final metrics: processed={self.processed_count}, errors={self.error_count}, runtime={runtime:.1f}s, rate={rate:.2f}/s"
            )
