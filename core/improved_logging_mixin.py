...
class ImprovedLoggingMixin:
    ...
    def log_task_complete(self, task_id: str, result_summary: str = None):
        ...
        # 結果の要約を追加（50文字まで）
        if result_summary:
            summary = result_summary[:50] + "..." if len(result_summary) > 50 else result_summary
            message += f" - {summary}"
    ...
    def log_task_error(self, task_id: str, error: Exception, 
                      context: str = None, will_retry: bool = False):
        """タスクエラーをログ（技術的詳細）"""
    ...
    def log_processing(self, action: str, target: str, 
    ...
    def log_state_change(self, component: str, old_state: str, 
...
class ExampleWorker(ImprovedLoggingMixin):
    ...
    def process_task(self, task_id: str, data: Dict):
        ...
        try:
            ...
            # パフォーマンスログ
            self.log_performance("data processing", process_time, 
