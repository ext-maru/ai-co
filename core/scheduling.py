...
class ScheduledWorker:
    ...
    def schedule_task(self, name: str, schedule_type: str, schedule_config: Dict[str, Any],
                     task_data: Dict[str, Any]) -> ScheduledTask:
        """タスクをスケジュール"""
        task = ScheduledTask(
...
class ScheduleManager:
    ...
    def record_execution(self, task_id: str, success: bool, error_message: Optional[str] = None):
        """実行履歴を記録"""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
...
def validate_cron_expression(cron_expr: str) -> bool:
    ...
    for i, (part, (min_val, max_val), field_name) in enumerate(zip(parts, field_ranges, field_names)):
        try:
...
def parse_cron_field(field_value: str, field_type: str) -> set:
    ...
    for part in field_value.split(','):
        ...
        if '/' in part:
            ...
            # Apply step
            values.update({v for v in base_values if v % step == (min(base_values) % step)})
...
def calculate_next_run(schedule: Schedule, current_time: datetime) -> datetime:
    ...
    if schedule.schedule_type == ScheduleType.CRON:
        ...
        next_run = current_time.replace(hour=schedule.hour or 0, minute=0, second=0, microsecond=0)
        ...
        return next_run.replace(hour=schedule.hour or 0, minute=0, second=0, microsecond=0)
