...
class SecureTaskExecutor:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.allowed_commands = {
            ...
            'git', 'pip', 'pytest', 'ruff', 'black', 'mypy',
            'ls', 'cat', 'echo', 'grep', 'find', 'sed', 'awk'
        }
        
        # 禁止パターン（ブラックリスト）
        self.forbidden_patterns = [
            r'rm\s+-rf\s+/', r'sudo', r'chmod\s+777',
            r'eval\s*\(', r'exec\s*\(', r'__import__',
            r'os\.system', r'subprocess\.call',
            r'curl.*\|.*sh', r'wget.*\|.*sh',  # リモートスクリプト実行
            r'dd\s+if=/dev/zero',  # ディスク埋め
            r':(){ :|:& };:',  # フォーク爆弾
            r'>\s*/dev/sd',  # デバイス直接書き込み
            r'mkfs\.', r'fdisk',  # ファイルシステム操作
        ]
        
        # 環境変数のホワイトリスト
        self.allowed_env_vars = {
            'PATH', 'HOME', 'USER', 'LANG', 'LC_ALL',
            'PYTHONPATH', 'NODE_PATH', 'PWD', 'TMPDIR'
        }
