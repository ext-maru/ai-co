...
from libs.elder_council_summoner import ElderCouncilSummoner, UrgencyLevel, TriggerCategory, CouncilTrigger
from libs.elder_tree_hierarchy import (
    ElderTreeHierarchy, get_elder_tree, ElderMessage, 
    ElderRank, SageType, ElderDecision
)
...
class DialogTaskWorker(TaskWorker):
    ...
    def process_dialog_task(self, ch, method, properties, body):
        ...
        try:
            ...
            # 対話の複雑さを分析
            dialog_context = self.analyze_dialog_complexity(instruction, {
                'conversation_id': conversation_id,
                'message_count': len(messages)
            })
            
            logger.info(f"📊 Dialog Analysis - Complexity: {dialog_context.complexity}, Sentiment: {dialog_context.sentiment}")
            ...
            if self.elder_tree and dialog_context.requires_elder_guidance:
                ...
                try:
                    rag_insights = loop.run_until_complete(
                        self.consult_rag_sage_for_context(conversation_id, instruction, messages)
                    )
                    
                    # 複雑な対話の場合はClaude Elderにエスカレート
                    if dialog_context.complexity == "complex" or dialog_context.sentiment == "urgent":
                        elder_guidance = loop.run_until_complete(
                            self.escalate_to_claude_elder(
                                ...
                                {"instruction": instruction, "context": conversation_context}
                            )
                        )
                    ...
                    if "policy" in instruction.lower() or "ポリシー" in instruction:
                        ...
                        if council_decision:
                            dialog_context.elder_recommendations.extend(council_decision.get('guidelines', []))
            ...
            if "詳細" in instruction or "？" in instruction:
                ...
                if dialog_context.elder_recommendations:
                    question += "\n\n参考情報: " + ", ".join(dialog_context.elder_recommendations[:2])
                ...
                if elder_guidance and elder_guidance.get('suggested_approach') == 'empathetic_technical':
            ...
            if self.elder_tree and (dialog_context.complexity != "simple" or len(messages) > 10):
                insights = {
                ...
                def report_async():
                    ...
                    loop.run_until_complete(
                        self.report_dialog_insights_to_knowledge_sage(conversation_id, insights)
                    )
                    loop.close()
            ...
            logger.info(f"✅ 対話応答送信: {conversation_id} (Elder Tree: {response.get('elder_tree_engaged', False)})")
    ...
    async def consult_rag_sage_for_context(self, conversation_id: str, instruction: str, messages: List[Dict]) -> Optional[Dict[str, Any]]:
        """RAG賢者に対話コンテキストを相談"""
        if not self.elder_tree:
            return None
            
        try:
            logger.info(f"🧙‍♂️ Consulting RAG Sage for dialog context: {conversation_id}")
            ...
            logger.info(f"✅ RAG Sage provided context insights with confidence: {context_insights['confidence']}")
    ...
    async def report_dialog_insights_to_knowledge_sage(self, conversation_id: str, insights: Dict[str, Any]):
        """Knowledge賢者に対話インサイトを報告"""
        if not self.elder_tree:
            return
            
        try:
            logger.info(f"📚 Reporting dialog insights to Knowledge Sage: {conversation_id}")
    ...
    async def escalate_to_claude_elder(self, conversation_id: str, reason: str, context: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Claude Elderに複雑な対話をエスカレート"""
        if not self.elder_tree:
            return None
            
        try:
            logger.info(f"🤖 Escalating complex dialog to Claude Elder: {conversation_id}")
    ...
    async def request_elder_council_for_policy(self, conversation_id: str, policy_question: str) -> Optional[Dict[str, Any]]:
        """Elder評議会に対話ポリシー決定を要請"""
        if not self.elder_council_summoner:
            return None
            
        try:
            logger.info(f"🏛️ Requesting Elder Council for policy decision: {conversation_id}")
            ...
            trigger = CouncilTrigger(
                ...
                metrics={"conversation_length": len(self.conversation_manager.db.get_messages(conversation_id))},
    ...
    def analyze_dialog_complexity(self, instruction: str, context: Dict[str, Any]) -> DialogContext:
        """対話の複雑さを分析"""
        ...
        if any(word in instruction.lower() for word in ["複雑", "difficult", "urgent", "critical"]):
            complexity = "complex"
            requires_elder = True
        elif any(word in instruction.lower() for word in ["詳細", "specific", "technical"]):
            complexity = "moderate"
        ...
        if any(word in instruction.lower() for word in ["urgent", "急", "immediately"]):
            sentiment = "urgent"
            requires_elder = True
        elif any(word in instruction.lower() for word in ["problem", "issue", "error", "失敗"]):
            sentiment = "negative"
        elif any(word in instruction.lower() for word in ["thank", "great", "excellent", "素晴らしい"]):
            sentiment = "positive"
    ...
    def get_elder_tree_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "worker_elder_rank": self.elder_rank.value if hasattr(self, 'elder_rank') else None
        }
