...
try:
    ...
    print("Google APIs not installed. Install with: pip install google-api-python-client google-auth-httplib2 google-auth-oauthlib")
    sys.exit(1)
...
class EmailNotificationWorker:
    ...
    def process_email_task(self, ch, method, properties, body):
        ...
        try:
            ...
            task_id = task.get('task_id', f'email_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    ...
    def start(self):
        ...
        self.channel.basic_consume(queue='email_queue', on_message_callback=self.process_email_task)
