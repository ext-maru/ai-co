...
from core.monitoring.worker_controller import WorkerController
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOG_DIR / "pm_worker.log"), logging.StreamHandler()],
)
        ...
        def monitor_and_scale():
            ...
            """monitor_and_scaleメソッド"""
                try:
                    ...
                    logger.info(
                        f"📊 メトリクス: ワーカー={metrics['active_workers']}, キュー={metrics['queue_length']}"
                    )
            ...
            while self.scaling_enabled:  # スケーリングと同じフラグを使用
                try:
                    ...
                    for worker_info in workers:
                        health_status = self.health_checker.check_worker_health(
                            worker_info
                        )
