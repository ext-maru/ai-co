...
class TaskWorker:
    ...
    def process_task(self, ch, method, properties, body):
        try:
            ...
            # 🧠 RAG: 過去履歴を含めたプロンプト構築
            enhanced_prompt = self.rag.build_context_with_github(
...
"""
            ...
            if rag_applied:
                logger.info(
                    f"✨ RAG適用: プロンプト拡張 {len(prompt)} → {len(enhanced_prompt)} 文字"
                )
            ...
            if self.check_claude_cli():
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        evolution_result = self.handle_evolution(
                            output_text, task_id, prompt
                        )
                        if evolution_result:
    ...
    def handle_evolution(self, claude_output, task_id, original_prompt):
        ...
        try:
            ...
            for i, code_block in enumerate(code_blocks):
                logger.info(
                    f"🧬 コードブロック{i+1}: 言語={code_block['language']}, 長さ={len(code_block['content'])}文字"
                )
