        ...
        if self.categorical_columns:
            ...
            for col in self.categorical_columns:
                cat_stats[col] = {
                    ...
                    'most_common': self.data[col].mode().iloc[0] if not self.data[col].empty else None,
                    'frequency': self.data[col].value_counts().head(5).to_dict()
                }
            self.analysis_results['categorical_stats'] = cat_stats
            print("\nCategorical Columns Summary:")
            for col, stats in cat_stats.items():
                print(f"{col}: {stats['unique_values']} unique values, most common: {stats['most_common']}")
    ...
    def identify_patterns_and_anomalies(self):
        ...
        if self.numeric_columns:
            ...
            for col, info in outliers.items():
                print(f"  {col}: {info['count']} outliers ({info['percentage']:.1f}%) - {info['method']}")
        ...
        if self.categorical_columns:
            ...
            for col in self.categorical_columns:
                ...
                if len(value_counts) > 1:
                    top_category_percentage = (value_counts.iloc[0] / len(self.data)) * 100
            ...
            if cat_patterns:
                ...
                for col, pattern in cat_patterns.items():
                    print(f"  {col}: {pattern['type']} - {pattern['dominant_category']} ({pattern['percentage']:.1f}%)")
...
"""
        ...
        if self.categorical_columns:
            report += "- `categorical_distributions.png` - Distribution plots for categorical variables\n"
        ...
        if insights:
            for i, insight in enumerate(insights, 1):
                report += f"### {i}. {insight['category']} - {insight['priority']} Priority\n"
