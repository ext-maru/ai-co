...
async def debug_quality_calculation():
    ...
    # 品質スコア計算の詳細をプリント
    syntax_score = pm_worker._calculate_category_score(analysis_results["syntax_issues"])
    logic_score = pm_worker._calculate_category_score(analysis_results["logic_issues"])
    performance_score = pm_worker._calculate_category_score(analysis_results["performance_issues"])
    security_score = pm_worker._calculate_security_score(analysis_results["security_issues"])
    ...
    # 実際の計算
    actual_score = await pm_worker._calculate_quality_score(analysis_results, code_metrics)
