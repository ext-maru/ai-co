...
async def create_contract_upload(
    ...
    try:
        ...
        return await service.create_contract_upload(
            user_id=current_user["id"], user_name=user_name, data=data
        )
...
async def list_contract_uploads(
    ...
    return ContractUploadListResponse(
        items=uploads, total=total, skip=skip, limit=limit
    )
...
async def list_all_contract_uploads(
    ...
    uploads = await service.list_contract_uploads(
        user_id=None, status=status, skip=skip, limit=limit
    )  # 全ユーザー対象
...
async def get_pending_contracts(
    ...
    uploads = await service.list_contract_uploads_by_statuses(
        statuses=pending_statuses, skip=skip, limit=limit
    )
