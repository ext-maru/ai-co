...
from libs.deployment_integration import DeploymentIntegration, get_deployment_integration
...
class TestDeploymentIntegration(unittest.TestCase):
    ...
    def test_should_use_github_actions(self):
        ...
        # GitHub Actionsに設定
        self.config_manager.set_deployment_method(self.test_project, "development", "github_actions")
    ...
    def test_should_use_ssh(self):
        ...
        # SSHに設定
        self.config_manager.set_deployment_method(self.test_project, "development", "ssh")
    ...
    def test_hybrid_deployment(self):
        ...
        # ハイブリッドに設定
        self.config_manager.set_deployment_method(self.test_project, "development", "hybrid")
        self.assertTrue(self.integration.should_use_github_actions())
        self.assertTrue(self.integration.should_use_ssh())
    ...
    def test_integrate_with_command(self):
        ...
        # ビルドコマンド
        result = self.integration.integrate_with_command('build', {
            'project': self.test_project
        })
        self.assertIn('build_command', result)
        
        # テストコマンド
        result = self.integration.integrate_with_command('test', {
            'project': self.test_project
        })
    ...
    def test_deployment_method_override(self):
        ...
        # GitHub Actions強制
        result = self.integration.integrate_with_command('deploy', {'use_github_actions': True})
...
class TestDeploymentIntegrationEdgeCases(unittest.TestCase):
    ...
    def test_no_config_directory(self):
        ...
        # 一時的な設定ディレクトリ名を使用
        import uuid
        temp_config_dir = f"deployment-configs-test-{uuid.uuid4()}"
        
        try:
            # 新しいインスタンスを作成（存在しないディレクトリを指定）
            from libs.project_deployment_config import ProjectDeploymentManager
            manager = ProjectDeploymentManager(config_dir=temp_config_dir)
