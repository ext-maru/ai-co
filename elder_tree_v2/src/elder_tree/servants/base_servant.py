...
class ElderServantBase(A2AServer):
    ...
    def _setup_prometheus_metrics(self):
        ...
        self.iron_will_violations = Counter(
            'servant_iron_will_violations_total',
            'Iron Will violations detected',
            ['servant_name', 'violation_type', 'tribe']
        )
        
        self.active_connections_gauge = Gauge(
            'servant_active_sage_connections',
            'Active sage connections',
            ['servant_name', 'tribe']
        )
    ...
    def _register_servant_handlers(self):
        """サーバント共通ハンドラー登録 (python-a2a 0.5.9対応)"""
        
        @self.handle("execute_task")
    ...
    async def _consult_sages_before_task(
        ...
        consultation_tasks = []
        for sage_name in relevant_sages:
    ...
    async def collaborate_with_sage(
        ...
        """
        sage_instance = self.sage_instances.get(sage_name)
        if sage_instance and self.sage_connections.get(sage_name, False):
            try:
