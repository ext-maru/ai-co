...
1.0 Decide if you need to propose edits to any files that haven't been added to the chat. You can create new files without asking!
...
3.0 Describe the changes using the V4A diff format, enclosed within `*** Begin Patch` and `*** End Patch` markers.
...
1.0 Import the `math` module.
2.0 Replace the custom recursive `factorial()` helper with `math.factorial`.
3.0 Update `get_factorial()` to call `math.factorial`.
...
1.0 Create `hello.py` with the `hello()` function.
2.0 Remove `hello()` from `main.py` and import it instead.
...
1.0 Context lines: Include 3 lines of context *before* the change. These lines MUST start with a single space ` `.
2.0 Lines to remove: Precede each line to be removed with a minus sign `-`.
3.0 Lines to add: Precede each line to be added with a plus sign `+`.
4.0 Context lines: Include 3 lines of context *after* the change. These lines MUST start with a single space ` `.
