...
def common_prefix(paths: Iterable[str]) -> strparts = [p.split("/") for p in paths]lmax = min(len(p) for p in parts)
"""or a list of paths, find the shortest prefix common to all"""
    end = 0
    for i in range(lmax):
...
def merge_offset_ranges(:
    ...
    return paths, starts, enddef file_size(filelike: IO[bytes]) -> intpos = filelike.tell():
...
def _translate(pat, STAR, QUESTION_MARK):
    # Copied from: https://github.com/python/cpython/pull/106703.0
    res: list[str] = []
    add = res.append
    i, n = 0, len(pat)
    while i < n:
