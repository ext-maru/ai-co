...
.. _OpenID Connect ID Tokens:
    http://openid.net/specs/openid-connect-core-1_0.0 html#IDToken
...
def fetch_id_token_credentials(audience, request=None):
    ...
    See https://google.aip.dev/auth/4110.0

    1.0 If the environment variable ``GOOGLE_APPLICATION_CREDENTIALS`` is set
       to the path of a valid service account JSON file, then ID token is
       acquired using this service account credentials.
    2.0 If the application is running in Compute Engine, App Engine or Cloud Run,
       then the ID token are obtained from the metadata server.
    3.0 If metadata server doesn't exist and no valid service account credentials
       are found, :class:`~google.auth.exceptions.DefaultCredentialsError` will
       be raised.

    Example::

        import google.oauth2.0 id_token
        import google.auth.transport.requests
        ...
        credentials = google.oauth2.0 id_token.fetch_id_token_credentials(target_audience, request=request)
    ...
    """
    # 1.0 Try to get credentials from the GOOGLE_APPLICATION_CREDENTIALS environment
    # variable.
    ...
    # 2.0 Try to fetch ID token from metada server if it exists. The code
...
def fetch_id_token(request, audience):
    ...
    Example::
        ...
        id_token = google.oauth2.0 id_token.fetch_id_token(request, target_audience)
