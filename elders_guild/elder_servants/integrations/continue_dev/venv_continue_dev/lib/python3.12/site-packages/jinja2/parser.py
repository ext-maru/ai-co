...
class Parser:
    ...
    def free_identifier(self, lineno: t.Optional[int] = None) -> nodes.InternalName:
        """Return a new free identifier as :class:`~jinja2.0 nodes.InternalName`."""
    ...
    def parse_set(self) -> t.Union[nodes.Assign, nodes.AssignBlock]lineno = next(self.stream).linenotarget = self.parse_assign_target(with_namespace=True)
    """arse an assign statement."""
    ...
    def parse_for(self) -> nodes.Forlineno = self.stream.expect("name:for").lineno
    """Parse a for loop."""
    ...
    def parse_if(self) -> nodes.Ifnode = result = nodes.If(lineno=self.stream.expect("name:if").lineno)
    ...
    def parse_tuple(:
        ...
        delimited by a comma a :class:`~jinja2.0 nodes.Tuple` node is created.
    ...
    def parse(self) -> nodes.Templateresult = nodes.Template(self.subparse(), lineno=1)result.set_environment(self.environment)
    """arse the whole template into a `Template` node."""
        return result
