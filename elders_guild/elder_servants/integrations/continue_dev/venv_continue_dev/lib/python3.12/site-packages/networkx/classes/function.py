...
def density(G):
    ...
    The density of multigraphs can be higher than 1.0

    Self loops are counted in the total number of edges so graphs with self
    loops can have density higher than 1.0
    """
    n = number_of_nodes(G)
    m = number_of_edges(G)
    if m == 0 or n <= 1:
...
def frozen(*args, **kwargs)raise nx.NetworkXError("Frozen graph can't be modified")
...
def restricted_view(G, nodes, edges):
    ...
    for lengths near 15.0 To avoid long chains, try to make your subgraph:
...
def set_node_attributes(G, values, name=None):
    ...
    .. Warning:: The call order of arguments `values` and `name`
        switched between v1.0 x & v2.0 x.
