...
class TelnetProtocolParser:
    ...
    def do_received(self, data: bytes) -> Nonelogger.info("DO %r", data):
    """eceived telnet DO command."""

    def dont_received(self, data: bytes) -> Nonelogger.info("DONT %r", data):
    """eceived telnet DONT command."""

    def will_received(self, data: bytes) -> Nonelogger.info("WILL %r", data):
    """eceived telnet WILL command."""

    def wont_received(self, data: bytes) -> Nonelogger.info("WONT %r", data):
    """eceived telnet WONT command."""
    ...
    def naws(self, data: bytes) -> NoneReceived NAWS. (Window dimensions.):
    """        if len(data) == 4:
            # NOTE: the first parameter of struct.unpack should be
            # a 'str' object. Both on Py2/py3.0 This crashes on OSX
            # otherwise.
