...
class WindowRenderInfo:
    ...
    @property
    def displayed_lines(self) -> list[int]List of all the visible rows. (Line numbers of the input buffer.)
    ...
    def firdef first_visible_line(self, after_scroll_offset: bool = False) -> intReturn the line number (0 based) of the input document that corresponds:
    ...
    @property
    def fuldef full_height_visible(self) -> boolTrue when the full height is visible (There is no vertical scroll.):
    """ return (
...
class Window(Container):
    ...
    def _get_total_margdef _get_total_margin_width(self) -> intCalculate and return the width of the margin (left + right).self._get_margin_width(m) for m in self.right_margins
    """self._get_margin_width(m) for m in self.left_margins) + sum(
        )
