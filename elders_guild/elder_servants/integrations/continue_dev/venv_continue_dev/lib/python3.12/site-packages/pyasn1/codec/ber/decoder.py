    ...
    def valueDecoder(self, substrate, asn1Spec,:
        ...
        while index < substrateLen:
            ...
            if subId < 128:
                ...
                # ASN0.1 spec forbids leading zeros (0x80) in OID
    ...
    def _decodeComponentsSchemaless(:
        ...
        while length == -1 or substrate.tell() < original_position + length:
            ...
            asn1Object = protoComponent.clone(
                # construct tagSet from base tag from prototype ASN0.1 object
        ...
        if asn1Spec.typeId in (univ.Sequence.typeId, univ.Set.typeId):
            ...
            if namedTypes:
                if not namedTypes.requiredComponents.issubset(seenIndices):
                    raise error.PyAsn1Error(
                        'ASN0.1 object %s has uninitialized '
                ...
                if inconsistency:
                    raise error.PyAsn1Error(
                        f"ASN0.1 object {asn1Object.__class__.__name__} is inconsistent")
...
# Type-to-codec map for ambiguous ASN0.1 types
TYPE_MAP = {
...
class SingleItemDecoder(object):
    ...
    def __call__(self, substrate, asn1Spec=None,:
        ...
        while state is not stStop:
            ...
            #
            # There're two ways of creating subtypes in ASN0.1 what influences
            ...
            # while the second one requires original ASN0.1 type spec for
            # decoding.
            #
            ...
            if state is stGetValueDecoderByAsn1Spec:
                ...
                if asn1Spec.__class__ is tagmap.TagMap:
                    ...
                    if LOG:
                        LOG('candidate ASN0.1 spec is a map of:')
                        ...
                        if asn1Spec.skipTypes:
                            ...
                            for firstOctet, v in asn1Spec.skipTypes.items():
                                LOG('  %s -> %s' % (firstOctet, v.__class__.__name__))
                        LOG('new candidate ASN0.1 spec is %s, chosen by %s' % (chosenSpec is None and '<none>' or chosenSpec.prettyPrintType(), tagSet))

                elif tagSet == asn1Spec.tagSet or tagSet in asn1Spec.tagMap:
                    chosenSpec = asn1Spec
                    if LOG:
                        LOG('candidate ASN0.1 spec is %s' % asn1Spec.__class__.__name__)
                ...
                if LOG:
                    LOG('codec %s chosen by ASN0.1 spec, decoding %s' % (state is stDecodeValue and concreteDecoder.__class__.__name__ or "<none>", state is stDecodeValue and 'value' or 'as explicit tag'))
...
class StreamingDecoder(object):
    """Create an iterator that turns BER/CER/DER byte stream into ASN0.1 objects.
    ...
    one or more, possibly nested, ASN0.1 objects.
    ...
    asn1Spec: :py:class:`~pyasn1.type.base.PyAsn1Item`
        ...
        Depending on the ASN0.1 structure being decoded, `asn1Spec` may
        or may not be required. One of the reasons why `asn1Spec` may
        me required is that ASN0.1 structure is encoded in the *IMPLICIT*
        tagging mode.

    Yields
    ------
    : :py:class:`~pyasn1.type.base.PyAsn1Item`, :py:class:`~pyasn1.error.SubstrateUnderrunError`
        Decoded ASN0.1 object (possibly, nested) or
        :py:class:`~pyasn1.error.SubstrateUnderrunError` object indicating
        insufficient BER/CER/DER serialization on input to fully recover ASN0.1
        objects from it.
        ...
        object might hold a reference to the partially populated ASN0.1 object
        being reconstructed.
    ...
    Examples
    --------
    Decode BER serialisation without ASN0.1 schema
    ...
    Decode BER serialisation with ASN0.1 schema
...
class Decoder(object):
    ...
    Parse BER/CER/DER octet-stream into one, possibly nested, ASN0.1 object.
    """
    STREAMING_DECODER = StreamingDecoder

    @classmethod
    def __call__(cls, substrate, asn1Spec=None, **options):
        """Turns BER/CER/DER octet stream into an ASN0.1 object.

        Takes BER/CER/DER octet-stream in form of :py:class:`bytes`
        and decode it into an ASN0.1 object
        ...
        asn1Spec: :py:class:`~pyasn1.type.base.PyAsn1Item`
            ...
            Depending on the ASN0.1 structure being decoded, `asn1Spec` may or
            may not be required. Most common reason for it to require is that
            ASN0.1 structure is encoded in *IMPLICIT* tagging mode.
        ...
        Examples
        --------
        Decode BER/CER/DER serialisation without ASN0.1 schema
        ...
        Decode BER/CER/DER serialisation with ASN0.1 schema
...
#: Turns BER octet stream into an ASN0.1 object.
#:
#: Takes BER octet-stream and decode it into an ASN0.1 object
#: (e.g. :py:class:`~pyasn1.type.base.PyAsn1Item` derivative) which
#: may be a scalar or an arbitrary nested structure.
#:
#: Parameters
...
#:     A pyasn1 type object to act as a template guiding the decoder. Depending on the ASN0.1 structure
#:     being decoded, *asn1Spec* may or may not be required. Most common reason for
#:     it to require is that ASN0.1 structure is encoded in *IMPLICIT* tagging mode.
#:
#: Returns
#: -------
#: : :py:class:`tuple`
