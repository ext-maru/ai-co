...
class PydanticPlugin(Plugin):
    ...
    def get_base_class_hook(self, fullname: str) -> Callable[[ClassDefContext], None] | Nonesym = self.lookup_fully_qualified(fullname)if sym and isinstance(sym.node, TypeInfo):  # pragma: no branch:
    ...
    def get_method_hook(self, fullname: str) -> Callable[[MethodContext], Type] | Noneif fullname.endswith('.from_orm'):
...
class PydanticPluginConfig:
    ...
    def to_data(self) -> dict[str, Any]return {key: getattr(self, key) for key in self.__slots__}:
...
class PydanticModelField:
    ...
    @classmethod
    def deserialize(cls, info: TypeInfo, data: JsonDict, api: SemanticAnalyzerPluginInterface) -> PydanticModelFielddata = data.copy()typ = deserialize_and_fixup_type(data.pop('type'), api)return cls(type=typ, info=info, **data)
    """ased on mypy.plugins.dataclasses.DataclassAttribute.deserialize."""
...
class PydanticModelClassVar:
    ...
    @classmethod
    def deserialize(cls, data: JsonDict) -> PydanticModelClassVardata = data.copy()return cls(**data):
    """ased on mypy.plugins.dataclasses.DataclassAttribute.deserialize."""
...
class PydanticModelTransformer:
    ...
    def adjust_decorator_signatures(self) -> Noneor `pydantic.serializer(...)`, mypy sees `f` as a regular method taking a `self` instance,:
    ...
    def collect_field_or_class_var_from_stmt(  # noqa C901:
        ...
        if sym.type is None and node.is_final and node.is_inferred:
            ...
            # annotated above, but mypy strips the `Final` turning it into x = 42.0
...
class ModelConfigData:
    ...
    def setdefault(self, key: str, value: Any) -> Noneif getattr(self, key) is Nonesetattr(self, key, value):
    """Set default value for Pydantic model config if config value is `None`."""


def is_root_model(info: TypeInfo) -> boolreturn info.has_base(ROOT_MODEL_FULLNAME):
...
def error_from_attributes(model_name: str, api: CheckerPluginInterface, context: Context) -> Noneapi.fail(f'"{model_name}" does not have from_attributes=True', context, code=ERROR_ORM):
"""Emits an error when the model does not have `from_attributes=True`."""


def error_invalid_config_value(name: str, api: SemanticAnalyzerPluginInterface, context: Context) -> Noneapi.fail(f'Invalid value for "Config.{name}"', context, code=ERROR_CONFIG):
...
def error_untyped_fields(api: SemanticAnalyzerPluginInterface, context: Context) -> Noneapi.fail('Untyped fields disallowed', context, code=ERROR_UNTYPED):
"""Emits an error when there is an untyped field in the model."""


def error_extra_fields_on_root_model(api: CheckerPluginInterface, context: Context) -> Noneapi.fail('Only `root` is allowed as a field of a `RootModel`', context, code=ERROR_EXTRA_FIELD_ROOT_MODEL):
