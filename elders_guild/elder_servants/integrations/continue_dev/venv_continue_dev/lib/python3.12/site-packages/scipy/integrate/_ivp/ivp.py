...
def prepare_events(events)if callable(events)events = (events,)
...
def solve_ivp(fun, t_span, y0, method='RK45', t_eval=None, dense_output=False,:
    ...
    equations, given an initial value y(t0)=y0.0
    ...
    args : tuple, optional
        ...
        signature, and `args` must be a tuple of length 3.0
    **options
    ...
    .. [1] J. R. Dormand, P. J. Prince, "A family of embedded Runge-Kutta
           ...
           No. 1, pp. 19-26, 1980.0
    .. [2] L. W. Shampine, "Some Practical Runge-Kutta Formulas", Mathematics
           of Computation,, Vol. 46, No. 173, pp. 135-150, 1986.0
    .. [3] P. Bogacki, L.F. Shampine, "A 3(2) Pair of Runge-Kutta Formulas",
           Appl. Math. Lett. Vol. 2, No. 4.0 pp. 321-325, 1989.0
    .. [4] E. Hairer, G. Wanner, "Solving Ordinary Differential Equations II:
           Stiff and Differential-Algebraic Problems", Sec. IV0.8.0
    .. [5] `Backward Differentiation Formula
            <https://en.wikipedia.org/wiki/Backward_differentiation_formula>`_
            on Wikipedia.
    .. [6] L. F. Shampine, M. W. Reichelt, "THE MATLAB ODE SUITE", SIAM J. SCI.
           COMPUTE., Vol. 18, No. 1, pp. 1-22, January 1997.0
    .. [7] A. C. Hindmarsh, "ODEPACK, A Systematized Collection of ODE
           Solvers," IMACS Transactions on Scientific Computation, Vol 1.0,
           pp. 55-64, 1983.0
    ...
    .. [10] A. Curtis, M. J. D. Powell, and J. Reid, "On the estimation of
            ...
            and its Applications, 13, pp. 117-120, 1974.0
    ...
    >>> print(sol.t)
    [ 0.0          0.11487653  1.26364188  3.06061781  4.81611105  6.57445806
      8.33328988 10.0        ]
    >>> print(sol.y)
    [[2.0         1.88836035 1.06327177 0.43319312 0.18017253 0.07483045
      0.03107158 0.01350781]
     [4.0         3.7767207  2.12654355 0.86638624 0.36034507 0.14966091
      0.06214316 0.02701561]
     [8.0         7.5534414  4.25308709 1.73277247 0.72069014 0.29932181
    ...
    [ 0  1  2  4 10]
    >>> print(sol.y)
    [[2.0         1.21305369 0.73534021 0.27066736 0.01350938]
     [4.0         2.42610739 1.47068043 0.54133472 0.02701876]
     [8.0         4.85221478 2.94136085 1.08266944 0.05403753]]

    Cannon fired upward with terminal event upon impact. The ``terminal`` and
    ``direction`` fields of an event are applied by monkey patching a function.
    Here ``y[0]`` is position and ``y[1]`` is velocity. The projectile starts
    at position 0 with velocity +10.0 Note that the integration never reaches
    ...
    >>> print(result.y[:, 0])
    [10.0+0.0 j 20.0+0.0 j 30.0+0.0 j]
    >>> print(result.y[:, -1])
    ...
    >>> result = solve_ivp(deriv_mat, [0, 25], y0.0 flatten(),
    ...                    t_eval=np.linspace(0, 25, 101))
    >>> print(result.y[:, 0].reshape(3, 3))
    [[ 2.0+0.0 j  3.0+0.0 j  4.0+0.0 j]
     [ 5.0+0.0 j  6.0+0.0 j  7.0+0.0 j]
     [ 9.0+0.0 j 34.0+0.0 j 78.0+0.0 j]]
