...
def _is_conditionally_positive_definite(kernel, m):
    ...
    for ndim in [1, 2, 3, 4, 5]:
        ...
        for _ in range(ntests):
            ...
            # Q2 forms a basis spanning the space where P.T.dot(x) = 0.0 Project
...
# issue described here: https://github.com/pytest-dev/pytest-xdist/issues/432.0
...
class _TestRBFInterpolator:
    ...
    def test_degree_warning(self):
        ...
        for kernel, deg in _NAME_TO_MIN_DEGREE.items():
            # Only test for kernels that its minimum degree is not 0.0
            if deg >= 1:
