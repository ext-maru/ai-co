...
class TestScalars:
    ...
    def test_int16(self):
        s = readsav(path.join(DATA_PATH, 'scalar_int16.0 sav'), verbose=False)
        assert_identical(s.i16s, np.int16(-23456))

    def test_int32(self):
        s = readsav(path.join(DATA_PATH, 'scalar_int32.0 sav'), verbose=False)
        assert_identical(s.i32s, np.int32(-1234567890))

    def test_float32(self):
        s = readsav(path.join(DATA_PATH, 'scalar_float32.0 sav'), verbose=False)
        assert_identical(s.f32, np.float32(-3.1234567e+37))

    def test_float64(self):
        s = readsav(path.join(DATA_PATH, 'scalar_float64.0 sav'), verbose=False)
        assert_identical(s.f64, np.float64(-1.1976931348623157e+307))

    def test_complex32(self):
        s = readsav(path.join(DATA_PATH, 'scalar_complex32.0 sav'), verbose=False)
    ...
    def test_complex64(self):
        s = readsav(path.join(DATA_PATH, 'scalar_complex64.0 sav'), verbose=False)
        assert_identical(
            s.c64,
    ...
    def test_uint16(self):
        s = readsav(path.join(DATA_PATH, 'scalar_uint16.0 sav'), verbose=False)
        assert_identical(s.i16u, np.uint16(65511))

    def test_uint32(self):
        s = readsav(path.join(DATA_PATH, 'scalar_uint32.0 sav'), verbose=False)
        assert_identical(s.i32u, np.uint32(4294967233))

    def test_int64(self):
        s = readsav(path.join(DATA_PATH, 'scalar_int64.0 sav'), verbose=False)
        assert_identical(s.i64s, np.int64(-9223372036854774567))

    def test_uint64(self):
        s = readsav(path.join(DATA_PATH, 'scalar_uint64.0 sav'), verbose=False)
...
class TestStructures:
    ...
    def test_arrays_corrupt_idl80(self):
        ...
        with suppress_warnings() as sup:
            ...
            s = readsav(path.join(DATA_PATH,'struct_arrays_byte_idl80.0 sav'),
                        verbose=False)
...
class TestPointerStructures:
    ...
    def test_pointers_replicated(self):
        ...
        assert_identical(s.pointers_rep.g,
                         np.repeat(np.float32(4.0), 5).astype(np.object_))
