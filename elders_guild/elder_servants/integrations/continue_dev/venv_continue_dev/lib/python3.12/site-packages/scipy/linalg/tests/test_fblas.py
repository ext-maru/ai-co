...
class BaseGer:
    def get_data(self,x_stride=1,y_stride=1):
        from numpy.random import normal, seed
        seed(1234)
        alpha = array(1.0, dtype = self.dtype)
        a = normal(0.0,1.0,(3,3)).astype(self.dtype)
...
class BaseGerComplex(BaseGer):
    def get_data(self,x_stride=1,y_stride=1):
        ...
        a = normal(0.0,1.0,(3,3)).astype(self.dtype)
        a = a + normal(0.0,1.0,(3,3)) * array(1j, dtype = self.dtype)
        x = normal(0.0,1.0,shape(a)[0]*x_stride).astype(self.dtype)
        x = x + x * array(1j, dtype = self.dtype)
        y = normal(0.0,1.0,shape(a)[1]*y_stride).astype(self.dtype)
        y = y + y * array(1j, dtype = self.dtype)
        return alpha,a,x,y
    def test_simple(self):
        alpha,a,x,y = self.get_data()
        # transpose takes care of Fortran vs. C(and Python) memory layout
        a = a * array(0.0,dtype = self.dtype)
