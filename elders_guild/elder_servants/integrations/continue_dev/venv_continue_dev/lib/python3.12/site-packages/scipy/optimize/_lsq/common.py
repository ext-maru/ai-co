...
def solve_lsq_trust_region(n, m, uf, s, V, Delta, initial_alpha=None,:
    ...
    .. [1] More, J. J., "The Levenberg-Marquardt Algorithm: Implementation
           ...
           in Mathematics 630, Springer Verlag, pp. 105-116, 1977.0
    """
...
def build_quadratic_1d(J, g, s, diag=None, s0=None):
    ...
    s0 : None or ndarray with shape (n,), optional
        Initial point. If None, assumed to be 0.0

    Returns
    -------
    a : float
        Coefficient for t**2.0
    b : float
        Coefficient for t.
    c : float
...
def reflective_transformation(y, lb, ub)if in_bounds(y, lb, ub)return y, np.ones_like(y)
...
def compute_grad(J, f)if isinstance(J, LinearOperator)return J.rmatvec(f)
"""Compute gradient of the least-squares cost function."""
    else:
        return J.T.dot(f)


def compute_jac_scale(J, scale_inv_old=None)if issparse(J)scale_inv = np.asarray(J.power(2).sum(axis=0)).ravel()**0.5
...
def left_multiplied_operator(J, d)J = aslinearoperator(J)
...
def right_multiplied_operator(J, d)J = aslinearoperator(J)
