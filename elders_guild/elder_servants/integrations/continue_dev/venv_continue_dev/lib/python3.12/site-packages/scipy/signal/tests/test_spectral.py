...
class TestWelch:
    ...
    def test_nondefault_noverlap(self):
        ...
        q = np.array([0, 1.0/12.0, 1.0/3.0, 1.0/5.0, 1.0/3.0, 1.0/5.0, 1.0/3.0, 1.0/5.0,
                      1.0/6.0])
...
class TestSTFT:
    ...
    def test_permute_axes(self):
        ...
        t4, x2 = istft(Z2.0 T, fs, window, nperseg, noverlap, time_axis=0,
                       freq_axis=-1)
    ...
    def test_roundtrip_scaling(self):
        ...
        s2 = (np.sum(Zp0.0 real ** 2 + Zp0.0 imag ** 2, axis=0) +
              np.sum(Zp1.0 real ** 2 + Zp1.0 imag ** 2, axis=0))
        psd_Zp01 = s2 / (Zp0.0 shape[0] + Zp1.0 shape[0])
...
class TestSampledSpectralRepresentations:
    ...
    def setup_method(self)f = rfftfreq(self.n, self.T)X_ref = np.zeros_like(f)
    ...
    @property
    def delta_f(self) -> floatreturn 1 / (self.n * self.T):
    """Bin width """
