...
class AiderAdapter(BaseOSSAdapter):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                result = subprocess.run(
                    [self.aider_path, "--yes", "--message", message, temp_file],
                    cwd=temp_dir,
                    capture_output=True,
                    text=True,
                    timeout=timeout,
    ...
    async def health_check(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.aider_path, "--version"], capture_output=True, timeout=5
            )
...
class Flake8Adapter(BaseOSSAdapter):
    ...
    async def health_check(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["python3", "-m", "flake8", "--version"], capture_output=True, timeout=5
            )
            healthy = result.returncode == 0
            self.status = AdapterStatus.ACTIVE if healthy else AdapterStatus.ERROR
            return healthy
...
class PyTestAdapter(BaseOSSAdapter):
    ...
    async def execute(self, request: AdapterRequest) -> AdapterResponse:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                timeout = request.timeout or self.capability.timeout
                result = subprocess.run(
                    ["python3", "-m", "pytest"] + test_args + [test_file],
                    capture_output=True,
                    text=True,
                    timeout=timeout,
                    cwd=temp_dir,
    ...
    async def health_check(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["python3", "-m", "pytest", "--version"], capture_output=True, timeout=5
            )
            healthy = result.returncode == 0
            self.status = AdapterStatus.ACTIVE if healthy else AdapterStatus.ERROR
            return healthy
...
async def elder_code_generation_fallback(request: AdapterRequest) -> AdapterResponse:
    ...
    return AdapterResponse(
        success=True,
        data={
            "generated_code": "# Elder System fallback " \
...
async def test_framework():
    ...
    print(
        f"  Success Rate: {status['metrics']['successful_requests']}/{status['metrics']['total_requests']}"
    )
