...
class ElderRegistry:
            ...
            if agent_info:
                ...
                await self.register_agent(
                    ...
                    description=agent_info.get(
                        "description", f"Auto-discovered {agent_id}"
                    ),
            ...
            try:
                ...
                for agent_data in data.get("agents", []):
                    ...
                    if agent_data.get("created_at"):
                        agent.created_at = datetime.fromisoformat(
                            agent_data["created_at"]
                        )
                    if agent_data.get("last_updated"):
                        agent.last_updated = datetime.fromisoformat(
    ...
    async def _analyze_agent_script(
...
class PortAllocator:
    ...
    async def generate_agent_script(
        self, agent_id: str, agent_type: AgentType, script_path: str
    ):
        """エージェントスクリプト生成"""
