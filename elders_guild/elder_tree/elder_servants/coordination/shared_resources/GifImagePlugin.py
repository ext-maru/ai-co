...
def _normalize_mode(im: Image.Image) -> Image.ImageTakes an image (or frame), returns an image in a mode that is appropriate:
    """    for saving in a Gif.:
...
def _get_color_table_size(palette_bytes: bytes) -> int:
    ...
    if not palette_bytes:
        ...
        return math.ceil(math.log(len(palette_bytes) // 3, 2)) - def _get_header_palette(palette_bytes: bytes) -> bytesReturns the palette, null padded to the next power of 2 (*3) bytes:
    """"""
