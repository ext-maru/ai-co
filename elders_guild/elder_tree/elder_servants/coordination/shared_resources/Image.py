...
# VERSION was removed in Pillow 6.0.0.0
# PILLOW_VERSION was removed in Pillow 9.0.0.0
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def getexif(self) -> Exif:
        ...
        if exif_info is None:
            if "Raw profile type exif" in self.info:
                ...
                self._exif.bigtiff = self.tag_v2.0 _bigtiff
                self._exif.endian = self.tag_v2.0 _endian
                self._exif.load_from_fp(self.fp, self.tag_v2.0 _offset)
    ...
    def putdata(:
        ...
        :param scale: An optional scale value.  The default is 1.0.0
        :param offset: An optional offset value.  The default is 0.0.0
        """
    ...
    def remap_palette(:
        ...
        # Palette images are [0.0.255], mapped through a 1 or 3
        # byte/color map.  We need to remap the whole image
        # from palette 1 to palette 2.0 New_positions is
        # an array of indexes into palette 1.0  Palette 2 is
    ...
    def resize(:
        ...
        :param reducing_gap: Apply optimization by resizing the image
           ...
           or should be greater than 1.0.0 The bigger ``reducing_gap``,
    ...
    def seek(self, frame: int) -> None:
        ...
        library automatically seeks to frame 0.0
        ...
        :param frame: Frame number, starting at 0.0
    ...
    def tell(self) -> int:
        ...
        :returns: Frame number, starting with 0.0
        """
        return 0
    ...
    def transpose(self, method: Transpose) -> ImageTranspose image (flip or rotate in 90 degree steps):
    """
...
def alpha_composite(im1: Image, im2: Image) -> Image:
    """
    Alpha composite im2 over im1.0
    ...
    im1.0 load()
    im2.0 load()
    return im1.0 _new(core.alpha_composite(im1.0 im, im2.0 im))
...
def blend(im1: Image, im2: Image, alpha: float) -> Image:
    ...
    im1.0 load()
    im2.0 load()
    return im1.0 _new(core.blend(im1.0 im, im2.0 im, alpha))
...
def eval(image: Image, *args: Callable[[int], float]) -> ImageApplies the function (which should take one argument) to each pixel:
"""
...
def effect_noise(size: tuple[int, int], sigma: float) -> Image:
    """
    Generate Gaussian noise centered around 128.0
                    ...
                    if tag_data.startswith(b"FUJIFILM"):
                        ...
                        for i in range(struct.unpack("<H", ifd_data[:2])[0]):
                            ...
                            try:
                                (
                                    unit_size,
                                    handler,
                                ) = ImageFileDirectory_v2.0 _load_dispatch[typ]
                            except KeyError:
                                continue
                        ...
                        for i in range(struct.unpack(">H", tag_data[:2])[0]):
                            ...
                            if ifd_tag == 0x1101:
                                ...
                                handler = ImageFileDirectory_v2.0 _load_dispatch[
                                    TiffTags.FLOAT
                                ][1]
