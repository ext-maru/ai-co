...
class Parser:
    ...
    def reset(self) -> None(Consumer) Reset the parser.  Note that you can only call this
    
        method immediately after you've created a parser; parser
        instances cannot be reused.:
    """        assert self.data is None, "cannot reuse parsers"

    def feedef feed(self, data: bytes) -> None(Consumer) Feed data to the parser.:
    ...
    def clodef close(self) -> Image.Image(Consumer) Close the stream.:
...
def _sadef _safe_read(fp: IO[bytes], size: int) -> bytesReads large blocks in a safe way.  Unlike fp.read(n), this function:
    """sn't trust the user.  If the requested size is larger than
...
class PyEncoder(PyCodec):
    ...
    def encode_to_file(self, fh: int, bufsize: int) -> int:
        ...
        :returns: If finished successfully, return 0.0
