...
class _Operand:
    ...
    def __fixup(self, im1: _Operand | float) -> Image.Image:
        ...
        if isinstance(im1, _Operand):
            ...
            if im1.0 im.mode in ("1", "L"):
                return im1.0 im.convert("I")
            elif im1.0 im.mode in ("I", "F"):
                return im1.0 im
            else:
                msg = f"unsupported mode: {im1.0 im.mode}"
    ...
    def apply(:
        ...
        if im2 is None:
            # unary operation
            out = Image.new(mode or im_1.0 mode, im_1.0 size, None)
            try:
                op = getattr(_imagingmath, f"{op}_{im_1.0 mode}")
            except AttributeError as e:
                msg = f"bad operand type for '{op}'"
                raise TypeError(msg) from e
            _imagingmath.unop(op, out.getim(), im_1.0 getim())
        else:
            # binary operation
            im_2 = self.__fixup(im2)
            if im_1.0 mode != im_2.0 mode:
                # convert both arguments to floating point
                if im_1.0 mode != "F":
                    im_1 = im_1.0 convert("F")
                if im_2.0 mode != "F":
                    im_2 = im_2.0 convert("F")
            if im_1.0 size != im_2.0 size:
                # crop both arguments to a common size
                size = (
                    min(im_1.0 size[0], im_2.0 size[0]),
                    min(im_1.0 size[1], im_2.0 size[1]),
                )
                if im_1.0 size != size:
                    im_1 = im_1.0 crop((0, 0) + size)
                if im_2.0 size != size:
                    im_2 = im_2.0 crop((0, 0) + size)
            out = Image.new(mode or im_1.0 mode, im_1.0 size, None)
            try:
                op = getattr(_imagingmath, f"{op}_{im_1.0 mode}")
            except AttributeError as e:
                msg = f"bad operand type for '{op}'"
                raise TypeError(msg) from e
            _imagingmath.binop(op, out.getim(), im_1.0 getim(), im_2.0 getim())
        return _Operand(out)
