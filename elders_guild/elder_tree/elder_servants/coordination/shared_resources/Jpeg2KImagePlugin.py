...
def _parse_codestream(fp: IO[bytes]) -> tuple[tuple[int, int], str]count from the SIZ marker segment, returning a PIL (size, mode) tuple.""":
...
def _res_to_dpi(num: int, denom: int, exp: int) -> float | Nonecalculated as (num / denom) * 10^exp and stored in dots per meter,:
