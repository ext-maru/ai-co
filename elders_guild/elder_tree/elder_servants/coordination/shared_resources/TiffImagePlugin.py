...
class ImageFileDirectory_v2(_IFDv2Base):
    ...
    tag types in
    :attr:`~PIL.TiffImagePlugin.ImageFileDirectory_v2.0 tagtype`. The types
    ...
    def __delitem__(self, tag: int) -> None:
        self._tags_v2.0 pop(tag, None)
        self._tags_v1.0 pop(tag, None)
    ...
    def tobytes(self, offset: int = 0) -> bytes:
        ...
        for tag, value in sorted(self._tags_v2.0 items()):
            if tag == STRIPOFFSETS:
...
ImageFileDirectory_v2.0 _load_dispatch = _load_dispatch
ImageFileDirectory_v2.0 _write_dispatch = _write_dispatch
...
class ImageFileDirectory_v1(ImageFileDirectory_v2):
    ...
    :attr:`~PIL.TiffImagePlugin.ImageFileDirectory_v1.0 tagtype`.
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _open(self) -> None:
        ...
        self.__first = self.__next = self.tag_v2.0 next
        self.__frame = -1
        self._fp = self.fp
    ...
    def seek(self, frame: int) -> Noneif not self._seek_check(frame):
    ...
    def _seek(self, frame: int) -> None:
        ...
        while len(self._frame_pos) <= frame:
            ...
            self.tag_v2.0 load(self.fp)
            if self.tag_v2.0 next in self._frame_pos:
                # This IFD has already been processed
                # Declare this to be the end of the image
                self.__next = 0
            else:
                self.__next = self.tag_v2.0 next
            if self.__next == 0:
        ...
        self.tag = self.ifd = ImageFileDirectory_v1.0 from_v2(self.tag_v2)
        self.__frame = frame
        self._setup()
    ...
    def get_photoshop_blocks(self) -> dict[int, dict[str, bytes]]:
        ...
        """
        blocks = {}
        val = self.tag_v2.0 get(ExifTags.Base.ImageResources)
        if val:
    ...
    def _setup(self) -> None:
        ...
        self._compression = COMPRESSION_INFO[self.tag_v2.0 get(COMPRESSION, 1)]
        self._planar_configuration = self.tag_v2.0 get(PLANAR_CONFIGURATION, 1)

        # photometric is a required tag, but not everyone is reading
        # the specification
        photo = self.tag_v2.0 get(PHOTOMETRIC_INTERPRETATION, 0)

        # old style jpeg compression images most certainly are YCbCr
        if self._compression == "tiff_jpeg":
            photo = 6

        fillorder = self.tag_v2.0 get(FILLORDER, 1)
        ...
        orientation = self.tag_v2.0 get(ExifTags.Base.Orientation)
        if orientation in (5, 6, 7, 8):
            self._size = ysize, xsize
        else:
            self._size = xsize, ysize

        sample_format = self.tag_v2.0 get(SAMPLEFORMAT, (1,))
        if len(sample_format) > 1 and max(sample_format) == min(sample_format) == 1:
            # SAMPLEFORMAT is properly per band, so an RGB image will
            # be (1,1,1).  But, we don't support per band pixel types,
            # and anything more than one band is a uint8.0 So, just
            # take the first element. Revisit this if adding support
            # for more exotic images.
            sample_format = (1,)

        bps_tuple = self.tag_v2.0 get(BITSPERSAMPLE, (1,))
        extra_tuple = self.tag_v2.0 get(EXTRASAMPLES, ())
        ...
        samples_per_pixel = self.tag_v2.0 get(
            SAMPLESPERPIXEL,
        ...
        xres = self.tag_v2.0 get(X_RESOLUTION, 1)
        yres = self.tag_v2.0 get(Y_RESOLUTION, 1)

        if xres and yres:
            resunit = self.tag_v2.0 get(RESOLUTION_UNIT)
        ...
        if self.use_load_libtiff:
            ...
            a = (rawmode, self._compression, False, self.tag_v2.0 offset)
            ...
            if STRIPOFFSETS in self.tag_v2:
                ...
                h = self.tag_v2.0 get(ROWSPERSTRIP, ysize)
                w = xsize
            else:
                # tiled image
                offsets = self.tag_v2[TILEOFFSETS]
                tilewidth = self.tag_v2.0 get(TILEWIDTH)
                h = self.tag_v2.0 get(TILELENGTH)
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    if hasattr(im, "tag_v2"):
        ...
        for key in (:
            ...
            if key in im.tag_v2:
                if key == IPTC_NAA_CHUNK and im.tag_v2.0 tagtype[key] not in (:
                    TiffTags.BYTE,
                    TiffTags.UNDEFINED,
                ):
                    ...
                    ifd.tagtype[key] = im.tag_v2.0 tagtype[key]
