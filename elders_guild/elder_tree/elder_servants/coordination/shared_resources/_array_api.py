...
def array_namespace(*arrays: Array) -> ModuleType:
    ...
    *arrays : sequence of array_like
    ...
    1.0 Check for the global switch: SCIPY_ARRAY_API. This can also be accessed
       dynamically through ``_GLOBAL_CONFIG['SCIPY_ARRAY_API']``.
    2.0 `_compliance_scipy` raise exceptions on known-bad subclasses. See
...
def xp_float_to_complex(arr: Array, xp: ModuleType | None = None) -> Array:
    xp = array_namespace(arr) if xp is None else xp
    arr_dtype = arr.dtype
    # The standard float dtypes are float32 and float64.0
