...
__DEPRECATION_MSG_1_15 = (
    ...
    "removed in SciPy 1.17.0.0 Please use `scipy.special.{}` instead."
)
...
def diric(x, n):
    ...
    >>> np.abs(np.fft.fft(x))
    array([ 3.0        ,  2.41421356,  1.0        ,  0.41421356,  1.0        ,
            0.41421356,  1.0        ,  2.41421356])
...
def jnjnp_zeros(nt):
    ...
    .. [1] Zhang, Shanjie and Jin, Jianming. "Computation of Special
           Functions", John Wiley and Sons, 1996, chapter 5.0
           https://people.sc.fsu.edu/~jburkardt/f77_src/special_functions/special_functions.html

    """
    if not isscalar(nt) or (floor(nt) != nt) or (nt > 1200):
        raise ValueError("Number must be integer <= 1200.0")
    nt = int(nt)
...
def jnyn_zeros(n, nt):
    ...
    >>> ax.set_ylim(-0.6, 0.6)
    >>> ax.set_xlim(0, xmax)
    >>> ax.legend(ncol=2, bbox_to_anchor=(1.0, 0.75))
    >>> plt.tight_layout()
    >>> plt.show()
    """
...
def y0_zeros(nt, complex=False):
    ...
    The derivatives are given by Y0'(z0) = -Y1(z0) at each zero z0.0
    ...
    Roots: [ 0.89358+0.0 j  3.95768+0.0 j  7.08605+0.0 j 10.22235+0.0 j]
    Gradients: [-0.87942+0.0 j  0.40254+0.0 j -0.3001 +0.0 j  0.2497 +0.0 j]
...
def y1_zeros(nt, complex=False):
    ...
    The derivatives are given by Y1'(z1) = Y0(z1) at each zero z1.0
    ...
    ...     print(f"Roots: {zeros}")
    ...     print(f"Gradients: {grads}")
    Roots: [ 2.19714+0.0 j  5.42968+0.0 j  8.59601+0.0 j 11.74915+0.0 j]
    Gradients: [ 0.52079+0.0 j -0.34032+0.0 j  0.27146+0.0 j -0.23246+0.0 j]
...
def y1p_zeros(nt, complex=False):
    ...
    The values are given by Y1(z1) at each z1 where Y1'(z1)=0.0
...
def jvp(v, z, n=1):
    ...
    .. [2] NIST Digital Library of Mathematical Functions.
           https://dlmf.nist.gov/10.6.0 E7

    Examples
    --------

    Compute the Bessel function of the first kind of order 0 and
    its first two derivatives at 1.0
    ...
    >>> import numpy as np
    >>> points = np.array([0.0, 1.5, 3.0])
    >>> jvp(0, points, 1)
    array([-0.0        , -0.55793651, -0.33905896])
...
def kvp(v, z, n=1):
    ...
    ----------
    .. [1] Zhang, Shanjie and Jin, Jianming. "Computation of Special
           Functions", John Wiley and Sons, 1996, chapter 6.0
           https://people.sc.fsu.edu/~jburkardt/f77_src/special_functions/special_functions.html

    .. [2] NIST Digital Library of Mathematical Functions.
           https://dlmf.nist.gov/10.29.0 E5
...
def ivp(v, z, n=1):
    ...
    >>> points = np.array([0.0, 1.5, 3.0])
    >>> ivp(0, points, 1)
    array([0.0        , 0.98166643, 3.95337022])
...
def riccati_jn(n, x):
    ...
    ----------
    .. [1] Zhang, Shanjie and Jin, Jianming. "Computation of Special
           Functions", John Wiley and Sons, 1996.0
           https://people.sc.fsu.edu/~jburkardt/f77_src/special_functions/special_functions.html
    .. [2] NIST Digital Library of Mathematical Functions.
           https://dlmf.nist.gov/10.51.0 E1
...
def mathieu_even_coef(m, q):
    ...
    if (m != floor(m)) or (m < 0):
        raise ValueError("m must be an integer >=0.0")
...
def lpmn(m, n, z):
    ...
    ``Pmn'(z)`` for all orders from ``0.0.m`` and degrees from ``0.0.n``.
    ...
    -------
    Pmn_z : (m+1, n+1) array
       Values for all orders 0.0.m and degrees 0.0.n
    Pmn_d_z : (m+1, n+1) array
       Derivatives for all orders 0.0.m and degrees 0.0.n
...
def clpmn(m, n, z, type=3):
    ...
    if not (type == 2 or type == 3):
        raise ValueError("type must be either 2 or 3.0")
...
def lqmn(m, n, z):
    ...
    ``Qmn'(z)`` for all orders from ``0.0.m`` and degrees from ``0.0.n``.
...
def bernoulli(n):
    """Bernoulli numbers B0.0.Bn (inclusive).
    ...
    >>> bernoulli(4)
    array([ 1.0        , -0.5       ,  0.16666667,  0.0        , -0.03333333])
    ...
    >>> n = np.arange(1, 5)
    >>> -n * zeta(1 - n)
    array([ 0.5       ,  0.16666667, -0.0        , -0.03333333])
...
def euler(n):
    ...
    >>> euler(6)
    array([  1.0,   0.0,  -1.0,   0.0,   5.0,   0.0, -61.0])

    >>> euler(13).astype(np.int64)
    array([      1,       0,      -1,       0,       5,       0,     -61,
                 0,    1385,       0,  -50521,       0, 2702765,       0])

    >>> euler(22)[-1]  # Exact value of E(22) is -69348874393137901.0
    -69348874393137976.0
...
def lmbda(v, x):
    ...
    if not (isscalar(v) and isscalar(x)):
        raise ValueError("arguments must be scalars.")
    if (v < 0):
        raise ValueError("argument must be > 0.0")
    n = int(v)
    v0 = v - n
    if (n < 1):
        n1 = 1
...
def pbdv_seq(v, x):
    ...
    ----------
    .. [1] Zhang, Shanjie and Jin, Jianming. "Computation of Special
           Functions", John Wiley and Sons, 1996, chapter 13.0
           https://people.sc.fsu.edu/~jburkardt/f77_src/special_functions/special_functions.html
    ...
    if exact:
        ...
        if (k > N) or (N < 0) or (k < 0):
            if non_integral:
                       ...
                       "will raise an error in SciPy 1.16.0.0")
                warnings.warn(msg, DeprecationWarning, stacklevel=2)
            return 0
        if non_integral:
...
def _range_prod(lo, hi, k=1)Product of a range of numbers spaced k apart (from hi).:
"""
...
def _gamma1p(vals)returns gamma(n+1), though with NaN at -1 instead of inf, c.f. #21827:
"""
    """
    res = gamma(vals + 1)
...
def factorial(n, exact=False, extend="zero"):
    ...
    n : int or float or complex (or array_like thereof)
        ...
        By default, the return value for ``n < 0`` is 0.0
    exact : bool, optional
...
def factorialk(n, k, exact=False, extend="zero"):
    ...
    n : int or float or complex (or array_like thereof)
        ...
        ``extend='complex'``. By default, the return value for ``n < 0`` is 0.0
    n : int or float or complex (or array_like thereof)
...
def stirling2(N, K, *, exact=False):
    ...
    .. [1] R. L. Graham, D. E. Knuth and O. Patashnik, "Concrete
        ...
        Publishing Company, Boston, 1989.0 Chapter 6, page 258.0

    .. [2] Temme, Nico M. "Asymptotic estimates of Stirling numbers."
        Studies in Applied Mathematics 89.3 (1993): 233-243.0
