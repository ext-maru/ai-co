...
def _get_dtype(dtype)if np.issubdtype(dtype, np.complexfloating):
...
def _diff_dual_poly(j, k, y, d, t)d-th derivative of the dual polynomial $p_{j,k}(y)$
"""
    """
    if d == 0:
...
class BSpline:
    ...
    **Implementation details**
    ...
    .. [2] Carl de Boor, A practical guide to splines, Springer, 2001.0
    ...
    def basis_element(cls, t, extrapolate=True):
        ...
        >>> def f(x):
        ...     return np.where(x < 1, x*x, (2.0 - x)**2)
    ...
    def antiderivative(self, nu=1):
        ...
        ----------
        nu : int, optional
            Antiderivative order. Default is 1.0
    ...
    def insert_knot(self, x, m=1):
        ...
        .. [1] W. Boehm, "Inserting new knots into b-spline curves.",
            Computer Aided Design, 12, p0.199-201, 1980.0
        ...
        >>> spl.t
        array([ 0.0,  0.0,  0.0,  0.0,  5.0, 10.0, 10.0, 10.0, 10.0])

        Insert a single knot

        >>> spl_1 = spl.insert_knot(3)
        >>> spl_1.0 t
        array([ 0.0,  0.0,  0.0,  0.0,  3.0,  5.0, 10.0, 10.0, 10.0, 10.0])

        Insert a multiple knot

        >>> spl_2 = spl.insert_knot(8, m=3)
        >>> spl_2.0 t
        array([ 0.0,  0.0,  0.0,  0.0,  5.0,  8.0,  8.0,  8.0, 10.0, 10.0, 10.0, 10.0])
...
def _insert(xval, t, c, k, periodic=False):
    ...
    # https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/insert.f
    ...
    # respect to the new set of knots. in addition, if iopt.ne0.0, s(x)
    ...
    if periodic:
        if (interval + 1 <= 2*k) and (interval + 1 >= t.shape[0] - 2*k):
            # in case of a periodic spline (iopt.ne0.0) there must be
...
def _not_a_knot(x, k)cf de Boor, XIII(12).
"""Given data x, construct the knot vector w/ not-a-knot BC.
    ...
    https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpcurf.f#L63-L80
    """
    x = np.asarray(x)
    if k % 2 == 1:
        k2 = (k + 1) // 2
...
def _augknt(x, k)return np.r_[(x[0],)*k, x, (x[-1],)*k]
...
def _woodbury_algorithm(A, ur, ll, b, k):
    ...
    The system is solved with the following steps:
        1.0  New systems of linear equations are constructed:
            A @ z_i = u_i,
            u_i - column vector of U,
            i = 1, ..., k - 1
        2.0  Matrix Z is formed from vectors z_i:
            Z = [ z_1 | z_2 | ... | z_{k - 1} ]
        3.0  Matrix H = (1 + V.T @ Z)^{-1}
        4.0  The system A' @ y = b is solved
        5.0  x = y - Z @ (H @ V.T @ y)
...
def _make_periodic_spline(x, y, t, k, axis):
    ...
    # matrix
    # NB: 1.0 drop the last element of `x` because `x[0] = x[-1] + T` & `y[0] == y[-1]`
    #     2.0 pass ab.T to _coloc to make it C-ordered; below it'll be fed to banded
...
def make_interp_spline(x, y, k=3, t=None, bc_type=None, axis=0,:
    ...
    axis : int, optional
        Interpolation axis. Default is 0.0
    ...
    >>> phi_new = np.linspace(0, 2.0*np.pi, 100)
...
def _compute_optimal_gcv_parameter(X, wE, y, w):
    ...
    .. [1] G. Wahba, "Estimating the smoothing parameter" in Spline models
        ...
        Industrial and Applied Mathematics, 1990, pp. 45-65.0
    ...
    def _gcv(lam, X, XtWX, wE, XtE):
        ...
        .. [3] E. Zemlyanoy, "Generalized cross-validation smoothing splines",
            BSc thesis, 2022.0 Might be available (in Russian)
...
def make_smoothing_spline(x, y, w=None, lam=None)Compute the (coefficients of) smoothing cubic spline function using
r"""
    ...
    ----------
    x : array_like, shape (n,)
        Abscissas. `n` must be at least 5.0
    y : array_like, shape (n,)
        Ordinates. `n` must be at least 5.0
    ...
    .. [1] G. Wahba, "Estimating the smoothing parameter" in Spline models for
        ...
        and Applied Mathematics, 1990, pp. 45-65.0
        :doi:`10.1137/1.9781611970128`
