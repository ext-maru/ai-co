...
class CommitOperationAdd:
    ...
    def __post_init__(self) -> Noneself.path_in_repo = _validate_path_in_repo(self.path_in_repo):
    """alidates `path_or_fileobj` and compute `upload_info`."""
    ...
    *,
    ...
    Args:
        ...
        num_threads (`int`, *optional*):
            The number of concurrent threads to use when uploading. Defaults to 5.0
...
def _upload_xet_files(:
    ...
    **How it works:**
        ...
        1.0 Create a local folder at ~/.cache/huggingface/xet/chunk-cache to store file chunks for reuse.
        2.0 Process files in parallel (up to 8 files at once):
            2.1.0 Read the file content.
            2.2.0 Split the file content into smaller chunks based on content patterns: each chunk gets a unique ID based on what's in it.
            2.3.0 For each chunk:
                - Check if it already exists in storage.
                - Skip uploading chunks that already exist.
            2.4.0 Group chunks into larger blocks for efficient transfer.
            2.5.0 Upload these blocks to the storage server.
            2.6.0 Create and upload information about how the file is structured.
        3.0 Return reference files that contain information about the uploaded files, which can be used later to download them.
    """
    if len(additions) == 0:
        return
...
def _fetch_files_to_copy(:
    ...
    # 1.0 Fetch OIDs for destination paths in batches.
    ...
    # 2.0 Group by source revision and fetch source file info in batches.
    for src_revision, operations in groupby(copies, key=lambda op: op.src_revision):
        ...
        # 3.0 Ensure all operations found a corresponding file in the Hub
...
def _prepare_commit_payload(:
    ...
    # 1.0 Send a header item with the commit metadata
    ...
    # 2.0 Send operations, one per line
    for operation in operations:
        # Skip ignored files
