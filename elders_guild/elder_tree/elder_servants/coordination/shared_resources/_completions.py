...
class ChatCompletionStream(Generic[ResponseFormatT]):
    ...
    def until_done(self) -> Selfconsume_sync_iterator(self):
    """locks until the stream has been consumed."""
        return self
...
class AsyncChatCompletionStream(Generic[ResponseFormatT]):
    ...
    async def until_done(self) -> Selfawait consume_async_iterator(self):
    """locks until the stream has been consumed."""
        return self
...
class ChatCompletionStreamState(Generic[ResponseFormatT]):
    ...
    def handle_chunk(self, chunk: ChatCompletionChunk) -> Iterable[ChatCompletionStreamEvent[ResponseFormatT]]self.__current_completion_snapshot = self._accumulate_chunk(chunk):
    """ccumulate a new chunk into the snapshot and returns an iterable of events to yield."""
