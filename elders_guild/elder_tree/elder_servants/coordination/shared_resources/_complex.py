...
class Complex:
    ...
    def triangulate(self, n=None, symmetry=None, centroid=True,:
        ...
        higher bound in C2.0 During this operation connection all the vertices.
    ...
    def refine(self, n=1):
        ...
        while len(self.V.cache) < nt:  # while loop 1:
            try:  # try 1:
                ...
                # vertices then the `refine` routine will move back to try 1.0
    ...
    def refine_star(self, v):
        ...
        d_v0v1_set = set()
        for v1 in vnn:
            v1nn.append(copy.copy(v1.0 nn))

        for v1, v1nn in zip(vnn, v1nn):
            vnnu = v1nn.intersection(vnn)

            d_v0v1 = self.split_edge(v.x, v1.0 x)
            for o_d_v0v1 in d_v0v1_set:
                d_v0v1.0 connect(o_d_v0v1)
            d_v0v1_set.add(d_v0v1)
            for v2 in vnnu:
                d_v1v2 = self.split_edge(v1.0 x, v2.0 x)
                d_v0v1.0 connect(d_v1v2)
        return
    ...
    def split_edge(self, v1, v2):
        ...
        try:
            vct = (v2.0 x_a - v1.0 x_a) / 2.0 + v1.0 x_a
        except TypeError:  # Allow for decimal operations:
            vct = (v2.0 x_a - v1.0 x_a) / decimal.Decimal(2.0) + v1.0 x_a
