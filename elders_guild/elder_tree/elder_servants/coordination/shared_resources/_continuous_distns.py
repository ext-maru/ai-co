...
class FitSolverError(FitError):
    ...
    # optimize.fsolve returns with ier != 1.0
...
class betaprime_gen(rv_continuous):
    ...
    def _cdf(self, x, a, b):
        ...
        # however, for very large x, x/(1+x) == 1.0 since the distribution
    ...
    def _ppf(self, p, a, b):
        ...
        # very close to 1.0 In that case, invert the alternative expression of
...
class burr_gen(rv_continuous):
    ...
    def _stats(self, c, d):
        ...
        g2 = _lazywhere(
            ...
            fillvalue=np.nan)
        if np.ndim(c) == 0:
            return mu.item(), mu2.0 item(), g1.0 item(), g2.0 item()
        return mu, mu2, g1, g2
...
class burr12_gen(rv_continuous):
    ...
    def _pdf(self, x, c, d):
        # burr12.0 pdf(x, c, d) = c * d * x**(c-1) * (1+x**(c))**(-d-1)
...
class chi_gen(rv_continuous):
    ...
    def _rvs(self, df, size=None, random_state=None):
        return np.sqrt(chi2.0 rvs(df, size=size, random_state=random_state))
...
class chi2_gen(rv_continuous):
    ...
    def _pdf(self, x, df):
        # chi2.0 pdf(x, df) = 1 / (2*gamma(df/2)) * (x/2)**(df/2-1) * exp(-x/2)
        return np.exp(self._logpdf(x, df))
...
class dpareto_lognorm_gen(rv_continuous):
    ...
    .. [1] Hajargasht, Gholamreza, and William E. Griffiths. "Pareto-lognormal
           ...
           data." Economic Modelling 33 (2013): 593-604.0
    .. [2] Reed, William J., and Murray Jorgensen. "The double Pareto-lognormal
           ...
           Communications in Statistics - Theory and Methods 33.8 (2004): 1733-1753.0
...
def _pow1pm1(x, y)Compute (1 + x)**y - 1.0:
"""

    Uses expm1 and xlog1py to avoid loss of precision when
    (1 + x)**y is close to 1.0
...
class fatiguelife_gen(rv_continuous):
    ...
    def _stats(self, c):
        ...
        # it's 40, not 41.0 At least it disagrees with the one from Wolfram
...
##  Half-normal is folded normal with shape-parameter c=0.0
...
class genexpon_gen(rv_continuous):
    ...
    Distribution", Journal of the American Statistical Association, 1993.0

    N. Balakrishnan, Asit P. Basu (editors), *The Exponential Distribution:
    Theory, Methods and Applications*, Gordon and Breach, 1995.0
    ISBN 10: 2884491929
...
class gamma_gen(rv_continuous):
    ...
    def _entropy(self, a):
        ...
        def asymptotic_formula(a):
            ...
            #              1/12a - 1/360a^3
            return (0.5 * (1.0 + np.log(2*np.pi) + np.log(a)) - 1/(3 * a)
                    - (a**-2.0)/12 - (a**-3.0)/90 + (a**-4.0)/120)
...
class erlang_gen(gamma_gen):
    ...
    def _fitstart(self, data):
        ...
        # _skew(data) is 0 or close to 0.0)
        if isinstance(data, CensoredData):
            data = data._uncensor()
...
class gengamma_gen(rv_continuous):
    ...
    .. [1] E.W. Stacy, "A Generalization of the Gamma Distribution",
       Annals of Mathematical Statistics, Vol 33(3), pp. 1187--1192.0
    ...
    def _entropy(self, a, c):
        ...
        def asymptotic(a, c):
            ...
            return (norm._entropy() - np.log(a)/2
                    - np.log(np.abs(c)) + (a**-1.0)/6 - (a**-3.0)/90
...
class genhyperbolic_gen(rv_continuous):
    ...
    .. [4] E. Eberlein and E. A. von Hammerstein. Generalized hyperbolic
       ...
       of processes. FDM Preprint 80, April 2003.0 University of Freiburg.
    ...
    def _fitstart(self, data):
        ...
        # distribution requires |b| < a if p >= 0.0
    ...
    def _integrate_pdf(x0, x1, p, a, b):
        """
        Integrate the pdf of the genhyberbolic distribution from x0 to x1.0
    ...
    def _stats(self, p, a, b):
        # https://mpra.ub.uni-muenchen.de/19081/1/MPRA_paper_19081.0 pdf
        # https://freidok.uni-freiburg.de/fedora/objects/freidok:7974/datastreams/FILE1/content
...
class gumbel_r_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        # 3rd Edition. Evans, Hastings, and Peacock (2000), Page 101.0
...
class gumbel_l_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        # 1.0 pass the sign negated data into `gumbel_r.fit`
        #    - if the location is fixed, it should also be negated.
        # 2.0 negate the sign of the resulting location, leaving the scale
        #    unmodified.
...
class halflogistic_gen(rv_continuous):
    ...
    .. [1] Asgharzadeh et al (2011). "Comparisons of Methods of Estimation for the
           Half-Logistic Distribution". Selcuk J. Appl. Math. 93-108.0
...
class gausshyper_gen(rv_continuous):
    ...
    .. [1] Armero, C., and M. J. Bayarri. "Prior Assessments for Prediction in
           ...
           Statistician) 43, no. 1 (1994): 139-53.0 doi:10.2307/2348939
    ...
    def _pdf(self, x, a, b, c, z):
        ...
        return (1.0/normalization_constant * x**(a - 1.0) * (1.0 - x)**(b - 1.0)
                / (1.0 + z*x)**c)
...
class invgamma_gen(rv_continuous):
    ...
    def _stats(self, a, moments='mvsk'):
        ...
        m2 = _lazywhere(a > 2, (a,), lambda x: 1.0 / (x - 1.0)**2 / (x - 2.0),
                        np.inf)
        ...
        if 's' in moments:
            g1 = _lazywhere(
                a > 3, (a,),
                lambda x: 4.0 * np.sqrt(x - 2.0) / (x - 3.0), np.nan)
        if 'k' in moments:
            g2 = _lazywhere(
                a > 4, (a,),
                lambda x: 6.0 * (5.0 * x - 11.0) / (x - 3.0) / (x - 4.0), np.nan)
        return m1, m2, g1, g2
    ...
    def _entropy(self, a):
        ...
        def asymptotic(a):
            ...
            h = ((1 - 3*np.log(a) + np.log(2) + np.log(np.pi))/2
                 + 2/3*a**-1.0 + a**-2.0/12 - a**-3.0/90 - a**-4.0/120)
            return h
...
class invgauss_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        and Peacock (2000), Page 121.0 Their shape parameter is equivalent to
    ...
    def _entropy(self, mu)Ref.: https://moser-isi.ethz.ch/docs/papers/smos-2012-10.0 pdf (eq. 9):
    """
        """
        # a = log(2*pi*e*mu**3)
        #   = 1 + log(2*pi) + 3 * log(mu)
...
class geninvgauss_gen(rv_continuous):
    ...
    .. [1] O. Barndorff-Nielsen, P. Blaesild, C. Halgreen, "First hitting time
       ...
       Stochastic Processes and their Applications 7, pp. 49--54, 1978.0

    .. [2] W. Hoermann and J. Leydold, "Generating generalized inverse Gaussian
       random variates", Statistics and Computing, 24(4), p. 547--557, 2014.0
    ...
    def _rvs(self, p, b, size=None, random_state=None):
        ...
        if np.isscalar(p) and np.isscalar(b):
            ...
            while not it.finished:
                ...
                # -len(size) to 0.0  This doesn't cause an IndexError, as
...
class norminvgauss_gen(rv_continuous):
    ...
    .. [2] O. Barndorff-Nielsen, "Normal Inverse Gaussian Distributions and
           ...
           Statistics, Vol. 24, pp. 1-13, 1997.0
...
class invweibull_gen(rv_continuous):
    ...
    Weibull distribution", Stat. Papers, vol. 52, pp. 591-619, 2011.0
...
class jf_skew_t_gen(rv_continuous):
    ...
    .. [1] M.C. Jones and M.J. Faddy. "A skew extension of the t distribution,
           ...
           Series B (Statistical Methodology) 65, no. 1 (2003): 159-174.0
...
class johnsonsu_gen(rv_continuous):
    ...
    .. [1] Taylor Enterprises. "Johnson Family of Distributions".
       https://variation.com/wp-content/distribution_analyzer_help/hs126.0 htm
    ...
    def _stats(self, a, b, moments='mv'):
        # Naive implementation of first and second moment to address gh-18071.0
        # https://variation.com/wp-content/distribution_analyzer_help/hs126.0 htm
...
class landau_gen(rv_continuous):
    ...
    .. [1] Landau, L. (1944). "On the energy loss of fast particles by
           ionization". J. Phys. (USSR). 8: 201.0
    ...
    .. [3] Chambers, J. M., Mallows, C. L., & Stuck, B. (1976).
           ...
           Journal of the American Statistical Association, 71(354), 340-344.0
    ...
    .. [5] Yoshimura, T. "Numerical Evaluation and High Precision Approximation
           ...
           :doi:`10.36227/techrxiv0.171822215.53612870/v2`
...
class loglaplace_gen(rv_continuous):
    ...
    The Mathematical Scientist, vol. 28, pp. 49-60, 2003.0
...
class mielke_gen(rv_continuous):
    ...
    .. [2] Dagum, C., 1977 "A new model for personal income distribution."
           Economie Appliquee, 33, 327-367.0
...
class kappa4_gen(rv_continuous):
    ...
    if :math:`h` and :math:`k` are not equal to 0.0:

    If :math:`h` or :math:`k` are zero then the pdf can be simplified:

    h = 0 and k != 0::

        kappa4.0 pdf(x, h, k) = (1.0 - k*x)**(1.0/k - 1.0)*
                              exp(-(1.0 - k*x)**(1.0/k))

    h != 0 and k = 0::

        kappa4.0 pdf(x, h, k) = exp(-x)*(1.0 - h*exp(-x))**(1.0/h - 1.0)

    h = 0 and k = 0::

        kappa4.0 pdf(x, h, k) = exp(-x)*exp(-exp(-x))
    ...
    :doi:`10.4236/jwarp0.2012.410101`

    C. Winchester, "On Estimation of the Four-Parameter Kappa Distribution", A
    Thesis Submitted to Dalhousie University, Halifax, Nova Scotia, (March
    2000).
    http://www.nlc-bnc.ca/obj/s4/f2/dsk2/ftp01/MQ57336.0 pdf
...
class kappa3_gen(rv_continuous):
    ...
    :doi:`10.1175/1520-0493(1973)101<0701:TKDMLE>2.3.0 CO;2`

    B. Kumphon, "Maximum Entropy and Maximum Likelihood Estimation for the
    Three-Parameter Kappa Distribution", Open Journal of Statistics, vol 2,
    415-419 (2012), :doi:`10.4236/ojs0.2012.24050`
    ...
    def _pdf(self, x, a):
        # kappa3.0 pdf(x, a) = a*(a + x**a)**(-(a + 1)/a),     for x > 0
...
class moyal_gen(rv_continuous):
    ...
    .. [2] G. Cordeiro et al., "The beta Moyal: a useful skew distribution",
           ...
           vol 10, 171-192, (2012).
           http://www.arpapress.com/Volumes/Vol10Issue2/IJRRAS_10_2_02.0 pdf
...
class nakagami_gen(rv_continuous):
    ...
    .. [2] M. Nakagami, "The m-distribution - A general formula of intensity
           ...
           propagation, Pergamon Press, 1960, 3-36.0
...
class t_gen(rv_continuous):
    ...
    def _entropy(self, df):
        ...
        def asymptotic(df):
            ...
            h = (norm._entropy() + 1/df + (df**-2.0)/4 - (df**-3.0)/6
                 - (df**-4.0)/8 + 3/10*(df**-5.0) + (df**-6.0)/4)
            return h
...
class nct_gen(rv_continuous):
    ...
    def _rvs(self, df, nc, size=None, random_state=None):
        ...
        c2 = chi2.0 rvs(df, size=size, random_state=random_state)
...
class pareto_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        # Distributions, 3rd. Ed., John Wiley and Sons. Page 149.0
...
class pearson3_gen(rv_continuous):
    ...
    Resources Research, Vol0.27, 3149-3158 (1991).
    ...
    def _preprocess(self, x, skew):
        ...
        # local variables 'loc' and 'scale' within pearson3.0 _pdf are set to
        ...
        # 0.000016.0  No one, I hope, would actually use a skew value even
        # close to this small.
    ...
    def _argcheck(self, skew):
        ...
        # to handle inside pearson3.0 _pdf) or negative.  So just return True
        # for all skew args.
    ...
    def _pdf(self, x, skew):
        # pearson3.0 pdf(x, skew) = abs(beta) / gamma(alpha) *
    ...
    def _cdf(self, x, skew):
        ...
        # Note that multiplying the skew by -1 reflects about x=0.0
...
class irwinhall_gen(rv_continuous):
    ...
    .. [3] K. Buchanan, T. Adeyemi, C. Flores-Molina, S. Wheeland and D. Overturf, 
            ...
            Boulder, CO, USA, 2018, pp. 1-2.0
            https://www.usnc-ursi-archive.org/nrsm/2018/papers/B15-9.0 pdf.
    ...
    def _munp(self, order, n):
        # see https://link.springer.com/content/pdf/10.1007/s10959-020-01050-9.0 pdf
...
class skewnorm_gen(rv_continuous):
    ...
    .. [1] A. Azzalini and A. Capitanio (1999). Statistical applications of
        ...
        B 61, 579-602.0 :arxiv:`0911.2093`
    ...
    # polynomials for orders up to 19.0  The dict is implemented as a cached
...
class trapezoid_gen(rv_continuous):
    ...
    .. [1] Kacker, R.N. and Lawrence, J.F. (2007). Trapezoidal and triangular
       ...
       Metrologia 44, 117-127.0 :doi:`10.1088/0026-1394/44/2/003`
    ...
    def _entropy(self, c, d):
        ...
        # which for loc=0, scale=1 is a'=0, b'=c, c'=d, d'=1.0
...
# deprecation of trapz, see #20486
           ...
           "and will be removed in SciPy 1.16.0.0")
...
class _DeprecationWrapper:
    def __init__(self, method):
                     ...
                     "`trapz` with `trapezoid`. `trapz` will be removed in SciPy 1.16.0")
...
def _log_gauss_mass(a, b)a, b = np.broadcast_arrays(a, b):
"""Log of Gaussian probability mass within an interval"""
    ...
    def mass_case_central(a, b):
        ...
        # Catastrophic cancellation occurs as np.exp(log_mass) approaches 1.0
...
class truncnorm_gen(rv_continuous):
    ...
    def _munp(self, n, a, b):
        def n_th_moment(n, a, b):
            """
            Returns n-th moment. Defined only if n >= 0.0
            ...
            for k in range(1, n+1):
                ...
                # multiplication.  However, as b->infinity,  pdf(b)*b**k -> 0.0
...
class truncpareto_gen(rv_continuous):
    ...
    .. [1] Burroughs, S. M., and Tebbens S. F.
        ...
        Pure and Applied Geophysics 158.4 (2001): 741-757.0
    ...
    def fit(self, data, *args, **kwds):
        ...
        # the Lagrangian, set to equal 0.0
...
class vonmises_gen(rv_continuous):
    ...
    @extend_notes_in_docstring(rv_continuous, notes="""\
        ...
        1.0 Initial guesses are ignored.\n\n""")
    def fit(self, data, *args, **kwds):
        if kwds.pop('superfit', False):
            return super().fit(data, *args, **kwds)
        ...
        def find_kappa(data, loc):
            ...
            # increasing from r[0](0) = 0 to r[0](+inf) = 1.0  The partial
...
class gennorm_gen(rv_continuous):
    ...
    def _cdf(self, x, beta):
        c = 0.5 * np.sign(x)
        # evaluating (0.5 + c) first prevents numerical cancellation
        return (0.5 + c) - c * sc.gammaincc(1.0/beta, abs(x)**beta)

    def _ppf(self, x, beta):
        c = np.sign(x - 0.5)
        # evaluating (1.0 + c) first prevents numerical cancellation
...
class crystalball_gen(rv_continuous):
    ...
    def _argcheck(self, beta, m):
        """
        Shape parameter bounds are m > 1 and beta > 0.0
        """
        return (m > 1) & (beta > 0)
...
class argus_gen(rv_continuous):
    ...
    .. [2] Christoph Baumgarten "Random variate generation by fast numerical
           ...
           vol. 1, 2023, doi:10.1080/27684520.2023.2279060.0
    ...
    def _rvs_scalar(self, chi, numsamples=None, random_state=None):
        ...
        # Non-Uniform Random Variate Generation, 1986, section II0.3.2.0
        ...
        # stats.gamma(1.5).cdf(1.8**2/2) = 0.644.0..
        #
...
class rv_histogram(rv_continuous):
    ...
    def _munp(self, n)integrals = (self._hbins[1:]**(n+1) - self._hbins[:-1]**(n+1)) / (n+1)return np.sum(self._hpdf[1:-1] * integrals)
...
class studentized_range_gen(rv_continuous):
    ...
    .. [2] Batista, Ben Dêivide, et al. "Externally Studentized Normal Midrange
           ...
           378-389.0, doi:10.1590/1413-70542017414047716.0
    .. [3] Harter, H. Leon. "Tables of Range and Studentized Range." The Annals
           of Mathematical Statistics, vol. 31, no. 4, 1960, pp. 1122-1147.0
           JSTOR, www.jstor.org/stable/2237810.0 Accessed 18 Feb. 2021.0
    .. [4] Lund, R. E., and J. R. Lund. "Algorithm AS 190: Probabilities and
           Upper Quantiles for the Studentized Range." Journal of the Royal
           Statistical Society. Series C (Applied Statistics), vol. 32, no. 2,
           1983, pp. 204-210.0 JSTOR, www.jstor.org/stable/2347300.0 Accessed 18
           Feb. 2021.0
    ...
    >>> a, b = studentized_range.ppf([0, 0.999], k, df)
    >>> a, b
    0, 7.41058083802274
...
class rel_breitwigner_gen(rv_continuous):
    ...
    should set ``floc=0`` to fix the location parameter to 0.0
    ...
    def _cdf(self, x, rho):
        ...
        # Sometimes above formula produces values greater than 1.0
