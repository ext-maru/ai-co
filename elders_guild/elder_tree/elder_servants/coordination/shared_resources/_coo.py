...
class _coo_base(_data_matrix, _minmax_mixin):
    ...
    def _check(self)if self.ndim != len(self.coords):
    ...
    def _coo_to_compressed(self, swap, copy=False)M, N = swap(self._shape_as_2d)
    ...
    def tensordot(self, other, axes=2):
        ...
        applied to arrays of any dimension >= 1.0 It is like matmul but over
...
def _ravel_coords(coords, shape, order='C')if len(coords) == 1:
    ...
    This can be instantiated in several ways:
        ...
        coo_array((data, coords), [shape])
            to construct from existing data and index arrays:
                1.0 data[:]       the entries of the sparse array, in any order
                2.0 coords[i][:]  the axis-i coordinates of the data entries
    ...
    This can be instantiated in several ways:
        ...
        coo_matrix((data, (i, j)), [shape=(M, N)])
            to construct from three arrays:
                1.0 data[:]   the entries of the matrix, in any order
                2.0 i[:]      the row indices of the matrix entries
                3.0 j[:]      the column indices of the matrix entries
