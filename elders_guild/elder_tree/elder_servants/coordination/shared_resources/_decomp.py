...
def eig(a, b=None, left=False, right=True, overwrite_a=False,:
    ...
    >>> linalg.eigvals(a)
    array([0.0+1.j, 0.0-1.j])

    >>> b = np.array([[0.0, 1.0], [1.0, 1.0]])
    >>> linalg.eigvals(a, b)
    array([ 1.0+0.j, -1.0+0.j])

    >>> a = np.array([[3.0, 0.0, 0.0], [0.0, 8.0, 0.0], [0.0, 0.0, 7.0]])
    >>> linalg.eigvals(a, homogeneous_eigvals=True)
    array([[3.0+0.j, 8.0+0.j, 7.0+0.j],
...
def eigh(a, b=None, *, lower=True, eigvals_only=False, overwrite_a=False,:
    ...
    Request eigenvalues that are less than 10.0
    ...
    if b is not None:
        ...
        if type not in [1, 2, 3]:
            raise ValueError('"type" keyword only accepts 1, 2, and 3.0')
...
def _check_select(select, select_range, max_ev, max_len)if isinstance(select, str)select = select.lower()
...
def eigvalsh(a, b=None, *, lower=True, overwrite_a=False,:
    ...
    >>> w = eigvalsh(A)
    >>> w
    array([-3.74637491, -0.76263923,  6.8502336, 12.42399079])
...
def eigvalsh_tridiagonal(d, e, select='a', select_range=None,:
    ...
    tol : float
        ...
        lies in an interval of this width. If <= 0.0 (default),
...
def hessenberg(a, calc_q=False, overwrite_a=False, check_finite=True):
    ...
    >>> H
    array([[  2.0        , -11.65843866,   1.42005301,   0.25349066],
           [ -9.94987437,  14.53535354,  -5.31022304,   2.43081618],
           [  0.0        ,  -1.83299243,   0.38969961,  -0.51527034],
           [  0.0        ,   0.0        ,  -3.83189513,   1.7494686]])
...
def cdf2rdf(w, v):
    ...
    >>> w, v = linalg.eig(X)
    >>> w
    array([ 1.0+0.j,  4.0+5.j,  4.0-5.j])
    >>> v
    array([[ 1.0000+0.j     , -0.1906-0.40016j, -0.1906+0.40016j],
           [ 0.0000+0.j     ,  0.0000-0.64788j,  0.0000+0.64788j],
           [ 0.0000+0.j     ,  0.64788+0.j     ,  0.64788-0.j     ]])
    ...
    array([[ 1.0,  0.0,  0.0],
           ...
           [ 0.0, -5.0,  4.0]])
    >>> vr
    array([[ 1.0     ,  0.40016, -0.1906],
           [ 0.0     ,  0.64788,  0.0     ],
           [ 0.0     ,  0.0     ,  0.64788]])

    >>> vr @ wr
    array([[ 1.0     ,  1.69593,  1.9246 ],
           [ 0.0     ,  2.59153,  3.23942],
           [ 0.0     , -3.23942,  2.59153]])
    >>> X @ vr
    array([[ 1.0     ,  1.69593,  1.9246 ],
