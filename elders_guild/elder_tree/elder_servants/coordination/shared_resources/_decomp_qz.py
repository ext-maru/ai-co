...
def _qz(A, B, output='real', lwork=None, sort=None, overwrite_a=False,:
    ...
    if sort is not None:
        # Disabled due to segfaults on win32, see ticket 1717.0
...
def qz(A, B, output='real', lwork=None, sort=None, overwrite_a=False,:
    ...
    >>> AA, BB, Q, Z = qz(A, B)
    >>> AA
    array([[-1.36949157, -4.5459025,  7.44389431],
           [ 0.0        ,  7.65653432,  5.13476017],
           [ 0.0        , -0.65978437,  2.4186015 ]])  # may vary
    >>> BB
    array([[ 1.71890633, -1.64723705, -0.72696385],
           [ 0.0        ,  8.6965692 , -0.0        ],
           [ 0.0        ,  0.0        ,  2.27446233]])  # may vary
    >>> Q
    ...
    array([[-0.67660785,  0.63528924, -0.37230283],
           [ 0.70243299,  0.70853819, -0.6753907],
    ...
    the output precision of NumPy arrays to 3 and display tiny values as 0.0

    >>> np.set_printoptions(precision=3, suppress=True)
    >>> AA
    array([[-1.369+0.j   ,  2.248+4.237j,  4.861-5.22j],
           [ 0.0   +0.j   ,  7.37+2.922j,  0.794+4.932j],
           [ 0.0   +0.j   ,  0.0   +0.j   ,  2.655-1.103j]])  # may vary
    >>> BB
    array([[ 1.719+0.j   , -1.115+1.j   , -0.763-0.646j],
           [ 0.0   +0.j   ,  7.24 +0.j   , -3.144+3.322j],
           [ 0.0   +0.j   ,  0.0   +0.j   ,  2.732+0.j   ]])  # may vary
    >>> Q
    array([[ 0.326+0.175j, -0.273-0.29j, -0.886-0.52j],
           [ 0.794+0.426j, -0.93+0.134j,  0.402-0.2j ],
    ...
    array([[ 1.0-0.j,  2.0-0.j, -1.0-0.j],
           [ 5.0+0.j,  5.0+0.j,  5.0-0.j],
           [ 2.0+0.j,  4.0+0.j, -8.0+0.j]])
    >>> Q @ BB @ Z.conj().T  # Should be B
    array([[ 1.0+0.j,  1.0+0.j, -3.0+0.j],
           [ 3.0-0.j,  1.0-0.j, -1.0+0.j],
           [ 5.0+0.j,  6.0+0.j, -2.0+0.j]])
