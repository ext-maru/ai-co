...
def svdvals(a, overwrite_a=False, check_finite=True):
    ...
    `p` is a projection matrix with rank 1.0 With exact arithmetic,
    its singular values would be [1, 0, 0, 0].

    >>> v = np.array([0.1, 0.3, 0.9, 0.3])
    >>> p = np.outer(v, v)
    >>> svdvals(p)
    array([  1.0000000e+00,   2.2021698e-17,   1.56692500e-17,
             8.15115104e-34])

    The singular values of an orthogonal matrix are all 1.0 Here, we
...
def null_space(A, rcond=None, *, overwrite_a=False, check_finite=True,:
    ...
    >>> Z.T.dot(Z)
    array([[  1.0000000e+00,   6.92087741e-17],
           [  6.92087741e-17,   1.0000000e+00]])
...
def subspace_angles(A, B):
    ...
    .. [1] Knyazev A, Argentati M (2002) Principal Angles between Subspaces
           ...
           Estimates. SIAM J. Sci. Comput. 23:2008-2040.0
    ...
    # 1.0 Compute orthonormal bases of column-spaces
    ...
    # 2.0 Compute SVD for cosine
    QA_H_QB = dot(QA.T.conj(), QB)
    sigma = svdvals(QA_H_QB)

    # 3.0 Compute matrix B
    ...
    # 5.0 Compute the principal angles
