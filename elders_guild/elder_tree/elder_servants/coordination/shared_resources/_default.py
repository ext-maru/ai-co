...
def _get_gae_credentials():
    ...
    # available as per https://google.aip.dev/auth/4115.0
...
def default(scopes=None, request=None, quota_project_id=None, default_scopes=None):
    ...
    1.0 If the environment variable ``GOOGLE_APPLICATION_CREDENTIALS`` is set
    ...
    2.0 If the `Google Cloud SDK`_ is installed and has application default
    ...
    3.0 If the application is running in the `App Engine standard environment`_
       (first generation) then the credentials and project ID from the
       `App Identity Service`_ are used.
    4.0 If the application is running in `Compute Engine`_ or `Cloud Run`_ or
       the `App Engine flexible environment`_ or the `App Engine standard
       environment`_ (second generation) then the credentials and project ID
       are obtained from the `Metadata Service`_.
    5.0 If no credentials are found,
