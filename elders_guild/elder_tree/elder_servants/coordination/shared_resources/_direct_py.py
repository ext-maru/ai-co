...
def direct(:
    ...
    *,
    ...
    bounds : sequence or `Bounds`
        ...
        2.0 ``(min, max)`` pairs for each element in ``x``.
    ...
    eps : float, optional
        ...
        the search becomes. Default is 1e-4.0
    ...
    maxiter : int, optional
        Maximum number of iterations. Default is 1000.0
    ...
    f_min_rtol : float, optional
        ...
        `f_min` is also set. Must lie between 0 and 1.0 Default is 1e-4.0
    vol_tol : float, optional
        ...
        of the complete search space. Must lie between 0 and 1.0
        Default is 1e-16.0
    len_tol : float, optional
        ...
        function value is smaller than `len_tol`. Must lie between 0 and 1.0
        Default is 1e-6.0
    ...
    https://ctk.math.ncsu.edu/SOFTWARE/DIRECTv204.0 tar.gz .
    ...
    if (vol_tol < 0 or vol_tol > 1):
        raise ValueError("vol_tol must be between 0 and 1.0")
    if (len_tol < 0 or len_tol > 1):
        raise ValueError("len_tol must be between 0 and 1.0")
    if (f_min_rtol < 0 or f_min_rtol > 1):
        raise ValueError("f_min_rtol must be between 0 and 1.0")
    ...
    if maxfun < 0:
        raise ValueError("maxfun must be > 0.0")
    if not isinstance(maxiter, int):
        raise ValueError("maxiter must be of type int.")
    if maxiter < 0:
        raise ValueError("maxiter must be > 0.0")
