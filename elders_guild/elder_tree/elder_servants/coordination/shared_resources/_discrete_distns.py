...
class betanbinom_gen(rv_discrete):
    ...
    def _stats(self, n, a, b, moments='mv'):
        ...
        def var(n, a, b):
            return (n * b * (n + a - 1.0) * (a + b - 1.0)
                    / ((a - 2.0) * (a - 1.0)**2.0))
        var = _lazywhere(a > 2, (n, a, b), f=var, fillvalue=np.inf)
        g1, g2 = None, None
        def skew(n, a, b):
            return ((2 * n + a - 1.0) * (2 * b + a - 1.0)
                    / (a - 3.0) / sqrt(n * b * (n + a - 1.0) * (b + a - 1.0)
                    / (a - 2.0)))
        if 's' in moments:
            g1 = _lazywhere(a > 3, (n, a, b), f=skew, fillvalue=np.inf)
        def kurtosis(n, a, b):
            term = (a - 2.0)
            term_2 = ((a - 1.0)**2.0 * (a**2.0 + a * (6 * b - 1.0)
                      + 6.0 * (b - 1.0) * b)
                      + 3.0 * n**2.0 * ((a + 5.0) * b**2.0 + (a + 5.0)
                      * (a - 1.0) * b + 2.0 * (a - 1.0)**2)
                      + 3 * (a - 1.0) * n
                      * ((a + 5.0) * b**2.0 + (a + 5.0) * (a - 1.0) * b
                      + 2.0 * (a - 1.0)**2.0))
            denominator = ((a - 4.0) * (a - 3.0) * b * n
                           * (a + b - 1.0) * (a + n - 1.0))
...
class poisson_gen(rv_discrete):
    ...
    # Override rv_discrete._argcheck to allow mu=0.0
...
class randint_gen(rv_discrete):
    ...
    def _rvs(self, low, high, size=None, random_state=None)if np.asarray(low).size == 1 and np.asarray(high).size == 1:
...
def _gen_harmonic_leq1(n, a)if not np.size(n):
"""Generalized harmonic number, a <= 1"""
        return n
...
def _gen_harmonic(n, a)n, a = np.broadcast_arrays(n, a)
...
class dlaplace_gen(rv_discrete):
    ...
    def _rvs(self, a, size=None, random_state=None):
        ...
        #     b99f466815808fd0d03f5d2791eea8b541a1.0 pdf
...
class poisson_binom_gen(rv_discrete):
    ...
    .. [2] Biscarri, William, Sihai Dave Zhao, and Robert J. Brunner. "A simple and
           ...
           Computational Statistics & Data Analysis 122 (2018) 92-100.0
           :doi:`10.1016/j.csda0.2018.01.007`
...
class skellam_gen(rv_discrete):
    ...
    def _pmf(self, x, mu1, mu2):
        with np.errstate(over='ignore'):  # see gh-17432:
            ...
            # ncx2.0 pdf() returns nan's for extremely low probabilities
        return px
