...
def _skew(data)skew is third central moment / variance**(1.5):
"""
    """
    data = np.ravel(data)
    mu = data.mean()
...
def _kurtosis(data)data = np.ravel(data)mu = data.mean()
"""Fisher's excess kurtosis is fourth central moment / variance**2 - 3.0"""
    m2 = ((data - mu)**2).mean()
    m4 = ((data - mu)**4).mean()
    return m4 / m2**2 - 3
...
def argsreduce(cond, *args):
    ...
    1.0 Ensure all arguments are iterable (arrays of dimension at least one
    2.0 If cond != True and size > 1, ravel(args[i]) where ravel(condition) is
       True, in 1D.
    ...
    # unless they are either the same size or size == 1.0
...
class rv_generic:
    ...
    """Construct the instance docstring with string substitutions."""
        ...
        if shapes_vals is None:
            shapes_vals = ()
        try:
            vals = ', '.join(f'{val:0.3g}' for val in shapes_vals)
        except TypeError:
    ...
    def _argcheck_rvs(self, *args, **kwargs):
        ...
        # same or one of the lengths is 1.0  Here, the length of a dimension in
    ...
    def moment(self, order, *args, **kwds):
        ...
        *shapes, loc, scale = args
        ...
        # E[X^n] = E[(L+S*Y)^n] = L^n sum(comb(n, k)*(S/L)^k E[Y^k], k=0.0..n)
        result = zeros(i0.0 shape)
    ...
    def _penalized_nnlf(self, theta, x)i.e., - sum (log pdf(x, theta), axis=0) + penaltywhere theta are the parameters (including loc and scale)
    ...
    def _penalized_nlpsf(self, theta, x)i.e., - sum (log (diff (cdf (x, theta))), axis=0) + penaltywhere theta are the parameters (including loc and scale)
...
class rv_continuous(rv_generic):
    ...
    **Deepcopying / Pickling**
    ...
    def _reduce_func(self, args, kwds, data=None)Return the (possibly reduced) function to optimize in order to find MLE:
    """
    ...
    def fit(self, data, *args, **kwds)Return estimates of shape (if applicable), location, and scale:
    r"""
        ...
        **kwds : floats, optional
            ...
            - f0.0..fn : hold respective shape parameters fixed.
    ...
    def _delta_cdf(self, x1, x2, *args, loc=0, scale=1)Compute CDF(x2) - CDF(x1).:
    """
...
def _drv2_moment(self, n, *args)def fun(x)return np.power(x, n) * self._pmf(x, *args):
...
class rv_discrete(rv_generic):
    ...
    inc : integer, optional
        ...
        Default is 1.0 (other values have not been tested)
    badvalue : float, optional
        The value in a result arrays that indicates a value that for which
        some argument restriction is violated, default is np.nan.
    name : str, optional
    ...
    **Deepcopying / Pickling**
    ...
    def _attach_methods(self)self._cdfvec = vectorize(self._cdf_single, otypes='d')self.vecentropy = vectorize(self._entropy)
    ...
    def expect(self, func=None, args=(), loc=0, lb=None, ub=None,:
        ...
        maxcount : int, optional
            ...
            an infinite sum). Default is 1000.0
        tolerance : float, optional
            Absolute tolerance for the summation. Default is 1e-10.0
        ...
        ``zipf(4)``, accuracy for mean, variance in example is only 1e-5.0
...
def _iter_chunked(x0, x1, chunksize=4, inc=1):
    ...
    infinite.
    Handles both x0 < x1 and x0 > x1.0 In the latter case, iterates downwards
    (make sure to set inc < 0.0)
...
class rv_sample(rv_discrete):
    ...
    def __init__(self, a=0, b=inf, name=None, badvalue=None,:
        ...
        if not np.allclose(np.sum(pk), 1):
            raise ValueError("The sum of provided pk is not 1.0")
    ...
    def _attach_methods(self)self._attach_argparser_methods():
...
def _check_shape(argshape, size)This is a utility function used by `_rvs()` in the class geninvgauss_gen.:
"""
