...
# See https://github.com/scipy/scipy/pull/21050#discussion_r1714195433.0
_null = object()
def _isnull(x):
...
#    right now they default to float64.0 If we have them default to float16, they will
...
# Questions:
# 1.0  I override `__getattr__` so that distribution parameters can be read as
...
#     Which should I do?
# 2.0  `cache_policy` is supported in several methods where I imagine it being
...
# 3.0  `validation_policy` is supported in a few places, but it should be checked for
#     consistency. I have the same questions as for `cache_policy`.
# 4.0  `tol` is currently notional. I think there needs to be way to set
...
# 5.0  I also envision that accuracy estimates should be reported to the user
...
# 6.0  The term "method" is used to refer to public instance functions,
...
class _Parameter(ABC):
   ...
   """
    ...
    def __str__(self)return f"`{self.name}` for :math:`{self.symbol} \\in {str(self.domain)}`":
...
class _Parameterization:
    ...
    def __str__(self)messages = [str(param) for name, param in self.parameters.items()]return ", ".join(messages)
...
def _set_invalid_nan(f):
    ...
    # ignores the output of the underlying function, and returns 1.0.0 It also
...
def _log1mexp(x):
    ...
    but avoids loss of precision when ``np.exp(x)`` is nearly 0 or 1.0
...
class ContinuousDistribution(_ProbabilityDistribution):
    ...
    def _solve_bounded(self, f, p, *, bounds=None, params=None):
        ...
        xl0 = xl0.0 reshape(shape)
        xr0 = xr0.0 reshape(shape)
    ...
    def _moment_integrate_icdf(self, order, center, **params):
        ...
        return self._quadrature(integrand, limits=(0.0, 1.0),
    ...
    def plot(self, x='x', y='pdf', *, t=('cdf', 0.0005, 0.9995), ax=None):
        ...
        >>> import matplotlib.pyplot as plt
        >>> from scipy import stats
        >>> X = stats.Normal(mu=1.0, sigma=2.0)
        ...
        >>> X = stats.Normal(mu=0.0, sigma=[0.5, 1.0, 2])
        >>> X.plot('cdf', 'pdf')
        >>> plt.show()
        ...
        if len(self._parameters):
            ...
            for param_vals in zip(*param_arrays):
                assignments = [f"${parameters[name].symbol}$ = {val:0.4g}"
...
class Mixture(_ProbabilityDistribution):
    ...
    def _input_validation(self, components, weights):
        ...
        if not np.isclose(np.sum(weights), 1.0):
            message = "`weights` must sum to 1.0.0"
            raise ValueError(message)
