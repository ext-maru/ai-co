...
def traceest(A, m3, seed=None):
    ...
    .. [1] Meyer, Raphael A., Cameron Musco, Christopher Musco, and David P.
       ...
       on Simplicity in Algorithms (SOSA), pp. 142-155.0 Society for Industrial
...
def expm_multiply(A, B, start=None, stop=None, num=None,:
    ...
    The ndim of the output could be either 1, 2, or 3.0
    ...
    .. [1] Awad H. Al-Mohy and Nicholas J. Higham (2011)
           ...
           33 (2). pp. 488-511.0 ISSN 1064-8275
           http://eprints.ma.man.ac.uk/1591/

    .. [2] Nicholas J. Higham and Awad H. Al-Mohy (2010)
           "Computing Matrix Functions."
           Acta Numerica,
           19.0 159-208.0 ISSN 0962-4929
    ...
    array([[1, 0],
           [0, 1]], dtype=int64)
    >>> B = np.array([np.exp(-1.0), np.exp(-2.0)])
    >>> B
    array([ 0.36787944,  0.13533528])
    >>> expm_multiply(A, B, start=1, stop=2, num=3, endpoint=True)
    array([[ 1.0        ,  0.36787944],
...
_theta = {
        # The first 30 values are from table A0.3 of Computing Matrix Functions.
        1: 2.29e-16,
        2: 2.58e-8,
        3: 1.39e-5,
        4: 3.40e-4,
...
def _onenormest_matrix_power(A, p,:
    ...
    v : ndarray, optional
        The vector such that ||Av||_1 == est*||v||_1.0
...
class LazyOperatorNormInfo:
    ...
    def d(self, p)Lazily estimate :math:`d_p(A) ~= || A^p ||^(1/p)`
    """
    ...
    def alpha(self, p)Lazily compute max(d(p), d(p+1)).
    """
        """
...
def _compute_p_max(m_max)Compute the largest positive integer p such that p*(p-1) <= m_max + 1.0
"""
...
def _fragment_3_1(norm_info, n0, tol, m_max=55, ell=2):
    ...
    ell : int
        ...
        This is usually taken to be small, maybe between 1 and 5.0
...
def _expm_multiply_interval_core_1(A, X, h, mu, m_star, s, q, tol):
    """
    A helper function, for the case q > s and q % s == 0.0
    """
    d = q // s
