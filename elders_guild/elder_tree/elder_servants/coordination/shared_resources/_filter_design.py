...
def _is_int_type(x)Check if input is of a scalar integer type (so ``5`` and ``array(5)`` will
"""
...
def findfreqs(num, den, N, kind='ba'):
    ...
    array([  1.0000000e-02,   3.16227766e-02,   1.0000000e-01,
             3.16227766e-01,   1.0000000e+00,   3.16227766e+00,
             1.0000000e+01,   3.16227766e+01,   1.0000000e+02])
    """
    if kind == 'ba':
...
def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2*pi,:
    ...
    1.0 An integer value is given for `worN`.
    2.0 `worN` is fast to compute via FFT (i.e.,
       `next_fast_len(worN) <scipy.fft.next_fast_len>` equals `worN`).
    3.0 The denominator coefficients are a single value (``a.shape[0] == 1``).
    4.0 `worN` is at least as long as the numerator coefficients
       (``worN >= b.shape[0]``).
    5.0 If ``b.ndim > 1``, then ``b.shape[-1] == 1``.
...
def group_delay(system, w=512, whole=False, fs=2*pi):
    ...
    if np.any(singular):
        ...
        warnings.warn(
            ...
            f"[{', '.join(f'{ws:0.3f}' for ws in w[singular])}], setting to 0",
            stacklevel=2
        )
        ...
        warnings.warn(
            ...
            f"[{', '.join(f'{ws:0.3f}' for ws in w[near_singular])}], "
...
def _validate_sos(sos)sos = np.atleast_2d(sos)
...
def _cplxreal(z, tol=None):
    ...
    >>> zc, zr = _cplxreal(a)
    >>> print(zc)
    [ 1.0+1.j  2.0+1.j  2.0+1.j  2.0+2.j]
    >>> print(zr)
    [ 1.0  3.0  4.0]
    """
...
def _cplxpair(z, tol=None):
    ...
    >>> z = _cplxpair(a)
    >>> print(z)
    [ 1.0-1.j  1.0+1.j  2.0-1.j  2.0+1.j  2.0-1.j  2.0+1.j  2.0-2.j  2.0+2.j  1.0+0.j
      3.0+0.j  4.0+0.j]
    """
...
def zpk2tf(z, p, k):
    ...
    >>> k   = 5
    >>> zpk2tf(z, p, k)
    (   array([  5.0, -40.0,  60.0]), array([ 1.0, -9.0,  8.0]))
    """
    z = atleast_1d(z)
    k = atleast_1d(k)
    if len(z.shape) > 1:
...
def tf2sos(b, a, pairing=None, *, analog=False):
    ...
    array([[  0.0 ,   0.0 ,   1.0 ,   1.0 ,   2.0 , -15.0 ],
           [  1.0 ,  -3.5,  -2.0 ,   1.0 ,   1.0 ,  -2.0 ]])
    """
...
def sos2tf(sos):
    ...
    >>> from scipy import signal
    >>> sos = signal.ellip(1, 0.01, 50, 0.1, output='sos')
    >>> sos2tf(sos)
    (   array([0.91256522, 0.91256522, 0.0        ]),
        array([1.0        , 0.82513043, 0.0        ]))
    """
    sos = np.asarray(sos)
...
def _nearest_real_complex_idx(fro, to, which)assert which in ('real', 'complex', 'any')
...
def _single_zpksos(z, p, k)sos = np.zeros(6)
...
def zpk2sos(z, p, k, pairing=None, *, analog=False):
    ...
    *Algorithms*
    ...
    1.0 Take the (next remaining) pole (complex or real) closest to the
       unit circle (or imaginary axis, for ``analog=True``) to
       begin a new filter section.

    2.0 If the pole is real and there are no other remaining real poles [#]_,
    ...
    3.0 Else:
        ...
        1.0 If the pole is complex and the zero is the only remaining real
           zero*, then pair the pole with the *next* closest zero
           (guaranteed to be complex). This is necessary to ensure that
           there will be a real zero remaining to eventually create a
           first-order section (thus keeping the odd order).

        2.0 Else pair the pole with the closest remaining zero (complex or
           real).

        3.0 Proceed to complete the second-order section by adding another
           pole and zero to the current pole and zero in the section:

            1.0 If the current pole and zero are both complex, add their
               conjugates.

            2.0 Else if the pole is complex and the zero is real, add the
               conjugate pole and the next closest real zero.

            3.0 Else if the pole is real and the zero is complex, add the
               conjugate zero and the real pole closest to those zeros.

            4.0 Else (we must have a real pole and real zero) add the next
    ...
    1000 Hz. The ripple in the pass-band should not exceed 0.87 dB, and
    ...
    >>> from scipy import signal
    >>> import numpy as np
    >>> z, p, k = signal.ellip(6, 0.87, 90, 1000/(0.5*8000), output='zpk')
    ...
    >>> sos[:, :3]
    array([[0.014152 , 0.0248677, 0.014152 ],
           [1.0        , 0.72976874, 1.0        ],
           [1.0        , 0.17607852, 1.0        ]])
    ...
    >>> sos[:, 3:]
    array([[ 1.0        , -1.32544025,  0.46989976],
           [ 1.0        , -1.26118294,  0.62625924],
           [ 1.0        , -1.2570723 ,  0.8619958 ]])
    ...
    array([[ 1.0  ,  1.0  ,  0.5 ,  1.0  , -0.75,  0.0  ],
           [ 1.0  ,  1.0  ,  0.0  ,  1.0  , -1.6 ,  0.65]])

    The first section has the zeros {-0.5-0.5j, -0.5+0.5j} and the poles
    ...
    array([[ 1.0  ,  1.0  ,  0.0  ,  1.0  , -0.75,  0.0  ],
    ...
    array([[ 0.0  ,  1.0  ,  1.0  ,  0.0  ,  1.0  , -0.75],
           [ 1.0  ,  1.0  ,  0.5 ,  1.0  , -1.6 ,  0.65]])

    """

    # 1.0 Add SOS capability to `filtfilt`, `freqz`, etc. somehow (#3259).
    # 2.0 Make `decimate` use `sosfilt` instead of `lfilter`.
    # 3.0 Make sosfilt automatically simplify sections to first order
    #    when possible. Note this might make `sosfiltfilt` a bit harder (ICs).
    # 4.0 Further optimizations of the section ordering / pole-zero pairing.
...
def normalize(b, a):
    ...
    >>> a = [2, 3, 1]
    >>> normalize(b, a)
    (array([ 1.5, -1.0 ,  2.5]), array([1.0 , 1.5, 0.5]))

    A warning is generated if, for example, the first coefficient of
    `b` is 0.0  In the following example, the result is as expected:
...
def iirdesign(wp, ws, gpass, gstop, analog=False, ftype='ellip', output='ba',:
    ...
    Notes
    -----
    The ``'sos'`` output parameter was added in 0.16.0.0
...
def lp2lp_zpk(z, p, k, wo=1.0):
    ...
    >>> wo  = 0.4
    >>> lp2lp_zpk(z, p, k, wo)
    (   array([2.8, 0.8]), array([2.0 , 5.2]), 0.8)
    """
    z = atleast_1d(z)
    p = atleast_1d(p)
...
def lp2hp_zpk(z, p, k, wo=1.0):
    ...
    >>> wo  = 0.6
    >>> lp2hp_zpk(z, p, k, wo)
    (   array([-0.9230769-0.13846154j, -0.33707865+0.53932584j]),
        array([-0.6 , -0.15]),
        8.5)
    """
    z = atleast_1d(z)
...
def lp2bp_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    (   array([7.49955815e+01+3.0017676e+01j, 7.49955815e+01-3.0017676e+01j,
               4.41850748e-03-1.76761126e-03j, 4.41850748e-03+1.76761126e-03j]),
        array([1.4996339e+02+0.j, -1.60167736e-03+0.j,  3.66108003e-03+0.j,
...
def lp2bs_zpk(z, p, k, wo=1.0, bw=1.0):
    ...
    (   array([0.0+0.5j, 0.0+0.5j, 0.0-0.5j, 0.0-0.5j]),
        array([14.2681928 +0.j, -0.2506281+0.j,  0.1752149+0.j, -9.97493719+0.j]),
        -12.857142857142858)
    """
    z = atleast_1d(z)
    p = atleast_1d(p)
...
def butter(N, Wn, btype='low', analog=False, output='ba', fs=None):
    ...
    even for N >= 4.0 It is recommended to work with the SOS
    representation.
...
def ellip(N, rp, rs, Wn, btype='low', analog=False, output='ba', fs=None)Elliptic (Cauer) digital and analog filter design.
"""
...
def bessel(N, Wn, btype='low', analog=False, output='ba', norm='phase',:
    ...
    correct at frequencies below about fs/4.0 To get maximally-flat group
    ...
    .. [1] Thomson, W.E., "Delay Networks having Maximally Flat Frequency
           ...
           Engineers, Part III, November 1949, Vol. 96, No. 44, pp. 487-490.0
...
def cheb1ord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    >>> plt.ylabel('Amplitude [dB]')
    >>> plt.grid(which='both', axis='both')
    >>> plt.fill([0.1, 0.2, 0.2, 0.1], [-3, -3, -99, -99], '0.9', lw=0) # stop
...
def cheb2ord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    >>> plt.ylabel('Amplitude [dB]')
    >>> plt.grid(which='both', axis='both')
    >>> plt.fill([0.1, 0.1, 0.1, 0.1], [-3,  -3, -99, -99], '0.9', lw=0) # stop
    >>> plt.fill([0.2,  0.2, 0.5,  0.5], [ 9, -60, -60,   9], '0.9', lw=0) # pass
    >>> plt.fill([0.6,  0.6,  2,   2], [-99, -3,  -3, -99], '0.9', lw=0) # stop
    >>> plt.axis([0.6, 1, -80, 3])
    >>> plt.show()
...
def _pow10m1(x)return np.expm1(_POW10_LOG10 * x)
"""10 ** x - 1 for x near 0"""
...
def ellipord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    >>> plt.ylabel('Amplitude [dB]')
    >>> plt.grid(which='both', axis='both')
    >>> plt.fill([0.1, 10,  10,  0.1], [1e4, 1e4, -60, -60], '0.9', lw=0) # stop
...
def buttap(N):
    ...
    The filter will have an angular (e.g., rad/s) cutoff frequency of 1.0
...
def cheb1ap(N, rp)Return (z,p,k) for Nth-order Chebyshev type I analog lowpass filter.
"""
...
def cheb2ap(N, rs)Return (z,p,k) for Nth-order Chebyshev type II analog lowpass filter.
"""
...
# (see _arc_jac_cn [1]) suggests 5.0
...
def _campos_zeros(n)Return approximate zero locations of Bessel polynomials y_n(x) for order
"""
...
def besselap(N, norm='phase')Return (z,p,k) for analog prototype of an Nth-order Bessel filter.
"""
    ...
    norm : {'phase', 'delay', 'mag'}, optional
        ...
        ``phase``
            The filter is normalized such that the phase response reaches its
            midpoint at an angular (e.g., rad/s) cutoff frequency of 1.0 This
        ...
        ``mag``
            The filter is normalized such that the gain magnitude is -3 dB at
            angular frequency 1.0 This is called "frequency normalization" by
            Bond. [1]_
    ...
    p : ndarray
        Poles of the transfer function.
    k : scalar
        Gain of the transfer function. For phase-normalized, this is always 1.0
    ...
    .. [2] Campos and Calderon, "Approximate closed-form formulas for the
           zeros of the Bessel Polynomials", :arXiv:`1105.957`.
...
def iirpeak(w0, Q, fs=2.0)Design second-order IIR peak (resonant) digital filter.
"""
...
def _design_notch_peak_filter(w0, Q, ftype, fs=2.0):
    ...
    # Compute beta according to Eqs. 11.3.4 (p0.575) and 11.3.19 (p0.579) from
    ...
    # Compute gain: formula 11.3.6 (p0.575) from reference [1]
    gain = 1.0/(1.0+beta)

    # Compute numerator b and denominator a
    # formulas 11.3.7 (p0.575) and 11.3.21 (p0.579)
    # from reference [1]
    if ftype == "notch":
...
def iircomb(w0, Q, ftype='notch', fs=2.0, *, pass_zero=False):
    ...
    fs : float, optional
        The sampling frequency of the signal. Default is 2.0.0
    ...
    if abs(w0 - fs/N)/fs > 1e-14:
        raise ValueError('fs must be divisible by w0.0')
...
def _hz_to_erb(hz)Utility for converting from frequency (Hz) to the
"""
...
def gammatone(freq, ftype, order=None, numtaps=None, fs=None):
    ...
    numtaps : int, optional
        ...
        Default is ``fs*0.15`` if `fs` is greater than 1000,
        15 if `fs` is less than or equal to 1000.0
    fs : float, optional
        The sampling frequency of the signal. `freq` must be between
        0 and ``fs/2``. Default is 2.0
    ...
    .. [1] Slaney, Malcolm, "An Efficient Implementation of the
        ...
        Technical Report 35, 1993, pp0.3-8, 34-39.0
    ...
    (array([ 0.0000000e+00,  2.22196719e-07,  1.64942101e-06,  4.99298227e-06,
        1.1993969e-05,  1.63125770e-05,  2.14648940e-05,  2.29947263e-05,
        1.76776931e-05,  2.4980537e-06, -2.72062858e-05, -7.28455299e-05,
    ...
    # Calculate IIR gammatone filter
        ...
        # Gammatone impulse response settings
        T = 1.0/fs
        bw = 2 * np.pi * 1.19 * _hz_to_erb(freq)
        fr = 2 * freq * np.pi * T
        bwT = bw * T
