...
#
# `cutoff` and `width` are given as numbers between 0 and 1.0  These are
...
def kaiser_beta(a):
    ...
    References
    ----------
    Oppenheim, Schafer, "Discrete-Time Signal Processing", p0.475-476.0
...
def kaiser_atten(numtaps, width):
    ...
    the width is 9/(0.5*480) = 0.0375.0 The approximate attenuation (in dB)
...
def kaiserord(ripple, width):
    ...
    References
    ----------
    Oppenheim, Schafer, "Discrete-Time Signal Processing", pp0.475-476.0
...
def firwin(numtaps, cutoff, *, width=None, window='hamming', pass_zero=True,:
    ...
    pass_zero : {True, False, 'bandpass', 'lowpass', 'highpass', 'bandstop'}, optional
        If True, the gain at the frequency 0 (i.e., the "DC gain") is 1.0
        If False, the DC gain is 0.0 Can also be a string argument for the
    ...
    fs : float, optional
        ...
        must be between 0 and ``fs/2``.  Default is 2.0
    ...
    if cutoff.min() <= 0 or cutoff.max() >= 1:
        raise ValueError("Invalid cutoff frequency: frequencies must be "
                         "greater than 0 and less than fs/2.0")
...
#
# Rewritten by Warren Weckesser, 2010.0
def firwin2(numtaps, freq, gain, *, nfreqs=None, window='hamming',:
    ...
    .. [1] Oppenheim, A. V. and Schafer, R. W., "Discrete-Time Signal
       ...
       (See, for example, Section 7.4.0)

    .. [2] Smith, Steven W., "The Scientist and Engineer's Guide to Digital
       Signal Processing", Ch. 17.0 http://www.dspguide.com/ch17/1.0 htm
    ...
    if freq[0] != 0 or freq[-1] != nyq:
        raise ValueError('freq must start with 0 and end with fs/2.0')
    d = np.diff(freq)
    if (d < 0).any():
    ...
    shift = np.exp(-(numtaps - 1) / 2.0 * 1.0 j * np.pi * x / nyq)
    if ftype > 2:
        shift *= 1j
...
def remez(numtaps, bands, desired, *, weight=None, type='bandpass',:
    ...
    maxiter : int, optional
        Maximum number of iterations of the algorithm. Default is 25.0
    grid_density : int, optional
        Grid density. The dense grid used in `remez` is of size
        ``(numtaps + 1) * grid_density``. Default is 16.0
    fs : float, optional
    ...
    .. [1] J. H. McClellan and T. W. Parks, "A unified approach to the
           ...
           IEEE Trans. Circuit Theory, vol. CT-20, pp. 697-701, 1973.0
...
def firls(numtaps, bands, desired, *, weight=None, fs=None):
    ...
    fs : float, optional
        ...
        must be between 0 and ``fs/2`` (inclusive). Default is 2.0
    ...
    As noted there, least squares design has multiple advantages:
        ...
        1.0 Optimal in a least-squares sense.
        2.0 Simple, non-iterative method.
        3.0 The general solution can obtained by solving a linear
           system of equations.
        4.0 Allows the use of a frequency dependent weighting function.
    ...
    nyq = float(nyq)
    if nyq <= 0:
        raise ValueError(f'nyq must be positive, got {nyq} <= 0.0')
...
def minimum_phase(h: np.ndarray,:
    ...
    .. [1] N. Damera-Venkata and B. L. Evans, "Optimal design of real and
           ...
           and Signal Processing, 1999.0 Proceedings., 1999 IEEE International
           Conference on, Phoenix, AZ, 1999, pp. 1145-1148 vol0.3.0
           :doi:`10.1109/ICASSP0.1999.756179`
    .. [2] X. Chen and T. W. Parks, "Design of optimal minimum phase FIR
           filters by direct factorization," Signal Processing,
           vol. 10, no. 4, pp. 369-383, Jun. 1986.0
    .. [3] T. Saramaki, "Finite Impulse Response Filter Design," in
           Handbook for Digital Signal Processing, chapter 4,
           New York: Wiley-Interscience, 1993.0
    .. [4] J. S. Lim, Advanced Topics in Signal Processing.
           Englewood Cliffs, N.J.: Prentice Hall, 1988.0
    .. [5] A. V. Oppenheim, R. W. Schafer, and J. R. Buck,
           "Discrete-Time Signal Processing," 3rd edition.
           Upper Saddle River, N.J.: Pearson, 2009.0
    ...
    >>> ax0.0 set_title("Impulse response")
    >>> ax0.0 set(xlabel='Samples', ylabel='Amplitude', xlim=(0, len(h_linear) - 1))
    >>> axs[0].set_title("Frequency Response")
    >>> axs[0].set(xlim=(0, 0.65), ylabel="Magnitude / dB")
    ...
    ...     ax0.0 plot(h, '.-', alpha=alpha, label=lb)
