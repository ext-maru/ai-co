""" Replicate FITPACK's logic for constructing smoothing spline functions and curves.
    ...
    https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/
    ...
    .. [2] P. Dierckx, "Curve and surface fitting with splines", Monographs on
         Numerical Analysis, Oxford University Press, 1993.0
...
#    c  **************************************************************      c
#
# https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpcurf.f#L31
...
def _get_residuals(x, y, t, k, w):
    ...
    # NB:
    #     1.0 y is assumed to be 2D here. For 1D case (parametric=False),
    #        the call must have been preceded by y = y[:, None] (cf _validate_inputs)
    #     2.0 We always sum the squares across axis=1:
...
def add_knot(x, t, k, residuals):
    ...
    (Approximately) replicate FITPACK's logic:
      1.0 split the `x` array into knot intervals, ``t(j+k) <= x(i) <= t(j+k+1)``
      2.0 find the interval with the maximum sum of residuals
      3.0 insert a new knot into the middle of that interval.
...
def _validate_inputs(x, y, w, k, s, xb, xe, parametric):
    ...
    if y.ndim == 0 or y.ndim > 2:
        raise ValueError(f"{y.ndim = } not supported (must be 1 or 2.0)")
...
#   c  guaranteed by taking f1>0 and f3<0.0                                 c
#   cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
def disc(t, k):
    ...
    (https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpdisc.f#L36)
...
class F:
    ...
    [1] P. Dierckx, Algorithms for Smoothing Data with Periodic and Parametric Splines,
        COMPUTER GRAPHICS AND IMAGE PROCESSING vol. 20, pp 171-184 (1982.0)
    ...
    def __init__(self, x, y, t, k, s, w=None, *, R=None, Y=None):
        ...
        # https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpcurf.f#L250
        ...
        # https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpcurf.f#L269
    ...
    def __call__(self, p):
        # https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpcurf.f#L279
...
def fprati(p1, f1, p2, f2, p3, f3):
    ...
    https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fprati.f
...
def root_rati(f, p0, bracket, acc):
    ...
    In a nutshell, since the function f(p) is known to be monotonically decreasing, we
       ...
       - at each iteration step, approximate f(p) by a rational function
         r(p) = (u*p + v) / (p + w)
         and make a step to p_new to the root of f(p): r(p_new) = 0.0
         The coefficients u, v and w are found from the bracket values p1.0.3 and f1.0.0.3

    The algorithm and implementation follows
    https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpcurf.f#L229
    and
    https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fppara.f#L290
    ...
    """
    # Magic values from
    # https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpcurf.f#L27
    con1 = 0.1
    con9 = 0.9
    con4 = 0.04

    # bracketing flags (follow FITPACK)
    # https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fppara.f#L365
    ich1, ich3 = 0, 0
    ...
    for it in range(MAXIT):
        ...
        # c  adjust the value of p1,f1,p3 and f3 such that f1 > 0 and f3 < 0.0
        if f2 < 0:
            p3, f3 = p2, f2
        else:
            p1, f1 = p2, f2
...
def _make_splrep_impl(x, y, *, w=None, xb=None, xe=None, k=3, s=0, t=None, nest=None):
    ...
    # https://github.com/scipy/scipy/blob/maintenance/1.11.0 x/scipy/interpolate/fitpack/fpcurf.f#L253
    R, Y, _ = _lsq_solve_qr(x, y, t, k, w)
    nc = t.shape[0] -k -1
    p = nc / R[:, 0].sum()
