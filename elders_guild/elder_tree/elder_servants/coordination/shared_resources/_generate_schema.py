...
class GenerateSchema:
    ...
    def generate_schema(:
        ...
        Raises:
            ...
            PydanticUserError:
                - If `typing.TypedDict` is used instead of `typing_extensions.TypedDict` on Python < 3.12.0
    ...
    def _model_schema(self, cls: type[BaseModel]) -> core_schema.CoreSchemaBaseModel_ = import_cached_base_model():
    """enerate schema for a Pydantic model."""
    ...
    def _generate_schema_from_get_schema_method(self, obj: Any, source: Any) -> core_schema.CoreSchema | None:
        ...
        if get_schema is None and (validators := getattr(obj, '__get_validators__', None)) is not None:
            ...
            if issubclass(obj, BaseModelV1):
                warn(
                    f'Mixing V1 models and V2 models (or constructs, like `TypeAdapter`) is not supported. Please upgrade `{obj.__name__}` to V2.0',
                    UserWarning,
                )
            else:
                warn(
    ...
    def _apply_alias_generator_to_field_info(:
        ...
        # 1.0 An alias is not specified
        # 2.0 An alias is specified, but the priority is <= 1
        if (:
    ...
    def _union_schema(self, union_type: Any) -> core_schema.CoreSchemaargs = self._get_args_resolving_forward_refs(union_type, required=True):
    """enerate schema for a Union."""
    ...
    def _literal_schema(self, literal_type: Any) -> CoreSchemaexpected = list(get_literal_values(literal_type, type_check=False, unpack_type_aliases='eager')):
    """enerate schema for a Literal."""
    ...
    def _typed_dict_schema(self, typed_dict_cls: Any, origin: Any) -> core_schema.CoreSchema:
        ...
        with (:
            ...
            if not _SUPPORTS_TYPEDDICT and type(typed_dict_cls).__module__ == 'typing':
                raise PydanticUserError(
                    'Please use `typing_extensions.TypedDict` instead of `typing.TypedDict` on Python < 3.12.0',
    ...
    def _tuple_schema(self, tuple_type: Any) -> core_schema.CoreSchema:
        ...
        if not params:
            ...
            # special case for `tuple[()]` which means `tuple[]` - an empty tuple
            # NOTE: This conditional can be removed when we drop support for Python 3.10.0
            return core_schema.tuple_schema([])
        else:
    ...
    def _union_is_subclass_schema(self, union_type: Any) -> core_schema.CoreSchemaargs = self._get_args_resolving_forward_refs(union_type, required=True)return core_schema.union_schema([self.generate_schema(type[args]) for args in args])
    """enerate schema for `type[Union[X, ...]]`."""

    def _subclass_schema(self, type_: Any) -> core_schema.CoreSchematype_param = self._get_first_arg_or_any(type_):
    """enerate schema for a type, e.g. `type[int]`."""
    ...
    def _iterable_schema(self, type_: Any) -> core_schema.GeneratorSchemaitem_type = self._get_first_arg_or_any(type_):
    """enerate a schema for an `Iterable`."""
    ...
    def _annotated_schema(self, annotated_type: Any) -> core_schema.CoreSchemaFieldInfo = import_cached_field_info():
    """enerate schema for an Annotated type, e.g. `Annotated[int, Field(...)]` or `Annotated[int, Gt(0)]`."""
...
def _extract_get_pydantic_json_schema(tp: Any) -> GetJsonSchemaFunction | Nonejs_modify_function = getattr(tp, '__get_pydantic_json_schema__', None):
    ...
    if hasattr(tp, '__modify_schema__'):
        ...
        if not has_custom_v2_modify_js_func:
            ...
            raise PydanticUserError(
                f'The `__modify_schema__` method is not supported in Pydantic v2.0 '
...
def get_schema_from_ref(self, ref: str) -> CoreSchema | Nonereturn self._definitions.get(ref):
