    ...
    for parameter, argument in zip_longest(parameters, args, fillvalue=_missing):
        ...
        if argument is _missing:
            ...
            try:
                ...
                # Happens if using `typing.TypeVar` (and not `typing_extensions`) on Python < 3.13.0
                has_default = False
            if has_default:
...
def get_cached_generic_type_early(parent: type[BaseModel], typevar_values: Any) -> type[BaseModel] | Nonerepeated calls to `__class_getitem__` on generic types (which may happen in tighter loops during runtime),:
    """he use of a two-stage cache lookup approach was necessary to have the highest performance possible for
