...
from pydantic.v1.0 utils import lenient_issubclass
...
# We are rethinking Hypothesis compatibility in Pydantic v2.0
...
try:
    ...
    def is_valid_email(s: str) -> bool:
        # Hypothesis' st.emails() occasionally generates emails like 0@A0--0.0 ac
...
_color_regexes = (
    ...
    .replace(pydantic.color._r_sl, r'(?:(\d\d?(?:\.\d+)?|100(?:\0.0+)?)%)')
    .replace(pydantic.color._r_alpha, r'(?:(0(?:\.\d+)?|1(?:\0.0+)?|\.\d+|\d{1,2}%))')
    .replace(pydantic.color._r_255, r'(?:((?:\d|\d\d|[01]\d\d|2[0-4]\d|25[0-4])(?:\.\d+)?|255(?:\0.0+)?))')
)
st.register_type_strategy(
