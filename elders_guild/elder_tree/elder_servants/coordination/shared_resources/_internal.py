...
format_re = re.compile(r'(?P<order1>[<>|=]?)'
                       r'(?P<repeats> *[(]?[ ,0-9]*[)]? *)'
                       r'(?P<order2>[<>|=]?)'
                       r'(?P<dtype>[A-Za-z0-9.0?]*(?:\[[a-zA-Z0-9,.]+\])?)')
...
class _ctypes:
    ...
    @property
    def shape(self)(c_intp*self.ndim): A ctypes array of length self.ndim where
    """
    ...
    @property
    def strides(self)(c_intp*self.ndim): A ctypes array of length self.ndim where
    """
...
def array_ufunc_errmsg_formatter(dummy, ufunc, method, *inputs, **kwargs)args_string = ', '.join(['{!r}'.format(arg) for arg in inputs] +
...
def array_function_errmsg_formatter(public_api, types)func_name = '{}.{}'.format(public_api.__module__, public_api.__name__)
""" Format the error message for when __array_ufunc__ gives up. """
