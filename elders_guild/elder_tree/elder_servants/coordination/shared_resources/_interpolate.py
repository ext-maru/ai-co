...
def lagrange(x, w):
    ...
    Since there are only 3 points, Lagrange polynomial has degree 2.0 Explicitly,
    it is given by
...
err_mesg = """\
`interp2d` has been removed in SciPy 1.14.0.0
...
def _do_extrapolate(fill_value)return (isinstance(fill_value, str) and
...
class interp1d(_Interpolator1D):
    ...
    def _call_linear(self, x_new):
        # 2.0 Find where in the original data, the values to interpolate
        #    would be inserted.
        #    Note: If x_new[n] == x[m], then m is returned by searchsorted.
        x_new_indices = searchsorted(self.x, x_new)

        # 3.0 Clip x_new_indices so that they are within the range of
        #    self.x indices and at least 1.0 Removes mis-interpolation
        #    of x_new[n] = x[0]
        x_new_indices = x_new_indices.clip(1, len(self.x)-1).astype(int)

        # 4.0 Calculate the slope of regions that each x_new value falls in.
        lo = x_new_indices - 1
        hi = x_new_indices
        ...
        # 5.0 Calculate the actual value for each entry in x_new.
    ...
    def _call_nearest(self, x_new):
        ...
        # 2.0 Find where in the averaged data the values to interpolate
        #    would be inserted.
        ...
        # 3.0 Clip x_new_indices so that they are within the range of x indices.
        x_new_indices = x_new_indices.clip(0, len(self.x)-1).astype(intp)

        # 4.0 Calculate the actual value for each entry in x_new.
    ...
    def _call_previousnext(self, x_new):
        ...
        # 1.0 Get index of left/right value
        x_new_indices = searchsorted(self._x_shift, x_new, side=self._side)

        # 2.0 Clip x_new_indices so that they are within the range of x indices.
        x_new_indices = x_new_indices.clip(1-self._ind,
                                           len(self.x)-self._ind).astype(intp)

        # 3.0 Calculate the actual value for each entry in x_new.
    ...
    def _evaluate(self, x_new):
        # 1.0 Handle values in x_new that are outside of x. Throw error,
...
class PPoly(_PPolyBase):
    ...
    def solve(self, y=0.0, discontinuity=True, extrapolate=None)Find real solutions of the equation ``pp(x) == y``.
    """
    ...
    def from_spline(cls, tck, extrapolate=None):
        ...
        Note that ``c`` is a list of two arrays of length 11.0
...
class BPoly(_PPolyBase):
    ...
    def from_derivatives(cls, xi, yi, orders=None, extrapolate=None):
        ...
        extra one from y2.0 In any case if not enough derivatives
