...
def _get_delta(A, b, c, x, y, z, tau, kappa, gamma, eta, sparse=False,:
    ...
    while i <= n_corrections:
        ...
        # 1.0 scipy.linalg.cho_factor/scipy.linalg.cho_solve,
        # 2.0 scipy.linalg.solve w/ sym_pos = True,
        # 3.0 scipy.linalg.solve w/ sym_pos = False, and if all else fails
        # 4.0 scipy.linalg.lstsq
        # For sparse systems, the order is:
        # 1.0 sksparse.cholmod.cholesky (if available)
        # 2.0 scipy.sparse.linalg.factorized (if umfpack available)
        # 3.0 scipy.sparse.linalg.splu
...
def _ip_hsd(A, b, c, c0, alpha0, beta, maxiter, disp, tol, sparse, lstsq,:
    ...
    tol : float
        Termination tolerance; see [4]_ Section 4.5.0
    sparse : bool
...
def _linprog_ip(c, c0, A, b, callback, postsolve_args, maxiter=1000, tol=1e-8,:
                disp=False, alpha0=0.99995, beta=0.1, sparse=False, lstsq=False,
    ...
    1.0 ``sksparse.cholmod.cholesky`` (if scikit-sparse and SuiteSparse are installed)

    2.0 ``scipy.sparse.linalg.factorized``
    ...
    .. [10] Andersen, Erling D., et al. Implementation of interior point methods
            for large scale linear programming. HEC/Universite de Geneve, 1996.0:
