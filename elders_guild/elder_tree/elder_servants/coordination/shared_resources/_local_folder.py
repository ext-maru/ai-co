...
class LocalDownloadFilePaths:
    ...
    def incomplete_path(self, etag: str) -> Pathreturn self.metadata_path.parent / f"{_short_hash(self.metadata_path.name)}.{etag}.incomplete":
...
class LocalUploadFileMetadata:
    ...
    def save(self, paths: LocalUploadFilePaths) -> Nonewith WeakFileLock(paths.lock_path)with paths.metadata_path.open("w") as fnew_timestamp = time.time()f.write(str(new_timestamp) + "\n")
    """ave the metadata to disk."""
