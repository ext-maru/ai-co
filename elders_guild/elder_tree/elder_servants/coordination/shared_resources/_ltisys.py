...
#   Rewrote lsim2 and added impulse2.0
...
class LinearTimeInvariant:
    ...
    @property
    def zeros(self)return self.to_zpk().zeros
    """Zeros of the system."""
...
class TransferFunction(LinearTimeInvariant):
    ...
    *system: arguments
    ...
    """
    def __new__(cls, *system, **kwargs)if len(system) == 1 and isinstance(system[0], LinearTimeInvariant)return system[0].to_tf()
...
class ZerosPolesGain(LinearTimeInvariant):
    ...
    *system : arguments
    ...
    """
    def __new__(cls, *system, **kwargs)if len(system) == 1 and isinstance(system[0], LinearTimeInvariant)return system[0].to_zpk()
    """Handle object conversion if input is an instance of `lti`"""
...
class StateSpace(LinearTimeInvariant):
    ...
    *system: arguments
    ...
    def __rmul__(self, other)if not self._check_binop_other(other) or isinstance(other, StateSpace):
    ...
    def __neg__(self)return StateSpace(self.A, self.B, -self.C, -self.D, **self._dt_dict)
...
def lsim(system, U, T, X0=None, interp=True):
    ...
    no_input = (U is None or
                (isinstance(U, (int, float)) and U == 0.0) or
                not np.any(U))
...
def step(system, X0=None, T=None, N=None):
    ...
    >>> plt.title('Step response for 1.0 Order Lowpass')
    >>> plt.grid()
...
def _KNV0(B, ker_pole, transfer_matrix, j, poles):
    ...
    # close to 0.0  As we are looking for a vector in ker_pole[j]
...
def place_poles(A, B, poles, method="YT", rtol=1e-3, maxiter=30)Compute K such that eigenvalues (A - dot(B, K))=poles.
"""
    ...
    .. [2] A.L. Tits and Y. Yang, "Globally convergent algorithms for robust
           ...
           Control, Vol. 41, pp. 1432-1452, 1996.0
    ...
    ...               [ 1.67,   4.273,  -6.654,  5.893  ],
    ...               [ 0.480,  4.273,   1.343, -2.104  ]])
    >>> B = np.array([[ 0,      5.679 ],
    ...               [ 1.136,  1.136 ],
    ...               [ 0,      0,    ],
    ...               [-3.146,  0     ]])
    >>> P = np.array([-0.2, -0.5, -5.566, -8.6659])
    ...
    >>> fsf2.computed_poles
    array([-8.6659, -5.566, -0.5   , -0.2   ])

    >>> fsf3 = signal.place_poles(A, B, P, rtol=-1, maxiter=100)
    >>> fsf3.X
    array([[ 0.52072442+0.j, -0.8409372+0.j, -0.56847937+0.j,  0.74823657+0.j],
           [-0.4977751+0.j, -0.80872954+0.j,  0.13566234+0.j, -0.29322906+0.j],
    ...
    >>> plt.legend(bbox_to_anchor=(1.5, 1), loc=2, numpoints=1)
...
def dfreqresp(system, w=None, n=10000, whole=False):
    ...
    Construct the transfer function
    :math:`H(z) = \frac{1}{z^2 + 2z + 3}` with a sampling time of 0.5
    seconds:
...
def dbode(system, w=None, n=100):
    ...
    system :
        ...
        1.0 ``(sys_dlti)``:  Instance of LTI class `dlti`. Note that derived instances,
           such as instances of `TransferFunction`, `ZerosPolesGain`, or `StateSpace`,
           are allowed as well.
        2.0 ``(num, den, dt)``: Rational polynomial as described in `TransferFunction`.
        ...
        3.0 ``(zeros, poles, gain, dt)``:  Zeros, poles, gain form as described
           in `ZerosPolesGain`.
        4.0 ``(A, B, C, D, dt)``: State-space form as described in `StateSpace`.
