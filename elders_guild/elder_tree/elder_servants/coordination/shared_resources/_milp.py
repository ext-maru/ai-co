...
def _constraints_to_components(constraints):
    ...
    1.0 A LinearConstraint
    2.0 A tuple representing a LinearConstraint
    3.0 An invalid object
    4.0 A sequence of composed entirely of objects of type 1/2
    5.0 A sequence containing at least one object of type 3

    We want to accept 1, 2, and 4 and reject 3 and 5.0
    """
...
def milp(c, *, integrality=None, bounds=None, constraints=None, options=None):
    ...
    constraints : sequence of scipy.optimize.LinearConstraint, optional
        ...
        1.0 A single `LinearConstraint` object
        2.0 A single tuple that can be converted to a `LinearConstraint` object
           as ``LinearConstraint(*constraints)``
        3.0 A sequence composed entirely of objects of type 1.0 and 2.0
