...
def _has_struct(elem)return (isinstance(elem, np.ndarray) and (elem.size > 0) and (elem.ndim > 0) and
...
def varmats_from_mat(file_obj):
    ...
    returned file-like objects are set at 0.0
...
class VarWriter5:
    ...
    def write_numeric(self, arr):
        ...
        try:
            ...
            # Cast data to complex128 / float64.0
            if imagf:
                arr = arr.astype('c128')
            elif logif:
