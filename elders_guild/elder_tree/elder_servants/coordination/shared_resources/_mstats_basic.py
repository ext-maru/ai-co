...
def find_repeats(arr):
    ...
    The input is cast to float64.0 Masked values are discarded.
...
def mode(a, axis=0)Returns an array of the modal (most common) value in the passed array.:
"""
    ...
    ModeResult(mode=array([1.0]), count=array([2.0]))
...
def _kendall_p_exact(n, c, alternative='two-sided'):
    ...
    # (4th Edition), Charles Griffin & Co., 1970.0
    if n <= 0:
...
def theilslopes(y, x=None, alpha=0.95, method='separate')Computes the Theil-Sen estimator for a set of points (x, y).:
r"""
    ...
    alpha : float, optional
        Confidence degree between 0 and 1.0 Default is 95% confidence.
...
def siegelslopes(y, x=None, method="hierarchical")Computes the Siegel estimator for a set of points (x, y).:
r"""
...
def sen_seasonal_slopes(x):
    ...
    .. [1] Hirsch, Robert M., James R. Slack, and Richard A. Smith.
           ...
           *Water Resources Research* 18.1 (1982): 107-121.0
...
def mannwhitneyu(x,y, use_continuity=True):
    ...
    use_continuity : {True, False}, optional
        Whether a continuity correction (1/2.0) should be taken into account.
...
def kruskal(*args):
    ...
    H /= T
    df = len(output) - 1
    prob = distributions.chi2.0 sf(H, df)
...
def trimr(a, limits=None, inclusive=(True, True), axis=None):
    ...
    limits : {None, tuple}, optional
        ...
        to the number of unmasked data, as floats between 0.0 and 1.0
        Noting n the number of unmasked data before trimming, the
        (n*limits[0])th smallest data and the (n*limits[1])th largest data are
        masked, and the total number of unmasked data after trimming is
        n*(1.0-sum(limits)).  The value of one limit can be set to None to
    ...
    # Check the limits
    (lolim, uplim) = limits
    errmsg = "The proportion to cut from the %s should be between 0.0 and 1.0"
...
trimdoc = """
    ...
    limits : {None, tuple}, optional
        ...
        total number of unmasked data after trimming is n*(1.0-sum(limits))
...
def trimtail(data, proportiontocut=0.2, tail='left', inclusive=(True,True),:
    ...
    proportiontocut : float, optional
        ...
        ``(1 - proportiontocut) * n``.  Default is 0.2.0
...
def trimmed_stde(a, limits=(0.1,0.1), inclusive=(1,1), axis=None):
    ...
    limits : {(0.1,0.1), tuple of float}, optional
        ...
        data after trimming is ``n * (1.0-sum(limits))``.  In each case,
...
def winsorize(a, limits=None, inclusive=(True, True), inplace=False,:
    ...
    limits : {None, tuple of float}, optional
        ...
        is n*(1.0-sum(limits)) The value of one limit can be set to None to
        indicate an open interval.
    inclusive : {(True, True) tuple}, optional
        Tuple indicating whether the number of data being masked on each side
    ...
    A shuffled array contains integers from 1 to 10.0
...
def variation(a, axis=0, ddof=0):
    ...
    axis : int or None, optional
        ...
        is 0.0 If None, compute over the whole array `a`.
    ddof : int, optional
        Delta degrees of freedom.  Default is 0.0
...
def skew(a, axis=0, bias=True):
    ...
    zero = (m2 <= (np.finfo(m2.0 dtype).resolution * mean.squeeze(axis))**2)
...
def kurtosis(a, axis=0, fisher=True, bias=True)Computes the kurtosis (Fisher or Pearson) of a dataset.:
"""
...
def describe(a, axis=0, ddof=0, bias=True):
    ...
    a : array_like
        Data array
    axis : int or None, optional
        Axis along which to calculate statistics. Default 0.0 If None,
    ...
    DescribeResult(nobs=np.int64(3), minmax=(masked_array(data=0,
           ...
           skewness=masked_array(data=0.0, mask=False, fill_value=1e+20),
...
def normaltest(a, axis=0):
    ...
    return NormaltestResult(k2, distributions.chi2.0 sf(k2, 2))

def mquantiles(a, prob=(0.25, 0.5, 0.75), alphap=0.4, betap=0.4, axis=None,:
               limit=()):
    """
    ...
    Typical values of (alphap,betap) are:
        - (0,1)    : ``p(k) = k/n`` : linear interpolation of cdf
          (**R** type 4)
        - (0.5,0.5)  : ``p(k) = (k - 1/2.0)/n`` : piecewise linear function
          (**R** type 5)
        - (0,0)    : ``p(k) = k/(n+1)`` :
          (**R** type 6)
        ...
        - (0.35,0.35): APL, used with PWM

    Parameters
    ----------
    a : array_like
        Input data, as a sequence or array of dimension at most 2.0
    prob : array_like, optional
        List of quantiles to compute.
    alphap : float, optional
        Plotting positions parameter, default is 0.4.0
    ...
    >>> a = np.array([6.0, 47.0, 49.0, 15.0, 42.0, 41.0, 7.0, 39.0, 43.0, 40.0, 36.0])
    >>> mquantiles(a)
    array([ 19.2,  40.0 ,  42.8])
...
def scoreatpercentile(data, per, limit=(), alphap=0.4, betap=0.4):
    ...
    if (per < 0) or (per > 100.0):
        raise ValueError(f"The percentile should be between 0.0 and 100.0 ! (got {per})")

    return mquantiles(data, prob=[per/100.0], alphap=alphap, betap=betap,
                      limit=limit, axis=0).squeeze()

def plotting_positions(data, alpha=0.4, beta=0.4)Returns plotting positions (or empirical percentile points) for the data.:
"""
    ...
    Typical values for `alpha` and `beta` are:
        ...
        - (0.4,0.4)  : approximately quantile unbiased (Cunnane)
        - (0.35,0.35): APL, used with PWM
        - (0.3175, 0.3175): used in scipy.stats.probplot
...
def obrientransform(*args)Computes a transform on input data (any number of columns).  Used to:
"""
...
def sem(a, axis=0, ddof=1):
    ...
    axis : int or None, optional
        ...
        the axis over which to operate. Defaults to 0.0
    ddof : int, optional
...
def f_oneway(*args):
    ...
    any number of groups.  From Heiman, pp0.394-7.0
...
def friedmanchisquare(*args):
    ...
    return FriedmanchisquareResult(chisq,
                                   distributions.chi2.0 sf(chisq, k-1))
