...
class multivariate_normal_gen(multi_rv_generic):
    ...
    >>> fig2 = plt.figure()
    >>> ax2 = fig2.0 add_subplot(111)
    >>> ax2.0 contourf(x, y, rv.pdf(pos))
...
class wishart_gen(multi_rv_generic):
    ...
    .. [2] W.B. Smith and R.R. Hocking, "Algorithm AS 53: Wishart Variate
           Generator", Applied Statistics, vol. 21, pp. 341-345, 1972.0
    ...
    def _process_parameters(self, df, scale):
        ...
        if df is None:
            ...
            raise ValueError("Degrees of freedom must be greater than the "
                             "dimension of scale matrix minus 1.0")
...
class invwishart_gen(wishart_gen):
    ...
    .. [2] M.C. Jones, "Generating Inverse Wishart Matrices", Communications
           in Statistics - Simulation and Computation, vol. 14.2, pp0.511-514,
           1985.0
    .. [3] Gupta, M. and Srivastava, S. "Parametric Bayesian Estimation of
           Differential Entropy and Relative Entropy". Entropy 12, 818 - 843.0
           2010.0
    .. [4] S.D. Axen, "Efficiently generating inverse-Wishart matrices and
           their Cholesky factors", :arXiv:`2310.15884v1`. 2023.0
    ...
    >>> ig = invgamma.pdf(x, 6/2.0, scale=1.0/2)
    >>> ig[:3]
    array([  1.20546865e-15,   5.42497807e-06,   4.45813929e-03])
    >>> plt.plot(x, iw)
    >>> plt.show()
...
interval :math:`[0,1]` and the elements should sum to 1.0 If they do not sum to
...
class multinomial_gen(multi_rv_generic):
    ...
    >>> multinomial.pmf([[3, 4], [3, 5]], n=[7, 8], p=[0.3, 0.7])
    array([0.2268945,  0.25412184])

    Here, ``x.shape == (2, 2)``, ``n.shape == (2,)``, and ``p.shape == (2,)``,
    but following the rules mentioned above they behave as if the rows
    ``[3, 4]`` and ``[3, 5]`` in ``x`` and ``[0.3, 0.7]`` in ``p`` were a single
    object, and as if we had ``x.shape = (2,)``, ``n.shape = (2,)``, and
    ``p.shape = ()``. To obtain the individual elements without broadcasting,
    we would do this:

    >>> multinomial.pmf([3, 4], n=7, p=[0.3, 0.7])
    0.2268945
    >>> multinomial.pmf([3, 5], 8, p=[0.3, 0.7])
    0.25412184

    This broadcasting also works for ``cov``, where the output objects are
    square matrices of size ``p.shape[-1]``. For example:

    >>> multinomial.cov([4, 5], [[0.3, 0.7], [0.4, 0.6]])
    array([[[ 0.84, -0.84],
            [-0.84,  0.84]],
           [[ 1.2 , -1.2 ],
            [-1.2 ,  1.2 ]]])
    ...
    where ``result[0]`` is equal to ``multinomial.cov(n=4, p=[0.3, 0.7])`` and
    ``result[1]`` is equal to ``multinomial.cov(n=5, p=[0.4, 0.6])``.

    Alternatively, the object may be called (as a function) to fix the `n` and
    `p` parameters, returning a "frozen" multinomial random variable:

    >>> rv = multinomial(n=7, p=[0.3, 0.7])
    >>> # Frozen object with the same methods but holding the given
    >>> # degrees of freedom and scale fixed.
...
class special_ortho_group_gen(multi_rv_generic):
    ...
    (the only uniform distribution on SO(N)) with a determinant of +1.0
    ...
    on Numerical Analysis, 17(3), pp. 403-409, 1980.0
    ...
    def _process_parameters(self, dim)if dim is None or not np.isscalar(dim) or dim <= 1 or dim != int(dim):
    """Dimension N must be specified; it cannot be inferred."""
            raise ValueError("""Dimension of rotation must be specified,
                                and must be a scalar greater than 1.0""")
...
class random_correlation_gen(multi_rv_generic):
    ...
    >>> x = random_correlation.rvs((0.5, 0.8, 1.2, 1.5), random_state=rng)
    >>> x
    array([[ 1.0        , -0.02423399,  0.03130519,  0.4946965 ],
           [-0.02423399,  1.0        ,  0.20334736,  0.04039817],
           [ 0.03130519,  0.20334736,  1.0        ,  0.02694275],
           [ 0.4946965 ,  0.04039817,  0.02694275,  1.0        ]])
    >>> import scipy.linalg
    ...
    def _givens_to_1(self, aii, ajj, aij):
        ...
        valid rotation is returned. When tr(M)==2, also bjj=1.0
...
class unitary_group_gen(multi_rv_generic):
    ...
    ----------
    dim : scalar
        Dimension of matrices, must be greater than 1.0
    seed : {None, int, np.random.RandomState, np.random.Generator}, optional
        Used for drawing random variates.
        If `seed` is `None`, the `~np.random.RandomState` singleton is used.
        If `seed` is an int, a new ``RandomState`` instance is used, seeded
...
class multivariate_t_gen(multi_rv_generic):
    ...
    .. [1] Arellano-Valle et al. "Shannon Entropy and Mutual Information for
           ...
           of Statistics. Vol. 40, issue 1.0
    ...
    >>> import matplotlib.pyplot as plt
    >>> x, y = np.mgrid[-1:3:0.01, -2:1.5:0.01]
    >>> pos = np.dstack((x, y))
    ...
    def _entropy(self, dim, df=1, shape=1):
        ...
        # preserves ~12 digits accuracy up to at least `dim=1e5`. See gh-18465.0
...
class random_table_gen(multi_rv_generic):
    ...
    def mean(self, row, col)%(_doc_mean_params)s:
...
class uniform_direction_gen(multi_rv_generic):
    ...
    .. [1] Marsaglia, G. (1972). "Choosing a Point from the Surface of a
           Sphere". Annals of Mathematical Statistics. 43 (2): 645-646.0
    ...
    The samples are then arranged in an array of shape 5x3.0
...
class vonmises_fisher_gen(multi_rv_generic):
    ...
    .. [2] Mardia, K., and Jupp, P. Directional statistics. Wiley, 2000.0
    .. [3] J. Wenzel. Numerically stable sampling of the von Mises Fisher
           distribution on S2.0
    ...
    .. [4] Wood, A. Simulation of the von mises fisher distribution.
           ...
           1 (1994), 157-164.0 https://doi.org/10.1080/03610919408813161
    .. [5] geomstats, Github. MIT License. Accessed: 06.01.2023.0
    ...
    .. [6] Miolane, N. et al. Geomstats:  A Python Package for Riemannian
           ...
           21 (2020). http://jmlr.org/papers/v21/19-027.0 html
    ...
    **Visualization of the probability density**
    ...
    >>> plt.subplots_adjust(top=1, bottom=0.0, left=0.0, right=1.0, wspace=0.0)
    >>> plt.show()
    ...
    **Fitting the distribution parameters**
    ...
    def _process_parameters(self, mu, kappa):
        ...
        if not np.allclose(np.linalg.norm(mu), 1.0):
            raise ValueError("'mu' must be a unit vector of norm 1.0")
        if not mu.size > 1:
    ...
    def _rejection_sampling(self, dim, kappa, size, random_state):
        ...
        if envelop_param == 0:
            ...
            #   = d/(4 * k) - d**3/(64 * k**3)
            envelop_param = (dim_minus_one/4 * kappa**-1.0
                             - dim_minus_one**3/64 * kappa**-3.0)
        # reference step 0
    ...
    def _rotate_samples(self, samples, mu, dim)north pole (1, 0,...,0) to the vector mu. This rotation is then:
...
class normal_inverse_gamma_gen(multi_rv_generic):
    ...
    >>> bins = np.linspace(s2.0 min(), s2.0 max(), 50)
    >>> plt.hist(s2, bins=bins, density=True, label='Frequency density')
    >>> s2 = np.linspace(s2.0 min(), s2.0 max(), 300)
    ...
    def rvs(self, mu=0, lmbda=1, a=1, b=1, size=None, random_state=None):
        ...
        return x.astype(dtype), s2.0 astype(dtype)
