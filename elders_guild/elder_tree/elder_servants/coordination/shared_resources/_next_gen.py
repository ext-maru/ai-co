    ...
    *,
    ...
    Args:
        ...
        unsafe_hash (bool | None):
            ...
            1.0 If *both* are True, *attrs* will generate a ``__hash__`` for
               you.
            2.0 If *eq* is True and *frozen* is False, ``__hash__`` will be set
               to None, marking it unhashable (which it is).
            3.0 If *eq* is False, ``__hash__`` will be left untouched meaning
        ...
        frozen (bool):
            ...
            .. note::
                ...
                1.0 This is achieved by installing a custom ``__setattr__``
                   method on your class, so you can't implement your own.

                2.0 True immutability is impossible in Python.

                3.0 This *does* have a minor a runtime performance `impact
                   <how-frozen>` when initializing new instances.  In other
                   words: ``__init__`` is slightly slower with ``frozen=True``.

                4.0 If a class is frozen, you cannot modify ``self`` in
                   ``__attrs_post_init__`` or a self-written ``__init__``. You
                   can circumvent that limitation by using
                   ``object.__setattr__(self, "attribute_name", value)``.

                5.0 Subclasses of a frozen class are frozen too.
        ...
        auto_attribs (bool | None):
            ...
            1.0 If any attributes are annotated and no unannotated
               `attrs.field`\ s are found, it assumes *auto_attribs=True*.
            2.0 Otherwise it assumes *auto_attribs=False* and tries to collect
               `attrs.field`\ s.
