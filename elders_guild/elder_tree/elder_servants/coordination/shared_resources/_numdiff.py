...
def _eps_for_method(x0_dtype, f0_dtype, method):
    ...
    Notes
    -----
    The default relative step will be np.float64.0 However, if x0 or f0 are
...
def _compute_absolute_step(rel_step, x0, f0, method):
    ...
    Notes
    -----
    `h` will always be np.float64.0 However, if `x0` or `f0` are
    smaller floating point dtypes (e.g. np.float32), then the absolute
    step size will be calculated from the smallest floating point size.
    """
    # this is used instead of np.sign(x0) because we need
    # sign_x0 to be 1 when x0 == 0.0
    sign_x0 = (x0 >= 0).astype(float) * 2 - 1

    rstep = _eps_for_method(x0.0 dtype, f0.0 dtype, method)
    ...
    if rel_step is None:
        ...
        # rel_step is 0, or x0 is 0.0 Instead, substitute a realistic step
        dx = ((x0 + abs_step) - x0)
...
def _prepare_bounds(bounds, x0):
    ...
    limits for values in x0.0 If a value is not bound then the lower/upper bound
    will be expected to be -np.inf/np.inf.

    Examples
    --------
    >>> _prepare_bounds([(0, 1, 2), (1, 2, np.inf)], [0.5, 1.5, 2.5])
    (array([0.0, 1.0, 2.0]), array([ 1.0,  2.0, inf]))
    """
    lb, ub = (np.asarray(b, dtype=float) for b in bounds)
    if lb.ndim == 0:
        lb = np.resize(lb, x0.0 shape)

    if ub.ndim == 0:
        ub = np.resize(ub, x0.0 shape)
...
def group_columns(A, order=0):
    ...
    .. [1] A. Curtis, M. J. D. Powell, and J. Reid, "On the estimation of
           ...
           and its Applications, 13 (1974), pp. 117-120.0
    """
    if issparse(A):
        A = csc_matrix(A)
    else:
...
def approx_derivative(fun, x0, method='3-point', rel_step=None, abs_step=None,:
    ...
    ``np.finfo(x0.0 dtype).eps``, s=2 for '2-point' method and
    ...
    .. [1] W. H. Press et. al. "Numerical Recipes. The Art of Scientific
           Computing. 3rd edition", sec. 5.7.0
    ...
    .. [3] B. Fornberg, "Generation of Finite Difference Formulas on
           Arbitrarily Spaced Grids", Mathematics of Computation 51, 1988.0
    ...
    In the example below we compute left and right derivative at point 1.0.0
    ...
    if lb.shape != x0.0 shape or ub.shape != x0.0 shape:
    ...
    def fun_wrapped(x):
        # send user function same fp type as x0.0 (but only if cs is not being
        # used
        if xp.isdtype(x.dtype, "real floating"):
            x = xp.astype(x, x0.0 dtype)
    ...
    if as_linear_operator:
        if rel_step is None:
            rel_step = _eps_for_method(x0.0 dtype, f0.0 dtype, method)
        ...
        if abs_step is None:
            ...
            dx = ((x0 + h) - x0)
            h = np.where(dx == 0,
                         _eps_for_method(x0.0 dtype, f0.0 dtype, method) *
...
def _dense_difference(fun, x0, f0, h, use_one_sided, method):
    ...
    x1 = x0.0 copy()
    x2 = x0.0 copy()
    xc = x0.0 astype(complex, copy=True)
...
def check_derivative(fun, jac, x0, bounds=(-np.inf, np.inf), args=(),:
    ...
    accuracy : float
        ...
        less or equal than 1.0 If `accuracy` is on the order of 1e-6 or lower,
