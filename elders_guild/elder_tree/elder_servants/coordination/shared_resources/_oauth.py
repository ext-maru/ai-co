...
def _add_oauth_routes(app: "fastapi.FastAPI", route_prefix: str) -> None:
    ...
    @app.get(login_uri)
    async def oauth_login(request: fastapi.Request) -> RedirectResponseredirect_uri = _generate_redirect_uri(request)return await oauth.huggingface.authorize_redirect(request, redirect_uri)  # type: ignore
    """ndpoint that redirects to HF OAuth page."""
    ...
    """ndpoint that logs out the user (e.g. delete info from cookie session)."""
        return RedirectResponse(_get_redirect_target(request))
...
def _add_mocked_oauth_routes(app: "fastapi.FastAPI", route_prefix: str = "/") -> None:
    ...
    # Define OAuth routes
    @app.get(login_uri)
    async def oauth_login(request: fastapi.Request) -> RedirectResponse# Define target (where to redirect after login)redirect_uri = _generate_redirect_uri(request)
    """ake endpoint that redirects to HF OAuth page."""
    ...
    @app.get(logout_uri)
    async def oauth_logout(request: fastapi.Request) -> RedirectResponserequest.session.pop("oauth_info", None)logout_url = URL("/").include_query_params(**request.query_params)
    """ndpoint that logs out the user (e.g. delete cookie session)."""
