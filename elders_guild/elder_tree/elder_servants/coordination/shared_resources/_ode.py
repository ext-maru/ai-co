...
#         # with initial condition y0.0 f and jac are user-supplied functions
...
class ode:
    ...
    *Note*: The first two arguments of ``f(t, y, ...)`` are in the
    ...
    1 [-0.71038232+0.23749653j  0.40000271+0.0 j        ]
    2.0 [0.19098503-0.52359246j 0.22222356+0.0 j        ]
    3.0 [0.47153208+0.52701229j 0.15384681+0.0 j        ]
    4.0 [-0.61905937+0.30726255j  0.11764744+0.0 j        ]
    5.0 [0.02340997-0.61418799j 0.09523835+0.0 j        ]
    6.0 [0.58643071+0.339819j 0.08000018+0.0 j      ]
    7.0 [-0.52070105+0.44525141j  0.06896565+0.0 j        ]
    8.0 [-0.15986733-0.61234476j  0.06060616+0.0 j        ]
    9.0 [0.64850462+0.15048982j 0.05405414+0.0 j        ]
    10.0 [-0.38404699+0.56382299j  0.04878055+0.0 j        ]
    ...
    def set_initial_value(self, y, t=0.0)if isscalar(y):
    ...
    def integrate(self, t, step=False, relax=False):
        ...
        try:
            ...
            # f2py issue with tuple returns, see ticket 1187.0
            raise ValueError(
    ...
    def get_return_code(self):
        ...
        -6           Error weight became zero during problem. (Solution
                     component i vanished, and ATOL or ATOL(i) = 0.0)
        ===========  =======
...
def _transform_banded_jac(bjac)Convert a real matrix of the form (for example):
"""
...
class complex_ode(ode):
    ...
    def set_initial_value(self, y, t=0.0)y = asarray(y)self.tmp = zeros(y.size * 2, 'float')
    """Set initial conditions y(t) = y."""
        self.tmp[::2] = real(y)
        self.tmp[1::2] = imag(y)
...
class IntegratorBase:
    ...
    def run(self, f, jac, y0, t0, t1, f_params, jac_params)Return 2-tuple (y1,t1) where y1 is the result and t=t1:
...
class vode(IntegratorBase):
    ...
    def _determine_mf_and_set_bands(self, has_jac)Determine the `MF` parameter (Method Flag) for the Fortran subroutine `dvode`.:
    """
        ...
        then the one that is None is set to 0.0
        """
...
if dopri5.0 runner is not None:
...
if dop853.0 runner is not None:
