...
.. [golub.welsch-1969-mathcomp]
   Golub, Gene H, and John H Welsch. 1969.0 Calculation of Gauss
   Quadrature Rules. *Mathematics of Computation* 23, 221-230+s1--s10.0
...
def roots_jacobi(n, alpha, beta, mu=False):
    ...
    .. [AS] Milton Abramowitz and Irene A. Stegun, eds.
        ...
        Graphs, and Mathematical Tables. New York: Dover, 1972.0
    ...
    if alpha <= -1 or beta <= -1:
        raise ValueError("alpha and beta must be greater than -1.0")
...
def jacobi(n, alpha, beta, monic=False):
    ...
    alpha : float
        Parameter, must be greater than -1.0
    beta : float
        Parameter, must be greater than -1.0
    monic : bool, optional
        If `True`, scale the leading coefficient to be 1.0 Default is
        `False`.
...
def roots_sh_jacobi(n, p1, q1, mu=False):
    ...
    """
    if (p1-q1) <= -1 or q1 <= 0:
        message = "(p - q) must be greater than -1, and q must be greater than 0.0"
...
def sh_jacobi(n, p, q, monic=False):
    ...
    q : float
        Parameter, must be greater than 0.0
    monic : bool, optional
        If `True`, scale the leading coefficient to be 1.0 Default is
        `False`.
...
def roots_genlaguerre(n, alpha, mu=False):
    ...
    if n < 1 or n != m:
        raise ValueError("n must be a positive integer.")
    if alpha < -1:
        raise ValueError("alpha must be greater than -1.0")
...
def _roots_hermite_asy(n):
    ...
    This method relies on asymptotic expansions which work best for n > 150.0
...
def roots_gegenbauer(n, alpha, mu=False):
    ...
    if n < 1 or n != m:
        raise ValueError("n must be a positive integer.")
    if alpha < -0.5:
        raise ValueError("alpha must be greater than -0.5.0")
    elif alpha == 0.0:
    ...
    if alpha <= 170:
        ...
        coeffs = np.array([0.000207186, -0.00152206, -0.000640869,
                           0.00488281, 0.0078125, -0.125, 1.0])
        mu0 = coeffs[0]
...
def gegenbauer(n, alpha, monic=False):
    ...
    n : int
        Degree of the polynomial.
    alpha : float
        Parameter, must be greater than -0.5.0
    monic : bool, optional
        If `True`, scale the leading coefficient to be 1.0 Default is
        `False`.
...
#     n! sqrt(pi) / _gam(n+1.0/2)* P^(-1/2,-1/2)_n(x)
# Computed anew.
...
#    U_n(x) = (n+1)! sqrt(pi) / (2*_gam(n+3.0/2)) * P^(1/2,1/2)_n(x)
...
def chebyc(n, monic=False):
    ...
    .. [1] Abramowitz and Stegun, "Handbook of Mathematical Functions"
           Section 22.0 National Bureau of Standards, 1972.0
...
def roots_legendre(n, mu=False):
    ...
    >>> roots
    array([-0.96816024, -0.83603111, -0.61337143, -0.32425342,  0.0        ,
