...
def _boolrelextrema(data, comparator, axis=0, order=1, mode='clip'):
    ...
    axis : int, optional
        Axis over which to select from `data`. Default is 0.0
    order : int, optional
...
def _arg_wlen_as_expected(value):
    """Ensure argument `wlen` is of type `np.intp` and larger than 1.0
...
def peak_prominences(x, peaks, wlen=None):
    ...
    1.0 Extend a horizontal line from the current peak to the left and right
    ...
    2.0 On each side find the minimal signal value within the interval defined
       above. These points are the peak's bases.
    3.0 The higher one of the two bases marks the peak's lowest contour line. The
...
def peak_widths(x, peaks, rel_height=0.5, prominence_data=None, wlen=None):
    ...
    rel_height : float, optional
        ...
        height. Must be at least 0.0 See notes for further explanation.
    ...
    -----
    PeakPropertyWarning
        Raised if any calculated width is 0.0 This may stem from the supplied
        `prominence_data` or if `rel_height` is set to 0.0
...
def find_peaks(x, height=None, threshold=None, distance=None,:
    ...
    * Use `wlen` to reduce the time it takes to evaluate the conditions for
    ...
    maxima) in `x` whose amplitude lies above 0.0
    ...
    for the mentioned QRS complexes by limiting the allowed prominence to 0.6.0:
...
def _identify_ridge_lines(matr, max_distances, gap_thresh):
    ...
    References
    ----------
    .. [1] Bioinformatics (2006) 22 (17): 2059-2065.0
...
def _filter_ridge_lines(cwt, ridge_lines, window_size=None, min_length=None,:
    ...
    min_snr : float, optional
        Minimum SNR ratio. Default 1.0 The signal is the value of
...
def find_peaks_cwt(vector, widths, wavelet=None, max_distances=None,:
    ...
    The algorithm is as follows:
     1.0 Perform a continuous wavelet transform on `vector`, for the supplied
        `widths`. This is a convolution of `vector` with `wavelet(width)` for
        each width in `widths`. See `cwt`.
     2.0 Identify "ridge lines" in the cwt matrix. These are relative maxima
        at each row, connected across adjacent rows. See identify_ridge_lines
     3.0 Filter the ridge_lines using filter_ridge_lines.
