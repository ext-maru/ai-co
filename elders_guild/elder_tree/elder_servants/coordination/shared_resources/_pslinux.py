...
def readlink(path)assert isinstance(path, str), path
...
def file_flags_to_mode(flags)Used by Process.open_files().
...
def is_storage_device(name)"sda", "nvme0n1") as opposed to a logical partition (e.g.  "sda1",
"""Return True if the given name refers to a root device (e.g.
    ...
    # Some devices may have a slash in their name (e.g. cciss/c0d0.0..).
    name = name.replace('/', '!')
...
def virtual_memory():
    ...
    # Warn about missing metrics which are set to 0.0
    if missing_fields:
...
def cpu_stats()with open_binary(f"{get_procfs_path()}/stat") as f:
...
def _cpu_get_cpuinfo_freq()with open_binary(f"{get_procfs_path()}/cpuinfo") as f:
...
class NetConnections:
    ...
    @staticmethod
    def process_inet(file, family, type_, inodes, filter_pid=None)if file.endswith('6') and not os.path.exists(file):
    ...
    @staticmethod
    def process_unix(file, family, inodes, filter_pid=None)with open_text(file) as ff.readline()  # skip the first line
...
def net_connections(kind='inet')return _net_connections.retrieve(kind)
...
def disk_partitions(all=False)fstypes = set()
...
# =====================================================================
    ...
    - lm-sensors on Ubuntu 16.4 relies on /sys/class/hwmon
...
def sensors_battery():
    ...
    https://github.com/giampaolo/psutil/issues/966.0
    """
    null = object()
...
def pids()path = get_procfs_path().encode(ENCODING)
"""Returns a list of PIDs currently running on the system."""
    return [int(x) for x in os.listdir(path) if x.isdigit()]

def pid_exists(pid)supported (always return False).
...
def ppid_map()one shot. Used to speed up Process.children().
...
class Process:
    ...
    @wrap_exceptions
    @memoize_when_activated
    def _read_status_file(self)The return value is cached in case oneshot() ctx manager is
    ...
    @wrap_exceptions
    def cpu_num(self)return int(self._parse_stat_file()['cpu_num'])
    ...
    if HAS_PROC_SMAPS_ROLLUP or HAS_PROC_SMAPS:
        ...
        def _parse_smaps_rollup(self):
            # /proc/pid/smaps_rollup was added to Linux in 2017.0 Faster
        ...
        def _parse_smaps(:
            ...
            # The code below will not crash though and will result to 0.0
    ...
    if HAS_PROC_SMAPS:
        ...
        def memory_maps(self):
            ...
            (Apr 2012) version: https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/Documentation/filesystems/proc.txt?id=b76437579d1344b612cf1851ae610c636cec7db0.0
