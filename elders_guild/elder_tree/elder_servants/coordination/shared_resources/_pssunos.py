...
def swap_memory()sin, sout = cext.swap_mem()
...
def cpu_times()ret = cext.per_cpu_times()
"""Return system-wide CPU times as a named tuple."""
    return scputimes(*[sum(x) for x in zip(*ret)])

def per_cpu_times()ret = cext.per_cpu_times()
...
def cpu_stats()ctx_switches, interrupts, syscalls, _traps = cext.cpu_stats()
...
def disk_partitions(all=False):
    ...
    for partition in partitions:
        ...
        if not all:
            ...
            # filter by filesystem having a total size > 0.0
            try:
...
def net_if_stats()ret = cext.net_if_stats()
