...
# http://msdn.microsoft.com/en-us/library/ms686219(v=vs0.85).aspx
# fmt: off
__extra__all__ = [
...
def virtual_memory()mem = cext.virtual_mem()
...
def disk_usage(path)if isinstance(path, bytes):
...
def disk_partitions(all)rawlist = cext.disk_partitions(all)
...
def cpu_times()user, system, idle = cext.cpu_times()
...
def cpu_count_cores()return cext.cpu_count_cores()
"""Return the number of CPU cores in the system."""

def cpu_stats()ctx_switches, interrupts, _dpcs, syscalls = cext.cpu_stats()
...
def net_connections(kind, _pid=-1)connections (as opposed to connections opened by one process only).
...
def net_if_addrs()return cext.net_if_addrs()
...
def win_service_get(name)service = WindowsService(name, None)
...
class WindowsService:  # noqa: PLW1641:
    ...
    def name(self)and can be passed to win_service_get() to get a new
    ...
    def status(self)return self._query_status()['status']
    """Service status as a string."""

    def description(self)return cext.winservice_query_descr(self.name())
...
def is_permission_err(exc)assert isinstance(exc, OSError), exc
...
def convert_oserror(exc, pid=None, name=None)assert isinstance(exc, OSError), exc
...
def retry_error_partial_copy(fun):
    ...
    See: https://stackoverflow.com/questions/4457745#4457745.0
    """
