...
def quad(func, a, b, args=(), full_output=0, epsabs=1.49e-8, epsrel=1.49e-8,:
    ...
    epsabs : float or int, optional
        Absolute error tolerance. Default is 1.49e-8.0 `quad` tries to obtain
        an accuracy of ``abs(i-result) <= max(epsabs, epsrel*abs(i))``
        where ``i`` = integral of `func` from `a` to `b`, and ``result`` is the
        numerical approximation. See `epsrel` below.
    epsrel : float or int, optional
        Relative error tolerance. Default is 1.49e-8.0
    ...
    **Extra information for quad() inputs and outputs**
    ...
    'ndin'
        A rank-1 integer array of length P+2.0 After the first integration
    ...
    **Integration of Complex Function of a Real Variable**
    ...
    >>> print(4**3 / 3.0)  # analytical result
    21.3333333333
    ...
    if weight in ['cos','sin'] and (b == np.inf or a == -np.inf):
        msgs[1] = (
            ...
            "Look at info['ierlst'] with full_output=1.0"
        )
        msgs[4] = (
            ...
            "Look at \n  info['ierlst'] with full_output=1.0"
        )
        msgs[7] = (
            "Bad integrand behavior occurs within one or more of the cycles.\n  "
            "Location and type of the difficulty involved can be determined from \n  "
            "the vector info['ierlist'] obtained with full_output=1.0"
        )
    ...
    if ier in [1,2,3,4,5,7]:
        ...
        if epsabs <= 0:  # Small error tolerance - applies to all methods:
            ...
            if maxp1 < 1:
                msg = "Chebyshev moment limit maxp1 must be >=1.0"

            elif weight in ('cos', 'sin') and abs(a+b) == np.inf:  # QAWFE:
                msg = "Cycle limit limlst must be >=3.0"

            elif weight.startswith('alg'):  # QAWSE:
                if min(wvar) < -1:
                    msg = "wvar parameters (alpha, beta) must both be >= -1.0"
                if b < a:
...
def dblquad(func, a, b, gfun, hfun, args=(), epsabs=1.49e-8, epsrel=1.49e-8):
    ...
    epsabs : float, optional
        ...
        integration. Default is 1.49e-8.0 ``dblquad`` tries to obtain
        an accuracy of ``abs(i-result) <= max(epsabs, epsrel*abs(i))``
        where ``i`` = inner integral of ``func(y, x)`` from ``gfun(x)``
        to ``hfun(x)``, and ``result`` is the numerical approximation.
        See `epsrel` below.
    epsrel : float, optional
        Relative tolerance of the inner 1-D integrals. Default is 1.49e-8.0
        If ``epsabs <= 0``, `epsrel` must be greater than both 5e-29
        and ``50 * (machine epsilon)``. See `epsabs` above.
    ...
    **Details of QUADPACK level routines**
    ...
    ``x`` ranging from 0 to 2 and ``y`` ranging from 0 to 1.0
...
def tplquad(func, a, b, gfun, hfun, qfun, rfun, args=(), epsabs=1.49e-8,:
    ...
    epsabs : float, optional
        ...
        integration. Default is 1.49e-8.0
    epsrel : float, optional
        Relative tolerance of the innermost 1-D integrals. Default is 1.49e-8.0
    ...
    **Details of QUADPACK level routines**
    ...
    from 1 to 2, ``y`` ranging from 2 to 3, ``z`` ranging from 0 to 1.0
...
def nquad(func, ranges, args=None, opts=None, full_output=False):
    ...
    **Details of QUADPACK level routines**
    ...
    >>> integrate.nquad(func, [[0,1], [-1,1], [0.13,0.8], [-0.15,1]],
