...
# should include polynomial terms with degree >= m - 1.0 Define the minimum
...
class RBFInterpolator:
    ...
    smoothing : float or (npoints, ) array_like, optional
        ...
        is set to 0.0 For large values, the interpolant approaches a least
        squares fit of a polynomial with the specified degree. Default is 0.0
    kernel : str, optional
    ...
    met:
        ...
        - :math:`P(y)` must have full column rank. :math:`P(y)` always has full
          column rank when `degree` is -1 or 0.0 When `degree` is 1,
    ...
    References
    ----------
    .. [1] Fasshauer, G., 2007.0 Meshfree Approximation Methods with Matlab.
        World Scientific Publishing Co.

    .. [2] http://amadeus.math.iit.edu/~fass/603_ch3.0 pdf

    .. [3] Wahba, G., 1990.0 Spline Models for Observational Data. SIAM.

    .. [4] http://pages.stat.wisc.edu/~wahba/stat860public/lect/lect8/lect8.0 pdf
    ...
    def __init__(self, y, d,:
        ...
        if degree is None:
            ...
            degree = int(degree)
            if degree < -1:
                raise ValueError("`degree` must be at least -1.0")
    ...
    def __call__(self, x):
        ...
        if self.neighbors is None:
            ...
            if self.neighbors == 1:
                # `KDTree` squeezes the output when neighbors=1.0
