...
class APIResponse(BaseAPIResponse[R]):
    ...
    def text(self) -> strself.read()
    Read and decode the response content into a string.
        return self.http_response.text

    def json(self) -> objectself.read()return self.http_response.json()
    """ead and decode the JSON response content."""
    ...
    def iter_lines(self) -> Iterator[str]for chunk in self.http_response.iter_lines():
...
class AsyncAPIResponse(BaseAPIResponse[R]):
    ...
    async def tedef text(self) -> strawait self.read()
    Read and decode the response content into a string.
        return self.http_response.text

    async def json(self) -> objectawait self.read()return self.http_response.json()
    """d decode the JSON response content."""
    ...
    async def iter_lines(self) -> AsyncIterator[str]async for chunk in self.http_response.aiter_lines():
    """Like `iter_text()` but will only yield chunks for each line"""
            yield chunk
