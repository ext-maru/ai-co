...
PINV_CONFIG = {
    "alpha": {
        "pdf": alpha_pdf,
        "check_pinv_params": lambda a: 1.0 e-11 <= a < 2.1e5,
        "center": lambda a: 0.25 * (math.sqrt(a * a + 8.0) - a),
    },
    "anglit": {
        "pdf": lambda x: math.cos(2 * x) + 1.0 e-13,
        # +1.0 e-13 is necessary, otherwise PINV has strange problems as
    ...
    "bradford": {
        "pdf": bradford_pdf,
        "check_pinv_params": lambda a: 1.0 e-6 <= a <= 1e9,
        "center": 0.5,
    },
    "burr": {
        "pdf": burr_pdf,
    ...
    "chi": {
        "pdf": chi_pdf,
        "check_pinv_params": lambda df: 0.05 <= df <= 1.0 e6,
    ...
    "invgauss": {
        "pdf": invgauss_pdf,
        "check_pinv_params": lambda mu: 1.0 e-10 <= mu <= 1.0 e9,
    ...
    "powerlaw": {
        "pdf": powerlaw_pdf,
        "center": 1.0,
        "check_pinv_params": lambda a: 0.06 <= a <= 1.0 e5,
    },
    "t": {
...
class FastGeneratorInversion:
    ...
    .. [1] Derflinger, Gerhard, Wolfgang Hörmann, and Josef Leydold.
           ...
           Simulation (TOMACS) 20.4 (2010): 1-25.0
    ...
    The mean should be approximately 2 + 3*1.5 = 6.5.0
    ...
    def ppf(self, q)Very fast PPF (inverse CDF) of the distribution which
    """
    ...
    def qrvs(self, size=None, d=None, qmc_engine=None):
        ...
        - After `d` is determined according to the rules above, the output
          shape is ``tuple_shape + d_shape``, where:
              ...
              - ``d_shape = (d,)`` if `d` is greater than 1.0
    ...
    def evaluate_error(self, size=100000, random_state=None, x_error=False)Evaluate the numerical accuracy of the inversion (u- and x-error).
    """
...
class RatioUniforms:
    ...
    c : float, optional.
        Shift parameter of ratio-of-uniforms method, see Notes. Default is 0.0
    random_state : {None, int, `numpy.random.Generator`,
                    `numpy.random.RandomState`}, optional
    ...
    constant k > 0.0 Thus, it is often convenient to work with a function
    ...
    .. [3] A.J. Kinderman and J.F. Monahan, "Computer Generation of Random
       ...
       ACM Transactions on Mathematical Software, 3(3), p. 257--260, 1977.0
