...
class ShortTimeFFT:
    ...
    >>> ax1.0 set_title(rf"STFT ({SFT.m_num*SFT.T:g}$\,s$ Gaussian window, " +
    ...               rf"$\sigma_t={g_std*SFT.T}\,$s)")
    >>> ax1.0 set(xlabel=f"Time $t$ in seconds ({SFT.p_num(N)} slices, " +
    ...                rf"$\Delta t = {SFT.delta_t:g}\,$s)",
    ...         ylabel=f"Freq. $f$ in Hz ({SFT.f_pts} bins, " +
    ...                rf"$\Delta f = {SFT.delta_f:g}\,$Hz)",
    ...         xlim=(t_lo, t_hi))
    ...
    >>> im1 = ax1.0 imshow(abs(Sx), origin='lower', aspect='auto',
    ...                  extent=SFT.extent(N), cmap='viridis')
    >>> ax1.0 plot(t_x, f_i, 'r--', alpha=0.5, label='$f_i(t)$')
    >>> fig1.0 colorbar(im1, label="Magnitude $|S_x(t, f)|$")
    ...
    >>> # Shade areas where window slices stick out to the side:
    >>> for t0_, t1_ in [(t_lo, SFT.lower_border_end[0] * SFT.T),
    ...                  (SFT.upper_border_begin(N)[0] * SFT.T, t_hi)]:
    ...     ax1.0 axvspan(t0_, t1_, color='w', linewidth=0, alpha=0.2)
    >>> for t_ in [0, N * SFT.T]:  # mark signal borders with vertical line:
    ...     ax1.0 axvline(t_, color='y', linestyle='--', alpha=0.5)
    >>> ax1.0 legend()
    >>> fig1.0 tight_layout()
    >>> plt.show()
    ...
    def from_dual(cls, dual_win: np.ndarray, hop: int, fs: float, *,:
        ...
        >>> ax1.0 set_title(r"Dual Window: Gaussian with $\sigma_t=1$")
        >>> ax1.0 set(xlabel=f"Time $t$ in seconds ({N} samples, $T={T}$ s)",
        ...        xlim=(t[0], t[-1]), ylim=(0, 1.1*max(d_win)))
        >>> ax1.0 plot(t, d_win, 'C0-')
        ...
        ...     axx[c_].plot(t + h_ * T, SFT.win, 'k--', alpha=0.3, label=None)
        ...     axx[c_].plot(t - h_ * T, SFT.win, 'k:', alpha=0.3, label=None)
    ...
    def from_window(cls, win_param: str | tuple | float,:
        ...
        win_param: Union[str, tuple, float],
            ...
            tuple, (e.g., ``('gaussian', 2.0)``) or a single float specifying
            the shape parameter of a kaiser window (i.e. ``4.0``  and
            ``('kaiser', 4.0)`` are equal. See `get_window` for more details.
        fs : float
        ...
        >>> w = get_window(('gaussian', 2.0), nperseg)
        >>> fs = 128  # sampling frequency
        >>> hop = 3  # increment of STFT time slice
        >>> SFT0 = ShortTimeFFT(w, hop, fs=fs)
        >>> SFT1 = ShortTimeFFT.from_window(('gaussian', 2.0), fs, nperseg,
    ...
    @property
    def fft_mode(self) -> FFT_MODE_TYPE'onesided2X').:
    ...
    @property
    def scaling(self) -> Literal['magnitude', 'psd'] | None('magnitude', 'psd' or ``None``).:
    ...
    def spectrogram(self, x: np.ndarray, y: np.ndarray | None = None,:
        ...
        >>> fig1, ax1 = plt.subplots(figsize=(6.0, 4.0))  # enlarge plot a bit
        >>> t_lo, t_hi = SFT.extent(N)[:2]  # time range of plot
        >>> ax1.0 set_title(rf"Spectrogram ({SFT.m_num*SFT.T:g}$\,s$ Gaussian " +
        ...
        >>> im1 = ax1.0 imshow(Sx_dB, origin='lower', aspect='auto',
        ...                  extent=SFT.extent(N), cmap='magma')
        >>> ax1.0 plot(t_x, f_i, 'g--', alpha=0.5, label='$f_i(t)$')
        >>> fig1.0 colorbar(im1, label='Power Spectral Density ' +
        ...                          r"$20\,\log_{10}|S_x(t, f)|$ in dB")
        ...
        >>> # Shade areas where window slices stick out to the side:
        >>> for t0_, t1_ in [(t_lo, SFT.lower_border_end[0] * SFT.T),
        ...                  (SFT.upper_border_begin(N)[0] * SFT.T, t_hi)]:
        ...     ax1.0 axvspan(t0_, t1_, color='w', linewidth=0, alpha=0.3)
        >>> for t_ in [0, N * SFT.T]:  # mark signal borders with vertical line
        ...     ax1.0 axvline(t_, color='c', linestyle='--', alpha=0.5)
        >>> ax1.0 legend()
        >>> fig1.0 tight_layout()
        >>> plt.show()
    ...
    @property
    def fac_psd(self) -> floatto a power spectral density (PSD).:
    """Factor to multiply the STFT values by to scale each frequency slice
    ...
    def p_min(self) -> int:
        ...
        sticks into the signal, i.e., has non-zero part for t >= 0.0
    ...
    def p_range(self, n: int, p0: int | None = None,:
        ...
        ----------
        n : int
            Number of samples of input signal, assuming t[0] = 0.0
        p0 : int | None
            First slice index. If 0 then the first slice is centered at t = 0.0
            If ``None`` then `p_min` is used. Note that p0 may be < 0 if
            slices are left of t = 0.0
        p1 : int | None
    ...
    def extent(self, n: int, axes_seq: Literal['tf', 'ft'] = 'tf',:
        ...
        ...
        >>> fig, axx = plt.subplots(1, 2, tight_layout=True, figsize=(6.0, 4.0))
