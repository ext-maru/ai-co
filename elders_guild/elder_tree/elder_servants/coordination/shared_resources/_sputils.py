...
def upcast_char(*args)t = _upcast_memo.get(args)
...
def getdata(obj, dtype=None, copy=False) -> np.ndarrayThis is a wrapper of `np.array(obj, dtype=dtype, copy=copy)`
...
def safely_cast_index_arrays(A, idx_dtype=np.int32, msg=""):
    ...
    idx_dtype : dtype
        ...
        Most of scipy.sparse uses either int64 or int32.0
    msg : string, optional
...
def get_index_dtype(arrays=(), maxval=None, check_contents=False)Based on input (integer) arrays `a`, determine a suitable index data
"""
...
def get_sum_dtype(dtype: np.dtype) -> np.dtypeif dtype.kind == 'u' and np.can_cast(dtype, np.uint):
...
defdef isscalarlike(x) -> boolreturn np.isscalar(x) or (isdense(x) and x.ndim == 0):
    """ either a scalar, an array scalar, or a 0-dim array?"""
