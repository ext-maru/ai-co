...
class EmpiricalDistributionFunction:
    ...
    def confidence_interval(self, confidence_level=0.95, *, method='linear'):
        ...
        message = "`confidence_level` must be a scalar between 0 and 1.0"
...
def ecdf(sample: "npt.ArrayLike | CensoredData") -> ECDFResult:
    ...
    **Right-censored Data**
    ...
    >>> res = stats.ecdf(sample)
    >>> res.sf.quantiles
    array([37.0, 43.0, 47.0, 56.0, 60.0, 62.0, 71.0, 77.0, 80.0, 81.0])
    >>> res.sf.probabilities
    array([1.0   , 1.0   , 0.875, 0.75 , 0.75 , 0.75 , 0.75 , 0.5  , 0.25 , 0.0   ])
...
def _ecdf_uncensored(sample):
    ...
    # [1]0.81 "the fraction of [observations] that are less than or equal to x
    events = np.cumsum(counts)
    n = sample.size
    cdf = events / n

    # [1]0.89 "the relative frequency of the sample that exceeds x in value"
    sf = 1 - cdf
