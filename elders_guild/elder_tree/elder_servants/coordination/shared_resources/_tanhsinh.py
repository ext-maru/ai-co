...
def tanhsinh(f, a, b, *, args=(), log=False, maxlevel=None, minlevel=2,:
    ...
    .. [4] Takahasi, Hidetosi, and Masatake Mori. "Double exponential formulas for
           ...
           Mathematical Sciences 9.3 (1974): 721-741.0
    ...
    def check_termination(work)stop = xp.zeros(work.Sn.shape, dtype=bool)
...
def _compute_pair(k, h0, xp):
    # Compute the abscissa-weight pairs for each level k. See [1] page 9.0
    ...
    # When level k == 0, the zeroth xj corresponds with xj = 0.0 To simplify
...
def _estimate_error(work, xp):
    ...
    # about whether it starts at level 0 or level 1.0 We start at level 0, so
