...
def annotated_type(tp: Any, /) -> Any | Nonereturn tp.__origin__ if typing_objects.is_annotated(get_origin(tp)) else None:
"""Return the type of the `Annotated` special form, or `None`."""

def unpack_type(tp: Any, /) -> Any | Nonereturn get_args(tp)[0] if typing_objects.is_unpack(get_origin(tp)) else None:
...
def parent_frame_namespace(*, parent_depth: int = 2, force: bool = False) -> dict[str, Any] | None:
    ...
    # https://github.com/pydantic/pydantic/pull/10113#discussion_r1714981531.0
...
if sys.version_info >= (3, 10):
    ...
    the implementation in CPython 3.10.8.0
    """
