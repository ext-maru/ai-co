...
def _determine_next_job(status: LargeUploadStatus) -> Optional[Tuple[WorkerJob, List[JOB_ITEM_T]]]:
    with status.lock:
        ...
        # 2.0 Commit if at least 100 files are ready to commit
        elif status.nb_workers_commit == 0 and status.queue_commit.qsize() >= 150:
            status.nb_workers_commit += 1

            return (WorkerJob.COMMIT, _get_n(status.queue_commit, status.target_chunk()))

        # 3.0 Get upload mode if at least 100 files
        ...
        # 4.0 Preupload LFS file if at least 1 file and no worker is preuploading LFS
        ...
        # 5.0 Compute sha256 if at least 1 file and no worker is computing sha256
        ...
        # 7.0 Preupload LFS file if at least 1 file
        ...
        # 8.0 Compute sha256 if at least 1 file
        elif status.queue_sha256.qsize() > 0:
            status.nb_workers_sha256 += 1

            return (WorkerJob.SHA256, _get_one(status.queue_sha256))

        # 9.0 Get upload mode if at least 1 file
        ...
        # 11.0 Commit if at least 1 file all other queues are empty and all workers are waiting
        ...
        # 12.0 If all queues are empty, exit
        ...
        # 13.0 If no task is available, wait
        else:
            status.nb_workers_waiting += 1
