...
def _lazyselect(condlist, choicelist, arrays, default=0)Mimic `np.select(condlist, choicelist)`.
"""
    ...
    array([   0.0,    1.0,    4.0,   0.0,   64.0,  125.0])

    >>> a = -np.ones_like(x)
    >>> _lazyselect([x < 3, x > 3],
    ...             [lambda x, a: x**2, lambda x, a: a * x**3],
    ...             (x, a), default=np.nan)
    array([   0.0,    1.0,    4.0,   nan,  -64.0, -125.0])
...
# https://github.com/django/django/pull/4846.0
...
def _argmin(a, keepdims=False, axis=None):
    ...
    If axis is not None, a.shape[axis] must be greater than 0.0
    """
...
def _dict_formatter(d, n=0, mplus=1, sorter=None):
    ...
    if isinstance(d, dict):
        ...
        # By default, NumPy arrays print with linewidth=76.0 `n` is
