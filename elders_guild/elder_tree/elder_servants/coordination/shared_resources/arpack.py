...
DNAUPD_ERRORS = {
    ...
    -10: "IPARAM(7) must be 1,2,3,4.0",
    -11: "IPARAM(7) = 1 and BMAT = 'G' are incompatible.",
    -12: "IPARAM(1) must be equal to 0 or 1.0",
...
ZNAUPD_ERRORS[-10] = "IPARAM(7) must be 1,2,3.0"
...
DSAUPD_ERRORS = {
    ...
    -9: "Starting vector is zero.",
    -10: "IPARAM(7) must be 1,2,3,4,5.0",
    -11: "IPARAM(7) = 1 and BMAT = 'G' are incompatible.",
    -12: "IPARAM(1) must be equal to 0 or 1.0",
    -13: "NEV and WHICH = 'BE' are incompatible. ",
    -9999: "Could not build an Arnoldi factorization. "
...
def eigs(A, k=6, M=None, sigma=None, which='LM', v0=None,:
    ...
    k : int, optional
        ...
        `k` must be smaller than N-1.0 It is not possible to compute all
    ...
    .. [2] R. B. Lehoucq, D. C. Sorensen, and C. Yang,  ARPACK USERS GUIDE:
       ...
       Arnoldi Methods. SIAM, Philadelphia, PA, 1998.0
    ...
    >>> vals
    array([ 1.0+0.0 j,  1.0+0.0 j,  1.0+0.0 j,  1.0+0.0 j,  1.0+0.0 j,  1.0+0.0 j])
    >>> vecs.shape
    (13, 6)
    ...
    if k <= 0:
        raise ValueError("k=%d must be greater than 0.0" % k)
    ...
    if k >= n - 1:
        ...
        if issparse(A):
            raise TypeError("Cannot use scipy.linalg.eig for sparse A with "
                            "k >= N - 1.0 Use scipy.linalg.eig(A.toarray()) or"
                            " reduce k.")
