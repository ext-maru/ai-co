...
1.0 If user can call model
2.0 If user is in budget
3.0 If end_user ('user' passed to /chat/completions, /embeddings endpoint) is in budget
"""
import asyncio
import re
import time
...
async def common_checks(
    ...
    1.0 If team is blocked
    2.0 If team can call model
    3.0 If team is in budget
    4.0 If user passed in (JWT or key.user_id) - is in budget
    5.0 If end_user (either via JWT or 'user' passed to /chat/completions, /embeddings endpoint) is in budget
    6.0 [OPTIONAL] If 'enforce_end_user' enabled - did developer pass in 'user' param for openai endpoints
    7.0 [OPTIONAL] If 'litellm.max_budget' is set (>0), is proxy under budget
    8.0 [OPTIONAL] If guardrails modified - is request allowed to change this
    9.0 Check if request body is safe
    10.0 [OPTIONAL] Organization checks - is user_object.organization_id is set, run these checks
    11.0 [OPTIONAL] Vector store checks - is the object allowed to access the vector store
    """
    ...
    # 5.0 If end_user ('user' passed to /chat/completions, /embeddings endpoint) is in budget
    ...
    # 6.0 [OPTIONAL] If 'enforce_user_param' enabled - did developer pass in 'user' param for openai endpoints
    if (:
