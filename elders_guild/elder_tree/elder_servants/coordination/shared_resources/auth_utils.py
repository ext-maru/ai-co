...
async def pre_db_read_auth_checks(
    ...
    """
    1.0 Checks if request size is under max_request_size_mb (if set)
    2.0 Check if request body is safe (example user has not set api_base in request body)
    3.0 Check if IP address is allowed (if set)
    4.0 Check if request route is an allowed route on the proxy (if set)
    ...
    # Check 1.0 request size
    await check_if_request_size_is_safe(request=request)

    # Check 2.0 Request body is safe
    ...
    # Check 3.0 Check if IP address is allowed
    ...
    # Check 4.0 Check if request route is an allowed route on the proxy
...
def _has_user_setup_sso()Check if the user has set up single sign-on (SSO) by verifying the presence of Microsoft client ID, Google client ID or generic client ID and UI username environment variables.
"""
