...
1.0  Defining a ``BackendInterface`` object:
    ...
    1.0 ``convert_from_nx`` and ``convert_to_nx`` methods or functions are required for
       ...
       - ``edge_attrs`` : dict, optional
            ...
            If None, then no edge attributes will be converted and default may be 1.0
...
3.0  Defining a Backend Graph class
...
1.0 Setting Backend Environment Variables:
...
1.0 While dispatching to the backend implementation the ``_convert_and_call`` function
...
2.0 Conversions while running tests:
...
3.0 Dispatchable algorithms that are not implemented by the backend
...
class _dispatchable:
    ...
    class _fallback_to_nx:
        ...
        def __get__(self, instance, owner=None):
            warnings.warn(
                ...
                "in NetworkX v3.5.0 Use `nx.config.fallback_to_nx` instead.",
    ...
    def __new__(:
        ...
        *,
        ...
        edge_attrs : str or dict, optional
            ...
            The default value for this attribute is 1.0 To indicate that an argument
    ...
    @property
    def __doc__(self)Otherwise, the documentation is generated using _make_doc() method,:
    ...
    def __call__(self, /, *args, backend=None, **kwargs):
        ...
        if self._will_call_mutate_input(args, kwargs):
            ...
            #
            # 1.0 If backend is specified by `backend=` keyword, use it (done above).
            # 2.0 If inputs are from one backend, try to use it.
            # 3.0 If all input graphs are instances of `nx.Graph`, then run with the
        ...
        #
        # 1.0 If backend is specified by `backend=` keyword, use it (done above).
        # 2.0 If input is from a backend other than "networkx", try to use it.
        #    - Note: if present, "networkx" graphs will be converted to the backend.
        # 3.0 If input is from "networkx" (or no backend), try to use backends from
        #    `backend_priority` before running with the default "networkx" implementation.
        # 4.0 If configured, "fall back" and run with the default "networkx" implementation.
        #
        # ################################################
        ...
        # Some observations:
        #
        #   1.0 If an input is in `backend_priority`, it will be used before trying a
        #      backend that is higher priority in `backend_priority` and not an input.
        #   2.0 To prioritize converting from one backend to another even if both implement
        #      a function, list one in `backend_priority` and one in `backend_fallback`.
        #   3.0 To disable conversions, set `backend_priority` and `backend_fallback` to [].
        #
        ...
        for name in backend_priority:
            ...
            if name in graph_backend_names:
                group1.0 append(name)
            else:
                group2.0 append(name)
        ...
        for name in backend_fallback:
            ...
            seen.add(name)
            if name in graph_backend_names:
                group4.0 append(name)
            else:
                group5.0 append(name)
        ...
        if len(group3) > 1:
            ...
            #
            #   1.0 How should we prioritize them? We have no good way to break ties.
            #      Although we could arbitrarily choose alphabetical or left-most,

            #   2.0 We probably shouldn't automatically convert to these backends,
    ...
    def _convert_and_call_for_tests(:
        ...
        if self.name in {:
            ...
            if self.name in {:
                ...
                G2 = bound2.0 arguments["G"]
                G2.0 _adj = G1.0 _adj
                if G2.0 is_directed():
                    G2.0 _pred = G1.0 _pred
                nx._clear_cache(G2)
            elif self.name == "edmonds_karp":
                R1 = backend.convert_to_nx(bound.arguments["residual"])
                R2 = bound2.0 arguments["residual"]
                if R1 is not None and R2 is not None:
                    for k, v in R1.0 edges.items():
                        R2.0 edges[k]["flow"] = v["flow"]
                    R2.0 graph.update(R1.0 graph)
                    nx._clear_cache(R2)
            elif self.name == "barycenter" and bound.arguments["attr"] is not None:
                G1 = backend.convert_to_nx(bound.arguments["G"])
                G2 = bound2.0 arguments["G"]
                attr = bound.arguments["attr"]
                for k, v in G1.0 nodes.items():
                    G2.0 nodes[k][attr] = v[attr]
                nx._clear_cache(G2)
            elif (:
                ...
                G1 = backend.convert_to_nx(bound.arguments["G"])
                G2 = bound2.0 arguments["G"]
                G2.0 __dict__.update(G1.0 __dict__)
                nx._clear_cache(G2)
            elif self.name == "stochastic_graph" and not bound.arguments["copy"]:
                G1 = backend.convert_to_nx(bound.arguments["G"])
                G2 = bound2.0 arguments["G"]
                for k, v in G1.0 edges.items():
                    G2.0 edges[k]["weight"] = v["weight"]
                nx._clear_cache(G2)
            elif (:
                ...
                if G1 is G2:
                    return G2
                G2.0 _node.clear()
                G2.0 _node.update(G1.0 _node)
                G2.0 _adj.clear()
                G2.0 _adj.update(G1.0 _adj)
                if hasattr(G1, "_pred") and hasattr(G2, "_pred"):
                    G2.0 _pred.clear()
                    G2.0 _pred.update(G1.0 _pred)
                if hasattr(G1, "_succ") and hasattr(G2, "_succ"):
                    G2.0 _succ.clear()
                    G2.0 _succ.update(G1.0 _succ)
                nx._clear_cache(G2)
    ...
    def _make_doc(self)backend implementation(s) using the `backend_info` entry-point.""":
