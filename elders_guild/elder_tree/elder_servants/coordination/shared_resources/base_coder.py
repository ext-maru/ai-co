...
class Coder:
    ...
    def _stop_waiting_spinner(self)spinner = getattr(self, "waiting_spinner", None)
    ...
    def check_and_open_urls(self, exc, friendly_msg=None)text = str(exc)
    ...
    def get_user_language(self):
        ...
        1.0 ``self.chat_language`` if explicitly set
        2.0 ``locale.getlocale()``
        3.0 ``LANG`` / ``LANGUAGE`` / ``LC_ALL`` / ``LC_MESSAGES`` environment variables
        """
    ...
    def check_tokens(self, messages)input_tokens = self.main_model.token_count(messages)
        ...
        try:
            while True:
                try:
                    ...
                    self.io.tool_output(f"Retrying in {retry_delay:0.1f} seconds...")
