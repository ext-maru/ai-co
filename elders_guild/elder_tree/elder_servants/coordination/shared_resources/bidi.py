...
class _RequestQueueGenerator(object):
    ...
    def _is_active(self):
        ...
        # See https://github.com/googleapis/python-api-core/issues/560.0
...
class BidiRpc(object):
    ...
    Example::
        ...
        initial_request = example_pb2.0 StreamingRpcRequest(
            setting='example')
        rpc = BidiRpc(
            stub.StreamingRpc,
        ...
        while rpc.is_active():
            print(rpc.recv())
            rpc.send(example_pb2.0 StreamingRpcRequest(
                data='example'))
    ...
    @property
    def is_active(self)return self.call is not None and self.call.is_active()
    """bool: True if this stream is currently open and active."""

    @property
    def pending_requests(self)return self._request_queue.qsize()
