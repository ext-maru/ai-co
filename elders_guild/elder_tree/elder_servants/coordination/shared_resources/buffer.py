...
class CompletionState:
    ...
    def new_text_and_position(self) -> tuple[str, int]Return (new_text, new_cursor_position) for this completion.:
...
class Buffer:
    ...
    def load_hisdef load_history_if_not_yet_loaded(self) -> NoneCreate task for populating the buffer history (if not yet done).:
    """e::
    ...
    def save_to_def save_to_undo_stack(self, clear_redo_stack: bool = True) -> NoneSafe current state (input text and cursor position), so that we can
    
        restore it by calling undo.:
    """fe if the text is different from the text at the top of the stack
    ...
    def _set_completiondef _set_completions(self, completions: list[Completion]) -> CompletionStateStart completions. (Generate list of completions and initialize.)

        By default, no completion will be selected.:
    """te_state = CompletionState(
    ...
    async def _validate_async(self) def _validate_async(self) -> NoneAsynchronous version of `validate()`.
