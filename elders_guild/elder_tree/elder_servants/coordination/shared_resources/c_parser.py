...
class CParser(PLYParser):
    ...
    # To understand what's going on here, read sections A0.8.5 and
    # A0.8.6 of K&R2 very carefully.
    #
    ...
    #
    # K&R2 A0.8.6.2: Array Declarators
    #
    ...
    ##
    ## Grammar productions
    ## Implementation of the BNF defined in K&R2 A0.13
    ##
    ...
    # Note, according to C18 A0.2.2 6.7.10 static_assert-declaration _Static_assert
    ...
    @parameterized(('id', 'ID'), ('typeid', 'TYPEID'), ('typeid_noparen', 'TYPEID'))
        ...
        # version of PLY embedded with pycparser; see PLY Google Code issue 30.0
    ...
    def p_labeled_statement_1(self, p)p[0] = c_ast.Label(p[1], p[3], self._token_coord(p, 1))
    """ labeled_statement : ID COLON pragmacomp_or_statement """

    def p_labeled_statement_2(self, p)p[0] = c_ast.Case(p[2], [p[4]], self._token_coord(p, 1))
    """ labeled_statement : CASE constant_expression COLON pragmacomp_or_statement """

    def p_labeled_statement_3(self, p)p[0] = c_ast.Default([p[3]], self._token_coord(p, 1))
    """ labeled_statement : DEFAULT COLON pragmacomp_or_statement """

    def p_selection_statement_1(self, p)p[0] = c_ast.If(p[3], p[5], None, self._token_coord(p, 1))
    """ selection_statement : IF LPAREN expression RPAREN pragmacomp_or_statement """

    def p_selection_statement_2(self, p)p[0] = c_ast.If(p[3], p[5], p[7], self._token_coord(p, 1))
    ...
    def p_iteration_statement_1(self, p)p[0] = c_ast.While(p[3], p[5], self._token_coord(p, 1))
    """ iteration_statement : WHILE LPAREN expression RPAREN pragmacomp_or_statement """

    def p_iteration_statement_2(self, p)p[0] = c_ast.DoWhile(p[5], p[2], self._token_coord(p, 1))
    """ iteration_statement : DO pragmacomp_or_statement WHILE LPAREN expression RPAREN SEMI """

    def p_iteration_statement_3(self, p)p[0] = c_ast.For(p[3], p[5], p[7], p[9], self._token_coord(p, 1))
    """ iteration_statement : FOR LPAREN expression_opt SEMI expression_opt SEMI expression_opt RPAREN pragmacomp_or_statement """

    def p_iteration_statement_4(self, p)p[0] = c_ast.For(c_ast.DeclList(p[3], self._token_coord(p, 1)),
    ...
    def p_jump_statement_1(self, p)p[0] = c_ast.Goto(p[2], self._token_coord(p, 1))
    """ jump_statement  : GOTO ID SEMI """

    def p_jump_statement_2(self, p)p[0] = c_ast.Break(self._token_coord(p, 1))
    """ jump_statement  : BREAK SEMI """

    def p_jump_statement_3(self, p)p[0] = c_ast.Continue(self._token_coord(p, 1))
    ...
    def p_typedef_name(self, p)p[0] = c_ast.IdentifierType([p[1]], coord=self._token_coord(p, 1))
    """ typedef_name : TYPEID """
    ...
    def p_cast_expression_2(self, p)p[0] = c_ast.Cast(p[2], p[4], self._token_coord(p, 1))
    ...
    def p_postfix_expression_2(self, p)p[0] = c_ast.ArrayRef(p[1], p[3], p[1].coord)
    ...
    def p_identifier(self, p)p[0] = c_ast.ID(p[1], self._token_coord(p, 1))
    """ identifier  : ID """
