...
class CachingFileSystem(AbstractFileSystem):
    ...
    def load_cache(self)self._metadata.load()
    """Read set of stored blocks from file"""
        self._mkcache()
        self.last_cache = time.time()

    def save_cache(self)self._mkcache()
    """Save set of stored blocks from file"""
        self._metadata.save()
        self.last_cache = time.time()
        self._cache_size = None

    def _check_cache(self)self._mkcache()
    ...
    def _check_file(self, path)path = self._strip_protocol(path)
    ...
    def _open(:
        ...
        **kwargs,
        ...
        # for any file that is read over a network, e.g. S3.0
