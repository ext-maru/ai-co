...
class CachedContent:
    ...
    @classmethod
    def _from_obj(cls, obj: CachedContent | protos.CachedContent | dict) -> CachedContentself = cls.__new__(cls)self._proto = protos.CachedContent()
    """reates an instance of CachedContent form an object, without calling `get`."""
        self._update(obj)
        return self

    def _update(self, updates)if isinstance(updates, CachedContent):
    ...
    def delete(self) -> Noneclient = get_default_cache_client():
    """eletes `CachedContent` resource."""
