...
class ExtrasCandidate(Candidate):
    ...
    1.0 It has a unique name, of the form foo[extra]. This causes the resolver
       to treat it as a separate node in the dependency graph.
    2.0 When we're getting the candidate's dependencies,
       a) We specify that we want the extra dependencies as well.
       b) We add a dependency on the base candidate.
          See below for why this is needed.
    3.0 We return None for the underlying InstallRequirement, as the base
       candidate will provide it, and we don't want to end up with duplicates.

    The dependency on the base candidate is needed so that the resolver can't
    decide that it should recommend foo[extra1] version 1.0 and foo[extra2]
    version 2.0.0 Having those candidates depend on foo=1.0 and foo=2.0
    ...
    def __init__(:
        ...
        *,
        ...
        # marker comparison logic specified in PEP 685.0
    ...
    @property
    def name(self) -> strreturn format_name(self.base.project_name, self.extras):
    """he normalised name of the project the candidate refers to"""
