...
class Manager:
    ...
    def run_parallel(self) -> Nonewith _mp_prefork(self.plugins, self.options)pool = _try_initialize_processpool(self.jobs, self.argv):
    """un the checkers in parallel."""
    ...
    def stop(self) -> Noneself._process_statistics():
    """top checking files."""
...
class FileChecker:
    ...
    def _make_processor(self) -> processor.FileProcessor | None:
        try:
            ...
            # as an E902.0 We probably *want* a better error code for this
            # going forward.
