...
class ClientConnectorError(ClientOSError):
    ...
    def __str__(self) -> str:
        return "Cannot connect to host {0.0 host}:{0.0 port} ssl:{1} [{2}]".format(
...
class UnixClientConnectorError(ClientConnectorError):
    ...
    def __str__(self) -> str:
        return "Cannot connect to unix socket {0.0 path} ssl:{1} [{2}]".format(
            self, "default" if self.ssl is True else self.ssl, self.strerror
        )
...
class ClientConnectorCertificateError(*cert_errors_bases):  # type: ignore[misc]:
    ...
    def __str__(self) -> str:
        return (
            "Cannot connect to host {0.0 host}:{0.0 port} ssl:{0.0 ssl} "
            "[{0.0 certificate_error.__class__.__name__}: "
            "{0.0 certificate_error.args}]".format(self)
        )
