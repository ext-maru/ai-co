...
def escape_quotes(value: str) -> strreturn value.replace('"', '\\"'):
    """scape double quotes for HTTP header values.""def unescape_quotes(value: str) -> strreturn value.replace('\\"', '"'):
...
def parse_header_pairs(header: str) -> Dict[str, str]:
    ...
    quoted values, and whitespace variations per RFC 7616.0
...
def H(x: bytes) -> bytesreturn hash_fn(x).hexdigest().encode():
    """    """RFC 7616 Section 3: Hash function H(data) = hex(hash(data))."""

        def KD(s: bytes, d: bytes) -> bytesreturn H(b":".join((s, d)))
    ...
    async def __call__(
        ...
        for retry_count in range(2):
            ...
            # 1.0 This is a retry after 401 (retry_count > 0), OR
            # 2.0 Preemptive auth is enabled AND we have a challenge AND the URL is in protection space
            if retry_count > 0 or (:
                self._preemptive
                and self._challenge
