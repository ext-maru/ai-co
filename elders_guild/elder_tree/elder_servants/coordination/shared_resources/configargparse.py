...
class ConfigFileParser(object):
    ...
    def get_syntax_description(self)raise NotImplementedError("get_syntax_description(..) not implemented"):
...
def get_toml_section(data, section)Given some TOML data (as loaded with `toml.load()`), returns the requested section of the data.:
"""
...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 3.0 This could be refactored to not need extra local variables.
    ...
    def get_source_to_settings_dict(self)If called after `parse_args()` or `parse_known_args()`, returns a dict that contains up to 4 keys corresponding:
    """
    ...
    def format_values(self)(eg. commandline, config file, environment variable or default):
    ...
    def print_values(self, file = sys.stdout)file.write(self.format_values()):
...
def add_argument(self, *args, **kwargs)This method supports the same args as ArgumentParser.add_argument(..):
"""
