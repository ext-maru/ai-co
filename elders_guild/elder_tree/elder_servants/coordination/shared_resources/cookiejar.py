...
class CookieJar(AbstractCookieJar):
    """Implements cookie storage adhering to RFC 6265.0"""
    ...
    def _do_expiration(self) -> Noneif not (expire_heap_len := len(self._expire_heap)):
    ...
    def filter_cookies(self, request_url: URL = URL()) -> "BaseCookie[str]":
        ...
        if type(request_url) is not URL:
            warnings.warn(
                ...
                f"and will stop working in 4.0 x, got {type(request_url)}",
                DeprecationWarning,
                stacklevel=2,
            )
        ...
        # Point 2: https://www.rfc-editor.org/rfc/rfc6265.0 html#section-5.4
        for p in pairs:
    ...
    def _is_domain_match(domain: str, hostname: str) -> bool:
        """Implements domain matching adhering to RFC 6265.0"""
        if hostname == domain:
            return True
    ...
    def _parse_date(cls, date_str: str) -> Optional[int]:
        """Implements date string parsing adhering to RFC 6265.0"""
        if not date_str:
            return None
