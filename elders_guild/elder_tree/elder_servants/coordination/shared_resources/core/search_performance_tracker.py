...
class QueryPatternAnalyzer:
    ...
    def _extract_frequent_terms(self, query: str) -> List[str]:
        ...
        # 上位の語句を返す
        return sorted(
            meaningful_words, key=lambda x: self.term_frequency.get(x, 0), reverse=True
        )[:5]
...
class SearchPerformanceTracker(EldersServiceLegacy):
    ...
    async def track_search_operation(
    ...
    def _assess_result_quality(self, results: List[Dict[str, Any]]) -> float:
        ...
        # 総合品質スコア
        quality = (
            variance_factor * 0.3 + top_relevance * 0.5 + result_count_factor * 0.2
        )
        ...
        return {
            ...
            "quality": {
                ...
                "avg_result_count": statistics.mean(
                    [m.num_results for m in recent_metrics]
                ),
            },
            "cache": {
    ...
    def _analyze_query_patterns(
        self, metrics: List[SearchPerformanceMetrics]
    ) -> Dict[str, Any]:
        """クエリパターンを分析"""
        if not metrics:
            return {}
