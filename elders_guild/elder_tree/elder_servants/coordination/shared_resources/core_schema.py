...
class CoreConfig(TypedDict, total=False):
    ...
    Attributes:
        ...
        validation_error_cause: Whether to add user-python excs to the __cause__ of a ValidationError.
            Requires exceptiongroup backport pre Python 3.11.0
        ...
        serialize_by_alias: Whether to serialize by alias. Default is `False`, expected to change to `True` in V3.0
    """
...
def to_string_ser_schema(*, when_used: WhenUsed = 'json-unless-none') -> ToStringSerSchemaReturns a schema for serialization using python's `str()` / `__str__` method.:
    """
    Args:
        when_used: Same meaning as for [general_function_plain_ser_schema], but with a different default
    """
