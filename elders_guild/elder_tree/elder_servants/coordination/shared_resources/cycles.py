...
def cycle_basis(G, root=None):
    ...
    .. [1] Paton, K. An algorithm for finding a fundamental set of
       cycles of a graph. Comm. ACM 12, 9 (Sept 1969), 514-518.0
...
def simple_cycles(G, length_bound=None):
    ...
    multigraphs, a pair of parallel edges is considered a cycle of length 2.0
    Likewise, self-loops are considered to be cycles of length 1.0  We define
    ...
    .. [1] Finding all the elementary circuits of a directed graph.
       D. B. Johnson, SIAM Journal on Computing 4, no. 1, 77-84, 1975.0
    ...
    .. [3] Enumerating the cycles of a digraph: a new preprocessing strategy.
       G. Loizou and P. Thanish, Information Sciences, v. 27, 163-182, 1982.0
    .. [4] A search strategy for the elementary cycles of a directed graph.
       J.L. Szwarcfiter and P.E. Lauer, BIT NUMERICAL MATHEMATICS,
       v. 16, no. 2, 192-204, 1976.0
...
def _directed_cycle_search(G, length_bound):
    ...
    We generate all cycles of G through binary partition.
        ...
        1.0 Pick a node v in G which belongs to at least one cycle
            a. Generate all cycles of G which contain the node v.
            b. Recursively generate all cycles of G \\ v.

    This is accomplished through the following:

        1.0 Compute the strongly connected components SCC of G.
        2.0 Select and remove a biconnected component C from BCC.  Select a
           non-tree edge (u, v) of a depth-first search of G[C].
        3.0 For each simple cycle P containing v in G[C], yield P.
        4.0 Add the biconnected components of G[C \\ v] to BCC.
...
def _undirected_cycle_search(G, length_bound):
    ...
    We generate all cycles of G through binary partition.
        ...
        1.0 Pick an edge (u, v) in G which belongs to at least one cycle
            a. Generate all cycles of G which contain the edge (u, v)
            b. Recursively generate all cycles of G \\ (u, v)

    This is accomplished through the following:

        1.0 Compute the biconnected components BCC of G.
        2.0 Select and remove a biconnected component C from BCC.  Select a
           non-tree edge (u, v) of a depth-first search of G[C].
        3.0 For each (v -> u) path P remaining in G[C] \\ (u, v), yield P.
        4.0 Add the biconnected components of G[C] \\ (u, v) to BCC.
...
def chordless_cycles(G, length_bound=None):
    ...
    to have a prescribed minimum length; we do not.
        ...
        1.0 We interpret self-loops to be chordless cycles, except in multigraphs
        ...
        2.0 We interpret directed two-cycles to be chordless cycles, except in
           multi-digraphs when any edge in a two-cycle has a parallel copy.

        3.0 We interpret parallel pairs of undirected edges as two-cycles, except
           when a third (or more) parallel edge exists between the two nodes.

        4.0 Generalizing the above, edges with parallel clones may not occur in
           chordless cycles.
    ...
    been modified in the following ways:
        ...
        1.0 Recursion is avoided, per Python's limitations

        2.0 The labeling function is not necessary, because the starting paths
            are chosen (and deleted from the host graph) to prevent multiple
            occurrences of the same path

        3.0 The search is optionally bounded at a specified length

        4.0 Support for directed graphs is provided by extending cycles along
            forward edges, and blocking nodes along forward and reverse edges

        5.0 Support for multigraphs is provided by omitting digons from the set
            of forward edges
    ...
    # edges.
    #
    # 1.0 If we have 2 or more edges in parallel between the nodes (u, v), we
    #    must not construct longer cycles along (u, v).
    # 2.0 If G is not directed, then a pair of parallel edges between (u, v) is a
    #    chordless cycle unless there exists a third (or more) parallel edge.
    # 3.0 If G is directed, then parallel edges do not form cycles, but do
    ...
    if directed:
        ...
        # (w ~ v ~ u), so we use combinations of v's neighbors of length 2.0
        def stems(C, v):
...
def minimum_cycle_basis(G, weight=None):
    ...
    References:
        ...
        [2] de Pina, J. 1995.0 Applications of shortest path methods.
...
def _min_cycle_basis(G, weight):
    ...
    while set_orth:
        ...
        # now update set_orth so that k+1,k+2.0.. th elements are
