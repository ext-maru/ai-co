...
def _chardet_dammit(s: bytes) -> Optional[str]if chardet_module is None or isinstance(s, str):
...
class EncodingDetector:
    ...
    1.0 Encodings you specifically tell EncodingDetector to try first
       (the ``known_definite_encodings`` argument to the constructor).

    2.0 An encoding determined by sniffing the document's byte-order mark.

    3.0 Encodings you specifically tell EncodingDetector to try if
       byte-order mark sniffing fails (the ``user_encodings`` argument to the
       constructor).

    4.0 An encoding declared within the bytestring itself, either in an
       XML declaration (if the bytestring is to be interpreted as an XML
       document), or in a <meta> tag (if the bytestring is to be
       interpreted as an HTML document.)

    5.0 An encoding detected through textual analysis by chardet,
    ...
    def encodings(self) -> Iterator[_Encoding]:
        ...
        # As a last-ditch effort, try utf-8 and windows-1252.0
    ...
    def find_declared_encoding(:
        ...
        if search_entire_document:
            ...
            xml_endpos = 1024
            html_endpos = max(2048, int(len(markup) * 0.5))
...
class UnicodeDammit:
    ...
    def detwingle(:
        ...
        subset ISO-8859-1), embedded in UTF-8.0
