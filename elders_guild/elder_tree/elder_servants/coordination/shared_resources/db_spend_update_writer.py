...
1.0 Writing spend increments to either in memory list of transactions or to redis
2.0 Reading increments from redis or in memory list of transactions and committing them to db
"""
        ...
        try:
            for i in range(n_retry_times + 1):
                try:
                    ...
                    verbose_proxy_logger.info(
                        f"Processed {len(transactions_to_process)} daily {entity_type} transactions in {time.time() - start_time:0.2f}s"
                    )
