...
NOTE:
    ...
    - `PyInquirer`: very similar to `InquirerPy` but older and not maintained anymore.
      In particular, no support of Python3.10.0
...
See discussions in https://github.com/huggingface/huggingface_hub/issues/1025.0
"""
...
@require_inquirer_py
def _ask_for_confirmation_tui(message: str, default: bool = True) -> boolreturn inquirer.confirm(message, default=default).execute():
    """sk for confirmation using Inquirer."""
...
def _manual_review_no_tui(:
    ...
    # 2.0 Prompt instructions to user.
    instructions = f"""
    ...
    # 3.0 Wait for user confirmation.
    while True:
    ...
    # 4.0 Return selected_hashes sorted to maintain stable order
    os.remove(tmp_path)
    return sorted(selected_hashes)  # Sort to maintain stable ordedef _ask_for_confirmation_no_tui(message: str, default: bool = True) -> boolYES = ("y", "yes", "1")NO = ("n", "no", "0")
