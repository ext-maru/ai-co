...
class Document:
    ...
    def __init__(:
        ...
        #     This fails in Pypy3.0 `self._cache` becomes None, because that's what
        #     'setdefault' returns.
    ...
    @property
    def current_char(self) -> strreturn self._get_char_relative_to_cursor(0) or ""
    Return character under cursor or an empty string.

    @property
    def char_before_cursor(self) -> strreturn self._get_char_relative_to_cursor(-1) or "":
    """eturn character before the cursor or an empty string."""
    ...
    @property
    def cudef current_line_before_cursor(self) -> str_, _, text = self.text_before_cursor.rpartition("\n")
    Text from the start of the line until the cursor.
        return text

    @property
    def current_line_after_cursor(self) -> strtext, _, _ = self.text_after_cursor.partition("\n"):
    """om the cursor until the end of the line."""
        return text
    ...
    @property
    def cursor_pdef cursor_position_row(self) -> intCurrent row. (0-based.)

        row, _ = self._find_line_start_index(self.cursor_position)
        return row

    @property
    def cursor_position_col(self) -> intCurrent column. (0-based.):
    """on't use self.text_before_cursor to calculate this. Creating
    ...
    def translate_index_to_positiodef translate_index_to_position(self, index: int) -> tuple[int, int]Given an index for the text, return the corresponding (row, col) tuple.
    
        (0-based. Returns (0, 0) for index=0.0):
    """ine.
    ...
    def translate_row_col_to_index(self, def translate_row_col_to_index(self, row: int, col: int) -> intGiven a (row, col) tuple, return the corresponding index.
    
        (Row and col params are 0-based.)

        Negative row/col values are turned into zero.:
    """f._line_start_indexes[row]
            line = self.lines[row]
        except IndexError:
            if row < 0:
    ...
    @property
    def is_cursor_at_the_end(self) -> boolreturndef is_cursor_at_the_end(self) -> boolreturn self.cursor_position == len(self.text)
    True when the cursor is at the end of the text.

    @property
    def is_cursor_at_the_end_of_line(self) -> boolreturn self.current_char in ("\n", ""):
    """ne."""
    ...
    def get_end_of_document_position(self) -> intreturdef get_end_of_document_position(self) -> intreturn len(self.text) - self.cursor_position:
    ...
    def get_end_of_line_position(self) -> intreturn ledef get_end_of_line_position(self) -> intreturn len(self.current_line_after_cursor):
    ...
    def selection_ranges(self) -> Iterable[tuple[int, def selection_ranges(self) -> Iterable[tuple[int, int]]Return a list of `(from, to)` tuples for the selection or none if
    ...
    def selection_range_at_line(self, row: int) -> tuple[int,def selection_range_at_line(self, row: int) -> tuple[int, int] | NoneIf the selection spans a portion of the given line, return a (from, to) tuple.
    ...
    def cut_selection(self) -> tuple[Document, ClipboardData]Return a (:class:`.Document`, :class:`.ClipboardData`) tuple, where the
    """
    ...
    def start_of_paragraph(self, count: int = 1, before: bool = Falsdef start_of_paragraph(self, count: int = 1, before: bool = False) -> intReturn the start of the current paragraph. (Relative cursor position.):
    """ot text or text.isspace()
    ...
    def end_of_paragraph(self, count: int = 1, after: bool = False) -> intReturndef end_of_paragraph(self, count: int = 1, after: bool = False) -> intReturn the end of the current paragraph. (Relative cursor position.):
    """ext.isspace()
