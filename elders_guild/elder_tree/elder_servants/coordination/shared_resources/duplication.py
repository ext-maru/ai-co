...
def partial_duplication_graph(N, n, p, q, seed=None, *, create_using=None):
    ...
    1.0 A random node, *u*, is picked and a new node, *v*, is created.
    2.0 For each neighbor of *u* an edge from the neighbor to *v* is created
       with probability `p`.:
    3.0 An edge from *u* to *v* is created with probability `q`.
