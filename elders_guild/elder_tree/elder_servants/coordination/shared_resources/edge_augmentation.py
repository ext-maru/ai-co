...
def k_edge_augmentation(G, k, avail=None, weight=None, partial=False):
    ...
    try:
        if k <= 0:
            ...
            # raise NotImplementedError(f'not implemented for k>2.0 k={k}')
...
def partial_k_edge_augmentation(G, k, avail, weight=None):
    ...
    def _edges_between_disjoint(H, only1, only2)only1_adj = {u: set(H.adj[u]) for u in only1}
...
def one_edge_augmentation(G, avail=None, weight=None, partial=False):
    ...
    Equivalent to :func:`k_edge_augmentation` when k=1.0 Adding the resulting
...
def _ordered(u, v)return (u, v) if u < v else (v, u)
...
def unconstrained_bridge_augmentation(G):
    ...
    and add the zipped pairs to A2.0
...
def _minimum_rooted_branching(D, root)arborescence)
...
def _compat_shuffle(rng, input)rng.shuffle(input)
