...
@register_pydub_effect
def normalize(seg, headroom=0.1)headroom is how close to the maximum volume to boost the signal up to (specified in dB)
"""
    """
...
def compress_dynamic_range(seg, threshold=-20.0, ratio=4.0, attack=5.0, release=50.0):
    ...
    Keyword Arguments:
        ...
        ratio - default: 4.0
            ...
            a setting of 4:1 in a pro-audio compressor like the Waves C1.0
...
def invert_phase(seg, channels=(1, 1))channels- specifies which channel (left or right) to reverse the phase of.
"""
...
@register_pydub_effect
def low_pass_filter(seg, cutoff)cutoff - Frequency (in Hz) where higher frequency signal will begin to
"""
...
@register_pydub_effect
def high_pass_filter(seg, cutoff)cutoff - Frequency (in Hz) where lower frequency signal will begin to
"""
...
@register_pydub_effect
def pan(seg, pan_amount)pan_amount should be between -1.0 (100% left) and +1.0 (100% right)
"""
...
@register_pydub_effect
def apply_gain_stereo(seg, left_gain=0.0, right_gain=0.0)left_gain - amount of gain to apply to the left channel (in dB)
"""
