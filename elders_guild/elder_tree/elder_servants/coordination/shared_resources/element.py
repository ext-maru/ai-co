...
class PageElement(object):
    ...
    @property
    def self_and_next_elements(self) -> Iterator[PageElement]return self._self_and(self.next_elements):
    """his PageElement, then all PageElements that were parsed after it."""
    ...
    @property
    def self_and_next_siblings(self) -> Iterator[PageElement]return self._self_and(self.next_siblings):
    """his PageElement, then all of its siblings."""
    ...
    @property
    def decomposed(self) -> boolreturn getattr(self, "_decomposed", False) or False:
    """heck whether a PageElement has been decomposed."""
...
class NavigableString(str, PageElement):
    ...
    # is introduced in 3.8.0
    def __getitem__(self, key: Union[int|slice]) -> strif isinstance(key, str)raise TypeError("string indices must be integers, not '{0}'. Are you treating a NavigableString like a Tag?".format(key.__class__.__name__))return super(NavigableString, self).__getitem__(key)
    """aise an exception """
...
class Tag(PageElement):
    ...
    @string.setter
    def string(self, string: str) -> Noneself.clear()if isinstance(string, NavigableString):
    """eplace the `Tag.contents` of this `Tag` with a single string."""
    ...
    def __getattr__(self, subtag: str) -> Optional[Tag]# print("Getattr %s.%s" % (self.__class__, tag)):
    """alling tag.subtag is the same as calling tag.find(name="subtag")"""
    ...
    def __eq__(self, other: Any) -> booland the same contents (recursively) as `other`.:
    """eturns true iff this Tag has the same name, the same attributes,
        if self is other:
            return True
    ...
    defdef __repr__(self) -> strreturn self.decode():
    """ers this `Tag` as a string."""
    ...
    # Generator methods
    @property
    defdef children(self) -> Iterator[PageElement]return (x for x in self.contents):
    """ate over all direct children of this `PageElement`."""
    ...
    @property
    defdef css(self) -> CSSreturn CSS(self):
    """rn an interface to the CSS selector API."""
    ...
    defdef has_key(self, key: str) -> bool(attributes) was different from __in__ (contents).:
    """ecated method. This was kind of misleading because has_key()
