...
def issequence(seq)Is seq a sequence (ndarray, list or tuple)?
"""
...
def apply_along_axis(func1d, axis, arr, *args, **kwargs)(This docstring should be overwritten)
"""
    """
...
def apply_over_axes(func, a, axes)(This docstring will be overwritten)
"""
    """
    val = asarray(a)
    N = a.ndim
...
def average(a, axis=None, weights=None, returned=False, *,:
    ...
    weights : array_like, optional
        ...
        The only constraint on `weights` is that `sum(weights)` must not be 0.0
    ...
    >>> x = np.ma.arange(6.0).reshape(3, 2)
    >>> x
    masked_array(
      data=[[0.0, 1.0],
            [2.0, 3.0],
...
def mask_rows(a, axis=np._NoValue):
    ...
    This function is a shortcut to ``mask_rowcols`` with `axis` equal to 0.0
...
def mask_cols(a, axis=np._NoValue):
    ...
    This function is a shortcut to ``mask_rowcols`` with `axis` equal to 1.0
...
def in1d(ar1, ar2, assume_unique=False, invert=False):
    ...
    See Also
    --------
    isin       : Version of this function that preserves the shape of ar1.0
...
def cov(x, y=None, rowvar=True, bias=False, allow_masked=True, ddof=None):
    ...
    bias : bool, optional
        ...
        the keyword ``ddof`` in numpy versions >= 1.5.0
...
def _ezclump(mask)Finds the clumps (groups of data with the same values) for a 1D bool array.
"""
