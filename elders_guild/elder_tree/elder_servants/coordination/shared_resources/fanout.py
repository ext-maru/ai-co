...
class FanoutCache:
    ...
    def get(:
        ...
        except (Timeout, sqlite3OperationalError):
            return default
    ...
    def __iter__(self)iterators = (iter(shard) for shard in self._shards)
    """Iterate keys in cache including expired items."""
        return it.chain.from_iterable(iterators)

    def __reversed__(self)iterators = (reversed(shard) for shard in reversed(self._shards))
    """Reverse iterate keys in cache including expired items."""
        return it.chain.from_iterable(iterators)

    def __len__(self)return sum(len(shard) for shard in self._shards)
