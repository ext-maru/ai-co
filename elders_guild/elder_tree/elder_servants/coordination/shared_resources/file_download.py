    ...
    if follow_relative_redirects:
        ...
        if 300 <= response.status_code <= 399:
            ...
            if parsed_target.netloc == "":
                # This means it is a relative 'location' headers, as allowed by RFC 7231.0
...
def xet_get(:
    ...
    **How it works:**
        ...
        1.0 Create a local cache folder at `~/.cache/huggingface/xet/chunk-cache` to store reusable file chunks
        2.0 Download files in parallel:
            2.1.0 Prepare to write the file to disk
            2.2.0 Ask the server "how is this file split into chunks?" using the file's unique hash
                The server responds with:
                - Which chunks make up the complete file
                - Where each chunk can be downloaded from
            2.3.0 For each needed chunk:
...
def _normalize_etag(etag: Optional[str]) -> Optional[str]:
    ...
    more context, see `TestNormalizeEtag` tests and https://github.com/huggingface/huggingface_hub/pull/1428.0
...
def _create_relative_symlink(src: str, dst: str, new_blob: bool = False) -> Nonereturn _create_symlink(src=src, dst=dst, new_blob=new_blob):
...
def _create_symlink(src: str, dst: str, new_blob: bool = False) -> None:
    ...
    - Relative paths seems to be better handled on Windows. Issue was reported 3 times in less than a week when
      ...
      https://github.com/huggingface/diffusers/issues/2729 and https://github.com/huggingface/transformers/pull/22228.0
...
def _cache_commit_hash_for_specific_revision(storage_folder: str, revision: str, commit_hash: str) -> None:
    ...
    if revision != commit_hash:
        ...
        if not ref_path.exists() or commit_hash != ref_path.read_text():
            ...
            # See https://github.com/huggingface/huggingface_hub/issues/1216.0
        ...
        try:
            ...
            if target_dir_free < expected_size:
                warnings.warn(
                    ...
                    f"The expected file size is: {expected_size / 1e6:0.2f} MB. "
                    f"The target location {target_dir} only has {target_dir_free / 1e6:0.2f} MB free disk space."
                )
            return
...
def _hf_hub_download_to_cache_dir(:
    *,
    ...
    # 1.0 we passed local_files_only.
    # 2.0 we don't have a connection
    # 3.0 Hub is down (HTTP 500, 503, 504)
    # 4.0 repo is not found -for example private or gated- and invalid/missing token sent
    # 5.0 Hub is blocked by a firewall or proxy is not set correctly.
...
def _hf_hub_download_to_local_dir(:
    *,
    ...
    if not force_download and paths.file_path.is_file():
        ...
        if local_metadata is None and REGEX_SHA256.0 match(etag) is not None:
