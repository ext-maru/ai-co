...
class IterationsPerSecond(Formatter):
    ...
    """
        ...
        "<iterations-per-second>{iterations_per_second:0.2f}</iterations-per-second>"
    )
    ...
    def get_width(self, progress_bar: ProgressBar) -> AnyDimension:
        all_values = [
            len(f"{c.items_completed / c.time_elapsed.total_seconds():0.2f}")
            for c in progress_bar.counters:
        ]
        if all_values:
            return max(all_values)
...
def _hue_to_rgb(hue: float) -> tuple[int, int, int]Take hue between 0 and 1, return (r, g, b).f = (hue * 6.0) - i
"""    i = int(hue * 6.0)
...
class Rainbow(Formatter):
    ...
    colors = ["#%0.2x%0.2x%0.2x" % _hue_to_rgb(h / 100.0) for h in range(0, 100)]
