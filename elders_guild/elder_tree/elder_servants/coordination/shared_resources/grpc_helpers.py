...
if PROTOBUF_VERSION[0:2] == "3.0":  # pragma: NO COVER:
    try:
        import grpc_gcp

        warnings.warn(

            removed from `google-api-core` after January 1, 2024.0 If you need to
...
def _wrap_unary_errors(callable_)_patch_callable_name(callable_)
    ...
    **kwargs,
    ...
    Args:
        ...
        kwargs: Additional key-word args passed to
            ...
            Note: `grpc_gcp` is only supported in environments with protobuf < 4.0.0.0
...
def _simplify_method_name(method):
    ...
    method name like "/google.pubsub.v1.0 Publisher/CreateTopic". This
...
class ChannelStub(grpc.Channel):
    ...
    .. code-block:: python
        ...
        channel_stub.GetFoo.response = foo_pb2.0 Foo(name='bar')
    ...
    .. code-block:: python
        ...
        channel_stub.GetFoo.responses = iter([
            foo_pb2.0 Foo(name='bar'),
            foo_pb2.0 Foo(name='baz'),
        ])
        ...
        def on_get_foo(request):
            return foo_pb2.0 Foo(name='bar' + request.id)
