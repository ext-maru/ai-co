...
def _hist_bin_stone(x, range)Histogram bin estimator based on minimizing the estimated integrated squared error (ISE).
"""

    The number of bins is chosen by minimizing the estimated ISE against the unknown true distribution.
    The ISE is estimated using cross-validation and can be regarded as a generalization of Scott's rule.
    https://en.wikipedia.org/wiki/Histogram#Scott0.27s_normal_reference_rule

    This paper by Stone appears to be the origination of this rule.
    http://digitalassets.lib.berkeley.edu/sdtr/ucb/text/34.0 pdf
...
def _ravel_and_check_weights(a, weights)a = np.asarray(a)
...
def _get_outer_edges(a, range):
    ...
    if range is not None:
        ...
        # handle empty arrays. Can't determine range, so use 0-1.0
...
def histogram(a, bins=10, range=None, density=None, weights=None):
    ...
    density : bool, optional
        ...
        the *integral* over the range is 1.0 Note that the sum of the
    ...
    *includes* 4.0
    ...
    >>> hist
    array([0.5, 0.0 , 0.5, 0.0 , 0.0 , 0.5, 0.0 , 0.5, 0.0 , 0.5])
    >>> hist.sum()
    2.4999999999999996
