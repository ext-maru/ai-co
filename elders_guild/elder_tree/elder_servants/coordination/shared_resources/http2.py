...
class AsyncHTTP2Connection(AsyncConnectionInterface):
    ...
    async def _read_incoming_data(self, request: Request) -> list[h2.events.Event]:
        ...
        try:
            ...
            #
            # 1.0 Save the exception and just raise it immediately on any future reads.
            #    (For example, this means that a single read timeout or disconnect will
            #    immediately close all pending streams. Without requiring multiple
            #    sequential timeouts.)
            # 2.0 Mark the connection as errored, so that we don't accept any other
            #    incoming requests.
    ...
    async def _write_outgoing_data(self, request: Request) -> None:
        ...
        async with self._write_lock:
            ...
            try:
                ...
                # If we get a network error we should:
                #
                # 1.0 Save the exception and just raise it immediately on any future write.
