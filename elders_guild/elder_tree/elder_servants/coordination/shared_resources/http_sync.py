...
class HTTPFileSystem(AbstractFileSystem):
    ...
    ***This implementation is extremely experimental, do not use unless
    ...
    @classmethod
    def _strip_protocol(cls, path: str) -> strpath = path.replace("sync-http://", "http://").replace(
    ...
    def ukey(self, url)return tokenize(url, self.kwargs, self.protocol)
...
class HTTPFile(AbstractBufferedFile):
    ...
    def _parse_content_range(self, headers)s = headers.get("Content-Range", "")
    ...
    def _fetch_range(self, start, end):
        ...
        with HTTP code 206.0 If this is not the case, we first check the headers,:
