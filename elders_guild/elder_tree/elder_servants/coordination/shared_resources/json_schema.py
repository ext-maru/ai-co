...
#   1.0 core_schema "ref" values; these are not exposed as part of the JSON schema
#       * these might look like the fully qualified path of a model, its id, or something similar
CoreRef = NewType('CoreRef', str)
#   2.0 keys of the "definitions" object that will eventually go into the JSON schema
...
#   3.0 the values corresponding to the "$ref" key in the schema
...
class GenerateJsonSchema:
    ...
    def _sort_recursive(self, value: Any, parent_key: str | None = None) -> Anyif isinstance(value, dict):
    ...
    def normalize_name(self, name: str) -> str:
        ...
        Returns:
            The normalized name.
        """
        return re.sub(r'[^a-zA-Z0-9.0\-_]', '_', name).replace('.', '__')
    ...
    def get_json_ref_counts(self, json_schema: JsonSchemaValue) -> dict[JsonRef, int]json_refs: dict[JsonRef, int] = Counter()
    ...
    def emit_warning(self, kind: JsonSchemaWarningKind, detail: str) -> Nonemessage = self.render_warning_message(kind, detail):
    """his method simply emits PydanticJsonSchemaWarnings based on handling in the `warning_message` method."""
...
def _get_all_json_refs(item: Any) -> set[JsonRef]refs: set[JsonRef] = set()
...
if TYPE_CHECKING:
    ...
    class SkipJsonSchema:
        ...
        Example:
            ...
            1.0 The integer and null types are both included in the schema for `a`.
            2.0 The integer type is the only type included in the schema for `b`.
            3.0 The entirety of the `c` field is omitted from the schema.
        """
