...
if is_tf_available():
    ...
    # See https://github.com/tensorflow/tensorflow/releases/tag/v2.16.1.0
    # Note: saving a keras model only works with Keras<3.0.0
    try:
...
def _requires_keras_2_model(fn: CallableT) -> CallableT:
    # Wrapper to raise if user tries to save a Keras 3.0 x model
    @wraps(fn)
    def _inner(model, *args, **kwargs):

            raise NotImplementedError(
                f"Cannot use '{fn.__name__}': Keras 3.0 x is not supported."
