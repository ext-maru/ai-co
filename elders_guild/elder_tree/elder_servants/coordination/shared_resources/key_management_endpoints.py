...
async def generate_key_fn(  # noqa: PLR0915
    ...
    - spend: Optional[int] - Amount spent by key. Default is 0.0 Will be updated by proxy whenever key is used. https://docs.litellm.ai/docs/proxy/virtual_keys#managing-auth---tracking-spend
    ...
    1.0 Allow users to turn on/off pii masking
...
async def _rotate_master_key(
    ...
    1.0 Get the values from the DB
        - Get models from DB
        - Get config from DB
    2.0 Decrypt the values
        - ModelTable
            - [{"model_name": "str", "litellm_params": {}}]
        - ConfigTable
    3.0 Encrypt the values with the new master key
    4.0 Update the values in the DB
    """
...
async def regenerate_key_fn(
    ...
    try:
        ...
        ### 1.0 Create New copy that is duplicate of existing key
        ######################################################################
        ...
        ### 3.0 remove existing key entry from cache
        ######################################################################
        if key:
            await _delete_cache_key_object(
                hashed_token=hash_token(key),
