...
class LassoGuardrail(CustomGuardrail):
    ...
    def _process_lasso_response(self, response: Dict[str, Any]) -> Noneif response and response.get("violations_detected") is Trueviolated_deputies = self._parse_violated_deputies(response):
    """rocess the Lasso API response and raise exceptions if violations are detected."""
