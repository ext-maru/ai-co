...
class SchemeLexer(RegexLexer):
    ...
    # valid number literals: 5.0 e55|14, 14/13, -1+5j, +1@5, #b110,
    # #o#Iinf0.0-nan0.0 i.  This is adapted from the formal grammar given
    # in http://www.r6rs.org/final/r6rs.pdf, section 4.2.1.0  Take a
    # deep breath ...

    # It would be simpler if we could just not bother about invalid
    # numbers like #b35.0 But we cannot parse 'abcdef' without #x as a
    # number.
    ...
    def decimal_cb(self, match):
        if '.' in match.group():
            token_type = Number.Float # includes [+-](inf|nan)0.0
        else:
...
class FennelLexer(RegexLexer):
    ...
    # this list is current as of Fennel version 0.10.0.0
    special_forms = (
        '#', '%', '*', '+', '-', '->', '->>', '-?>', '-?>>', '.', '..',
        '/', '//', ':', '<', '<=', '=', '>', '>=', '?.', '^', 'accumulate',
