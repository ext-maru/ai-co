...
def lsmr(A, b, damp=0.0, atol=1e-6, btol=1e-6, conlim=1e8,:
    ...
    damp : float
        ...
        is solved without regularization. Default is 0.0
    atol, btol : float, optional
        ...
        atol * norm(A) * norm(r)``.  If both tolerances are 1.e-6 (default),
        the final ``norm(r)`` should be accurate to about 6
        digits. (The final ``x`` will usually have fewer correct digits,
        depending on ``cond(A)`` and the size of LAMBDA.)  If `atol`
        or `btol` is None, a default value of 1.e-6 will be used.
    ...
    conlim : float, optional
        ...
        as large as 1.e+12 (say).  For least-squares problems,
        `conlim` should be less than 1.e+8.0 If `conlim` is None, the
        default value is 1e+8.0  Maximum precision can be obtained by
        setting ``atol = btol = conlim = 0``, but the number of
        iterations may then be excessive. Default is 1e8.0
    ...
    .. [1] D. C.-L. Fong and M. A. Saunders,
           ...
           SIAM J. Sci. Comput., vol. 33, pp. 2950-2971, 2011.0
           :arxiv:`1006.758`
    ...
    >>> b = np.array([1.0, 0.1, -1.0], dtype=float)
    >>> x, istop, itn, normr = lsmr(A, b)[:4]
    >>> istop
    2
    >>> x
    array([ 1.0333333, -0.99666667])
    >>> A.dot(x)-b
    array([ 0.0333333, -0.0333333,  0.0333333])
    >>> normr
    0.05773502691896255
