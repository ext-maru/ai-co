...
def minimize(:
    ...
    **kwargs,
    ...
    .. [1] J. Nocedal and S. J. Wright. *Numerical Optimization*. Springer Ser.
       ...
       2006.0 `doi:10.1007/978-0-387-40065-5
    ...
    .. [2] M. J. D. Powell. A direct search optimization method that models the
       ...
       Analysis*, volume 275 of Math. Appl., pages 51--67.0 Springer, Dordrecht,
       Netherlands, 1994.0 `doi:10.1007/978-94-015-8330-5_4
    ...
    .. math::
        ...
        \begin{aligned}
            ...
            \text{s.t.}                 & \quad 5x_1 - x_2 + x_3 \ge 0,\\
                                        ...
                                        & \quad x_1^2 + x_2^2 + 4x_2 \le x_3.0
        \end{aligned}
...
def _set_default_constants(**kwargs):
    ...
    if constants[Constants.INCREASE_RADIUS_THRESHOLD] <= 1.0:
        raise ValueError(
            "The constant increase_radius_threshold must be greater than 1.0"
        )
    if (:
        ...
        raise ValueError(
            "The constant increase_radius_factor must be greater than 1.0"
        )
    if (:
        ...
        raise ValueError(
            "The constant decrease_radius_threshold must be greater than 1.0"
        )
    if (:
        Constants.INCREASE_RADIUS_FACTOR in constants
    ...
    if (:
        ...
        raise ValueError(
            "The constant large_resolution_threshold must be greater than 1.0"
        )
    if (:
    ...
    if (:
        ...
        raise ValueError(
            "The constant penalty_increase_factor must be greater than 1.0"
        )
    if (:
        Constants.PENALTY_INCREASE_THRESHOLD in constants
    ...
    if constants[Constants.THRESHOLD_RATIO_CONSTRAINTS] <= 1.0:
        raise ValueError(
            "The constant threshold_ratio_constraints must be greater than 1.0"
        )
    constants.setdefault(
    ...
    if constants[Constants.LARGE_GRADIENT_FACTOR] <= 1.0:
        raise ValueError(
            "The constant large_gradient_factor must be greater than 1.0"
        )
    constants.setdefault(
    ...
    if constants[Constants.RESOLUTION_FACTOR] <= 1.0:
        raise ValueError(
            "The constant resolution_factor must be greater than 1.0"
        )
    constants.setdefault(
