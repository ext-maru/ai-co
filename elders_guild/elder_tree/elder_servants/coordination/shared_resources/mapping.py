...
class FSMap(MutableMapping):
    ...
    def clear(self)logger.info("Clear mapping at %s", self.root)
    ...
    def delitems(self, keys)self.fs.rm([self._key_to_str(k) for k in keys])
    """Remove multiple keys from the store"""

    def _key_to_str(self, key)if not isinstance(key, str)# raise TypeError("key must be of type `str`, got `{type(key).__name__}`"
    ...
    def __getitem__(self, key, default=None)k = self._key_to_str(key)
    """Retrieve data"""
        try:
    ...
    def pop(self, key, default=None)result = self.__getitem__(key, default)
    """Pop data"""
        try:
            del self[key]
        except KeyError:
    ...
    def __setitem__(self, key, value)key = self._key_to_str(key)
    """Store value in key"""
    ...
    def __contains__(self, key)path = self._key_to_str(key)
