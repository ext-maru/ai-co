...
class MlflowLogger(CustomLogger):
    ...
    def _handle_stream_event(self, kwargs, response_obj, start_time, end_time):
        ...
        1.0 For the first chunk, start a new span and store it in the map.
        2.0 For subsequent chunks, add the chunk as an event to the span.
        3.0 For the final chunk, end the span and remove the span from the map.
        """
        from mlflow.entities import SpanStatusCode
    ...
    def _construct_input(self, kwargs)inputs = {"messages": kwargs.get("messages")}
