...
class WindowCursor:
    ...
    **Note:**: The current implementation is suited for static and sliding window managers, but it also means
    ...
    def _destroy(self)self.unuse_region()
    ...
    def __copy__(self)cpy = type(self)()
    ...
    def buffer(self)to the window size. Please note that it might be smaller than you requested when calling use_region()
    ...
    def file_size(self)return self._rlist.file_size()
    """:return: size of the underlying file"""

    def path_or_fd(self)return self._rlist.path_or_fd()
...
class SlidingWindowMapManager(StaticWindowMapManager):
    ...
    **Note:** in the current implementation, we will automatically unload windows if we either cannot
    ...
    def __init__(self, window_size=-1, max_memory_size=0, max_open_handles=sys.maxsize)super().__init__(window_size, max_memory_size, max_open_handles)
