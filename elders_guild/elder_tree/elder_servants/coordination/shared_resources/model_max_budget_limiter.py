...
class _PROXY_VirtualKeyModelMaxBudgetLimiter(RouterBudgetLimiting):
    ...
    async def _get_virtual_key_spend_for_model(
        ...
        Lookup model in this order:
            1.0 model: directly look up `model`
            2.0 If 1, does not exist, check if passed as {custom_llm_provider}/model
        """
    ...
    def _get_request_model_budget_config(:
        ...
        1.0 Check if `model` is in `internal_model_max_budget`
        2.0 If not, check if `model` without custom llm provider is in `internal_model_max_budget`
        """
