...
def nonisomorphic_trees(order, create="graph"):
    ...
    create : one of {"graph", "matrix"} (default="graph")
       ...
       if matrix is selected a list of adjacency matrices will be returned.:
          ...
          version 3.5.0 In the future, `nonisomorphic_trees` will yield graph
...
def _layout_to_matrix(layout)given layout (level sequence)."""
...
def _layout_to_graph(layout)given layout(level sequence)"""
