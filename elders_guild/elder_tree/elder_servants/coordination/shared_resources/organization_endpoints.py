...
async def new_organization(
    ...
    verbose_proxy_logger.info(
        f"new_organization_row: {json.dumps(new_organization_row, indent}"
    )
...
async def organization_member_add(
    ...
    1.0 Check if organization exists
    2.0 Creates a new Internal User if the user_id or user_email is not found in LiteLLM_UserTable
    3.0 Add Internal User to the `LiteLLM_OrganizationMembership` table
    """
    try:
        from litellm.proxy.proxy_server import prisma_client
