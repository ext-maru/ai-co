...
# --------------------------------------------------------------------------- #
def _norm(line: str) -> strreturn line.rstrip("\r"):
    """trip CR so comparisons work for both LF and CRLF input."""
...
def find_context(lines: List[str], context: List[str], start: int, eof: bool) -> Tuple[int, int]:
    ...
    return find_context_core(lines, context, startdef peek_next_section(lines: List[str], index: int) -> Tuple[List[str], List[Chunk], int, bool]Parses one section (context, -, + lines) of an Update block.Returns: (context_lines, chunks_in_section, next_index, is_eof)
    """"""
    """
    ...
    return context_lines, chunks, index, idef identify_files_needed(text: str) -> List[str]lines = text.splitlines():
    """s()
...
class PatchCoder(Coder):
    ...
    edit_format = "patch"
    gpt_prompts = PatchProdef get_edits(self) -> List[EditResult]Parses the LLM response content (containing the patch) into a list of
