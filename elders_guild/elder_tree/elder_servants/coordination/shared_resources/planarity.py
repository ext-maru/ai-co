...
@nx._dispatchable(returns_graph=True)
def check_planarity_recursive(G, counterexample=False)planarity_state = LRPlanarity(G)
...
class Interval:
    ...
    def copy(self)return Interval(self.low, self.high)
...
class ConflictPair:
    ...
    def lowest(self, planarity_state)if self.left.empty():
...
class LRPlanarity:
    ...
    def lr_planarity_recursive(self)if self.G.order() > 2 and self.G.size() > 3 * self.G.order() - 6:
