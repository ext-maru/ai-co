...
def as_series(alist, trim=True):
    ...
    [array([1.0]), array([0.0, 1.0, 2.0]), array([0.0, 1.0])]

    >>> pu.as_series([2, [1.1, 0.0]])
    [array([2.0]), array([1.1])]

    >>> pu.as_series([2, [1.1, 0.0]], trim=False)
...
def trimcoef(c, tol=0):
    ...
    array([0.0,  0.0,  3.0,  0.0,  5.0])
    >>> pu.trimcoef((0,0,1e-3,0,1e-5,0,0),1e-3) # item == tol is trimmed
    array([0.0])
    >>> i = complex(0,1) # works for complex
    >>> pu.trimcoef((3e-4,1e-3*(1-i),5e-4,2e-5*(1+i)), 1e-3)
    array([0.003+0.j   , 0.01 -0.01j])
...
def mapdomain(x, old, new):
    ...
    array([-1.0 , -0.6, -0.2,  0.2,  0.6,  1.0 ])
    >>> x_out = pu.mapdomain(x, old_domain, new_domain); x_out
    array([ 0.0        ,  1.25663706,  2.51327412,  3.76991118,  5.2654825, # may vary
            6.28318531])
...
def _vander_nd_flat(vander_fs, points, degrees)Like `_vander_nd`, but flattens the last ``len(degrees)`` axes into a single axis
"""
