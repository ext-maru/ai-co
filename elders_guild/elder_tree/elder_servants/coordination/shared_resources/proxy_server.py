...
def _resolve_pydantic_type(typ) -> Listorigin = get_origin(typ):
"""Resolve the actual TypedDict class from a potentially wrapped type."""
    typs = []
    if origin is Union:  # Check if it's a Union (like Optional):
        for arg in get_args(typ):
...
class ProxyConfig:
    ...
    def initialize_secret_manager(self, key_management_system: Optional[str]):
        ...
        if key_management_system is not None:
            if key_management_system == KeyManagementSystem.AZURE_KEY_VAULT.value:
                ...
                AWSSecretsManagerV2.0 load_aws_secret_manager(use_aws_secret_manager=True)
    ...
    async def _delete_deployment(self, db_models: list) -> int(Helper function of add deployment) -> combined to reduce prisma db calls- Create all up list of model id's (db + config)
    """
...
async def model_list(
    ...
    """
    1.0 Get all models key/user/team has access to
    2.0 Filter out models that are not model access groups
    3.0 Return the models
    """
    if only_model_access_groups is True:
...
async def get_all_team_models(
    ...
    1.0 Get all teams user is in
    2.0 Get all models across all teams
    3.0 Return {"model_id": ["team_id1", "team_id2"]}
    """
...
async def fallback_login(request: Request):
    ...
    PROXY_BASE_URL should be the your deployed proxy endpoint, e.g. PROXY_BASE_URL="https://litellm-production-7002.0 up.railway.app/"
    Example:
    """
