...
@pytest.fixture
def fast(request):  # type: ignore[no-untyped-def]return request.config.getoption("--aiohttp-fast")
"""--fast config option"""
...
def pytest_pycollect_makeitem(collector, name, obj):  # type: ignore[no-untyped-def]if collector.funcnamefilter(name) and inspect.iscoroutinefunction(obj)return list(collector._genfunctions(name, obj))
"""Fix pytest collecting for coroutines."""

def pytest_pyfunc_call(pyfuncitem):  # type: ignore[no-untyped-def]fast = pyfuncitem.config.getoption("--aiohttp-fast")
