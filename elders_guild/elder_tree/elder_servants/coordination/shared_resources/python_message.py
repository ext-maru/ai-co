...
def _PropertyName(proto_field_name)clients can use to get and (in some cases) set the value:
...
def _ReraiseTypeErrorWithFieldName(message_name, field_name)exc = sys.exc_info()[1]if len(exc.args) == 1 and type(exc) is TypeError:
...
def _AddPropertiesForNonRepeatedScalarField(field, cls):
  ...
  def field_setter(self, new_value):
    ...
    try:
      ...
      raise TypeError(
          'Cannot set %s to %0.1024r: %s' % (field.full_name, new_value, e))
...
def _IsPresent(item)value should be included in the list returned by ListFields().""":
...
def _AddClearFieldMethod(message_descriptor, cls)def ClearField(self, field_name):
"""Helper for _AddMessageMethods()."""
    try:
...
def _AddClearExtensionMethod(cls)def ClearExtension(self, field_descriptor)extension_dict._VerifyExtensionHandle(self, field_descriptor):
"""Helper for _AddMessageMethods()."""
...
def _AddHasExtensionMethod(cls)def HasExtension(self, field_descriptor)extension_dict._VerifyExtensionHandle(self, field_descriptor):
"""Helper for _AddMessageMethods()."""
...
def _AddEqualsMethod(message_descriptor, cls)def __eq__(self, other):
"""Helper for _AddMessageMethods()."""
...
def _AddStrMethod(message_descriptor, cls)def __str__(self)return text_format.MessageToString(self):
"""Helper for _AddMessageMethods()."""
  cls.__str__ = __str__

def _AddReprMethod(message_descriptor, cls)def __repr__(self)return text_format.MessageToString(self):
"""Helper for _AddMessageMethods()."""
...
def _AddMergeFromStringMethod(message_descriptor, cls)def MergeFromString(self, serialized)serialized = memoryview(serialized)length = len(serialized)
"""Helper for _AddMessageMethods()."""
    try:
...
def _AddMessageMethods(message_descriptor, cls)_AddListFieldsMethod(message_descriptor, cls)_AddHasFieldMethod(message_descriptor, cls)
...
class _Listener(object):
  ...
  def __init__(self, parent_message)parent_message: The message whose _Modified() method we should call whenwe receive Modified() messages.
"""Args:
    """
...
class _OneofListener(_Listener):
  ...
  def __init__(self, parent_message, field)parent_message: The message whose _Modified() method we should call whenwe receive Modified() messages.
"""Args:
