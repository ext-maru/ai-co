...
def eqp_kktfact(H, c, A, b):
    ...
    # Optimization" p0.452 in Eq. (16.4).
...
def sphere_intersections(z, d, trust_radius,:
    ...
    # Look at Matrix Computation p0.97
...
def box_intersections(z, d, lb, ub,:
    ...
    if not entire_line:
        if tb < 0 or ta > 1:
            ...
            # Restrict intersection interval between 0 and 1.0
            ta = max(0, ta)
            tb = min(1, tb)
...
def inside_box_boundaries(x, lb, ub)return (lb <= x).all() and (x <= ub).all()
"""Check if lb <= x <= ub."""
...
def modified_dogleg(A, Y, b, trust_radius, lb, ub):
    ...
    .. [1] Byrd, Richard H., Mary E. Hribar, and Jorge Nocedal.
           ...
           programming." SIAM Journal on Optimization 9.4 (1999): 877-900.0
    """
    # Compute minimum norm minimizer of 1/2*|| A x + b ||^2.0
    ...
    # Return the best solution among x1 and x2.0
...
def projected_cg(H, c, Z, Y, b, trust_radius=np.inf,:
    ...
    info : Dict
        Dictionary containing the following:
            ...
            - stop_cond : Reason for algorithm termination:
                1.0 Iteration limit was reached;
                2.0 Reached the trust-region boundary;
                3.0 Negative curvature detected;
                4.0 Tolerance was satisfied.
    ...
    .. [1] Gould, Nicholas IM, Mary E. Hribar, and Jorge Nocedal.
           "On the solution of equality constrained quadratic
            ...
            SIAM Journal on Scientific Computing 23.4 (2001): 1376-1395.0
    """
    CLOSE_TO_ZERO = 1e-25
    ...
    H_p = H.dot(p)
    rt_g = norm(g)**2  # g.T g = r.T Z g = r.T g (ref [1]_ p0.1389)
    ...
    for i in range(max_iter):
        ...
        rt_g_next = norm(g_next)**2  # g.T g = r.T g (ref [1]_ p0.1389)
        beta = rt_g_next / rt_g
        p = - g_next + beta*p
