...
def recursive_fill_fields(input, output):
    ...
    * `output` should be at least the same size as `input`
    ...
    >>> a = np.array([(1, 10.0), (2, 20.0)], dtype=[('A', np.int64), ('B', np.float64)])
    >>> b = np.zeros((3,), dtype=a.dtype)
    >>> rfn.recursive_fill_fields(a, b)
    array([(1, 10.0), (2, 20.0), (0,  0.0)], dtype=[('A', '<i8'), ('B', '<f8')])
...
def merge_arrays(seqarrays, fill_value=-1, flatten=False,:
    ...
    --------
    >>> from numpy.lib import recfunctions as rfn
    >>> rfn.merge_arrays((np.array([1, 2]), np.array([10.0, 20.0, 30.0])))
    array([( 1, 10.0), ( 2, 20.0), (-1, 30.0)],
          dtype=[('f0', '<i8'), ('f1', '<f8')])

    >>> rfn.merge_arrays((np.array([1, 2], dtype=np.int64),
    ...         np.array([10.0, 20.0, 30.0])), usemask=False)
     array([(1, 10.0), (2, 20.0), (-1, 30.0)],
             dtype=[('f0', '<i8'), ('f1', '<f8')])
...
def rename_fields(base, namemapper):
    ...
    --------
    >>> from numpy.lib import recfunctions as rfn
    >>> a = np.array([(1, (2, [3.0, 30.0])), (4, (5, [6.0, 60.0]))],
...
def _get_fields_and_offsets(dt, offset=0)Returns a flat list of (dtype, count, offset) tuples of all the
"""
...
def structured_to_unstructured(arr, dtype=None, copy=False, casting='unsafe')Converts an n-D structured array into an (n+1)-D unstructured array.
"""
...
def unstructured_to_structured(arr, dtype=None, names=None, align=False,:
    ...
    array([( 0, ( 1.0,  2), [ 3.0,  4.0]), ( 5, ( 6.0,  7), [ 8.0,  9.0]),
           (10, (11.0, 12), [13.0, 14.0]), (15, (16.0, 17), [18.0, 19.0])],
          dtype=[('a', '<i4'), ('b', [('f0', '<f4'), ('f1', '<u2')]), ('c', '<f4', (2,))])
...
def apply_along_fields(func, arr):
    ...
    array([ 2.66666667,  5.33333333,  8.66666667, 11.0        ])
...
def assign_fields_by_name(dst, src, zero_unassigned=True):
    ...
    zero_unassigned : bool, optional
        ...
        was the behavior of numpy <= 1.13.0 If False, those fields
        are not modified.
    """
...
def stack_arrays(arrays, defaults=None, usemask=True, asrecarray=False,:
    ...
    >>> zz = np.array([('a', 10.0, 100.0), ('b', 20.0, 200.0), ('c', 30.0, 300.0)],
...
def join_by(key, r1, r2, jointype='inner', r1postfix='1', r2postfix='2',:
    ...
    jointype : {'inner', 'outer', 'leftouter'}, optional
        If 'inner', returns the elements common to both r1 and r2.0
        If 'outer', returns the common elements as well as the elements of
        r1 not in r2 and the elements of not in r2.0
