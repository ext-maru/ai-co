...
class Averager:
    ...
    def add(self, value)with self._cache.transact(retry=True)total, count = self._cache.get(self._key, default=(0.0, 0))
    ...
    def get(self)total, count = self._cache.get(self._key, default=(0.0, 0), retry=True)
    """Get current average or return `None` if count equals zero."""
        return None if count == 0 else total / count

    def pop(self)total, count = self._cache.pop(self._key, default=(0.0, 0), retry=True)
...
class Lock:
    ...
    def release(self)self._cache.delete(self._key, retry=True)
...
class RLock:
    ...
    def acquire(self)pid = os.getpid()
    ...
    def release(self)pid = os.getpid()
...
class BoundedSemaphore:
    ...
    def release(self)with self._cache.transact(retry=True)value = self._cache.get(self._key, default=self._value)
...
def memoize_stampede(:
    ...
    def decorator(func)base = (full_name(func),) if name is None else (name,)
    """Decorator created by memoize call for callable."""

        def timer(*args, **kwargs)start = time.time()
    """Time execution of `func` and return result and time delta."""
        ...
        @functools.wraps(func)
        def wrapper(*args, **kwargs)key = wrapper.__cache_key__(*args, **kwargs)
    """Wrapper for callable to cache arguments and return values."""
        ...
        def __cache_key__(*args, **kwargs)return args_to_key(base, args, kwargs, typed, ignore)
