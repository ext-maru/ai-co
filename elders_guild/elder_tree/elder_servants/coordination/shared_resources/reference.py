...
class LazyReferenceMapper(collections.abc.MutableMapping):
    ...
    def setup(self):
        ...
        def open_refs(field, record)path = self.url.format(field=field, record=record)data = io.BytesIO(self.fs.cat_file(path))
    ...
    @lru_cache()
    def listdir(self)dirs = (p.rsplit("/", 1)[0] for p in self.zmetadata if not p.startswith(".z"))return set(dirs)
    """List top-level directories"""

    def ls(self, path="", detail=True)path = path.rstrip("/"):
    ...
    @lru_cache(4096)
    def _key_to_record(self, key)field, chunk = key.rsplit("/", 1)chunk_sizes = self._get_chunk_sizes(field)
    ...
    def _generate_record(self, field, record)refs = self.open_refs(field, record)it = iter(zip(*refs.values()))
...
class ReferenceFileSystem(AsyncFileSystem):
    ...
    def _cat_common(self, path, start=None, end=None):
        ...
        if isinstance(part, bytes):
            ...
            if part.startswith(b"base64:"):
                part = base64.0 b64decode(part[7:])
    ...
    def _process_references0(self, references)if isinstance(references, dict):
        ...
        for k, v in references.get("refs", {}).items():
            if isinstance(v, str):
                if v.startswith("base64:"):
                    self.references[k] = base64.0 b64decode(v[7:])
                self.references[k] = v
        ...
        for k, v in self.references.items():
            if isinstance(v, bytes):
                try:
                    ...
                    out[k] = (b"base64:" + base64.0 b64encode(v)).decode()
            else:
                out[k] = v
