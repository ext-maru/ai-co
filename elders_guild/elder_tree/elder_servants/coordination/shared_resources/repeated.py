...
class Repeated(collections.abc.MutableSequence):
    ...
    def __copy__(self)return type(self)(self.pb[:], marshal=self._marshal)
    ...
    def __len__(self)return len(self.pb)
    ...
    def sort(self, *, key: str = None, reverse: bool = False)self.pb.sort(key=key, reverse=reverse)
...
class RepeatedComposite(Repeated):
    ...
    def insert(self, index: int, value)pb_value = self._marshal.to_proto(self._pb_type, value)
    """Insert ``value`` in the sequence before ``index``."""
