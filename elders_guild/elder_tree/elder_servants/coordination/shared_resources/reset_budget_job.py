...
class ResetBudgetJob:
    ...
    async def reset_budget_for_litellm_endusers(self)Resets the budget for all LiteLLM End-Users (Customers) if their budget has expired
    """
        ...
        try:
            ...
            if len(failed_endusers) > 0:  # If any endusers failed to reset:
                raise Exception(
                    f"Failed to reset {len(failed_endusers)} endusers: {json.dumps(failed_endusers, default}"
                )
    ...
    async def reset_budget_for_litellm_keys(self):
        ...
        try:
            ...
            if len(failed_keys) > 0:  # If any keys failed to reset:
                raise Exception(
                    f"Failed to reset {len(failed_keys)} keys: {json.dumps(failed_keys, default}"
                )
    ...
    async def reset_budget_for_litellm_users(self):
        ...
        try:
            ...
            if len(failed_users) > 0:  # If any users failed to reset:
                raise Exception(
                    f"Failed to reset {len(failed_users)} users: {json.dumps(failed_users, default}"
                )
    ...
    async def reset_budget_for_litellm_teams(self):
        ...
        try:
            ...
            if len(failed_teams) > 0:  # If any teams failed to reset:
                raise Exception(
                    f"Failed to reset {len(failed_teams)} teams: {json.dumps(failed_teams, default}"
                )
