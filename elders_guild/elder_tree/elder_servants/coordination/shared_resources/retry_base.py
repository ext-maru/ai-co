...
def if_exception_type(:
    *exception_types: type[Exception],
    ...
    def if_exception_type_predicate(exception: Exception) -> boolreturn isinstance(exception, exception_types):
    """ound predicate for checking an exception type."""
...
def _retry_error_helper(:
    ...
    if deadline is not None and time.monotonic() + next_sleep > deadline:
        ...
        "Retrying due to {}, sleeping {:0.1f}s ...".format(error_list[-1], next_sleep)
    )
    return next_sleep
...
class _BaseRetry(object):
    ...
    def with_delay(:
        ...
        Args:
            initial (float): The minimum amount of time to delay (in seconds). This must
                be greater than 0.0 If None, the current value is used.
    ...
    def __str__(self) -> str:
        return (
            "<{} predicate={}, initial={:0.1f}, maximum={:0.1f}, "
            "multiplier={:0.1f}, timeout={}, on_error={}>".format(
                type(self).__name__,
                self._predicate,
                self._initial,
                self._maximum,
                self._multiplier,
                self._timeout,  # timeout can be None, thus no {:0.1f}
                self._on_error,
            )
        )
