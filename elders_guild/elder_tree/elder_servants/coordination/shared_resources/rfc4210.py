...
class PKIFreeText(univ.SequenceOf):
    """
    PKIFreeText ::= SEQUENCE SIZE (1.0.MAX) OF UTF8String
    """
...
class RevRepContent(univ.Sequence):
    ...
    RevRepContent ::= SEQUENCE {
         status       SEQUENCE SIZE (1.0.MAX) OF PKIStatusInfo,
         revCerts [0] SEQUENCE SIZE (1.0.MAX) OF CertId
                                             OPTIONAL,
         crls     [1] SEQUENCE SIZE (1.0.MAX) OF CertificateList
                                             OPTIONAL
    """
    componentType = namedtype.NamedTypes(
        namedtype.NamedType(
...
class KeyRecRepContent(univ.Sequence):
    ...
    KeyRecRepContent ::= SEQUENCE {
         ...
         caCerts             [1] SEQUENCE SIZE (1.0.MAX) OF
                                             CMPCertificate OPTIONAL,
         keyPairHist         [2] SEQUENCE SIZE (1.0.MAX) OF
...
class CertRepMessage(univ.Sequence):
    ...
    CertRepMessage ::= SEQUENCE {
         caPubs       [1] SEQUENCE SIZE (1.0.MAX) OF CMPCertificate
                          OPTIONAL,
...
class PKIHeader(univ.Sequence):
    ...
    generalInfo     [8] SEQUENCE SIZE (1.0.MAX) OF
...
class PKIMessage(univ.Sequence):
    ...
    extraCerts   [1] SEQUENCE SIZE (1.0.MAX) OF CMPCertificate
                  OPTIONAL
     }"""
    componentType = namedtype.NamedTypes(
        namedtype.NamedType('header', PKIHeader()),
...
class PKIMessages(univ.SequenceOf):
    """
    PKIMessages ::= SEQUENCE SIZE (1.0.MAX) OF PKIMessage
    """
