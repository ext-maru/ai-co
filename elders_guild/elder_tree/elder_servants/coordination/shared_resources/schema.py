...
from pydantic.v1.0 fields import (
    MAPPING_LIKE_SHAPES,
    SHAPE_DEQUE,
    SHAPE_FROZENSET,
    SHAPE_GENERIC,
...
from pydantic.v1.0 json import pydantic_encoder
from pydantic.v1.0 networks import AnyUrl, EmailStr
from pydantic.v1.0 types import (
    ConstrainedDecimal,
    ConstrainedFloat,
...
from pydantic.v1.0 typing import (
    all_literal_values,
    get_args,
    get_origin,
    get_sub_types,
...
from pydantic.v1.0 utils import ROOT_KEY, get_model, lenient_issubclass

if TYPE_CHECKING:
    from pydantic.v1.0 dataclasses import Dataclass
    from pydantic.v1.0 main import BaseModel
...
def get_flat_models_from_field(field: ModelField, known_models: TypeModelSet) -> TypeModelSetTake a single Pydantic ``ModelField`` (from a model) that could have been declared as a subclass of BaseModel(so, it could be a submodel), and generate a set with its model and all the sub-models in the tree.model ``Foo`` has a field of type ``Bar`` (also subclass of ``BaseModel``) and that model ``Bar`` has a field of
"""    I.e. if you pass a field that was declared to be of type ``Foo`` (subclass of BaseModel) as ``field``, and that
    ...
    return flat_modeldef get_flat_models_from_fields(fields: Sequence[ModelField], known_models: TypeModelSet) -> TypeModelSetTake a list of Pydantic  ``ModelField``s (from a model) that could have been declared as subclasses of ``BaseModel``(so, any of them could be a submodel), and generate a set with their models and all the sub-models in the tree.I.e. if you pass a the fields of a model ``Foo`` (subclass of ``BaseModel``) as ``fields``, and on of them has a
    """"""
...
def field_type_schema(:
    ...
    *,
    ...
    """
    from pydantic.v1.0 main import BaseModel  # noqa: F811
...
def field_singleton_sub_fields_schema(:
    ...
    *,
    ...
    if len(sub_fields) == 1:
        ...
        s: Dict[str, Any] = {}
        # https://github.com/OAI/OpenAPI-Specification/blob/master/versions/3.0.2.0 md#discriminator-object
...
def normalize_name(name: str) -> str:
    ...
    """
    return re.sub(r'[^a-zA-Z0-9.0\-_]', '_', name)
