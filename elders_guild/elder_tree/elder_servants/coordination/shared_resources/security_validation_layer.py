...
class SecurityValidationLayer:
    ...
    async def execute_comprehensive_security_scan(
        self, code: str, file_path: str = None, context: Dict[str, Any] = None
    ) -> SecurityScanResult:
        """
        包括的セキュリティスキャン実行
        ...
        try:
            ...
            for tool_name, tool_config in self.oss_security_tools.items():
                tool_vulns = await self._execute_oss_security_tool(
                    tool_name, code, file_path
                )
            ...
            # 全体リスクレベル判定
            overall_risk = self._determine_overall_risk_level(
                all_vulnerabilities, security_score
            )

            # Elder コンプライアンス判定
            elder_compliance = await self._check_elder_security_compliance(
    ...
    async def _execute_elder_security_checks(
    ...
    async def _execute_oss_security_tool(
        self, tool_name: str, code: str, file_path: str = None
    ) -> List[SecurityVulnerability]:
        """OSS セキュリティツール実行"""
        vulnerabilities = []
            ...
            for issue in bandit_issues:
                ...
                vuln = SecurityVulnerability(
                    ...
                    description=issue.get(
                        "issue_text", "Security issue detected by Bandit"
                    ),
                    category=SecurityCategory.STATIC_ANALYSIS,
                    risk_level=self._bandit_severity_to_risk(
    ...
    def _determine_overall_risk_level(
        self, vulnerabilities: List[SecurityVulnerability], security_score: float
    ) -> SecurityRiskLevel:
        """全体リスクレベル判定"""
        # クリティカル脆弱性が1つでもあれば CRITICAL
        critical_vulns = [
            v for v in vulnerabilities if v.risk_level == SecurityRiskLevel.CRITICAL
        ]
        if critical_vulns:
    ...
    async def _check_elder_security_compliance(
        self, code: str, vulnerabilities: List[SecurityVulnerability]
    ) -> bool:
    ...
    def _generate_security_recommendations(
        ...
        for category, vulns in categories.items():
            ...
            if category == SecurityCategory.CODE_INJECTION:
                ...
                recommendations.append(
                    "🔒 Move secrets to secure storage (env vars, vault)"
                )
                ...
                recommendations.append(
                    "🔐 Use strong cryptographic algorithms (SHA-256+)"
                )
...
'''
    ...
    result = await validator.execute_comprehensive_security_scan(
        secure_code, "secure_elder.py"
    )
...
'''
    ...
    result3 = await validator.execute_comprehensive_security_scan(
        mixed_code, "mixed.py"
    )
    ...
    print(
        f"  ✅ Secure Code: {result.security_score}% ({result.overall_risk_level.value})"
    )
    print(
        f"  ⚠️ Vulnerable Code: {result2.security_score}% ({result2.overall_risk_level.value})"
    )
    print(
        f"  ⚖️ Mixed Code: {result3.security_score}% ({result3.overall_risk_level.value})"
    )
