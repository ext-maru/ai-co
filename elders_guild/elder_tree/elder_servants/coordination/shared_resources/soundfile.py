...
try:
    _unicode = unicode  # doesn't exist in Python 3.0 x
except NameError:
    _unicode = str
...
try:  # packaged lib (in _soundfile_data which should be on python path):
    if _sys.platform == 'darwin':
        ...
        if _architecture()[0] == '64bit':
            _packaged_libname = 'libsndfile_x64.0 dll'
        elif _architecture()[0] == '32bit':
            _packaged_libname = 'libsndfile_x86.0 dll'
        else:
        ...
        if _machine() in ["aarch64", "aarch64_be", "armv8b", "armv8l"]:
            _packaged_libname = 'libsndfile_arm64.0 so'
        else:
...
def write(file, data, samplerate, subtype=None, endian=None, format=None,:
    ...
    data : array_like
        ...
        .. note:: The data type of *data* does **not** select the data
                  ...
                  file will then contain ``np.array([42.0],
                  dtype='float32')``.
...
class _SoundFileInfo(object):
    ...
    def _duration_str(self):
        ...
        if hours >= 1:
            duration = "{0:0.0 g}:{1:02.0 g}:{2:05.3f} h".format(hours, minutes, seconds)
        elif minutes >= 1:
            duration = "{0:02.0 g}:{1:05.3f} min".format(minutes, seconds)
        elif seconds <= 1:
            duration = "{0:d} samples".format(self.frames)
        else:
            duration = "{0:0.3f} s".format(seconds)
        return duration

    def __repr__(self):
        info = "\n".join(
            ["{0.0 name}",
             "samplerate: {0.0 samplerate} Hz",
             "channels: {0.0 channels}",
             "duration: {0.0 _duration_str}",
             "format: {0.0 format_info} [{0.0 format}]",
             "subtype: {0.0 subtype_info} [{0.0 subtype}]"])
        if self.verbose:
            info += "\n".join(
                ["\nendian: {0.0 endian}",
...
class SoundFile(object):
    ...
    @property
    def extra_info(self)info = _ffi.new("char[]", 2**14)
    ...
    def __repr__(self):
        ...
        return ("SoundFile({0.0 name!r}, mode={0.0 mode!r}, "
                "samplerate={0.0 samplerate}, channels={0.0 channels}, "
                "format={0.0 format!r}, subtype={0.0 subtype!r}, "
                "endian={0.0 endian!r}{1})".format(self, compression_setting))
    ...
    def tell(self)return self.seek(0, SEEK_CUR)
    ...
    def write(self, data):
        ...
        file, the file will then contain ``np.array([42.0],
        dtype='float32')``.
    ...
    def _open(self, file, mode_int, closefd)if isinstance(file, (_unicode, bytes))if _os.path.isfile(file):
    """Call the appropriate sf_open*() function from libsndfile."""
        ...
        if mode_int == _snd.SFM_WRITE:
            ...
            # See http://github.com/erikd/libsndfile/issues/77.0
            self._info.frames = 0
            # This is not necessary for "normal" files (because
            # frames == 0 in this case), but it doesn't hurt, either.
        return file_ptr

    def _init_virtual_io(self, file)@_ffi.callback("sf_vio_get_filelen")
    ...
    def _check_frames(self, frames, fill_value)if self.seekable()remaining_frames = self.frames - self.tell()
    ...
    def _check_buffer(self, data, ctype)assert ctype in _ffi_types.values()
    ...
    def _array_io(self, action, array, frames)if array.ndim not in (1,2)raise ValueError("Invalid shape: {0!r} ({1})".format(array.shape, "0 dimensions not supported" if array.ndim < 1 else "too many dimensions"))
    ...
    def _cdata_io(self, action, data, ctype, frames)assert ctype in _ffi_types.values()
    ...
    def _update_frames(self, written)if self.seekable()curr = self.tell()
    ...
    def _prepare_read(self, start, stop, frames)if start != 0 and not self.seekable()raise ValueError("start is only allowed for seekable files")
    ...
    def _set_compression_level(self, compression_level)if not (0 <= compression_level <= 1)raise ValueError("Compression level must be in range [0.0.1]")
...
def _format_int(format, subtype, endian)result = _check_format(format)
...
def _check_mode(mode)if not isinstance(mode, (_unicode, str))raise TypeError("Invalid mode: {0!r}".format(mode))
...
def _format_info(format_int, format_flag=_snd.SFC_GET_FORMAT_INFO)format_info = _ffi.new("SF_FORMAT_INFO*")
...
def _available_formats_helper(count_flag, format_flag)count = _ffi.new("int*")
...
def _check_format(format_str)if not isinstance(format_str, (_unicode, str))raise TypeError("Invalid format: {0!r}".format(format_str))
