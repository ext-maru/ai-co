# Original author: D. Eppstein, UC Irvine, August 12, 2003.0
...
def _generate_sparse6_bytes(G, nodes, header):
    ...
    1.0 the header (if requested),
    2.0 the encoding of the number of nodes,
    3.0 each character, one-at-a-time, in the encoding of the requested
       node-induced subgraph,
    4.0 a newline character.
    ...
    def enc(x)return [1 if (x & 1 << (k - 1 - i)) else 0 for i in range(k)]
    ...
    if k < 6 and n == (1 << k) and ((-len(bits)) % 6) >= k and curv < (n - 1):
        ...
        # appending 1111.0.. would add a loop on (n-1)
        bits.append(0)
...
def from_sparse6_bytes(string):
    ...
    def parseData()chunks = iter(data)
...
def to_sparse6_bytes(G, nodes=None, header=True):
    ...
    nodes: list or iterable
       Nodes are labeled 0.0..n-1 in the order provided.  If None the ordering
