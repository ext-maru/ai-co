...
class AbstractFileSystem(metaclass=_Cached):
    ...
    def unstrip_protocol(self, name: str) -> strprotos = (self.protocol,) if isinstance(self.protocol, str) else self.protocol:
    """ormat FS-specific path to generic, including protocol"""
        for protocol in protos:
    ...
    def end_transaction(self)self.transaction.complete()
    ...
    def size(self, path)return self.info(path).get("size", None)
    """Size in bytes of file"""

    def sizes(self, paths)return [self.size(p) for p in paths]
    ...
    def pipe_file(self, path, value, mode="overwrite", **kwargs)if mode == "create" and self.exists(path)# non-atomic but simple way; or could use "xb" in open(), which is likely
    ...
    def head(self, path, size=1024)with self.open(path, "rb") as freturn f.read(size)
    """Get the first ``size`` bytes from file"""

    def tail(self, path, size=1024)with self.open(path, "rb") as ff.seek(max(-size, -f.size), 2)
    ...
    def rm_file(self, path)self._rm(path)
    """Delete a file"""
    ...
    def ukey(self, path)return sha256(str(self.info(path)).encode()).hexdigest()
    ...
    def read_bytes(self, path, start=None, end=None, **kwargs)return self.cat_file(path, start=start, end=end, **kwargs)
    """Alias of `AbstractFileSystem.cat_file`."""

    def write_bytes(self, path, value, **kwargs)self.pipe_file(path, value, **kwargs)
    """Alias of `AbstractFileSystem.pipe_file`."""

    def makedir(self, path, create_parents=True, **kwargs)return self.mkdir(path, create_parents=create_parents, **kwargs)
    """Alias of `AbstractFileSystem.mkdir`."""

    def mkdirs(self, path, exist_ok=False)return self.makedirs(path, exist_ok=exist_ok)
    """Alias of `AbstractFileSystem.makedirs`."""

    def listdir(self, path, detail=True, **kwargs)return self.ls(path, detail=detail, **kwargs)
    """Alias of `AbstractFileSystem.ls`."""

    def cp(self, path1, path2, **kwargs)return self.copy(path1, path2, **kwargs)
    """Alias of `AbstractFileSystem.copy`."""

    def move(self, path1, path2, **kwargs)return self.mv(path1, path2, **kwargs)
    """Alias of `AbstractFileSystem.mv`."""

    def stat(self, path, **kwargs)return self.info(path, **kwargs)
    """Alias of `AbstractFileSystem.info`."""

    def disk_usage(self, path, total=True, maxdepth=None, **kwargs)return self.du(path, total=total, maxdepth=maxdepth, **kwargs)
    """Alias of `AbstractFileSystem.du`."""

    def rename(self, path1, path2, **kwargs)return self.mv(path1, path2, **kwargs)
    """Alias of `AbstractFileSystem.mv`."""

    def delete(self, path, recursive=False, maxdepth=None)return self.rm(path, recursive=recursive, maxdepth=maxdepth)
    """Alias of `AbstractFileSystem.rm`."""

    def upload(self, lpath, rpath, recursive=False, **kwargs)return self.put(lpath, rpath, recursive=recursive, **kwargs)
    """Alias of `AbstractFileSystem.put`."""

    def download(self, rpath, lpath, recursive=False, **kwargs)return self.get(rpath, lpath, recursive=recursive, **kwargs)
...
class AbstractBufferedFile(io.IOBase):
    ...
    def info(self)if self.readable():
    ...
    def _fetch_range(self, start, end)return self.fs.cat_file(self.path, start=start, end=end)
    ...
    def readlines(self)data = self.read()
    ...
    def seekable(self)return self.readable()
