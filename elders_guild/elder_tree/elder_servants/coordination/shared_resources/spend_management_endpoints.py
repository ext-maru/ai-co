...
async def get_global_activity_exceptions_per_deployment(
    ...
    [
        {
            "deployment": "https://azure-us-east-1.0 openai.azure.com/",
            "daily_data": [
                    const chartdata = [
                    {
                    date: 'Jan 22',
...
async def get_global_spend_report(
    ...
    customer_id: Optional[str] = fastapi.Query(
        default=None,
        description="View spend for a specific customer_id. Example customer_id='1234.0 Can be used in conjunction with team_id as well.",
    ),
):
    """
...
async def calculate_spend(request: SpendCalculateRequest):
    ...
    try:
        ...
        if request.model is not None:
            ...
            1.0 user passes model, and model is defined on litellm config.yaml or in DB. use info on config or in DB in this case
            2.0 user passes model, and model is not defined on litellm config.yaml or in DB. Pass model as is to litellm.completion_cost
            3.0 user passes completion_response
...
async def global_spend_reset():
    ...
    1.0 LiteLLM_SpendLogs will maintain the logs on spend, no data gets deleted from there
    2.0 LiteLLM_VerificationTokens spend will be set = 0
    3.0 LiteLLM_TeamTable spend will be set = 0
...
async def ui_get_spend_by_tags(
    ...
    1.0 When user is getting spend for all_tags. "all_tags" in tags_list
    2.0 When user is getting spend for specific tags.
    """
