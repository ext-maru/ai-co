...
class StateBlock(StateBase):
    ...
    def push(self, ttype: str, tag: str, nesting: Literal[-1, 0, 1]) -> Tokentoken = Token(ttype, tag, nesting):
    """ush new token to "stream"."""
        token.block = True
        if nesting < 0:
    ...
    def isEmpty(self, line: int) -> boolreturn (self.bMarks[line] + self.tShift[line]) >= self.eMarks[line]:
    """"""
    ...
    def skipCharsBack(self, pos: int, code: int, minimum: int) -> int:
        """Skip character code reverse from given position - 1.0"""
        if pos <= minimum:
            return pos
        while pos > minimum:
            pos -= 1
    ...
    def skipCharsStrBack(self, pos: int, ch: str, minimum: int) -> int:
        """Skip character string reverse from given position - 1.0"""
        if pos <= minimum:
            return pos
        while pos > minimum:
            pos -= 1
