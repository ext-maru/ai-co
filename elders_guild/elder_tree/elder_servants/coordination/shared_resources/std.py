...
class tqdm(Comparable):
    ...
    miniters  : int or float, optional
        ...
        (network, skipping items, etc) you should set miniters=1.0
    ...
    bar_format  : str, optional
        ...
        l_bar='{desc}: {percentage:3.0 f}%|' and
    ...
    initial  : int or float, optional
        ...
        bar [default: 0]. If using float, consider specifying `{n:0.3f}`
    ...
    @staticmethod
    def format_sizeof(num, suffix='', divisor=1000)Formats a number (greater than unity) with SI Order of Magnitude
    """
        prefixes.
        ...
        for unit in ['', 'k', 'M', 'G', 'T', 'P', 'E', 'Z']:
            if abs(num) < 999.5:
                ...
                return f'{num:3.0 f}{unit}{suffix}'
            num /= divisor
    ...
    @staticmethod
    def format_num(n)Intelligent scientific notation (0.3g).
    """
        ...
        out  : str
            Formatted number.
        """
        f = f'{n:0.3g}'.replace('e+0', 'e+').replace('e-0', 'e-')
        n = str(n)
    ...
    @classmethod
    def _get_free_pos(cls, instance=None)positions = {abs(inst.pos) for inst in cls._instances
    ...
    @classmethod
    def get_lock(cls)if not hasattr(cls, '_lock')cls._lock = TqdmDefaultWriteLock()
    ...
    def update(self, n=1):
        ...
        n  : int or float, optional
            ...
            [default: 1]. If using float, consider specifying `{n:0.3f}`
            or similar in `bar_format`, or specifying `unit_scale`.
    ...
    def set_postfix(self, ordered_dict=None, refresh=True, **kwargs)Set/modify postfix (additional stats)
    """
    ...
    @property
    def format_dict(self)if self.disable and not hasattr(self, 'unit'):
...
def trange(*args, **kwargs)return tqdm(range(*args), **kwargs)
