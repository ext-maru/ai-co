"""Implementation of the StyleGuide used by Flake8.0"""
...
class DecisionEngine:
    ...
    def make_decision(self, code: str) -> Decisionselected = self.was_selected(code)ignored = self.was_ignored(code)
    """ecide if code should be ignored or selected."""
...
class StyleGuideManager:
    ...
    def processing_file(self, filename: str) -> Generator[StyleGuide]guide = self.style_guide_for(filename)with guide.processing_file(filename):
    """ecord the fact that we're processing the file's results."""
            yield guide
    ...
    def handle_error(:
        ...
        :param code:
            The error code found, e.g., E123.0
        :param filename:
...
class StyleGuide:
    ...
    @contextlib.contextmanager
    def processing_file(self, filename: str) -> Generator[StyleGuide]self.formatter.beginning(filename):
    """ecord the fact that we're processing the file's results."""
        yield self
