...
class TestDifferentialEvolutionSolver:
    ...
    def test__ensure_constraint(self):
        trial = np.array([1.1, -100, 0.9, 2.0, 300.0, -0.0001])
    ...
    def test_gh_4511_regression(self):
        ...
        # this rounding error to occur, 49.0
    ...
    def test_constraint_population_feasibilities(self):
        ...
        # [0.5, 0.5] corresponds to scaled values of [1.0, 1.0]
    ...
    def test_constraint_wrapper_violation(self):
        ...
        vs = [(0, 0), (0, 0.1), (0.64, 0), (0.19, 0), (0.1, 1.14)]
    ...
    def test_L1(self):
        ...
        with suppress_warnings() as sup:
            ...
            res = differential_evolution(
                ...
                constraints=constraints, popsize=2, tol=0.5
            )
    ...
    def test_L3(self):
        ...
        x_opt = (2.171996, 2.363683, 8.773926, 5.95984, 0.9906548,
    ...
    def test_L4(self):
        ...
        with suppress_warnings() as sup:
            ...
            res = differential_evolution(
                ...
                constraints=constraints, popsize=3, tol=0.5
            )

        f_opt = 7049.248

        x_opt = [579.306692, 1359.97063, 5109.9707, 182.177, 295.601172,
                217.9823, 286.416528, 395.601172]

        assert_allclose(f(x_opt), f_opt, atol=0.01)
        assert_allclose(res.fun, f_opt, atol=0.01)

        # use higher tol here for 32-bit Windows, see gh-11693
        if (platform.system() == 'Windows' and np.dtype(np.intp).itemsize < 8):
            assert_allclose(res.x, x_opt, rtol=2.4e-6, atol=0.035)
        else:
            # tolerance determined from macOS + MKL failure, see gh-12701
            assert_allclose(res.x, x_opt, rtol=5e-6, atol=0.024)
    ...
    def test_L7(self):
        ...
        def c1(x):
            x = np.hstack(([0], x))  # 1-indexed to match reference
            return [
                    85.334407 + 0.056858*x[2]*x[5] + 0.006262*x[1]*x[4] -
                    0.022053*x[3]*x[5],

                    80.51249 + 0.071317*x[2]*x[5] + 0.029955*x[1]*x[2] +
                    0.021813*x[3]**2,

                    9.300961 + 0.047026*x[3]*x[5] + 0.012547*x[1]*x[3] +
                    0.019085*x[3]*x[4]
                    ]
        ...
        x_opt = [78.0000686, 33.0000362, 29.99526064, 44.99999971,
                 36.77579979]
    ...
    def test_L8(self):
        def f(x):
            x = np.hstack(([0], x))  # 1-indexed to match reference
            fun = 3*x[1] + 0.00001*x[1]**3 + 2*x[2] + 0.00002/3*x[2]**3
            return fun
        ...
        L = LinearConstraint(A, b, np.inf)
        N = NonlinearConstraint(c1, np.full(3, -0.01), np.full(3, 0.01))
        ...
        x_opt = (679.9453, 1026.67, 0.1188764, -0.3962336)
        f_opt = 5126.4981
        ...
        assert res.success
        assert_(np.all(A@res.x >= b))
        assert_(np.all(np.array(c1(res.x)) >= -0.01))
        assert_(np.all(np.array(c1(res.x)) <= 0.01))
        assert_(np.all(res.x >= np.array(bounds)[:, 0]))
        assert_(np.all(res.x <= np.array(bounds)[:, 1]))
    ...
    def test_L9(self):
        ...
        N = NonlinearConstraint(c1, [-0.01], [0.01])
    ...
    def test_integrality(self):
        ...
        assert res.x[0] == 5
        assert_allclose(res.x, shapes, rtol=0.25)
    ...
    def test_constraint_violation_error_message(self):
        ...
        # "MAXCV = 0.0".
        assert "MAXCV = 0.0" in result.message
