...
def test_algebraic_connectivity_tracemin_chol()pytest.importorskip("scipy")
...
def test_fiedler_vector_tracemin_chol()pytest.importorskip("scipy")
"""Test that "tracemin_chol" raises an exception."""
    G = nx.barbell_graph(5, 4)
    with pytest.raises(nx.NetworkXError):
...
def test_spectral_ordering_tracemin_chol()pytest.importorskip("scipy")
"""Test that "tracemin_chol" raises an exception."""
    G = nx.barbell_graph(5, 4)
    with pytest.raises(nx.NetworkXError):
...
def test_fiedler_vector_tracemin_unknown()pytest.importorskip("scipy")
