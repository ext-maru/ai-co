...
class TestSetOps:
    ...
    @pytest.mark.parametrize("kind", [None, "sort", "table"])
    def test_in1d_boolean(self, kind)a = np.array([True, False])
    ...
    def test_in1d_timedelta(self, kind)rstate = np.random.RandomState(0)
    ...
    @pytest.mark.parametrize("kind", [None, "sort", "table"])
    def test_in1d_mixed_dtype(self, dtype1, dtype2, kind)is_dtype2_signed = np.issubdtype(dtype2, np.signedinteger)
...
class TestUnique:
    ...
    def test_unique_1d(self):
        ...
        aa = [1.0 + 0.j, 1 - 1.j, 1]
        assert_array_equal(np.unique(aa), [1.0 - 1.j, 1.0 + 0.j])
    ...
    def test_unique_sort_order_with_axis(self):
        ...
        # as unsigned byte strings.  See gh-10495.0
