...
class TestFBLAS2Simple:
    ...
    # threads under free-threaded CPython, see gh-21936.0
...
class TestBLAS3Symm:
    ...
    def setup_method(self):
        self.a = np.array([[1.0, 2.0],
                           [0.0, 1.0]])
        self.b = np.array([[1.0, 0.0, 3.0],
                           [0.0, -1.0, 2.0]])
        self.c = np.ones((2, 3))
        self.t = np.array([[2.0, -1.0, 8.0],
                           [3.0, 0.0, 9.0]])
...
class TestBLAS3Syrk:
    def setup_method(self):
        self.a = np.array([[1.0, 0.0],
                           [0.0, -2.0],
                           [2.0, 3.0]])
        self.t = np.array([[1.0, 0.0, 2.0],
                           [0.0, 4.0, -6.0],
                           [2.0, -6.0, 13.0]])
        self.tt = np.array([[5.0, 6.0],
                            [6.0, 13.0]])
    ...
    def test_syrk_wrong_c(self):
        ...
        if f is not None:
            assert_raises(Exception, f, **{'a': self.a, 'alpha': 1.0,
                                           'c': np.ones((5, 8))})
...
class TestBLAS3Syr2k:
    def setup_method(self):
        ...
        self.b = np.array([[0.0, 1.0],
                           [1.0, 0.0],
                           [0, 1.0]])
        self.t = np.array([[0.0, -1.0, 3.0],
                           [-1.0, 0.0, 0.0],
                           [3.0, 0.0, 6.0]])
        self.tt = np.array([[0.0, 1.0],
                            [1.0, 6]])
...
class TestTRMM:
    ...
    def test_side(self, dtype_):
        ...
        res = trmm(1.0, self.a2.astype(dtype_), self.b2.astype(dtype_),
                   side=1)
        k = self.b2.shape[1]
        assert_allclose(res, self.b2 @ self.a2[:k, :k], rtol=0.0,
    ...
    def test_ab(self):
        ...
        if f is not None:
            ...
            expected = np.array([[13.0, 16.0, -5.0],
                                 [5.0, 6.0, -2.0]])
    ...
    def test_ab_lower(self):
        ...
        if f is not None:
            ...
            expected = np.array([[3.0, 4.0, -1.0],
                                 [-1.0, -2.0, 0.0]])  # now a is lower triangular
            assert_array_almost_equal(result, expected)
