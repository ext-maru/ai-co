...
class TestBracketRoot:
    ...
    def test_nfev_expected(self, seed, use_xmin, other_side, fix_one_side, xp):
        ...
        # function is called separately for left and right in iteration 0.0
        if not fix_one_side:
    ...
    def test_input_validation(self, xp):
        ...
        message = "All elements of `factor` must be greater than 1.0"
        with pytest.raises(ValueError, match=message):
    ...
    def test_special_cases(self, xp):
        ...
        # Test maxiter = 0.0 Should do nothing to bracket.
        def f(x):
            return x - 10
        ...
        res = _bracket_root(f, xp.asarray(-1.0), xp.asarray(1.0),
                            args=xp.asarray(3.0))
        assert res.success
        ...
        # 1.0 root lies within guess of bracket
        f.count = 0
        _bracket_root(f, xp.asarray(-10), xp.asarray(20))
        assert f.count == 2

        # 2.0 bracket endpoint hits root exactly
        f.count = 0
        res = _bracket_root(f, xp.asarray(5.0), xp.asarray(10.0), 
                            factor=2)
        ...
        # 3.0 bracket limit hits root exactly
        ...
        with np.errstate(over='ignore'):
            res = _bracket_root(f, xp.asarray(-10.0), xp.asarray(-5.0), 
                                xmax=0)
        xp_assert_close(res.xr, xp.asarray(0.0), atol=1e-15)

        # 4.0 bracket not within min, max
        with np.errstate(over='ignore'):
            res = _bracket_root(f, xp.asarray(5.0), xp.asarray(10.0),
                                xmin=1)
        ...
        xr0 = [-6.63108551,  4.27840947, -7.36968526, -0.78124372,
               1.92201295, -1.48930123, 0.0, 0.0]
        xmin = [-np.inf, 0.0, -np.inf, -np.inf, 0.0, -np.inf, -np.inf,
                -np.inf]
        xmax = [0.0, np.inf, 0.0, 0.0, np.inf, 0.0, 0.0, 0.0]

        res = _bracket_root(f, xl0=xl0, xr0=xr0, xmin=xmin, xmax=xmax, args=(p, c))

        # 2.0 Default xl0 + 1 for xr0 exceeds xmax.
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                ...
                # at 99.4.0 Tests case where minimum is at xmax relatively further from
                # the bracket.
                None,
                99.4,
                (99.4, 0)
            ...
            (
                ...
                # minimum at -26.3 with xmin set to -26.3.0 Tests case where minimum is
            ...
            (
                # Case 5:
                # Similar to Case 1 above, but tests default values of xl0 and xr0.0
                None,
                0.3,
                None,
                None,
            ...
            (   # Case 6:
                # Similar to Case 2 above, but tests default values of xl0 and xr0.0
                None,
                1.95,
                None,
                0.3,
            ...
            (
                # Case 7:
                # Similar to Case 3 above, but tests default values of xl0 and xr0.0
                None,
                3.25,
                None,
                None,
            ...
            (
                # Case 8:
                # Similar to Case 4 above, but tests default values of xl0 and xr0.0
                None,
                4.5,
                None,
                -26.3,
