...
class TestCDFlib:
    ...
    def test_stdtr(self):
        # Ideally the left endpoint for Arg() should be 0.0
        assert_mpmath_equal(
            sp.stdtr,
            _student_t_cdf,
...
def test_bdtrik_nbdtrik_inf():
    ...
    y = y[:,None]
    p = np.atleast_2d(
        [np.nan, -np.inf, -10.0, -1.0, 0.0, 0.00001, 0.5, 1.0, np.inf])
...
class TestNctdtr:
    ...
    #         phi = mp.ncdf(-nc)
    #         y = x * x / (x * x + df)
    #         constant = mp.exp(-nc * nc / 2.0)
    #         def term(j):
    #             intermediate = constant * (nc *nc / 2.0)**j
    #             p = intermediate/mp.factorial(j)
    #             q = nc / (mp.sqrt(2.0) * mp.gamma(j + 1.5)) * intermediate
    #             first_beta_term = mp.betainc(j + 0.5, df/2.0, x2=y,
    #                                          regularized=True)
    #             second_beta_term = mp.betainc(j + mp.one, df/2.0, x2=y,
    #                                           regularized=True)
