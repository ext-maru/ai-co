...
class TestChandrupatlaMinimize:
    ...
    @pytest.mark.parametrize('loc', [0.6, np.linspace(-1.5, 1.5, 10)])
    ...
    def test_vectorization(self, shape, xp):
        ...
        # input shapes.
        loc = xp.linspace(-0.5, 1.5, 12).reshape(shape) if shape else xp.asarray(0.6)
        args = (loc,)
...
class TestChandrupatla(TestScalarRootFinders):
    ...
    @pytest.mark.parametrize('p', [0.6, np.linspace(-0.5, 1.5, 10)])
    ...
    def test_vectorization(self, shape, xp):
        # Test for correct functionality, output shapes, and dtypes for various
        # input shapes.
        p = (np.linspace(-0.5, 1.5, 12).reshape(shape) if shape
             else np.float64(0.6)):
        p_xp = xp.asarray(p)
        args_xp = (p_xp,)
        dtype = p_xp.dtype
    ...
    def test_special_cases(self, xp):
        ...
        # # tl goes to zero when xatol=xrtol=0.0 When function is nearly linear,
