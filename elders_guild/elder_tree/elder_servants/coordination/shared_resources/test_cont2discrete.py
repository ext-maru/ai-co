...
class TestC2D:
    ...
    def test_discrete_approx(self):
        ...
        # computed by lsim2.0  This is what the discrete approximation
    ...
    def test_simo_tf(self):
        # See gh-5753
        tf = ([[1, 0], [1, 1]], [1, 1])
        num, den, dt = c2d(tf, 0.1)

        assert dt == 0.1  # sanity check
...
class TestC2dLti:
    def test_c2d_ss(self):
        ...
        A_res = np.array([[0.985136404135682, 0.04876671474795],
                          [0.09753342949590, 0.965629718236502]])
        B_res = np.array([[0.00122937599964], [0.49135527547844]])

        sys_ssc = lti(A, B, C, D)
        sys_ssd = sys_ssc.to_discrete(0.5)
    ...
    def test_c2d_tf(self):
        ...
        xp_assert_close(sys.den, den_res, atol=0.2)
        xp_assert_close(sys.num, num_res, atol=0.2)
