...
class Test_RealDomain:
    ...
    def test_contains(self, shapes, inclusive_a, inclusive_b, data):
        ...
        # Apparently, `np.float16([2]) < np.float32(2.009766)` is False
        # but `np.float16([2]) < np.float32([2.009766])` is True
        ...
        # fixed with NEP50.0 Just do what makes the test pass.
...
class TestFullCoverage:
    ...
    def test_ContinuousDistribution__repr__(self):
        ...
        assert repr(X) == "Uniform(a=array([0.0, 0.0, 0.0, 0.0]), b=1)"

        X = Uniform(a=np.zeros(4, dtype=np.float32), b=np.ones(4, dtype=np.float32))
        assert repr(X) == (
            "Uniform(a=array([0.0, 0.0, 0.0, 0.0], dtype=float32),"
            " b=array([1.0, 1.0, 1.0, 1.0], dtype=float32))"
        )
...
class TestMixture:
    ...
    def test_basic(self, shape):
        ...
        assert stats.ks_1samp(y.ravel(), X.cdf).pvalue > 0.5
