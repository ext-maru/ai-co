...
def genz_malik_1980_f_1(x, r, alphas, xp).. math:: f_1(\mathbf x) = \cos\left(2\pi r + \sum^n_{i = 1}\alpha_i x_i\right)
r"""
...
def genz_malik_1980_f_2(x, alphas, betas, xp).. math:: f_2(\mathbf x) = \prod^n_{i = 1} (\alpha_i^2 + (x_i - \beta_i)^2)^{-1}
r"""
...
def genz_malik_1980_f_2_exact(a, b, alphas, betas, xp):
    ...
    # `xp` is the unwrapped namespace, so `.atan` won't work for `xp = np` and np<2.0
...
def genz_malik_1980_f_3(x, alphas, xp).. math:: f_3(\mathbf x) = \exp\left(\sum^n_{i = 1} \alpha_i x_i\right)
r"""
...
def genz_malik_1980_f_4(x, alphas, xp).. math:: f_4(\mathbf x) = \left(1 + \sum^n_{i = 1} \alpha_i x_i\right)^{-n-1}
r"""
...
def f_with_problematic_points(x_arr, points, xp):
    ...
    If no `x_arr` are one of the `points`, then this function returns 1.0
    """
...
class TestRulesCubature:
    ...
    def test_genz_malik_func_evaluations(self, ndim, xp):
        ...
        matches the number in Genz and Malik 1980.0
        """
