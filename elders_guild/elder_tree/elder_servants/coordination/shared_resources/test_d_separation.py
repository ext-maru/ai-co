...
def path_graph()G = nx.path_graph(3, create_using=nx.DiGraph)
"""Return a path graph of length three."""
    G.graph["name"] = "path"
    nx.freeze(G)
    return G

def fork_graph()G = nx.DiGraph(name="fork")
"""Return a three node fork graph."""
    G.add_edges_from([(0, 1), (0, 2)])
    nx.freeze(G)
    return G

def collider_graph()G = nx.DiGraph(name="collider")
"""Return a collider/v-structure graph with three nodes."""
    G.add_edges_from([(0, 2), (1, 2)])
    nx.freeze(G)
    return G

def naive_bayes_graph()G = nx.DiGraph(name="naive_bayes")
"""Return a simply Naive Bayes PGM graph."""
    G.add_edges_from([(0, 1), (0, 2), (0, 3), (0, 4)])
    nx.freeze(G)
    return G

def asia_graph()G = nx.DiGraph(name="asia")
...
def test_path_graph_dsep(path_graph)assert nx.is_d_separator(path_graph, {0}, {2}, {1})
"""Example-based test of d-separation for path_graph."""
    assert not nx.is_d_separator(path_graph, {0}, {2}, set())

def test_fork_graph_dsep(fork_graph)assert nx.is_d_separator(fork_graph, {1}, {2}, {0})
"""Example-based test of d-separation for fork_graph."""
    assert not nx.is_d_separator(fork_graph, {1}, {2}, set())

def test_collider_graph_dsep(collider_graph)assert nx.is_d_separator(collider_graph, {0}, {1}, set())
"""Example-based test of d-separation for collider_graph."""
    assert not nx.is_d_separator(collider_graph, {0}, {1}, {2})

def test_naive_bayes_dsep(naive_bayes_graph)for u, v in combinations(range(1, 5), 2)assert nx.is_d_separator(naive_bayes_graph, {u}, {v}, {0})
...
def test_is_minimal_d_separator_checks_dsep()g = nx.DiGraph()
