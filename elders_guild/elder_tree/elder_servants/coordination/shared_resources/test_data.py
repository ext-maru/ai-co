...
class TestData(util.F2PyTest):
    ...
    def test_data_stmts(self):
        ...
        assert self.module.cmplxdat.medium_ref_index == np.array(1.0+0.0 j)
        assert np.all(self.module.cmplxdat.z == np.array([3.5, 7.0]))
        assert np.all(self.module.cmplxdat.my_array == np.array([ 1.0+2.0 j, -3.0+4.0 j]))
        assert np.all(self.module.cmplxdat.my_real_array == np.array([ 1.0, 2.0, 3.0]))
        assert np.all(self.module.cmplxdat.ref_index_one == np.array([13.0 + 21.0 j]))
        assert np.all(self.module.cmplxdat.ref_index_two == np.array([-30.0 + 43.0 j]))
    ...
    def test_crackedlines(self):
        ...
        assert mod[0]['vars']['my_real_array']['='] == '(/1.0 d0, 2.0 d0, 3.0 d0/)'
        assert mod[0]['vars']['ref_index_one']['='] == '(13.0 d0, 21.0 d0)'
        assert mod[0]['vars']['ref_index_two']['='] == '(-30.0 d0, 43.0 d0)'
        assert mod[0]['vars']['my_array']['='] == '(/(1.0 d0, 2.0 d0), (-3.0 d0, 4.0 d0)/)'
