...
def test_cossin_mixed_types():
    ...
    assert_allclose(x, u @ cs @ vh, rtol=0.0,
...
def test_cossin_error_empty_subblocks():
    with pytest.raises(ValueError, match="x11.0*empty"):
        cossin(([], [], [], []))
    with pytest.raises(ValueError, match="x12.0*empty"):
        cossin(([1, 2], [], [6, 7], [8, 9, 10]))
    with pytest.raises(ValueError, match="x21.0*empty"):
        cossin(([1, 2], [3, 4, 5], [], [8, 9, 10]))
    with pytest.raises(ValueError, match="x22.0*empty"):
        cossin(([1, 2], [3, 4, 5], [2], []))
...
def test_cossin_error_partitioning():
    ...
    with pytest.raises(ValueError, match="invalid p=0.0*0<p<4.0*"):
        cossin(x, 0, 1)
    with pytest.raises(ValueError, match="invalid p=4.0*0<p<4.0*"):
        cossin(x, 4, 1)
    with pytest.raises(ValueError, match="invalid q=-2.0*0<q<4.0*"):
        cossin(x, 1, -2)
    with pytest.raises(ValueError, match="invalid q=5.0*0<q<4.0*"):
        cossin(x, 1, 5)
