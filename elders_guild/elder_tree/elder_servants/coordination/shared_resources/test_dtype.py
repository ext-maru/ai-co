...
class TestBuiltin:
    @pytest.mark.parametrize('t', [int, float, complex, np.int32, str, object,
                                   np.compat.unicode])
    def test_run(self, t)dt = np.dtype(t)
...
class TestRecord:
    def test_equivalent_record(self)a = np.dtype([('yo', int)])
...
class TestSubarray:
    ...
    def test_equivalent_record(self)a = np.dtype((int, (2, 3)))
    """Test whether equivalent subarray dtypes hash the same."""
        b = np.dtype((int, (2, 3)))
        assert_dtype_equal(a, b)

    def test_nonequivalent_record(self)a = np.dtype((int, (2, 3)))
    ...
    def test_shape_equal(self)assert_dtype_equal(np.dtype('f8'), np.dtype(('f8', tuple())))
    ...
    def test_shape_simple(self)assert_dtype_not_equal(np.dtype('f8'), np.dtype(('f8', (1,))))
...
class TestStructuredObjectRefcounting:
    ...
    def test_structured_object_indexing(self, shape, index, items_changed,:
        ...
        # When using 0 and 1.0  If it fails again, should remove all explicit
...
class TestFromCTypes:
    ...
    def test_pairs(self, pair)Check that np.dtype('x,y') matches [np.dtype('x'), np.dtype('y')]
    """
...
def test_result_type_integers_and_unitless_timedelta64():
    # Regression test for gh-20077.0  The following call of `result_type`
