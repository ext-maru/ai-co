...
def test_rvs_and_ppf(distname, args):
    ...
    assert stats.cramervonmises_2samp(rvs1, rvs2).pvalue > 0.1

    # check ppf
    q = [0.01, 0.1, 0.5, 0.9, 0.999]
...
def test_rvs_ppf_loc_scale():
    ...
    assert stats.cramervonmises(r_rescaled, "norm").pvalue > 0.1
    q = [0.01, 0.1, 0.5, 0.9, 0.999]
...
def test_domain_argus_large_chi():
    ...
    prob = cdf(ub) - cdf(lb)
    assert stats.cramervonmises(r, lambda x: cdf(x) / prob).pvalue > 0.5
...
def test_setting_loc_scale():
    ...
    assert stats.cramervonmises_2samp(r1, (r2 - 3) / 2.5).pvalue > 0.5
    # reset values to default loc=0, scale=1
    rng.loc = 0
    rng.scale = 1
    r2 = rng.rvs(1000)
    assert stats.cramervonmises_2samp(r1, r2).pvalue > 0.5

def test_ignore_shape_range():
    msg = "No generator is defined for the shape parameters"
    with pytest.raises(ValueError, match=msg):
        rng = FastGeneratorInversion(stats.t(0.3))
    rng = FastGeneratorInversion(stats.t(0.3), ignore_shape_range=True)
