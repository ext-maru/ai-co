...
class BaseGraphTester:
    ...
    def test_nbunch_iter_node_format_raise(self):
        ...
        # For more information, see pull request #1813.0
        G = self.Graph()
        nbunch = [("x", set())]
        with pytest.raises(nx.NetworkXError):
...
class TestEdgeSubgraph:
    ...
    def test_correct_nodes(self)assert [0, 1, 3, 4] == sorted(self.H.nodes())
    """Tests that the subgraph has the correct nodes."""

    def test_correct_edges(self)assert [(0, 1, "edge01"), (3, 4, "edge34")] == sorted(self.H.edges(data="name"))
