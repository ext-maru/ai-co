...
def mp_hyp2f1(a, b, c, z):
    """Return mpmath hyp2f1 calculated on same branch as scipy hyp2f1.0
    ...
    """
    on_branch_cut = z.real > 1.0 and abs(z.imag) < 1.0 e-15
    cond1 = abs(c - a - round(c - a)) < 1.0 e-15 and round(c - a) <= 0
    cond2 = abs(c - b - round(c - b)) < 1.0 e-15 and round(c - b) <= 0
    # Make sure imaginary part is *exactly* zero
    if on_branch_cut:
        z = z.real + 0.0 j
    if on_branch_cut and not (cond1 or cond2):
        z_mpmath = z.real + 1.0 e-15j
    else:
        z_mpmath = z
...
class TestHyp2f1ExtremeInputs:
    ...
    @pytest.mark.parametrize("z", [4.0 + 1.0 j])
    def test_inf_a_b(self, a, b, c, z):
        if np.any(np.isinf(np.asarray([a, b]))):
            assert(np.isnan(hyp2f1(a, b, c, z)))

    def test_large_a_b(self):
        assert(np.isnan(hyp2f1(10**7, 1.0, 3.0, 4.0 + 1.0 j)))
        assert(np.isnan(hyp2f1(-10**7, 1.0, 3.0, 4.0 + 1.0 j)))

        assert(np.isnan(hyp2f1(1.0, 10**7, 3.0, 4.0 + 1.0 j)))
        assert(np.isnan(hyp2f1(1.0, -10**7, 3.0, 4.0 + 1.0 j)))
