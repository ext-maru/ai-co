...
class TestEppsSingleton:
    ...
    def test_statistic_2(self):
        ...
        y = np.array((10, 4, 0, 5, 10, 10, 0, 5, 6, 7, 10, 3, 1, 7, 0, 8, 1,
                      5, 8, 10))
        w, p = epps_singleton_2samp(x, y)
        assert_allclose(w, 8.900, atol=0.01)
        assert_almost_equal(p, 0.6364, decimal=3)
...
class TestCvm:
    ...
    # Cramér-von Mises Statistics, 1996.0
    def test_cdf_4(self):
        assert_allclose(
                _cdf_cvm([0.2983, 0.4111, 0.12331, 0.94251], 4),
                [0.1, 0.5, 0.5, 0.999],
                atol=1e-4)

    def test_cdf_10(self):
        assert_allclose(
                _cdf_cvm([0.2657, 0.3830, 0.12068, 0.56643], 10),
                [0.1, 0.5, 0.5, 0.975],
                atol=1e-4)

    def test_cdf_1000(self):
        assert_allclose(
                _cdf_cvm([0.2481, 0.3658, 0.11889, 1.16120], 1000),
                [0.1, 0.5, 0.5, 0.999],
                atol=1e-4)

    def test_cdf_inf(self):
        assert_allclose(
                _cdf_cvm([0.2480, 0.3656, 0.11888, 1.16204]),
                [0.1, 0.5, 0.5, 0.999],
                atol=1e-4)
    ...
    def test_cdf_large_n(self):
        ...
        assert_allclose(
                _cdf_cvm([0.2480, 0.3656, 0.11888, 1.16204, 100], 10000),
                _cdf_cvm([0.2480, 0.3656, 0.11888, 1.16204, 100]),
                atol=1e-4)
    ...
    def test_low_p(self):
        # _cdf_cvm can return values larger than 1.0 In that case, we just
    ...
    def test_values_R(self):
        ...
        assert_allclose(res.pvalue, 0.02026417, atol=1e-6)
        ...
        assert_allclose(res.pvalue, 0.04433406, atol=1e-6)
...
class TestMannWhitneyU:
    ...
    x = [210.52110, 110.190630, 307.918612]
    y = [436.8811482466416, 416.37397329768191, 179.96975939463582,
         197.8118754228619, 34.38757281225756, 138.54220550921517,
         128.7769351470246, 265.92721427951852, 275.6617533155341,
         592.34083395416258, 448.73177590617018, 300.61495185038905,
         187.97508449019588]

    # This test was written for mann_whitney_u in gh-4933.0
    ...
    # options(digits = 16)
    # x = c(210.52110, 110.190630, 307.918612)
    # y = c(436.8811482466416, 416.37397329768191, 179.96975939463582,
    #       197.8118754228619, 34.38757281225756, 138.54220550921517,
    #       128.7769351470246, 265.92721427951852, 275.6617533155341,
    #       592.34083395416258, 448.73177590617018, 300.61495185038905,
    #       187.97508449019588)
    ...
    def test_tie_correct(self):
        ...
        dy = np.array([0, 1, 0, 1, 0])*0.1
        dy2 = np.array([0, 0, 1, 0, 0])*0.1
        y = [y0-0.1, y0-dy, y0-dy2, y0, y0+dy2, y0+dy, y0+0.1]
    ...
    pn3 = {1: [0.25, 0.5, 0.75], 2: [0.1, 0.2, 0.4, 0.6],
           3: [0.5, 0.1, 0.2, 0.35, 0.5, 0.65]}
    pn4 = {1: [0.2, 0.4, 0.6], 2: [0.67, 0.133, 0.267, 0.4, 0.6],
           3: [0.28, 0.57, 0.114, 0.2, 0.314, 0.429, 0.571],
           4: [0.14, 0.29, 0.57, 0.1, 0.171, 0.243, 0.343, 0.443, 0.557]}
    pm5 = {1: [0.167, 0.333, 0.5, 0.667],
           2: [0.47, 0.95, 0.19, 0.286, 0.429, 0.571],
           3: [0.18, 0.36, 0.71, 0.125, 0.196, 0.286, 0.393, 0.5, 0.607],
           4: [0.08, 0.16, 0.32, 0.56, 0.95, 0.143,
               0.206, 0.278, 0.365, 0.452, 0.548],
           5: [0.04, 0.08, 0.16, 0.28, 0.48, 0.75, 0.111,
               0.155, 0.21, 0.274, 0.345, 0.421, 0.5, 0.579]}
    pm6 = {1: [0.143, 0.286, 0.428, 0.571],
           2: [0.36, 0.71, 0.143, 0.214, 0.321, 0.429, 0.571],
           3: [0.12, 0.24, 0.48, 0.83, 0.131,
               0.19, 0.274, 0.357, 0.452, 0.548],
           4: [0.05, 0.1, 0.19, 0.33, 0.57, 0.86, 0.129,
           ...
           5: [0.02, 0.04, 0.09, 0.15, 0.26, 0.41, 0.63, 0.89,
               0.123, 0.165, 0.214, 0.268, 0.331, 0.396, 0.465, 0.535],
           6: [0.01, 0.02, 0.04, 0.08, 0.13, 0.21, 0.32, 0.47,
               0.66, 0.9, 0.12, 0.155, 0.197, 0.242, 0.294, 0.350,
               0.409, 0.469, 0.531]}
    ...
    def test_gh_9184(self, use_continuity, alternative, method, pvalue_exp):
        ...
        # R code:
        # options(digits=16)
        # x <- c(0.80, 0.83, 1.89, 1.4, 1.45, 1.38, 1.91, 1.64, 0.73, 1.46)
        ...
        statistic_exp = 35
        x = (0.80, 0.83, 1.89, 1.4, 1.45, 1.38, 1.91, 1.64, 0.73, 1.46)
...
class TestSomersD(_TestPythranFunc):
    ...
    def test_like_kendalltau(self):
        ...
        expected = (0.00000000000000, 1.00000000000000)
        ...
        # Cross-check with result from SAS FREQ:
        expected = (0.911111111111110, 0.00000000000000)
        res = stats.somersd(x, y)
        assert_allclose(res.statistic, expected[0], atol=1e-15)
        assert_allclose(res.pvalue, expected[1], atol=1e-15)
        ...
        # Cross-check with result from SAS FREQ:
        expected = (-0.9111111111111111, 0.00000000000000)
        res = stats.somersd(x, y)
        assert_allclose(res.statistic, expected[0], atol=1e-15)
        assert_allclose(res.pvalue, expected[1], atol=1e-15)
    ...
    def test_asymmetry(self):
        ...
        p = 0.92891940883700  # same p-value for either direction
        res = stats.somersd(x, y)
    ...
    def test_somersd_large_inputs_gh18132(self):
        ...
        val_sklearn = -0.01528138777036947
...
class TestBarnardExact:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ([[43, 40], [10, 39]], (3.555406779643, 0.00362832367)),
            ([[100, 2], [1000, 5]], (-1.776382925679, 0.135126970878)),
            ([[2, 7], [8, 2]], (-2.518474945157, 0.19210815430)),
            ([[5, 1], [10, 10]], (1.449486150679, 0.156277546306)),
            ([[5, 15], [20, 20]], (-1.851640199545, 0.66363501421)),
            ([[5, 16], [20, 25]], (-1.609639949352, 0.116984852192)),
            ([[10, 5], [10, 1]], (-1.449486150679, 0.177536588915)),
            ([[5, 0], [1, 4]], (2.581988897472, 0.13671875000)),
            ([[0, 1], [3, 2]], (-1.95445115010, 0.509667991877)),
    ...
    @pytest.mark.parametrize(
        "input_sample,expected",
        [
            ([[2, 7], [8, 2]], (-2.518474945157, 0.09886140845)),
...
class TestBoschlooExact:
    ...
    @pytest.mark.parametrize(
        "input_sample,expected",
        [
            ([[2, 7], [8, 2]], (0.1852173, 0.09886142)),
            ([[5, 1], [10, 10]], (0.9782609, 0.9450994)),
            ([[5, 16], [20, 25]], (0.8913823, 0.5827348)),
            ([[10, 5], [10, 1]], (0.1652174, 0.8565611)),
            ([[5, 0], [1, 4]], (1, 1)),
            ([[0, 1], [3, 2]], (0.5, 0.34375)),
            ([[2, 7], [8, 2]], (0.1852173, 0.09886142)),
            ([[7, 12], [8, 3]], (0.6406797, 0.3410916)),
    ...
    @pytest.mark.parametrize(
        "input_sample,expected",
        [
            ([[43, 40], [10, 39]], (0.002875544, 0.001615562)),
            ([[2, 7], [8, 2]], (0.9990149, 0.9918327)),
            ([[5, 1], [10, 10]], (0.1652174, 0.9008534)),
            ([[5, 15], [20, 20]], (0.9849087, 0.9706997)),
            ([[5, 16], [20, 25]], (0.972349, 0.9524124)),
            ([[5, 0], [1, 4]], (0.2380952, 0.06865367)),
            ([[0, 1], [3, 2]], (1, 1)),
            ([[0, 2], [6, 4]], (1, 1)),
            ([[2, 7], [8, 2]], (0.9990149, 0.9918327)),
    ...
    @pytest.mark.parametrize(
        "input_sample,expected",
        [
            ([[43, 40], [10, 39]], (0.002875544, 0.003231115)),
            ([[2, 7], [8, 2]], (0.1852173, 0.1977228)),
            ([[5, 1], [10, 10]], (0.1652174, 0.1801707)),
            ([[5, 16], [20, 25]], (0.8913823, 0.116547)),
            ([[5, 0], [1, 4]], (0.2380952, 0.1373073)),
            ([[0, 1], [3, 2]], (0.5, 0.6875)),
            ([[2, 7], [8, 2]], (0.1852173, 0.1977228)),
            ([[7, 12], [8, 3]], (0.6406797, 0.6821831)),
        ],
    )
    ...
    def test_two_sided_gt_1(self):
        ...
        # the minimum of the one-sided p-values does. See gh-15345.0
        tbl = [[1, 1], [13, 12]]
    ...
    def test_against_fisher_exact(self, alternative):
        ...
        # p-value of `fisher_exact` (for one-sided tests). See gh-15345.0
        tbl = [[2, 7], [8, 2]]
...
class TestCvm_2samp:
    ...
    def test_exact_pvalue(self, statistic, m, n, pval):
        ...
        # two-sample Cramer-von-Mises criterion, 1962.0
...
class TestTukeyHSD:
    ...
    sas_diff_size = """
    Comparison LowerCL Difference UpperCL Significance
    2 - 1	0.2679292645	3.645	7.22070736	    1
    2 - 3	0.5934764007	4.34	8.86523599	    1
    1 - 2	-7.22070736	-3.645	-0.2679292645	1
    1 - 3	-2.682070736	0.695	4.72070736	    0
    3 - 2	-8.86523599	-4.34	-0.5934764007	1
    3 - 1	-4.72070736	-0.695	2.682070736	    0
    """

    sas_extreme = """
    Comparison LowerCL Difference UpperCL Significance
    2 - 3	1.561605075	    4.34	7.118394925	    1
    2 - 1	2.740784879	    6.8	9.419215121	    1
    3 - 2	-7.118394925	-4.34	-1.561605075	1
    3 - 1	-1.964526566	1.74	5.444526566	    0
    1 - 2	-9.419215121	-6.8	-2.740784879	1
    ...
    def test_compare_sas(self, data, res_expect_str, atol):
        ...
        for i, j, l, s, h, sig in res_expect:
            ...
            assert_allclose((res_tukey.pvalue[i, j] <= 0.5), sig == 1)

    matlab_sm_siz = """
        1	2	-8.2491590248597	-4.6	-0.9508409751403	0.144483269098
        1	3	-3.9091590248597	-0.26	3.3891590248597	0.9803107240900
        2	3	0.6908409751403	4.34	7.9891590248597	0.203311368795
        """

    matlab_diff_sz = """
        1	2	-7.2207069748501	-3.645	-0.26792930251500 0.3371498443080
        1	3	-2.68207069748500	0.695	4.7207069748500 0.85572267328807
        2	3	0.59347644287720	4.34	8.8652355712281 0.2259047020620
        """
    ...
    def test_compare_r(self):
        ...
        1 - 3 14.722222  5.1623978 24.28205 0.014315
        1 - 2 10.00000  0.4401756 19.55982 0.384598
        """
    ...
    def test_rand_symm(self):
        ...
        # mean difference in a self comparison is 0.0
...
class TestPoissonMeansTest:
    @pytest.mark.parametrize("c1, n1, c2, n2, p_expect", (
        # example from [1], 6.0 Illustrative examples: Example 1
        [0, 100, 3, 100, 0.884],
        [2, 100, 6, 100, 0.1749]
    ))
    ...
    @pytest.mark.parametrize("c1, n1, c2, n2, p_expect, alt, d", (
        ...
        [1, 1, 50, 15, 0.9322998607245102, 'less', 0]
    ))
    ...
    def test_input_validation(self):
        ...
        message = '`k1` and `k2` must be greater than or equal to 0.0'
        with assert_raises(ValueError, match=message):
        ...
        message = '`n1` and `n2` must be greater than 0.0'
        with assert_raises(ValueError, match=message):
        ...
        message = 'diff must be greater than or equal to 0.0'
        with assert_raises(ValueError, match=message):
...
class TestBWSTest:
    ...
    @pytest.mark.parametrize(('alternative', 'statistic', 'pvalue'),
                             [('two-sided', 1.7510204081633, 0.1264422777777),
                              ('less', -1.7510204081633, 0.5754662004662),
