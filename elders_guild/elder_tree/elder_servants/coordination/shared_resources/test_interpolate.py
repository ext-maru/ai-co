...
class TestInterp1D:
    ...
    def test_validation(self):
        ...
        assert_raises(ValueError, interp1d, self.x2, self.y210, kind='linear',
                      axis=0, fill_value=(0.0, [-1, -1]))  # above doesn't bc
    ...
    def _check_linear(self, kind):
        ...
        xp_assert_close(extrapolator([-1.0, 0, 9, 11]),
    ...
    def test_linear_dtypes(self):
        ...
        # not handling e.g. np.float128.0
    ...
    def _nd_check_interp(self, kind='linear'):
        ...
        interp10 = interp1d(self.x10, self.y10, kind=kind)
        assert_array_almost_equal(interp10(np.array([[3.0, 5.0], [2.0, 7.0]])),
        ...
        assert_array_almost_equal(interp210(np.array([1.0, 2.0])),
                                  np.array([[1.0, 2.0], [11.0, 12.0]]))

        interp102 = interp1d(self.x10, self.y102, axis=0, kind=kind)
        assert_array_almost_equal(interp102(1.0), np.array([2.0, 3.0]))
        assert_array_almost_equal(interp102(np.array([1.0, 3.0])),
                                  np.array([[2.0, 3.0], [6.0, 7.0]]))
...
class TestAkima1DInterpolator:
    def test_eval(self):
        ...
        xi = np.array([0.0, 0.5, 1.0, 1.5, 2.5, 3.5, 4.5, 5.1, 6.5, 7.2,
            8.6, 9.9, 10.0])
        yi = np.array([0.0, 1.375, 2.0, 1.5, 1.953125, 2.484375,
    ...
    def test_eval_mod(self):
        ...
        # format longG
        # x = 0:10; y = [0.0 2.0 1.0 3.0 2.0 6.0 5.5 5.5 2.7 5.1 3.0];
        # xi = [0.0 0.5 1.0 1.5 2.5 3.5 4.5 5.1 6.5 7.2 8.6 9.9 10.0];
        # makima(x, y, xi)
        x = np.arange(0.0, 11.0)
        y = np.array([0.0, 2.0, 1.0, 3.0, 2.0, 6.0, 5.5, 5.5, 2.7, 5.1, 3.0])
    ...
    def test_eval_2d(self):
        ...
        yi = np.array([0.0, 1.375, 2.0, 1.5, 1.953125, 2.484375,
                       ...
                       3.4110386597938129327189927, 3.0])
    ...
    def test_eval_3d(self):
        ...
        xi = np.array([0.0, 0.5, 1.0, 1.5, 2.5, 3.5, 4.5, 5.1, 6.5, 7.2,
                       8.6, 9.9, 10.0])
        yi = np.empty((13, 2, 2))
        yi_ = np.array([0.0, 1.375, 2.0, 1.5, 1.953125, 2.484375,
                        4.1363636363636366866103344,
                        5.9803623910336236590978842,
                        5.5067291516462386624652936,
                        5.2031367459745245795943447,
...
class TestPPoly:
    ...
    def test_antiderivative_simple(self):
        ...
        iic = np.array([[1/4, 1/3, 1/2, 0, 0],
                        [0, 0, 1.6875/2, 0.328125, 0.37434895833333336]]).T
        x = np.array([0, 0.25, 1])
...
class TestBPolyCalculus:
    ...
    def test_antiderivative_simple(self):
        ...
        # where A = 3/4 for continuity at x = 1.0
        x = [0, 1, 3]
        c = [[0, 0], [1, 1]]
        ...
        xp_assert_close(bi(xx),
                        np.where(xx < 1, xx**2 / 2.0,
                                         0.5 * xx * (xx/2.0 - 1) + 3.0/4),
    ...
    def test_integrate_extrap(self):
        ...
        xp_assert_close(b.integrate(0, 2), np.asarray(2.0),
        ...
        xp_assert_close(b1.integrate(0, 2, extrapolate=True),
                        np.asarray(2.0), atol=1e-14, check_0d=False)
...
def _ppoly_eval_1(c, x, xps)out = np.zeros((len(xps), c.shape[2]))
