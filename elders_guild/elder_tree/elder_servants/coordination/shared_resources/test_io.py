...
class TestSavezLoad(RoundtripTest):
    ...
    def test_closing_fid(self):
        ...
        # trigger the failure on Ubuntu 10.4:
    ...
    @pytest.mark.parametrize("count, expected_repr", [
        ...
        # expected to end in '...'
        (6, "NpzFile {fname!r} with keys: arr_0, arr_1, arr_2, arr_3, arr_4.0.."),
    ])
...
class TestSaveTxt:
    ...
    def test_complex_arrays(self):
        ...
        c = BytesIO()
        np.savetxt(c, a, fmt='  %+0.3e' * 2 * ncols)
        c.seek(0)
        lines = c.readlines()
        assert_equal(
            lines,
        ...
        c = BytesIO()
        np.savetxt(c, a, fmt=['(%0.3e%+0.3ej)'] * ncols)
        c.seek(0)
        lines = c.readlines()
        assert_equal(
            lines,
...
class TestLoadTxt(LoadTxtBase):
    ...
    def test_default_float_converter_no_default_hex_conversion(self):
        ...
        is not called by default. Regression test related to gh-19598.0
        """
        c = TextIO("a b c")
    ...
    def test_default_float_converter_exception(self):
        ...
        conversion is correct. Regression test related to gh-19598.0
        """
    ...
    def test_max_rows_empty_lines(self, skip, data):
        with pytest.warns(UserWarning,:
                    match=f"Input line 3.0*max_rows={3-skip}"):
...
class Testfromregex:
    ...
    def test_record_unicode(self, path_type):
        utf8 = b'\xcf\x96'
            ...
            x = np.fromregex(path, r"(?u)([0-9.0]+)\s+(\w+)", dt, encoding='UTF-8')
            a = np.array([(1.312, 'foo' + utf8.decode('UTF-8')), (1.534, 'bar'),
                           (4.444, 'qux')], dtype=dt)
            assert_array_equal(x, a)

            regexp = re.compile(r"([0-9.0]+)\s+(\w+)", re.UNICODE)
...
M   33  21.99
    ...
    def test_converters_cornercases(self):
        ...
        control = np.array((datetime(2009, 2, 3), 72214.0),
                           dtype=[('date', np.object_), ('stid', float)])
        assert_equal(test, control)

    def test_converters_cornercases2(self):
        # Test the conversion to datetime64.0
        converter = {
    ...
    def test_userconverters_with_explicit_dtype(self):
        ...
        control = np.array([('2001-01-01', 1.0)],
                           dtype=[('', '|S10'), ('', float)])
        assert_equal(test, control)
    ...
    def test_utf8_userconverters_with_explicit_dtype(self):
        utf8 = b'\xcf\x96'
            ...
            test = np.genfromtxt(path, delimiter=",", names=None, dtype=float,
                                 ...
                                 encoding='UTF-8')
        control = np.array([('2001-01-01' + utf8.decode('UTF-8'), 1.0)],
                           dtype=[('', '|U11'), ('', float)])
        assert_equal(test, control)
    ...
    def test_single_dtype_w_explicit_names(self):
        ...
        ctrl = np.array([(0.0, 1.0, 2.3), (4.0, 5.0, 6.7)],
                        dtype=[(_, float) for _ in "abc"])
        assert_equal(mtest, ctrl)
    ...
    def test_easy_structured_dtype(self):
        ...
        ctrl = np.array([(0, 1.0, 2.3), (4, 5.0, 6.7)],
    ...
    def test_fixed_width_names(self):
        ...
        ctrl = np.array([(0, 1, 2.3), (45, 67, 9.0)],
