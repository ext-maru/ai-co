...
def sol_event_dense_output_LSODA(t):
    return np.exp(t ** 2 / 2 - 2 * t + np.log(0.5) - 6)
...
def test_duplicate_timestamps():
    ...
    sol = solve_ivp(upward_cannon, [0, np.inf], [0, 0.1],
                    max_step=0.5 * 0.01 / 9.80665,
                    events=hit_ground, dense_output=True)
    assert_allclose(sol.sol(0.1), np.asarray([-0.0039033, -0.8806632]),
                    rtol=1e-5, atol=1e-8)
    assert_allclose(sol.t_events, np.asarray([[0.0203943]]), rtol=1e-5, atol=1e-8)
    assert_allclose(sol.y_events, [np.asarray([[ 0.0, -0.1 ]])], atol=1e-9)
    assert sol.success
...
def test_args():
    ...
    # system with equilibria at z=0 and z=1.0 If k > 0, z=1
...
def test_tbound_respected_oscillator(method):
    ...
    def run_sim2(t_end, n_timepoints=10, shortest_delay_line=10000000):
        init_state = np.array([134.8298555, 138.82348612, 100.0, 0.0])
        t0 = 100.0
        t1 = 200.0
