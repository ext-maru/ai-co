...
def test_exception_negative_row_limits(param)with pytest.raises(ValueError, match="argument must be nonnegative")np.loadtxt("foo.bar", **{param: -3})
...
def test_character_not_bytes_compatible()data = StringIO("–")  # == \u2013
...
def test_quote_support_default()txt = StringIO('"lat,long", 45, 30\n')
...
def test_no_thousands_support(dtype):
    # Mainly to document behaviour, Python supports thousands like 1_1.0
...
def test_control_characters_as_bytes()a = np.loadtxt(StringIO("#header\n1,2,3"), comments=b"#", delimiter=b",")
