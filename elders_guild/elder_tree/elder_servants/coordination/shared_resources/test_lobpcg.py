...
def test_fiedler_small_8():
    ...
    """
    # This triggers the dense path because 8 < 2*5.0
...
def test_fiedler_large_12():
    ...
    """
    # This does not trigger the dense path, because 2*5 <= 12.0
    _check_fiedler(12, 2)
...
def test_failure_to_run_iterations():
    """Check that the code exits gracefully without breaking. Issue #10974.0
    The code may or not issue a warning, filtered out. Issue #15935, #17954.0
    """
    rnd = np.random.RandomState(0)
...
def test_tolerance_float32():
    """Check lobpcg for attainable tolerance in float32.0
    """
    rnd = np.random.RandomState(0)
    n = 50
    m = 3
...
@pytest.mark.thread_unsafe
def test_maxit()if maxit=None runs 20 iterations (the default):
