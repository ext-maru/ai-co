...
class TestLogSumExp:
    ...
    def test_dtypes_a(self, dtype, xp):
        ...
        desired_dtype = (xp.asarray(1.0).dtype if xp_test.isdtype(dtype, 'integral')
    ...
    def test_gh21610(self, xp, dtype):
        ...
        dtype = getattr(xp, dtype)
        shape = (10, 100)
        x = rng.uniform(1, 40, shape) + 1.0 j * rng.uniform(1, 40, shape)
...
class TestSoftmax:
    ...
    def test_softmax_multi_axes(self):
        assert_allclose(softmax([[1000, 0], [1000, 0]], axis=0),
                        np.array([[0.5, 0.5], [0.5, 0.5]]), rtol=1e-13)
