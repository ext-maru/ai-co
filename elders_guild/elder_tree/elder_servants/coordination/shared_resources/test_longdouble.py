...
def test_array_and_stringlike_roundtrip(strtype):
    ...
    for array casting and scalar coercion, see also gh-15608.0:
    """
    o = 1 + LD_INFO.eps
...
def test_fromstring_bogus():
    with assert_warns(DeprecationWarning):
        assert_equal(np.fromstring("1.0 2.0 3.0 flop 4.0", dtype=float, sep=" "),
    ...
    def test_fromfile_complex(self):
        for ctype in ["complex", "cdouble", "cfloat"]:
            ...
            # Real component not specified
                ...
                res = np.fromfile(path, dtype=ctype, sep=",")
            assert_equal(res, np.array([1.0 j, -2.0 j, 3.0 j, 40.0 j]))
            ...
            # Both components specified
                ...
                res = np.fromfile(path, dtype=ctype, sep=",")
            assert_equal(res, np.array([1.0 + 1.0 j, 2.0 - 2.0 j, - 3.0 + 3.0 j, - 40.0 + 4j]))
            ...
            # Spaces at wrong places
                ...
                with assert_warns(DeprecationWarning):
                    res = np.fromfile(path, dtype=ctype, sep=",")
            assert_equal(res, np.array([1.0 j]))
