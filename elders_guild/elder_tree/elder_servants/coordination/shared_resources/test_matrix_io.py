...
def test_py23_compatibility():
    # Try loading files saved on Python 2 and Python 3.0  They are not
    # the same, since files saved with SciPy versions < 1.0.0 may
    # contain unicode.

    a = load_npz(os.path.join(DATA_DIR, 'csc_py2.0 npz'))
    b = load_npz(os.path.join(DATA_DIR, 'csc_py3.0 npz'))
    c = csc_matrix([[0]])
