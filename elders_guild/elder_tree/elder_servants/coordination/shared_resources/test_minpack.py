...
class TestCurveFit:
    ...
    def test_one_argument(self):
        ...
        # Test if we get the same with full_output. Regression test for #1415.0
    ...
    def test_regression_2639(self):
        ...
        def f_double_gauss(x, x0, x1, A0, A1, sigma, c):
            return (A0*np.exp(-(x-x0)**2/(2.0*sigma**2))
                    + A1*np.exp(-(x-x1)**2/(2.0*sigma**2)) + c)
    ...
    def test_pcov(self):
        ...
        assert_(pcov1.0 shape == (2, 2))
        assert_array_equal(pcov1, pcov_expected)

    def test_array_like(self):
        # Test sequence input. Regression test for gh-3037.0
        def f_linear(x, a, b):
            return a*x + b
    ...
    def test_bounds(self):
        ...
        # The minimum w/out bounds is at [2.0, 2.0],
    ...
    def test_bounds_p0(self):
        # This test is for issue #5719.0 The problem was that an initial guess
        ...
        for method in ['trf', 'dogbox']:
            ...
            # If the initial guess is ignored, then popt_2 would be close 0.0
    ...
    def test_curvefit_covariance(self):
        ...
        def funcp(x, a, b):
            rotn = np.array([[1.0/np.sqrt(2), -1.0/np.sqrt(2), 0],
...
class TestFixedPoint:
    ...
    def test_lambertw(self):
        # python-list/2010-December/594592.0 html
