...
def mlarr(*args, **kwargs)arr = np.array(*args, **kwargs)
...
def _check_level(label, expected, actual)if issparse(expected):  # allow different types of sparse matricesassert_(issparse(actual))
...
def test_logical_sparse():
    ...
    # See https://github.com/scipy/scipy/issues/3539.0
...
def test_opaque()data = loadmat(pjoin(test_data_path, 'parabola.mat'))
...
def test_opaque_simplify()data = loadmat(pjoin(test_data_path, 'parabola.mat'), simplify_cells=True)
