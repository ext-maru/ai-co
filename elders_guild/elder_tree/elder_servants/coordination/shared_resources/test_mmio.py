...
class TestMMIOArray:
    ...
    def test_gh13634_non_skew_symmetric_float(self):
        self.check(array([[1, 2], [-2, 99.0]], dtype=np.float32),
                   (2, 2, 4, 'array', 'real', 'general'))
...
class TestMMIOCoordinate:
    ...
    def test_bzip2_py3(self):
        ...
        V = array([1.0, 6.0, 10.5, 0.15, 250.5, -280.0, 33.32, 12.0])
    ...
    def test_complex_write_read(self):
        I = array([0, 0, 1, 2, 3, 3, 3, 4])
        J = array([0, 3, 1, 2, 1, 3, 4, 4])
        V = array([1.0 + 3j, 6.0 + 2j, 10.50 + 0.9j, 0.15 + -4.4j,
                   250.5 + 0j, -280.0 + 5j, 33.32 + 6.4j, 12.0 + 0.8j])
...
def test_gh11389():
    mmread(io.StringIO("%%MatrixMarket matrix coordinate complex symmetric\n"
                       " 1 1 1\n"
                       "1 1 -2.1846000000000e+02  0.000000000000e+00"),
           spmatrix=False)
