...
def test_hyp2f1_real_some_points():
    pts = [
        ...
        (3.0/2, -0.5, 3, 0.99),
        (2, 2.5, -3.25, 0.999),
        (-8, 18.16500331508873, 10.805295997850628, 0.90875647507000001),
        (-10, 900, -10.5, 0.99),
        (-10, 900, 10.5, 0.99),
        (-1, 2, 1, 1.0),
        (-1, 2, 1, -1.0),
    ...
    for a in [-10, -5, -1.8, 1.8, 5, 10]:
        for b in [-2.5, -1, 1, 7.4]:
            for c in [-9, -1.8, 5, 20.4]:
                for z in [-10, -1.1, -0.99, 0, 0.6, 0.95, 1.5, 10]:
                    try:
...
def test_loggamma_taylor_transition():
    ...
    r = LOGGAMMA_TAYLOR_RADIUS + np.array([-0.1, -0.1, 0, 0.1, 0.1])
...
def test_loggamma_taylor():
    # Test around the zeros at z = 1, 2.0
...
def test_rgamma_zeros():
    # Test around the zeros at z = 0, -1, -2, ...,  -169.0 (After -169 we
...
class TestSystematic:
    ...
    def test_lanczos_sum_expg_scaled(self):
        ...
        e = np.exp(1)
        g = 6.24680040776729583740234375
