...
NbufferT = [
    ...
    #       value y2 Info2                            name z2         Name Value
    #                name   value    y3       z3
    ([3, 2], (6j, 6.0, (b'nn', [6j, 4j], [6.0, 4.0], [1, 2]), b'NN', True),
     b'cc', ('NN', 6j), [[6.0, 4.0], [6.0, 4.0]], 8),
    ([4, 3], (7j, 7.0, (b'oo', [7j, 5j], [7.0, 5.0], [2, 1]), b'OO', False),
     b'dd', ('OO', 7j), [[7.0, 5.0], [7.0, 5.0]], 9),
    ]
...
class CreateZeros:
    ...
    def test_zeros0D(self)h = np.zeros((), dtype=self._descr)assert_(normalize_descr(self._descr) == h.dtype.descr)
    ...
    def test_zerosSD(self)h = np.zeros((2,), dtype=self._descr)assert_(normalize_descr(self._descr) == h.dtype.descr)
    ...
    def test_zerosMD(self)h = np.zeros((2, 3), dtype=self._descr)assert_(normalize_descr(self._descr) == h.dtype.descr)
...
class CreateValues:
    ...
    def test_tuple(self)h = np.array(self._buffer, dtype=self._descr)assert_(normalize_descr(self._descr) == h.dtype.descr)
    ...
    def test_list_of_tuple(self)h = np.array([self._buffer], dtype=self._descr)assert_(normalize_descr(self._descr) == h.dtype.descr)
    ...
    def test_list_of_list_of_tuple(self)h = np.array([[self._buffer]], dtype=self._descr)assert_(normalize_descr(self._descr) == h.dtype.descr)
...
class ReadValuesNested:
    ...
    def test_access_top_fields(self)h = np.array(self._buffer, dtype=self._descr):
    ...
    def test_nested1_acessors(self)h = np.array(self._buffer, dtype=self._descr):
    ...
    def test_nested2_acessors(self)h = np.array(self._buffer, dtype=self._descr):
    ...
    def test_nested1_descriptor(self)h = np.array(self._buffer, dtype=self._descr)assert_(h.dtype['Info']['value'].name == 'complex128')
    ...
    def test_nested2_descriptor(self)h = np.array(self._buffer, dtype=self._descr)assert_(h.dtype['Info']['Info2']['value'].name == 'void256')
...
class TestScalarTypeNames:
    ...
    def test_names_reflect_attributes(self, t)assert getattr(np, t.__name__) is t:
    """ Test that names correspond to where the type is under ``np.`` """

    @pytest.mark.parametrize('t', numeric_types)
    def test_names_are_undersood_by_dtype(self, t)assert np.dtype(t.__name__).type is t:
