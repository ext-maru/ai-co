...
class TestODR:
    ...
    def test_explicit(self):
        ...
        explicit_dat = Data([0.0,0.0,5.0,7.0,7.5,10.0,16.0,26.0,30.0,34.0,34.5,100.0],
                        [1265.0,1263.6,1258.0,1254.0,1253.0,1249.8,1237.0,1218.0,1220.6,
                         1213.8,1215.5,1212.0])
        ...
        assert_array_almost_equal(
            out.sd_beta,
            np.array([1.349270280543437, 1.583997785262061, 0.063321988657267]),
        )
        assert_array_almost_equal(
            out.cov_beta,
            np.array([[4.4949592379003039e-01, -3.7421976890364739e-01,
                 -8.978217468468912e-04],
               [-3.7421976890364739e-01, 1.529686462751804e+00,
                 -1.9453521827942002e-03],
               [-8.978217468468912e-04, -1.9453521827942002e-03,
    ...
    def test_implicit(self):
        ...
        implicit_dat = Data([
            [0.5,1.2,1.6,1.86,2.12,2.36,2.44,2.36,2.6,1.74,1.34,0.9,-0.28,
             -0.78,-1.36,-1.9,-2.5,-2.88,-3.18,-3.44],
            [-0.12,-0.6,-1.0,-1.4,-2.54,-3.36,-4.0,-4.75,-5.25,-5.64,-5.97,-6.32,
             -6.44,-6.44,-6.41,-6.25,-5.88,-5.5,-5.24,-4.86]],
            1,
        )
        implicit_odr = ODR(implicit_dat, implicit_mod,
            beta0=[-1.0, -3.0, 0.9, 0.2, 0.8])

        out = implicit_odr.run()
        assert_array_almost_equal(
            out.beta,
            np.array([-0.9993809167281279, -2.9310484652026476, 0.875730502693354,
                0.162299708984738, 0.797537982976416]),
        )
        assert_array_almost_equal(
            out.sd_beta,
            np.array([0.1113840353364371, 0.1097673310686467, 0.041060738314314,
                0.027500347539902, 0.034962501532468]),
        )
        assert_allclose(
            out.cov_beta,
            np.array([[2.1089274602333052e+00, -1.9437686411979040e+00,
                  7.263550868344446e-02, -4.7175267373474862e-02,
                  5.2515575927380355e-02],
               [-1.9437686411979040e+00, 2.481509222414456e+00,
                 -6.1600515853057307e-02, 4.6268827806232933e-02,
                 -5.8822307501391467e-02],
               [7.263550868344446e-02, -6.1600515853057307e-02,
    ...
    def test_multi(self):
        ...
        multi_y = np.array([
            [4.22, 4.167, 4.132, 4.38, 4.19, 3.956, 3.884, 3.784, 3.713,
             3.633, 3.54, 3.433, 3.358, 3.258, 3.193, 3.128, 3.59, 2.984,
        ...
        multi_dat = Data(multi_x, multi_y, wd=1e-4/np.power(multi_x, 2),
            we=multi_we)
        multi_odr = ODR(multi_dat, multi_mod, beta0=[4.0,2.0,7.0,0.4,0.5],
        ...
        assert_array_almost_equal(
            out.beta,
            np.array([4.3799880305938963, 2.4333057577497703, 8.028845899503978,
                0.5101147161764654, 0.5173902330489161]),
        )
        assert_array_almost_equal(
            out.sd_beta,
            np.array([0.130625231081944, 0.130499785273277, 0.1167085962217757,
                0.132642749596149, 0.288529201353984]),
        )
        assert_array_almost_equal(
            out.cov_beta,
            np.array([[0.064918418231375, 0.036159705923791, 0.438637051470406,
                -0.058700836512467, 0.11281212888768],
               [0.036159705923791, 0.064793789429006, 0.517610978353126,
                -0.051181304940204, 0.130726943624117],
               [0.438637051470406, 0.517610978353126, 0.5182263323095322,
                -0.563083340093696, 0.1269490939468611],
               [-0.058700836512467, -0.051181304940204, -0.563083340093696,
                 0.066939246261263, -0.140184391377962],
               [0.11281212888768, 0.130726943624117, 0.1269490939468611,
                -0.140184391377962, 0.316733013820852]]),
        )
    ...
    def test_pearson(self):
        ...
        p_sx = np.array([0.3,0.3,0.4,0.35,0.7,0.11,0.13,0.22,0.74,1.0])
        p_sy = np.array([1.0,0.74,0.5,0.35,0.22,0.22,0.12,0.12,0.1,0.4])
        ...
        assert_array_almost_equal(
            out.sd_beta,
            np.array([0.3590121690702467, 0.706291186037444]),
        )
        assert_array_almost_equal(
            out.cov_beta,
            np.array([[0.854275622946333, -0.161807025443155],
               [-0.161807025443155, 0.03306337993922]]),
        )

        rout = pr_odr.run()
        assert_array_almost_equal(
            rout.beta,
            np.array([11.4192022410781231, -2.850374506165474]),
        )
        assert_array_almost_equal(
            rout.sd_beta,
        ...
        assert_array_almost_equal(
            ...
            np.array([[0.6391799462548782, -0.1955657291119177],
               [-0.1955657291119177, 0.624888159223392]]),
        )
    ...
    def test_lorentz(self):
        ...
        l_sx = np.array([0.00972971,0.00948268,0.00707632,0.00706679,
            0.00706074, 0.00703918,0.00698955,0.00456856,
            0.00455207,0.00662717,0.00654619,0.00652694,
            0.00000859202,0.0106589,0.0106378,0.0125483, 0.0140818,0.0241839])
    ...
    def test_ifixx(self):
        x1 = [-2.1, -0.99, -0.01, 1.2, 1.98]
        x2 = [3.98, 1.1, 0.01, 0.998, 4.1]
    ...
    def test_ticket_11800(self):
        ...
        std_dev_x = 0.1
        x_error = np.array([[0.0063445, 0.0515731, 0.0162719, 0.1022866,
            -0.1624845, 0.0482652, 0.0275988, -0.0714734, -0.0929201, -0.0687301],
            [-0.0831623, -0.0821211, -0.0203459, 0.0938266, -0.0701829,
            0.032169, 0.0259194, -0.0581017, -0.030283, 0.1014164]])

        std_dev_y = 0.5
        y_error = np.array([[0.5275304, 0.4519563, -0.7524086, 0.3575642,
            0.4745194, 0.3806645, 0.7061601, -0.0753604, -0.2592543, -0.2394929],
            [0.3632366, 0.6642266, 0.8373122, 0.3988822, -0.092536,
            -0.3750469, -0.3198903, 0.1642066, 0.1293648, -0.5627085]])
    ...
    def test_work_array_del_init(self):
        ...
        rd4 = RealData(x, y, sx=x_err, covy=[[0.1]])
        rd5 = RealData(x, y, sx=x_err, covy=np.full((1, 1, n_data), 0.1))
