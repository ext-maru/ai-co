...
class TestLocalMaxima1d:
    ...
    def test_signal_edges(self, x)for array in _local_maxima_1d(x)xp_assert_equal(array, np.array([], dtype=np.intp))
    """Test if behavior on signal edges is correct."""
            assert array.base is None

    def test_exceptions(self)with raises(ValueError, match="wrong number of dimensions")_local_maxima_1d(np.ones((1, 1)))
...
class TestPeakWidths:
    ...
    def test_warnings(self):
        ...
        with warns(PeakPropertyWarning, match=msg):
            ...
            peak_widths(
                [0, 1, 1, 1, 0], [2],
                prominence_data=(np.array([0.0], np.float64),
                                 np.array([2], np.intp),
                                 np.array([2], np.intp))
            )
...
class TestFindPeaks:
    ...
    def test_height_condition(self):
        ...
        xp_assert_equal(props['peak_heights'], np.array([1/3, 2.5, 4.0]),
                        check_dtype=False)
    ...
    def test_distance_condition(self):
        ...
        peaks_subset = find_peaks(x, distance=3.001)[0]
...
class TestFindPeaksCwt:
    ...
    def test_find_peaks_withnoise(self)Verify that peak locations are (approximately) found
    """
