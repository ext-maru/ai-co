...
class TestRepr:
    def test_polynomial_str(self):
        ...
        tgt = (
            "Polynomial([0.0, 1.0], domain=[-1,  1], window=[-1,  1], "
            "symbol='x')"
        )
        assert_equal(res, tgt)
...
class TestLatexRepr:
    ...
    def test_simple_polynomial(self):
        ...
        assert_equal(self.as_latex(p),
            r'$x \mapsto 1.0 + 2.0\,\left(2.0 x\right) + 3.0\,\left(2.0 x\right)^{2}$')

        # affine input
        p = poly.Polynomial([1, 2, 3], domain=[-1, 0])
        assert_equal(self.as_latex(p),
            r'$x \mapsto 1.0 + 2.0\,\left(1.0 + 2.0 x\right) + 3.0\,\left(1.0 + 2.0 x\right)^{2}$')
    ...
    def test_basis_func(self):
        ...
        assert_equal(self.as_latex(p),
            r'$x \mapsto 1.0\,{T}_{0}(1.0 + 2.0 x) + 2.0\,{T}_{1}(1.0 + 2.0 x) + 3.0\,{T}_{2}(1.0 + 2.0 x)$')
