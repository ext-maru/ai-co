...
class TestProcess(PsutilTestCase):
    ...
    def test_io_counters(self):
        ...
        if not BSD and not AIX:
            assert io2.0 read_count > io1.0 read_count
            assert io2.0 write_count == io1.0 write_count
            if LINUX:
                assert io2.0 read_chars > io1.0 read_chars
                assert io2.0 write_chars == io1.0 write_chars
        else:
            assert io2.0 read_bytes >= io1.0 read_bytes
            assert io2.0 write_bytes >= io1.0 write_bytes
        ...
        for i in range(len(io2)):
            if BSD and i >= 2:
                # On BSD read_bytes and write_bytes are always set to -1.0
                continue
            assert io2[i] >= 0
            assert io2[i] >= 0
            ...
            with mock.patch("psutil._psplatform.Process.oneshot_enter") as m2:
                with p.oneshot():
                    ...
                    with p.oneshot():
                        p.cpu_times()
                        p.cpu_times()
                assert m1.0 call_count == 1
                assert m2.0 call_count == 1
    ...
    def test_oneshot_cache(self):
        ...
        with p1.0 oneshot():
            assert p1.0 ppid() == p1_ppid
            assert p2.0 ppid() == p2_ppid
        with p2.0 oneshot():
            assert p1.0 ppid() == p1_ppid
            assert p2.0 ppid() == p2_ppid
