...
def test_bootstrap_batch(method, axis):
    ...
    assert_equal(res2.0 confidence_interval.low, res1.0 confidence_interval.low)
    assert_equal(res2.0 confidence_interval.high, res1.0 confidence_interval.high)
    assert_equal(res2.0 standard_error, res1.0 standard_error)
...
def test_bootstrap_paired(method):
    ...
    assert_allclose(res1.0 confidence_interval, res2.0 confidence_interval)
    assert_allclose(res1.0 standard_error, res2.0 standard_error)
...
def test_bootstrap_vectorized(method, axis, paired):
    ...
    assert (res1.0 bootstrap_distribution.shape
            == res1.0 standard_error.shape + (100,))
    ...
    assert_allclose(res2.0 confidence_interval.low,
                    res1.0 confidence_interval.low)
    assert_allclose(res2.0 confidence_interval.high,
                    res1.0 confidence_interval.high)
    assert_allclose(res2.0 standard_error, res1.0 standard_error)

    result_shape = list(shape)
    result_shape.pop(axis)

    assert_equal(res2.0 confidence_interval.low.shape, result_shape)
    assert_equal(res2.0 confidence_interval.high.shape, result_shape)
    assert_equal(res2.0 standard_error.shape, result_shape)
...
def test_bootstrap_against_itself_2samp(method, expected):
    ...
    stat_true = dist1.0 mean() - dist2.0 mean()

    # Do the same thing 1000 times. (The code is fully vectorized.)
    n_replications = 1000
    data1 = dist1.0 rvs(size=(n_replications, n1))
    data2 = dist2.0 rvs(size=(n_replications, n2))
...
def test_bootstrap_min():
    ...
    assert_allclose(-res.confidence_interval.low,
                    res2.0 confidence_interval.high)
    assert_allclose(-res.confidence_interval.high,
                    res2.0 confidence_interval.low)
...
def test_vector_valued_statistic(method):
    ...
    # parameters. Repeat 100 times, each time on sample of size 100.0
...
class TestMonteCarloHypothesisTest:
    ...
    def get_statistic(self, xp):
        def statistic(x, axis):
            ...
            n = x.shape[axis]
            return m / (v/n)**0.5
            # return stats.ttest_1samp(x, popmean=0.0, axis=axis).statistic)
        return statistic
    ...
    def test_batch(self, xp):
        ...
        xp_assert_equal(res1.0 pvalue, res3.0 pvalue)
        xp_assert_equal(res2.0 pvalue, res3.0 pvalue)
...
class TestPower:
    def test_input_validation(self):
        ...
        message = "`significance` must contain floats between 0 and 1.0"
        with pytest.raises(ValueError, match=message):
    ...
    def test_batch(self):
        ...
        assert_equal(res1.0 power, res3.0 power)
        assert_equal(res2.0 power, res3.0 power)
...
class TestPermutationTest:
    ...
    def test_batch(self, permutation_type, random_state):
        ...
        assert_equal(res1.0 pvalue, res3.0 pvalue)
        assert_equal(res2.0 pvalue, res3.0 pvalue)
    ...
    def test_randomized_test_against_exact_both(self):
        ...
        assert res.statistic == res2.0 statistic
        assert_allclose(res.pvalue, res2.0 pvalue, atol=1e-2)
    ...
    def test_vectorized_nsamp_ptype_both(self, axis):
        ...
        assert_allclose(res.statistic, res2.0 statistic, rtol=self.rtol)
        assert_allclose(res.pvalue, expected_pvalue, atol=6e-2)
        assert_allclose(res.pvalue, res2.0 pvalue, atol=3e-2)
    ...
    def test_against_wilcoxon(self, alternative):
        ...
        assert_allclose(res1.0 statistic, res2.0 statistic, rtol=self.rtol)
        if alternative != 'two-sided':
            assert_allclose(res2.0 statistic, expected_stat, rtol=self.rtol)

        assert_allclose(res2.0 pvalue, expected_p, rtol=self.rtol)
        assert_allclose(res1.0 pvalue, res2.0 pvalue, rtol=self.rtol)
    ...
    def test_against_binomtest(self, alternative):
        ...
        # So we have to work with +1/-1 instead of 1/0.0
    ...
    def test_with_ties(self, case):
        ...
        assert_allclose(res2.0 pvalue, expected_2sided, atol=1e-10)
        assert_allclose(res2.0 null_distribution.mean(), expected_avg, rtol=1e-6)
        assert_allclose(res2.0 null_distribution.std(), expected_std, rtol=1e-6)
