...
def compare_coeffs_to_alt(window_length, order):
    ...
    # of savgol_coeffs and alt_sg_coeffs for pos from 0 to window_length - 1.0
...
def test_sg_coeffs_exact():
    ...
    # Check the same input, but use deriv=1.0  dy is the exact result.
    dy = 1.5 * x ** 2 - 1
    h = savgol_coeffs(window_length, polyorder, deriv=1, delta=delta)
    y1 = convolve1d(y, h)
    xp_assert_close(y1[halflen:-halflen], dy[halflen:-halflen])

    # Check the same input, but use deriv=2.0 d2y is the exact result.
    d2y = 3.0 * x
...
def test_sg_coeffs_deriv_gt_polyorder():
    """
    If deriv > polyorder, the coefficients should be all 0.0
...
def test_sg_filter_trivial()x = np.array([1.0])
""" Test some trivial edge cases for savgol_filter()."""
    ...
    # (1, 0) at 0.0 This is just the average of the three values, hence 1.0.0
    x = np.array([3.0])
...
def test_sg_filter_interp_edges():
    ...
    # Transpose everything, and test again with axis=0.0
