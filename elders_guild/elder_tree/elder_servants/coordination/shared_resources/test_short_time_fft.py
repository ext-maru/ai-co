...
1.0 Test function `_calc_dual_canonical_window`.
2.0 Test for invalid parameters and exceptions in `ShortTimeFFT` (until the
    `test_from_window` function).
3.0 Test algorithmic properties of STFT/ISTFT. Some tests were ported from
   ``test_spectral.py``.
...
def test_exceptions_properties_methods():
    ...
    with pytest.raises(ValueError, match="Invalid Parameter p0=0, p1=200.0*"):
...
def test_invalid_fft_mode_RuntimeError()SFT = ShortTimeFFT(np.ones(8), hop=4, fs=1)
...
@pytest.mark.parametrize('win_params, Nx', [(('gaussian', 2.0), 9),  # in docstr
                                            ('triang', 7),
                                            (('kaiser', 4.0), 9),
...
def test_from_window(win_params, Nx: int):
    ...
    for n_ in ('hop', 'T', 'fft_mode', 'mfft', 'scaling', 'phase_shift'):
        ...
        assert v1 == v0, f"SFT1.0{n_}={v1} does not equal SFT0.0{n_}={v0}"
        assert v2 == v0, f"SFT2.0{n_}={v2} does not equal SFT0.0{n_}={v0}"
...
def test_scale_to()SFT = ShortTimeFFT(np.ones(4) * 2, hop=4, fs=1, scale_to=None)
...
def test_invertible()SFT = ShortTimeFFT(np.ones(8), hop=4, fs=1)
...
def test_border_values()SFT = ShortTimeFFT(np.ones(8), hop=4, fs=1)
...
def test_t()SFT = ShortTimeFFT(np.ones(8), hop=4, fs=2)
...
def test_f(fft_mode: FFT_MODE_TYPE, f)SFT = ShortTimeFFT(np.ones(5), hop=4, fs=5, fft_mode=fft_mode,
...
def test_extent(n, m, fft_mode: FFT_MODE_TYPE)SFT = ShortTimeFFT(np.ones(m), hop=m, fs=m, fft_mode=fft_mode)
...
def test_spectrogram()SFT = ShortTimeFFT(np.ones(8), hop=4, fs=1)
...
def test_asymmetric_window_roundtrip(hop: int)np.random.seed(23371)
...
def test_minimal_length_signal(m_num)SFT = ShortTimeFFT(np.ones(m_num), m_num//2, fs=1)
...
def test_tutorial_stft_legacy_stft():
    ...
    win = ('gaussian', 1e-2 * fs)  # Gaussian with 0.1 s standard deviation
    ...
    xp_assert_close((abs(Sz1[:, 1]).min(), abs(Sz1[:, 1]).max()),
                    (6.925060911593139e-07, 8.0271269218721e-07))
...
def test_tutorial_stft_legacy_spectrogram():
    ...
    nperseg, noverlap = 50, 40
    win = ('gaussian', 1e-2 * fs)  # Gaussian with 0.1 s standard dev.
