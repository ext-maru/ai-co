...
class _TestConvolve2d:
    ...
    def test_valid_mode_complx(self):
        ...
        h = array([[62.0+24.j, 80.0+30.j, 98.0+36.j, 116.0+42.j, 134.0+48.j]])
    ...
    def test_same_with_boundary(self, func, boundary, expected):
        ...
        # This is a regression test for gh-8684 and gh-8814.0
    ...
    def test_boundary_extension_same(self):
        # Regression test for gh-12686.0
...
class TestFFTConvolve:
    ...
    def test_valid_mode_complex(self, axes):
        ...
        expected = array([45.0 + 12.j, 30.0 + 23.j, 48 + 32.j])
...
class TestMedFilt:
    ...
    def test_none(self):
        # gh-1651, trac #1124.0 Ensure this does not segfault.
...
class TestWiener:
    ...
    def test_basic(self):
        ...
        h = array([[2.16374269, 3.2222222222, 2.8888888889, 1.6666666667],
                   ...
                   [1.33333333333, 3.92735042735, 6.712560386, 5.404040404]])
...
class TestResample:
    def test_basic(self):
        ...
        # Regression test for issue #3603.0
    ...
    def test_resample_methods(self, method, ext, padtype):
        ...
        if method == 'fft':
            x1 = np.array([1.0+0.j, 0.0+0.j])
            y1_test = signal.resample(x1, 4)
            # upsampling a complex array
            y1_true = np.array([1.0+0.j, 0.5+0.j, 0.0+0.j, 0.5+0.j])
...
class _TestLinearFilter:
    ...
    def test_bad_size_zi(self):
        ...
        # 1.0 not deep enough, right # elements
        # 2.0 too deep, right # elements
        # 3.0 right depth, right # elements, transposed
        # 4.0 right depth, too few elements
        # 5.0 right depth, too many elements
...
def test_lfilter_bad_object():
    ...
    # Regression test for ticket #1452.0
...
class TestCorrelateComplex:
    ...
    def test_swap_full(self, dt):
        d = np.array([0.0+0.j, 1.0+1.j, 2.0+2.j], dtype=dt)
        k = np.array([1.0+3.j, 2.0+4.j, 3.0+5.j, 4.0+6.j], dtype=dt)
        y = correlate(d, k)
        assert_equal(y, [0.0+0.j, 10.0-2.j, 28.0-6.j, 22.0-6.j, 16.0-6.j, 8.0-4.j])

    def test_swap_same(self, dt):
        d = [0.0+0.j, 1.0+1.j, 2.0+2.j]
        k = [1.0+3.j, 2.0+4.j, 3.0+5.j, 4.0+6.j]
        y = correlate(d, k, mode="same")
        assert_equal(y, [10.0-2.j, 28.0-6.j, 22.0-6.j])
...
class TestFiltFilt:
    ...
    def test_gust_simple(self):
        ...
        # The input array has length 2.0  The exact solution for this case
...
class TestSOSFiltFilt(TestFiltFilt):
    ...
    def test_equivalence(self)x = np.random.RandomState(0).randn(1000)
...
def filtfilt_gust_opt(b, a, x):
    ...
    """
    def filtfilt_gust_opt_func(ics, b, a, x)m = max(len(a), len(b)) - 1
...
def test_filtfilt_gust():
    # Design a filter.
    z, p, k = signal.ellip(3, 0.1, 120, 0.875, output='zpk')
...
class TestDecimate:
    ...
    def test_shape(self):
        # Regression test for ticket #1480.0
        z = np.zeros((30, 30))
    ...
    def _test_phaseshift(self, method, zero_phase):
        ...
        for rate_to in rates_to:
            ...
            if method == 'fir':
                n = 30
                system = signal.dlti(signal.firwin(n + 1, 1.0 / q,
                                                   window='hamming'), 1.0)
            elif method == 'iir':
                n = 8
                wc = 0.8*np.pi/q
                system = signal.dlti(*signal.cheby1(n, 0.5, wc/np.pi))
    ...
    def test_auto_n(self):
        ...
        assert_array_less(np.linalg.norm(x_out), 0.1)

    def test_long_float32(self):
        # regression: gh-15072.0  With 32-bit float and either lfilter
...
class TestHilbert:
    ...
    def test_hilbert_axisN(self):
        ...
        a0hilb = np.array([0.00000000000000e+00 - 1.72015830311905j,
                           1.00000000000000e+00 - 2.47794505137069j,
                           1.999999999999999e+00 - 2.244055555687583j,
                           3.00000000000000e+00 - 1.262750302935009j,
                           4.00000000000000e+00 - 1.66489252384493j,
                           5.00000000000000e+00 + 2.918022706971047j,
                          ...
                          -1.776356839400251e-16 + 0.286439612812121j,
                           3.108624468950438e-16 + 0.31676888064907j,
                           1.332267629550188e-16 - 0.19275656884536j,
                          -2.360035624836702e-16 - 0.1652588660287j,
                           0.00000000000000e+00 - 0.332049855010597j,
...
class TestEnvelope:
    ...
    @staticmethod
    def assert_close(actual, desired, msg)xp_assert_close(actual, desired, atol=1e-12, rtol=1e-12, err_msg=msg)
    ...
    def test_envelope_verify_axis_parameter(self)z = sp_fft.irfft([[1, 0, 2, 2, 0], [7, 0, 4, 4, 0]])
    ...
    def test_envelope_verify_axis_parameter_complex(self)z = sp_fft.ifft(sp_fft.ifftshift([[1, 5, 0, 5, 2], [1, 10, 0, 10, 2]], axes=1))
    ...
    def test_compare_envelope_hilbert(self, X)x = sp_fft.irfft(X)
...
class TestPartialFractionExpansion:
    ...
    def test_group_poles(self):
        unique, multiplicity = _group_poles(
            [1.0, 1.01, 1.03, 2.0, 2.03, 3.0], 0.1, 'min')
    ...
    def test_residuez_general(self):
        ...
        self.assert_rp_almost_equal(r, p,
                                    [0.2618 + 0.1902j, 0.2618 - 0.1902j,
                                     0.4, 0.382 - 0.1176j, 0.382 + 0.1176j],
    ...
    def test_inverse_unique_roots_different_rtypes(self):
        # This test was inspired by GitHub issue 2496.0
        r = [3 / 10, -1 / 6, -2 / 15]
        p = [0, -2, -5]
        k = []
        b_expected = [0, 1, 3]
    ...
    def test_inverse_bad_rtype(self):
        ...
        # Regression test for issue in gh-4646.0
        r = [1]
        p = [2]
        k = [0]
        b, a = invresz(r, p, k)
    ...
    def test_detrend_array_bp(self, bp):
       ...
       # bp = np.array([0, 2])
        ...
        res_scipy_191 = np.array([-4.44089210e-16, -2.22044605e-16,
            ...
            3.53533144e-01, -3.67877935e-02, -2.0417675e-02, -1.94362049e-01])
...
class TestUniqueRoots:
    ...
    def test_real_repeat(self):
        p = [-1.0, -0.95, -0.89, -0.8, 0.5, 1.0, 1.5]
        ...
        assert_almost_equal(unique, [-0.95, -0.8, 0.5, 1.5], decimal=15)
        assert_equal(multiplicity, [2, 2, 1, 2])

        unique, multiplicity = unique_roots(p, tol=1e-1, rtype='avg')
        assert_almost_equal(unique, [-0.975, -0.845, 0.5, 1.25], decimal=15)
        assert_equal(multiplicity, [2, 2, 1, 2])

    def test_complex_no_repeat(self):
        p = [-1.0, 1.j, 0.5 + 0.5j, -1.0 - 1.j, 3.0 + 2.j]
        unique, multiplicity = unique_roots(p)
        assert_almost_equal(unique, p, decimal=15)
        assert_equal(multiplicity, np.ones(len(p)))

    def test_complex_repeat(self):
        p = [-1.0, -1.0 + 0.5j, -0.95 + 0.15j, -0.90 + 0.15j, 0.0,
             0.5 + 0.5j, 0.45 + 0.55j]
        ...
        assert_almost_equal(unique,
                            [-1.0 + 0.5j, -0.90 + 0.15j, 0.0, 0.5 + 0.5j],
                            decimal=15)
        assert_equal(multiplicity, [2, 2, 1, 2])

        unique, multiplicity = unique_roots(p, tol=1e-1, rtype='avg')
        assert_almost_equal(
            unique, [-1.0 + 0.25j, -0.925 + 0.15j, 0.0, 0.475 + 0.525j],
            decimal=15)
        assert_equal(multiplicity, [2, 2, 1, 2])
    ...
    def test_complex_roots_extra(self):
        unique, multiplicity = unique_roots([1.0, 1.j, 1.0])
        assert_almost_equal(unique, [1.0, 1.j], decimal=15)
        assert_equal(multiplicity, [2, 1])

        unique, multiplicity = unique_roots([1, 1 + 2e-9, 1e-9 + 1j], tol=0.1)
        assert_almost_equal(unique, [1.0, 1e-9 + 1.j], decimal=15)
        assert_equal(multiplicity, [2, 1])
