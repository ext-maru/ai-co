...
class TestProcessAPIs(PsutilTestCase):
    ...
    def test_wait_procs(self):
        ...
        t = time.time()
        gone, alive = psutil.wait_procs(procs, timeout=0.1, callback=callback)
        ...
        def test_1(procs, callback):
            gone, alive = psutil.wait_procs(
                procs, timeout=0.3, callback=callback
            )
            assert len(gone) == 1
            assert len(alive) == 2
            return gone, alive
...
class TestCpuAPIs(PsutilTestCase):
    ...
    def test_cpu_percent(self):
        last = psutil.cpu_percent(interval=0.01)
        for _ in range(100):
    ...
    def test_per_cpu_percent(self):
        last = psutil.cpu_percent(interval=0.01, percpu=True)
    ...
    def test_cpu_times_percent(self):
        last = psutil.cpu_times_percent(interval=0.01)
        for _ in range(100):
    ...
    def test_per_cpu_times_percent(self):
        last = psutil.cpu_times_percent(interval=0.01, percpu=True)
        assert len(last) == psutil.cpu_count()
        for _ in range(100):
