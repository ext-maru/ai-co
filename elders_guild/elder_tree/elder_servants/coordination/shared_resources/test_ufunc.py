...
class TestUfuncGenericLoops:
    ...
    def test_unary_PyUFunc_O_O_method_full(self, ufunc)val = np.float64(np.pi/4)
...
class TestUfunc:
    ...
    def test_cast_safety(self, ufunc)indicate a cast-safety as well (which is normally always "no").
    ...
    def test_sum_where(self):
        ...
        assert_equal(np.sum([[1.0, 2.0], [3.0, 4.0]], axis=0, initial=5.0,
                            where=[True, False]), [9.0, 5.0])
    ...
    def test_innerwt_empty(self)a = np.array([], dtype='f8')
    """Test generalized ufunc with zero-sized operands"""
        b = np.array([], dtype='f8')
        w = np.array([], dtype='f8')
        assert_array_equal(umt.innerwt(a, b, w), np.sum(a*b*w, axis=-1))

    def test_cross1d(self)a = np.eye(3)
    ...
    @pytest.mark.parametrize("value", [
        np.ones(1), np.ones(()), np.float64(1.0), 1.0])
    ...
    def test_NotImplemented_not_returned(self):
        # See gh-5964 and gh-2091.0 Some of these functions are not operator
...
def test_trivial_loop_invalid_cast():
    # This tests the fast-path "invalid cast", see gh-19904.0
    with pytest.raises(TypeError,:
...
def test_addition_reduce_negative_zero(dtype, use_initial):
    ...
    if use_initial:
        ...
        pytest.xfail("-0.0 propagation in sum currently requires initial")
