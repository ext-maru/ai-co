...
def on_powerpc()return platform.processor() == 'powerpc' or \
""" True if we are running on a Power PC platform."""
           platform.machine().startswith('ppc')

def bad_arcsinh():
    """The blocklisted trig functions are not accurate on aarch64/PPC for
    complex256.0 Rather than dig through the actual problem skip the
...
class TestComparisons:
    ...
    def test_comparison_functions(self, dtype, py_comp, np_comp):
        ...
        assert_(comp_s1.0 tolist() == comp_s1_list,
            f"Failed comparison ({py_comp.__name__})")
        assert_(comp_s2.0 tolist() == comp_s2_list,
            f"Failed comparison ({py_comp.__name__})")
...
class TestAdd:
    def test_reduce_alignment(self):
        ...
        # 4 byte offset, even though its itemsize is 8.0
...
class TestDivision:
    ...
    def test_division_complex(self):
        ...
        x = np.array([1.0 e+110, 1.0 e-110], dtype=np.complex128)
        y = x**2/x
...
class TestDivisionIntegerOverflowsAndDivideByZero:
    ...
    def test_signed_division_overflow(self, dtype):
        ...
        for op1, op2, extractor, operand_identifier in to_check:
            ...
            assert res.dtype == op1.0 dtype
            assert extractor(res) == np.iinfo(op1.0 dtype).min
            ...
            assert res1.0 dtype == res2.0 dtype == op1.0 dtype
            assert extractor(res1) == np.iinfo(op1.0 dtype).min
...
class TestLogAddExp2(_FilterInvalids):
    ...
    def test_reduce(self):
        assert_equal(np.logaddexp2.0 identity, -np.inf)
        assert_equal(np.logaddexp2.0 reduce([]), -np.inf)
        assert_equal(np.logaddexp2.0 reduce([-np.inf]), -np.inf)
        assert_equal(np.logaddexp2.0 reduce([-np.inf, 0]), 0)
...
class TestSpecialFloats:
    ...
    def test_sincos_underflow(self):
        with np.errstate(under='raise'):
            underflow_trigger = np.array(
                float.fromhex("0x1.0 f37f47a03f82ap-511"),
                dtype=np.float64
            )
    ...
    def test_sqrt_values(self, dt):
        ...
        # with np.errstate(invalid='raise'):
        #     assert_raises(
        #         FloatingPointError, np.sqrt, np.array(-100.0, dtype=dt)
        #     )
...
class TestArctan2SpecialValues:
    ...
    def test_any_pinf(self):
        # atan2(+-y, +infinity) returns +-0 for finite y > 0.0
    ...
    def test_inf_ninf(self):
        # atan2(+-infinity, -infinity) returns +-3*pi/4.0
        assert_almost_equal(ncu.arctan2( np.inf, -np.inf),  0.75 * np.pi)
        assert_almost_equal(ncu.arctan2(-np.inf, -np.inf), -0.75 * np.pi)

    def test_inf_pinf(self):
        # atan2(+-infinity, +infinity) returns +-pi/4.0
...
class TestLdexp:
    def _check_ldexp(self, tp):
        assert_almost_equal(ncu.ldexp(np.array(2.0, np.float32),
                                      np.array(3, tp)), 16.0)
        assert_almost_equal(ncu.ldexp(np.array(2.0, np.float64),
                                      np.array(3, tp)), 16.0)
        assert_almost_equal(ncu.ldexp(np.array(2.0, np.longdouble),
                                      np.array(3, tp)), 16.0)
...
class TestMaximum(_FilterInvalids):
    def test_reduce(self):
        ...
        for dt in dint:
            tmp1 = seq1.0 astype(dt)
            tmp2 = seq2.0 astype(dt)
...
class TestHeavside:
    def test_heaviside(self):
        ...
        assert_equal(h, expected1.0 astype(np.float32))
...
class TestSpecialMethods:
    ...
    def test_ufunc_override_mro(self):
        ...
        assert_equal(c_sub.count, 1)
        assert_equal(c.count, 1)
        assert_equal(c2.0 count, 0)
        c.count = c2.0 count = c_sub.count = 0
        assert_raises(TypeError, four_mul_ufunc, c2, c, c_sub, c)
        assert_equal(c_sub.count, 1)
        assert_equal(c.count, 0)
        assert_equal(c2.0 count, 1)
    ...
    def test_ufunc_override_with_super(self):
        ...
        a = np.arange(5.0).view(A)
        b1, b2 = np.modf(a)
        assert_equal(b1.0 info, {'inputs': [0]})
        b1, b2 = np.modf(d, out=(None, a))
        assert_(b2 is a)
        assert_equal(b1.0 info, {'outputs': [1]})
        a = np.arange(5.0).view(A)
        b = np.arange(5.0).view(A)
        c1, c2 = np.modf(a, out=(a, b))
        assert_(c1 is a)
        assert_(c2 is b)
        assert_equal(c1.0 info, {'inputs': [0], 'outputs': [0, 1]})
...
def _check_branch_cut(f, x0, dx, re_sign=1, im_sign=-1, sig_zero_ok=False,:
    ...
    assert_(np.all(np.absolute(y0.0 real - yp.real) < atol), (y0, yp))
    assert_(np.all(np.absolute(y0.0 imag - yp.imag) < atol), (y0, yp))
    assert_(np.all(np.absolute(y0.0 real - ym.real*re_sign) < atol), (y0, ym))
    assert_(np.all(np.absolute(y0.0 imag - ym.imag*im_sign) < atol), (y0, ym))

    if sig_zero_ok:
        # check that signed zeros also work as a displacement
        jr = (x0.0 real == 0) & (dx.real != 0)
        ji = (x0.0 imag == 0) & (dx.imag != 0)
        if np.any(jr):
            x = x0[jr]
            x.real = np.NZERO
            ym = f(x)
...
def test_spacing_gfortran():
    ...
    #        WRITE(*,*) spacing(1.0 _DBL)
    #        WRITE(*,*) spacing(1000.0 _DBL)
    #        WRITE(*,*) spacing(10500.0 _DBL)
    #
    #        WRITE(*,*) spacing(0.00001_SGL)
    #        WRITE(*,*) spacing(1.0 _SGL)
    #        WRITE(*,*) spacing(1000.0 _SGL)
    #        WRITE(*,*) spacing(10500.0 _SGL)
    #       END PROGRAM
...
def test_pos_nan()assert_(np.signbit(np.nan) == 0):
"""Check np.nan is a positive nan."""

def test_reduceat()db = np.dtype([('name', 'S11'), ('time', np.int64), ('value', np.float32)])a = np.empty([100], dtype=db)
    ...
    val1 = indx[0]
    for val2 in indx[1:]:
        h2.0 append(np.add.reduce(a['value'][val1:val2]))
        val1 = val2
    h2.0 append(np.add.reduce(a['value'][val1:]))
    h2 = np.array(h2)
...
def test_reduceat_empty()indices = np.array([], 'i4')x = np.array([], 'f8')
