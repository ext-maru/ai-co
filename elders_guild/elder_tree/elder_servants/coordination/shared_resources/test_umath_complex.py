...
# to the C99 standard. See ticket 1574.0
...
class TestClog:
    ...
    @platform_skip
    @pytest.mark.skipif(platform.machine() == "armv5tel", reason="See gh-413.0")
    def test_special_values(self):
        xl = []
        yl = []
        ...
        # clog(- inf + i inf) returns +inf + i3pi /4.0
        ...
        # clog(+ inf + i inf) returns +inf + ipi /4.0
        x = np.array([complex(np.inf, np.inf)], dtype=complex)
...
class TestSpecialComplexAVX:
    ...
    def test_array(self, stride, astype):
        arr = np.array([complex(np.nan , np.nan),
                        ...
                        complex(0.0     , 0.0),
                        complex(0.0     , np.nan),
                        complex(np.nan , 0.0)], dtype=astype)
...
class TestComplexAbsoluteMixedDTypes:
    ...
    def test_array(self, stride, astype, func):
        ...
        vec = np.array([
               (0, 0.0, 0, -31.666483, 200, 0.0, 0.0,  1.0      ,  3.0+4.0 j   ,  613090),
               (1, 0.0, 0, 260.91525 ,  42, 0.0, 0.0,  1.0      ,  5.0+12.0 j  ,  787315),
               (1, 0.0, 0,  52.15155 ,  42, 0.0, 0.0,  1.0      ,  8.0+15.0 j  ,  806641),
               (1, 0.0, 0,  52.430195,  42, 0.0, 0.0,  1.0      ,  7.0+24.0 j  , 1363540),
               (2, 0.0, 0, 304.43646 ,  58, 0.0, 0.0,  1.0      ,  20.0+21.0 j ,  787323),
               (3, 0.0, 0, 299.42108 ,  52, 0.0, 0.0,  1.0      ,  12.0+35.0 j ,  787332),
               (4, 0.0, 0,  39.4836  ,  28, 0.0, 0.0,  9.182192,  9.0+40.0 j  ,  787304),
               (4, 0.0, 0,  76.83787 ,  28, 0.0, 0.0,  1.0      ,  28.0+45.0 j, 1321869),
               (5, 0.0, 0, 143.26366 ,  24, 0.0, 0.0, 10.996129,  11.0+60.0 j ,  787299)], dtype=dtype)
