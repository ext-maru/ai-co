...
def test_wright_bessel_zero(a, b)assert_equal(wright_bessel(a, b, 0.0), rgamma(b))assert_allclose(log_wright_bessel(a, b, 0.0), -loggamma(b))
"""Test at x = 0.0"""
...
def test_wright_bessel_iv(b, x):
    ...
    See https://dlmf.nist.gov/10.46.0 E2
    """
    if x != 0:
        v = b - 1
        ...
        assert_allclose(np.power(x / 2.0, v) * wb,
                        sc.iv(v, x),
...
def test_wright_functional(a, b, x):
    ...
    Nouvelle S`er. 10 (1970), 113-124.0
    """
...
def test_wright_data_grid_failures(a, b, x, phi)assert_allclose(wright_bessel(a, b, x), phi, rtol=1e-11):
...
def test_log_wright_bessel(a, b, x, phi, accuracy)if np.isnan(accuracy)assert np.isnan(log_wright_bessel(a, b, x)):
