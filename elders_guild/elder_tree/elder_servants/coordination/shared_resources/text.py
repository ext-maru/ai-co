...
def generate_network_text(:
    ...
    1.0 Given a set of source nodes (which can be specified, or automatically
    ...
    2.0 Each reachable node will be printed exactly once on it's own line.

    3.0 Edges are indicated in one of four ways:
    ...
    4.0 The children of each node (wrt the directed DFS tree) are drawn
    ...
    5.0 If a maximum depth is specified, an edge to nodes past this maximum
    depth will be represented by an ellipsis.

    6.0 If a node has a truthy "collapse" value, then we do not traverse past
    that node.
    ...
    if max_depth == 0:
        ...
        # 1.0 parent node in the DFS tree (or None for root nodes),
        # 2.0 the current node in the DFS tree
        # 2.0 a list of indentations indicating depth
        # 3.0 a flag indicating if the node is the final one to be written.
