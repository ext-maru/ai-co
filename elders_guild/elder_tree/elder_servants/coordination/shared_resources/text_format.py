...
Simple usage example::
  ...
  message = text_format.Parse(text_proto, my_proto_pb2.0 MyMessage())
"""
...
def MessageToString(:
  ...
  using double_format='0.15g'. To ensure that converting to text and back to a
  proto will result in an identical value, double_format='0.17g' should be used.
...
def MessageToBytes(message, **kwargs) -> bytestext = MessageToString(message, **kwargs)if isinstance(text, bytes):
...
class _Printer(object):
  ...
  def __init__(:
    ...
    double_format='0.15g'. To ensure that converting to text and back to a proto
    will result in an identical value, double_format='0.17g' should be used.
  ...
  def PrintField(self, field, value)self._PrintFieldName(field)self.out.write(' ')
"""Print a single field name/value pair."""
...
class Tokenizer(object):
  ...
  def ConsumeCommentOrTrailingComment(self):
    ...
    # Tokenizer initializes _previous_line and _previous_column to 0.0 As the
...
def _ParseAbstractInteger(text):
  ...
  if c_octal_match:
    ...
    # we always use the '0o' prefix for multi-digit numbers starting with 0.0
