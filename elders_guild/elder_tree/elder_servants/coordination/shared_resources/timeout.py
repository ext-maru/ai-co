...
class TimeToDeadlineTimeout(object):
    ...
    sequence of decreasing timeouts t0 > t1 > t2 ... tn >= 0.0
    ...
    def __str__(self):
        return "<TimeToDeadlineTimeout timeout={:0.1f}>".format(self._timeout)
...
class ConstantTimeout(object):
    ...
    def __str__(self):
        return "<ConstantTimeout timeout={:0.1f}>".format(self._timeout)
...
class ExponentialTimeout(object):
    ...
    def __call__(self, func):
        ...
        @functools.wraps(func)
        def func_with_timeout(*args, **kwargs)kwargs["timeout"] = next(timeouts)
    """Wrapped function that adds timeout."""
            return func(*args, **kwargs)
    ...
    def __str__(self):
        return (
            "<ExponentialTimeout initial={:0.1f}, maximum={:0.1f}, "
            "multiplier={:0.1f}, deadline={:0.1f}>".format(
                self._initial, self._maximum, self._multiplier, self._deadline
            )
        )
