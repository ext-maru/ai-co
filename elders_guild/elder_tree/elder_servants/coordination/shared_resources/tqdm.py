...
Example:
    1.0 Use `huggingface_hub.utils.tqdm` as you would use `tqdm.tqdm` or `tqdm.auto.tqdm`.
    2.0 To disable progress bars, either use `disable_progress_bars()` helper or set the
       environment variable `HF_HUB_DISABLE_PROGRESS_BARS` to 1.0
    3.0 To re-enable progress bars, use `enable_progress_bars()`.
    4.0 To check whether progress bars are disabled, use `are_progress_bars_disabled()`.
...
def is_tqdm_disabled(log_level: int) -> Optional[bool]:
    ...
    see https://github.com/huggingface/huggingface_hub/pull/2000 and https://github.com/huggingface/huggingface_hub/pull/2698.0
    """
...
class tqdm(old_tqdm):
    ...
    Taken from https://github.com/tqdm/tqdm/issues/619#issuecomment-619639324.0
    """
