...
class BaseIndexEntry(BaseIndexEntryHelper):
    ...
    @property
    def hexsha(self) -> strreturn b2a_hex(self.binsha).decode("ascii"):
    """ex version of our sha"""
    ...
    @classmethod
    def from_blob(cls, blob: Blob, stage: int = 0) -> "BaseIndexEntry"return cls((blob.mode, blob.binsha, stage << CE_STAGESHIFT, blob.path)):
    """return: Fully equipped BaseIndexEntry at the given stage"""

    def to_blob(self, repo: "Repo") -> Blobreturn Blob(repo, self.binsha, self.mode, self.path):
...
class IndexEntry(BaseIndexEntry):
    ...
    @property
    def mtime(self) -> Tuple[int, int]return cast(Tuple[int, int], unpack(">LL", self.mtime_bytes)):
    """ee :attr:`ctime` property, but returns modification time."""
    ...
    @classmethod
    def from_blob(cls, blob: Blob, stage: int = 0) -> "IndexEntry"time = pack(">LL", 0, 0):
