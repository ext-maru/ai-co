...
async def _user_api_key_auth_builder(  # noqa: PLR0915
    ...
    try:
        ...
        if valid_token is not None:
            ...
            # Run checks for
            # 1.0 If token can call model
            ## 1a. If token can call fallback models (if client-side fallbacks given)
            # 2.0 If user_id for this token is in budget
            # 3.0 If the user spend within their own team is within budget
            # 4.0 If 'user' passed to /chat/completions, /embeddings endpoint is in budget
            # 5.0 If token is expired
            # 6.0 If token spend is under Budget for the token
            # 7.0 If token spend per model is under budget per model
            # 8.0 If token spend is under team budget
            # 9.0 If team spend is under team budget
            ...
            # Check 2.0 If user_id for this token is in budget - done in common_checks()
            ...
            # Check 3.0 Check if user is in their team budget
            ...
            # Check 4.0 Token Spend is under budget
            ...
            # Check 5.0 Soft Budget Check
            await _virtual_key_soft_budget_check(
                valid_token=valid_token,
                proxy_logging_obj=proxy_logging_obj,
            )

            # Check 5.0 Token Model Spend is under Model budget
