...
class Operator(enum.Enum):
    # Section 2.2.0 Expressions
    #      expression    =  "{" [ operator ] variable-list "}"
    #      operator      =  op-level2 / op-level3 / op-reserve
    #      op-level2     =  "+" / "#"
    #      op-level3     =  "." / "/" / ";" / "?" / "&"
    #      op-reserve    =  "=" / "," / "!" / "@" / "|"
    default = ""  # 3.2.2.0 Simple String Expansiona: {var}
    # Operator Level 2 (op-level2)
    reserved = "+"  # 3.2.3.0 Reserved Expansion: {+var}
    fragment = "#"  # 3.2.4.0 Fragment Expansion: {#var}
    # Operator Level 3 (op-level3)
    # 3.2.5.0 Label Expansion with Dot-Prefix: {.var}
    label_with_dot_prefix = "."
    path_segment = "/"  # 3.2.6.0 Path Segment Expansion: {/var}
    path_style_parameter = (
        ";"  # 3.2.7.0 Path-Style Parameter Expansion: {;var}
    )
    form_style_query = "?"  # 3.2.8.0 Form-Style Query Expansion: {?var}
    # 3.2.9.0 Form-Style Query Continuation: {&var}
    ...
    def expansion_separator(self) -> str:
        ...
        Per `Section 3.2.1.0 Variable Expansion`_:
        ...
        .. _`Section 3.2.1.0 Variable Expansion`:
