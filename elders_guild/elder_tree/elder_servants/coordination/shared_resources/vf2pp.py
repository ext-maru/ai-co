...
.. [1] Jüttner, Alpár & Madarasi, Péter. (2018). "VF2++—An improved subgraph
   isomorphism algorithm". Discrete Applied Mathematics. 242.0
   https://doi.org/10.1016/j.dam0.2018.2.18
...
def vf2pp_all_isomorphisms(G1, G2, node_label=None, default_label=None):
    """Yields all the possible mappings between G1 and G2.0
...
def _initialize_parameters(G1, G2, G2_degree, node_label=None, default_label=-1):
    ...
    state_params: namedtuple
        ...
        reverse_mapping: dict
            The reverse mapping as extended so far. Maps nodes from G2 to nodes of G1.0 It's basically "mapping" reversed
...
def _find_candidates(:
    ...
    """Given node u of G1, finds the candidates of u from G2.0

    Parameters
    ----------
    u: Graph node
        The node from G1 for which to find the candidates from G2.0
...
def _update_Tinout(new_node1, new_node2, graph_params, state_params):
    ...
    Notes
    -----
    This function should be called right after the feasibility checks are passed, and node1 is mapped to node2.0 The
...
def _restore_Tinout(popped_node1, popped_node2, graph_params, state_params):
    ...
    state_params: namedtuple
        ...
        T1_tilde, T2_tilde: set
            Ti_out contains all the nodes from Gi, that are neither in the mapping nor in Ti
    """
    # If the node we want to remove from the mapping, has at least one covered neighbor, add it to T1.0
    G1, G2, _, _, _, _, _ = graph_params
    (
        mapping,
        reverse_mapping,
    ...
    # Case where the node is not present in neither the mapping nor T1.0 By definition, it should belong to T1_tilde
    if not is_added:
