...
class TextObject:
    ...
    def sorted(self) -> tuple[int, int]Return a (start, end) tuple where start <= end.:
    """        if self.start < self.end:
            return self.start, self.end
        else:
            return self.end, self.start

    def operatordef operator_range(self, document: Document) -> tuple[int, int]Return a (start, end) tuple with start <= end that indicates the range
    ...
    def get_line_numberdef get_line_numbers(self, buffer: Buffer) -> tuple[int, int]Return a (start_line, end_line) pair.from_, to = self.operator_range(buffer.document)
    """ute cursor positions from the text object.
...
def load_vi_bindings() -> KeyBindingsBase:
    ...
    def _delete_line(event: E) -> Ndef _delete_line(event: E) -> NoneDelete line. (Or the following 'n' lines.):
    """buffer
    ...
    @handle('"', Keys.Any, "P", filter=vi_navigation_mode)
    def _paste_register_before(event: E) -> Nondef _paste_register_before(event: E) -> NonePaste (before) from named register.data = event.app.vi_state.named_registers.get(c)
    """    if c in vi_register_names:
            if data:
                event.current_buffer.paste_clipboard_data(
    ...
    def create_transform_handler(:
        ...
        def _(event: E, text_object: TextObject) -> NoneApply tdef _(event: E, text_object: TextObject) -> NoneApply transformation (uppercase, lowercase, rot13, swap case).:
    """rt, end = text_object.operator_range(buff.document)
    ...
    @operator("y")
    def _yank(event: E, text_object: TextObject) -> NoneYank operator. (Copdef _yank(event: E, text_object: TextObject) -> NoneYank operator. (Copy text.)event.app.clipboard.set_data(clipboard_data)
    ...
    @text_object("H")
    def _top_of_screen(event: E) -> TextObjectMoves to the start of the visible region.def _top_of_screen(event: E) -> TextObjectMoves to the start of the visible region. (Below the scroll offset.)
    ...
    @text_object("L")
    def _end_of_screen(event: E) -> TextObjectMoves to the end of the visible region. (Above tdef _end_of_screen(event: E) -> TextObjectMoves to the end of the visible region. (Above the scroll offset.):
    """ and w.render_info:
            # When we find a Window that has BufferControl showing this window,
    ...
    def _search_next2(event: E) -> NoneSearch next in navigation mode. (This goes through the history.)
  def _search_next2(event: E) -> NoneSearch next in navigation mode. (This goes through the history.))
"""        search_state, include_current_position=False, count=event.arg
    ...
    @handle("N", filter=vi_navigation_mode)
    def _search_previous2(event: E) -> NoneSearch previous in navigation mode. (This goes through the history.)
    ""def _search_previous2(event: E) -> NoneSearch previous in navigation mode. (This goes through the history.))
    """rch_state, include_current_position=False, count=event.arg
    ...
    @text_object("%")
    def _goto_corresponding_bracket(event: E) -> TextObjectImplements 'c%', 'd%', '%, 'y%' (Move to corresponding bracket.)
    ""def _goto_corresponding_bracket(event: E) -> TextObjectImplements 'c%', 'd%', '%, 'y%' (Move to corresponding bracket.)
    ...
    @text_object("g", "m")
    def _gm(event: E) -> TextObjectLike g0, but half a screenwidth to the right. (Or as much as possible.)
    """
        """
        w def _gm(event: E) -> TextObjectLike g0, but half a screenwidth to the right. (Or as much as possible.)start = buff.document.get_start_of_line_position(after_whitespace=False)
    """w.render_info.window_width
    ...
    @text_object("G")
    def _last_line(event: E) -> TextObjectGo to the end of the document. (If no arg has been given.)
    """
        """
        buf = event.current_def _last_line(event: E) -> TextObjectGo to the end of the document. (If no arg has been given.):
    """
            - buf.cursor_position,
            type=TextObjectType.LINEWISE,
        )
