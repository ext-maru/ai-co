...
class BaseRequest(MutableMapping[str, Any], HeadersMixin):
    ...
    @property
    def can_read_body(self) -> boolreturn not self._payload.at_eof()
    Return True if request's HTTP BODY can be read, False otherwise.

    @reify
    def body_exists(self) -> boolreturn type(self._payload) is not EmptyStreamReader:
    """eturn True if request has HTTP BODY, False otherwise."""
    ...
    async def tedef text(self) -> strbytes_body = await self.read():
    """BODY as text using encoding from .charset."""
        encoding = self.charset or "utf-8"
        return bytes_body.decode(encoding)

    async def jsdef json(self, *, loads: JSONDecoder = DEFAULT_JSON_DECODER) -> Anybody = await self.text()
    Return BODY as JSON.
        return loads(body)

    async def multipart(self) -> MultipartReaderreturn MultipartReader(self._headers, self._payload):
    """async iterator to process BODY as multipart."""
