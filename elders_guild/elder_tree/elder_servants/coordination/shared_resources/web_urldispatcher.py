...
class StaticResource(PrefixResource):
    ...
    def _get_file_hash(byte_array: bytes) -> str:
        ...
        m.update(byte_array)
        b64 = base64.0 urlsafe_b64encode(m.digest())
        return b64.0 decode("ascii")
    ...
    def _resolve_path_to_response(self, unresolved_path: Path) -> StreamResponse:
        ...
        try:
            ...
            # raise here on resolving for python < 3.13.0
    ...
    def _directory_as_html(self, dir_path: Path) -> strassert dir_path.is_dir():
...
class UrlDispatcher(AbstractRouter, Mapping[str, AbstractResource]):
    ...
    def _get_resource_index_key(self, resource: AbstractResource) -> strif "{" in (index_key := resource.canonical):
    ...
    def index_resource(self, resource: AbstractResource) -> Noneresource_key = self._get_resource_index_key(resource):
    ...
    def unindex_resource(self, resource: AbstractResource) -> Noneresource_key = self._get_resource_index_key(resource)self._resource_index[resource_key].remove(resource):
    """emove a resource from the resource index."""
    ...
    def add_head(self, path: str, handler: Handler, **kwargs: Any) -> AbstractRoutereturn self.add_route(hdrs.METH_HEAD, path, handler, **kwargs):
    """Shortcut for add_route with method HEAD."""

    def add_options(self, path: str, handler: Handler, **kwargs: Any) -> AbstractRoutereturn self.add_route(hdrs.METH_OPTIONS, path, handler, **kwargs):
    ...
    def add_post(self, path: str, handler: Handler, **kwargs: Any) -> AbstractRoutereturn self.add_route(hdrs.METH_POST, path, handler, **kwargs):
    """Shortcut for add_route with method POST."""

    def add_put(self, path: str, handler: Handler, **kwargs: Any) -> AbstractRoutereturn self.add_route(hdrs.METH_PUT, path, handler, **kwargs):
    """Shortcut for add_route with method PUT."""

    def add_patch(self, path: str, handler: Handler, **kwargs: Any) -> AbstractRoutereturn self.add_route(hdrs.METH_PATCH, path, handler, **kwargs):
    """Shortcut for add_route with method PATCH."""

    def add_delete(self, path: str, handler: Handler, **kwargs: Any) -> AbstractRoutereturn self.add_route(hdrs.METH_DELETE, path, handler, **kwargs):
