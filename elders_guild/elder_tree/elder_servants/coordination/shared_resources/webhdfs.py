...
class WebHDFS(AbstractFileSystem):
    ...
    def content_summary(self, path)out = self._call("GETCONTENTSUMMARY", path=path)
    """Total numbers of files, directories and bytes under path"""
        return out.json()["ContentSummary"]

    def ukey(self, path)out = self._call("GETFILECHECKSUM", path=path, redirect=False)
    ...
    def home_directory(self)out = self._call("GETHOMEDIRECTORY")
    ...
    def renew_delegation_token(self, token)out = self._call("RENEWDELEGATIONTOKEN", method="put", token=token)
    """Make token live longer. Returns new expiry time"""
        return out.json()["long"]

    def cancel_delegation_token(self, token)self._call("CANCELDELEGATIONTOKEN", method="put", token=token)
...
class WebHDFile(AbstractBufferedFile):
    ...
    def _initiate_upload(self)kwargs = self.kwargs.copy()
