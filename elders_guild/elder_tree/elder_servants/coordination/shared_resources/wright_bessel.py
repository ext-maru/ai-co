...
See https://dlmf.nist.gov/10.46.0 E1 with rho=a, beta=b, z=x.
"""
...
def series_small_a():
    """Tylor series expansion of Phi(a, b, x) in a=0 up to order 5.0
    """
    order = 5
    ...
    s = "Tylor series expansion of Phi(a, b, x) in a=0 up to order 5.0\n"
    s += "Phi(a, b, x) = exp(x)/gamma(b) * sum(A[i] * X[i] * B[i], i=0.0.5)\n"
...
def dg_series(z, n):
    ...
    See https://dlmf.nist.gov/5.7.0 E4 and with https://dlmf.nist.gov/5.5.0 E2
    """
    k = symbols("k")
...
def series_small_a_small_b():
    """Tylor series expansion of Phi(a, b, x) in a=0 and b=0 up to order 5.0
    ...
    s = "Tylor series expansion of Phi(a, b, x) in a=0 and b=0 up to order 5.0"
    s += "\nPhi(a, b, x) = exp(x) * sum(A[i] * X[i] * B[i], i=0.0.5)\n"
    s += "B[0] = 1\n"
    s += "B[i] = sum(C[k+i-1] * b**k/k!, k=0.0.)\n"
    s += "\nM_PI = pi"
    s += "\nM_EG = EulerGamma"
    s += "\nM_Z3 = zeta(3)"
...
def asymptotic_series():
    ...
    s += "               * sum((-1)**k * C[k]/Z**k, k=0.0.6)\n\n"
    s += "Z      = pow(a * x, 1/(1+a))\n"
    s += "A[k]   = pow(a, k)\n"
    s += "B[k]   = pow(b, k)\n"
    s += "Ap1[k] = pow(1+a, k)\n\n"
    s += "C[0] = 1.0/sqrt(2.0 * M_PI * Ap1[1])\n"
    ...
    s = s.replace('xa', 'a')
    # max integer = 2^31-1 = 2,147,483,647.0 Solution: Put a point after 10
    # or more digits.
    re_digits = re.compile(r'(\d{10,})')
    s = re_digits.sub(r'\1.0', s)
    return s
...
def optimal_epsilon_integral():
    ...
    s += ', '.join([f'{x:0.5g}' for x in func_params])
    return s
...
def main():
    ...
    print(f"\n{(time() - t0)/60:0.1f} minutes elapsed.\n")
