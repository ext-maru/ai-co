...
class AICommandCategorizer:
    ...
    def find_duplicates(self, categorized_commands: Dict) -> Dict[str, List[str]]:
        ...
        for group in duplicate_groups:
            existing = [
                ...
                in [
                    c["old_name"]
                    for cat in categorized_commands.values()
    ...
    def generate_report(self):
        ...
        json_path = (
            reports_dir
            / f"ai_command_categorization_{self.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
...
"""
        ...
        for category, count in report["statistics"].items():
            if count > 0:
                md_content += (
                    f"| {report['categorization'][category]['name']} | {count} |\n"
                )
        ...
        md_path = (
            json_path.parent
            / f"AI_COMMAND_CATEGORIZATION_{self.timestamp.strftime('%Y%m%d')}.md"
        )
