...
# Git環境をクリーンアップ（Issue #141対応）
log "🧹 Git環境のクリーンアップ..."
if git status --porcelain | grep -q .; then
    log "📦 ローカル変更を一時保存..."
    git stash push -u -m "Auto-stash before auto-issue processing $(date +%Y%m%d_%H%M%S)" || true
fi
...
# stashがある場合は復元を試みる
if git stash list | grep -q "Auto-stash before auto-issue processing"; then
    log "📦 一時保存した変更を復元中..."
    git stash pop || log "⚠️ stashの復元に失敗しました（手動で確認してください）"
fi
