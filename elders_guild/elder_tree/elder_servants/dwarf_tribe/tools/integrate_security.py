        ...
        try:
            ...
            for match in reversed(init_matches):
                ...
                if lines_after and lines_after[0].strip():
                    ...
                    if indent_match:
                        ...
                        security_init = (
                            f"{indent}self.security_manager = get_security_manager()\n"
                        )
                        content = (
                            content[:init_end] + security_init + content[init_end:]
                        )

            # Add input validation to key methods
            # Find methods that accept string parameters
            method_pattern = (
                r"(async def|def) (create_|update_|merge_|list_|get_)([^(]+)\([^)]*\):"
            )
            ...
            for match in reversed(method_matches):
                ...
                if len(lines_after) > 1:
                    body_line = next(
                        (line for line in lines_after[1:] if line.strip()), None
                    )
                    if body_line:
