...
class HealthChecker:
    ...
    def check_worker_health(self, worker_name: str) -> Dict[str, Any]:
        ...
        if process_check["exists"]:
            ...
            if config.get("queue_name"):
                health_data["checks"]["queue"] = self._check_queue(config["queue_name"])

            # エラー率チェック
            health_data["checks"]["errors"] = self._check_error_rate(worker_name)
    ...
    def _check_process(self, process_name: str) -> Dict[str, Any]:
        ...
        return {"exists": False, "pid": None, "status": "not_found"}
    ...
    def _check_queue(self, queue_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"queue_name": queue_name, "healthy": False, "error": str(e)}
    ...
    def _check_error_rate(self, worker_name: str) -> Dict[str, Any]:
        ...
        if len(history) < 5:
            return {"error_rate": 0.0, "healthy": True, "sample_size": len(history)}
    ...
    def get_worker_trend(self, worker_name: str, minutes: int = 30) -> Dict[str, Any]:
        ...
        recent_history = [
            h for h in history if datetime.fromisoformat(h["timestamp"]) > cutoff_time
        ]
        ...
        avg_second_half = sum(scores[len(scores) // 2 :]) / (
