...
class TestLearningMagic:
    ...
    async def test_learn_from_success_pattern(self, learning_magic):
        """成功パターンからの学習テスト"""
        success_case = {
            "scenario": "API endpoint optimization",
    ...
    async def test_identify_learning_opportunities(self, learning_magic, sample_knowledge_base):
        """学習機会の特定テスト"""
        result = await learning_magic.identify_opportunities(sample_knowledge_base)
    ...
    async def test_integrate_sage_knowledge(self, learning_magic, mock_sage_responses):
        """4賢者知識統合テスト"""
        result = await learning_magic.integrate_sage_knowledge(mock_sage_responses)
        
        assert result["success"] is True
        integrated = result["integrated_knowledge"]
        
        # 各賢者の知識が統合されているか確認
        assert "cross_domain_insights" in integrated
        assert "knowledge_gaps" in integrated
        assert "improvement_suggestions" in integrated
        
        # 統合された洞察の品質確認
        insights = integrated["cross_domain_insights"]
        assert len(insights) > 0
        assert all("confidence" in insight for insight in insights)
        assert all(insight["confidence"] >= 0.5 for insight in insights)
        
    async def test_synthesize_patterns(self, learning_magic):
        """パターン合成テスト"""
        individual_patterns = [
            {
                "domain": "backend",
...
class TestLearningMagicIntegration:
    ...
    async def test_four_sages_learning_coordination(self):
        """4賢者学習協調テスト"""
