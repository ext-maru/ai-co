...
from elders_guild.elder_tree.conversation_manager import ConversationManager
from elders_guild.elder_tree.rag_manager import RAGManager
from elders_guild.elder_tree.self_evolution_manager import SelfEvolutionManager
from elders_guild.elder_tree.pm_git_integration import PMGitIntegration
...
class TestElderLegacyIntegration:
    ...
    async def test_conversation_rag_integration(self):
        ...
        # マネージャー初期化
        conv_db = Path(self.temp_dir) / "conversations.db"
        rag_db = Path(self.temp_dir) / "rag.db"
    ...
    async def test_evolution_git_integration(self):
        ...
        # マネージャー初期化
        evolution_manager = SelfEvolutionManager(self.temp_dir)
    ...
    async def test_full_elder_legacy_flow(self):
        ...
        # 全コンポーネント初期化
        conv_manager = ConversationManager(str(Path(self.temp_dir) / "conv.db"))
        rag_manager = RAGManager(str(Path(self.temp_dir) / "rag.db"))
