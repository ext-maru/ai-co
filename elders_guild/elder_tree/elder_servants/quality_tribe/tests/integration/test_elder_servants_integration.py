...
from elders_guild.elder_tree.elder_servants.registry.servant_registry import ServantRegistry, get_registry

# Elder Flow imports (モック用)
try:
    from elders_guild.elder_tree.elder_system.flow.elder_flow_core import ElderFlowCore
except ImportError:
    ElderFlowCore = None
...
class TestDwarfServant(DwarfServant):
    ...
    def __init__(self, name: str, domain: str):
        # nameをservant_idとしても使用し、specializationはnameから推定
        specialization = name.replace("dwarf_", "").replace("_", " ").title()
        super().__init__(name, name, specialization)
        self.capabilities = [
...
class TestWizardServant(WizardServant):
    ...
    def __init__(self, name: str, domain: str):
        # nameをservant_idとしても使用し、specializationはnameから推定
        specialization = name.replace("wizard_", "").replace("_", " ").title()
        super().__init__(name, name, specialization)
        self.capabilities = [
...
class TestElfServant(ElfServant):
    ...
    def __init__(self, name: str, domain: str):
        # nameをservant_idとしても使用し、specializationはnameから推定
        specialization = name.replace("elf_", "").replace("_", " ").title()
        super().__init__(name, name, specialization)
        self.capabilities = [
...
class ElderServantsIntegrationTester:
    ...
    async def setup_servants(self):
        ...
        for i, spec in enumerate(knight_specializations):
            ...
            self.registry.register(
                TestKnightServant, name, ServantDomain.INCIDENT_KNIGHTS
            )
    ...
    async def test_individual_servants(self) -> Dict[str, Any]:
        ...
        for servant_info in all_servants:
            ...
            test_request = ServantRequest(
                ...
                task_type=servant_info["capabilities"][0]
                if servant_info["capabilities"]
                else "test",
                priority="medium",
    ...
    async def test_organization_coordination(self) -> Dict[str, Any]:
        ...
        # デバッグ情報
        print(f"🔍 組織別サーバント数:")
        print(f"  ドワーフ: {len(dwarf_servants)} サーバント")
        print(f"  ウィザード: {len(wizard_servants)} サーバント")
        print(f"  エルフ: {len(elf_servants)} サーバント")
        print(f"  騎士: {len(knight_servants)} サーバント")
        ...
        collaboration_scenarios = [
            ...
            },
            {
                ...
                "participants": [
                    ...
                    (
                        "wizard",
                        wizard_servants[1] if len(wizard_servants) > 1 else None,
                    ),
                ],
            },
            {
                ...
                "participants": [
                    ...
                    (
                        "wizard",
                        wizard_servants[2] if len(wizard_servants) > 2 else None,
                    ),
                ],
            },
        ]
        ...
        for scenario in collaboration_scenarios:
            try:
                    ...
                    scenario_result["participant_results"].append(
                        {
                            ...
                            "processing_time": response.metrics.get(
                                "processing_time", 0
                            ),
                        }
                    )
                ...
                results["coordination_tests"].append(
                    {"scenario": scenario["name"], "status": "error", "error": str(e)}
                )
        ...
        for scenario in flow_test_scenarios:
            try:
                ...
                flow_result = await self.mock_elder_flow.execute(
                    scenario["task_name"], scenario["priority"]
                )
                ...
                for servant_type in scenario["expected_servants"]:
                    ...
                    if servant_type == "dwarf":
                        servants = self.registry.find_by_domain(
    ...
    async def test_load_balancing(self) -> Dict[str, Any]:
        ...
        async def process_task(request):
            try:
                ...
                return {"task_id": request.task_id, "status": "error", "error": str(e)}
        ...
        for results in test_results:
            if isinstance(results, dict):
                ...
                if "successful_collaborations" in results:
                    total_collab = (
                        results["successful_collaborations"]
                        + results["failed_collaborations"]
                    )
                    if total_collab > 0:
                        collab_rate = (
                            results["successful_collaborations"] / total_collab
                        )
...
class TestElderServantsIntegration:
    ...
    """負荷分散テスト"""
        ...
        active_servants = sum(
            1
            for servant_data in results["load_distribution"].values()
    ...
    """ヘルスチェックテスト"""
        ...
        healthy_count = sum(
            1
            for status in health_status["servants_status"].values()
    ...
    """完全統合ワークフローテスト"""
        ...
        assert (
            results["organization_coordination_tests"]["successful_collaborations"] >= 2
        )
