...
with patch.dict('sys.modules', {
    ...
    # テスト対象のインポート
    from elders_guild.quality_servants.quality_watcher_servant import QualityWatcherServant
...
class TestServantFeatures:
    ...
    async def test_quality_certificate_generation(self, comprehensive_guardian):
        """品質証明書生成機能のテスト"""
        # 最新結果を返すようモック
        # シンプルなクラスを作成してjson.dumps()で__dict__が正しくシリアライズできるようにする
        class MockResult:
            def __init__(self):
                self.overall_quality_score = 92.0
                self.target_path = "/test/path"
                self.documentation_score = 91
                self.security_score = 93
                self.config_score = 90
                self.performance_score = 94
                self.doc_completeness = 95
                self.api_doc_coverage = 98
                self.security_vulnerabilities = 0
                self.security_compliance_level = "HIGH"
                self.config_consistency = 92
                self.config_best_practices_score = 90
                self.performance_efficiency = 94
                self.optimization_level = "ADVANCED"
        
        result_mock = MockResult()
