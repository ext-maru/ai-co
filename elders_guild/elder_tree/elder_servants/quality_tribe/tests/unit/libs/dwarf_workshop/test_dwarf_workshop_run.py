...
async def test_api_architect():
    ...
    try:
        from elders_guild.elder_tree.elder_servants.dwarf_workshop.api_architect import APIArchitect
...
async def test_database_shaper():
    ...
    try:
        from elders_guild.elder_tree.elder_servants.dwarf_workshop.database_shaper import DatabaseShaper
...
async def test_existing_servants():
    ...
    # D03: RefactorSmith
    try:
        from elders_guild.elder_tree.elder_servants.dwarf_workshop.refactor_smith import RefactorSmith
    ...
    try:
        from elders_guild.elder_tree.elder_servants.dwarf_workshop.performance_tuner import (
            PerformanceTuner,
        )
    ...
    # D05: BugHunter
    try:
        from elders_guild.elder_tree.elder_servants.dwarf_workshop.bug_hunter import BugHunter

        bug_hunter = BugHunter()
        print(f"✅ D05 BugHunter 初期化成功: {bug_hunter.servant_id}")
        ...
        test_results.append(("D05 BugHunter", result.status.name == "COMPLETED"))

    except Exception as e:
        print(f"❌ D05 BugHunter エラー: {e}")
        test_results.append(("D05 BugHunter", False))

    # D06: SecurityGuard
    try:
        from elders_guild.elder_tree.elder_servants.dwarf_workshop.security_guard import SecurityGuard
