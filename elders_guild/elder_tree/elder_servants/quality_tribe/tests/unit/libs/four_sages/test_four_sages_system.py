...
# 4賢者のインポート
from elders_guild.elder_tree.four_sages.base_sage import BaseSage, SageRegistry
...
from elders_guild.elder_tree.four_sages.task.task_sage import TaskPriority, TaskSage, TaskStatus
...
class TestKnowledgeSage:
    ...
    @pytest.fixture
    def knowledge_sage(self, temp_dir):
        return KnowledgeSage(knowledge_base_path=temp_dir)
...
class TestTaskSage:
    ...
    @pytest.fixture
    def task_sage(self, temp_dir):
        return TaskSage(data_path=temp_dir)
...
class TestIncidentSage:
    ...
    @pytest.fixture
    def incident_sage(self, temp_dir):
        return IncidentSage(data_path=temp_dir)
...
class TestRAGSage:
    ...
    @pytest.fixture
    def rag_sage(self, temp_dir):
        return RAGSage(data_path=temp_dir)
...
class TestFourSagesIntegration:
    ...
    def temp_dirs(self):
        """各賢者用の一時ディレクトリ"""
        dirs = {
            "knowledge": tempfile.mkdtemp(),
            "task": tempfile.mkdtemp(),
            "incident": tempfile.mkdtemp(),
    ...
    @pytest.fixture
    def four_sages_registry(self, temp_dirs):
        """
        4賢者を統合したレジストリ"""
        registry = SageRegistry()

        # 4賢者を作成し登録
        knowledge_sage = KnowledgeSage(knowledge_base_path=temp_dirs["knowledge"])
        task_sage = TaskSage(data_path=temp_dirs["task"])
        incident_sage = IncidentSage(data_path=temp_dirs["incident"])
        rag_sage = RAGSage(data_path=temp_dirs["rag"])
