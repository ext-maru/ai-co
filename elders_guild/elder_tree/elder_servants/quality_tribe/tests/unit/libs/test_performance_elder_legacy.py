...
class TestElderLegacyPerformance:
    ...
    async def test_conversation_manager_performance(self):
        ...
        db_path = Path(self.temp_dir) / "perf_conv.db"
    ...
    async def test_rag_manager_performance(self):
        """RAGManager性能テスト"""
        db_path = Path(self.temp_dir) / "perf_rag.db"
    ...
    async def test_concurrent_operations(self):
        """並行処理性能テスト"""
        # マネージャー初期化
        conv_manager = ConversationManager(str(Path(self.temp_dir) / "concurrent_conv.db"))
        rag_manager = RAGManager(str(Path(self.temp_dir) / "concurrent_rag.db"))
