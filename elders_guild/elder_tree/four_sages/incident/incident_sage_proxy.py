...
class IncidentSageProxy(BaseSageProxy):
    ...
    async def report_incident(
        self, severity: str, title: str, details: Dict[str, Any]
    ) -> Dict[str, Any]:
        """インシデントを報告"""
        return await self.call_sage(
            "report_incident", severity=severity, title=title, details=details
        )

    async def analyze_incident(self, incident_id: str) -> Dict[str, Any]:
        """インシデントを分析"""
        return await self.call_sage("analyze_incident", incident_id=incident_id)

    async def get_incident_status(self, incident_id: str) -> Dict[str, Any]:
        """インシデントステータスを取得"""
        return await self.call_sage("get_incident_status", incident_id=incident_id)
    ...
    async def resolve_incident(
        ...
        """インシデントを解決"""
        return await self.call_sage(
            "resolve_incident", incident_id=incident_id, resolution=resolution
        )
    ...
    async def get_incident_list(
        ...
        """インシデントリストを取得"""
        return await self.call_sage(
            "get_incident_list", status=status, severity=severity
        )
    ...
    async def emergency_response(
        ...
        """緊急対応"""
        return await self.call_sage(
            "emergency_response", incident_type=incident_type, context=context
        )

    async def get_incident_stats(self, period: Optional[str] = None) -> Dict[str, Any]:
        """インシデント統計を取得"""
        return await self.call_sage("get_incident_stats", period=period)
