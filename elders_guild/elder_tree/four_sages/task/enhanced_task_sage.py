...
from elders_guild.elder_tree.four_sages.task.dynamic_priority_engine import DynamicPriorityEngine
from elders_guild.elder_tree.four_sages.task.execution_time_predictor import ExecutionTimePredictor
from elders_guild.elder_tree.four_sages.task.resource_optimization_engine import ResourceOptimizationEngine
from elders_guild.elder_tree.four_sages.task.task_sage import TaskEntry, TaskSage, TaskStatus
from elders_guild.elder_tree.four_sages.task.task_scheduling_optimizer import (
...
class EnhancedTaskSage(TaskSage):
    ...
    async def _optimize_schedule_request(
        ...
        try:
            ...
            tasks = await self.database.search_tasks(
                status=TaskStatus(status), limit=request.get("limit", 100)
            )
            ...
            # リソース最適化も実行
            resource_result = await self.resource_engine.optimize_resource_allocation(
                tasks
            )
            ...
            # ガントチャートデータも生成
            gantt_data = self.scheduling_optimizer.get_gantt_chart_data(
                optimized_schedule
            )
            ...
            return {
                ...
                "metrics": {
                    ...
                    "parallelization_factor": resource_result.get(
                        "parallel_groups", []
                    ),
                    "estimated_total_time": sum(
                        st.predicted_time for st in optimized_schedule
                    ),
                },
            }
    ...
    async def _predict_completion_request(
        self, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        """完了時間予測リクエスト"""
        try:
            ...
            # 動的優先度計算
            dynamic_priority = await self.priority_engine.calculate_dynamic_priority(
                task
            )

            # 現在の実行状況を考慮した開始時刻予測
            pending_tasks = await self.database.search_tasks(
                status=TaskStatus.PENDING, limit=100
            )

            # 簡易的な開始時刻計算
            queue_position = len([t for t in pending_tasks if t.id != task_id])
            estimated_start = datetime.now() + timedelta(
                seconds=queue_position * 300
            )  # 5分/タスク
            ...
            return {
                ...
                "accuracy_metrics": self.time_predictor.get_model_accuracy(
                    task.task_type
                ),
            }
    ...
    async def _analyze_resources_request(
        self, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        """リソース分析リクエスト"""
        try:
            # 現在のシステムリソース
            current_resources = (
                await self.resource_engine.resource_monitor.get_current_usage()
            )

            # 実行中タスクのリソース使用状況
            executing_tasks = await self.database.search_tasks(
                status=TaskStatus.IN_PROGRESS, limit=50
            )
            ...
            # 平均リソース使用状況
            avg_resources = self.resource_engine.resource_monitor.get_average_usage(
                minutes=10
            )
            ...
            return {
                ...
                "resource_analysis": {
                    "current": {
                        ...
                        "disk_io_mb": current_resources.disk_io_read_mb
                        + current_resources.disk_io_write_mb,
    ...
    async def _get_recommendations_request(
        self, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        """タスク推奨リクエスト"""
        try:
            ...
            priority_recommendations = (
                await self.priority_engine.get_priority_recommendations(limit)
            )
            ...
            # リソース最適化推奨
            pending_tasks = await self.database.search_tasks(
                status=TaskStatus.PENDING, limit=50
            )

            resource_optimization = (
                await self.resource_engine.optimize_resource_allocation(
                    pending_tasks[:limit]
                )
            )
            ...
            return {
                ...
                "recommendations": {
                    ...
                    "resource_recommendations": resource_optimization.get(
                        "recommendations", []
                    ),
                    "summary": {
                        "high_priority_count": len(
                            [
                                ...
                                if r["dynamic_priority"] > 100
                            ]
                        ),
                        "optimal_parallel_groups": len(
                            resource_optimization.get("parallel_groups", [])
                        ),
    ...
    async def _update_tracking_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if task.actual_duration:
                ...
                await self.priority_engine.update_priority_learning(
