...
from elders_guild.elder_tree.four_sages.task.task_sage import TaskEntry, TaskPriority, TaskType
...
class PredictionModel:
    ...
    def predict(self, features: TimeFeatures) -> float:
        ...
        # 依存関係による調整
        predicted_time += (
            features.dependency_count * self.coefficients["dependency_factor"]
        )
        ...
        # 優先度による調整
        priority_mult = self.coefficients["priority_multiplier"].get(
            features.priority, 1.0
        )
        predicted_time *= priority_mult

        # 説明文の長さによる調整
        predicted_time += (features.description_length / 100) * self.coefficients[
            "description_length_factor"
        ]
        ...
        if features.historical_avg_time > 0:
            ...
            predicted_time = (
                predicted_time * (1 - self.coefficients["historical_weight"])
        ...
        if features.estimated_hours > 0:
            ...
            predicted_time = (
                predicted_time * (1 - self.coefficients["estimation_weight"])
    ...
    def update_model(
        self, features: TimeFeatures, actual_time: float, predicted_time: float
    ):
        """実際の実行時間に基づいてモデルを更新"""
        # 予測誤差を記録
        if actual_time > 0:
...
class ExecutionTimePredictor:
    ...
    async def _initialize_model_from_history(self, task_type: TaskType):
        ...
        try:
            ...
            for task_data in tasks:
                if task_data.get("status") == "completed" and task_data.get(
    ...
    async def _get_historical_avg_time(self, task_type: TaskType) -> float:
        ...
        try:
            ...
            avg_execution_time = metrics_summary.get("stats", {}).get(
