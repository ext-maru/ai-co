...
class CompletionReportTracker:
    ...
    def __init__(
        self, storage_dir: str = "/home/aicompany/ai_co/data/completion_reports"
    ):
        """
        初期化
        ...
        logger.info(
            f"CompletionReportTracker initialized with directory: {storage_dir}"
        )
    ...
    def update_task_status(
        self, task_id: str, status: str, update_data: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        タスクステータスを更新
    ...
    def submit_completion_report(
        ...
        try:
            ...
            report_file = os.path.join(
                self.storage_dir, f"{completion_report['report_id']}.json"
            )
            ...
            self.active_tasks[task_id]["completion_report"] = completion_report[
                "report_id"
            ]

            # 完了したタスクをアクティブリストから移動
            if completion_report["status"] in [
            ...
            logger.info(
                f"Completion report submitted: {completion_report['report_id']}"
            )
    ...
    def get_active_tasks(
        self, assignee: Optional[str] = None, task_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        アクティブなタスクを取得
