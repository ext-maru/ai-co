...
class KnowledgeSage(BaseSage):
    ...
    async def _store_knowledge(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3connect(self.db_path) as conn:
            ...
            # 重複チェック
            cursor.execute(
                "SELECT id FROM knowledge_entries WHERE hash = ?", (content_hash,)
            )
            if cursor.fetchone():
                return {
                    "success": False,
    ...
    async def _get_recommendations(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        search_result = await self._search_knowledge(
            {"query": context, "category": category, "limit": 5}
        )
    ...
    async def _get_insights(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        insights = {
            "knowledge_base_stats": {
                ...
                "most_accessed": [
                    {"title": title, "access_count": count}
                    for title, count in popular_knowledge
                ],
            },
            "recommendations": [
        ...
        return {"success": True, "topic": topic, "insights": insights}
