...
class MockChannel:
    ...
    def queue_bind(self, queue, exchange, routing_key=""):
        ...
        binding = {"queue": queue, "exchange": exchange, "routing_key": routing_key}
    ...
    def basic_publish(
        ...
        if mandatory and not routed:
            raise Exception(
                f"Message could not be routed: exchange={exchange}, routing_key={routing_key}"
            )
    ...
    def basic_nack(self, delivery_tag, multiple=False, requeue=True):
        ...
        if not self.is_open:
            ...
            f"Message nacked: {delivery_tag} (multiple: {multiple}, requeue: {requeue})"
        )
...
class MockConnection:
    ...
    def __init__(
        self, host="localhost", port=5672, virtual_host="/", credentials=None, **kwargs
    ):
        self.host = host
        self.port = port
