...
class ReportManager:
        ...
        if result["success"]:
            ...
            if self.config.get("notification_enabled"):
                self._send_notification(
                    f"New task registered: {task_data.get('title', task_id)}", "info"
                )

        return result

    def submit_report(
        self, task_id: str, report_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        完了報告を提出し、分析と決定を行う
        ...
        try:
            # 報告を提出
            submission_result = self.tracker.submit_completion_report(
                task_id, report_data
            )
            ...
            report = next(
                (
                    r
                    for r in reports
                    if r["report_id"] == submission_result["report_id"]
                ),
                None,
            )

            if not report:
                return {"success": False, "error": "Report not found after submission"}
            ...
            if self.config.get("notification_enabled"):
                self._send_completion_notification(
                    task_id, report, result.get("analysis")
                )
        ...
        for report in reports:
            if report["task_id"] == task_id:
                ...
                if report["report_id"] in self.analyzer.analysis_cache:
                    overview["analysis"] = self.analyzer.analysis_cache[
                        report["report_id"]
                    ]
                else:
                    overview["analysis"] = self.analyzer.analyze_completion_report(
                        report
                    )
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_tasks": {
                ...
                "critical": len(
                    [t for t in active_tasks if t.get("priority") == "CRITICAL"]
                ),
    ...
    def _send_high_priority_notification(self, task_id: str, decision: Dict[str, Any]):
        """高優先度の決定通知を送信"""
        actions = decision.get("recommended_actions", [])
        if actions:
            priority_actions = [a["title"] for a in actions[:2]]
            message = (
                f"High confidence decision for {task_id}: {', '.join(priority_actions)}"
            )
