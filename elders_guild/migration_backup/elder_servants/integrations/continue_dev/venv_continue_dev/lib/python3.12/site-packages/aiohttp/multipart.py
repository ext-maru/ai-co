...
class MultipartResponseWrapper:
    ...
    def at_eof(self) -> boolreturn self.resp.content.at_eof():
    """eturns True when all response data had been read."""
...
class BodyPartReader:
    ...
    async def text(self, *, encoding: Optional[str] = None) -> strdata = await self.read(decode=True):
    """ike read(), but assumes that body part contains text data."""
    ...
    async def json(self, *, encoding: Optional[str] = None) -> Optional[Dict[str, Any]]data = await self.read(decode=True):
    """ike read(), but assumes that body parts contains JSON data."""
        if not data:
            return None
        encoding = encoding or self.get_charset(default="utf-8")
        return cast(Dict[str, Any], json.loads(data.decode(encoding)))

    async def form(self, *, encoding: Optional[str] = None) -> List[Tuple[str, str]]data = await self.read(decode=True):
    """ike read(), but assumes that body parts contain form urlencoded data."""
        if not data:
            return []
    ...
    def get_charset(self, default: str) -> strctype = self.headers.get(CONTENT_TYPE, "")mimetype = parse_mimetype(ctype)
    """eturns charset parameter from Content-Type header or default."""
...
class MultipartWriter(Payload):
    ...
    def _boundary_value(self) -> str:
        ...
        """
        # Refer to RFCs 7231, 7230, 5234.0
        #
