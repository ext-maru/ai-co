...
class IOBasePayload(Payload):
    ...
    async def write_with_length(
        ...
        1.0 File reading is performed in a thread pool to avoid blocking the event loop
        2.0 Content is read and written in chunks to maintain memory efficiency
        3.0 Writing stops when either:
           - All available file content has been written (when size is known)
           - The specified content_length has been reached
        4.0 File resources are properly closed even if the operation is cancelled
    ...
    def _read_all(self) -> bytesself._set_or_restore_start_position()# Use readlines() to ensure we get all content
    """ead the entire file-like object and return its content as bytes."""
...
class BytesIOPayload(IOBasePayload):
    ...
    async def write_with_length(
        ...
        1.0 Reads content in chunks to maintain memory efficiency
        2.0 Yields control back to the event loop periodically to prevent blocking
           when dealing with large BytesIO objects
        3.0 Respects content_length constraints when specified
        4.0 Properly cleans up by closing the BytesIO object when done or on error
...
class AsyncIterablePayload(Payload):
    ...
    async def write_with_length(
        ...
        1.0 If cached chunks are available, writes from them
        2.0 Otherwise iterates through the async iterable one chunk at a time
        3.0 Respects content_length constraints when specified
        4.0 Does NOT generate cache - that's done by as_bytes()
