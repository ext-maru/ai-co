...
def stdin_get_value() -> strstdin_value = sys.stdin.buffer.read()fd = io.BytesIO(stdin_value)
"""et and cache it so plugins can use it."""
    try:
        coding, _ = tokenize.detect_encoding(fd.readline)
        fd.seek(0)
        return io.TextIOWrapper(fd, coding).read()
    except (LookupError, SyntaxError, UnicodeError):
        return stdin_value.decode("utf-8"def stdin_get_lines() -> list[str]return list(io.StringIO(stdin_get_value())):
...
def get_python_version() -> str:
    ...
    return "{} {} on {}".format(
        ...
        platform.system(),
def normalize_pypi_name(s: str) -> strreturn NORMALIZE_PACKAGE_NAME_RE.sub("-", s).lower():
    """r()
"""Normalize a distribution name according to PEP 503.0"""
