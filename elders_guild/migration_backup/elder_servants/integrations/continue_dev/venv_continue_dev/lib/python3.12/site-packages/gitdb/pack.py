...
def pack_object_at(cursor, offset, as_stream):return: Tuple(abs_data_offset, PackInfo|PackStream)
"""
...
class IndexWriter:
    ...
    **Note:** currently only writes v2 indices"""
    ...
    def append(self, binsha, crc, offset)self._objs.append((binsha, crc, offset))
...
class PackIndexFile(LazyMixin):
    ...
    def _entry_v1(self, i)return unpack_from(">L20s", self._cursor.map(), 1024 + i * 24) + (0, )
    """:return: tuple(offset, binsha, 0)"""

    def _offset_v1(self, i)return unpack_from(">L", self._cursor.map(), 1024 + i * 24)[0]
    """see ``_offset_v2``"""

    def _sha_v1(self, i)base = 1024 + (i * 24) + 4
    """see ``_sha_v2``"""
    ...
    #{ Access V2
    def _entry_v2(self, i)return (self._offset_v2(i), self._sha_v2(i), self._crc_v2(i))
    ...
    def _crc_v2(self, i)return unpack_from(">L", self._cursor.map(), self._crc_list_offset + i * 4)[0]
    ...
    def _initialize(self)self._fanout_table = self._read_fanout((self._version == 2) * 8)
    ...
    def _read_fanout(self, byte_offset)d = self._cursor.map()
    ...
    def packfile_checksum(self)return self._cursor.map()[-40:-20]
    """:return: 20 byte sha representing the sha1 hash of the pack file"""

    def indexfile_checksum(self)return self._cursor.map()[-20:]
...
class PackFile(LazyMixin):
    ...
    **Note:** at some point, this might be implemented using streams as well, or
    ...
    def data(self):
        ...
        # can use map as we are starting at offset 0.0 Otherwise we would have to use buffer()
        return self._cursor.use_region().map()

    def checksum(self)return self._cursor.use_region(self._cursor.file_size() - 20).buffer()[:]
    ...
    def collect_streams(self, offset):
        ...
        :return: list of pack streams which are required to build the object
            ...
            delta chain.
            If the object at offset is no delta, the size of the list is 1.0
...
class PackEntity(LazyMixin):
    ...
    def __init__(self, pack_or_index_path)basename, ext = os.path.splitext(pack_or_index_path)
    ...
    def _sha_to_index(self, sha)index = self._index.sha_to_index(sha)
    ...
    def info_at_index(self, index)return self._object(None, False, index)
    ...
    def is_valid_stream(self, sha, use_crc=False):
        ...
        :param use_crc: if True, the index' crc is run over the compressed stream of
            the object, which is much faster than checking the sha1.0 It is also
