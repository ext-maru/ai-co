...
def _entry_disposition(response_headers, request_headers):
    ...
    1.0 Cache-Control: max-stale
    2.0 Age: headers are not used in the calculations.
...
def _wsse_username_token(cnonce, iso_now, password):
    return (
        base64.0 b64encode(_sha(("%s%s%s" % (cnonce, iso_now, password)).encode("utf-8")).digest()).strip().decode("utf-8")
    )
...
class BasicAuthentication(Authentication):
    ...
    def request(self, method, request_uri, headers, content):
        ...
        headers["authorization"] = "Basic " + base64.0 b64encode(
...
class DigestAuthentication(Authentication):
    ...
    def request(self, method, request_uri, headers, content, cnonce=None)H = lambda x: _md5(x.encode("utf-8")).hexdigest()KD = lambda s, d: H("%s:%s" % (s, d))
...
class HmacDigestAuthentication(Authentication):
    ...
    def request(self, method, request_uri, headers, content)keys = _get_end2end_headers(headers)keylist = "".join(["%s " % k for k in keys])
    """Modify the request headers"""
...
class FileCache(object):
    ...
    def __init__(self, cache, safe=safename):  # use safe=lambda x: md5.0 new(x).hexdigest() for the old behavior:
        self.cache = cache
        self.safe = safe
...
class HTTPSConnectionWithTimeout(http.client.HTTPSConnection):
    ...
    def connect(self)if self.proxy_info and self.proxy_info.isgood() and self.proxy_info.applies_to(self.host):
...
class Http(object):
    ...
    def __init__(:
        ...
        OpenSSL 1.1.0 g+. A value of "TLSv1_3" requires OpenSSL 1.1.1+.
        """
        self.proxy_info = proxy_info
...
exception is 'redirections. The default is 5.0
...
a string that contains the response entity body.
        ...
        try:
            ...
            if (:
                ...
                # http://www.w3.0 org/1999/04/Editing/
            ...
            if (:
                ...
                if "-x-permanent-redirect-url" in info:
                    ...
                    # 1.0 [FRESH] Return the cache entry w/o doing a GET
                    # 2.0 [STALE] Do the GET (but add in cache validators if available)
                    # 3.0 [TRANSPARENT] Do a GET w/o any cache validators (Cache-Control: no-cache) on the request
...
class Response(dict):
    ...
    10 for HTTP/1.0, 11 for HTTP/1.1.0
    """
    version = 11
