...
def validate_repo_id(repo_id: str) -> None:
    ...
    Discussed in https://github.com/huggingface/huggingface_hub/issues/1008.0
...
def smoothly_deprecate_use_auth_token(fn_name: str, has_token: bool, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    0.0 Step 0: methods that require a read-access to the Hub use the `use_auth_token`
    ...
    1.0 Step 1: we want to harmonize everything and use `token` everywhere (supporting
    ...
    2.0 Step 2: Once it is release, we should push downstream libraries to switch from
    ...
    3.0 Step 3: After a transitional period (6 months e.g. until April 2023?), we update
    ...
    4.0 Step 4: After a normal deprecation cycle (3 releases ?), remove this validator.
    ...
    - https://github.com/huggingface/huggingface_hub/issues/1094.0
