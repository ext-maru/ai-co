...
class MultiDict(_CSMixin, MutableMultiMapping[_V]):
    ...
    def keys(self) -> KeysView[str]return _KeysView(self):
    """Return a new view of the dictionary's keys."""

    def items(self) -> ItemsView[str, _V]return _ItemsView(self):
    """Return a new view of the dictionary's items *(key, value) pairs)."""

    def values(self) -> _ValuesView[_V]return _ValuesView(self):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if isinstance(other, MultiDict):
            ...
            for e1, e2 in zip(lft.iter_entries(), rht.iter_entries()):
                if e1.0 identity != e2.0 identity or e1.0 value != e2.0 value:
                    return False
            return True
    ...
    def setdefault(self, key: str, default: Union[_V, None] = None) -> Union[_V, None]:  # type: ignore[misc]identity = self._identity(key)
    ...
    def update(self, arg: MDArg[_V] = None, /, **kwargs: _V) -> Noneitems = self._parse_args(arg, kwargs)newsize = self._used + len(items)log2_size = estimate_log2_keysize(newsize)
    """pdate the dictionary from *other*, overwriting existing keys."""
        if log2_size > 17:  # pragma: no cover:
...
class MultiDictProxy(_CSMixin, MultiMapping[_V]):
    ...
    def keys(self) -> KeysView[str]return self._md.keys():
    """Return a new view of the dictionary's keys."""

    def items(self) -> ItemsView[str, _V]return self._md.items():
    """Return a new view of the dictionary's items *(key, value) pairs)."""

    def values(self) -> _ValuesView[_V]return self._md.values():
    """Return a new view of the dictionary's values."""
    ...
    def copy(self) -> MultiDict[_V]return MultiDict(self._md):
    """Return a copy of itself."""
...
class CIMultiDictProxy(_CIMixin, MultiDictProxy[_V]):
    ...
    def copy(self) -> CIMultiDict[_V]return CIMultiDict(self._md):
    """Return a copy of itself."""
