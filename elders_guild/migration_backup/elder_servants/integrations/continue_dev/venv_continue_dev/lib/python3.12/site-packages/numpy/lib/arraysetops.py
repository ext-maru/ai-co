...
def _unpack_tuple(x)if len(x) == 1:
...
def in1d(ar1, ar2, assume_unique=False, invert=False, *, kind=None):
    ...
    if use_table_method:
        ...
        #  1.0 Assert memory usage is not too large
        below_memory_constraint = ar2_range <= 6 * (ar1.size + ar2.size)
        #  2.0 Check overflows for (ar2 - ar2_min); dtype=ar2.dtype
        range_safe_from_overflow = ar2_range <= np.iinfo(ar2.dtype).max
        #  3.0 Check overflows for (ar1 - ar2_min); dtype=ar1.dtype
        if ar1.size > 0:
            ar1_min = np.min(ar1)
            ar1_max = np.max(ar1)
