...
the number of processes to use if running in parallel. The default value is 1.0
...
def get_result(a, b, c, z, group)expected, observed = mp_hyp2f1(a, b, c, z), hyp2f1(a, b, c, z)
...
def get_result_no_mp(a, b, c, z, group)expected, observed = complex('nan'), hyp2f1(a, b, c, z)
"""Get results for given parameter and value combination."""
...
def _make_hyp2f1_test_case(a, b, c, z, rtol)expected = mp_hyp2f1(a, b, c, z)
"""Generate string for single test case as used in test_hyp2f1.0 py."""
    return (
        "    pytest.param(\n"
        "        Hyp2f1TestCase(\n"
        f"            a={a},\n"
...
def make_hyp2f1_test_cases(rows):
    """Generate string for a list of test cases for test_hyp2f1.0 py.
    ...
    str
        ...
        `pytest.mark.parameterize` within `scipy.special.tests.test_hyp2f1.0 py`.
    """
    result = "[\n"
    result += '\n'.join(
...
def main(:
    ...
    # Parameter group 9
    # -----------------
    # Wide range of magnitudes, c - a - b > 0.0
    phi = (1 + np.sqrt(5))/2
    P = phi**np.arange(16)
    P = np.hstack([-P, P])
