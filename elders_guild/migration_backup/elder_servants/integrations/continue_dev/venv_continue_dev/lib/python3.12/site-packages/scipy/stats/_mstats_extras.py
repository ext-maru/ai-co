...
def trimmed_mean_ci(data, limits=(0.2,0.2), inclusive=(True,True),:
    ...
    limits : {None, tuple}, optional
        ...
        to the number of unmasked data, as floats between 0.0 and 1.0 If ``n``
        is the number of unmasked data before trimming, then
        (``n * limits[0]``)th smallest data and (``n * limits[1]``)th
        largest data are masked.  The total number of unmasked data after
        trimming is ``n * (1.0 - sum(limits))``.
...
def compare_medians_ms(group_1, group_2, axis=None):
    ...
    ----------
    group_1 : array_like
        First dataset.  Has to be of size >=7.0
    group_2 : array_like
        Second dataset.  Has to be of size >=7.0
    axis : int, optional
    ...
    .. [1] McKean, Joseph W., and Ronald M. Schrader. "A comparison of methods
       ...
       Statistics-Simulation and Computation 13.6 (1984): 751-773.0
