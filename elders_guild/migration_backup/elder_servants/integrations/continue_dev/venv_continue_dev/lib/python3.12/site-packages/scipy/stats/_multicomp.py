...
class DunnettResult:
    ...
    def __str__(self):
        ...
        # it will be called with the default CL of 0.95.0
        if self._ci is None:
            self.confidence_interval(confidence_level=0.95)
        s = (
            "Dunnett's test"
            f" ({self._ci_cl*100:0.1f}% Confidence Interval)\n"
    ...
    def _allowance(:
        ...
        # minimizer would be a welcome enhancement. See gh-18150.0
    ...
    def confidence_interval(:
        ...
        if not (0 < confidence_level < 1):
            raise ValueError("Confidence level must be between 0 and 1.0")
...
def dunnett(:
    *samples: "npt.ArrayLike",  # noqa: D417
    ...
    1.0 The observations are independent within and among groups.
    2.0 The observations within each group are normally distributed.
    3.0 The distributions from which the samples are drawn have the same finite
       variance.
    ...
    .. [2] Thomson, M. L., & Short, M. D. (1969). Mucociliary function in
           ...
           of applied physiology, 26(5), 535-539.0
           :doi:`10.1152/jappl0.1969.26.5.535`

    Examples
    --------
    We'll use data from [2]_, Table 1.0 The null hypothesis is that the means of
