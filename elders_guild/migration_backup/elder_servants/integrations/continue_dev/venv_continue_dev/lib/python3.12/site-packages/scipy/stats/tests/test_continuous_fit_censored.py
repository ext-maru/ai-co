...
def test_beta():
    ...
    assert_allclose(b, 1.27066, rtol=5e-6)
    assert loc == 0
    assert scale == 1
...
def test_chi2_mixed()Test fitting just the shape parameter (df) of chi2 to mixed data.
"""
    ...
    * one interval-censored [[4, 8]].
    ...
    assert_allclose(df, 5.60329, rtol=5e-6)
    assert loc == 0
    assert scale == 1
...
def test_invgauss():
    ...
    assert_allclose(mu, 1.66716, rtol=5e-5)
    assert loc == 0
...
def test_laplace():
    ...
    +     return(f)
    + }
    > left <- c(NA, -41.564, 50.0, 15.7384, 50.0, 10.452, -2.684,
    +           -19.5399, 50.0,   9.005, 27.1227, 4.3113, -3.7372,
    +           25.3111, 14.7987,  34.887,  50.0, 42.8496, 18.5862,
    +           32.8921, 9.448, -27.4591, NA, 19.5083, -9.7199)
    > right <- c(-50.0, -41.564,  NA, 15.7384, NA, 10.452, -2.684,
    +            -19.5399, NA, 9.005, 27.1227, 4.3113, -3.7372,
    +            25.3111, 14.7987, 34.887, NA,  42.8496, 18.5862,
    +            32.8921, 9.448, -27.4591, -50.0, 19.5083, -9.7199)
    > data <- data.frame(left=left, right=right)
    ...
    obs = np.array([-50.0, -41.564, 50.0, 15.7384, 50.0, 10.452, -2.684,
                    -19.5399, 50.0, 9.005, 27.1227, 4.3113, -3.7372,
                    25.3111, 14.7987, 34.887, 50.0, 42.8496, 18.5862,
                    32.8921, 9.448, -27.4591, -50.0, 19.5083, -9.7199])
...
def test_logistic():
    ...
    +          10.9044, 21.629,  9.6985,  NA    , 12.9016, 39.164 , 34.6396,
    +          NA    , 20.3665, 16.5889, 18.952, 45.3818, 35.3306,  8.4949,
    +          3.4041,  NA    ,  7.2828, 37.1265,  6.5969, 17.6868, 17.4977,
    +          16.3391, 36.541)
    > right = c(13.5401, 37.4235, 11.906 , 13.998 ,  0.0    ,  0.4023,  0.0    ,
    +           10.9044, 21.629,  9.6985,  0.0    , 12.9016, 39.164 , 34.6396,
    +           0.0    , 20.3665, 16.5889, 18.952, 45.3818, 35.3306,  8.4949,
    +           3.4041,  0.0    ,  7.2828, 37.1265,  6.5969, 17.6868, 17.4977,
    +           16.3391, 36.541)
    > data = data.frame(left=left, right=right)
    ...
    2.931505 1.546879
    """
    # Values that are zero are left-censored; the true values are less than 0.0
    x = np.array([13.5401, 37.4235, 11.906, 13.998, 0.0, 0.4023, 0.0, 10.9044,
                  21.629, 9.6985, 0.0, 12.9016, 39.164, 34.6396, 0.0, 20.3665,
                  16.5889, 18.952, 45.3818, 35.3306, 8.4949, 3.4041, 0.0,
...
def test_ncx2()Test fitting the shape parameters (df, ncp) of ncx2 to mixed data.
"""
    ...
    * 2 right-censored values [8, 8].
    ...
    with np.errstate(over='ignore'):  # remove context when gh-14901 is closed:
        df, ncp, loc, scale = ncx2.fit(data, floc=0, fscale=1,
                                       optimizer=optimizer)
    assert_allclose(df, 1.52871, rtol=5e-6)
...
def test_weibull_min_sas1():
    ...
    assert_allclose(c, 1.584, rtol=1e-4)
...
def test_weibull_min_sas2():
    ...
    assert_allclose(loc, 122.3, rtol=5e-4)
