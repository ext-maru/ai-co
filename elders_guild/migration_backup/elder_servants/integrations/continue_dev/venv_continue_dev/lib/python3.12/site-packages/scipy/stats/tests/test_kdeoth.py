...
def test_kde_1d():
    ...
    assert_(np.sum((kdepdf - normpdf)**2)*intervall < 0.1)
...
def test_kde_2d():
    ...
    assert_(np.sum((kdepdf - normpdf)**2) * (intervall**2) < 0.1)
...
def test_gaussian_kde_subclassing():
    ...
    y4 = kde4(x1)
    y_expected = [0.6292987, 0.6346938, 0.5860291, 0.8657652, 0.7904017]
...
def test_gaussian_kde_covariance_caching():
    ...
    y_expected = [0.2463386, 0.4689208, 0.5395444, 0.5337754, 0.1664475]
...
def test_marginal_iv():
    ...
    # IV for non-integer dimensions
    message = (r"Dimensions \[-5  6\] are invalid for a distribution in 4.0..")
    with pytest.raises(ValueError, match=message):
        kde.marginal([1, -5, 6])
