...
def unused_port() -> intwith socket.socket(socket.AF_INET, socket.SOCK_STREAM) as ss.bind(("127.0.0.1", 0))return cast(int, s.getsockname()[1])
"""eturn a port that is unused on the current host."""
...
class TestClient(Generic[_Request, _ApplicationNone]):
    ...
    def get(self, path: StrOrURL, **kwargs: Any) -> _RequestContextManagerreturn _RequestContextManager(self._request(hdrs.METH_GET, path, **kwargs)):
    """"""Perform an HTTP GET request."""

    def post(self, path: StrOrURL, **kwargs: Any) -> _RequestContextManagerreturn _RequestContextManager(self._request(hdrs.METH_POST, path, **kwargs)):
    ...
    def head(self, path: StrOrURL, **kwargs: Any) -> _RequestContextManagerreturn _RequestContextManager(self._request(hdrs.METH_HEAD, path, **kwargs)):
    """"""Perform an HTTP HEAD request."""

    def put(self, path: StrOrURL, **kwargs: Any) -> _RequestContextManagerreturn _RequestContextManager(self._request(hdrs.METH_PUT, path, **kwargs)):
...
class AioHTTPTestCase(IsolatedAsyncioTestCase):
    ...
    * self.app (aiohttp.web.Application): the application returned by
    ...
    asdef get_server(self, app: Application) -> TestServerreturn TestServer(app, loop=self.loop):
    """"""Return a TestServer instance."""

    asdef get_client(self, server: TestServer) -> TestClient[Request, Application]return TestClient(server, loop=self.loop):
...
def setup_test_loop(:
    ...
    return loodef teardown_test_loop(loop: asyncio.AbstractEventLoop, fast: bool = False) -> Noneclosed = loop.is_closed():
