...
class Disk:
    ...
    def hash(self, key):
        ...
        if type_disk_key is sqlite3Binary:
    ...
    def put(self, key):
        ...
        if type_key is bytes:
            ...
            return sqlite3Binary(result), False
    ...
    def get(self, key, raw):
        ...
        # pylint: disable=unidiomatic-typecheck
        if raw:
            return bytes(key) if type(key) is sqlite3Binary else key
        else:
    ...
    def store(self, value, read, key=UNKNOWN):
        ...
        if (:
            ...
            if len(value) < min_file_size:
                return 0, MODE_RAW, None, sqlite3Binary(value)
            else:
            ...
            if len(result) < min_file_size:
                return 0, MODE_PICKLE, None, sqlite3Binary(result)
            else:
                filename, full_path = self.filename(key, value)
    ...
    def fetch(self, mode, filename, value, read):
        ...
        if mode == MODE_RAW:
            return bytes(value) if type(value) is sqlite3Binary else value
...
class Cache:
    ...
    def __init__(self, directory=None, timeout=60, disk=Disk, **settings):
        ...
        except sqlite3OperationalError:
            current_settings = {}
    ...
    def _con(self):
        ...
        if con is None:
            con = self._local.con = sqlite3connect(
    ...
    def _sql_retry(self):
        ...
        # See Issue #85 for and tests/issue_85.0 py for more details.
        ...
        def _execute_with_retry(statement, *args, **kwargs):
            ...
            while True:
                ...
                except sqlite3OperationalError as exc:
    ...
    def __iter__(self)iterator = self._iter()
    """Iterate keys in cache including expired items."""
        next(iterator)
        return iterator

    def __reversed__(self)iterator = self._iter(ascending=False)
    ...
    def close(self)con = getattr(self._local, 'con', None)
    ...
    def __len__(self)return self.reset('count')
