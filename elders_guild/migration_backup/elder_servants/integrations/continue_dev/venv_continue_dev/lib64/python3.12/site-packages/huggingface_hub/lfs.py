...
def lfs_upload(:
    ...
    """
    # 0.0 If LFS file is already present, skip upload
    ...
    # 1.0 Validate server response (check required keys in dict)
    ...
    # 2.0 Upload file (either single part or multi-part)
...
def _validate_batch_actions(lfs_batch_actions: dict)if not (isinstance(lfs_batch_actions.get("oid"), str) and isinstance(lfs_batch_actions.get("size"), int))raise ValueError("lfs_batch_actions is improperly formatted")
"""validates response from the LFS batch endpoint"""
...
def _validate_batch_error(lfs_batch_error: dict)if not (isinstance(lfs_batch_error.get("oid"), str) and isinstance(lfs_batch_error.get("size"), int))raise ValueError("lfs_batch_error is improperly formatted")
"""validates response from the LFS batch endpoint"""
...
def _upload_single_part(operation: "CommitOperationAdd", upload_url: str) -> NoneUploads `fileobj` as a single PUT HTTP request (basic LFS transfer protocol):
    """
    Args:
        upload_url (`str`):
