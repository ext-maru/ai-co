...
def _convert_stringified_numbers(value)if isinstance(value, str):
...
def register_model(model_cost: Union[str, dict]):  # noqa: PLR0915Register new / Override existing models (and their pricing) to specific providers.
"""
...
def get_optional_params(  # noqa: PLR0915:
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params: List[str])Check if the params passed to completion() are supported by the provider:
    """
...
def _get_model_info_helper(  # noqa: PLR0915:
    ...
    try:
        ...
        if custom_llm_provider == "huggingface":
            ...
            1.0 'custom_llm_provider/model' in litellm.model_cost. Checks "groq/llama3-8b-8192" if model="llama3-8b-8192" and custom_llm_provider="groq"
            2.0 'model' in litellm.model_cost. Checks "gemini-1.5-pro-002" in  litellm.model_cost if model="gemini-1.5-pro-002" and custom_llm_provider=None
            3.0 'combined_stripped_model_name' in litellm.model_cost. Checks if 'gemini/gemini-1.5-flash' in model map, if 'gemini/gemini-1.5-flash-001' given.
            4.0 'stripped_model_name' in litellm.model_cost. Checks if 'ft:gpt-3.5-turbo' in model map, if 'ft:gpt-3.5-turbo:my-org:custom_suffix:id' given.
            5.0 'split_model' in litellm.model_cost. Checks "llama3-8b-8192" in litellm.model_cost if model="groq/llama3-8b-8192"
            """
            ...
            if _input_cost_per_token is None:
                ...
                verbose_logger.debug(
                    "model={}, custom_llm_provider={} has no input_cost_per_token in model_cost_map. Defaulting to 0.0".format(
            ...
            if _output_cost_per_token is None:
                # default value to 0, be noisy about this
                verbose_logger.debug(
                    "model={}, custom_llm_provider={} has no output_cost_per_token in model_cost_map. Defaulting to 0.0".format(
                        model, custom_llm_provider
                    )
                )
...
def get_model_info(model: str, custom_llm_provider: Optional[str] = None) -> ModelInfoGet a dict for the maximum tokens (context window), input_cost_per_token, output_cost_per_token  for a given model.:
"""
...
class AvailableModelsCache(InMemoryCache):
    ...
    def _check_env_changed(self) -> boolcurrent_hash = self._get_env_hash():
...
def _infer_valid_provider_from_env_vars(:
    ...
    for provider in litellm.provider_list:
        ...
        expected_provider_key_1 = f"{env_provider_1.0 upper()}_API_KEY"
        expected_provider_key_2 = f"{env_provider_2.0 upper()}_API_KEY"
        if (:
...
def is_base64_encoded(s: str) -> bool:
    try:
        ...
        decoded_bytes = base64.0 b64decode(s, validate=True)

        # Check if the original string can be re-encoded to the same string
        return base64.0 b64encode(decoded_bytes).decode("utf-8") == s
    except Exception:
        return False
...
class ProviderConfigManager:
    ...
    def get_provider_anthropic_messages_config(:
        ...
        # The 'BEDROCK' provider corresponds to Amazon's implementation of Anthropic Claude v3.0
...
def extract_duration_from_srt_or_vtt(srt_or_vtt_content: str) -> Optional[float]Extracts the total duration (in seconds) from SRT or VTT content.:
"""
