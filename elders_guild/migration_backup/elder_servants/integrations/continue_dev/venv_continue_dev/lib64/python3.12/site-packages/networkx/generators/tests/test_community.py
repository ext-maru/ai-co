...
def test_gaussian_random_partition_graph():
    G = nx.gaussian_random_partition_graph(100, 10, 10, 0.3, 0.1)
    assert len(G) == 100
    G = nx.gaussian_random_partition_graph(100, 10, 10, 0.3, 0.1, directed=True)
    assert len(G) == 100
    G = nx.gaussian_random_partition_graph(
        100, 10, 10, 0.3, 0.1, directed=False, seed=42
    )
    assert len(G) == 100
    assert not isinstance(G, nx.DiGraph)
    G = nx.gaussian_random_partition_graph(
        100, 10, 10, 0.3, 0.1, directed=True, seed=42
    )
    assert len(G) == 100
...
def test_stochastic_block_model():
    sizes = [75, 75, 300]
    probs = [[0.25, 0.5, 0.2], [0.5, 0.35, 0.7], [0.2, 0.7, 0.40]]
    ...
    badprobs1 = [[0.25, 0.5, 1.2], [0.5, 0.35, 0.7], [0.2, 0.7, 0.40]]
    badprobs2 = [[0.25, 0.5, 0.2], [0.5, -0.35, 0.7], [0.2, 0.7, 0.40]]
    probs_rect1 = [[0.25, 0.5, 0.2], [0.5, -0.35, 0.7]]
    probs_rect2 = [[0.25, 0.5], [0.5, -0.35], [0.2, 0.7]]
    asymprobs = [[0.25, 0.5, 0.1], [0.5, -0.35, 0.7], [0.2, 0.7, 0.40]]
...
def test_no_scipy_zeta():
    ...
    assert abs(zeta2 - nx.generators.community._hurwitz_zeta(2, 1, 0.001)) < 0.1
...
def test_generate_min_degree_itr():
    with pytest.raises(:
        ...
        nx.generators.community._generate_min_degree(2, 2, 1, 0.1, 0)
