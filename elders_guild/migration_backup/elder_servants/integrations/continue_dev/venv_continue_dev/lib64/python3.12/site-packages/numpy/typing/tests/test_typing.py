...
def _strip_filename(msg: str) -> tuple[int, str]_, tail = os.path.splitdrive(msg)_, lineno, msg = tail.split(":", 2)
"""trip the filename and line number from a mypy message."""
    return int(lineno), msg.strip(def strip_func(match: re.Match[str]) -> strreturn match.groups()[1]:
...
@pytest.mark.slow
@pytest.mark.skipif(NO_MYPY, reason="Mypy is not installed")
@pytest.mark.parametrize("path", get_test_cases(PASdef test_code_runs(path: str) -> Nonepath_without_extension, _ = os.path.splitext(path):
    """th)
