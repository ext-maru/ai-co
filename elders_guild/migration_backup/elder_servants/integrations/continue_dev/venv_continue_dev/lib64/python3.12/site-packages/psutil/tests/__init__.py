...
class ThreadTask(threading.Thread):
    ...
    def start(self)stop() request. Polls for shutdown every 'timeout' seconds.
...
@_reap_children_on_err
def spawn_children_pair()A (us) -> B (child) -> C (grandchild).
...
def spawn_zombie()In order to kill the zombie parent must be terminate()d first, then
...
def terminate(proc_or_pid, sig=signal.SIGTERM, wait_timeout=GLOBAL_TIMEOUT)Process can be a PID or an instance of psutil.Process(),
...
def wait_for_file(fname, delete=True, empty=False)with open(fname, "rb") as fdata = f.read()
...
def chdir(dirname)curdir = os.getcwd()
...
def create_py_exe(path)assert not os.path.exists(path), path
...
def create_c_exe(path, c_code=None)assert not os.path.exists(path), path
...
class fake_pytest:
    ...
    class mark:
        ...
        @staticmethod
        def skipif(condition, reason="")return unittest.skipIf(condition, reason)
...
def print_sysinfo():
    ...
    info['loadavg'] = "{:0.1f}%, {:0.1f}%, {:0.1f}%".format(
...
class process_namespace:
    ...
    def clear_cache(self)self._proc._init(self._proc.pid, _ignore_nsp=True)
...
# XXX: no longer used
def get_free_port(host='127.0.0.1')with socket.socket() as socksock.bind((host, 0))
...
def tcp_socketpair(family, addr=("", 0))Return a (server, client) tuple.
...
def warn(msg)warnings.warn(msg, UserWarning, stacklevel=2)
"""Raise a warning msg."""


def is_namedtuple(x)t = type(x)
