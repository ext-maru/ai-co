...
class RegexLexerMeta(LexerMeta):
    ...
    def _process_regex(cls, regex, rflags, state)if isinstance(regex, Future)regex = regex.get()
    """Preprocess the regular expression component of a token definition."""
        return re.compile(regex, rflags).match

    def _process_token(cls, token)assert type(token) is _TokenType or callable(token), \
    """Preprocess the token component of a token definition."""
            f'token type must be simple type or callable, not {token!r}'
        return token

    def _process_new_state(cls, new_state, unprocessed, processed)if isinstance(new_state, str):
    ...
    def _process_state(cls, unprocessed, processed, state)assert isinstance(state, str), f"wrong state name {state!r}"
    """Preprocess a single state definition."""
        ...
        for tdef in unprocessed[state]:
            ...
            if isinstance(tdef, _inherit):
                ...
                # 1.0 the state has no counterpart in any parent
                # 2.0 the state includes more than one 'inherit'
                continue
...
class ProfilingRegexLexer(RegexLexer, metaclass=ProfilingRegexLexerMeta):
    ...
    def get_tokens_unprocessed(self, text, stack=('root',)):
        ...
        print()
        print('Profiling result for %s lexing %d chars in %0.3f ms' %
