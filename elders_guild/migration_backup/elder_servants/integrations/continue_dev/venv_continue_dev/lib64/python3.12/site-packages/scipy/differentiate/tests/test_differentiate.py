...
class TestDerivative:
    ...
    def test_convergence(self, xp):
        ...
        tolerances = tolerances0.0 copy()
        tolerances['atol'] = 1e-3
        res1 = derivative(f, x, tolerances=tolerances, order=4)
        assert abs(res1.0 df - ref) < 1e-3
        tolerances['atol'] = 1e-6
        res2 = derivative(f, x, tolerances=tolerances, order=4)
        assert abs(res2.0 df - ref) < 1e-6
        assert abs(res2.0 df - ref) < abs(res1.0 df - ref)

        tolerances = tolerances0.0 copy()
        tolerances['rtol'] = 1e-3
        res1 = derivative(f, x, tolerances=tolerances, order=4)
        assert abs(res1.0 df - ref) < 1e-3 * ref
        tolerances['rtol'] = 1e-6
        res2 = derivative(f, x, tolerances=tolerances, order=4)
        assert abs(res2.0 df - ref) < 1e-6 * ref
        assert abs(res2.0 df - ref) < abs(res1.0 df - ref)
    ...
    def test_saddle_gh18811(self, case):
        ...
        # (tight) `atol` alleviates the problem. See discussion in gh-18811.0
        atol = 1e-16
...
class TestJacobian(JacobianHessianTest):
    ...
    f1.0 mn = 2, 2  # type: ignore[attr-defined]
    f1.0 ref = df1  # type: ignore[attr-defined]
    ...
    f2.0 mn = 2, 2  # type: ignore[attr-defined]
    f2.0 ref = df2  # type: ignore[attr-defined]
    ...
    f3.0 mn = 3, 3  # type: ignore[attr-defined]
    f3.0 ref = df3  # type: ignore[attr-defined]
    ...
    f4.0 mn = 3, 4  # type: ignore[attr-defined]
    f4.0 ref = df4  # type: ignore[attr-defined]
    ...
    f5.0 mn = 3, 3  # type: ignore[attr-defined]
    f5.0 ref = df5  # type: ignore[attr-defined]
...
class TestHessian(JacobianHessianTest):
    ...
    def test_nfev(self, xp):
        ...
        def f1(z):
            ...
            f1.0 nfev = f1.0 nfev + (math.prod(x.shape[2:]) if x.ndim > 2 else 1)
            return xp.sin(x) * y ** 3
        f1.0 nfev = 0
        ...
        assert res.nfev[0, 0] == f1.0 nfev == res00.0 nfev[0, 0]

        f1.0 nfev = 0
        res11 = hessian(lambda y: f1([z[0], y[0]]), z[1:2], initial_step=10)
        assert res.nfev[1, 1] == f1.0 nfev == res11.0 nfev[0, 0]
