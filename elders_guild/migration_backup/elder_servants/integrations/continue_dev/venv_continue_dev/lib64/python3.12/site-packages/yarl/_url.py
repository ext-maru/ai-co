...
@lru_cache
def pre_encoded_url(url_str: str) -> "URL"self = object.__new__(URL)val = split_url(url_str)
"""arse pre-encoded URL."""
...
class URL:
    ...
    def __str__(self) -> str:
        ...
        if (port := self.explicit_port) is not None and port == DEFAULT_PORTS.get(:
            ...
            # https://datatracker.ietf.org/doc/html/rfc3986.0 html#section-6.2.3
    ...
    def host_port_subcomponent(self) -> Union[str, None]:
        ...
        if raw[-1] == ".":
            ...
            # See https://github.com/aio-libs/aiohttp/issues/3636.0
    ...
    @cached_property
def _parsed_query(self) -> list[tuple[str, str]]return query_to_pairs(self._query):
    ...
    @cached_property
    def path_qs(self) -> strreturn self.path if not (q := self.query_string) else f"{self.path}?{q}"
    ...
    @cached_property
def name(self) -> strreturn UNQUOTER(self.raw_name):
...
def without_query_params(self, *query_params: str) -> "URL"params_to_remove = set(query_params) & self.query.keys():
...
def joinpath(self, *other: str, encoded: bool = False) -> "URL"return self._make_child(other, encoded=encoded):
    """"""Return a new URL with the elements in other appended to the path."""

    def human_repr(self) -> struser = human_quote(self.user, "#/:?@[]")
...
@rewrite_moddef cache_clear() -> None_idna_encode.cache_clear()_idna_decode.cache_clear()
