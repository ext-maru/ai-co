...
class CodebaseAnalyzer:
    ...
    async def analyze_codebase(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "patterns": patterns,
            "categorized_files": {
                k: [str(f) for f in v] for k, v in categorized_files.items()
            },
            "analysis_metadata": {
        ...
        for node in ast.walk(tree):
            ...
            # Try-Except (エラーハンドリング)
                ...
                for handler in node.handlers:
                    exc_type = (
                        self._ast_to_string(handler.type)
                        if handler.type
                        else "Exception"
                    )
    ...
    def get_summary_stats(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        class_stats = {
            ...
            "avg_methods": (
                sum(c.get("method_count", 0) for c in classes) / len(classes)
                if classes
                else 0
            ),
        }
        ...
        func_stats = {
            ...
            "with_docstrings": sum(
                1 for f in functions if f.get("has_docstring", False)
            ),
            "with_type_hints": sum(
                1 for f in functions if f.get("has_type_hints", False)
            ),
        }
        ...
        return {
            ...
            "logging_methods": Counter(
                patterns.get("logging_patterns", [])
            ).most_common(5),
        }
