...
class ASTAnalyzer:
    ...
    def _detect_tech_indicators(self, content: str) -> Dict[str, float]:
        ...
        indicators["aws"] = sum(1 for kw in aws_keywords if kw in content_lower) / len(
            aws_keywords
        )

        # Web関連
        web_keywords = ["flask", "django", "fastapi", "request", "response", "api"]
        indicators["web"] = sum(1 for kw in web_keywords if kw in content_lower) / len(
            web_keywords
        )
        ...
        indicators["data"] = sum(
            1 for kw in data_keywords if kw in content_lower
        ) / len(data_keywords)

        # データベース関連
        db_keywords = ["sql", "database", "postgresql", "mysql", "sqlite", "query"]
        indicators["database"] = sum(
            1 for kw in db_keywords if kw in content_lower
        ) / len(db_keywords)

        # テスト関連
        test_keywords = ["test", "pytest", "mock", "assert", "fixture"]
        indicators["testing"] = sum(
            1 for kw in test_keywords if kw in content_lower
        ) / len(test_keywords)

        # 非同期関連
        async_keywords = ["async", "await", "asyncio", "coroutine"]
        indicators["async"] = sum(
            1 for kw in async_keywords if kw in content_lower
        ) / len(async_keywords)

        # セキュリティ関連
        security_keywords = ["auth", "token", "password", "encrypt", "hash", "security"]
        indicators["security"] = sum(
            1 for kw in security_keywords if kw in content_lower
    ...
    def _get_exception_name(self, exc_type: ast.expr) -> Optional[str]:
        ...
        try:
            if isinstance(exc_type, ast.Name):
                ...
                return (
                    f"{exc_type.value.id}.{exc_type.attr}"
                    if hasattr(exc_type.value, "id")
                    else exc_type.attr
                )
...
class PatternExtractor:
    ...
    def extract_class_patterns(
        ...
        for analysis in analyses:
            ...
            tech_indicators = analysis.get("tech_indicators", {})
            primary_tech = (
                max(tech_indicators.items(), key=lambda x: x[1])[0]
                if tech_indicators
    ...
    def analyze_naming_conventions(
        ...
        if class_names:
            pascal_case_count = sum(
                1 for name in class_names if name[0].isupper() and "_" not in name
            )
...
class CodebaseAnalysisEngine:
    ...
    def analyze_codebase(self, tech_stack: Dict[str, Any]) -> CodebaseIntelligence:
        ...
        error_handling = self.pattern_extractor.extract_error_handling_patterns(
            analyses
        )
        ...
        self.logger.info(
            f"Analysis complete: {len(import_patterns)} import patterns, "
    ...
    def _filter_relevant_files(
        self, files: List[Path], tech_stack: Dict[str, Any]
    ) -> List[Path]:
        ...
        return [
            file_path for file_path, _ in relevant_files[:10]
        ]  # 最大10ファイル (最適化)
