...
class QualityMetricsMonitor:
    ...
    def start_monitoring(self):
        ...
        self.logger.info(
            f"🔄 Quality monitoring started (interval: {self.monitoring_interval}s)"
        )
    ...
    def record_pipeline_execution(
        self, pipeline_result, execution_time: float, pipeline_id: str
    ):
        """
        パイプライン実行結果記録
        ...
        # 実行時間メトリクス
        self._record_metric(
            MetricType.EXECUTION_TIME, execution_time, timestamp, pipeline_id
        )

        # Elder承認率計算・記録
        elder_approval = (
            1.0 if pipeline_result.overall_status == "ELDER_APPROVED" else 0.0
        )
        self._record_metric(
            MetricType.ELDER_APPROVAL_RATE, elder_approval, timestamp, pipeline_id
        )
        ...
        self._record_metric(MetricType.ERROR_RATE, error_rate, timestamp, pipeline_id)
    ...
    def _generate_alert(
        ...
        if existing_key not in self.active_alerts:
            ...
            if (
                alert_level in [AlertLevel.CRITICAL, AlertLevel.EMERGENCY]
                and self.elder_council_reporting["auto_escalation"]
            ):
    ...
    def get_current_metrics_summary(self) -> Dict[str, Any]:
        ...
        for metric_type in MetricType:
            ...
            recent_values = [m.value for m in history if m.timestamp >= hour_ago]
    ...
    def generate_monitoring_report(self, period_hours: int = 24) -> MonitoringReport:
        """監視レポート生成"""
        ...
        average_quality_score = (
            statistics.mean([m.value for m in quality_metrics])
            if quality_metrics
            else 0.0
        )
        elder_approval_rate = (
            (successful_executions / total_executions * 100.0)
            if total_executions > 0
            else 0.0
        )
        average_execution_time = (
            statistics.mean([m.value for m in execution_metrics])
        ...
        if len(efficiency_metrics) >= 10:
            first_half = statistics.mean(
                [m.value for m in efficiency_metrics[: len(efficiency_metrics) // 2]]
            )
        ...
        resolved_alerts = [
            a for a in self.resolved_alerts if start_time <= a.timestamp <= end_time
        ]
        ...
        elder_council_summary = {
            ...
            "overall_health": (
                "GOOD"
                if elder_approval_rate >= 90.0
                else "NEEDS_ATTENTION" if elder_approval_rate >= 75.0 else "CRITICAL"
            ),
    ...
    def _should_generate_elder_report(self) -> bool:
        ...
        time_since_last = (
            datetime.now() - self.elder_council_reporting["last_report_time"]
        )
        return (
            time_since_last.total_seconds()
            >= self.elder_council_reporting["report_interval"]
        )
        ...
        for key, alert in list(self.active_alerts.items()):
            if current_time - alert.timestamp > auto_resolve_threshold:
                ...
                if alert.metric_type in self.metrics_history:
                    recent_metrics = list(self.metrics_history[alert.metric_type])[
                        -5:
                    ]  # 最新5件
                    if recent_metrics and self._is_alert_resolved(
                        alert, recent_metrics
                    ):
                        self.resolve_alert(
                            alert.alert_id, "自動解決: メトリクス改善確認"
                        )

    def _is_alert_resolved(
        self, alert: QualityAlert, recent_metrics: List[QualityMetric]
    ) -> bool:
        """アラート解決判定"""
    ...
    def get_monitoring_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_elder_report": self.elder_council_reporting[
                "last_report_time"
            ].isoformat(),
        }
