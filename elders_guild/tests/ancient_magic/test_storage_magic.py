...
class TestStorageMagic:
    ...
    async def test_restore_from_backup(self, storage_magic, temp_storage_dir):
        ...
        test_data = {"important": True, "value": 12345}
    ...
    async def test_storage_magic_invalid_intent(self, storage_magic):
        """無効な意図での魔法発動テスト"""
        result = await storage_magic.cast_magic("invalid_storage_intent", {})
    ...
    async def test_retrieve_nonexistent_data(self, storage_magic):
        """存在しないデータの取得テスト"""
        retrieval_params = {
            "storage_id": "nonexistent_id_12345",
            "format": "json"
        }
    ...
    async def test_large_data_handling(self, storage_magic, temp_storage_dir):
        ...
        # 大容量テストデータ生成（1MB程度）
        large_data = {
            "large_array": ["test_data_item_" + str(i) for i in range(10000)],
            "metadata": {
                "size": "large",
                "items": 10000,
