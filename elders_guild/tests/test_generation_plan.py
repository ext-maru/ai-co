...
class ElderTreeTestGenerator:
    ...
    def _identify_test_targets(self) -> List[Dict[str, Any]]:
        ...
        # 4賢者
        sage_files = [
            "agents/knowledge_sage.py",
            "agents/task_sage.py", 
            "agents/incident_sage.py",
            "agents/rag_sage.py"
        ]
        
        # サーバント
        servant_files = [
            "servants/base_servant.py",
            "servants/dwarf_servant.py",
            "servants/rag_wizard_servant.py",
            "servants/elf_servant.py",
            "servants/incident_knight_servant.py"
        ]
        
        # ワークフロー
        workflow_files = [
            "workflows/elder_flow.py"
        ]
        
        for file_path in sage_files + servant_files + workflow_files:
    ...
    async def generate_test_plan(self) -> Dict[str, Any]:
        """テスト計画の生成"""
        test_plan = {
            "total_files": len(self.test_targets),
...
# シナリオファイルの読み込み
scenarios('../features/{feature_file}.feature')
...
@when('アクション実行')
async def execute_action({fixture_name}):
    """アクションの実行"""
...
@pytest.mark.benchmark(group="{component_type}")
def test_{method_name}_performance(benchmark: BenchmarkFixture):
    """パフォーマンステスト"""
...
async def main():
    ...
    generator = ElderTreeTestGenerator()
