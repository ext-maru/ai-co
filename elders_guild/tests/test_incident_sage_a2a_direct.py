...
class TestIncidentSageA2ADirect:
    ...
    async def run_direct_tests(self) -> Dict[str, Any]:
        """直接テスト実行"""
        print("🚨 Incident Sage A2A Agent - 直接テストスイート開始")
        print("=" * 70)
        print("💡 A2A依存なしでビジネスロジック直接テスト")
        
        test_methods = [
            ("processor_initialization_test", self.test_processor_initialization),
            ("incident_detection_test", self.test_incident_detection),
            ("incident_response_test", self.test_incident_response),
            ("quality_assessment_test", self.test_quality_assessment),
            ("alert_management_test", self.test_alert_management),
            ("monitoring_management_test", self.test_monitoring_management),
            ("pattern_learning_test", self.test_pattern_learning),
            ("correlation_analysis_test", self.test_correlation_analysis),
            ("automated_remediation_test", self.test_automated_remediation),
            ("similar_incidents_search_test", self.test_similar_incidents_search),
            ("statistics_comprehensive_test", self.test_statistics_comprehensive),
            ("health_check_test", self.test_health_check),
            ("error_handling_test", self.test_error_handling),
            ("performance_test", self.test_performance_basic)
        ]
...
async def main():
    ...
    # テスト実行
    test_suite = TestIncidentSageA2ADirect()
    results = await test_suite.run_direct_tests()
