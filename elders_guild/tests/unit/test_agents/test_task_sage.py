...
class TestTaskSage:
    ...
    async def test_get_task_statistics(self, task_sage):
        """タスク統計テスト"""
        # 様々なステータスのタスクを作成
        statuses = ["pending"] * 5 + ["in_progress"] * 3 + ["completed"] * 7 + ["cancelled"] * 2
        priorities = ["low"] * 4 + ["medium"] * 6 + ["high"] * 5 + ["critical"] * 2
    ...
    async def test_elder_flow_task_request(self, task_sage):
        """Elder Flow用タスクリクエストテスト"""
        # Elder Flowからのタスク推定リクエスト
        flow_message = Mock()
        flow_message.data = {
    ...
    async def test_bulk_task_creation_performance(self, task_sage, benchmark):
        """大量タスク作成パフォーマンステスト"""
        # 100個のタスクデータを準備
        tasks_data = [TaskFactory() for _ in range(100)]
