...
class TestCodeCrafter:
    ...
    async def test_test_code_generation(self, code_crafter, code_spec):
        """テストコード生成の詳細テスト"""
        test_code = await code_crafter._generate_test_code(code_spec)
        
        # 生成されたテストコードの構造を検証
        assert "import pytest" in test_code
        assert "class Test" in test_code
        assert "def test_calculate_fibonacci_basic" in test_code
        assert "def test_calculate_fibonacci_edge_cases" in test_code
    ...
    async def test_implementation_code_generation(self, code_crafter, code_spec):
        """実装コード生成テスト"""
        impl_code = await code_crafter._generate_implementation_code(code_spec, None)
        
        # 生成されたコードの検証
        assert "def calculate_fibonacci(n: int) -> int:" in impl_code
...
"""
    ...
    async def test_complex_code_generation(self, code_crafter):
        """複雑なコード生成テスト"""
        complex_spec = {
            "function_name": "process_data_pipeline",
            "parameters": [
    ...
    async def test_sage_consultation_integration(self, code_crafter, code_spec, mock_sage_consultation):
        ...
        # 賢者協議をモック
        with patch.object(code_crafter, 'consult_sages_before_task', 
                         return_value=mock_sage_consultation):
    ...
    async def test_dwarf_servant_inheritance(self, code_crafter):
        """DwarfServant継承テスト"""
        # DwarfServantの特性を継承していることを確認
        assert code_crafter.tribe == "dwarf"
        assert code_crafter.code_style == "black"
        assert code_crafter.test_framework == "pytest"
        assert code_crafter.coverage_target == 95

    @pytest.mark.asyncio
    async def test_quality_gate_integration(self, code_crafter):
        """品質ゲート統合テスト"""
