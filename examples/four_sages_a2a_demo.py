...
class FourSagesOrchestrator:
    ...
    async def demonstrate_collaboration(self):
        ...
        try:
            ...
            logger.info(
                f"RAG enhanced search results: {len(rag_response.payload.data['retrieved_documents'])} documents"
            )
            ...
            logger.info(
                f"Knowledge analysis completed: {len(knowledge_response.payload.data['results'])} insights"
            )
            ...
            logger.info(
                f"Task assigned to: {task_response.payload.data['assigned_to']}"
            )
            ...
            logger.info(
                f"Pattern analysis: {pattern_response.payload.data['analysis']['complexity']}"
            )
            ...
            logger.info(
                f"Incident assessment: {incident_response.payload.data['risk_assessment']['impact']}"
            )
            ...
            logger.info(
                f"Recovery task assigned: {recovery_task.payload.data['task_id']}"
            )
    ...
    async def display_metrics(self):
        ...
        for sage_id, client in self.sages.items():
            ...
            logger.info(
                f"{sage_id}: Sent={metrics['messages_sent']}, "
