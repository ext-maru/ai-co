...
async def main():
    ...
    async def track_recovery(event):
        ...
        logger.info(f"🔧 Recovery attempt: {event['worker_id']} - {event['action'].value}")
    ...
    logger.info(f"📊 Health check service running on http://localhost:{health_service.port}/health")
    logger.info(f"📈 Metrics available at http://localhost:{health_service.port}/metrics")
    logger.info(f"📉 Prometheus metrics at http://localhost:{health_service.port}/metrics/prometheus")
    ...
    for worker_id in monitor.get_tracked_workers():
        state = monitor.get_worker_state(worker_id)
        logger.info(f"  - {worker_id}: {state.status.value} (failures: {state.consecutive_failures}, recoveries: {state.recovery_attempts})")
