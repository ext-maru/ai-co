...
class ConnectionPool:
    ...
    def _create_connection(self) -> sqlite3.Connection:
        try:
            ...
            self._logger.debug(f"Created new database connection ({self._created_connections})")
            return conn
...
class DatabaseManager:
    ...
    def select_one(self, query: str, parameters: Optional[Tuple] = None) -> Optional[sqlite3.Row]:
        with self.get_connection() as conn:
            try:
                cursor = conn.execute(query, parameters or ())
    ...
    def select_all(self, query: str, parameters: Optional[Tuple] = None) -> List[sqlite3.Row]:
        with self.get_connection() as conn:
            try:
                cursor = conn.execute(query, parameters or ())
    ...
    def select_many(self, query: str, parameters: Optional[Tuple] = None, size: int = 100) -> List[sqlite3.Row]:
        with self.get_connection() as conn:
            try:
                cursor = conn.execute(query, parameters or ())
...
if __name__ == "__main__":
    ...
    with DatabaseManager(config) as db:
        ...
        with db.transaction() as tx:
            tx.execute("INSERT INTO users (name, email) VALUES (?, ?)", ("Jane Doe", "jane@example.com"))
            tx.execute("INSERT INTO users (name, email) VALUES (?, ?)", ("Bob Smith", "bob@example.com"))
