...
class AIGitAssistant:
    ...
    def get_branch_status(self) -> Dict:
        ...
        # ステージングファイル取得
        success, staged_files = self._run_git_command(
            ["git", "diff", "--cached", "--name-only"]
        )
        staged_list = staged_files.split("\n") if staged_files else []

        # コミット数取得
        success, ahead_behind = self._run_git_command(
            [
                "git",
                "rev-list",
                "--count",
...
"""
    ...
    def intelligent_merge_check(self, target_branch: str) -> Dict:
        ...
        # コンフリクトチェック
        success, merge_base = self._run_git_command(
            ["git", "merge-base", current_branch, target_branch]
        )
        ...
        # 差分確認
        success, conflicts = self._run_git_command(
            ["git", "merge-tree", merge_base, current_branch, target_branch]
        )
    ...
    def create_release_notes(self, from_tag: str, to_tag: str = "HEAD") -> str:
        ...
        for commit in commits:
            ...
            if any(
                keyword in commit.lower() for keyword in ["feat", "add", "新機能", "追加"]
            ):
                features.append(commit)
            elif any(
                keyword in commit.lower() for keyword in ["fix", "bug", "修正", "バグ"]
            ):
                fixes.append(commit)
            else:
