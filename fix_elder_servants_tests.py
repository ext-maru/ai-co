    ...
    def _fix_path_import_issues(self):
        ...
        for test_file in test_files:
            try:
                ...
                if "PROJECT_ROOT = Path(" in content and "from pathlib import Path" in content:
                    lines = content.split('\n')
                    ...
                    for i, line in enumerate(lines):
                        if "PROJECT_ROOT = Path(" in line and path_usage_line is None:
                            path_usage_line = i
                        if "from pathlib import Path" in line and path_import_line is None:
                            path_import_line = i
...
sys.path.insert(0, str(PROJECT_ROOT))"""
        ...
        for test_file in test_files:
            try:
                ...
                if "PROJECT_ROOT" in content and ("parent.parent.parent.parent.parent" in content or "name 'Path' is not defined" in content):
                    self._standardize_project_root_setup(test_file, content, standard_setup)
                    
            except Exception as e:
                print(f"❌ PROJECT_ROOT修正失敗: {test_file} - {e}")
                self.error_files.append(test_file)
                
    def _standardize_project_root_setup(self, test_file: Path, content: str, standard_setup: str):
    ...
    def _add_conditional_import(self, test_file: Path, dependency: str, description: str):
        """条件付きインポートの追加"""
        try:
            content = test_file.read_text()
