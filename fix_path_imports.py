            ...
            if file.endswith('.py'):
                ...
                try:
                    ...
                    if 'PROJECT_ROOT = Path(' in content:
                        ...
                        if path_usage_line is not None and (path_import_line is None or path_import_line > path_usage_line):
                            ...
                            sys_import_pos = None
                            for i in range(insert_pos, min(path_usage_line, len(lines))):
                            ...
                            if sys_import_pos is not None:
                                lines.insert(sys_import_pos + 1, 'from pathlib import Path')
                            else:
                                lines.insert(insert_pos, 'from pathlib import Path')
                            
                            # Remove duplicate Path import if exists later
                            if path_import_line is not None and path_import_line > path_usage_line:
                                lines.pop(path_import_line + 1)  # +1 because we inserted a line
