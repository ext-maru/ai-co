...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s [ElderReporter] %(levelname)s: %(message)s'
)
...
class ElderReportingSystem:
    ...
    def _get_default_rules(self):
        ...
        return {
            "定期報告": {
                "通常時": {"頻度": "1時間", "内容": ["基本メトリクス"], "形式": "Markdown"}
            }
        }
    ...
    def check_urgency_level(self, metrics):
        ...
        # CRITICAL チェック
        if self._check_conditions(metrics, urgency_rules.get('CRITICAL', {}).get('条件', [])):
        ...
        # MEDIUM チェック
        if self._check_conditions(metrics, urgency_rules.get('MEDIUM', {}).get('条件', [])):
    ...
    def should_report(self, urgency_level, event_type="regular"):
        ...
        if event_type == "regular":
            ...
            if report_type in self.last_report_time:
                ...
                if frequency == "1時間" and (current_time - last_time).total_seconds() < 3600:
                    return False
                elif frequency == "15分" and (current_time - last_time).total_seconds() < 900:
                    return False
...
"""
    ...
    def save_report(self, report_content, report_format, urgency_level):
        ...
        # ファイル名決定
        if urgency_level in [UrgencyLevel.CRITICAL, UrgencyLevel.HIGH]:
            filename = f"alert_{urgency_level.value}_{timestamp.strftime('%Y%m%d_%H%M%S')}"
        else:
