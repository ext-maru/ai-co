...
try:
    from integration.unified_entity_manager import (
        ...
        IncidentEntity, TaskEntity, create_knowledge_entity,
        create_incident_entity, create_task_entity
    )
...
class UnifiedSystemSetup:
    ...
    def _initialize_database(self):
        ...
        if db_path.exists():
            backup_path = self.integration_dir / "backups" / f"unified_entities_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    ...
    def _migrate_knowledge_base_files(self):
        ...
        logger.info(f"ナレッジファイル処理完了: {self.migration_stats['knowledge_files_processed']}件")
    ...
    def _migrate_table_data(self, cursor: sqlite3.Cursor, table_name: str, db_source: str):
        """テーブルデータのマイグレーション"""
        try:
            ...
            for row in rows:
                ...
                if entity_type == 'incident':
                    ...
                    # 汎用エンティティとして処理
                    self._create_generic_entity_from_row(row_dict, table_name, db_source)
    ...
    def _create_incident_from_row(self, row_data: Dict, source: str):
        ...
        try:
            title = row_data.get('title') or row_data.get('description', 'Unknown Incident')[:50]
    ...
    def _create_generic_entity_from_row(self, row_data: Dict, table_name: str, source: str):
        """行データから汎用エンティティ作成"""
        try:
            title = (row_data.get('title') or 
                    row_data.get('name') or 
                    f"{table_name}_{row_data.get('id', 'unknown')}")
            
            content = (row_data.get('description') or 
                      row_data.get('content') or 
            ...
            for i, entity1 in enumerate(all_entities):
                for entity2 in all_entities[i+1:]:
                    ...
                    if relationship_type:
                        from integration.unified_entity_manager import EntityRelationship
    ...
    def _generate_setup_report(self):
        ...
        report = {
            ...
            'setup_status': 'success' if not self.migration_stats['errors'] else 'partial_success'
        }
        
        # レポートファイル出力
        report_path = self.integration_dir / f"setup_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _determine_domain_from_content(self, content: str) -> str:
        ...
        if any(word in content_lower for word in ['python', 'javascript', 'code', 'function']):
            return 'development'
        elif any(word in content_lower for word in ['server', 'database', 'api', 'deploy']):
            return 'operations'
        elif any(word in content_lower for word in ['error', 'incident', 'problem', 'issue']):
            return 'incident'
    ...
    def _extract_tags_from_content(self, content: str) -> List[str]:
        ...
        potential_tags = [
            'python', 'javascript', 'api', 'database', 'error', 'fix',
            'guide', 'documentation', 'best-practice', 'troubleshooting',
            'configuration', 'system', 'worker', 'task', 'incident'
        ]
    ...
    def _detect_relationship(self, entity1: BaseEntity, entity2: BaseEntity) -> Optional[str]:
        """エンティティ間の関係性を検出"""
        # 簡易的な関係性検出
        
        # インシデントと知識の関係
        if isinstance(entity1, IncidentEntity) and isinstance(entity2, KnowledgeEntity):
            if any(tag in entity2.metadata.get('tags', []) for tag in ['fix', 'solution', 'resolve']):
                return 'resolved_by'
        ...
        # 同じドメインの知識エンティティ
        if (isinstance(entity1, KnowledgeEntity) and isinstance(entity2, KnowledgeEntity)):
