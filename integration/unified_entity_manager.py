...
class UnifiedEntityManager:
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                if type_data:
                    ...
                    """
                    params = (
                        entity.id, entity.type, entity.title, entity.content,
    ...
    def get_entity(self, entity_id: str) -> Optional[BaseEntity]:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "SELECT * FROM unified_entities WHERE id = ?",
                    (entity_id,)
                )
                row = cursor.fetchone()
    ...
    def delete_entity(self, entity_id: str) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "DELETE FROM unified_entities WHERE id = ?",
                    (entity_id,)
                )
                conn.commit()
    ...
    def list_entities(self, 
                     ...
                     limit: int = 100,
                     offset: int = 0,
                     filters: Dict[str, Any] = None) -> List[BaseEntity]:
        """エンティティ一覧取得"""
        try:
            with self._get_connection() as conn:
                ...
                if filters:
                    for key, value in filters.items():
                        if key == 'status':
                            ...
                            conditions.append("json_extract(metadata, '$.priority') = ?")
                            params.append(value)
                        elif key == 'category':
    ...
    def create_relationship(self, relationship: EntityRelationship) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                logger.info(f"Relationship created: {relationship.source_id} -> {relationship.target_id}")
                return True
    ...
    def get_relationships(self, entity_id: str, direction: str = 'both') -> List[EntityRelationship]:
        """エンティティの関係性取得"""
        try:
            with self._get_connection() as conn:
    ...
    def find_related_entities(self, entity_id: str, 
                            ...
                            max_depth: int = 2) -> List[BaseEntity]:
        """関連エンティティ検索"""
        try:
            related_ids = set()
            ...
            with self._get_connection() as conn:
                while to_process:
                    ...
                    if relationship_types:
                        ...
                        """
                        params = [current_id] + relationship_types + [current_id] + relationship_types
                    else:
                        sql = """
    ...
    def search_entities(self, query: str,
        ...
        try:
            with self._get_connection() as conn:
                ...
                sql = f"SELECT * FROM unified_entities WHERE ({' AND '.join(sql_parts)})"
    ...
    def _row_to_entity(self, row: sqlite3.Row) -> BaseEntity:
        ...
        search_metadata = json.loads(row['search_metadata']) if row['search_metadata'] else {}
        ...
        if row['type'] == 'knowledge':
            knowledge_data = json.loads(row['knowledge_data']) if row['knowledge_data'] else {}
            ...
            incident_data = json.loads(row['incident_data']) if row['incident_data'] else {}
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                # エンティティ数
                cursor = conn.execute("SELECT type, COUNT(*) FROM unified_entities GROUP BY type")
...
def create_knowledge_entity(title: str, content: str, 
                           confidence_score: float = 0.8,
                           domain: str = 'general') -> KnowledgeEntity:
    """知識エンティティ作成ヘルパー"""
...
def create_incident_entity(title: str, content: str,
                          severity: str = 'medium',
                          affected_systems: List[str] = None) -> IncidentEntity:
    """インシデントエンティティ作成ヘルパー"""
...
def create_task_entity(title: str, content: str,
                      task_type: str = 'general',
                      assigned_worker: str = None) -> TaskEntity:
    """タスクエンティティ作成ヘルパー"""
    entity = TaskEntity(
        id=str(uuid.uuid4()),
        title=title,
