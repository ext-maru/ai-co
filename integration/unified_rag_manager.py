...
try:
    ...
    from integration.unified_entity_manager import (
        UnifiedEntityManager, BaseEntity, KnowledgeEntity, 
        IncidentEntity, TaskEntity
    )
except ImportError:
    # フォールバック: 相対インポート
...
class SearchQuery:
    ...
    intent: Optional[str] = None  # 'problem_solving', 'knowledge_acquisition', 'history_lookup'
...
class SearchPreprocessor:
    ...
    def process(self, query: SearchQuery) -> SearchQuery:
        ...
        # フィルタ調整
        processed_query.filters = self._adjust_filters(processed_query.filters, processed_query.intent)
    ...
    def _adjust_filters(self, filters: Optional[Dict[str, Any]], intent: str) -> Dict[str, Any]:
        """意図に基づくフィルタ調整"""
        if filters is None:
            filters = {}
...
class ContextAssembler:
    ...
    def assemble(self, 
        ...
        if query_intent == 'problem_solving':
            ...
            ordered_results = self._order_for_problem_solving(primary_results, related_entities)
        elif query_intent == 'knowledge_acquisition':
            # 知識獲得: 知識 → 例 → 関連情報
            ordered_results = self._order_for_knowledge_acquisition(primary_results, related_entities)
        else:
            # 一般: 関連度順
    ...
    def _order_for_problem_solving(self, primary: List[BaseEntity], related: List[BaseEntity]) -> List[BaseEntity]:
        """問題解決向けの順序付け"""
        # インシデント → 解決策となる知識 → その他
        ...
        others = [e for e in primary + related if not isinstance(e, (IncidentEntity, KnowledgeEntity))]
        
        return incidents + knowledge + others
    
    def _order_for_knowledge_acquisition(self, primary: List[BaseEntity], related: List[BaseEntity]) -> List[BaseEntity]:
        """知識獲得向けの順序付け"""
        # 信頼度の高い知識 → その他
        ...
        knowledge_entities.sort(
            key=lambda e: e.knowledge_data.get('confidence_score', 0.0),
            reverse=True
        )
        
        return knowledge_entities + other_entities
    
    def _format_entity(self, entity: BaseEntity, relationships: List[Dict[str, Any]]) -> str:
        """エンティティのフォーマット"""
        ...
        if isinstance(entity, KnowledgeEntity):
            formatted += f"**信頼度**: {entity.knowledge_data.get('confidence_score', 0.0):.2f}\n"
            ...
            formatted += f"**進捗**: {entity.task_data.get('completion_percentage', 0)}%\n"
...
class UnifiedRAGManager:
    ...
    async def search(self, query: Union[str, SearchQuery]) -> SearchResult:
        ...
        try:
            ...
            if processed_query.include_relationships and primary_results:
                related_entities, relationships = await self._execute_relationship_search(
            ...
            result = SearchResult(
                ...
                suggestions=self._generate_suggestions(processed_query, primary_results)
            )
            ...
            logger.info(f"Search completed: {len(primary_results)} primary, {len(related_entities)} related")
            return result
    ...
    async def _execute_relationship_search(self, 
                                         primary_results: List[BaseEntity],
                                         query: SearchQuery) -> Tuple[List[BaseEntity], List[Dict[str, Any]]]:
        """関係性検索実行"""
        try:
            related_entities = []
            all_relationships = []
            ...
            for entity in primary_results:
                ...
                entity_related = self.entity_manager.find_related_entities(
                    entity.id,
                    max_depth=query.max_relationship_depth
                )
    ...
    def _apply_filters(self, entities: List[BaseEntity], filters: Dict[str, Any]) -> List[BaseEntity]:
        ...
        for filter_key, filter_value in filters.items():
            if filter_key == 'status':
                filtered = [e for e in filtered if e.metadata.get('status') == filter_value]
            elif filter_key == 'priority':
                if isinstance(filter_value, list):
                    filtered = [e for e in filtered if e.metadata.get('priority') in filter_value]
                else:
                    filtered = [e for e in filtered if e.metadata.get('priority') == filter_value]
            elif filter_key == 'category':
                filtered = [e for e in filtered if e.metadata.get('category') == filter_value]
            elif filter_key == 'tags':
                if isinstance(filter_value, list):
                    filtered = [e for e in filtered 
                              if any(tag in e.metadata.get('tags', []) for tag in filter_value)]
                else:
                    filtered = [e for e in filtered if filter_value in e.metadata.get('tags', [])]
    ...
    def _calculate_confidence_scores(self, 
                                   primary_results: List[BaseEntity],
                                   related_entities: List[BaseEntity],
                                   query: SearchQuery) -> Dict[str, float]:
        """信頼度スコア計算"""
        scores = {}
    ...
    def _generate_suggestions(self, query: SearchQuery, results: List[BaseEntity]) -> List[str]:
        """検索提案生成"""
        suggestions = []
