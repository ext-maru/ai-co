...
class CommonFixes:
    ...
    def _flush_dns_cache(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(['sudo', 'systemctl', 'flush-dns'], 
    ...
    def _fix_directory_permissions(self) -> bool:
        ...
        try:
            ...
            for dir_name in important_dirs:
                ...
                if dir_path.exists():
                    subprocess.run(['chmod', '-R', '755', str(dir_path)], 
    ...
    def _fix_executable_permissions(self) -> bool:
        ...
        try:
            ...
            if scripts_dir.exists():
                subprocess.run(['chmod', '+x', str(scripts_dir / '*')], 
    ...
    def _reinstall_requirements(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(cmd, shell=True, capture_output=True, 
                                  text=True, timeout=300, cwd=str(self.ai_co_path))
    ...
    def _restart_workers(self) -> bool:
        ...
        try:
            ...
            stop_result = subprocess.run([str(self.ai_co_path / 'commands' / 'ai_stop.py')], 
                                       capture_output=True, timeout=30)
            time.sleep(2)
            
            start_result = subprocess.run([str(self.ai_co_path / 'commands' / 'ai_start.py')], 
                                        capture_output=True, timeout=30)
    ...
    def _reset_rabbitmq_queues(self) -> bool:
        ...
        try:
            ...
            for queue in queues:
                subprocess.run(['sudo', 'rabbitmqctl', 'purge_queue', queue], 
                              capture_output=True, timeout=10)
            return True
        except:
            return False
    ...
    def _fix_rabbitmq_permissions(self) -> bool:
        ...
        try:
            # RabbitMQユーザー権限確認
            result = subprocess.run(['sudo', 'rabbitmqctl', 'list_users'], 
                                  capture_output=True, timeout=10)
            return True  # 情報収集のみ
        except:
            return False
    ...
    def get_fix_statistics(self) -> Dict:
        ...
        successful_fixes = sum(1 for fix in self.fix_history 
                             if fix['result']['status'] == 'resolved')
        
        return {
            'total_fixes': total_fixes,
            'successful_fixes': successful_fixes,
            'success_rate': successful_fixes / total_fixes if total_fixes > 0 else 0,
            'average_resolution_time': sum(fix['result']['resolution_time'] 
                                         for fix in self.fix_history) / total_fixes if total_fixes > 0 else 0
        }