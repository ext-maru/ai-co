...
class ServiceHealer:
    ...
    async def sage_council_healing(self, incident_data: Dict) -> Dict:
        ...
        self.logger.info(
            f"🧙‍♂️ Starting sage council for incident: {incident_data.get('incident_id')}"
        )
    ...
    async def _consult_knowledge_sage(self, incident_data: Dict) -> Dict:
        ...
        try:
            ...
            if incident_kb_path.exists():
                ...
                # 簡易類似性検索
                error_keywords = set(
                    incident_data.get("description", "").lower().split()
                )
            ...
            return {
                "similar_incidents": sorted(
                    similar_incidents, key=lambda x: x["similarity_score"], reverse=True
                )[:3],
    ...
    async def _consult_rag_mystic(self, incident_data: Dict) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "external_references": await self._find_external_references(
                    incident_data
                ),
    ...
    async def _create_healing_plan(
        self, incident_data: Dict, sage_inputs: Dict
    ) -> Dict:
        """統合治癒計画立案"""
        plan = {
            "strategy": "unknown",
        ...
        # 各賢者の提案を統合
        knowledge_confidence = sage_inputs.get("knowledge", {}).get(
            "knowledge_confidence", 0
        )
        crisis_confidence = (
            sage_inputs.get("crisis", {}).get("confidence_level") == "high"
        )
        ...
        plan["estimated_duration"] = sum(
            phase.get("duration", 5) for phase in plan["phases"]
        )
    ...
    async def _execute_healing_plan(self, healing_plan: Dict) -> Dict:
        ...
        execution_result["overall_success"] = (
            successful_phases / total_phases >= 0.7 if total_phases > 0 else False
        )
    ...
    async def _evaluate_and_learn(self, healing_session: Dict) -> str:
        ...
        try:
            ...
            if execution_result["overall_success"]:
                ...
                # 成功パターンを学習
                pattern_key = (
                    f"{incident_data.get('category')}_{incident_data.get('priority')}"
                )
    ...
    def _estimate_healing_effort(self, incident_data: Dict) -> int:
        ...
        priority_multiplier = {"critical": 2, "high": 1.5, "medium": 1, "low": 0.8}.get(
            priority, 1
        )
    ...
    def _create_escalation_phases(
        ...
        phases = [
            ...
            {
                "name": "manual_intervention_preparation",
                "duration": 5,
                "critical": False,
            },
        ]
