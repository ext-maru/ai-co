...
class WorkerRestart:
    ...
    def _generate_recommendations(self, diagnosis: Dict) -> List[str]:
        ...
        stopped_workers = [
            name
            for name, status in diagnosis["worker_status"].items()
            if not status["is_running"]
        ]

        if stopped_workers:
            critical_stopped = [
                name for name in stopped_workers if self.worker_types[name]["critical"]
            ]
            if critical_stopped:
                recommendations.append(
                    f"Critical workers stopped: {', '.join(critical_stopped)} - Immediate restart required"
                )

            recommendations.append(
                f"Restart stopped workers: {', '.join(stopped_workers)}"
            )
        ...
        if high_load_workers:
            recommendations.append(
                f"High resource usage workers: {', '.join(high_load_workers)} - Consider restart"
            )
        ...
        if system_health.get("cpu_percent", 0) > 90:
            recommendations.append(
                "High system CPU usage - Check for runaway processes"
            )

        if system_health.get("memory_percent", 0) > 90:
            recommendations.append(
                "High system memory usage - Memory cleanup recommended"
            )
        ...
        try:
            ...
            if success:
                ...
                if new_status["is_running"]:
                    ...
                    restart_result["new_pid"] = (
                        new_status["pids"][0] if new_status["pids"] else None
                    )
                else:
    ...
    def restart_all_workers(
        self, only_critical: bool = False, force: bool = False
    ) -> Dict:
        """全ワーカー再起動"""
        restart_all_result = {
            "start_time": datetime.now().isoformat(),
        ...
        for worker_name in workers_to_restart:
            ...
            if restart_result["success"]:
                ...
                restart_all_result["failed_restarts"].append(
                    {"worker": worker_name, "error": restart_result["error"]}
                )
                self.logger.error(
                    f"❌ {worker_name} restart failed: {restart_result['error']}"
                )
    ...
    def emergency_recovery(self) -> Dict:
        ...
        if diagnosis["issues_found"]:
            ...
            recovery_result["recovery_actions"].append(
                {"action": "critical_worker_restart", "result": restart_result}
            )
        ...
        if system_health.get("memory_percent", 0) > 95:
            # メモリクリーンアップ
            recovery_result["recovery_actions"].append(
                {"action": "memory_cleanup", "result": self._emergency_memory_cleanup()}
            )
        ...
        if not system_health.get("rabbitmq_available", True):
            ...
            recovery_result["recovery_actions"].append(
                {"action": "rabbitmq_recovery", "result": rabbitmq_result}
            )
        ...
        critical_workers_running = all(
            status["is_running"]
            for name, status in final_diagnosis["worker_status"].items()
    ...
    def get_restart_statistics(self) -> Dict:
        ...
        successful_restarts = sum(
            1 for restart in self.restart_history if restart["success"]
        )
        ...
        return {
            ...
            "most_problematic_worker": max(
                worker_restart_counts.items(), key=lambda x: x[1]
            )[0]
            if worker_restart_counts
            else None,
        }
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["diagnose", "restart", "restart_all", "emergency", "stats"],
        help="Action to perform",
    )
    parser.add_argument("--worker", help="Specific worker name for restart action")
    parser.add_argument(
        "--force", action="store_true", help="Force restart (kill instead of terminate)"
    )
    parser.add_argument(
        "--critical-only", action="store_true", help="Only restart critical workers"
    )
