...
class ErrorIncidentBridge:
    ...
    def _determine_priority(self, error: Dict) -> str:
        ...
        # High: 重要な機能のエラー
        if any(word in error_type for word in ['connection', 'permission', 'authentication']):
            return 'high'
    ...
    def check_error_incident_consistency(self):
        ...
        if self.error_history_file.exists():
            ...
            for error in error_history.get("error_history", []):
                if "incident_id" in error:
                    # インシデントが存在するか確認
                    incident = self.incident_manager.get_incident_by_id(error["incident_id"])
                    if not incident:
...
if __name__ == "__main__":
    ...
    parser.add_argument("action", choices=["migrate", "report", "check"],
                      help="実行するアクション")
