...
# インシデントカテゴリとステータスの定義
IncidentCategory = Literal["error", "failure", "request", "change", "security", "performance"]
...
class IncidentManager:
    ...
    def load_history(self):
        ...
        if self.history_file.exists():
            ...
            self.history = {
                ...
                "incidents": [],
                "category_statistics": {cat: {"count": 0, "open": 0, "avg_resolution_time": None} 
                                      for cat in ["error", "failure", "request", "change", "security", "performance"]},
                "priority_statistics": {pri: {"count": 0, "open": 0, "avg_resolution_time": None}
    ...
    def create_incident(self, 
                       ...
                       impact: str,
                       assignee: str = "ai_system") -> str:
        """新規インシデントを作成"""
        # インシデントID生成
        date_part = datetime.now().strftime('%Y%m%d')
        count = len([i for i in self.history["incidents"] if i["incident_id"].startswith(f"INC-{date_part}")]) + 1
        ...
        for incident in self.history["incidents"]:
            if incident["incident_id"] == incident_id:
                ...
                if "status" in updates:
                    ...
                    if old_status in ["open", "in_progress"] and new_status in ["resolved", "closed"]:
                        self.history["metadata"]["open_incidents"] -= 1
                        self.history["metadata"]["resolved_incidents"] += 1
                        self.history["category_statistics"][incident["category"]]["open"] -= 1
                        self.history["priority_statistics"][incident["priority"]]["open"] -= 1
    ...
    def resolve_incident(self, 
                        ...
                        knowledge_updates: List[str] = None) -> bool:
        """インシデントを解決"""
        resolution = {
        ...
        for incident in self.history["incidents"]:
            if incident["incident_id"] == incident_id:
                created_time = datetime.fromisoformat(incident["timestamp"].replace('Z', '+00:00'))
                resolved_time = datetime.now(timezone.utc)
                resolution_time = (resolved_time - created_time).total_seconds() / 3600  # 時間単位
                
                # 統計更新
                self._update_resolution_statistics(incident["category"], incident["priority"], resolution_time)
    ...
    def get_open_incidents(self, category: Optional[IncidentCategory] = None) -> List[Dict]:
        """オープンなインシデントを取得"""
        incidents = [i for i in self.history["incidents"] if i["status"] in ["open", "in_progress"]]
        if category:
    ...
    def analyze_patterns(self) -> Dict:
        ...
        # カテゴリ別の頻度
        category_counts = {cat: data["count"] for cat, data in self.history["category_statistics"].items()}
        if category_counts:
            analysis["most_common_category"] = max(category_counts, key=category_counts.get)
        ...
        analysis["recurring_issues"] = [(title, count) for title, count in title_counts.items() if count > 1]
        
        # 平均解決時間
        for cat, data in self.history["category_statistics"].items():
            if data["avg_resolution_time"]:
                analysis["avg_resolution_times"][cat] = f"{data['avg_resolution_time']:.1f}時間"
    ...
    def generate_report(self) -> str:
        ...
        report.append("|----------|------|----------|--------------|")
        for cat, data in self.history["category_statistics"].items():
            avg_time = f"{data['avg_resolution_time']:.1f}h" if data['avg_resolution_time'] else "N/A"
    ...
    def _update_resolution_statistics(self, category: str, priority: str, resolution_time: float):
        """解決時間統計を更新"""
        # カテゴリ統計
        ...
        if cat_stats["avg_resolution_time"] is None:
            ...
            cat_stats["avg_resolution_time"] = (
                (cat_stats["avg_resolution_time"] * (count - 1) + resolution_time) / count
            )
        ...
        if pri_stats["avg_resolution_time"] is None:
            ...
            pri_stats["avg_resolution_time"] = (
                (pri_stats["avg_resolution_time"] * (count - 1) + resolution_time) / count
            )
...
if __name__ == "__main__":
    ...
    if args.action == "create":
        if not all([args.category, args.priority, args.title, args.description, args.components, args.impact]):
            print("エラー: create には --category, --priority, --title, --description, --components, --impact が必要です")
            sys.exit(1)
        ...
        if incidents:
            ...
            for inc in incidents:
                print(f"ID: {inc['incident_id']} | {inc['priority'].upper()} | {inc['category']}")
                print(f"   {inc['title']}")
