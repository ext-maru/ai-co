...
class ComprehensiveKnowledgeMigrator:
    ...
    def analyze_knowledge_base(self) -> Dict[str, Any]:
        ...
        logger.info(f"📊 Analysis complete: {self.stats.total_files} files, {self.stats.total_size/1024/1024:.2f}MB")
        return analysis
    ...
    def _categorize_file(self, file_path: Path) -> str:
        ...
        elif 'learning' in path_str or 'feedback' in path_str or 'evolution' in path_str:
            return 'ai_learning'
    ...
    def _is_special_file(self, file_path: Path) -> bool:
        """特殊ファイル判定"""
        return (
            file_path.is_symlink() or
            file_path.name.startswith('.') or
            file_path.suffix in ['.py', '.sh'] or
            file_path.stat().st_size > 100000 or  # 100KB以上
            'postgresql' in file_path.name.lower() or
            any(lang in file_path.name for lang in ['japanese', '日本語'])
        )
    ...
    async def migrate_all_knowledge(self, dry_run: bool = False) -> Dict[str, Any]:
        """全ナレッジの完全移行"""
        logger.info(f"🚀 Starting {'DRY RUN' if dry_run else 'LIVE'} complete knowledge migration...")
        ...
        logger.info(f"✅ Migration complete: {self.stats.successful_migrations}/{self.stats.total_files} files")
        return summary
    
    async def _migrate_category(self, category: str, files: List[str], dry_run: bool) -> Dict[str, Any]:
        """カテゴリ別移行"""
        results = {
            'category': category,
        ...
        for file_path_str in files:
            ...
            try:
                ...
                # 移行実行
                if not dry_run:
                    spell_id = await self._migrate_single_file(file_path, content, category)
    ...
    async def _migrate_single_file(self, file_path: Path, content: str, category: str) -> str:
        """単一ファイル移行"""
        # メタデータ準備
        metadata = {
    ...
    def _determine_magic_school(self, file_path: Path, content: str) -> MagicSchool:
        ...
        if 'incident' in path_str or 'error' in content_lower or 'crisis' in content_lower:
            return MagicSchool.CRISIS_SAGE
        elif 'task' in path_str or 'project' in content_lower or 'workflow' in content_lower:
    ...
    def _generate_tags(self, file_path: Path, content: str) -> List[str]:
        ...
        content_lower = content.lower()
        tag_keywords = [
            'claude', 'elder', 'council', 'sage', 'incident', 'task', 'rag',
            'postgresql', 'migration', 'test', 'error', 'system', 'ai'
        ]
    ...
    def _generate_migration_summary(self, analysis: Dict[str, Any], results: Dict[str, Any]) -> Dict[str, Any]:
        """移行サマリー生成"""
        duration = (self.stats.end_time - self.stats.start_time).total_seconds()
        
        return {
            'migration_id': hashlib.md5(f"{self.stats.start_time}".encode()).hexdigest()[:16],
    ...
    async def verify_migration(self) -> Dict[str, Any]:
        ...
        original_md_files = [f for f in original_files if f.suffix in ['.md', '.json'] and f.is_file()]
        
        verification = {
            'original_files_count': len(original_md_files),
            'migrated_spells_count': len(grimoire_spells),
            'coverage_percentage': (len(grimoire_spells) / len(original_md_files) * 100) if original_md_files else 0,
            'missing_files': [],
            'verification_passed': False
        }
        
        # 不足ファイル検出
        migrated_paths = {spell.get('metadata', {}).get('original_path') for spell in grimoire_spells}
        ...
        logger.info(f"✅ Verification complete: {verification['coverage_percentage']:.1f}% coverage")
        return verification
