...
from libs.env_config import get_config, config
...
class A2AError(Exception):
    """A2A通信エラー"""
    def __init__(self, code: A2AErrorCode, message: str, details: Dict[str, Any] = None):
        self.code = code
    ...
    def validate_message(message: A2AMessage) -> bool:
        ...
        try:
            ...
            if not message.header.message_id:
                raise A2AError(A2AErrorCode.MISSING_REQUIRED_FIELD, "message_id is required")
            
            if not message.header.source or not message.header.target:
                raise A2AError(A2AErrorCode.MISSING_REQUIRED_FIELD, "source and target are required")
            
            if not message.payload.method:
                raise A2AError(A2AErrorCode.MISSING_REQUIRED_FIELD, "method is required")
            
            # バージョンチェック
            if message.header.version != "1.0":
                raise A2AError(A2AErrorCode.UNSUPPORTED_VERSION, f"Unsupported version: {message.header.version}")
            ...
            raise A2AError(A2AErrorCode.INVALID_MESSAGE_FORMAT, f"Invalid message format: {str(e)}")
...
class A2AClient:
    ...
    def __init__(self, agent_info: AgentInfo, config: Dict[str, Any] = None):
        ...
        self.security_manager = SecurityManager(self.config.get('A2A_SECRET_KEY', 'default-secret'))
    ...
    async def connect(self):
        ...
        try:
            # RabbitMQ接続
            rabbitmq_url = self.config.get('RABBITMQ_URL', 'amqp://guest:guest@localhost/')
            ...
            raise A2AError(A2AErrorCode.SERVICE_UNAVAILABLE, f"Connection failed: {str(e)}")
    ...
    async def _setup_agent_queue(self):
        """エージェント専用キューを設定"""
        queue_name = f"a2a.{self.agent_info.agent_type.value}.{self.agent_info.agent_id}"
    ...
    async def _handle_incoming_message(self, message: aio_pika.IncomingMessage):
        ...
        try:
            ...
            if 'a2a_message' in locals():
                await self._send_error_response(a2a_message, 
        ...
        try:
            ...
            if wait_for_response:
                try:
                    ...
                    raise A2AError(A2AErrorCode.DELIVERY_TIMEOUT, 
...
async def create_a2a_client(agent_id: str) -> A2AClient:
    ...
    if agent_id not in AGENT_REGISTRY:
        raise A2AError(A2AErrorCode.AGENT_NOT_FOUND, f"Agent {agent_id} not found in registry")
...
if __name__ == "__main__":
    ...
    async def test_a2a():
        try:
            ...
            task_client.register_handler(MessageType.KNOWLEDGE_QUERY, handle_knowledge_query)
