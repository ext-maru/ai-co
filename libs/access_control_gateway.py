...
from libs.security_level_enforcer import (
    SecurityLevelEnforcer, SecurityContext, AccessRequest, AccessResult, 
    PermissionLevel, DataClassification, AccessViolationType
)
from libs.resource_isolation_manager import (
    ResourceIsolationManager, IsolationContext, ResourceQuota, ResourceUsage, 
    ResourceAlert, ResourceType
)
...
class AccessControlGateway:
    ...
    def __init__(self):
        ...
        self.logger.info("🚪 AccessControlGateway initialized with integrated security systems")
    ...
    def _initialize_default_policies(self):
        ...
        default_policies = [
            ...
            AccessPolicy(
                ...
                security_levels=[SecurityLevel.SANDBOX, SecurityLevel.RESTRICTED, SecurityLevel.DEVELOPMENT],
                conditions={
                    "protected_classifications": ["CONFIDENTIAL", "RESTRICTED", "TOP_SECRET"],
            ...
            AccessPolicy(
                ...
                policy_type=PolicyType.ACCESS_CONTROL,
                security_levels=[SecurityLevel.SANDBOX, SecurityLevel.RESTRICTED, SecurityLevel.DEVELOPMENT, SecurityLevel.TRUSTED],
                conditions={
    ...
    async def process_access_request(self, user_id: str, resource_id: str, action: str,
            ...
            try:
                ...
                response = GatewayResponse(
                    ...
                    audit_trail={
                        ...
                        'resource_context_id': resource_context.context_id if resource_context else None,
                        'policy_evaluations': len(policy_results),
                        'access_result': access_result.granted if access_result else None,
    ...
    async def _get_or_create_security_context(self, user_id: str, 
    ...
    async def _get_or_create_resource_context(self, user_id: str, 
                                             security_level: SecurityLevel) -> Optional[IsolationContext]:
        """リソースコンテキスト取得または作成"""
        ...
        with self.session_lock:
            ...
            if security_level in [SecurityLevel.SANDBOX, SecurityLevel.RESTRICTED]:
                ...
                # セッション更新
                if session_key in self.integrated_sessions:
                    self.integrated_sessions[session_key]['resource_context'] = resource_context
                
                return resource_context
            
            return None
    
    async def _evaluate_policies(self, request: GatewayRequest) -> List[PolicyEvaluationResult]:
        """ポリシー評価"""
        ...
        for policy in sorted_policies:
            ...
            if result.matched and policy.action == PolicyAction.DENY and policy.priority >= 90:
                break
        
        return results
    
    def _evaluate_policy_conditions(self, policy: AccessPolicy, 
                                   request: GatewayRequest) -> Dict[str, Any]:
        """ポリシー条件評価"""
        ...
        for condition_key, condition_value in policy.conditions.items():
            ...
            if condition_key == "max_risk_score":
                ...
                if not met:
                    matched = False
                    reasons.append(f"Risk score {current_risk} exceeds limit {condition_value}")
            
            elif condition_key == "resource_threshold":
                # リソース閾値条件
                if request.resource_context:
                    usage = self.resource_manager.get_resource_usage(request.resource_context.context_id)
                    if usage:
                        cpu_usage = usage.cpu_percent / request.resource_context.resource_quota.cpu_percent
                        met = cpu_usage <= condition_value
                        conditions_met[condition_key] = met
                        if not met:
                            matched = False
                            reasons.append(f"Resource usage {cpu_usage:.2f} exceeds threshold {condition_value}")
                    else:
                ...
                # データ分類保護条件
                data_class = request.request_metadata.get('data_classification', 'PUBLIC')
                met = data_class not in condition_value
                conditions_met[condition_key] = met
                if not met:
                    matched = False
                    reasons.append(f"Data classification {data_class} requires protection")
            
            elif condition_key == "cross_level_access":
                # クロスレベルアクセス条件
                resource_level = request.request_metadata.get('target_security_level')
                if resource_level:
                    met = resource_level == request.security_context.security_level.value
                ...
                if not met:
                    matched = False
                    reasons.append(f"Threat level {current_threat} does not match {condition_value}")
    ...
    async def _check_access_control(self, request: GatewayRequest) -> Optional[AccessResult]:
        """アクセス制御チェック"""
    ...
    def _make_final_decision(self, request: GatewayRequest,
        ...
        for result in policy_results:
            if result.matched:
                if result.action == PolicyAction.DENY:
                    ...
                    decision["restrictions"].append(f"Policy {result.policy_id}: {result.reason}")
                    return decision  # 拒否は即座に返す
                
                elif result.action == PolicyAction.RESTRICT:
                    decision["restrictions"].append(f"Restricted by policy {result.policy_id}")
                
                elif result.action == PolicyAction.REQUIRE_APPROVAL:
                    decision["granted"] = False
                    decision["action"] = "require_approval"
                    decision["restrictions"].append(f"Manual approval required: {result.policy_id}")
    ...
    async def _record_audit_log(self, request: GatewayRequest, response: GatewayResponse,
                               policy_results: List[PolicyEvaluationResult]):
        """監査ログ記録"""
    ...
    def _run_compliance_check(self):
        ...
        try:
            ...
            violations = len([log for log in recent_logs if log['compliance_status'] == 'violation'])
            
            if total_recent > 0:
                violation_rate = violations / total_recent
                if violation_rate > 0.1:  # 10%以上の違反率
                    self.logger.warning(f"⚠️ High compliance violation rate: {violation_rate:.1%}")
...
if __name__ == "__main__":
    ...
    async def test_gateway():
        ...
        try:
            ...
            print(f"   Average Response Time: {status['metrics']['average_response_time_ms']:.1f}ms")
