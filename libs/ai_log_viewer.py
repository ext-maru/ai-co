...
class AILogViewer:
    ...
    def get_latest_command_logs(self, limit=5):
        ...
        if self.cmd_logs.exists():
            log_files = sorted(self.cmd_logs.glob("*.log"), 
    ...
    def get_latest_program_logs(self, limit=5):
        ...
        if self.program_logs.exists():
            log_files = sorted(self.program_logs.glob("*.log"), 
                             key=lambda x: x.stat().st_mtime, 
                             reverse=True)[:limit]
    ...
    def get_execution_summary(self):
        """実行サマリーを取得"""
        summary = {
            'command_logs': len(list(self.cmd_logs.glob("*.log"))) if self.cmd_logs.exists() else 0,
            'program_logs': len(list(self.program_logs.glob("*.log"))) if self.program_logs.exists() else 0,
