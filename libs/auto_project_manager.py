#!/usr/bin/env python3
"""
AI Company Automated Project Manager with Docker Isolation and GitHub Integration
...
from libs.project_risk_analyzer import ProjectRiskAnalyzer, RiskLevel as AnalyzerRiskLevel
from libs.shared_enums import SecurityLevel, ProjectType
...
class AutoProjectManager:
    ...
    def _initialize_workspace(self):
        ...
        workspace_dirs = [
            ...
            self.workspace_root / ".ai_company"   # Management metadata
        ]
    ...
    def _create_security_config(self):
        ...
        config_file = self.workspace_root / ".ai_company" / "security_config.json"
...
""")
    ...
    def analyze_project_risk(self, project_content: str, requirements: Dict) -> ProjectRisk:
        ...
        risk_factors = [f"{factor.description} (severity: {factor.severity:.1f})" 
    ...
    def create_project(self, project_name: str, requirements: Dict, 
                      security_override: Optional[SecurityLevel] = None) -> Tuple[Path, ProjectRisk]:
        ...
        self._create_docker_environment(project_path, safe_name, security_level, requirements)
    ...
    def _create_project_structure(self, project_path: Path, requirements: Dict, 
                                 security_level: SecurityLevel):
        """Create standardized project directory structure"""
        
        # Create base directories
        directories = [
            "src", "tests", "docs", "config", "scripts", "data", "logs",
            ".ai_company", ".docker"
        ]
        ...
        metadata_file = project_path / ".ai_company" / "project.json"
    ...
    def _detect_project_type(self, requirements: Dict) -> str:
        ...
        if any(term in content for term in ['ml', 'machine learning', 'tensorflow', 'pytorch', 'jupyter']):
            if any(term in content for term in ['research', 'experiment', 'notebook']):
                return 'ml_research'
            else:
                return 'ml_production'
        
        elif any(term in content for term in ['web', 'api', 'fastapi', 'flask', 'django']):
            if any(term in content for term in ['frontend', 'react', 'vue', 'fullstack']):
                return 'fullstack'
            else:
                return 'web_api'
        ...
        elif any(term in content for term in ['microservice', 'service', 'distributed']):
            return 'microservices'
        
        elif any(term in content for term in ['data', 'analytics', 'pandas', 'analysis']):
            return 'data_science'
        
        else:
            return 'web_api'  # Default fallback
    
    def _create_docker_environment(self, project_path: Path, project_name: str, 
                                  security_level: SecurityLevel, requirements: Dict = None):
        ...
        docker_template = self.docker_manager.get_template_for_project(project_type, security_level)
        ...
        if docker_template:
            ...
            # Log template usage
            self._log_docker_template_usage(project_name, project_type, docker_template, created_files)
        else:
            # Fallback to basic Docker setup
            self._create_basic_docker_environment(project_path, project_name, security_level)
    
    def _create_basic_docker_environment(self, project_path: Path, project_name: str, 
                                        security_level: SecurityLevel):
        ...
        source_dockerfile = (self.workspace_root / "docker" / 
                           f"Dockerfile.{security_level.value}")
        ...
        compose_template = self.workspace_root / "docker" / "docker-compose.template.yml"
...
# AI Company Project Start Script
...
# AI Company Project Stop Script
...
""")
    ...
    def _log_docker_template_usage(self, project_name: str, project_type: str, 
                                  template, created_files: Dict):
    ...
    def _create_readme(self, project_path: Path, requirements: Dict, 
                      security_level: SecurityLevel):
        """Create project README with security information"""
        
        readme_content = f"""# {project_path.name}

AI Company Auto-Generated Project
...
├── data/                   # Data files
├── logs/                   # Log files
├── .ai_company/           # AI Company metadata
...
This project is managed by AI Company's automated development system:

```bash
# Submit tasks to AI Company
...
1. All development must follow TDD (Test-Driven Development)
2. Use the provided Docker environment for consistency
3. Security policies are enforced automatically
4. Submit changes through AI Company workflow

---
Generated by AI Company - Automated Development System  
...
"""
    ...
    def _log_project_creation(self, project_path: Path, risk_assessment: ProjectRisk, 
                             security_level: SecurityLevel):
            ...
            if level_dir.exists():
                for project_dir in level_dir.iterdir():
                    if project_dir.is_dir():
                        metadata_file = project_dir / ".ai_company" / "project.json"
        ...
        for security_level in SecurityLevel:
            ...
            if project_path.exists():
                ...
                if metadata_file.exists():
                    ...
                    # Check Docker status
                    docker_running = self._check_docker_status(project_name, security_level.value)
    ...
    def _check_docker_status(self, project_name: str, security_level: str) -> bool:
        ...
        try:
            result = subprocess.run(
                ["docker", "ps", "--filter", f"name={project_name}_{security_level}", "--format", "{{.Names}}"],
...
if __name__ == "__main__":
    ...
    try:
        project_path, risk = manager.create_project("test_auto_project", test_requirements)
