...
class HybridWorkerOptimizer:
    ...
    async def hybrid_optimization(self, items: List[Any], 
                                threshold: int = 100) -> OptimizationResult:
        """ハイブリッド最適化
...
def orchestrate_hybrid_job(job_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = loop.run_until_complete(
        optimizer.hybrid_optimization(items, threshold)
    )
        ...
        for method in methods:
            ...
            for i in range(iterations):
                ...
                if method == 'celery':
                    # Celeryのみ
                    optimizer = HybridWorkerOptimizer()
                    result = optimizer.optimize_with_celery(items, 'process_small_batch')
                elif method == 'ray':
                    # Rayのみ
                    optimizer = HybridWorkerOptimizer()
