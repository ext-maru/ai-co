...
class ClaudeElderChatAPI:
    ...
    def __init__(self):
        ...
        self.elder_council = (
            ElderCouncilAutoDecision() if ElderCouncilAutoDecision else None
        )
        self.servant_dispatcher = (
            ElderServantDispatcher() if ElderServantDispatcher else None
        )
    ...
    async def _deploy_servant(self, servant_type: str, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await asyncio.to_thread(
                self.servant_dispatcher.deploy_servant, servant_type, message
            )
    ...
    async def _query_elder_wisdom(self, query: str) -> Dict[str, Any]:
        ...
        try:
            # RAG Elder Wizards使用
            result = await asyncio.to_thread(self.rag_wizards.search_knowledge, query)
    ...
    async def _summon_elder_council(self, topic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await asyncio.to_thread(
                self.elder_council.deliberate_decision, topic
            )
...
class ClaudeElderWebSocketServer:
                ...
                try:
                    ...
                    if data.get("type") == "chat":
                        ...
                        response = await self.chat_api.process_chat_message(
                            data.get("message", ""), data.get("user_id", "anonymous")
                        )
                        ...
                        await websocket.send(
                            json.dumps({"type": "history_response", "data": history})
                        )
    ...
    async def start_server(self):
        ...
        try:
            ...
            self.logger.info(
                f"Starting Claude Elder WebSocket server on {self.host}:{self.port}"
            )
