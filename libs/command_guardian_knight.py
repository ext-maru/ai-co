...
class CommandGuardianKnight(IncidentKnight):
    ...
    def __init__(self, knight_id: str = "command_guardian_001"):
        ...
        self.protected_commands = {
            ...
            'ai-elder-council': {'path': 'commands/ai_elder_council.py', 'critical': False},
            'ai-worker-recovery': {'path': 'commands/ai_worker_recovery.py', 'critical': True},
            'ai-incident-knights': {'path': 'commands/ai_incident_knights.py', 'critical': False},
            ...
            try:
                ...
                if not cmd_exists:
                    issues.append(Issue(
                        ...
                        severity=IssueSeverity.CRITICAL if cmd_info['critical'] else IssueSeverity.HIGH,
                        title=f"Command {cmd_name} missing",
                        description=f"Protected command {cmd_name} is not available",
                        affected_component=cmd_name,
                        detected_at=datetime.now(),
                        metadata={'command': cmd_name, 'path': cmd_info.get('path')}
                    ))
                    continue
    ...
    async def _check_imports(self, tree: ast.AST, file_path: Path, content: str) -> List[Issue]:
                    ...
                    if req and not req.startswith('#'):
                        ...
                        if not await self._package_installed(package_name):
                            issues.append(Issue(
                                ...
                                detected_at=datetime.now(),
                                metadata={'package': package_name, 'requirement': req}
                            ))
