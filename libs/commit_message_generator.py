...
class CommitMessageGenerator:
    ...
    def generate_conventional_commit(self, analysis) -> str:
        ...
        # 本文とフッター
        body = self._build_body(files_changed, commit_type) if self.config.include_body else ""
        footer = self._build_footer(breaking_change, files_changed) if self.config.include_footer else ""
    ...
    def _build_subject(self, commit_type: str, scope: Optional[str],
                      description: str, breaking_change: bool) -> str:
        """サブジェクト行を構築"""
        # 絵文字プレフィックス
        emoji = ""
        ...
        # 説明文を適切な長さに調整
        max_desc_length = self.config.max_subject_length - len(emoji) - len(type_str) - len(scope_str) - 2
        ...
        if len(files_changed) > 5:
            ...
            for dir_name, files in dirs.items():
                if len(files) <= 3:
                    ...
                    lines.append(f"- {dir_name}/: {files[0]}, {files[1]} and {len(files)-2} more")
        else:
    ...
    def generate_smart_message(self, files: List[str], changes_summary: Dict[str, Any] = None) -> str:
        """ファイル変更から自動的にメッセージ生成"""
        if not files:
    ...
    def _analyze_files_for_commit(self, files: List[str], changes_summary: Dict[str, Any] = None) -> Dict[str, Any]:
        """ファイル変更を分析してコミット情報を推定"""
        file_categories = {
            'source': [], 'test': [], 'docs': [], 'config': [],
            'build': [], 'ci': [], 'other': []
        }
        ...
        for file in files:
            ...
            elif file_lower.endswith(('.yml', '.yaml')) and ('ci' in file_lower or '.github' in file):
    ...
    def suggest_message_improvements(self, message: str) -> Dict[str, Any]:
        ...
        if not re.match(r'^(feat|fix|docs|style|refactor|perf|test|build|ci|chore)(\(.+\))?: .+', first_line):
        ...
        # 動詞チェック（英語）
        if not re.search(r'\b(add|fix|update|remove|refactor|implement|enhance|improve)\b', first_line.lower()):
        ...
        return {
            ...
            'is_conventional': bool(re.match(r'^(feat|fix|docs|style|refactor|perf|test|build|ci|chore)', first_line))
        }
...
def generate_smart_message(files: List[str], changes_summary: Dict[str, Any] = None) -> str:
    """スマートメッセージ生成"""
...
def create_elders_guild_commit(commit_type: str, scope: str = None, description: str = "",
...
if __name__ == "__main__":
    ...
    # エルダーズギルド形式
    elders_msg = create_elders_guild_commit('feat', 'git', 'AI-driven automation system')
