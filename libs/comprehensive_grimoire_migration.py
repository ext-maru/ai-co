...
class ContentAnalyzer:
    ...
    def __init__(self):
        ...
        self.magic_school_keywords = {
            MagicSchool.KNOWLEDGE_SAGE: [
                'knowledge', 'learn', 'pattern', 'insight', 'wisdom', 'analysis',
                'study', 'research', 'understanding', 'concept', 'theory'
            ],
            MagicSchool.TASK_ORACLE: [
                'task', 'workflow', 'process', 'procedure', 'step', 'execution',
                'schedule', 'priority', 'planning', 'project', 'management'
            ],
            MagicSchool.CRISIS_SAGE: [
                'error', 'incident', 'problem', 'issue', 'bug', 'failure',
                'emergency', 'crisis', 'recovery', 'fix', 'troubleshoot', 'debug'
            ],
            MagicSchool.SEARCH_MYSTIC: [
                'search', 'query', 'retrieval', 'find', 'lookup', 'index',
                'semantic', 'context', 'similarity', 'match', 'relevance'
            ]
        }
        ...
        self.spell_type_keywords = {
            SpellType.KNOWLEDGE: [
                'guide', 'concept', 'theory', 'principle', 'overview', 'introduction'
            ],
            SpellType.PROCEDURE: [
                'how to', 'steps', 'procedure', 'process', 'workflow', 'method'
            ],
            SpellType.CONFIGURATION: [
                'config', 'setting', 'parameter', 'option', 'environment', 'setup'
            ],
            SpellType.TEMPLATE: [
                'template', 'example', 'sample', 'boilerplate', 'scaffold'
            ],
            SpellType.REFERENCE: [
                'reference', 'api', 'documentation', 'spec', 'manual', 'list'
            ]
        }
    ...
    def _infer_magic_school(self, content: str, path_obj: Path) -> MagicSchool:
        ...
        elif 'incident' in path_lower or 'error' in path_lower or 'bug' in path_lower:
    ...
    def _calculate_power_level(self, content: str, path_obj: Path) -> int:
        ...
        # 重要キーワード
        important_keywords = [
            'important', 'critical', 'essential', 'main', 'core', 'primary',
            'master', 'guide', 'overview', 'architecture', 'design'
        ]
        content_lower = content.lower()
    ...
    def _assess_technical_complexity(self, content: str) -> float:
        ...
        # 技術用語の密度
        technical_terms = [
            'algorithm', 'architecture', 'async', 'await', 'class', 'function',
            'implementation', 'interface', 'method', 'module', 'object',
            'parameter', 'protocol', 'schema', 'system', 'vector', 'database'
        ]
    ...
    def _assess_importance(self, content: str, path_obj: Path) -> float:
        ...
        if 'knowledge_base' in path_lower:
            ...
            score += 0.1
        elif any(x in path_lower for x in ['tmp', 'temp', 'backup', 'old']):
            score -= 0.2
        ...
        importance_indicators = [
            ...
            'architecture', 'design', 'specification', 'standard'
        ]
...
class ComprehensiveGrimoireMigration:
    ...
    async def analyze_files(self, file_paths: List[str]) -> List[FileAnalysis]:
        ...
        self.logger.info(f"Analysis complete: {len(analyses)} files, {duplicates} duplicates")
        
        return analyses
    
    def create_migration_batches(self, analyses: List[FileAnalysis]) -> List[MigrationBatch]:
        """移行バッチ作成"""
    ...
    async def migrate_batch(self, batch: MigrationBatch) -> MigrationResult:
        ...
        self.logger.info(f"Starting migration of batch {batch.batch_id} ({len(batch.files)} files)")
    ...
    async def execute_full_migration(self, root_path: str = ".") -> Dict[str, Any]:
        ...
        try:
            ...
            if high_priority_batches:
                self.logger.info(f"Migrating {len(high_priority_batches)} high priority batches...")
            ...
            if medium_priority_batches:
                self.logger.info(f"Migrating {len(medium_priority_batches)} medium priority batches...")
            ...
            if low_priority_batches:
                self.logger.info(f"Migrating {len(low_priority_batches)} low priority batches...")
            ...
            final_report = {
                ...
                "summary": {
                    ...
                    "average_files_per_second": total_successful / total_duration if total_duration > 0 else 0
                },
