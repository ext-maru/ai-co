...
class ConfigGuardianKnight(IncidentKnight):
    ...
    async def diagnose(self, issue: Issue) -> Diagnosis:
        ...
        diagnosis_data["auto_repair_possible"] = (
            len(diagnosis_data["available_sources"]) > 0
            or diagnosis_data["backup_available"]
        )
    ...
    async def _escalate_to_elder_council(
        self, issue: Issue, diagnosis: Diagnosis, resolution: Resolution = None
    ):
        """エルダー会議へのエスカレーション"""
        escalation_data = {
            "timestamp": datetime.now().isoformat(),
