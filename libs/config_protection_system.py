...
class ConfigFileProtectionHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        if not event.is_directory:
            filepath = Path(event.src_path)
            if self.protection_system.is_protected_file(filepath):
                asyncio.create_task(self.protection_system.handle_modification(filepath))
    ...
    def on_moved(self, event):
        if not event.is_directory:
            ...
            if self.protection_system.is_protected_file(src_path):
                asyncio.create_task(self.protection_system.handle_move(src_path, dest_path))
...
class ConfigProtectionSystem:
    ...
    def _create_snapshots(self):
        ...
        for rel_path in self.protected_files:
            ...
            if file_path.exists():
                ...
                logger.debug(f"📸 スナップショット作成: {rel_path}")
        ...
        for rel_path in self.protected_files:
            ...
            if file_path.exists():
                try:
                    ...
                    logger.debug(f"🔒 イミュータブル化: {rel_path}")
    ...
    def _cleanup_old_backups(self, keep: int = 10):
        ...
        backups = sorted(backup_parent.iterdir(), key=lambda x: x.stat().st_mtime, reverse=True)
        for backup in backups[keep:]:
            if backup.is_dir():
                shutil.rmtree(backup)
                logger.debug(f"🗑️ 古いバックアップ削除: {backup.name}")
    ...
    async def _restore_from_snapshot(self, filepath: Path):
        ...
        try:
            rel_path = filepath.relative_to(self.project_root)
            snapshot_path = self.protection_dir / "snapshots" / f"{str(rel_path).replace('/', '_')}.snapshot"
    ...
    def _create_change_backup(self, filepath: Path):
        ...
        try:
            ...
            logger.debug(f"📁 変更前バックアップ: {backup_name}")
    ...
    async def _send_alert(self, level: str, message: str):
        """アラート送信"""
        # Elder Councilに報告
        alert_file = Path("knowledge_base/elder_council_requests") / f"config_protection_alert_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
## 確認事項
1. ファイルが正常に復元されているか確認
2. 不正なアクセスがないか調査
3. 必要に応じて追加のセキュリティ対策を実施
