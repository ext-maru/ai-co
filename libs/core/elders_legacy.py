...
class EldersLegacyBase(Generic[TRequest, TResponse], ABC):
    ...
    def __init__(self, component_id: str, domain: EldersLegacyDomain):
        ...
        self.logger.info(f"EldersLegacy component {component_id} ({domain.value}) initialized")
    ...
    async def execute_with_quality_gate(self, request: TRequest) -> TResponse:
        ...
        try:
            ...
            # 品質スコア更新
            quality_score = await self._calculate_quality_score(result, execution_time_ms)
            self._update_quality_scores(quality_score)
            
            # Iron Will基準チェック
            if not self.passes_iron_will():
                self.logger.warning(f"Iron Will criteria not met: {self.get_iron_will_summary()}")
    ...
    def get_iron_will_summary(self) -> Dict[str, float]:
        """Iron Will品質基準の要約を取得"""
        return {criteria.value: score for criteria, score in self.quality_scores.items()}
    
    async def _calculate_quality_score(self, result: TResponse, execution_time_ms: float) -> float:
        """品質スコア計算"""
        quality_score = 0.0
    ...
    async def health_check(self) -> Dict[str, Any]:
        """ヘルスチェック"""
        is_healthy = (
            self.passes_iron_will() and
            self.execution_stats["average_quality_score"] >= 95.0
        )
...
class EldersLegacyRegistry:
    ...
    def get_components_by_domain(self, domain: EldersLegacyDomain) -> List[EldersLegacyBase]:
        """ドメイン別コンポーネント取得"""
