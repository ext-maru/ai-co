...
class RAGSageProxy(BaseSageProxy):
    ...
    async def generate_answer(self, question: str, context: Optional[List[Dict]] = None) -> Dict[str, Any]:
        """回答を生成"""
        return await self.call_sage(
            "generate_answer",
            question=question,
    ...
    async def index_document(self, document: Dict[str, Any], metadata: Optional[Dict] = None) -> Dict[str, Any]:
        """ドキュメントをインデックス"""
        return await self.call_sage(
            "index_document",
            document=document,
    ...
    async def update_index(self, document_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        """インデックスを更新"""
        return await self.call_sage(
            "update_index",
