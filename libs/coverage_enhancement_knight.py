...
class CoverageEnhancementKnight:
        ...
        try:
            ...
            if coverage_json_path.exists():
                ...
                for filepath, data in coverage_data.get('files', {}).items():
                    file_coverage[filepath] = data.get('summary', {}).get('percent_covered', 0.0)
    ...
    def _estimate_function_priority(self, node: ast.FunctionDef, source_code: str) -> str:
    ...
    def _generate_test_file_path(self, source_file: Path) -> Path:
        ...
        if source_file.parts[0] in ['libs', 'workers', 'commands']:
            ...
            test_path = test_dir / 'unit' / source_file.parent / f"test_{source_file.name}"
        else:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and \
                   node.name == target.function_name:
                    function_node = node
                    break
            ...
            self.logger.error(f"❌ Error generating test for {target.function_name}: {e}")
            return ""
    
    def _create_test_template(self, target: CoverageTarget, func_node: ast.FunctionDef, source_code: str) -> str:
...
'''
    ...
    def _generate_test_cases(self, func_node: ast.FunctionDef, args: List[str]) -> Dict[str, str]:
        ...
        for node in ast.walk(func_node):
            if isinstance(node, ast.Raise):
                test_cases['error_handling'] = 'with pytest.raises(Exception):\n            pass  # Test exception raising'
            elif isinstance(node, ast.Return):
                test_cases['basic'] = 'result = {target.function_name}()\n        assert result is not None'
        
        return test_cases
    
    def implement_coverage_improvement(self, target: CoverageTarget) -> CoverageImprovement:
        ...
        try:
            ...
            if improvement.success:
                self.logger.info(f"✅ Completed {target.function_name} (+{coverage_gain:.1f}% coverage)")
            else:
                self.logger.warning(f"⚠️ Tests created but failed: {improvement.error_message}")
            
            return improvement
            
        except Exception as e:
            self.logger.error(f"❌ Failed to improve coverage for {target.function_name}: {e}")
    ...
    def scan_for_coverage_targets(self) -> List[CoverageTarget]:
        ...
        for py_file in self.project_root.rglob('*.py'):
            ...
            if any(pattern.replace('*', '') in py_file.name for pattern in self.test_patterns):
        ...
        targets.sort(key=lambda t: (priority_order.get(t.priority, 4), t.estimated_effort))
    ...
    def start_coverage_monitoring(self):
        ...
        self.work_thread = threading.Thread(target=self._coverage_work_loop, daemon=True)
                ...
                if idle_status.get('suitable_for_tasks', False):
                    if self.coverage_targets.empty():
                        ...
                        for i, target in enumerate(targets[:10]):  # Limit to top 10
                            priority = 0 if target.priority == 'CRITICAL' else \
                                     1 if target.priority == 'HIGH' else \
                                     2 if target.priority == 'MEDIUM' else 3
    ...
    def get_knight_status(self) -> Dict:
        ...
        return {
            ...
            'total_coverage_gain': sum(imp.coverage_gain for imp in self.completed_improvements),
            'last_activity': self.completed_improvements[-1].completion_time if self.completed_improvements else None
        }
    ...
    try:
        if args.scan:
            ...
            for target in targets[:10]:
                print(f"   • {target.function_name} ({target.priority}, {target.estimated_effort}min)")
