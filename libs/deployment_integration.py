...
from libs.project_deployment_config import DeploymentConfig, ProjectDeploymentManager
...
class DeploymentIntegration:
    ...
    def get_deployment_method(self) -> str:
        ...
        try:
            return self.config_manager.get_deployment_strategy(
                self.current_project, self.current_environment
            )
    ...
    def execute_deployment(self, deployment_type: str = None) -> Dict[str, Any]:
        """デプロイメントを実行"""
        if not self.current_project:
            return {"success": False, "error": "No project context set"}
        ...
        if not config:
            return {"success": False, "error": "Failed to get deployment configuration"}
        ...
        if "error" in dry_run_result:
            return {"success": False, "error": dry_run_result["error"]}
    ...
    def _detect_project_type(self, project_path: Path) -> str:
        ...
        if (project_path / "package.json").exists():
            ...
            # Pythonプロジェクト
            if (project_path / "app.py").exists() or (
    ...
    def integrate_with_command(
        self, command_name: str, command_args: Dict[str, Any]
    ) -> Dict[str, Any]:
        """既存コマンドとの統合"""
        # プロジェクトを自動検出または指定
        ...
        if project:
            self.set_project_context(
                project, command_args.get("environment", "development")
            )
