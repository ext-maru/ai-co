...
class DeploymentSafeguard:
    ...
    def _pre_deployment_checks(self):
        ...
        if error_checks:
            raise RuntimeError(f"Pre-deployment checks failed: {'; '.join(error_checks)}")
    ...
    def _create_safety_snapshot(self, operation_name: str) -> str:
        ...
        for rel_path in self.critical_files:
            ...
            if file_path.exists():
                ...
                logger.debug(f"💾 バックアップ: {rel_path}")
        ...
        for rel_path in self.critical_files:
            ...
            if file_path.exists():
                try:
                    ...
                    logger.debug(f"🔒 ロック: {rel_path}")
                except FileNotFoundError:
                    # chattrコマンドが存在しない場合
                    # ファイル権限を読み取り専用に変更
                    file_path.chmod(0o444)
                    logger.debug(f"🔒 読み取り専用: {rel_path}")
        ...
        for rel_path in self.critical_files:
            ...
            if file_path.exists():
                try:
                    ...
                    logger.debug(f"🔓 ロック解除: {rel_path}")
                except FileNotFoundError:
                    # 権限を元に戻す
                    file_path.chmod(0o644)
                    logger.debug(f"🔓 権限復元: {rel_path}")
    ...
    def _post_deployment_verification(self):
        ...
        if error_results:
            raise RuntimeError(f"Post-deployment verification failed: {'; '.join(error_results)}")
    ...
    def safe_execute(self, command: str, operation_name: str = "command_execution") -> subprocess.CompletedProcess:
        """安全なコマンド実行"""
        # コマンド検証
        ...
        with self.deployment_protection(operation_name):
            ...
            if result.returncode != 0:
                logger.warning(f"⚠️ コマンドエラー (code {result.returncode}): {result.stderr}")
            else:
    ...
    def get_safeguard_status(self) -> Dict:
        ...
        for rel_path in self.critical_files:
            ...
            status['critical_files_status'][rel_path] = {
                ...
                'writable': file_path.is_file() and os.access(file_path, os.W_OK) if file_path.exists() else False
            }
