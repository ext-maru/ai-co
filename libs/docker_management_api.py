...
Phase 1 Week 3: Docker Management API (todo_27)
...
from libs.docker_template_manager import DockerTemplateManager, DockerTemplate
...
class ContainerCreateRequest(BaseModel):
    ...
    security_level: SecurityLevel = Field(SecurityLevel.SANDBOX, description="セキュリティレベル")
    runtime: RuntimeEnvironment = Field(RuntimeEnvironment.PYTHON_39, description="ランタイム環境")
    ports: Optional[List[str]] = Field(default=[], description="ポートマッピング (例: '8080:80')")
...
class DockerManager:
    ...
    def create_container(self, request: ContainerCreateRequest) -> ContainerInfo:
        ...
        try:
            ...
            self.template_manager.create_project_structure(
    ...
    def execute_action(self, container_id: str, action: ContainerAction) -> Dict[str, Any]:
        """コンテナアクションを実行"""
        try:
            container = self.client.containers.get(container_id)
    ...
    def get_container_stats(self, container_id: str) -> ContainerStats:
        ...
        try:
            ...
            # ネットワーク統計
            network_rx = sum(net['rx_bytes'] for net in stats['networks'].values()) / 1024 / 1024  # MB
            network_tx = sum(net['tx_bytes'] for net in stats['networks'].values()) / 1024 / 1024  # MB
...
@app.get("/containers/{container_id}/stats", response_model=ContainerStats, tags=["Container Stats"])
