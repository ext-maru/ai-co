...
# 既存システム統合
try:
    from libs.worker_monitoring_dashboard import WorkerMonitoringDashboard, MetricsCollector
...
class DockerComposeManager:
    ...
    def generate_compose_config(self, services_config: Dict[str, Any]) -> Dict[str, Any]:
        """Docker Compose設定を生成"""
        compose_config = {
            'version': '3.8',
            'services': {},
    ...
    def _generate_service_definition(self, service_name: str, config: Dict[str, Any], role: str) -> Dict[str, Any]:
        """個別サービス定義を生成"""
        service_def = {
        ...
        if config.get('health_check', True):
            service_def['healthcheck'] = {
                'test': config.get('health_check_command', 'curl -f http://localhost:8080/health || exit 1'),
    ...
    def _generate_cluster_service_definition(self, service_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """クラスターサービス定義を生成"""
    ...
    def _add_volume_definitions(self, compose_config: Dict[str, Any], services_config: Dict[str, Any]):
        """ボリューム定義を追加"""
        # 共通ボリューム
    ...
    def deploy_services(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = ['docker-compose', '-f', str(self.compose_path), 'up', '-d', '--remove-orphans']
    ...
    def _update_single_service(self, service_name: str, new_image: Optional[str]) -> Dict[str, Any]:
        """単一サービスの更新"""
        try:
            cmd = ['docker-compose', '-f', str(self.compose_path), 'up', '-d', service_name]
            if new_image:
                # 新しいイメージを指定してプル
...
class ServiceHealthMonitor:
    ...
    def _get_docker_client(self):
        ...
        try:
            ...
            logger.warning("Docker Python client not available, using subprocess fallback")
            return None
    ...
    def get_service_metrics(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = ['docker', 'stats', service_name, '--no-stream', '--format', 'table {{.CPUPerc}}\t{{.MemUsage}}']
...
class FailoverManager:
    ...
    def should_trigger_failover(self, failure_event: Dict[str, Any]) -> bool:
        ...
        # フェイルオーバー判定
        consecutive_failures = failure_event.get('consecutive_failures', self.failure_counts[service])
        
        # 閾値チェック
        if consecutive_failures >= self.config.failover_threshold:
            logger.warning(f"Service {service} exceeded failure threshold ({consecutive_failures} >= {self.config.failover_threshold})")
            return True
        
        # 重大な失敗タイプの場合は即座にフェイルオーバー
        critical_failure_types = ['container_exit', 'health_check_timeout', 'resource_exhaustion']
        if failure_type in critical_failure_types:
            logger.error(f"Critical failure detected for service {service}: {failure_type}")
            return True
    ...
    def execute_failover(self, failure_event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if strategy['action'] == 'restart_container':
                ...
                recovery_result = {'success': False, 'error': f"Unknown strategy: {strategy['action']}"}
            ...
            # 影響評価
            failover_time = (failover_event.completed_at - failover_event.started_at).total_seconds()
    ...
    def _determine_failover_strategy(self, failure_event: Dict[str, Any]) -> Dict[str, Any]:
        """フェイルオーバー戦略を決定"""
        service = failure_event['service']
        failure_type = failure_event['type']
        ...
        if failure_type in ['container_exit', 'health_check_failed']:
            ...
            backup_service = f"{base_service}-backup" if not service.endswith('-backup') else f"{base_service}-primary"
            return {
    ...
    def _restart_container(self, service: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                return {
                    'success': True,
                    'actions': [{'type': 'restart_container', 'service': service, 'timestamp': datetime.now().isoformat()}],
                    'downtime_seconds': 30
                }
            else:
                return {
    ...
    def _scale_up_service(self, service: str, strategy: Dict[str, Any]) -> Dict[str, Any]:
        """サービスをスケールアップ"""
        try:
            ...
            cmd = ['docker-compose', '-f', str(compose_path), 'up', '-d', '--scale', f"{service}={scale_factor}"]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
            
            if result.returncode == 0:
                return {
                    'success': True,
                    'actions': [{'type': 'scale_up', 'service': service, 'scale_factor': scale_factor, 'timestamp': datetime.now().isoformat()}],
                    'downtime_seconds': 0
                }
            else:
                return {
    ...
    def _switch_to_backup(self, service: str, strategy: Dict[str, Any]) -> Dict[str, Any]:
        """バックアップサービスに切り替え"""
        try:
            ...
            if stop_result.returncode == 0:
                actions.append({'type': 'stop_service', 'service': service, 'timestamp': datetime.now().isoformat()})
            
            if start_result.returncode == 0:
                actions.append({'type': 'start_backup', 'service': target_service, 'timestamp': datetime.now().isoformat()})
        ...
        try:
            if recovery_strategy == 'restart_and_fallback':
                ...
                if not restart_result['success'] and backup_service:
                    fallback_result = self._switch_to_backup(failed_service, {'target_service': backup_service})
                ...
                # 既存サービスをスケールアップ
                scale_result = self._scale_up_service(failed_service, {'scale_factor': 2})
    ...
    def detect_cascading_failure(self, failure_events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """カスケード障害を検知"""
        ...
        if len(recent_failures) >= 3:
            ...
            time_span = max(datetime.fromisoformat(e['timestamp']) for e in recent_failures) - \
    ...
    def generate_emergency_response_plan(self, cascade_info: Dict[str, Any]) -> Dict[str, Any]:
        """緊急時対応計画を生成"""
        emergency_actions = []
        ...
        return {
            'priority': 'critical',
            'estimated_total_time': sum(action['estimated_time'] for action in emergency_actions),
                ...
                if health_status['success']:
                    ...
                    for service in health_status['services']:
                        if service['health_status'] != 'healthy':
                            ...
                            if self.failover_manager.should_trigger_failover(failure_event):
                                logger.warning(f"Triggering failover for service {service['name']}")
                                failover_result = self.failover_manager.execute_failover(failure_event)
                                
                                if failover_result['success']:
                                    logger.info(f"Failover completed for {service['name']}")
                                else:
                                    logger.error(f"Failover failed for {service['name']}: {failover_result.get('error')}")
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_failovers = {
                ...
                if fo.completed_at is None or (datetime.now() - fo.completed_at).seconds < 300
            }
            
            # システム可用性計算
            if health_status['success'] and health_status['services']:
                availability = (health_status['healthy_count'] / len(health_status['services'])) * 100
            else:
                availability = 0.0
