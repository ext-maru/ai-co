...
class ElderCouncilSummoner:
    ...
    def __init__(self):
        ...
        self.worker_health_monitor = (
            WorkerHealthMonitor() if WorkerHealthMonitor else None
        )
    ...
    def _collect_system_metrics(self) -> SystemEvolutionMetrics:
        ...
        try:
            ...
            worker_health_score = (
                healthy_workers / total_workers if total_workers > 0 else 0.0
            )
    ...
    def _evaluate_triggers(self, metrics: SystemEvolutionMetrics):
        ...
        # Critical triggers
        if metrics.worker_health_score < (
            1 - self.thresholds["critical"]["worker_failure_rate"]
        ):
            self._create_trigger(
                ...
                ["worker_system"],
                [
                    "Immediate worker system stabilization",
        ...
        if (
            ...
            > self.thresholds["critical"]["queue_critical_backlog"]
        ):
            self._create_trigger(
        ...
        if (
            metrics.four_sages_consensus_rate
            < self.thresholds["critical"]["four_sages_failure"]
        ):
            self._create_trigger(
        ...
        if (
            metrics.system_complexity_score
            > self.thresholds["high"]["architectural_complexity"]
        ):
            self._create_trigger(
    ...
    def _create_trigger(
        ...
        title: str,
        description: str,
        metrics: SystemEvolutionMetrics,
        affected_systems: List[str],
        suggested_agenda: List[str],
    ):
        """トリガー作成"""
    ...
    def _perform_auto_analysis(
        self, category: TriggerCategory, metrics: SystemEvolutionMetrics
    ) -> Dict[str, Any]:
        """自動分析実行"""
        analysis = {
    ...
    def _analyze_trends(
        self, current_metrics: SystemEvolutionMetrics
    ) -> Dict[str, Any]:
        """トレンド分析"""
    ...
    def _justify_urgency(
        ...
        if category == TriggerCategory.SYSTEM_FAILURE:
            justifications.append(
                "System stability directly affects operational continuity"
            )

        if metrics.worker_health_score < 0.5:
            justifications.append(
                "Majority of workers are failing, critical intervention needed"
            )
        ...
        return (
            "; ".join(justifications) if justifications else "Standard threshold breach"
        )
    ...
    def _consider_council_summoning(self, trigger: CouncilTrigger):
        ...
        for pending_id, pending_council in self.pending_councils.items():
            if trigger.category in pending_council.get("categories", []):
                ...
                logger.info(
                    f"Added trigger {trigger.trigger_id} to existing council {pending_id}"
                )
                return
        ...
        logger.info(
            f"Elder Council summoning requested: {council_id} ({trigger.urgency.value})"
        )
    ...
    def _create_council_request_document(self, council_request: Dict[str, Any]):
        ...
        # ファイル保存
        document_file = (
            PROJECT_ROOT / "knowledge_base" / f"{council_id}_elder_council_request.md"
        )
...
"""
            ...
            for sage_name, assessment in sages_input.get(
                "individual_assessments", {}
            ).items():
                document += f"""
**{sage_name}**:
...
"""
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "urgency_distribution": {
                level.value: sum(
                    1 for t in self.triggers.values() if t.urgency == level
                )
    ...
    def _coordinate_emergency_response(
        ...
        try:
            ...
            priority_actions = sorted(
                ...
                key=lambda x: {"IMMEDIATE": 0, "HIGH": 1, "MEDIUM": 2}.get(
                    x["priority"], 3
                ),
            )
            emergency_response["priority_queue"] = [
                action["action"] for action in priority_actions
            ]

            # 4賢者への緊急協調要請
            if self.four_sages_enabled:
                sages_coordination = self._request_emergency_sages_coordination(
                    emergency_response
                )
            ...
            emergency_response["auto_executable"] = [
                action
                for action in emergency_response["actions"]
                if action.get("assigned_to")
                and self._is_system_available(action["assigned_to"])
            ]
    ...
    def _validate_council_decisions(
        self, emergency_response: Dict[str, Any]
    ) -> Dict[str, Any]:
        """評議会決定の検証"""
        try:
            ...
            validation_result["risk_assessment"] = {
                ...
                "safe_actions": sum(
                    1 for v in validation_result["validations"] if v["status"] == "SAFE"
                ),
            ...
            if (
                ...
                and validation_result["risk_assessment"]["high_risk_actions"] == 0
            ):
    ...
    def _request_emergency_sages_coordination(
        self, emergency_response: Dict[str, Any]
    ) -> Dict[str, Any]:
        """4賢者への緊急協調要請"""
        try:
