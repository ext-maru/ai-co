...
from libs.elder_flow_violation_types_original import (
    ...
    ELDER_FLOW_VIOLATION_RULES, ViolationRule
)
...
class PDCAMetrics:
    ...
    def is_improving(self) -> bool:
        """改善傾向にあるか判定"""
        return (
            self.violation_rate < self.previous_violation_rate or
            self.auto_fix_success_rate > self.previous_auto_fix_rate
        )
    ...
    def get_improvement_percentage(self) -> float:
        ...
        if self.previous_violation_rate > 0:
            return (
                ...
                self.previous_violation_rate * 100
            )
        return 0.0
...
class PDCACycle:
    ...
    def plan(self) -> List[ImprovementAction]:
        ...
        actions.sort(key=lambda a: (
            0 if a.priority == ActionPriority.CRITICAL else
        ...
        for action in actions:
            try:
                ...
                action.complete_execution(
                    success=success,
                    result="実行完了" if success else "実行失敗"
                )
    ...
    def check(self, executed_actions: List[ImprovementAction]) -> Dict[str, Any]:
        ...
        evaluation = {
            ...
            'action_results': {
                ...
                'success_rate': (successful_actions / len(executed_actions) * 100)
                               if executed_actions else 0
            },
    ...
    def _create_improvement_action(
        ...
        if violation_type in action_templates:
            ...
            return ImprovementAction(
                ...
                target_violation_type=ViolationType(violation_type)
                if violation_type in [v.value for v in ViolationType] else None
            )
    ...
    def _generate_recommendations(self, metrics: PDCAMetrics) -> List[str]:
        ...
        if metrics.auto_fix_success_rate < 70:
            recommendations.append("自動修正成功率が低いため、修正ロジックの見直しを推奨")
    ...
    def _update_knowledge_base(self, evaluation: Dict[str, Any]) -> List[str]:
        ...
        learning_file = kb_path / f"learning_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
class PDCAEngine:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'last_cycle_time': last_cycle.completed_at.isoformat() if last_cycle else None,
            'last_cycle_duration': last_cycle.get_duration().total_seconds() if last_cycle else None
        }

        # 平均サイクル時間を計算
        if self.cycle_history:
            durations = [c.get_duration().total_seconds() for c in self.cycle_history[-10:]]
        ...
        if len(self.cycle_history) >= 2:
            ...
            if prev_metrics:
                summary['violation_rate_trend'] = (
                    'improving' if latest_metrics.violation_rate < prev_metrics.violation_rate
                    else 'worsening'
                )
                summary['improvement_percentage'] = (
                    (prev_metrics.violation_rate - latest_metrics.violation_rate) /
                    prev_metrics.violation_rate * 100
                ) if prev_metrics.violation_rate > 0 else 0
