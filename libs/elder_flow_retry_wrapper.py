...
class ElderFlowRetryWrapper:
    ...
    async def execute_with_retry(
        ...
        while attempt <= max_retries:
            try:
                # Elder Flow実行を試みる（デフォルトでClaude Elder魂統合）
                ...
                if result.get("error") == "Task already running":
                    ...
                    if auto_retry:
                        ...
                        if choice == "R":
                            ...
                            print(
                                f"   {cleanup_result['cleaned_locks']}個のロックをクリーンアップしました"
                            )
                            attempt += 1
                            continue
                        else:
    ...
    async def _wait_for_task_completion(
        self, task_name: str, pid: int, check_interval: int = 2
    ):
        """
        タスクの完了を待機
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        result = await execute_elder_flow_with_retry(
            task_name=task_name, auto_retry=False
        )
        print(f"\n実行結果: {result}")
    else:
