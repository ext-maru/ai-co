...
class ElderFlowUltimateEvolution:
    ...
    def _display_phase_result(self, phase_info, result):
        ...
        print(f"\n🧙‍♂️ 4賢者活動:")
        print(
            f"  📚 ナレッジ: {sages_contributions['knowledge_sage']['knowledge_entries_found']}件"
        )
        print(
            f"  📋 タスク: {sages_contributions['task_sage']['optimizations_suggested']}件"
        )
        print(
            f"  🚨 インシデント: {sages_contributions['incident_sage']['risks_identified']}件"
        )
        print(
            f"  🔍 RAG: {sages_contributions['rag_sage']['similar_patterns_found']}件"
        )
    ...
    def _display_ultimate_evolution_report(self, total_results):
        ...
        total_knowledge = sum(
            r["result"]["sages_contributions"]["knowledge_sage"][
                "knowledge_entries_found"
            ]
            for r in total_results
        )
