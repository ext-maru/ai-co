...
from libs.unified_auth_provider import ElderRole, SageType, User, AuthSession
...
class ElderProjectManager:
            ...
            try:
                ...
                for project_id, project_data in data.items():
                    ...
                    if project_data.get('estimated_completion'):
                        project_data['estimated_completion'] = datetime.fromisoformat(project_data['estimated_completion'])
                    if project_data.get('actual_completion'):
                        project_data['actual_completion'] = datetime.fromisoformat(project_data['actual_completion'])
    ...
    def scan_projects(self) -> List[ProjectInfo]:
        ...
        to_remove = [pid for pid in self.project_index.keys() if pid not in existing_ids]
    ...
    def _analyze_project_directory(self, project_dir: Path) -> Optional[ProjectInfo]:
        ...
        try:
            ...
            size_bytes = sum(f.stat().st_size for f in project_dir.rglob('*') if f.is_file())
    ...
    def register_project(self, project_info: ProjectInfo):
        ...
        metadata = {
            ...
            'estimated_completion': project_info.estimated_completion.isoformat() if project_info.estimated_completion else None,
            'actual_completion': project_info.actual_completion.isoformat() if project_info.actual_completion else None
        }
    ...
    def get_project_list(self, user: User, filters: Dict[str, Any] = None) -> List[ProjectInfo]:
        ...
        if user.elder_role == ElderRole.SERVANT:
            ...
            projects = [p for p in projects if 
                       p.owner == user.username or 
                       p.elder_role in [ElderRole.SERVANT, ElderRole.SAGE]]
        ...
        if filters:
            ...
            if 'priority' in filters:
                projects = [p for p in projects if p.priority.value == filters['priority']]
            if 'owner' in filters:
    ...
    def get_project_details(self, project_id: str, user: User) -> Optional[ProjectInfo]:
        ...
        if user.elder_role == ElderRole.SERVANT and project.owner != user.username:
            ...
            if project.owner != user.username and project.elder_role not in [ElderRole.SERVANT, ElderRole.SAGE]:
                return None
        
        return project
    
    def update_project_status(self, project_id: str, status: ProjectStatus, user: User) -> bool:
        """プロジェクトステータスを更新"""
        if project_id not in self.project_index:
            return False
    ...
    def get_project_statistics(self, user: User) -> Dict[str, Any]:
        ...
        for project in projects:
            ...
            # 優先度別カウント
            priority_counts[project.priority.value] = priority_counts.get(project.priority.value, 0) + 1
            
            # Elder権限別カウント
            elder_role_counts[project.elder_role.value] = elder_role_counts.get(project.elder_role.value, 0) + 1
...
class ElderProjectCLI:
    ...
    def format_project_list(self, projects: List[ProjectInfo], show_details: bool = False) -> str:
        """プロジェクト一覧をフォーマット"""
        if not projects:
        ...
        for project in projects:
            ...
            output.append(f"   {priority_icon} {project.priority.value.upper()} | {elder_icon} {project.elder_role.value}")
            output.append(f"   📝 {project.description}")
            output.append(f"   👤 {project.owner} | 📅 {project.updated_at.strftime('%Y-%m-%d %H:%M')}")
            
            if show_details:
                output.append(f"   📁 {project.file_count} files | 💾 {project.size_mb} MB")
    ...
    def format_project_details(self, project: ProjectInfo) -> str:
        ...
        if project.estimated_completion:
            output.append(f"   予定完了: {project.estimated_completion.strftime('%Y-%m-%d %H:%M:%S')}")
        if project.actual_completion:
            output.append(f"   実際完了: {project.actual_completion.strftime('%Y-%m-%d %H:%M:%S')}")
        output.append("")
        ...
        # 進捗情報
        output.append("📊 **進捗情報**")
        progress_bar = "█" * int(project.progress * 20) + "░" * (20 - int(project.progress * 20))
