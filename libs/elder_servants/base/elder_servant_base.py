...
class ElderServantBase(ABC, Generic[TRequest, TResponse]):
    ...
    @abstractmethod
    async def process_request(self, request: ServantRequest[TRequest]) -> ServantResponse[TResponse]:
        """
        リクエストを処理する抽象メソッド
    ...
    async def execute_with_quality_gate(self, request: ServantRequest[TRequest]) -> ServantResponse[TResponse]:
        """
    ...
    def _check_iron_will_criteria(self) -> bool:
        ...
        for metric, threshold in criteria.items():
            if self._quality_scores.get(metric, 0) < threshold:
                self.logger.warning(f"{metric} score {self._quality_scores[metric]} below threshold {threshold}")
                return False
        
        return True
    
    async def _update_quality_scores(self, request: ServantRequest[TRequest], response: ServantResponse[TResponse]):
        ...
        return {
            **self._metrics,
            ...
            "success_rate": (
                ...
                if self._metrics["tasks_processed"] > 0 else 0
            )
        }
