...
class BugHunter(DwarfServant[Dict[str, Any], Dict[str, Any]]):
    ...
    async def execute_task(self, task: Dict[str, Any]) -> TaskResult:
        ...
        try:
            ...
            if task_type == "static_analysis":
                result_data = await self._static_analysis(payload.get("source_code", ""))
                ...
                result_data = await self._auto_fix_suggestion(
                    payload.get("source_code", ""),
                    payload.get("bug_report", {})
                )
            elif task_type == "code_smell_detection":
                result_data = await self._code_smell_detection(payload.get("source_code", ""))
            elif task_type == "vulnerability_scan":
                result_data = await self._vulnerability_scan(payload.get("source_code", ""))
            elif task_type == "quality_metrics":
                result_data = await self._quality_metrics(payload.get("source_code", ""))
            elif task_type == "regression_analysis":
                result_data = await self._regression_analysis(
                    payload.get("original_code", ""),
                    payload.get("modified_code", "")
                )
            elif task_type == "performance_bug_detection":
                result_data = await self._performance_bug_detection(
                    payload.get("source_code", ""),
                    payload.get("profile_data", {})
                )
            elif task_type == "memory_leak_detection":
                result_data = await self._memory_leak_detection(payload.get("source_code", ""))
            else:
    ...
    async def _static_analysis(self, source_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "code_quality_score": self._calculate_quality_score(issues, source_code),
    ...
    async def _bug_detection(self, source_code: str, test_cases: List[Dict[str, Any]]) -> Dict[str, Any]:
        """バグ検出・分類"""
        if not source_code:
        ...
        try:
            ...
            # テストケースベースの検証
            if test_cases:
                test_bugs = await self._validate_with_test_cases(source_code, test_cases)
            ...
            return {
                "bug_report": {
                    ...
                    "severity_distribution": self._get_severity_distribution(bugs_found)
                },
    ...
    async def _auto_fix_suggestion(self, source_code: str, bug_report: Dict[str, Any]) -> Dict[str, Any]:
        """自動修正提案"""
        ...
        try:
            ...
            return {
                ...
                "auto_fixable_count": len([f for f in optimized_fixes if f.get("auto_apply", False)]),
                "manual_review_count": len([f for f in optimized_fixes if not f.get("auto_apply", False)])
            }
    ...
    async def _code_smell_detection(self, source_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for smell_name, smell_pattern in self.code_smell_patterns.items():
                ...
                for instance in smell_instances:
                    smells_detected.append({
                        ...
                        "refactoring_suggestion": smell_pattern.get("refactoring", "")
                    })
            ...
            return {
                ...
                "maintainability_score": self._calculate_maintainability_score(smells_detected),
    ...
    async def _vulnerability_scan(self, source_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for vuln_name, vuln_pattern in self.vulnerability_patterns.items():
                matches = self.security_scanner.scan_for_vulnerability(tree, vuln_pattern)
                for match in matches:
    ...
    async def _quality_metrics(self, source_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics = {
                ...
                "maintainability_index": self._calculate_maintainability_index(tree, source_code)
            }
            
            # 品質指標
            quality_indicators = {
                "comment_ratio": metrics["comment_lines"] / max(metrics["lines_of_code"], 1),
                "function_complexity_avg": metrics["complexity"] / max(metrics["functions"], 1),
                "class_size_avg": metrics["logical_lines"] / max(metrics["classes"], 1) if metrics["classes"] > 0 else 0
            }
            
            # 総合品質スコア
            overall_quality = self._calculate_overall_quality(metrics, quality_indicators)
            ...
            return {
                ...
                "recommendations": self._generate_quality_recommendations(metrics, quality_indicators)
            }
    ...
    async def _validate_bug_hunting_quality(self, result_data: Dict[str, Any]) -> float:
        ...
        try:
            # 検出した問題数による加点
            issues_found = len(result_data.get("issues", [])) + len(result_data.get("bugs_found", [])) + len(result_data.get("vulnerabilities", []))
...
class SecurityScanner:
    ...
    def scan_for_vulnerability(self, tree: ast.AST, pattern: Dict[str, Any]) -> List[Dict[str, Any]]:
        """脆弱性スキャン実行"""
        matches = []
...
class PatternMatcher:
    ...
    def find_pattern_matches(self, tree: ast.AST, pattern: Dict[str, Any]) -> List[Dict[str, Any]]:
        """パターンマッチング実行"""
        matches = []
