...
class HybridCodeCraftsman(ElderServant):
    ...
    def __init__(self):
        super().__init__(
            ...
            capabilities=[
                ServantCapability("hybrid_code_generation", "Generate code using Elder+OSS", 
                                ["prompt", "context"], ["generated_code"], 2),
                ServantCapability("code_refactoring", "Refactor code with hybrid approach",
                                ["code", "goals"], ["refactored_code"], 2)
            ]
        )
    ...
    def _should_use_elder_first(self, prompt: str, context: Dict) -> bool:
        """Elder First戦略判定"""
        elder_indicators = [
            "elder", "iron will", "quality", "pattern", "architecture",
            "sage", "servant", "guild", "hierarchy"
        ]
        
        oss_indicators = [
            "simple", "basic", "quick", "draft", "prototype", "example"
        ]
        
        prompt_lower = prompt.lower()
        elder_score = sum(1 for indicator in elder_indicators if indicator in prompt_lower)
    ...
    async def _elder_code_generation(self, prompt: str, context: Dict) -> Dict[str, Any]:
        """Elder システムによるコード生成"""
    ...
    async def _oss_code_generation(self, prompt: str, context: Dict) -> Dict[str, Any]:
        ...
        if response.success:
            return {
                "generated_code": response.data.get("result", {}).get("result_data", {}).get("generated_code", ""),
                "source": "oss_continue_dev",
                "explanation": response.data.get("result", {}).get("result_data", {}).get("explanation", ""),
    ...
    async def _oss_code_enhancement(self, elder_result: Dict, prompt: str) -> Dict[str, Any]:
        """OSS ツールによるコード強化"""
        ...
        if response.success:
            ...
            return {"enhanced_code": code, "source": "no_enhancement", "quality_score": elder_result.get("quality_score", 0.80)}
    
    async def _elder_code_enhancement(self, oss_result: Dict, prompt: str) -> Dict[str, Any]:
        """Elder システムによるコード強化"""
        ...
        return {
            ...
            "original_code": code,
            "elder_patterns": ["Iron Will Validation", "Execution Monitoring", "Quality Gates"],
            "source": "elder_enhancement",
            "quality_score": 0.93,
            "iron_will_compliant": True
        }
    
    async def _merge_elder_oss_results(self, elder_result: Dict, oss_result: Dict) -> Dict[str, Any]:
        """Elder → OSS 結果統合"""
        return {
            "final_code": oss_result.get("enhanced_code", elder_result.get("generated_code", "")),
            "elder_base": elder_result,
            "oss_enhancement": oss_result,
            "merge_strategy": "elder_first",
            "combined_quality": (elder_result.get("quality_score", 0.8) * 0.6 + 
    ...
    async def _merge_oss_elder_results(self, oss_result: Dict, elder_result: Dict) -> Dict[str, Any]:
        """OSS → Elder 結果統合"""
        return {
            "final_code": elder_result.get("enhanced_code", oss_result.get("generated_code", "")),
            "oss_base": oss_result,
            "elder_enhancement": elder_result,
            "merge_strategy": "oss_first",
            "combined_quality": (oss_result.get("quality_score", 0.8) * 0.4 + 
    ...
    async def _intelligent_merge(self, elder_result: Dict, oss_result: Dict, prompt: str) -> Dict[str, Any]:
        """インテリジェント結果統合"""
        ...
        if elder_quality > oss_quality + 0.1:
            ...
            enhancement_result = oss_result if base_result == elder_result else elder_result
        
        # コード統合
        base_code = base_result.get("generated_code", "")
        enhancement_code = enhancement_result.get("enhanced_code", enhancement_result.get("generated_code", ""))
...
"""
    ...
    async def _validate_hybrid_quality(self, results: Dict) -> float:
        ...
        # Elder パターン使用ボーナス
        elder_patterns_bonus = 0.05 if "elder" in results.get("merge_strategy", "").lower() else 0
        
        # OSS活用ボーナス
        oss_bonus = 0.05 if any("oss" in str(v).lower() for v in results.values()) else 0
        
        final_quality = min(1.0, base_quality + iron_will_bonus + elder_patterns_bonus + oss_bonus)
...
class HybridTestGuardian(ElderServant):
    ...
    def __init__(self):
        super().__init__(
            ...
            capabilities=[
                ServantCapability("hybrid_test_generation", "Generate tests using Elder+OSS", 
                                ["code", "test_type"], ["test_code"], 2),
                ServantCapability("test_execution", "Execute tests with hybrid approach",
                                ["test_code"], ["test_results"], 1)
            ]
        )
        self.oss_framework = create_oss_adapter_framework()
...
"""
    ...
    async def _generate_pytest_tests(self, code: str, elder_tests: Dict) -> Dict[str, Any]:
        """PyTest テスト生成"""
...
""",
    ...
    async def _merge_test_approaches(self, elder_tests: Dict, pytest_tests: Dict) -> Dict[str, Any]:
        """テストアプローチ統合"""
...
"""
        ...
        return {
            ...
            "total_tests": elder_tests.get("test_count", 0) + pytest_tests.get("test_count", 0) + 3
        }
...
class HybridQualityInspector(ElderServant):
    ...
    def __init__(self):
        super().__init__(
            ...
            capabilities=[
                ServantCapability("hybrid_quality_check", "Quality check using Elder+OSS", 
                                ["code", "file_path"], ["quality_report"], 2),
                ServantCapability("iron_will_validation", "Validate Iron Will compliance",
                                ["code"], ["compliance_report"], 1)
            ]
        )
        self.oss_framework = create_oss_adapter_framework()
    ...
    async def _integrate_quality_results(self, elder_analysis: Dict, flake8_analysis: Dict) -> Dict[str, Any]:
        """品質結果統合"""
...
async def test_hybrid_servants():
    ...
    print(f"✅ Success: {result['success']}")
    print(f"🎯 Quality Score: {result['result_data'].get('overall_quality_score', 'N/A')}")
