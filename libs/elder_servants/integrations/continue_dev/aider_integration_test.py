...
"""
            ...
            if result.returncode == 0:
                ...
                with open(os.path.join(self.test_dir, 'test_elder_service.py'), 'r') as f:
                    content = f.read()
...
"""
            ...
            result = subprocess.run([
                ...
                '--message', request,
                'elder_service.py'
            ], cwd=self.test_dir, capture_output=True, text=True, timeout=90)
            ...
            if result.returncode == 0:
                ...
                found_patterns = sum(1 for pattern in elder_patterns if pattern.lower() in content.lower())
                
                if found_patterns >= 2:
                    print(f"✅ Aider Elder integration successful ({found_patterns}/4 patterns found)")
                    return True
                else:
                    print(f"❌ Aider Elder integration insufficient ({found_patterns}/4 patterns)")
                    return False
            else:
...
"""
            ...
            if result.returncode == 0:
                ...
                found_improvements = sum(1 for imp in improvements if imp.lower() in content.lower())
                
                if found_improvements >= 3:
                    print(f"✅ Aider Sage consultation integration successful ({found_improvements}/5 improvements)")
                    return True
                else:
                    print(f"❌ Aider Sage consultation insufficient ({found_improvements}/5 improvements)")
                    return False
            else:
    ...
    def run_integration_tests(self):
        ...
        try:
            ...
            if passed >= 4:  # Allow 1 failure
                print("🎉 Aider integration successful! Ready for 4 Sages collaboration.")
                return True
            else:
