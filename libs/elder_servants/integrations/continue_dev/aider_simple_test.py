...
def test_aider_simple():
    ...
    # Test 1: Basic help
    try:
        result = subprocess.run([aider_path, '--help'], 
    ...
    try:
        result = subprocess.run([aider_path, '--version'], 
                              capture_output=True, text=True, timeout=30)
        if result.returncode == 0:
...
''')
        ...
        subprocess.run(['git', 'commit', '-m', 'Initial'], cwd=test_dir, capture_output=True)
        ...
        result = subprocess.run([
            aider_path,
            '--yes',
            '--message', 'Add a docstring to the hello function',
            'test.py'
