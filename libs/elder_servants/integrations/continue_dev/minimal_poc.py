...
async def check_iron_will_quality(request: QualityCheckRequest):
    ...
    try:
        ...
        return {
            ...
            "full_report": {
                "overall_score": score,
                "code_quality": {"grade": "A" if score >= 95 else "B" if score >= 80 else "C"},
                "security_issues": [],
...
async def search_knowledge_base(request: KnowledgeSearchRequest):
    ...
    try:
        ...
        filtered_items = [item for item in mock_items if request.query.lower() in item["title"].lower() or request.query.lower() in item["content"].lower()]
...
async def list_servants():
    ...
    return {
        "success": True,
        "servants": MOCK_SERVANTS,
        "total": len(MOCK_SERVANTS)
    }
