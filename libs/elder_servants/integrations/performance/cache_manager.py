...
class ElderCacheManager(EldersServiceLegacy[CacheRequest, CacheResponse]):
    ...
    def __init__(
        ...
        # 初期化統計
        self.logger.info(
            f"Elder Cache Manager initialized with strategy: {strategy.value}"
        )
    ...
    def _get_ttl_for_key_type(self, key_type: CacheKeyType) -> int:
        ...
        return self.ttl_strategies.get(key_type, {}).get(
            self.strategy, self.config["default_ttl"]
        )
    ...
    async def get_quality_check_cache(
        self, file_hash: str, check_type: str
    ...
    async def set_quality_check_cache(
        self, file_hash: str, check_type: str, result: Dict[str, Any]
    ) -> bool:
        """品質チェック結果キャッシュ設定"""
    ...
    async def get_sage_consultation_cache(
        self, sage_type: str, consultation_context: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """4賢者相談結果キャッシュ取得"""
        ...
        return (
            ...
            if response.success and response.cached
            else None
        )

    async def set_sage_consultation_cache(
        self,
        sage_type: str,
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            iron_will_compliant = (
                hit_rate >= 70.0
                and error_rate <= 5.0  # 70%以上のヒット率
                and redis_healthy  # 5%以下のエラー率  # Redis接続健全
            )
...
# 便利な関数群
async def cache_quality_check_result(
    file_hash: str, check_type: str, result: Dict[str, Any]
) -> bool:
    """品質チェック結果キャッシュ（便利関数）"""
...
async def get_cached_quality_check(
    file_hash: str, check_type: str
) -> Optional[Dict[str, Any]]:
    """品質チェック結果取得（便利関数）"""
    cache_manager = await get_cache_manager()
...
async def cache_sage_consultation(
    sage_type: str, context: Dict[str, Any], result: Dict[str, Any]
) -> bool:
    """4賢者相談結果キャッシュ（便利関数）"""
    cache_manager = await get_cache_manager()
...
async def get_cached_sage_consultation(
    sage_type: str, context: Dict[str, Any]
) -> Optional[Dict[str, Any]]:
    """4賢者相談結果取得（便利関数）"""
    cache_manager = await get_cache_manager()
