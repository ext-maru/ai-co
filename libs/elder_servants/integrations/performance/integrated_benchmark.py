...
from libs.elder_servants.integrations.performance.cache_manager import (
    ElderCacheManager, CacheStrategy, CacheKeyType, CacheRequest
)
from libs.elder_servants.integrations.performance.async_optimizer import (
    AsyncWorkerOptimizer, OptimizationStrategy, ExecutionMode, 
    ResourceType, AsyncOptimizationRequest, OptimizationProfile, ResourceLimits
)
from libs.elder_servants.integrations.performance.lightweight_proxy import (
    LightweightElderProxy, ProxyConfig, ProxyMode, ProxyRequest
)
...
class IntegratedPerformanceBenchmark:
    ...
    async def _run_baseline_benchmark(self) -> BenchmarkResults:
        ...
        for i in range(self.config.concurrent_requests):
            task = asyncio.create_task(
                self._baseline_worker(response_times, i)
            )
            tasks.append(task)
    ...
    async def _run_cache_benchmark(self) -> BenchmarkResults:
        ...
        for i in range(self.config.concurrent_requests):
            task = asyncio.create_task(
                self._cache_worker(response_times, i)
            )
            tasks.append(task)
    ...
    async def _run_async_benchmark(self) -> BenchmarkResults:
        ...
        for i in range(self.config.concurrent_requests):
            task = asyncio.create_task(
                self._async_worker(response_times, i)
            )
            tasks.append(task)
    ...
    async def _execute_async_request(self, worker_id: int, request_num: int):
        ...
        async def optimized_task():
            ...
            # リソースタイプをランダム選択
            resource_types = [ResourceType.IO_BOUND, ResourceType.CPU_BOUND, ResourceType.NETWORK_BOUND]
            ...
            return {"worker_id": worker_id, "request_num": request_num, "result": result}
        
        opt_request = AsyncOptimizationRequest(
            task_id=f"benchmark_{worker_id}_{request_num}",
            coroutine_func=optimized_task,
            resource_type=random.choice([ResourceType.IO_BOUND, ResourceType.CPU_BOUND]),
            timeout_s=30
        )
    ...
    async def _run_proxy_benchmark(self) -> BenchmarkResults:
        ...
        for i in range(self.config.concurrent_requests):
            task = asyncio.create_task(
                self._proxy_worker(response_times, i)
            )
            tasks.append(task)
    ...
    async def _run_integrated_benchmark(self) -> BenchmarkResults:
        ...
        for i in range(self.config.concurrent_requests):
            task = asyncio.create_task(
                self._integrated_worker(response_times, i)
            )
            tasks.append(task)
    ...
    def _calculate_benchmark_results(self, test_name: str, response_times: List[float], 
                                   errors: int, total_time: float) -> BenchmarkResults:
        """ベンチマーク結果計算"""
        ...
        average_cpu = statistics.mean(self.cpu_usage_history) if self.cpu_usage_history else 0
    ...
    async def _analyze_performance_comparison(self) -> BenchmarkResults:
        ...
        for test_name, result in self.results.items():
            ...
            improvements[test_name] = {
                "response_time_improvement": (
                    (baseline.average_response_time_ms - result.average_response_time_ms) / 
                    baseline.average_response_time_ms * 100
                ) if baseline.average_response_time_ms > 0 else 0,
        ...
        # 最良の結果を特定
        best_response_time = min((r.average_response_time_ms for r in self.results.values() 
                                if r.average_response_time_ms > 0), default=0)
        best_throughput = max((r.throughput_rps for r in self.results.values()), default=0)
    ...
    async def _generate_benchmark_report(self) -> str:
        ...
        if "comparison" in self.results:
            ...
            overall_improvement = comparison.additional_metrics.get("overall_improvement_percent", 0)
            report_lines.extend([
                "## 🎯 総合評価",
...
# 便利関数
async def run_quick_benchmark(duration_seconds: int = 30, 
                            concurrent_requests: int = 20) -> Dict[str, BenchmarkResults]:
    """クイックベンチマーク実行"""
    ...
    async def main():
        ...
        for test_name, result in results.items():
            if hasattr(result, 'average_response_time_ms'):
                print(f"{test_name}: {result.average_response_time_ms:.2f}ms avg, {result.throughput_rps:.2f} RPS")
