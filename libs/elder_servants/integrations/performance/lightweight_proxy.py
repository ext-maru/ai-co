...
class LightweightElderProxy(EldersServiceLegacy[ProxyRequest, ProxyResponse]):
    ...
    def __init__(self, config: ProxyConfig = None):
        ...
        self.logger.info(
            f"Lightweight Elder Proxy initialized: {self.config.mode.value}"
        )
    ...
    async def process_request(self, request: ProxyRequest) -> ProxyResponse:
        ...
        try:
            ...
            if response.was_compressed:
                ...
                self.metrics.compression_saves_bytes += max(
                    0, estimated_original - response.response_size_bytes
                )
    ...
    async def _process_cached(self, request: ProxyRequest) -> ProxyResponse:
        ...
        cached_result = await cache_manager.get_quality_check_cache(
            request.request_id, f"{request.target_service}_{request.method}"
        )
        ...
        if response.success:
            ...
            # ホットキャッシュ保存
            if (
                request.config.cache_small_responses
                and response.response_size_bytes
                <= request.config.small_response_threshold
            ):
    ...
    async def _process_streaming(self, request: ProxyRequest) -> ProxyResponse:
        ...
        if data_size > request.config.streaming_chunk_size:
            ...
            compressed_data = await self._compress_data(
                response_data.data, request.config.compression_level
            )
        ...
        while True:
            try:
                ...
                request, future = await asyncio.wait_for(
                    self._lazy_queue.get(), timeout=5.0
                )
    ...
    async def get_proxy_metrics(self) -> Dict[str, Any]:
        ...
        avg_response_time = self.metrics.total_response_time_ms / max(
            self.metrics.total_requests, 1
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_healthy = (
                ...
                and metrics["average_response_time_ms"] <= 100.0
            )

            # リソース健全性
            resource_healthy = (
                len(self._hot_cache) < 10000
                and self.metrics.total_bytes_transferred  # ホットキャッシュサイズ制限
                < 1024 * 1024 * 1024  # 1GB制限
            )
    ...
    Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                return ProxyResponse(
                    request_id=request.request_id, success=True, data=result
                )
...
async def streaming_proxy_call(
    service_name: str, method_name: str, payload: Any
) -> Any:
    """ストリーミングプロキシ呼び出し"""
    return await quick_proxy_call(
        service_name, method_name, payload, ProxyMode.STREAMING
    )
