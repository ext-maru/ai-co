...
class StructuredLogger:
    ...
    def _create_log_entry(self, level: LogLevel, message: str, 
                         logger_name: str = "default", **kwargs) -> StructuredLogEntry:
        """ログエントリ作成"""
        # コンテキスト情報統合
        context = {}
    ...
    def critical(self, message: str, logger_name: str = "default", **kwargs):
        """クリティカルログ"""
        entry = self._create_log_entry(LogLevel.CRITICAL, message, logger_name, **kwargs)
        self._write_log(entry)
        ...
        with self.metrics_lock:
            for bucket in buckets:
                if value <= bucket:
                    ...
                    bucket_key = f"{name}_bucket_{hash(str(sorted(bucket_labels.items())))}"
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        with self.metrics_lock:
            summary = {
                ...
                "counters": len([m for m in self.metrics.values() if m.metric_type == MetricType.COUNTER]),
                "gauges": len([m for m in self.metrics.values() if m.metric_type == MetricType.GAUGE]),
                "histograms": len([m for m in self.metrics.values() if m.metric_type == MetricType.HISTOGRAM]),
                "last_update": max([m.timestamp for m in self.metrics.values()]).isoformat() if self.metrics else None
            }
...
class ElderIntegrationMonitor(EldersServiceLegacy[Dict[str, Any], Dict[str, Any]]):
    ...
    def _calculate_logs_per_minute(self) -> float:
        """1分あたりのログ数計算"""
        uptime_minutes = (datetime.now() - self.statistics["start_time"]).total_seconds() / 60
        if uptime_minutes > 0:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 総合判定
            overall_healthy = (
                base_health.get("success", False) and
...
async def record_metric(name: str, value: float, metric_type: str = "gauge", 
