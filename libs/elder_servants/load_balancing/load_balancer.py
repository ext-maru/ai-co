...
class LoadBalancer:
    ...
    async def start(self):
        ...
        self.logger.info(f"Load Balancer started with {len(self.servant_nodes)} servant nodes")
    ...
    async def route_request(self, request: ServantRequest, 
                          strategy: Optional[LoadBalancingStrategy] = None,
                          preferred_domain: Optional[ServantDomain] = None) -> LoadBalancingResult:
        """
        ...
        self.logger.debug(f"Routing request {request.task_id} with strategy {strategy.value}")
        ...
        try:
            ...
            # 代替サーバント取得
            alternatives = [node.servant for node in candidate_nodes 
                          if node != selected_node][:3]  # 上位3つ
            ...
            # ルーティング理由生成
            routing_reason = self._generate_routing_reason(selected_node, strategy, candidate_nodes)
    ...
    async def _get_candidate_nodes(self, request: ServantRequest, 
                                 preferred_domain: Optional[ServantDomain]) -> List[ServantNode]:
        """候補ノード取得"""
        candidates = []
    ...
    async def _round_robin_selection(self, request: ServantRequest, 
                                   candidates: List[ServantNode]) -> ServantNode:
        """ラウンドロビン選択"""
        if not candidates:
            return None
        ...
        domain_candidates = [node for node in candidates if node.servant.domain == domain]
    ...
    async def _least_connections_selection(self, request: ServantRequest, 
                                         candidates: List[ServantNode]) -> ServantNode:
        """最少接続数選択"""
        if not candidates:
            return None
    ...
    async def _weighted_round_robin_selection(self, request: ServantRequest, 
                                            candidates: List[ServantNode]) -> ServantNode:
        """重み付きラウンドロビン選択"""
        if not candidates:
            return None
    ...
    async def _least_response_time_selection(self, request: ServantRequest, 
                                           candidates: List[ServantNode]) -> ServantNode:
        """最短応答時間選択"""
        if not candidates:
            return None
    ...
    async def _hash_based_selection(self, request: ServantRequest, 
                                  candidates: List[ServantNode]) -> ServantNode:
        """ハッシュベース選択"""
        if not candidates:
            return None
    ...
    async def _adaptive_selection(self, request: ServantRequest, 
                                candidates: List[ServantNode]) -> ServantNode:
        """適応型選択"""
        if not candidates:
            return None
        ...
        for node in candidates:
            ...
            if max_response_time > 0:
                response_score = 1.0 - (node.load_metrics.response_time / max_response_time)
            else:
                response_score = 1.0
    ...
    async def _resource_based_selection(self, request: ServantRequest, 
                                      candidates: List[ServantNode]) -> ServantNode:
        """リソースベース選択"""
        if not candidates:
            return None
    ...
    async def _intelligent_selection(self, request: ServantRequest, 
                                   candidates: List[ServantNode]) -> ServantNode:
        """インテリジェント選択（AI統合）"""
        if not candidates:
            return None
        ...
        try:
            ...
            # 簡易タスクプロファイル作成
            from ..selection.servant_auto_selector import TaskProfile, TaskType, SelectionCriteria
            ...
            self.logger.warning(f"Intelligent selection failed, falling back to adaptive: {str(e)}")
            return await self._adaptive_selection(request, candidates)
    
    def _generate_routing_reason(self, selected_node: ServantNode, 
                               strategy: LoadBalancingStrategy, 
                               candidates: List[ServantNode]) -> str:
        """ルーティング理由生成"""
        reasons = []
    ...
    async def record_request_completion(self, servant_name: str, success: bool, 
        ...
        self.logger.debug(f"Recorded completion for {servant_name}: success={success}, time={response_time:.2f}ms")
    ...
    async def _update_response_time_metrics(self, node: ServantNode):
        ...
        if history:
            ...
            if node.total_requests > 0:
                node.load_metrics.error_rate = node.failed_requests / node.total_requests
            
            # スループット計算 (簡易版)
            if len(history) >= 2:
                recent_times = list(history)[-10:]  # 最新10件
                node.load_metrics.throughput = len(recent_times) / sum(recent_times) * 1000  # req/sec
    ...
    async def _update_load_metrics(self, node: ServantNode):
        ...
        # メモリ使用率推定
        node.load_metrics.memory_usage = min(1.0, node.current_requests / node.max_connections)
        ...
        for node in self.servant_nodes.values():
            try:
                ...
                if node.health_status != new_status:
                    self.logger.info(f"Health status changed for {node.servant.name}: {node.health_status.value} -> {new_status.value}")
                    node.health_status = new_status
                
                node.last_health_check = datetime.now()
                
            except Exception as e:
                self.logger.error(f"Health check failed for {node.servant.name}: {str(e)}")
    ...
    def _update_routing_stats(self, strategy: LoadBalancingStrategy, 
    ...
    async def get_load_balancer_status(self) -> Dict[str, Any]:
        ...
        return {
            "system_status": {
                ...
                "healthy_nodes": sum(1 for node in self.servant_nodes.values() 
                                   if node.health_status == HealthStatus.HEALTHY),
                "degraded_nodes": sum(1 for node in self.servant_nodes.values() 
                                    if node.health_status == HealthStatus.DEGRADED),
                "unhealthy_nodes": sum(1 for node in self.servant_nodes.values() 
                                     if node.health_status == HealthStatus.UNHEALTHY),
                "total_active_requests": sum(node.current_requests for node in self.servant_nodes.values()),
            ...
            "node_details": {
                name: {
                    ...
                    "current_requests": node.current_requests,
                    "total_requests": node.total_requests,
                    "success_rate": node.successful_requests / node.total_requests if node.total_requests > 0 else 0,
                    "load_metrics": {
                        "cpu_usage": node.load_metrics.cpu_usage,
                        "memory_usage": node.load_metrics.memory_usage,
                        "response_time": node.load_metrics.response_time,
    ...
    async def set_servant_maintenance(self, servant_name: str, maintenance: bool):
        ...
        if servant_name in self.servant_nodes:
            if maintenance:
                self.servant_nodes[servant_name].health_status = HealthStatus.MAINTENANCE
            else:
