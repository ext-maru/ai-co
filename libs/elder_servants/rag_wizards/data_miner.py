...
class DataMiner(WizardServant):
    ...
    def __init__(self, servant_id: str, name: str, specialization: str):
        ...
        self.supported_data_formats = {
            "csv", "json", "excel", "parquet", "sql"
        }
        
        # 統計メトリクス
        self.available_metrics = {
            "mean", "median", "mode", "std", "var", "min", "max",
            "q1", "q3", "iqr", "skewness", "kurtosis", "correlation"
        }
    ...
    async def process_request(self, request: ServantRequest) -> ServantResponse:
        ...
        try:
            ...
            research_results = await self.research_and_analyze(
                f"data_analysis_{request.data.get('analysis_type', 'statistical')}"
            )
            
            # リクエストデータの取得
            data_source = request.data["data_source"]
            analysis_type = request.data.get("analysis_type", "statistical_summary")
            output_format = request.data.get("output_format", "json")
            metrics = request.data.get("metrics", ["mean", "median", "std"])
            ...
            config = AnalysisConfig(
                ...
                max_categories=request.data.get("max_categories", 50),
                outlier_method=request.data.get("outlier_method", "iqr")
            )
            ...
            # 品質チェック
            quality_score = await self._assess_analysis_quality(analysis_results, config)
            ...
            metadata = {
                ...
                "data_summary": {
                    ...
                    "columns": analysis_results.get("data_info", {}).get("column_count", 0)
                }
            }
            ...
            return ServantResponse(
                ...
                metrics={
                    ...
                    "quality_score": quality_score,
                    "data_points_analyzed": analysis_results.get("data_info", {}).get("row_count", 0)
                }
            )
    ...
    async def _perform_analysis(self, data_source: Dict[str, Any], config: AnalysisConfig, context: Dict[str, Any]) -> Dict[str, Any]:
        """メインのデータ分析ロジック"""
        try:
            # データの読み込み
            ...
            if config.analysis_type == "statistical_summary":
                analysis_results = await self._analyze_statistical_summary(data_source, config.metrics, context)
            elif config.analysis_type == "trend_analysis":
                analysis_results = await self._analyze_trend_analysis(data_source, context.get("analysis_config", {}), context)
            elif config.analysis_type == "correlation_analysis":
                analysis_results = await self._analyze_correlation(data_source, context.get("analysis_config", {}), context)
                ...
                analysis_results = await self._analyze_anomaly_detection(df, config, context)
            else:
            ...
            return {
                "error": str(e),
                "data_info": {"row_count": 0, "column_count": 0}
            }
    ...
    async def _analyze_statistical_summary(self, data_source: Dict[str, Any], metrics: List[str], context: Dict[str, Any]) -> Dict[str, Any]:
        """統計サマリー分析"""
        ...
        if "correlation" in metrics and len(numeric_columns) > 1:
            results["correlations"] = self._calculate_correlations(df[numeric_columns], "pearson")
        
        return results

    async def _analyze_trend_analysis(self, data_source: Dict[str, Any], config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """トレンド分析"""
        df = self._load_csv_data(data_source)
        
        time_column = config.get("time_column", df.columns[0])
        value_columns = config.get("value_columns", [col for col in df.columns if col != time_column])
    ...
    async def _analyze_correlation(self, data_source: Dict[str, Any], config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """相関分析"""
        df = self._load_csv_data(data_source)
        ...
        if len(variables) > 1:
            correlation_results = self._calculate_correlations(df[variables], correlation_method)
    ...
    async def _analyze_clustering(self, df: pd.DataFrame, config: AnalysisConfig, context: Dict[str, Any]) -> Dict[str, Any]:
        """クラスタリング分析"""
        numeric_columns = df.select_dtypes(include=[np.number]).columns
    ...
    async def _analyze_anomaly_detection(self, df: pd.DataFrame, config: AnalysisConfig, context: Dict[str, Any]) -> Dict[str, Any]:
        """異常検知分析"""
        numeric_columns = df.select_dtypes(include=[np.number]).columns
        anomalies = {}
        ...
        return {
            ...
            "summary": {
                ...
                "affected_columns": len([col for col, info in anomalies.items() if info["count"] > 0])
            }
        }

    async def _analyze_generic(self, df: pd.DataFrame, config: AnalysisConfig, context: Dict[str, Any]) -> Dict[str, Any]:
        """汎用分析"""
        return {
            "basic_info": {
                "shape": df.shape,
    ...
    def _calculate_descriptive_statistics(self, df: pd.DataFrame, metrics: List[str]) -> Dict[str, Dict[str, float]]:
        """記述統計を計算"""
        results = {}
        numeric_columns = df.select_dtypes(include=[np.number]).columns
    ...
    def _calculate_correlations(self, df: pd.DataFrame, method: str = "pearson") -> Dict[str, Any]:
        """相関を計算"""
    ...
    def _perform_trend_analysis(self, df: pd.DataFrame, time_column: str, value_columns: List[str]) -> Dict[str, Dict[str, Any]]:
        """トレンド分析を実行"""
        trends = {}
    ...
    def _forecast_values(self, dates: np.ndarray, values: np.ndarray, periods: int) -> List[float]:
        """値を予測"""
        if len(values) < 2:
    ...
    async def _assess_analysis_quality(self, analysis_results: Dict[str, Any], config: AnalysisConfig) -> float:
        """分析品質を評価"""
        try:
            score = 0.0
            max_score = 100.0
    ...
    async def _generate_insights(self, analysis_results: Dict[str, Any], config: AnalysisConfig) -> List[str]:
        """分析結果からインサイトを生成"""
        insights = []
        ...
        try:
            ...
            if "summary_statistics" in analysis_results:
                ...
                for column, column_stats in stats.items():
                    if "mean" in column_stats and "std" in column_stats:
                        cv = column_stats["std"] / column_stats["mean"] if column_stats["mean"] != 0 else 0
                        if cv > 1:
                            insights.append(f"{column} shows high variability (CV = {cv:.2f})")
            ...
            if "trends" in analysis_results:
                ...
                increasing_trends = [col for col, trend in trends.items() if trend.get("trend_direction") == "increasing"]
                if increasing_trends:
                    insights.append(f"Increasing trends detected in: {', '.join(increasing_trends)}")
            
            # データ品質のインサイト
            if "data_quality" in analysis_results:
                quality = analysis_results["data_quality"]
                if quality.get("completeness_score", 1) < 0.9:
                    insights.append(f"Data completeness is {quality['completeness_score']*100:.1f}% - consider data cleaning")
    ...
    async def research_and_analyze(self, topic: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "statistical" in topic.lower():
                ...
                recommendations.extend([
                    "use_multiple_forecasting_models",
                ...
                recommendations.extend([
                    ...
                    "consider_confounding_variables"
                ])
