...
class ServantAutoSelector:
    ...
    async def select_optimal_servant(self, task_profile: TaskProfile, 
        ...
        try:
            ...
            # 代替案作成
            alternatives = sorted(
                [(servant, score) for servant, score in scored_candidates if servant != best_servant],
            ...
            # 実行時間推定
            estimated_time = await self._estimate_execution_time(best_servant, task_profile)
            ...
            # 選択履歴記録
            await self._record_selection_history(task_profile, selection_result, start_time)
            
            # 統計更新
            self._update_selection_stats(criteria, best_score)
            
            self.logger.info(f"Selected servant: {best_servant.name} (score: {best_score:.3f}, confidence: {confidence:.3f})")
            return selection_result
            
        except Exception as e:
            self.logger.error(f"Servant selection failed for task {task_profile.task_id}: {str(e)}")
            raise
    ...
    async def _update_servant_profiles(self):
        ...
        # キャッシュ有効性チェック
        if (current_time - self.last_cache_update) < self.cache_expiry and self.servant_profiles_cache:
            return
        ...
        self.logger.debug(f"Updated {len(self.servant_profiles_cache)} servant profiles")
    
    async def _create_servant_profile(self, servant: ElderServantBase, servant_info: Dict[str, Any]) -> ServantProfile:
        """個別サーバントプロファイル作成"""
    ...
    async def _calculate_specialization_scores(self, servant: ElderServantBase) -> Dict[ServantCapability, float]:
        """専門化スコア計算"""
    ...
    def _calculate_availability_score(self, servant: ElderServantBase, metrics: Dict[str, Any]) -> float:
        """可用性スコア計算"""
        # 成功率ベース
    ...
    async def _get_candidate_servants(self, task_profile: TaskProfile) -> List[ServantProfile]:
        """候補サーバント取得"""
        candidates = []
        
        # 優先ドメインが指定されている場合
        if task_profile.preferred_domain:
            domain_servants = self.registry.find_by_domain(task_profile.preferred_domain)
    ...
    async def _calculate_selection_scores(self, task_profile: TaskProfile, 
                                        candidates: List[ServantProfile], 
                                        criteria: SelectionCriteria) -> List[Tuple[ElderServantBase, float]]:
        """選択スコア計算"""
        scored_candidates = []
        
        for profile in candidates:
            score = await self._calculate_individual_score(task_profile, profile, criteria)
            scored_candidates.append((profile.servant, score))
        
        return scored_candidates
    
    async def _calculate_individual_score(self, task_profile: TaskProfile, 
    ...
    def _calculate_performance_score(self, task_profile: TaskProfile, profile: ServantProfile) -> float:
        """パフォーマンススコア計算"""
        # 処理速度
    ...
    def _calculate_quality_score(self, task_profile: TaskProfile, profile: ServantProfile) -> float:
        """品質スコア計算"""
    ...
    def _calculate_expertise_score(self, task_profile: TaskProfile, profile: ServantProfile) -> float:
        """専門性スコア計算"""
        expertise_scores = []
    ...
    def _get_criteria_weights(self, criteria: SelectionCriteria) -> Dict[str, float]:
        ...
        weights_map = {
            SelectionCriteria.PERFORMANCE: {
                "performance": 0.5, "availability": 0.2, "load_balance": 0.2, "expertise": 0.1
            },
            SelectionCriteria.QUALITY: {
                "quality": 0.5, "expertise": 0.3, "performance": 0.2
            },
            SelectionCriteria.AVAILABILITY: {
                "availability": 0.5, "load_balance": 0.3, "performance": 0.2
            },
            SelectionCriteria.EXPERTISE: {
                "expertise": 0.6, "quality": 0.25, "performance": 0.15
            },
            SelectionCriteria.LOAD_BALANCE: {
                "load_balance": 0.5, "availability": 0.3, "performance": 0.2
            },
            SelectionCriteria.COST_EFFICIENCY: {
                "cost_efficiency": 0.4, "performance": 0.3, "load_balance": 0.3
            }
        }
        
        return weights_map.get(criteria, {
            "performance": 0.25, "quality": 0.25, "availability": 0.2, 
            "expertise": 0.2, "load_balance": 0.1
        })
    
    def _apply_model_weights(self, scores: Dict[str, float], task_profile: TaskProfile) -> float:
        """学習済みモデル重み適用"""
        weighted_score = 0.0
    ...
    def _get_task_type_bonus(self, task_type: TaskType, scores: Dict[str, float]) -> float:
        """タスクタイプボーナス"""
        bonus_map = {
    ...
    def _calculate_confidence(self, best_score: float, all_scores: List[Tuple[ElderServantBase, float]]) -> float:
        """選択信頼度計算"""
        if len(all_scores) <= 1:
            return 1.0
        ...
        return (gap_confidence * 0.4 + variance_confidence * 0.3 + absolute_confidence * 0.3)
    
    async def _estimate_execution_time(self, servant: ElderServantBase, task_profile: TaskProfile) -> float:
        """実行時間推定"""
        metrics = servant.get_metrics()
    ...
    async def _generate_selection_reasoning(self, servant: ElderServantBase, 
        ...
        if profile:
            ...
            if criteria == SelectionCriteria.PERFORMANCE:
                ...
                avg_quality = sum(profile.quality_scores.values()) / len(profile.quality_scores) if profile.quality_scores else 0
                reasoning["key_factors"].append(f"品質スコア: {avg_quality:.1f}")
                
            elif criteria == SelectionCriteria.AVAILABILITY:
                reasoning["key_factors"].append(f"可用性スコア: {profile.availability_score:.2f}")
            ...
            max_expertise = max(profile.specialization_score.values()) if profile.specialization_score else 0
    ...
    async def _record_selection_history(self, task_profile: TaskProfile, 
                                      selection_result: SelectionResult, 
                                      start_time: float):
        """選択履歴記録"""
    ...
    async def provide_feedback(self, task_id: str, servant_name: str, 
    ...
    async def get_selection_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_status": {
                ...
                "cache_valid": (datetime.now() - self.last_cache_update) < self.cache_expiry
            }
        }
    
    # 便利メソッド
    async def quick_select(self, task_name: str, task_type: TaskType, 
                         required_capabilities: List[ServantCapability],
                         priority: str = "medium") -> ElderServantBase:
        """クイック選択（簡易インターフェース）"""
        task_profile = TaskProfile(
