...
class ElderFlowEngine(EldersFlowLegacy):
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        """エンジン機能情報"""
        return {
            "name": "Elder Flow Engine with PID Lock",
            "version": "1.1.0",
            "capabilities": [
...
async def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "status":
            ...
            result = {"error": "使用法: elder_flow_engine.py [status|cleanup|active|execute <task_name>]"}
    else:
        result = {"error": "コマンドを指定してください: status, cleanup, active, execute <task_name>"}
