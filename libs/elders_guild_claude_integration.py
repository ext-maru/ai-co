...
from .elders_guild_development_context import EldersDevelopmentContext, ClaudeContextBuilder
...
class ClaudeUsageOptimizer:
    ...
    def select_optimal_model(self, task_complexity: str, context_length: int) -> ClaudeModel:
        """タスクに最適なモデルを選択"""
        # コンテキストが短く、タスクが単純な場合
    ...
    def estimate_cost(self, model: ClaudeModel, input_tokens: int, output_tokens: int) -> float:
        """コスト推定"""
        # 概算価格（実際の価格は変動する可能性）
        pricing = {
...
"""
    ...
    async def _convene_sage_council(self, understanding: Dict[str, Any]) -> Dict[str, Any]:
        """4賢者会議を開催"""
        council_results = {}
    ...
    async def _create_execution_plan(self, council_decision: Dict[str, Any]) -> Dict[str, Any]:
        """実行計画を策定"""
        # 実装省略
        return {
            "steps": ["ステップ1", "ステップ2"],
