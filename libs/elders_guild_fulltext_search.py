...
class LanguageDetector:
    ...
    def detect_language(self, text: str) -> TextLanguage:
        ...
        # 日本語判定
        if (japanese_ratio >= self.config.japanese_char_threshold or
            hiragana_katakana_ratio >= self.config.hiragana_katakana_threshold):
...
class JapaneseTokenizer:
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        elif self.config.japanese_tokenizer == TokenizerType.SUDACHI and self.sudachi_tokenizer:
...
class FullTextSearchEngine:
    ...
    def __init__(self, config: FullTextSearchConfig, db_manager: EldersGuildDatabaseManager):
        self.config = config
    ...
    async def search(self, query: str,
                    ...
                    metadata_filter: Optional[Dict[str, Any]] = None) -> List[FullTextSearchResult]:
        """全文検索"""
        ...
        # キャッシュチェック
        cache_key = self._generate_cache_key(query, language, max_results, category, tags, metadata_filter)
        ...
        # 検索実行
        results = await self._execute_search(query, language, max_results, category, tags, metadata_filter)
    ...
    async def _execute_search(self, query: str, language: TextLanguage,
                            max_results: int, category: Optional[str],
                            tags: Optional[List[str]],
                            metadata_filter: Optional[Dict[str, Any]]) -> List[FullTextSearchResult]:
        """検索実行"""
        # 混合言語の場合は両方の言語で検索
        if self.language_detector.is_mixed_language(query):
            japanese_results = await self._search_by_language(query, TextLanguage.JAPANESE, max_results, category, tags, metadata_filter)
            english_results = await self._search_by_language(query, TextLanguage.ENGLISH, max_results, category, tags, metadata_filter)

            # 結果をマージ
            results = self._merge_results(japanese_results, english_results)
            return results[:max_results]
        else:
            return await self._search_by_language(query, language, max_results, category, tags, metadata_filter)

    async def _search_by_language(self, query: str, language: TextLanguage,
                                max_results: int, category: Optional[str],
                                tags: Optional[List[str]],
                                metadata_filter: Optional[Dict[str, Any]]) -> List[FullTextSearchResult]:
        """言語別検索"""
        async with self.db_manager.db_manager.get_connection() as conn:
            ...
            for result in results:
                ...
                # マッチしたトークンの抽出
                matched_tokens = self._extract_matched_tokens(query, knowledge, language)
    ...
    def _extract_matched_tokens(self, query: str, knowledge: KnowledgeEntity, language: TextLanguage) -> List[str]:
        """マッチしたトークンの抽出"""
        if language == TextLanguage.JAPANESE:
    ...
    def _merge_results(self, japanese_results: List[FullTextSearchResult],
                      english_results: List[FullTextSearchResult]) -> List[FullTextSearchResult]:
        """検索結果のマージ"""
        # IDでグループ化
        result_map = {}
    ...
    def _generate_cache_key(self, query: str, language: TextLanguage, max_results: int,
                          category: Optional[str], tags: Optional[List[str]],
                          metadata_filter: Optional[Dict[str, Any]]) -> str:
        """キャッシュキーの生成"""
        import hashlib
    ...
    async def fuzzy_search(self, query: str, max_results: Optional[int] = None) -> List[FullTextSearchResult]:
        """ファジー検索"""
        ...
        async with self.db_manager.db_manager.get_connection() as conn:
            ...
            results = await conn.fetch(fuzzy_query, query, self.config.fuzzy_threshold, max_results)
    ...
    async def advanced_search(self, query: str,
                            ...
                            max_results: Optional[int] = None) -> List[FullTextSearchResult]:
        """高度な検索"""
        ...
        async with self.db_manager.db_manager.get_connection() as conn:
            ...
            for field in search_fields:
                if field in boost_factors:
                    ...
                    if field == 'title':
                        search_expressions.append(f"ts_rank(search_vector, plainto_tsquery('japanese', $1)) * {boost}")
                        search_expressions.append(f"ts_rank(search_vector, plainto_tsquery('english', $1)) * {boost}")
                    elif field == 'content':
                        search_expressions.append(f"ts_rank(search_vector, plainto_tsquery('japanese', $1)) * {boost}")
                        search_expressions.append(f"ts_rank(search_vector, plainto_tsquery('english', $1)) * {boost}")
            ...
            results = await conn.fetch(advanced_query, query, self.config.min_score, max_results)
    ...
    async def get_search_statistics(self) -> Dict[str, Any]:
        ...
        async with self.db_manager.db_manager.get_connection() as conn:
            ...
            # 総文書数
            total_docs = await conn.fetchval("SELECT COUNT(*) FROM knowledge_sage.knowledge_entities")
            stats['total_documents'] = total_docs

            # 検索ベクターが設定されている文書数
            indexed_docs = await conn.fetchval("SELECT COUNT(*) FROM knowledge_sage.knowledge_entities WHERE search_vector IS NOT NULL")
...
async def main():
    ...
    try:
        ...
        for result in results:
            print(f"- {result.knowledge.title} (score: {result.relevance_score:.3f}, lang: {result.language.value})")

        # 英語検索
        english_results = await search_engine.search("database optimization", language=TextLanguage.ENGLISH)
