...
class KnowledgeSageIntegration:
    ...
    async def store_knowledge(
        self, knowledge_data: Dict[str, Any], context: Dict[str, Any]
    ) -> str:
        """
        知識永続化
    ...
    async def retrieve_knowledge(
        ...
        try:
            ...
            cached_results = await self.hybrid_storage.get_cached_search_results(
                query, context
            )
            ...
            self.knowledge_metrics["average_search_time"] = (
                ...
                * (self.knowledge_metrics["total_searches"] - 1)
                + processing_time
            ) / self.knowledge_metrics["total_searches"]
            self.status["last_activity"] = datetime.now().isoformat()
    ...
    async def evolve_knowledge(
        self, knowledge_id: str, evolution_data: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        知識進化
        ...
        try:
            # セキュリティチェック
            if not await self.security_layer.check_permission(
                context, "knowledge_evolve"
            ):
                raise PermissionError("Knowledge evolution permission denied")

            # 知識進化処理
            evolution_result = await self.knowledge_base.evolve_knowledge(
                knowledge_id, evolution_data
            )

            # HybridStorageに進化履歴保存
            await self.hybrid_storage.store_knowledge_evolution(
                knowledge_id, evolution_data, context
            )
    ...
    async def _process_knowledge(
        ...
        # 知識重要度評価
        processed_data["importance_score"] = await self._calculate_importance_score(
            processed_data
        )
    ...
    async def _combine_search_results(
        self, search_results: List[Dict[str, Any]], cached_results: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        検索結果統合
    ...
    async def _calculate_search_confidence(
        ...
        average_score = sum(result.get("score", 0.0) for result in top_results) / len(
            top_results
        )
    ...
    async def _handle_session_deleted(
        self, session_id: str, notification_data: Dict[str, Any]
    ):
        """セッション削除通知処理"""
        # セッション知識のクリーンアップ
