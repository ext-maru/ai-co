        ...
        while True:
            try:
                for worker_name, worker_def in WORKER_DEFINITIONS.items():
                    ...
                    # キュー積滞チェック
                    if queue_size > 100:
                        logger.warning(f"🚨 {self.name}: {queue_name}キューが積滞 ({queue_size}件)")
    ...
    def detect_worker_bottlenecks(self) -> Dict[str, Any]:
        ...
        for worker_name, status in self.forest.worker_statuses.items():
            ...
            if status.cpu_percent > 80:
                bottlenecks[worker_name] = {'type': 'high_cpu', 'value': status.cpu_percent}
                
            # メモリ使用量が多い
            if status.memory_mb > 500:
                bottlenecks[worker_name] = {'type': 'high_memory', 'value': status.memory_mb}
                
            # キューが詰まっている
            if status.queue_size > 100:
                bottlenecks[worker_name] = {'type': 'queue_backlog', 'value': status.queue_size}
        ...
        while True:
            try:
                for worker_name, status in self.forest.worker_statuses.items():
                    # メモリリークチェック
                    if status.memory_mb > 500:
                        logger.warning(f"💊 {self.name}: {worker_name}のメモリ使用量が高い ({status.memory_mb}MB)")
...
class ElfForestWorkerManager:
    ...
    async def start_worker(self, worker_name: str):
        ...
        log_path = PROJECT_ROOT / 'logs' / f'{worker_name}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    ...
    def display_dashboard(self):
        ...
        for worker_name, status in self.worker_statuses.items():
            ...
            print(f"│ {worker_name:<18} │ {state_icon}{status.status:<6} │{status.cpu_percent:4.0f}%│{status.memory_mb:6.0f}MB│{status.queue_size:9}│")
