...
class ErrorHistory:
    ...
    def record_error(self, function_name: str, error: Exception, 
...
def smart_retry(strategy: Optional[RetryStrategy] = None,
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            while attempt < strategy.max_attempts:
                try:
                    ...
                    if not retryable:
                        logger.error(f"Non-retryable error in {func.__name__}: {error_type} - {e}")
                        raise
                    ...
                    if attempt >= current_strategy.max_attempts:
                        logger.error(f"Max retries ({current_strategy.max_attempts}) exceeded for {func.__name__}")
                        raise
                    
                    # 待機時間計算
                    delay = current_strategy.get_delay(attempt - 1)
                    logger.warning(f"Error in {func.__name__} (attempt {attempt}/{current_strategy.max_attempts}): "
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if self._failure_count >= self.failure_threshold:
                    self._state = 'OPEN'
                    logger.error(f"Circuit breaker for {func.__name__} is OPEN after {self._failure_count} failures")
