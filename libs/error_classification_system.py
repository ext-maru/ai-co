...
class ErrorPattern:
    ...
    def extract(cls, error_text: str) -> "ErrorPattern":
        ...
        if any(
            ...
            for keyword in ["api key", "invalid key", "unauthorized"]
        ):
            return cls(
        ...
        # データベースエラー
        if any(
            keyword in text_lower for keyword in ["database", "connection", "sql", "db"]
        ):
            return cls(
        ...
        # タイムアウトエラー
        if any(
            keyword in text_lower for keyword in ["timeout", "timed out", "time out"]
        ):
            return cls(
        ...
        # ワーカーエラー
        if any(
            keyword in text_lower for keyword in ["worker", "process", "died", "failed"]
        ):
            return cls(
...
class ErrorAnalyzer:
    ...
    def analyze_trends(
        self, errors: List[Dict[str, Any]], window: str = "1h"
    ) -> Dict[str, Any]:
        """エラートレンドを分析"""
...
class ErrorClassificationSystem:
    ...
    def __init__(self, model_path: Optional[Path] = None):
        """エラー分類システム初期化"""
        self.model_path = (
            model_path or PROJECT_ROOT / "models" / "error_classification_model.pkl"
        )
    ...
    def _determine_priority(
        self, category: ErrorCategory, message: str, source: str
    ) -> str:
        """優先度の決定"""
    ...
    def _generate_suggested_actions(
        self, category: ErrorCategory, message: str
    ) -> List[str]:
        """推奨アクションの生成"""
        action_map = {
    ...
    def load_errors_from_log(
        self, log_path: Path, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """ログファイルからエラーを読み込み"""
        errors = []
    ...
    def incremental_train(self, new_errors: List[Dict[str, Any]]) -> None:
        ...
        if training_data:
            ...
            logger.info(
                f"Incremental training completed with {len(training_data)} samples"
            )
