...
class ErrorPatternPredictor(BaseManager):
    ...
    def _predict_temporal_patterns(self, current_time: datetime) -> List[ErrorPrediction]:
        """時間的パターンに基づく予測"""
        predictions = []
    ...
    def _predict_causal_patterns(self, context: Optional[Dict]) -> List[ErrorPrediction]:
        """因果関係に基づく予測"""
        predictions = []
                ...
                for row in cursor:
                    ...
                    if avg_delay <= self.prediction_horizon_hours * 60:
                        ...
                        prediction = ErrorPrediction(
                            ...
                            probability=probability,
                            confidence=self._calculate_confidence(correlation, 'causal'),
    ...
    def _predict_environmental_patterns(self, context: Optional[Dict]) -> List[ErrorPrediction]:
        """環境パターンに基づく予測"""
        predictions = []
    ...
    def _learn_from_occurrence(self, error_hash: str, error_info: Dict, 
                              context: Dict, occurred_at: datetime):
        """エラー発生から学習"""
        # 時間的パターンの学習
    ...
    def _learn_temporal_pattern(self, error_info: Dict, occurred_at: datetime):
        ...
        # 時間帯別の発生頻度を更新
        pattern_key = f"temporal_{error_type}_{occurred_at.weekday()}_{occurred_at.hour}"
        ...
        with sqlite3.connect(self.db_path) as conn:
            for event in recent_events[-5:]:  # 直近5イベントを確認
                ...
                if row:
                    ...
                    new_correlation = min(
                        current_correlation + (1 - current_correlation) * 0.1,
                        0.99
                    )
    ...
    def _generate_preventive_actions(self, error_type: str, 
                                   pattern_type: str, 
    ...
    def _merge_predictions(self, predictions: List[ErrorPrediction]) -> List[ErrorPrediction]:
        """重複する予測をマージ"""
        merged = {}
    ...
    def _record_prediction(self, prediction: ErrorPrediction):
        """予測を記録"""
        prediction_id = f"pred_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{prediction.error_type}"
    ...
    for pred in predictions:
        ...
        if pred.preventive_actions:
            ...
            for action in pred.preventive_actions:
                print(f"  - {action['action']}: {action['command']} (Priority: {action['priority']})")
