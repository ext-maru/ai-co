...
class DataExtractor:
    ...
    async def extract_from_logs(self, log_pattern: str = "*.log") -> AsyncIterator[Dict]:
        """ログファイルからデータ抽出"""
...
class DataTransformer:
    ...
    async def transform(self, data: pd.DataFrame, rules: Dict[str, Any]) -> pd.DataFrame:
        """ルールベースデータ変換"""
        result = data.copy()
...
class ETLPipeline:
    ...
    async def _load_phase(self, job: ETLJob, data: Any):
        ...
        if target_type == 'database':
            ...
            filename = target_parts[1] if len(target_parts) > 1 else f"{job.job_id}.json"
