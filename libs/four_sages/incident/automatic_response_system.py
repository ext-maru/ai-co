...
from libs.four_sages.incident.incident_sage import (
    IncidentEntry, IncidentCategory, IncidentSeverity, IncidentAction
)
...
class AutomaticResponseSystem(EldersServiceLegacy):
    ...
    def _configure_default_rules(self):
        ...
        default_rules = [
            ResponseRule(
                ...
                conditions=["service_failure", "service_unresponsive", "health_check_failed"],
            ...
            ResponseRule(
                ...
                conditions=["high_cpu_usage", "high_memory_usage", "resource_exhaustion"],
            ...
            ResponseRule(
                ...
                conditions=["deployment_failure", "post_deploy_errors", "version_incompatibility"],
            ...
            ResponseRule(
                ...
                conditions=["cache_corruption", "performance_degradation", "memory_pressure"],
    ...
    async def _match_rules(self, incident: IncidentEntry) -> List[ResponseRule]:
        ...
        # カテゴリベースの条件
        category_conditions = {
            IncidentCategory.SYSTEM_FAILURE: ["service_failure", "system_error", "crash"],
            IncidentCategory.PERFORMANCE_ISSUE: ["high_load", "slow_response", "performance_degradation"],
            IncidentCategory.NETWORK_ISSUE: ["network_error", "connection_failure", "timeout"],
            IncidentCategory.SECURITY_BREACH: ["security_alert", "unauthorized_access", "intrusion"],
            IncidentCategory.DATA_CORRUPTION: ["data_error", "corruption", "integrity_failure"],
            IncidentCategory.CONFIGURATION_ERROR: ["config_error", "misconfiguration", "deployment_failure"]
        }
    ...
    async def _execute_rule(self, rule: ResponseRule, incident: IncidentEntry) -> ResponseExecution:
        """ルール実行"""
    ...
    async def _execute_action_with_retry(self, action_func: Callable, 
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.info(f"🔄 Retrying action (attempt {attempt + 2}/{max_retries})")
    ...
    async def _record_response_to_incident(self, incident: IncidentEntry, 
                                         execution: ResponseExecution, 
                                         rule: ResponseRule):
        """インシデントに対応記録を追加"""
        try:
    ...
    async def _update_effectiveness_metrics(self, executions: List[ResponseExecution]):
        ...
        for execution in executions:
            ...
            if execution.completed_at:
                response_time = (execution.completed_at - execution.started_at).total_seconds()
                ...
                self.effectiveness_metrics["average_response_time"] = (
                    (current_avg * (total - len(executions)) + response_time) / total
                )
...
# エクスポート
__all__ = ['AutomaticResponseSystem', 'ResponseRule', 'ResponseExecution', 'ResponseStatus']