...
from libs.four_sages.incident.incident_sage import (
    IncidentSage, IncidentEntry, IncidentSeverity, 
    IncidentCategory, IncidentStatus, AlertLevel
)
...
class EnhancedIncidentSage(IncidentSage):
    ...
    async def _create_incident_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result.get("success"):
            ...
            if self.integration_config["auto_response_enabled"]:
                ...
                if incident:
                    response_result = await self.response_system.handle_incident(incident)
        ...
        try:
            ...
            if self.integration_config["preventive_alerts_enabled"]:
                ...
                if alert_result.get("alerts_generated", 0) > 0:
                    self.integration_metrics["preventive_alerts"] += alert_result["alerts_generated"]
    ...
    async def _pattern_analysis_loop(self):
        ...
        while True:
            try:
                ...
                # パターン分析実行
                result = await self.failure_detector.analyze_historical_failures(days_back=7)
                
                if result.get("success"):
                    patterns_found = result.get("patterns_found", 0)
                    self.integration_metrics["patterns_detected"] += patterns_found
                    
                    logger.info(f"✅ Pattern analysis completed: {patterns_found} patterns found")
    ...
    async def _handle_high_risk_prediction(self, prediction):
        ...
        try:
            ...
            if incident_response.get("success"):
                ...
                logger.warning(f"🔮 Preventive incident created: {incident_response['incident_id']}")
                
        except Exception as e:
            logger.error(f"❌ High risk prediction handling failed: {e}")
    
    # 新しいリクエストハンドラー
    async def _analyze_patterns_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """パターン分析リクエスト"""
    ...
    async def _get_integration_status_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """統合ステータス取得リクエスト"""
        return {
            "success": True,
            "integration_status": {
    ...
    async def _configure_integration_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """統合設定リクエスト"""
        ...
        return {
            "success": True,
            "updated_config": self.integration_config
        }
