...
class FailurePatternDetector(EldersAILegacy):
    ...
    def __init__(self, tracking_db_path: str = "elder_flow_tracking.db",
        ...
        super().__init__(
            name="FailurePatternDetector",
            model_type="pattern-detection-v1"
        )
    ...
    async def _extract_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """失敗パターン抽出"""
        patterns = []
    ...
    async def _extract_error_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """エラーメッセージパターン抽出"""
        ...
        for pattern_name, failures_list in error_groups.items():
            if len(failures_list) >= self.pattern_threshold:
                pattern = {
                    ...
                    "common_stderr": self._find_common_substring([f["stderr"] for f in failures_list])
                }
                patterns.append(pattern)
        
        return patterns
    
    async def _extract_time_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """時系列パターン抽出"""
        patterns = []
    ...
    async def _extract_resource_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """リソース使用パターン抽出"""
        patterns = []
        
        # 実行時間異常パターン
        execution_times = [f["execution_time"] for f in failures if f["execution_time"] > 0]
        if execution_times:
            ...
            if len(slow_failures) >= self.pattern_threshold:
                pattern = {
                    ...
                    "average_execution_time": sum(f["execution_time"] for f in slow_failures) / len(slow_failures),
    ...
    async def _extract_dependency_patterns(self, failures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """依存関係パターン抽出"""
        patterns = []
    ...
    def _find_related_failures(self, task_id: str, all_failures: List[Dict[str, Any]]) -> List[str]:
        """関連する失敗を検出"""
        related = set()
    ...
    def _find_common_substring(self, strings: List[str]) -> str:
        ...
        # 最も頻出する単語を返す
        common_words = [word for word, count in words_counter.most_common(5) 
    ...
    async def _evaluate_pattern_severity(self, pattern: Dict[str, Any]) -> IncidentSeverity:
        """パターン重要度評価"""
    ...
    async def _update_pattern_registry(self, patterns: List[Dict[str, Any]]):
        ...
        for pattern_data in patterns:
            pattern_id = f"{pattern_data['pattern_type']}_{pattern_data['pattern_name']}"
            ...
            if pattern_id in self.pattern_registry:
                ...
                pattern = FailurePattern(
                    ...
                    category=pattern_data.get("category", IncidentCategory.SYSTEM_FAILURE),
    ...
    async def predict_failure(self, current_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern_id, pattern in self.pattern_registry.items():
                probability = await self._calculate_failure_probability(pattern, current_metrics)
    ...
    async def _calculate_failure_probability(self, pattern: FailurePattern, 
        ...
        # 最近の発生頻度を考慮
        time_since_last = (datetime.now() - pattern.last_seen).total_seconds() / 3600  # 時間単位
