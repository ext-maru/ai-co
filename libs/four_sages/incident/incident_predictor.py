...
from libs.four_sages.incident.failure_pattern_detector import FailurePatternDetector, FailurePattern
from libs.four_sages.incident.incident_sage import IncidentSeverity, IncidentCategory
...
class FeatureExtractor:
    ...
    def __init__(self):
        self.feature_names = [
            ...
            "disk_usage", "network_latency", "active_connections",
            "queue_depth", "failure_count_1h", "failure_count_24h",
            "pattern_match_count", "time_of_day", "day_of_week",
            "is_business_hours", "recent_deployment"
        ]
    ...
    def get_feature_importance(self, importances: np.ndarray) -> List[Tuple[str, float]]:
        """特徴量の重要度を取得"""
...
class IncidentPredictor(EldersAILegacy):
    ...
    def __init__(self, pattern_detector: FailurePatternDetector):
        super().__init__(
            name="IncidentPredictor",
            model_type="prediction-v1"
        )
    ...
    async def train_model(self, historical_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """予測モデル訓練"""
        try:
            ...
            logger.info(f"✅ Model training completed - Category: {cat_accuracy:.2%}, Severity: {sev_accuracy:.2%}")
    ...
    async def predict_incident_risk(self, current_state: Dict[str, Any]) -> PredictionResult:
        """インシデントリスク予測"""
        try:
            # キャッシュチェック
            ...
            if cache_key in self.prediction_cache:
                ...
                if (datetime.now() - cached.prediction_timestamp).seconds < self.cache_ttl:
                    return cached
            ...
            # リスクスコア計算
            risk_score = self._calculate_risk_score(incident_probability, severity_probability)
    ...
    def _calculate_risk_score(self, incident_prob: Dict[str, float], 
                            severity_prob: Dict[str, float]) -> float:
        """リスクスコア計算"""
    ...
    def _analyze_contributing_factors(self, feature: FeatureVector, 
                                    X: np.ndarray) -> List[Tuple[str, float]]:
        """寄与要因分析"""
        ...
        for i, (name, importance) in enumerate(factors[:10]):  # 上位10個
            ...
            if name == "error_rate" and value > 0.05:
                ...
                contributing.append((f"Recent failures: {int(value)} in 1h", importance))
        
        return contributing[:5]  # 上位5つの要因
    
    async def _generate_recommendations(self, risk_level: str, 
                                      incident_prob: Dict[str, float],
                                      factors: List[Tuple[str, float]]) -> List[str]:
        """推奨アクション生成"""
        recommendations = []
        ...
        if likely_category != "none":
            ...
            recommendations.extend(
                category_recommendations.get(likely_category, [])
            )
    ...
    def _calculate_confidence(self, category_proba: np.ndarray, 
                            severity_proba: np.ndarray) -> float:
        """予測信頼度計算"""
        # エントロピーベースの信頼度
        # 確率分布が偏っているほど信頼度が高い
    ...
    async def _predict_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # パターン検出結果を含める
        patterns = await self.pattern_detector.process_request({
            "type": "get_patterns"
        })
