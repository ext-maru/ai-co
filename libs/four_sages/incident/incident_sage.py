...
class IncidentSage(BaseSage):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # インデックス作成
            conn.execute("CREATE INDEX IF NOT EXISTS idx_incident_severity ON incidents(severity)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_incident_status ON incidents(status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_incident_category ON incidents(category)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_incident_created ON incidents(created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_metrics_name ON system_metrics(metric_name)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_metrics_collected ON system_metrics(collected_at)")
        ...
        try:
            ...
            if request_type == "create_incident":
                ...
                result = {
                    ...
                    "supported_types": [
                        "create_incident", "update_incident", "get_incident", "list_incidents",
                        "resolve_incident", "create_alert", "acknowledge_alert", "record_metric",
                        "analyze_metrics", "security_scan", "get_dashboard", "escalate_incident"
                    ]
                }
    ...
    async def _create_incident(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                json.dumps(affected_systems), impact_description, reporter,
                assignee, json.dumps(tags), json.dumps(metadata)
            ))
            ...
            cursor.execute("""
                ...
                VALUES (?, 'created', ?, ?)
            """, (incident_id, json.dumps({"title": title, "severity": severity}), reporter))
        ...
        return {
            ...
            "auto_escalated": severity in [IncidentSeverity.HIGH.value, IncidentSeverity.CRITICAL.value]
        }
    ...
    async def _update_incident(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        allowed_fields = [
            ...
            "affected_systems", "impact_description", "root_cause",
            "resolution", "assignee", "tags", "metadata"
        ]
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for field, value in updates.items():
                if field in allowed_fields:
                    ...
                    if field in ["affected_systems", "tags", "metadata"] and isinstance(value, (list, dict)):
    ...
    async def _resolve_incident(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            resolution_time = int((datetime.now() - created_at).total_seconds() / 60)  # 分単位
            ...
            cursor.execute("""
                ...
                WHERE id = ?
            """, (
                resolution, root_cause, resolved_at, resolution_time,
    ...
    async def _create_alert(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                source_system, json.dumps(metadata)
            ))
    ...
    async def _record_metric(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # アラートルールチェック
        alert_triggered = await self._check_alert_rules(metric_name, metric_value, source_system)
    ...
    async def _check_alert_rules(self, metric_name: str, metric_value: float, source_system: str) -> Optional[str]:
        """アラートルールチェック"""
        alert_id = None
        
        # CPU使用率チェック
        if metric_name == "cpu_usage" and metric_value > self.alert_rules["cpu_usage_threshold"]:
            alert_result = await self._create_alert({
                "alert_type": "performance",
                "severity": IncidentSeverity.HIGH.value if metric_value > 95 else IncidentSeverity.MEDIUM.value,
                ...
                "metadata": {"metric_value": metric_value, "threshold": self.alert_rules["cpu_usage_threshold"]}
            })
            alert_id = alert_result.get("alert_id")
        
        # メモリ使用率チェック
        elif metric_name == "memory_usage" and metric_value > self.alert_rules["memory_usage_threshold"]:
            alert_result = await self._create_alert({
                "alert_type": "performance",
                "severity": IncidentSeverity.HIGH.value if metric_value > 95 else IncidentSeverity.MEDIUM.value,
                ...
                "source_system": source_system,
                "metadata": {"metric_value": metric_value, "threshold": self.alert_rules["memory_usage_threshold"]}
            })
            alert_id = alert_result.get("alert_id")
        
        # ディスク使用率チェック
        elif metric_name == "disk_usage" and metric_value > self.alert_rules["disk_usage_threshold"]:
            alert_result = await self._create_alert({
                "alert_type": "availability",
                "severity": IncidentSeverity.CRITICAL.value if metric_value > 98 else IncidentSeverity.HIGH.value,
                ...
                "source_system": source_system,
                "metadata": {"metric_value": metric_value, "threshold": self.alert_rules["disk_usage_threshold"]}
            })
            alert_id = alert_result.get("alert_id")
    ...
    async def _security_scan(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                json.dumps({"scan_type": scan_type, "findings_count": len(findings)})
            ))
    ...
    async def _get_dashboard(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        dashboard = {
            ...
            "health_status": "healthy" if active_alerts < 5 else "degraded" if active_alerts < 20 else "critical"
        }
    ...
    async def _escalate_incident(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 現在の重要度取得
            cursor.execute("SELECT severity FROM incidents WHERE id = ?", (incident_id,))
            ...
            new_severity = severity_escalation.get(current_severity[0], IncidentSeverity.CRITICAL.value)
