...
from libs.four_sages.incident.incident_sage import (
    IncidentSage, IncidentSeverity, IncidentCategory, AlertLevel
)
...
class PreventiveAlertSystem(EldersServiceLegacy):
    ...
    async def _check_rule(self, rule_name: str, rule_config: Dict[str, Any], 
                         current_metrics: Dict[str, float]) -> Optional[PreventiveAlert]:
        """ルールチェック"""
            ...
            if severity:
                return await self._create_alert(
                    ...
                    context={
                        ...
                        "threshold": warning_threshold if severity == AlertLevel.WARNING else critical_threshold,
        ...
        if "baseline_multiplier" in rule_config:
            return await self._check_baseline_rule(rule_name, rule_config, current_value)
        
        return None
    
    async def _check_baseline_rule(self, rule_name: str, rule_config: Dict[str, Any],
                                  current_value: float) -> Optional[PreventiveAlert]:
        """ベースライン比較ルールチェック"""
        metric_name = rule_config.get("metric")
    ...
    async def _analyze_trends(self) -> List[PreventiveAlert]:
        ...
        for metric_name, window in self.metric_windows.items():
            ...
            timestamps = [(t - window.timestamps[0]).total_seconds() for t in window.timestamps]
    ...
    async def _create_alert(self, alert_type: str, severity: AlertLevel, 
                          context: Dict[str, Any]) -> PreventiveAlert:
        """アラート作成"""
    ...
    def _format_alert_message(self, alert_type: str, severity: AlertLevel, 
    ...
    async def _is_duplicate_alert(self, alert: PreventiveAlert) -> bool:
        ...
        threshold = self.suppression_rules["duplicate_suppression"]["similarity_threshold"]
    ...
    async def _get_statistics(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                ...
                "active_alerts": sum(1 for a in self.alert_history if not a.resolved_at),
