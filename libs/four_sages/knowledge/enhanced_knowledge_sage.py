...
class EnhancedKnowledgeSage(KnowledgeSage):
    ...
    def _initialize_tag_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "architecture": ["architecture", "microservices", "design pattern", "scalability"]
        }

    def _initialize_category_keywords(self) -> Dict[str, List[str]]:
        """Initialize keywords for category classification"""
        return {
            "development": ["code", "implement", "function", "class", "method", "programming"],
            "architecture": ["design", "pattern", "architecture", "structure", "system"],
            "best_practices": ["best practice", "should", "recommendation", "guideline"],
            ...
            "processes": ["process", "workflow", "methodology", "procedure", "practice"]
        }
    ...
    async def assess_knowledge_quality(self, knowledge: Dict[str, Any]) -> float:
        ...
        # Completeness score
        has_all_fields = all(knowledge.get(field) for field in ["title", "content", "category", "tags"])
    ...
    async def store_knowledge(self, title: str, content: str, category: str = None, 
        ...
        if is_duplicate:
            self.logger.warning(f"Potential duplicate detected (similarity: {similarity:.2f})")
    ...
    async def batch_import_knowledge(self, knowledge_batch: List[Dict[str, Any]], 
    ...
    async def _set_knowledge_expiration(self, knowledge_id: str, expiration_date: datetime) -> None:
    ...
    async def get_knowledge_for_task(self, task_context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def get_incident_solutions(self, incident_context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def get_knowledge_analytics(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Popular tags
            cursor = conn.execute("SELECT tags FROM knowledge_entries WHERE tags IS NOT NULL")
            all_tags = []
        ...
        return {
            ...
            "access_patterns": [{"title": title, "count": count} for title, count in top_accessed]
        }
