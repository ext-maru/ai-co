...
class RAGSage(BaseSage):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_keywords_document ON keywords(document_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_search_hash ON search_queries(query_hash)")
            conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_keyword_document_unique ON keywords(keyword, document_id, chunk_id)")
        ...
        try:
            ...
            if request_type == "add_document":
                ...
                result = {
                    ...
                    "supported_types": [
                        "add_document", "search", "get_document", "list_documents",
                        "update_document", "delete_document", "create_context",
                        "search_in_context", "get_recommendations", "get_analytics", "reindex_documents"
                    ]
                }
    ...
    async def _add_document(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                document_id, title, content, document_type, source, language,
                content_hash, json.dumps(tags), json.dumps(metadata)
            ))
    ...
    async def _chunk_and_index_document(self, document_id: str, title: str, content: str):
        """文書のチャンキングとインデックシング"""
        # コンテンツをチャンクに分割（簡易実装）
        chunk_size = 500  # 文字数
        chunks = []
    ...
    async def _search(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not query_keywords:
            return {
                "success": False,
                "error": "No valid keywords found in query"
            }
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 最小スコアフィルタリング
            filtered_results = [(doc_id, score) for doc_id, score in sorted_results if score >= min_score]
            ...
            for rank, (document_id, score) in enumerate(filtered_results):
                ...
                if doc_row:
                    ...
                    cursor.execute("""
                        ...
                        LIMIT 3
                    """.format(','.join(['?'] * len(query_keywords))), 
                    ...
                    results.append({
                        ...
                        "title": doc_row[1],
                        "content_preview": doc_row[2][:300] + "..." if len(doc_row[2]) > 300 else doc_row[2],
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                query_id, query, query_hash, len(results), execution_time,
                json.dumps({"keywords": query_keywords, "filters": {"document_type": document_type, "language": language}})
            ))
            ...
            for result in results:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?)
                """, (query_id, result["document_id"], result["relevance_score"], result["rank"]))
    ...
    async def _calculate_relevance_scores(self, query_keywords: List[str], 
    ...
    async def _update_document(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        allowed_fields = ["title", "content", "document_type", "source", "language", "tags", "metadata"]
        update_fields = []
        params = []
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 文書存在確認
            cursor.execute("SELECT title, content FROM documents WHERE id = ?", (document_id,))
            ...
            if reindex_needed:
                ...
                if updated_doc:
                    # 再インデックシング実行
                    await self._chunk_and_index_document(document_id, updated_doc[0], updated_doc[1])
    ...
    async def _delete_document(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("DELETE FROM keywords WHERE document_id = ?", (document_id,))
            cursor.execute("DELETE FROM chunks WHERE document_id = ?", (document_id,))
            cursor.execute("DELETE FROM search_results WHERE document_id = ?", (document_id,))
    ...
    async def _create_context(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                context_id, context_name, description, json.dumps(document_ids),
                context_type, json.dumps(metadata)
            ))
    ...
    async def _search_in_context(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not context_id or not query:
            return {
                "success": False,
                "error": "Context ID and query are required"
            }
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # コンテキスト取得
            cursor.execute("SELECT document_ids FROM contexts WHERE id = ?", (context_id,))
        ...
        search_result = await self._search({
            "query": query,
            "limit": limit * 2  # コンテキストフィルタリングを考慮して多めに取得
        })
    ...
    async def _get_recommendations(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if not keywords:
                return {
                    "success": False,
                    "error": "No keywords found for document"
                }
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for doc_id, title, content in documents:
                try:
                    ...
                    cursor.execute("DELETE FROM keywords WHERE document_id = ?", (doc_id,))
