...
from libs.four_sages.task.task_sage import TaskEntry, TaskResource, TaskStatus
...
class SystemResourceMonitor:
    ...
    async def get_current_usage(self) -> SystemResource:
        ...
        try:
            ...
            if self._last_disk_io and self._last_timestamp:
                ...
                disk_read_mb = (disk_io.read_bytes - self._last_disk_io.read_bytes) / (1024 ** 2) / time_delta
                disk_write_mb = (disk_io.write_bytes - self._last_disk_io.write_bytes) / (1024 ** 2) / time_delta
            else:
                disk_read_mb = 0
                disk_write_mb = 0
            ...
            if self._last_net_io and self._last_timestamp:
                time_delta = (current_time - self._last_timestamp).total_seconds()
                net_send_mb = (net_io.bytes_sent - self._last_net_io.bytes_sent) / (1024 ** 2) / time_delta
                net_recv_mb = (net_io.bytes_recv - self._last_net_io.bytes_recv) / (1024 ** 2) / time_delta
            else:
                net_send_mb = 0
                net_recv_mb = 0
    ...
    def get_average_usage(self, minutes: int = 5) -> Optional[SystemResource]:
        ...
        avg_resource = SystemResource(
            ...
            memory_available_gb=sum(r.memory_available_gb for r in recent_data) / len(recent_data),
            disk_io_read_mb=sum(r.disk_io_read_mb for r in recent_data) / len(recent_data),
            disk_io_write_mb=sum(r.disk_io_write_mb for r in recent_data) / len(recent_data),
            network_io_send_mb=sum(r.network_io_send_mb for r in recent_data) / len(recent_data),
            network_io_recv_mb=sum(r.network_io_recv_mb for r in recent_data) / len(recent_data),
            timestamp=datetime.now()
        )
...
class ResourceOptimizationEngine:
    ...
    async def optimize_resource_allocation(self, tasks: List[TaskEntry]) -> Dict[str, Any]:
        """リソース配分を最適化"""
        try:
            # 現在のリソース使用状況
            ...
            logger.info(f"Resource optimization completed: {optimization_result['summary']}")
    ...
    async def _predict_resource_usage(self, tasks: List[TaskEntry]) -> Dict[str, TaskResourceUsage]:
        """タスクのリソース使用量を予測"""
        predictions = {}
        ...
        for task in tasks:
            ...
            # 履歴データで調整
            if historical_usage:
                predicted_usage.cpu_usage = (predicted_usage.cpu_usage * 0.3 + 
                                           historical_usage['cpu'] * 0.7)
                predicted_usage.memory_usage = (predicted_usage.memory_usage * 0.3 + 
    ...
    def _get_default_resource_usage(self, task: TaskEntry) -> Dict[str, float]:
        ...
        task_type_resources = {
            'development': {'cpu': 30.0, 'memory': 500.0, 'disk_io': 10.0, 'network_io': 5.0, 'duration': 1800},
            'maintenance': {'cpu': 20.0, 'memory': 300.0, 'disk_io': 5.0, 'network_io': 2.0, 'duration': 900},
            'investigation': {'cpu': 15.0, 'memory': 200.0, 'disk_io': 2.0, 'network_io': 10.0, 'duration': 600},
            'integration': {'cpu': 40.0, 'memory': 800.0, 'disk_io': 20.0, 'network_io': 15.0, 'duration': 2400},
            'optimization': {'cpu': 50.0, 'memory': 1000.0, 'disk_io': 15.0, 'network_io': 5.0, 'duration': 3600}
        }
        
        return task_type_resources.get(task.task_type.value, {
            'cpu': 25.0, 'memory': 400.0, 'disk_io': 5.0, 'network_io': 5.0, 'duration': 1200
        })
    
    async def _get_historical_resource_usage(self, task: TaskEntry) -> Optional[Dict[str, float]]:
        """履歴データからリソース使用量を取得"""
        try:
            # タスクタイプ別の履歴を検索
    ...
    async def _run_optimization(self, tasks: List[TaskEntry], 
                               available_resources: SystemResource,
                               task_requirements: Dict[str, TaskResourceUsage]) -> Dict[str, Any]:
        """最適化アルゴリズムを実行"""
        # 利用可能リソースを計算
        available_cpu = self.resource_limits['cpu_percent'] - available_resources.cpu_percent
        available_memory = self.resource_limits['memory_percent'] - available_resources.memory_percent
        
        # タスクを優先度順にソート
        sorted_tasks = sorted(tasks, key=lambda t: self.task_sage.scheduler.calculate_task_score(t), reverse=True)
        ...
        for task in sorted_tasks:
            ...
            # リソースが収まるかチェック
            if (current_cpu + req.cpu_usage <= available_cpu and 
                current_memory + req.memory_usage / 1024 <= available_memory):
                # 現在のグループに追加
        ...
        optimization_result = {
            ...
            'estimated_completion_time': self._estimate_total_time(parallel_groups, task_requirements),
    ...
    def _estimate_total_time(self, parallel_groups: List[List[TaskEntry]], 
                           task_requirements: Dict[str, TaskResourceUsage]) -> float:
        """全体の完了時間を推定"""
        total_time = 0.0
    ...
    def _generate_recommendations(self, tasks: List[TaskEntry],
                                available_resources: SystemResource,
                                task_requirements: Dict[str, TaskResourceUsage],
                                parallel_groups: List[List[TaskEntry]]) -> List[str]:
        """最適化の推奨事項を生成"""
        recommendations = []
        
        # CPU使用率が高い場合
        if available_resources.cpu_percent > 70:
            recommendations.append("High CPU usage detected. Consider deferring low-priority tasks.")
        
        # メモリ使用率が高い場合
        if available_resources.memory_percent > 80:
            recommendations.append("High memory usage. Optimize memory-intensive tasks.")
        
        # 並列度が低い場合
        if len(parallel_groups) > len(tasks) / 2:
            recommendations.append("Low parallelization. Consider breaking down large tasks.")
        
        # I/O負荷が高い場合
        if (available_resources.disk_io_read_mb + available_resources.disk_io_write_mb) > 50:
            recommendations.append("High I/O load. Schedule I/O-intensive tasks separately.")
        ...
        try:
            ...
            for i in range(5):  # 5秒間監視
                ...
                sample = {
                    ...
                    'memory_delta': current.memory_percent - start_resources.memory_percent
                }
                
                monitoring_result['samples'].append(sample)
                monitoring_result['peak_cpu'] = max(monitoring_result['peak_cpu'], sample['cpu_delta'])
                monitoring_result['peak_memory'] = max(monitoring_result['peak_memory'], sample['memory_delta'])
            
            # 平均値を計算
            if monitoring_result['samples']:
                monitoring_result['avg_cpu'] = sum(s['cpu_delta'] for s in monitoring_result['samples']) / len(monitoring_result['samples'])
                monitoring_result['avg_memory'] = sum(s['memory_delta'] for s in monitoring_result['samples']) / len(monitoring_result['samples'])
    ...
    def get_resource_efficiency_score(self) -> float:
        ...
        try:
            ...
            # 効率スコアを計算（0-1）
            cpu_efficiency = min(1.0, avg_usage.cpu_percent / self.resource_limits['cpu_percent'])
            memory_efficiency = min(1.0, avg_usage.memory_percent / self.resource_limits['memory_percent'])
            
            # I/O効率
            io_total = avg_usage.disk_io_read_mb + avg_usage.disk_io_write_mb
            io_efficiency = 1.0 - min(1.0, io_total / self.resource_limits['io_threshold_mb'])
            
            # 総合スコア
            efficiency_score = (cpu_efficiency * 0.4 + memory_efficiency * 0.4 + io_efficiency * 0.2)
