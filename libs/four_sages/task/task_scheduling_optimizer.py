...
from libs.four_sages.task.task_sage import TaskEntry, TaskStatus, TaskPriority, TaskDependency
...
class TaskSchedulingOptimizer:
    ...
    def __init__(self, task_sage, priority_engine: DynamicPriorityEngine,
    ...
    async def optimize_schedule(self, tasks: List[TaskEntry], 
                              constraints: Optional[SchedulingConstraint] = None) -> List[ScheduledTask]:
        """最適なスケジュールを生成"""
        try:
            if constraints:
            ...
            if cache_key in self.schedule_cache:
                ...
                if datetime.now() - cached_schedule['timestamp'] < timedelta(minutes=15):
                    return cached_schedule['schedule']
            
            # 動的優先度計算
            logger.info(f"Calculating dynamic priorities for {len(tasks)} tasks")
            for task in tasks:
                task.dynamic_priority = await self.priority_engine.calculate_dynamic_priority(task)
            
            # 実行時間予測
            logger.info("Predicting execution times")
            for task in tasks:
                predicted_time, confidence = await self.time_predictor.predict_execution_time(task)
                task.predicted_time = predicted_time * (1 + self.constraints.buffer_time_percent)
            ...
            logger.info(f"Schedule optimization completed: {len(optimized_schedule)} tasks scheduled")
    ...
    async def _generate_optimal_schedule(self, tasks: List[TaskEntry], 
                                       constraints: SchedulingConstraint) -> List[ScheduledTask]:
        """最適スケジュールを生成"""
        # 依存関係グラフを構築
        dependency_graph = self._build_dependency_graph(tasks)
        
        # トポロジカルソートで依存関係を考慮した順序を取得
        topological_order = self._topological_sort_with_priority(tasks, dependency_graph)
        ...
        for task in topological_order:
            # 依存関係チェック
            dependencies_met = self._check_dependencies_met(task, scheduled_tasks, dependency_graph)
            ...
            # 依存関係のあるタスクの完了時刻を考慮
            dependency_end_time = self._get_dependency_end_time(task, scheduled_tasks, dependency_graph)
    ...
    def _topological_sort_with_priority(self, tasks: List[TaskEntry], 
                                      dependency_graph: Dict[str, Set[str]]) -> List[TaskEntry]:
        """優先度を考慮したトポロジカルソート"""
        # 入次数を計算
            ...
            for other_task_id, deps in dependency_graph.items():
                if task.id in deps:
                    ...
                    if in_degree[other_task_id] == 0:
                        ...
                        heapq.heappush(available_tasks, (-other_task.dynamic_priority, other_task))
    ...
    def _check_dependencies_met(self, task: TaskEntry, scheduled_tasks: List[ScheduledTask],
    ...
    def _get_dependency_end_time(self, task: TaskEntry, scheduled_tasks: List[ScheduledTask],
                               dependency_graph: Dict[str, Set[str]]) -> datetime:
        """依存タスクの最大終了時刻を取得"""
        dependencies = dependency_graph.get(task.id, set())
        if not dependencies:
            return datetime.now()
    ...
    def _get_next_working_time(self, time: datetime) -> datetime:
        ...
        if time.hour < self.constraints.working_hours_start:
            return time.replace(hour=self.constraints.working_hours_start, minute=0, second=0)
        elif time.hour >= self.constraints.working_hours_end:
            # 翌日の勤務開始時間
            next_day = time + timedelta(days=1)
            return next_day.replace(hour=self.constraints.working_hours_start, minute=0, second=0)
        else:
            return time
    
    def _adjust_for_working_hours(self, time: datetime, constraints: SchedulingConstraint) -> datetime:
        """勤務時間に合わせて調整"""
        # 週末をスキップ
        ...
        if time.hour < constraints.working_hours_start:
            time = time.replace(hour=constraints.working_hours_start, minute=0, second=0)
    ...
    def _find_earlier_slot(self, task: TaskEntry, scheduled_tasks: List[ScheduledTask],
                         parallel_slot_end_times: List[datetime],
                         constraints: SchedulingConstraint) -> datetime:
        """期限に間に合う早いスロットを探す"""
        ...
        dependency_end = self._get_dependency_end_time(
            task, scheduled_tasks, self._build_dependency_graph([st.task for st in scheduled_tasks])
        )
    ...
    def _create_fallback_schedule(self, tasks: List[TaskEntry]) -> List[ScheduledTask]:
        ...
        # 優先度順にソート
        sorted_tasks = sorted(tasks, key=lambda t: getattr(t, 'dynamic_priority', 1.0), reverse=True)
    ...
    def _record_optimization_history(self, tasks: List[TaskEntry], schedule: List[ScheduledTask]):
        """最適化履歴を記録"""
        history_entry = {
            'timestamp': datetime.now(),
    ...
    def _calculate_deadline_compliance(self, schedule: List[ScheduledTask]) -> float:
        ...
        compliant_tasks = [
            st for st in tasks_with_deadline 
            if st.scheduled_end <= st.task.due_date
        ]
    ...
    async def get_schedule_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            'avg_total_duration': sum(h['total_duration'] for h in recent_history) / len(recent_history),
            'avg_parallelization': sum(h['parallelization_factor'] for h in recent_history) / len(recent_history),
            'avg_deadline_compliance': sum(h['deadline_compliance'] for h in recent_history) / len(recent_history),
            'total_optimizations': len(self.optimization_history),
            'cache_hit_rate': len(self.schedule_cache) / max(1, len(self.optimization_history))
        }
    ...
    def get_gantt_chart_data(self, schedule: List[ScheduledTask]) -> List[Dict[str, Any]]:
        """ガントチャート用のデータを生成"""
        gantt_data = []
