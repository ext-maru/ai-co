...
class FourSagesPostgresMCPIntegration(FourSagesIntegration):
    ...
    async def initialize_mcp_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.integration_status.update(
                {
                    "mcp_connected": True,
                    "sages_integrated": all(
                        r["success"] for r in integration_results.values()
                    ),
    ...
    async def _setup_sage_mcp_integration(self, sage_type: SageType) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "sage_type": sage_type.value, "error": str(e)}

    async def _create_sage_table(
        self, sage_type: SageType, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """賢者専用テーブル作成"""
        try:
            # 賢者専用テーブル作成のメタデータ
            ...
            store_response = await self.mcp_client.store(
                "Table creation", table_metadata
            )
    ...
    async def _store_sage_knowledge(
        self, sage_knowledge: SageKnowledge
    ) -> Dict[str, Any]:
        """賢者知識の保存"""
        try:
            metadata = {
            ...
            store_response = await self.mcp_client.store(
                sage_knowledge.content, metadata
            )
    ...
    async def knowledge_sage_search(
        ...
        try:
            ...
            if search_response.success:
                ...
                for result in search_response.data:
                    if result.get("type", "").startswith("knowledge_"):
                        result["sage_analysis"] = self._analyze_knowledge_relevance(
                            result, query
                        )
            ...
            return {"sage": "knowledge_sage", "status": "error", "message": str(e)}

    async def task_sage_management(
        ...
        try:
            ...
            task_metadata = {
                ...
                "priority": self._get_task_priority_score(
                    task_request.get("priority", "normal")
                ),
            }

            store_response = await self.mcp_client.store(
                "Task management", task_metadata
            )
            ...
            return {"sage": "task_sage", "status": "error", "message": str(e)}

    async def incident_sage_monitoring(
        ...
        try:
            ...
            incident_metadata = {
                ...
                "tags": [
                    "incident",
                    incident_data.get("severity", "normal"),
                    "monitoring",
                ],
                "priority": self._get_incident_priority_score(
                    incident_data.get("severity", "normal")
                ),
            }

            store_response = await self.mcp_client.store(
                "Incident tracking", incident_metadata
            )
            ...
            return {"sage": "incident_sage", "status": "error", "message": str(e)}

    async def rag_sage_enhanced_search(
        ...
        try:
            ...
            return {"sage": "rag_sage", "status": "error", "message": str(e)}
    ...
    async def four_sages_collaborative_analysis(
        ...
        try:
            ...
            analysis_tasks = [
                ...
                self.incident_sage_monitoring(
                    analysis_request.get("incident_data", {})
                ),
                self.rag_sage_enhanced_search(
                    analysis_request.get("query", ""), analysis_request.get("context")
                ),
            ]
            ...
            store_response = await self.mcp_client.store(
                "Four sages analysis", analysis_metadata
            )
    ...
    def _analyze_knowledge_relevance(
    ...
    def _get_task_priority_score(self, priority: str) -> int:
        """タスク優先度スコア"""
        priority_map = {"critical": 10, "high": 8, "normal": 5, "low": 3}
        return priority_map.get(priority, 5)

    def _get_incident_priority_score(self, severity: str) -> int:
        """インシデント優先度スコア"""
        severity_map = {"critical": 10, "high": 9, "medium": 7, "low": 4}
    ...
    def _analyze_search_relevance(
        ...
        return {
            ...
            "relevance_distribution": {
                ...
                "medium": len(
                    [r for r in results if 0.5 < get_similarity_score(r) <= 0.8]
                ),
                "low": len([r for r in results if get_similarity_score(r) <= 0.5]),
            },
            "average_relevance": (
                sum(get_similarity_score(r) for r in results) / len(results)
                if results
                else 0
            ),
    ...
    def _form_four_sages_consensus(
        self, integrated_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """4賢者コンセンサス形成"""
        successful_analyses = sum(
            1
            for analysis in integrated_analysis.values()
...
async def demo_four_sages_postgres_mcp():
    ...
    try:
        ...
        collaborative_result = await four_sages.four_sages_collaborative_analysis(
            analysis_request
        )
        ...
        print(
            f"   保存済み知識: {status_result['integration_status']['total_knowledge_stored']}"
        )
        print(
            f"   検索実行数: {status_result['integration_status']['total_searches_performed']}"
        )
