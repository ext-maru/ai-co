...
from libs.grimoire_database import EvolutionType, GrimoireDatabase
...
class FourSagesReviewer:
    ...
    async def _conduct_individual_review(
        self,
        sage_type: str,
    ...
    async def _knowledge_sage_review(
        self, spell_data: Dict[str, Any], request: ApprovalRequest
    ) -> SageReview:
        """ナレッジ賢者審査"""
        # 知識価値の評価
...
class GrandElderApprovalSystem:
    ...
    async def submit_approval_request(
        ...
        try:
            # 影響分析実行
            impact_analysis = await self._analyze_spell_impact(
                spell_id, permission_type
            )
    ...
    async def _analyze_spell_impact(
        self, spell_id: str, permission_type: PermissionType
    ) -> ImpactAnalysis:
        """呪文影響分析"""
        try:
    ...
    def _assess_rollback_complexity(
        self, dependency_count: int, is_eternal: bool
    ) -> str:
        """ロールバック複雑度評価"""
    ...
    async def _evaluate_sage_consensus(self, request: ApprovalRequest):
        ...
        try:
            ...
            # 自動承認の判定
            if (
                consensus_rate >= threshold["sage_consensus"]
                and not threshold["elder_required"]
            ):
                # 自動承認
            ...
            logger.info(
                f"🏛️ 4賢者コンセンサス評価完了: {request.request_id} - {request.status.value}"
            )
    ...
    async def grand_elder_approval(
        self, request_id: str, approved: bool, elder_name: str, notes: str = ""
    ) -> bool:
        """グランドエルダーによる最終承認"""
        try:
            # 申請取得
            ...
            if request.status != ApprovalStatus.ELDER_REVIEW:
                raise ValueError(
                    f"Request not ready for elder approval: {request.status}"
                )
    ...
    async def get_pending_requests(
        self, elder_name: Optional[str] = None
    ...
    async def _save_approval_request(self, request: ApprovalRequest):
        ...
        await self.database.request_spell_dispel(
            request.spell_id, request.request_reason, request.requested_by
        )
