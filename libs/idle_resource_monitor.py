...
class ResourceMetrics:
    ...
    def is_idle(self, thresholds: Dict[str, float]) -> bool:
        ...
        return (
            self.cpu_percent < thresholds.get('cpu', 20.0) and
            self.memory_percent < thresholds.get('memory', 80.0) and
            self.load_avg_1m < thresholds.get('load', 1.0)
        )
...
class IdleResourceMonitor:
        ...
        if is_idle:
            if self.current_idle_period is None:
                ...
                self.current_idle_period.avg_cpu = (
                    (self.current_idle_period.avg_cpu * (sample_count - 1) + metrics.cpu_percent) 
                    / sample_count
                )
                self.current_idle_period.avg_memory = (
                    (self.current_idle_period.avg_memory * (sample_count - 1) + metrics.memory_percent) 
                    / sample_count
                )
    ...
    def predict_next_idle_window(self) -> Optional[Dict]:
        ...
        if last_period.end_time:
            ...
            return {
                ...
                'confidence': min(0.8, len(recent_periods) / 10),  # More data = higher confidence
    ...
    def save_state(self, filepath: str):
        ...
        state = {
            ...
            'current_idle_period': asdict(self.current_idle_period) if self.current_idle_period else None,
...
def main():
    ...
    try:
        ...
        while time.time() - start_time < args.duration:
            ...
            if status['is_idle']:
                print(f"🟢 IDLE - Duration: {status['duration']:.1f}s, "
                      f"Confidence: {status['confidence']:.2f}")
            else:
                metrics = status['current_metrics']
                print(f"🔴 BUSY - CPU: {metrics['cpu_percent']:.1f}%, "
