...
class GitHubIssueCreator:
    ...
    def create_issue(
        ...
        try:
            ...
            # Issueを作成
            create_params = {"title": title, "body": body}
    ...
    def create_issue_from_template(
        self, repo_name: str, template_name: str, variables: Dict[str, Any]
    ) -> Issue:
        """テンプレートからIssueを作成
...
""",
    ...
    def create_sub_issues(
        self, repo_name: str, parent_issue_number: int, sub_tasks: List[Dict[str, str]]
    ) -> List[Issue]:
...
if __name__ == "__main__":
    ...
    try:
        if args.template:
            ...
            issue = creator.create_issue_from_template(
                args.repo, args.template, variables
            )
        else:
            # 通常のIssue作成
            creator = GitHubIssueCreator()
            issue = creator.create_issue(
                args.repo, args.title, args.body, args.labels, args.assignees
            )
