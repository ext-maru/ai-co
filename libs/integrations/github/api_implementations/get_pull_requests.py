...
from libs.integrations.github.security import get_security_manager, SecurityViolationError
...
class GitHubGetPullRequestsImplementation:
    ...
    def _apply_additional_filters(
        ...
        if creator:
            filtered = [
                pr for pr in filtered
                if pr.get("user", {}).get("login") == creator
            ]
        ...
        if mentioned:
            ...
            filtered = [
                pr for pr in filtered
                if mention_pattern in pr.get("body", "")
            ]
    ...
    def _enrich_pull_requests(self, prs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for pr in prs:
            ...
            enriched_pr["last_updated_days"] = self._calculate_age_days(pr["updated_at"])
            ...
            enriched_pr["status"] = {
                ...
                "has_conflicts": pr.get("mergeable_state") == "dirty" if "mergeable_state" in pr else None,
    ...
    def _calculate_statistics(self, prs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # 状態別カウント
        by_state = {
            "open": sum(1 for pr in prs if pr["state"] == "open" and not pr.get("draft")),
            "draft": sum(1 for pr in prs if pr.get("draft")),
            "closed": sum(1 for pr in prs if pr["state"] == "closed" and not pr.get("merged")),
        ...
        return {
            ...
            "largest_pr": max(prs, key=lambda x: x.get("size_info", {}).get("total_changes", 0)) if prs else None
        }
    ...
    def _summarize_reviews(self, reviews: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "latest_review": max(reviews, key=lambda x: x.get("submitted_at", "")) if reviews else None
        }
