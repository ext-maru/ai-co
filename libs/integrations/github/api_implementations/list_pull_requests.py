        ...
        try:
            ...
            if sort not in ["created", "updated", "popularity"]:
                raise ValidationError(
                    "Sort must be 'created', 'updated', or 'popularity'"
                )
            ...
            response = await self._make_api_call(
                method="GET", endpoint=f"/repos/{owner}/{repo}/pulls", params=params
            )
            ...
            logger.info(
                f"Pull requests retrieved successfully for {owner}/{repo}: {len(processed_prs)} PRs"
            )
            ...
            raise APIError(
                f"Unexpected error during pull requests list retrieval: {str(e)}"
            )
