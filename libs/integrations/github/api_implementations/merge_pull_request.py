...
class GitHubPullRequestMerger:
    ...
    async def merge_pull_request(self, owner: str, repo: str, pull_number: int, 
        ...
        try:
            ...
            if not pr_status["mergeable"]:
                raise ValidationError(f"Pull request not mergeable: {pr_status['reason']}")
    ...
    async def _check_pr_status(self, owner: str, repo: str, pull_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "mergeable": response.get("mergeable", False) and response.get("state") == "open",
                "reason": "PR is not mergeable" if not response.get("mergeable") else None
            }
