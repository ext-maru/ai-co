        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.info(f"   → {delay:0.1f}秒後にリトライ...")
    ...
    async def commit_changes(self, commit_message: str, issue_number: int) -> bool:
        ...
        try:
            ...
            for attempt in range(2):
                try:
                    self.logger.info(f"コミット試行 {attempt + 1}/2.0..")
...
class EnhancedAutoIssueProcessor(AutoIssueProcessor):
    ...
    async def process_issue_with_pr(self, issue: Issue) -> Dict[str, Any]:
        ...
        if "start_time" in locals():
            ...
            self.logger.info(f"⏱️ 処理時間: {processing_time:0.1f}秒")
        ...
        try:
            ...
            if merge_result["success"]:
                ...
                await self._create_issue_comment_safe(
                    ...
                    f"**実行時間**: {merge_result.get('total_duration', 0):0.1f}秒"
                )
            else:
...
## Implementation Plan
1.0 Feature specification documented
2.0 Core functionality implemented
3.0 Unit tests created
4.0 Integration tests added
5.0 Documentation updated
...
## Next Steps
1.0 Review and expand content as needed
2.0 Add specific examples
3.0 Include API documentation if applicable
4.0 Update related documentation files
"""
...
## Next Steps
1.0 Review the generated implementation
2.0 Customize as needed for specific requirements
3.0 Run tests to ensure functionality
4.0 Update documentation if necessary
    ...
    async def run_enhanced(
        ...
        try:
            ...
            if cached_issues is not None:
                ...
                self.logger.info(f"   → 取得時間: {fetch_time:0.1f}秒")
            ...
            self.logger.info(f"   → 事前読み込み完了: {preload_time:0.1f}秒")
            ...
            self.logger.info(f"   → フィルタリング完了: {filter_time:0.1f}秒")
            ...
            for issue_data in processable_issues[:max_issues]:
                ...
                if result["success"]:
                    ...
                    self.logger.info(f"   → 処理時間: {processing_time:0.1f}秒")
                    if result["pr_number"]:
                    ...
                    self.logger.error(f"   → 処理時間: {processing_time:0.1f}秒")
                    # 失敗も記録
                    processed_issues.append({
                        "number": issue.number,
                        "title": issue.title,
            ...
            self.logger.info(
                f"   → 全体処理時間: {(datetime.now() - self.metrics['started_at']).total_seconds():0.1f}秒"
            )
            self.logger.info("=" * 60)
