...
class GitHubInputValidator:
        ...
        try:
            ...
            if len(name) == 0 or len(name) > self.max_lengths["repo_name"]:
                raise ValidationError(
                    f"Repository name must be 1-{self.max_lengths['repo_name']} characters"
                )
            ...
            if "/" in name:
                ...
                for part in [owner, repo]:
                    if part.startswith(".") or part.endswith("."):
                        raise ValidationError(
                            "Repository owner and name cannot start or end with a dot"
                        )
                    if part.startswith("-") or part.endswith("-"):
                        raise ValidationError(
                            "Repository owner and name cannot start or end with a hyphen"
                        )
            else:
    ...
    def validate_description(self, description: str) -> bool:
        ...
        try:
            ...
            if len(description) > self.max_lengths["description"]:
                raise ValidationError(
                    f"Description must be less than {self.max_lengths['description']} characters"
                )
    ...
    def validate_issue_title(self, title: str) -> bool:
        ...
        try:
            ...
            if len(title) == 0 or len(title) > self.max_lengths["title"]:
                raise ValidationError(
                    f"Title must be 1-{self.max_lengths['title']} characters"
                )
    ...
    def validate_issue_body(self, body: str) -> bool:
        ...
        try:
            ...
            if len(body) > self.max_lengths["body"]:
                raise ValidationError(
                    f"Body must be less than {self.max_lengths['body']} characters"
                )
    ...
    def validate_username(self, username: str) -> bool:
        ...
        try:
            ...
            if len(username) == 0 or len(username) > self.max_lengths["username"]:
                raise ValidationError(
                    f"Username must be 1-{self.max_lengths['username']} characters"
                )
    ...
    def validate_branch_name(self, branch_name: str) -> bool:
        ...
        try:
            ...
            if (
                len(branch_name) == 0
                or len(branch_name) > self.max_lengths["branch_name"]
            ):
                raise ValidationError(
                    f"Branch name must be 1-{self.max_lengths['branch_name']} characters"
                )
