...
class GitHubIssueMonitor:
    ...
    def __init__(self, repo_owner: str = "ext-maru", repo_name: str = "ai-co",
                 check_interval: int = 30, state_file: str = "logs/issue_monitor_state.json"):
        """
        Args:
    ...
    async def _is_new_comment(self, comment: Dict[str, Any]) -> bool:
        ...
        if comment['user']['login'] == 'claude-elder' or 'claude-elder' in comment['body'].lower():
            return False
    ...
    async def _execute_command(self, issue: Dict[str, Any], comment: Dict[str, Any], command: Dict[str, Any]):
        """コマンドを実行"""
        ...
        # 実行中であることを報告
        await self._post_comment(issue_number, f"🤖 **Claude Elder**: コマンドを実行中...\n\n**コマンド**: {command_type}\n**内容**: {command_text}")
        ...
        try:
            ...
            # 結果を報告
            await self._post_comment(issue_number, f"✅ **Claude Elder**: コマンド実行完了\n\n{result}")
    ...
    async def _acknowledge_comment(self, issue: Dict[str, Any], comment: Dict[str, Any]):
        """通常のコメントに返信"""
        issue_number = issue['number']
        comment_author = comment['user']['login']
