...
class RepositoryValidator:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # 禁止されたリポジトリ（誤使用防止）
        self.forbidden_repos = self.config.get("forbidden_repositories", [
    ...
    def auto_correct_repository(self, repo_owner: str, repo_name: str) -> Dict[str, str]:
        """
        リポジトリを自動修正
        ...
        if self._is_forbidden_repo(repo_owner, repo_name):
            ...
            self.logger.warning(f"禁止リポジトリ {repo_owner}/{repo_name} をプライマリリポジトリに自動修正: {primary['owner']}/{primary['name']}")
            return primary
        ...
        if self.config.get("auto_correction", True):
            primary = self.get_primary_repository()
            self.logger.warning(f"未設定リポジトリ {repo_owner}/{repo_name} をプライマリリポジトリに自動修正: {primary['owner']}/{primary['name']}")
            return primary
    ...
    def log_repository_access(self, repo_owner: str, repo_name: str, 
                            action: str, result: bool, message: str):
        """リポジトリアクセスをログに記録"""
        log_entry = {
    ...
    def add_allowed_repository(self, owner: str, name: str, 
                              description: str = "", is_primary: bool = False):
        """許可リポジトリを追加"""
        new_repo = {
            "owner": owner,
            "name": name,
    ...
    def remove_allowed_repository(self, owner: str, name: str):
        """許可リポジトリを削除"""
        self.allowed_repos = [
            repo for repo in self.allowed_repos
...
def get_safe_repository(repo_owner: str, repo_name: str) -> Dict[str, str]:
    ...
    if not is_valid:
        ...
        validator.log_repository_access(
            repo_owner, repo_name, "auto_correction", 
            True, f"自動修正: {corrected['owner']}/{corrected['name']}"
        )
        return corrected
    
    validator.log_repository_access(
        repo_owner, repo_name, "access", True, message
    )
    return {"owner": repo_owner, "name": repo_name}