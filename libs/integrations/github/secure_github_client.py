...
from .repository_validator import get_repository_validator, RepositoryValidator
...
class SecureGitHubClient:
    ...
    def _validate_and_correct_repository(self, repo_owner: str, repo_name: str) -> Dict[str, str]:
        """
        リポジトリを検証し、必要に応じて修正
        ...
        if not is_valid:
            ...
            if self.auto_correction:
                # 自動修正
                corrected = self.validator.auto_correct_repository(repo_owner, repo_name)
                self.logger.info(f"リポジトリ自動修正: {repo_owner}/{repo_name} → {corrected['owner']}/{corrected['name']}")
                return corrected
            else:
                # 修正無効の場合はエラー
    ...
    async def create_issue(self, repo_owner: str, repo_name: str, 
                          title: str, body: str, labels: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            response = await self.rate_limiter.safe_request("POST", url, json=issue_data)
            ...
            if response and response.status_code == 201:
                ...
                self.validator.log_repository_access(
                    ...
                    "create_issue", True, f"Issue #{issue['number']} created"
                )
            ...
            self.validator.log_repository_access(
                repo_info['owner'], repo_info['name'], 
                "create_issue", False, f"Error: {str(e)}"
            )
    ...
    def close_issue(self, repo_owner: str, repo_name: str, issue_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.validator.log_repository_access(
                    repo_info['owner'], repo_info['name'], 
                    "close_issue", True, f"Issue #{issue_number} closed"
                )
            ...
            self.validator.log_repository_access(
                repo_info['owner'], repo_info['name'], 
                "close_issue", False, f"Error: {str(e)}"
            )
            
            raise
    
    def add_comment(self, repo_owner: str, repo_name: str, 
                   issue_number: int, comment: str) -> Dict[str, Any]:
        """
        セキュアなコメント追加
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                self.validator.log_repository_access(
                    repo_info['owner'], repo_info['name'], 
                    "add_comment", True, f"Comment added to Issue #{issue_number}"
                )
            ...
            self.validator.log_repository_access(
                repo_info['owner'], repo_info['name'], 
                "add_comment", False, f"Error: {str(e)}"
            )
            
            raise
    
    def list_issues(self, repo_owner: str, repo_name: str, 
                   state: str = "open", labels: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
        セキュアなIssue一覧取得
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.validator.log_repository_access(
                    repo_info['owner'], repo_info['name'], 
                    "list_issues", True, f"Retrieved {len(issues)} issues"
                )
            ...
            self.validator.log_repository_access(
                repo_info['owner'], repo_info['name'], 
                "list_issues", False, f"Error: {str(e)}"
            )
    ...
    def get_repository_info(self, repo_owner: str, repo_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.validator.log_repository_access(
                    repo_info['owner'], repo_info['name'], 
                    "get_repository_info", True, "Repository info retrieved"
                )
            ...
            self.validator.log_repository_access(
                repo_info['owner'], repo_info['name'], 
                "get_repository_info", False, f"Error: {str(e)}"
            )
