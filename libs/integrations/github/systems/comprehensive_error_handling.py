...
class GitHubErrorHandler:
    ...
    def _determine_recovery_action(self, error_info: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """リカバリアクションを決定"""
        severity = error_info["severity"]
        error_type = error_info["type"]
        ...
        # 権限エラー
        if error_info.get("status_code") == 403 and "permission" in error_info["message"].lower():
            return {
    ...
    def _log_error(self, error: Exception, error_info: Dict[str, Any], context: Dict[str, Any]) -> None:
        """エラーをログ出力"""
        severity = error_info["severity"]
        
        log_message = f"GitHub API Error: {error_info['type']} - {error_info['message']}"
    ...
    def retry_with_backoff(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        for attempt in range(self.retry_config["max_retries"]):
            try:
                ...
                logger.info(f"Retrying in {wait_time:.1f} seconds (attempt {attempt + 1}/{self.retry_config['max_retries']})")
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # エラーレポートをログ
                logger.error(f"Final error report: {json.dumps(handler.get_error_report(), indent=2)}")
                raise
