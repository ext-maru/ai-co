...
class ErrorPattern:
    ...
    def __init__(self, pattern: str, severity: ErrorSeverity, strategy: RecoveryStrategy, 
...
class ErrorRecoveryManager:
    ...
    def _initialize_error_patterns(self) -> List[ErrorPattern]:
        ...
        return [
            # 接続関連エラー
            ErrorPattern("ConnectionError", ErrorSeverity.HIGH, RecoveryStrategy.RETRY, 3, 2.0),
            ErrorPattern("TimeoutError", ErrorSeverity.HIGH, RecoveryStrategy.RETRY, 3, 1.5),
            ErrorPattern("ConnectTimeout", ErrorSeverity.HIGH, RecoveryStrategy.RETRY, 2, 2.0),
            
            # HTTP関連エラー
            ErrorPattern("HTTP 5", ErrorSeverity.HIGH, RecoveryStrategy.RETRY, 3, 2.0),
            ErrorPattern("HTTP 429", ErrorSeverity.MEDIUM, RecoveryStrategy.CIRCUIT_BREAK, 1, 5.0),
            ErrorPattern("HTTP 401", ErrorSeverity.CRITICAL, RecoveryStrategy.ESCALATE, 0, 0),
            ErrorPattern("HTTP 403", ErrorSeverity.CRITICAL, RecoveryStrategy.ESCALATE, 0, 0),
            ErrorPattern("HTTP 404", ErrorSeverity.LOW, RecoveryStrategy.FALLBACK, 1, 1.0),
            
            # GitHub API関連エラー
            ErrorPattern("rate limit", ErrorSeverity.HIGH, RecoveryStrategy.CIRCUIT_BREAK, 1, 10.0),
            ErrorPattern("abuse", ErrorSeverity.CRITICAL, RecoveryStrategy.ESCALATE, 0, 0),
            ErrorPattern("bad credentials", ErrorSeverity.CRITICAL, RecoveryStrategy.ESCALATE, 0, 0),
            
            # 一般的なエラー
            ErrorPattern("JSONDecodeError", ErrorSeverity.MEDIUM, RecoveryStrategy.RETRY, 2, 1.5),
            ErrorPattern("KeyError", ErrorSeverity.MEDIUM, RecoveryStrategy.FALLBACK, 1, 1.0),
            ErrorPattern("ValueError", ErrorSeverity.LOW, RecoveryStrategy.FALLBACK, 1, 1.0),
            
            # システムエラー
            ErrorPattern("MemoryError", ErrorSeverity.CRITICAL, RecoveryStrategy.ESCALATE, 0, 0),
    ...
    async def handle_error(self, error: Exception, context: Dict[str, Any], 
                          func: Optional[Callable] = None, *args, **kwargs) -> Dict[str, Any]:
        """包括的エラーハンドリング"""
        ...
        logger.error(f"🚨 Error occurred: {error} | Strategy: {error_info['strategy'].value}")
        
        try:
            # 回復戦略に基づく処理
            if error_info["strategy"] == RecoveryStrategy.RETRY:
                result = await self._execute_retry_strategy(error_info, func, *args, **kwargs)
            elif error_info["strategy"] == RecoveryStrategy.FALLBACK:
                result = await self._execute_fallback_strategy(error_info, func, *args, **kwargs)
    ...
    def _analyze_error(self, error: Exception, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for pattern in self.error_patterns:
            if pattern.pattern.lower() in error_str or pattern.pattern.lower() in error_type.lower():
                return {
                    "pattern": pattern,
    ...
    async def _execute_retry_strategy(self, error_info: Dict[str, Any], 
                                    func: Optional[Callable], *args, **kwargs) -> Dict[str, Any]:
        """リトライ戦略の実行"""
        if func is None:
        ...
        for attempt in range(max_retries):
            try:
                ...
                return {"success": True, "action": "retry_success", "attempts": attempt + 1, "result": result}
                
            except Exception as retry_error:
                logger.warning(f"⚠️ Retry attempt {attempt + 1} failed: {retry_error}")
                if attempt == max_retries - 1:
                    return {"success": False, "action": "retry_exhausted", "attempts": max_retries}
        
        return {"success": False, "action": "retry_failed"}
    
    async def _execute_fallback_strategy(self, error_info: Dict[str, Any], 
                                       func: Optional[Callable], *args, **kwargs) -> Dict[str, Any]:
        """フォールバック戦略の実行"""
        ...
        if error_info["pattern"]:
            ...
            if pattern_name in self.fallback_handlers:
                try:
                    ...
                    return {"success": True, "action": "fallback_success", "result": result}
        ...
        return {
            ...
            "result": {"error": "Operation failed, using fallback response", "data": None}
        }
    
    async def _execute_circuit_break_strategy(self, error_info: Dict[str, Any], 
        ...
        logger.warning(f"🔴 Circuit breaker activated for {circuit_name}, waiting {wait_time}s")
    ...
    async def _execute_escalate_strategy(self, error_info: Dict[str, Any], 
                                       context: Dict[str, Any]) -> Dict[str, Any]:
        """エスカレーション戦略の実行"""
        # 重要なエラーをログに記録
        ...
        escalation_data = {
            "timestamp": datetime.now().isoformat(),
            "severity": error_info["severity"].value,
            "error_pattern": error_info["pattern"].pattern if error_info["pattern"] else "unknown",
            "context": context
        }
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if total_errors == 0:
                ...
                recovery_rate = (self.recovery_metrics["recovered_errors"] / total_errors) * 100
...
def enhanced_error_handler(circuit_name: str = "default", enable_retry: bool = True, 
                          enable_fallback: bool = True, enable_circuit_breaker: bool = True):
    """拡張エラーハンドリングデコレータ"""
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if enable_circuit_breaker:
                    circuit_breaker = global_error_recovery_manager.get_circuit_breaker(circuit_name)
                ...
                if recovery_result.get("success", False) and "result" in recovery_result:
