...
class IntelligentTaskSplitter(BaseManager):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # インデックス作成
            conn.execute('CREATE INDEX IF NOT EXISTS idx_parent_task ON split_tasks(parent_task_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_task_status ON split_tasks(status)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_task_deps ON task_dependencies(task_id)')
    
    def analyze_task_complexity(self, task_description: str) -> Tuple[TaskComplexity, float]:
        """タスク複雑度分析"""
        try:
            ...
            # 追加要因の分析
            additional_factors = self._analyze_additional_complexity_factors(description_lower)
            ...
            if not any(complexity_scores.values()):
                ...
                base_complexity = max(complexity_scores.keys(), key=lambda k: complexity_scores[k])
    ...
    def _analyze_additional_complexity_factors(self, description: str) -> float:
        ...
        # 複数の技術要素
        tech_keywords = ['database', 'api', 'frontend', 'backend', 'ui', 'ux', 
                        'security', 'performance', 'scale', 'optimization']
    ...
    def split_into_subtasks(self, task_id: str, task_description: str, task_prompt: str = "") -> List[SubTask]:
        """タスクをサブタスクに分割"""
        try:
            logger.info(f"🔄 タスク分割開始: {task_id}")
            
            # 複雑度とタイプを分析
            complexity, complexity_score = self.analyze_task_complexity(task_description)
            ...
            if complexity in [TaskComplexity.SIMPLE, TaskComplexity.MODERATE]:
                # 単純・中程度は分割しない
                subtasks = [self._create_single_subtask(task_id, task_description, task_type, complexity)]
            else:
                # 複雑なタスクは分割
                subtasks = self._split_complex_task(task_id, task_description, task_prompt, task_type, complexity)
    ...
    def _create_single_subtask(self, task_id: str, description: str, task_type: TaskType, complexity: TaskComplexity) -> SubTask:
        """単一サブタスク作成"""
    ...
    def _split_complex_task(self, task_id: str, description: str, prompt: str, task_type: TaskType, complexity: TaskComplexity) -> List[SubTask]:
        """複雑なタスクの分割"""
        subtasks = []
        
        # タスクタイプに応じた分割戦略
        if task_type == TaskType.IMPLEMENTATION:
            subtasks = self._split_implementation_task(task_id, description, prompt, complexity)
        elif task_type == TaskType.DESIGN:
            subtasks = self._split_design_task(task_id, description, prompt, complexity)
        elif task_type == TaskType.RESEARCH:
            subtasks = self._split_research_task(task_id, description, prompt, complexity)
        elif task_type == TaskType.TESTING:
            subtasks = self._split_testing_task(task_id, description, prompt, complexity)
        elif task_type == TaskType.INTEGRATION:
            subtasks = self._split_integration_task(task_id, description, prompt, complexity)
        else:
            # 汎用的な分割
            subtasks = self._split_generic_task(task_id, description, prompt, task_type, complexity)
        
        return subtasks
    
    def _split_implementation_task(self, task_id: str, description: str, prompt: str, complexity: TaskComplexity) -> List[SubTask]:
        """実装タスクの分割"""
        subtasks = []
    ...
    def _split_design_task(self, task_id: str, description: str, prompt: str, complexity: TaskComplexity) -> List[SubTask]:
        """設計タスクの分割"""
        subtasks = []
        base_hours = self._estimate_hours_by_complexity(complexity)
    ...
    def _split_research_task(self, task_id: str, description: str, prompt: str, complexity: TaskComplexity) -> List[SubTask]:
        """調査タスクの分割"""
        subtasks = []
        base_hours = self._estimate_hours_by_complexity(complexity)
    ...
    def _split_testing_task(self, task_id: str, description: str, prompt: str, complexity: TaskComplexity) -> List[SubTask]:
        """テストタスクの分割"""
        subtasks = []
        base_hours = self._estimate_hours_by_complexity(complexity)
    ...
    def _split_integration_task(self, task_id: str, description: str, prompt: str, complexity: TaskComplexity) -> List[SubTask]:
        """統合タスクの分割"""
        subtasks = []
        base_hours = self._estimate_hours_by_complexity(complexity)
    ...
    def _split_generic_task(self, task_id: str, description: str, prompt: str, task_type: TaskType, complexity: TaskComplexity) -> List[SubTask]:
        """汎用的なタスク分割"""
        subtasks = []
        base_hours = self._estimate_hours_by_complexity(complexity)
    ...
    def _record_splitting_history(self, task_id: str, subtask_count: int, complexity_score: float):
        """分割履歴を記録"""
        with sqlite3.connect(self.db_path) as conn:
            # 時間削減見積もり
    ...
    def get_parallel_execution_groups(self, subtasks: List[SubTask]) -> List[List[SubTask]]:
        """並列実行可能なグループを特定"""
        try:
            # 依存関係に基づくトポロジカルソート
            groups = []
            ...
            while remaining_tasks:
                ...
                # 並列実行可能なタスクをグループ化
                parallel_group = [task for task in executable_tasks if task.can_parallel]
                sequential_tasks = [task for task in executable_tasks if not task.can_parallel]
...
if __name__ == "__main__":
    ...
    for test_case in test_cases:
        ...
        for i, subtask in enumerate(subtasks, 1):
            ...
            print(f"     Type: {subtask.task_type.value}, Hours: {subtask.estimated_hours}")
