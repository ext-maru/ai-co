...
class KnightSquad:
    ...
    def __init__(self, squad_id: str, squad_type: str):
        ...
        logger.info(
            f"⚔️ {squad_type.title()} Squad '{squad_id}' formed - Ready for duty!"
        )
    ...
    def equip_weapons_from_workshop(
        self, workshop_weapons: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """ドワーフ工房からの武具装備"""
        equipped_weapons = []
        ...
        for weapon_data in workshop_weapons:
            weapon_id = weapon_data.get(
                "weapon_id", weapon_data.get("tool_id", "unknown")
            )
            weapon_type = weapon_data.get(
                "weapon_type", weapon_data.get("tool_type", "unknown")
            )
            effectiveness = weapon_data.get(
                "effectiveness", weapon_data.get("detection_accuracy", 0.8)
            )
        ...
        # 分隊準備度更新
        self.combat_effectiveness = min(
            0.98, self.combat_effectiveness + total_effectiveness_boost
        )

        logger.info(
            f"⚔️ Squad {self.squad_id} equipped {len(equipped_weapons)} weapons from workshop"
        )
        ...
        return {
            ...
            "knights_equipped": len(
                set().union(*[w["assigned_knights"] for w in equipped_weapons])
            ),
        }
    ...
    def execute_defensive_maneuver(self, threat: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"🛡️ Squad {self.squad_id} defensive maneuver: {'SUCCESS' if maneuver_success else 'PARTIAL'}"
        )
    ...
    def execute_offensive_strike(self, target: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_attack_power = min(
            0.99, base_attack_power + weakness_bonus + equipment_bonus
        )
        ...
        logger.info(
            f"⚔️ Squad {self.squad_id} offensive strike: {'SUCCESS' if strike_success else 'MISS'}"
        )
    ...
    def conduct_reconnaissance(self, area: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 深度による難易度調整
        depth_multiplier = {"surface": 1.0, "medium": 0.8, "deep_analysis": 0.6}.get(
            depth, 0.8
        )
            ...
            intel_gathered.extend(
                [
                    ...
                    },
                    {
                        ...
                        "quality": intel_quality * 0.8,
                        "confidence": 0.8,
                    },
                    {
                        "type": "resource_utilization",
                        "quality": intel_quality * 0.9,
                        "confidence": 0.85,
                    },
                ]
            )
                ...
                [
                    {
                        ...
                        "quality": intel_quality * 0.7,
                        "confidence": 0.75,
                    },
                    {
                        "type": "vulnerability_assessment",
                        "quality": intel_quality * 0.6,
                        "confidence": 0.7,
                    },
                ]
            )
        ...
        logger.info(
            f"🔍 Squad {self.squad_id} reconnaissance complete: {len(intel_gathered)} intel items"
        )
    ...
    def _check_weapon_compatibility(self, weapon_type: str) -> float:
        ...
        compatibility_matrix = {
            "guardian": {
                ...
                "resource_guardian": 0.95,
                "memory_optimizer": 0.8,
                "performance_scout": 0.7,
                "cpu_balancer": 0.75,
                "cache_optimizer": 0.7,
            },
            "assault": {
                "cpu_balancer": 0.9,
                "performance_scout": 0.85,
                "cache_optimizer": 0.8,
                "anomaly_detector": 0.7,
                "memory_optimizer": 0.8,
                "resource_guardian": 0.6,
            },
            "scout": {
                "performance_scout": 0.95,
                "anomaly_detector": 0.9,
                "resource_guardian": 0.8,
                "memory_optimizer": 0.6,
                "cpu_balancer": 0.7,
                "cache_optimizer": 0.65,
            },
            "specialist": {
                "memory_optimizer": 0.95,
                "cpu_balancer": 0.9,
                "cache_optimizer": 0.9,
                "anomaly_detector": 0.85,
                "performance_scout": 0.8,
                "resource_guardian": 0.85,
            },
        }
    ...
    def _assign_weapon_to_knights(self, weapon_type: str) -> List[str]:
        ...
        # 最大2名まで割り当て
        return (
            specialized_knights[:2]
            if specialized_knights
            else [self.knights[0]["knight_id"]]
        )
    ...
    def _assign_knights_to_defense(
        self, target_systems: List[str]
    ) -> Dict[str, List[str]]:
        """防御への騎士配置"""
        assignments = {}
        available_knights = [
            k["knight_id"] for k in self.knights if k["status"] == "ready"
        ]

        knights_per_system = max(
            1, len(available_knights) // max(len(target_systems), 1)
        )
    ...
    def request_weapon_from_workshop(
        self,
        weapon_type: str,
        quantity: int = 1,
        priority: str = "medium",
        ...
        try:
            ...
            request = WeaponRequest(
                ...
                + timedelta(hours=1 if priority == "high" else 6),
            )
            ...
            logger.info(
                f"📤 Squad {self.squad_id} requested {quantity} {weapon_type} from workshop"
            )
            return request_id
    ...
    def receive_weapon_delivery(self, weapon_specs: List[Dict[str, Any]]) -> bool:
        ...
        try:
            for weapon_spec in weapon_specs:
                ...
                # 騎士に装備を割り当て
                available_knights = [
                    k for k in self.knights if "primary_weapon" not in k
                ]
    ...
    def _calculate_weapon_compatibility(self, weapon_type: str) -> float:
        ...
        compatibility_matrix = {
            "guardian": {
                "resource_guardian": 0.95,
                "emergency_shield": 0.9,
                "anomaly_detector": 0.85,
                "memory_optimizer": 0.8,
            },
            "assault": {
                "cpu_balancer": 0.95,
                "performance_scout": 0.9,
                "cache_optimizer": 0.85,
                "tactical_analyzer": 0.8,
            },
            "scout": {
                "anomaly_detector": 0.95,
                "performance_scout": 0.9,
                "tactical_analyzer": 0.85,
                "resource_guardian": 0.8,
            },
            "specialist": {
                "memory_optimizer": 0.9,
                "cpu_balancer": 0.9,
                "cache_optimizer": 0.85,
                "rapid_recovery_kit": 0.95,
            },
        }

        return compatibility_matrix.get(self.squad_type, {}).get(weapon_type, 0.7)

    def _select_offensive_tactics(
        self, target_type: str, weakness: Optional[str]
    ) -> List[str]:
        """攻撃戦術選定"""
        tactics_database = {
            "system_bottleneck": [
    ...
    def _generate_recon_recommendations(
        self, intel_data: List[Dict[str, Any]], threat_level: str
    ) -> List[str]:
        """偵察結果に基づく推奨アクション生成"""
        recommendations = []
        ...
        if threat_level == "critical":
            ...
            recommendations.extend(
                ["increased_surveillance", "optimize_current_defenses"]
            )
        else:
    ...
    def _gain_experience(self, action_type: str, difficulty: str, success: bool):
        """経験値獲得"""
        base_exp = {"low": 5, "medium": 10, "high": 15, "critical": 20}.get(
            difficulty, 10
        )
        ...
        for knight in self.knights:
            ...
            if knight["experience"] % 100 == 0:
                ...
                knight["equipment_proficiency"] = min(
                    0.99, knight["equipment_proficiency"] + 0.01
                )
...
class BattlefieldScout:
    ...
    def analyze_attack_patterns(
        ...
        recent_severity = statistics.mean(
            [severity_scores.get(s, 2) for s in incident_severities[-3:]]
        )
        early_severity = statistics.mean(
            [severity_scores.get(s, 2) for s in incident_severities[:3]]
        )
...
class WeaponryManager:
    ...
    def __init__(self):
        ...
        logger.info(
            "⚔️ WeaponryManager initialized - Ready to receive workshop deliveries"
        )

    def receive_weapons_from_workshop(
        self, workshop_delivery: Dict[str, Any]
    ) -> Dict[str, Any]:
        """ドワーフ工房からの武具受領"""
        delivery_id = workshop_delivery.get(
        ...
        avg_quality = statistics.mean(
            [qa["overall_score"] for qa in quality_assessments]
        )
        ...
        quality_feedback = {
            ...
            "quality_improvement_suggestions": self._generate_quality_suggestions(
                quality_assessments
            ),
        }
    ...
    def deploy_weapon_to_squad(
        ...
        suitable_weapons = [
            ...
            if weapon_data.get("weapon_type", weapon_data.get("tool_type"))
            == weapon_type
        ...
        if not suitable_weapons:
            return {
                ...
                "alternative_suggestions": self._suggest_alternative_weapons(
                    weapon_type
                ),
            }

        # 最高品質の武具選択
        best_weapon_id, best_weapon = max(
            suitable_weapons, key=lambda x: x[1]["quality_score"]
        )
        ...
        mission_compatibility = self._calculate_mission_compatibility(
            weapon_type, mission_type
        )
    ...
    def perform_maintenance_check(self) -> Dict[str, Any]:
        ...
        for weapon_id, weapon_data in self.weapon_inventory.items():
            ...
            needs_maintenance = (
                condition < 0.8
                or usage_hours > 200
                or (datetime.now() - last_maintenance).days > 7
            )
                ...
                maintenance_needed.append(
                    {
                        "weapon_id": weapon_id,
                        "weapon_type": weapon_data.get(
                            "weapon_type", weapon_data.get("tool_type")
                        ),
                        "condition": condition,
        ...
        avg_condition = (
            statistics.mean(overall_conditions) if overall_conditions else 1.0
        )
        ...
        return {
            ...
            "maintenance_rate": len(maintenance_needed)
            / max(len(self.weapon_inventory), 1),
        }
    ...
    def _assess_weapon_quality(self, weapon: Dict[str, Any]) -> Dict[str, Any]:
        ...
        overall_score = (
            accuracy_score * 0.4 + speed_score * 0.3 + reliability_score * 0.3
        )
    ...
    def _assess_tool_quality(self, tool: Dict[str, Any]) -> Dict[str, Any]:
        ...
        overall_score = (
            effectiveness_score * 0.5 + improvement_score * 0.3 + readiness_score * 0.2
        )
    ...
    def _generate_quality_suggestions(
        ...
        avg_scores = {
            "accuracy": statistics.mean(
                [
                    a.get("component_scores", {}).get("accuracy", 0.8)
                    for a in assessments
                ]
            ),
            "speed": statistics.mean(
                [a.get("component_scores", {}).get("speed", 0.8) for a in assessments]
            ),
            "reliability": statistics.mean(
                [
                    a.get("component_scores", {}).get("reliability", 0.8)
                    for a in assessments
                ]
            ),
        }
    ...
    def _calculate_mission_compatibility(
        self, weapon_type: str, mission_type: str
    ) -> float:
        """任務適合性計算"""
        compatibility_matrix = {
...
class TacticalCoordinator:
    ...
    def plan_coordinated_response(
        self, threat_scenario: Dict[str, Any]
    ) -> Dict[str, Any]:
        """協調対応計画立案"""
        ...
        # 分隊配置計画
        squad_assignments = self._plan_squad_assignments(
            threat_type, affected_systems, severity
        )

        # 実行タイムライン作成
        timeline = self._create_execution_timeline(
            squad_assignments, required_response_time
        )
    ...
    def coordinate_multi_squad_operation(
        ...
        coordination_success = (
            len(objectives_achieved) >= len(objectives) * 0.6
        )  # 60%で成功とする
    ...
    def execute_emergency_protocol(self, emergency: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 緊急プロトコル選定
        protocol_type = self._select_emergency_protocol(
            alert_level, threat_type, time_to_critical
        )
        ...
        for squad_id in mobilized_squads:
            emergency_action = self._execute_emergency_action(
                squad_id, threat_type, time_to_critical
            )
    ...
    # プライベートヘルパーメソッド群
    def _plan_squad_assignments(
        self, threat_type: str, affected_systems: List[str], severity: str
    ...
    def _create_execution_timeline(
        self, squad_assignments: Dict[str, Dict[str, Any]], response_time: int
    ) -> List[Dict[str, Any]]:
        """実行タイムライン作成"""
        timeline = []
        ...
        timeline.append(
            {
                ...
                "squads_involved": [
                    squad
                    for squad, data in squad_assignments.items()
    ...
    def _calculate_operation_success_probability(
        self,
        squad_assignments: Dict[str, Dict[str, Any]],
        threat_type: str,
        severity: str,
    ) -> float:
        """作戦成功確率計算"""
        ...
        final_probability = (
            (base_probability + squad_count_bonus)
            * threat_difficulty
            * severity_modifier
        )
    ...
    def _calculate_resource_requirements(
        self, squad_assignments: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        """リソース要求計算"""
        return {
    ...
    def _assign_objectives_to_squads(
        self, squads: List[str], objectives: List[str]
    ) -> Dict[str, List[str]]:
        """目標の分隊割り当て"""
        assignments = {}
    ...
    def _execute_squad_objectives(
        self, squad_id: str, objectives: List[str]
    ) -> Dict[str, Any]:
        """分隊目標実行"""
        execution_start = datetime.now()
    ...
    def _calculate_coordination_effectiveness(
        ...
        coordination_req: Dict[str, Any],
    ) -> float:
        """協調効果計算"""
        individual_success_rates = [
            result["success_rate"] for result in execution_results.values()
        ]
        ...
        total_effectiveness = min(
            0.98, avg_individual_success + coordination_bonus + timing_bonus
        )

        return total_effectiveness

    def _select_emergency_protocol(
        self, alert_level: str, threat_type: str, time_to_critical: int
    ) -> str:
        """緊急プロトコル選定"""
    ...
    def _emergency_squad_mobilization(
        self, alert_level: str, threat_type: str
    ) -> List[str]:
        """緊急分隊動員"""
        available_squads = [
            "guardian_alpha",
    ...
    def _execute_emergency_action(
        self, squad_id: str, threat_type: str, time_limit: int
    ) -> Dict[str, Any]:
        """緊急アクション実行"""
        ...
        time_pressure_penalty = max(
            0, (300 - time_limit) / 300 * 0.2
        )  # 5分未満で最大20%ペナルティ
...
class KnightBrigade:
    ...
    def __init__(self):
        ...
        logger.info(
            f"🏰 Knight Brigade '{self.brigade_name}' assembled: {self.brigade_id}"
        )
        logger.info(f"⚔️ {len(self.squads)} squads ready for deployment")

    def integrate_with_dwarf_workshop(
        self, workshop_connection: Dict[str, Any]
    ) -> Dict[str, Any]:
        """ドワーフ工房との統合"""
        ...
        # 工房との通信確立
        connection_test = self._test_workshop_communication(
            workshop_id, communication_channel
        )
        ...
        if connection_test["success"]:
            ...
            return {
                ...
                "delivery_schedule": {
                    ...
                    + timedelta(seconds=delivery_frequency),
                },
    ...
    def receive_workshop_intelligence(
        self, workshop_intel: Dict[str, Any]
    ) -> Dict[str, Any]:
        """工房からの情報受信・処理"""
        optimization_opportunities = workshop_intel.get(
        ...
        logger.info(
            f"📡 Workshop intelligence processed: {len(tactical_adjustments)} adjustments"
        )
    ...
    def execute_coordinated_defense(
        self, threat_scenario: Dict[str, Any]
    ) -> Dict[str, Any]:
        """協調防御実行"""
        ...
        for squad_id, assignment in defense_plan["squad_assignments"].items():
            if squad_id in self.squads:
                ...
                # 工房武具で装備強化
                workshop_weapons = self._request_immediate_weapons(
                    squad_id, attack_vectors
                )
                if workshop_weapons:
        ...
        for vector in attack_vectors:
            ...
            vector_neutralized = any(
                result.get("threat_neutralized", False)
                or result.get("target_eliminated", False)
        ...
        workshop_support_used = any(
            "workshop" in str(result).lower() for result in squad_results.values()
        )
        ...
        logger.info(
            f"🛡️ Coordinated defense complete: {'SUCCESS' if defense_success else 'PARTIAL'}"
        )
    ...
    def request_emergency_weapons(
        self, emergency_request: Dict[str, Any]
    ) -> Dict[str, Any]:
        """緊急武具要求"""
        ...
        self.weaponry_manager.workshop_connection["pending_requests"].append(
            request_record
        )
    ...
    def get_brigade_status(self) -> Dict[str, Any]:
        ...
        for squad_id, squad in self.squads.items():
            ...
            squad_status[squad_id] = {
                ...
                "experience_level": statistics.mean(
                    [k["experience"] for k in squad.knights]
                ),
            }
        ...
        weapon_inventory = {
            ...
            "available_weapons": len(
                [
                    w
                    for w in self.weaponry_manager.weapon_inventory.values()
        ...
        workshop_coordination = {
            ...
            "last_delivery": self.weaponry_manager.workshop_connection.get(
                "last_delivery"
            ),
            "pending_requests": len(
                self.weaponry_manager.workshop_connection["pending_requests"]
            ),
            "quality_feedback_pending": len(
                self.weaponry_manager.workshop_connection["quality_feedback"]
            ),
        }
    ...
    # プライベートヘルパーメソッド群
    def _test_workshop_communication(
        self, workshop_id: str, channel: str
    ) -> Dict[str, Any]:
        """工房通信テスト"""
        # 簡易通信テスト
        ...
        if test_success:
            ...
            return {"success": False, "error": "Communication channel unavailable"}
    ...
    def _distribute_intel_to_squads(
        ...
        for squad_id, squad in self.squads.items():
            ...
            if squad.squad_type == "scout":
                squad_briefing.extend(
                    [f"New threat: {t}" for t in intel["new_threats"]]
                )
            elif squad.squad_type == "guardian":
                squad_briefing.extend(
                    [
                        f"Defense priority: {opp['type']}"
                        for opp in intel["high_priority_opportunities"]
                    ]
                )
            elif squad.squad_type == "specialist":
                squad_briefing.extend(
                    [
                        f"Optimization target: {opp['type']}"
                        for opp in intel["high_priority_opportunities"]
                    ]
                )
    ...
    def _assess_threat_level(
        self, attack_type: str, attack_vectors: List[str], intensity: str
    ) -> str:
        """脅威レベル評価"""
        threat_score = 0
    ...
    def _request_immediate_weapons(
        self, squad_id: str, attack_vectors: List[str]
    ) -> List[Dict[str, Any]]:
        """即座武具要求"""
        # 攻撃ベクターに基づく武具選択
        weapon_mapping = {
        ...
        for vector in attack_vectors:
            ...
            available_weapons = [
                (wid, wdata)
                for wid, wdata in self.weaponry_manager.weapon_inventory.items()
    ...
    def _simulate_workshop_emergency_response(
...
if __name__ == "__main__":
    ...
    print(
        f"  Integration: {'SUCCESS' if integration['integration_successful'] else 'FAILED'}"
    )
    print(
        f"  Communication: {'ESTABLISHED' if integration['communication_established'] else 'FAILED'}"
    )
    ...
    print(
        f"  Defense Success: {'SUCCESS' if defense_result['defense_success'] else 'PARTIAL'}"
    )
