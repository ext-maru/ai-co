...
class KnowledgeConsolidator(BaseManager):
    ...
    def scan_project_structure(self) -> Dict[str, Any]:
        ...
        # 主要ディレクトリのスキャン
        key_dirs = [
            "core", "workers", "libs", "scripts", "commands", 
            "config", "templates", "knowledge_base", "web", "bin"
        ]
    ...
    def analyze_implementations(self) -> Dict[str, Any]:
        ...
        for worker_file in (self.project_root / "workers").glob("*_worker.py"):
            analysis["workers"][worker_file.stem] = self._analyze_python_file(worker_file)
            
        # マネージャーの分析
        for manager_file in (self.project_root / "libs").glob("*_manager.py"):
            analysis["managers"][manager_file.stem] = self._analyze_python_file(manager_file)
        ...
        for core_file in (self.project_root / "core").glob("*.py"):
            if not core_file.name.startswith('__'):
                analysis["core_modules"][core_file.stem] = self._analyze_python_file(core_file)
    ...
    def generate_documentation(self) -> Path:
        ...
        with open(doc_path, 'w', encoding='utf-8') as f:
            ...
            f.write("## 📈 統計情報\n\n")
            f.write("### ファイルタイプ別統計\n")
            for file_type, count in project_structure['statistics']['file_types'].items():
    ...
    def create_interactive_report(self) -> Path:
        ...
        self.logger.info(f"{EMOJI['success']} Interactive report created: {report_path}")
        return report_path
...
<body>
    ...
    def run_consolidation(self):
        ...
        try:
            ...
            self.logger.info(f"{EMOJI['party']} Knowledge consolidation completed successfully!")
