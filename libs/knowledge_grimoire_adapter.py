        ...
        if grimoire_enabled:
            try:
                ...
                config = RagGrimoireConfig(
                    database_url=os.getenv('GRIMOIRE_DATABASE_URL', "postgresql://localhost/grimoire"),
                ...
                try:
                    from libs.mock_grimoire_database import MockGrimoireDatabase, MockGrimoireVectorSearch
...
""")
    ...
    def _direct_postgresql_search(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
                ...
                for line in result.stdout.strip().split('\n'):
                    if line.strip():
                        ...
                        if len(parts) >= 2:
                            rows.append({
                                'spell_name': parts[0],
                                'content': parts[1][:1000] + ('...' if len(parts[1]) > 1000 else ''),
    ...
    def search_knowledge(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            combined_results.sort(key=lambda x: (
                x.get('similarity_score', 0.0) if x.get('source_system') == 'grimoire' else 0.0
            ), reverse=True)
    ...
    def add_knowledge(self, 
                     ...
                     save_to_legacy: bool = True) -> Dict[str, Any]:
        """新しい知識を追加（魔法書システム優先）"""
        result = {
        ...
        if save_to_legacy:
            try:
                # 既存システム用のファイル保存
                knowledge_file = Path("/home/aicompany/ai_co/knowledge_base") / f"{spell_name}.md"
...
"""
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "legacy_system": {
                "available": True,
                "knowledge_files": len(self.knowledge_manager.get_all_knowledge_files())
            },
            "grimoire_system": {
            ...
            if self.use_mock_grimoire:
                ...
                try:
                    ...
                    try:
                        ...
                        status["grimoire_system"]["available"] = grimoire_status["integration_active"]
                        status["grimoire_system"]["stats"] = grimoire_status.get("grimoire_stats", {})
...
class KnowledgeSageGrimoireIntegration(KnowledgeGrimoireAdapter):
    ...
    def evolve_knowledge(self, 
                        spell_name: str, 
                        new_content: str, 
                        evolution_reason: str = "Knowledge evolution") -> Dict[str, Any]:
        ...
        try:
            ...
            if similar_spells:
                ...
                # 進化エンジンがあれば使用
                if hasattr(self.grimoire_integration, 'evolution_engine') and \
                   self.grimoire_integration.evolution_engine:
