...
class LearningOptimizer(BaseManager):
    ...
    def record_execution(self, error_type: str, strategy: Dict, 
    ...
    def get_optimized_strategy(self, error_type: str, context: Optional[Dict] = None) -> Optional[Dict]:
        """最適化された戦略を取得"""
        # キャッシュチェック
    ...
    def optimize_all_strategies(self) -> Dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT DISTINCT error_type FROM strategy_executions"
            )
    ...
    def _optimize_strategy_for_error_type(self, error_type: str) -> Optional[OptimizationResult]:
        """特定エラータイプの戦略を最適化"""
        # 実行履歴を取得
        ...
        if optimized:
            # 改善度を計算
            improvement = self._calculate_improvement(current_best, optimized, executions)
    ...
    def _learn_from_executions(self, error_type: str):
        ...
        self.logger.info(f"Learned model for {error_type} from {len(executions)} executions")
    ...
    def _get_feature_names(self) -> List[str]:
        """特徴量名を取得"""
        return [
            'hour_of_day', 'day_of_week', 'system_load', 'memory_usage',
            'error_frequency', 'execution_time', 'resource_usage',
            'is_reactive', 'is_preventive', 'is_adaptive'
        ]
        ...
        for strategy in top_strategies:
            ...
            if self._evaluate_strategy(strategy, executions) > 0.7:
                combined['strategies'].append({
                    'condition': self._extract_success_condition(strategy, executions),
                    'action': strategy
                })
    ...
    def _evolutionary_optimization(self, strategy: Dict, executions: List[Dict]) -> Dict:
        """進化的最適化"""
        population_size = 20
        generations = 10
        mutation_rate = 0.1
        ...
        for generation in range(generations):
            # 適応度を評価
            fitness_scores = [
                (s, self._evaluate_strategy(s, executions)) 
                for s in population
            ]
        ...
        # 最良個体を返す
        best_strategy = max(population, key=lambda s: self._evaluate_strategy(s, executions))
    ...
    def _mutate_strategy(self, strategy: Dict) -> Dict:
        ...
        if 'retry_count' in mutated and np.random.random() < 0.3:
            mutated['retry_count'] = max(1, mutated['retry_count'] + np.random.randint(-1, 2))
    ...
    def _crossover_strategies(self, strategy1: Dict, strategy2: Dict) -> Dict:
        ...
        for key in all_keys:
            if key in strategy1 and key in strategy2:
                # 両方にある場合はランダムに選択
                child[key] = strategy1[key] if np.random.random() < 0.5 else strategy2[key]
    ...
    def _calculate_context_match(self, context: Dict, patterns: Dict) -> float:
        ...
        for key, pattern_value in patterns.items():
            if key in context:
                ...
                # 値の類似度
                if isinstance(context[key], (int, float)) and isinstance(pattern_value.get('value'), (int, float)):
                    diff = abs(context[key] - pattern_value['value'])
                    similarity = 1.0 / (1.0 + diff)
                else:
                    similarity = 1.0 if context[key] == pattern_value.get('value') else 0.0
    ...
    def _calculate_improvement(self, original: Dict, optimized: Dict, 
                             executions: List[Dict]) -> float:
        """改善度を計算"""
    ...
    def _save_optimized_strategy(self, error_type: str, strategy: Dict, improvement: float):
        """最適化された戦略を保存"""
    ...
    def _extract_success_condition(self, strategy: Dict, executions: List[Dict]) -> Dict:
        """成功条件を抽出"""
        # 成功した実行のコンテキストを分析
        successful_contexts = []
    ...
    def _update_optimization_stats(self, results: Dict):
        ...
        if results['improvements'] > 0:
            ...
            new_avg_improvement = sum(
                r.improvement_score for r in results['optimized_strategies']
            ) / new_improvements if new_improvements > 0 else 0
            
            # 全体平均を更新
            self.optimization_stats['avg_improvement'] = (
                (current_avg * (total_optimizations - 1) + new_avg_improvement) / 
                total_optimizations
            ) if total_optimizations > 0 else new_avg_improvement
