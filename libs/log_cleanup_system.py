...
class LogCleanupSystem:
    ...
    def cleanup_by_size_threshold(self, target_size_mb: float = 500) -> CleanupStats:
        ...
        if current_size <= target_size_mb:
            logger.info(f"Current size {current_size:.1f}MB is within target {target_size_mb}MB")
            return stats
        ...
        for log_file in log_files:
            ...
            if log_file.age_days > rule["max_age_days"] or log_file.size_mb > rule["max_size_mb"]:
    ...
    def execute_scheduled_cleanup(self) -> Dict[str, any]:
        ...
        cleanup_result["actions_taken"].append(f"サイズ最適化: {size_stats.files_processed}個処理")
        ...
        cleanup_result["after"] = {
            ...
            "reduction_mb": cleanup_result["before"]["total_size_mb"] - sum(f.size_mb for f in final_log_files)
        }
    ...
    def generate_cleanup_report(self) -> Dict[str, any]:
        ...
        # タイプ別統計
        type_stats = defaultdict(lambda: {"count": 0, "total_size_mb": 0, "avg_age_days": 0})
        ...
        return {
            ...
            "summary": {
                ...
                "largest_file": max(log_files, key=lambda x: x.size_mb).path.name if log_files else None,
                "oldest_file": max(log_files, key=lambda x: x.age_days).path.name if log_files else None
            },
            "type_statistics": dict(type_stats),
            "recommendations": recommendations,
            "cleanup_preview": {
                "estimated_compression_savings_mb": sum(f.size_mb * 0.9 for f in log_files if f.size_mb > 10),
                "deletable_old_files": len([f for f in log_files if f.age_days > 30]),
                "large_files_to_compress": len([f for f in log_files if f.size_mb > 20])
            }
        }
...
def main():
    ...
    for log_type, stats in report['type_statistics'].items():
        print(f"   {log_type}: {stats['count']}個, {stats['total_size_mb']:.1f}MB, 平均{stats['avg_age_days']:.1f}日")
    ...
    # レポート保存
    cleanup_system.export_report("/home/aicompany/ai_co/ai_todo/log_cleanup_report.json")
    print(f"\n📄 詳細レポートを保存しました")
