...
class ManaSystem:
    ...
    def update_mana(self) -> None:
        ...
        for spirit_name, spirit_data in self.spirit_mana.items():
            ...
            if random.random() < 0.1:  # 10%の確率で活動
                drain_amount = spirit_data["drain_rate"] * time_diff * random.uniform(5, 15)
            ...
            spirit_data["current"] = min(
                spirit_data["current"] + regen_amount,
                spirit_data["max"]
            )
    ...
    def restore_mana(self, spirit: str, amount: float) -> Dict[str, Any]:
        ...
        spirit_data["current"] = min(spirit_data["current"] + amount, spirit_data["max"])
...
if __name__ == "__main__":
    ...
    for spirit, data in status["spirits"].items():
        print(f"  {data['name']}: {data['current']}/{data['max']} ({data['percentage']}%)")
