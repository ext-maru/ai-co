...
"""
                    ...
                    if notify_slack and result.returncode == 0:
                        await self._notify_slack(f"✅ Command {task_name} completed successfully")
                    elif notify_slack:
                        await self._notify_slack(f"❌ Command {task_name} failed with code {result.returncode}")
        ...
        async def check_result(task_name: str) -> Dict[str, Any]:
            ...
            if result:
                # ログファイルの内容も取得
                log_file = AI_COMMANDS_DIR / "logs" / f"{task_name}_{result.get('timestamp', '')}.log"
                if log_file.exists():
        ...
        async def get_execution_summary() -> Dict[str, Any]:
            ...
            if recent_logs:
                success_count = sum(1 for log in recent_logs if log.get('exit_code') == 0)
                summary['recent_success_rate'] = f"{(success_count / len(recent_logs)) * 100:.1f}%"
