...
try:
    ...
    from libs.grand_elder_approval_system import GrandElderApprovalSystem, FourSagesReviewer
    ...
    class MockGrimoireVectorSearch:
        async def initialize(self): return True
        async def index_spell(self, spell_id, spell_data): return True
...
class MDFileAnalyzer:
    ...
    async def _extract_basic_info(self, file_path: Path, content: str) -> Dict[str, Any]:
        """基本情報抽出"""
        stat = file_path.stat()
    ...
    async def _suggest_classification(self, content: str, basic_info: Dict[str, Any]) -> Dict[str, Any]:
        """分類提案"""
        ...
        if any(keyword in content_lower for keyword in ['task', 'todo', 'プロジェクト', 'タスク', '進捗']):
            magic_school = MagicSchool.TASK_ORACLE
        elif any(keyword in content_lower for keyword in ['error', 'incident', 'エラー', '障害', '緊急', 'fix']):
            magic_school = MagicSchool.CRISIS_SAGE
        elif any(keyword in content_lower for keyword in ['search', 'find', 'rag', '検索', '探索']):
            magic_school = MagicSchool.SEARCH_MYSTIC
        
        # 呪文タイプの判定
        spell_type = SpellType.KNOWLEDGE  # デフォルト
        
        if any(keyword in content_lower for keyword in ['guide', 'how to', '手順', 'step', 'procedure']):
            spell_type = SpellType.PROCEDURE
        elif any(keyword in content_lower for keyword in ['config', 'setting', '設定', 'configuration']):
            spell_type = SpellType.CONFIGURATION
        elif any(keyword in content_lower for keyword in ['template', 'テンプレート', 'example']):
            spell_type = SpellType.TEMPLATE
        elif any(keyword in content_lower for keyword in ['reference', 'api', 'doc', 'リファレンス']):
    ...
    def _estimate_power_level(self, content: str, basic_info: Dict[str, Any]) -> int:
        ...
        # 重要キーワードによる加点
        important_keywords = ['critical', 'important', '重要', '必須', 'required', 'essential']
...
class DuplicateDetector:
    ...
    async def detect_duplicates(self, analyzed_files: List[Dict[str, Any]]) -> List[List[str]]:
        """重複検出実行"""
        try:
            # ハッシュベースの完全一致検出
            ...
            logger.info(f"🔍 重複検出完了: {len(exact_duplicates)}個の完全一致, {len(similar_groups)}個の類似グループ")
    ...
    async def _find_exact_duplicates(self, analyzed_files: List[Dict[str, Any]]) -> List[List[str]]:
        """完全一致検出"""
        hash_groups = {}
    ...
    async def _find_similar_content(self, analyzed_files: List[Dict[str, Any]]) -> List[List[str]]:
        """類似コンテンツ検出"""
...
class MigrationEngine:
    ...
    async def run_full_migration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            duplicate_groups = await self.duplicate_detector.detect_duplicates(analyzed_files)
            ...
            # Phase 5: 重複処理戦略決定
            deduplication_plan = await self._create_deduplication_plan(duplicate_groups, classified_files)
            
            # Phase 6: バッチ移行実行
            logger.info("🚀 Phase 6: バッチ移行実行開始")
            migration_results = await self._execute_migration_batches(classified_files, deduplication_plan)
            
            # Phase 7: 統計レポート生成
            migration_end = datetime.now(timezone.utc)
            final_report = await self._generate_migration_report(
                migration_id, migration_start, migration_end, migration_results, duplicate_groups
            )
    ...
    async def _classify_with_four_sages(self, analyzed_files: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """4賢者による分類"""
        classified_files = []
        ...
        for file_analysis in analyzed_files:
            ...
            try:
                ...
                logger.error(f"❌ 4賢者分類エラー {file_analysis.get('file_path', 'unknown')}: {e}")
    ...
    async def _get_sage_classification(self, file_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """4賢者分類取得"""
        # 簡略化実装：分析結果ベースで分類
    ...
    def _calculate_crisis_score(self, file_analysis: Dict[str, Any]) -> float:
        ...
        if aicompany_analysis.get('incident_content', 0) > 0:
            score += 0.4
        if any(keyword in basic_info.get('file_name', '').lower() 
    ...
    async def _create_deduplication_plan(self, duplicate_groups: List[List[str]], 
                                       classified_files: List[Dict[str, Any]]) -> Dict[str, Any]:
        """重複除去計画作成"""
        plan = {
        ...
        for file_analysis in classified_files:
            if 'error' not in file_analysis:
                ...
                basic_info = file_analysis.get('basic_info', {})
                file_info_map[basic_info.get('relative_path', file_path)] = file_analysis
        ...
        logger.info(f"📋 重複除去計画作成完了: {len(plan['merge_candidates'])}個の自動統合, "
                   f"{len(plan['manual_review_required'])}個の手動レビュー")
        
        return plan
    
    async def _select_master_file(self, duplicate_group: List[str], 
                                file_info_map: Dict[str, Any]) -> str:
        """マスターファイル選定"""
        scored_files = []
    ...
    async def _execute_migration_batches(self, classified_files: List[Dict[str, Any]], 
                                       deduplication_plan: Dict[str, Any]) -> List[MigrationResult]:
        """バッチ移行実行"""
        migration_results = []
        ...
        for i in range(0, len(classified_files), self.batch_size):
            ...
            batch_results = await self._process_migration_batch(batch, skip_files, batch_id)
    ...
    async def _process_migration_batch(self, batch: List[Dict[str, Any]], 
                                     skip_files: Set[str], batch_id: str) -> List[MigrationResult]:
        """単一バッチ処理"""
        batch_results = []
    ...
    async def _migrate_single_file(self, file_analysis: Dict[str, Any], 
                                 skip_files: Set[str]) -> MigrationResult:
        """単一ファイル移行"""
        file_path = file_analysis.get('file_path', '')
    ...
    async def _build_spell_data(self, file_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        spell_type_value = spell_type.value if hasattr(spell_type, 'value') else str(spell_type)
        ...
        spell_data = {
            ...
            'migration_metadata': {
                ...
                'ai_company_relevance': sage_classification.get('ai_company_relevance', 0.0)
            }
        }
        
        return spell_data
    
    def _generate_spell_name(self, basic_info: Dict[str, Any], 
    ...
    async def _generate_migration_report(self, migration_id: str, start_time: datetime,
                                       end_time: datetime, migration_results: List[MigrationResult],
                                       duplicate_groups: List[List[str]]) -> Dict[str, Any]:
        """移行レポート生成"""
        ...
        for status in MigrationStatus:
            status_counts[status.value] = sum(1 for r in migration_results if r.status == status)
        
        # 成功率計算
        success_rate = (status_counts.get('completed', 0) / max(len(migration_results), 1)) * 100
        ...
        for result in migration_results:
            if result.sage_classification:
                ...
                sage_distribution[primary_sage] = sage_distribution.get(primary_sage, 0) + 1
        ...
        logger.info(f"✅ 移行完了: {status_counts.get('completed', 0)}/{len(migration_results)}ファイル "
