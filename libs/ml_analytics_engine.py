...
class FeatureEngineer:
    ...
    async def engineer_features(self, df: pd.DataFrame, feature_config: Dict) -> pd.DataFrame:
        """特徴量エンジニアリング実行"""
        result = df.copy()
        ...
        if model_type in [ModelType.REGRESSION, ModelType.CLASSIFICATION]:
            ...
            if algorithm == 'kmeans':
                metrics = {
                    ...
                    'silhouette_score': silhouette_score(X, model.labels_) if len(set(model.labels_)) > 1 else 0
                }
            else:
                metrics = {
...
class PredictionEngine:
    ...
    async def predict(self, model: MLModel, X: pd.DataFrame) -> Union[np.ndarray, pd.DataFrame]:
        """予測実行"""
        ...
        if model.model_type == ModelType.CLUSTERING:
            result = pd.DataFrame({
                'cluster': predictions,
                'confidence': 1.0  # クラスタリングの場合は信頼度1.0
            })
        else:
            # 確率予測が可能な場合
...
class AnomalyDetector:
    ...
    async def _calculate_anomaly_scores(self, df: pd.DataFrame, anomalies: np.ndarray) -> np.ndarray:
        """異常スコア計算"""
        numeric_cols = df.select_dtypes(include=[np.number]).columns
...
class MLAnalyticsEngine:
    ...
    async def train_and_save_model(self,
                                  ...
                                  model_params: Dict = None) -> MLModel:
        """モデル訓練と保存"""
    ...
    async def predict_with_model(self, model_id: str, data: pd.DataFrame) -> pd.DataFrame:
        """保存済みモデルで予測"""
    ...
    async def detect_anomalies_in_data(self, data: pd.DataFrame, method: str = 'statistical') -> pd.DataFrame:
        """データの異常検知"""
