...
class MockGrimoireDatabase:
    ...
    def __init__(self, database_url: Optional[str] = None):
        ...
        self.logger.info("🎭 Mock Grimoire Database initialized (PostgreSQL alternative)")
    ...
    def add_spell(self, spell_name: str, content: str, 
                  spell_type: str = "knowledge", magic_school: str = "general",
            ...
            if word in self.index_cache:
                for spell_id in self.index_cache[word]:
                    if spell_id not in seen_ids and spell_id in self.spells_cache:
                        ...
                        # 簡易スコア（出現回数）
                        score = sum(1 for w in query_words if w in spell['content'].lower())
...
class MockGrimoireVectorSearch:
    ...
    async def search(self, query: str, limit: int = 10, threshold: float = 0.0) -> List[Dict[str, Any]]:
