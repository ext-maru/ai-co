...
class ModelTrainer:
    ...
    def prepare_training_data(self, raw_data: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray]:
        """学習データ準備"""
        try:
            # データ形式の確認と変換
    ...
    def _create_time_series_windows(self, data: np.ndarray, 
    ...
    def _feature_engineering(self, X: np.ndarray, 
...
class ModelEvaluator:
    ...
    def _simple_cross_validation(self, model: Any, X: np.ndarray, 
                                y: np.ndarray, n_folds: int) -> Dict[str, Any]:
        """簡易クロスバリデーション"""
    ...
    def _get_feature_importance(self, model: Any, X: np.ndarray) -> Optional[List[float]]:
        """特徴量重要度取得"""
    ...
    def _generate_learning_curve(self, model: Any, X: np.ndarray, 
    ...
    def _assess_overfitting(self, cv_scores: Dict[str, Any], 
...
class TrainedModel:
    ...
    def __init__(self, model: Any, scaler: Any, model_type: str, 
