    ...
    def discover_instances(self, include_stale: bool = False) -> List[CCInstance]:
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            if not include_stale:
                ...
                cursor = conn.execute(query + " ORDER BY current_load ASC", (cutoff.isoformat(),))
            else:
...
class TaskDistributor:
    ...
    def distribute_task(self, task: DistributedTask) -> CCInstance:
        ...
        capable_instances = [
            ...
            if task.task_type in inst.capabilities
        ]
        ...
        logger.info(f"Distributed task {task.task_id} to instance {selected.instance_id}")
        return selected
    
    def get_instance_tasks(self, instance_id: str) -> List[Dict[str, Any]]:
        """Get tasks assigned to an instance"""
        return [
            record for record in self.distribution_history
...
class ConflictResolver:
    ...
    def detect_conflict(self, task1: DistributedTask, task2: DistributedTask) -> Optional[TaskConflict]:
    ...
    def merge_tasks(self, task1: DistributedTask, task2: DistributedTask) -> Optional[DistributedTask]:
...
class CCCommunicator:
    ...
    def receive_messages(self, recipient_id: str, only_unread: bool = False) -> List[CCMessage]:
...
class MultiCCCoordinator:
    ...
    def check_system_health(self) -> Dict[str, Any]:
        ...
        health_status = {
            ...
            'healthy_instances': len([i for i in instances if i.current_load < i.max_capacity * 0.9]),
    ...
    async def start_message_processing(self):
        ...
        self.communicator.register_handler(
            MessageType.TASK_HANDOFF.value,
        ...
        if self._message_handler:
            for msg_type in MessageType:
                self.communicator.register_handler(msg_type.value, self._message_handler)
