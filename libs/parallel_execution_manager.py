...
from libs.intelligent_task_splitter import IntelligentTaskSplitter, SubTask, TaskComplexity
...
class ParallelExecutionManager(BaseManager):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # インデックス作成
            conn.execute('CREATE INDEX IF NOT EXISTS idx_execution_groups_project ON execution_groups(project_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_execution_history_task ON execution_history(task_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_resource_usage_type ON resource_usage(resource_type)')
    ...
    def _connect_rabbitmq(self):
        ...
        try:
            ...
            # キュー宣言
            self.channel.queue_declare(queue='ai_tasks', durable=True, arguments={'x-max-priority': 10})
            self.channel.queue_declare(queue='parallel_tasks', durable=True, arguments={'x-max-priority': 10})
    ...
    def create_execution_group(self, project_id: str, phase: str, 
                             tasks: List[SubTask], max_parallel: int = None) -> str:
        """実行グループ作成"""
        try:
            group_id = f"group_{project_id}_{phase}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    group_id, project_id, phase, len(tasks), max_parallel,
                    execution_group.priority, estimated_duration,
                    json.dumps({k.value: v for k, v in resource_requirements.items()}),
    ...
    def execute_group_parallel(self, group_id: str) -> bool:
        ...
        try:
            ...
            # リソースチェック
            if not self._check_resource_availability(execution_group.resource_requirements):
            ...
            # タスクを依存関係に基づいて分離
            independent_tasks, dependent_tasks = self._separate_tasks_by_dependencies(execution_group.tasks)
            
            # 独立タスクを並列実行
            parallel_results = []
            if independent_tasks:
                parallel_results = self._execute_tasks_parallel(independent_tasks, execution_group.max_parallel)
    ...
    def _separate_tasks_by_dependencies(self, tasks: List[SubTask]) -> Tuple[List[SubTask], List[SubTask]]:
        """タスクを依存関係に基づいて分離"""
        independent_tasks = []
        dependent_tasks = []
    ...
    def _execute_tasks_parallel(self, tasks: List[SubTask], max_parallel: int) -> List[ExecutionResult]:
        """並列タスク実行"""
        try:
            ...
            if self.channel:
                ...
                self.channel.basic_publish(
                    ...
                    properties=pika.BasicProperties(
                        delivery_mode=2,  # 永続化
                        priority=min(task.priority + 3, 10)
                    )
                )
    ...
    def _order_tasks_by_dependencies(self, tasks: List[SubTask]) -> List[SubTask]:
        ...
        while remaining:
            ...
            for task in remaining:
                if not task.dependencies or all(dep in [t.id for t in ordered] for dep in task.dependencies):
    ...
    def _check_task_dependencies(self, task: SubTask, completed_results: List[ExecutionResult]) -> bool:
        """タスクの依存関係チェック"""
    ...
    def _calculate_resource_requirements(self, tasks: List[SubTask]) -> Dict[ResourceType, float]:
        """リソース要件計算"""
    ...
    def _estimate_execution_time(self, tasks: List[SubTask], max_parallel: int) -> float:
        """実行時間推定"""
        ...
        parallel_time = 0.0
        if parallel_tasks:
            avg_parallel_time = sum(t.estimated_hours for t in parallel_tasks) / len(parallel_tasks)
    ...
    def _check_resource_availability(self, requirements: Dict[ResourceType, float]) -> bool:
        ...
        for resource_type, required in requirements.items():
            ...
            if current_usage + required > limit:
                logger.warning(f"リソース不足: {resource_type.value} ({current_usage + required} > {limit})")
                return False
    ...
    def _update_resource_usage(self, usage: Dict[ResourceType, float], add: bool = True):
        """リソース使用量更新"""
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                ''', (
                    ...
                    result.output,
                    result.error,
                    json.dumps({k.value: v for k, v in result.resource_usage.items()})
                ))
        except Exception as e:
            logger.error(f"実行結果保存エラー: {e}")
    
    def _complete_group_execution(self, group_id: str, results: List[ExecutionResult], start_time: datetime):
        """グループ実行完了処理"""
        try:
            end_time = datetime.now()
            duration = (end_time - start_time).total_seconds()
            ...
            status = ExecutionStatus.COMPLETED if failed_count == 0 else ExecutionStatus.FAILED
            ...
            logger.info(f"📊 グループ実行完了: {group_id} ({duration:.1f}秒, 成功{len(results)-failed_count}/{len(results)})")
    ...
    def get_resource_usage(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "utilization": {
                k.value: (v / self.resource_limits[k] * 100) if self.resource_limits[k] > 0 else 0
...
if __name__ == "__main__":
    ...
    # テストタスク作成
    from libs.intelligent_task_splitter import SubTask, TaskType, TaskComplexity
    ...
    group_id = manager.create_execution_group(test_project_id, test_phase, test_tasks, max_parallel=2)
    ...
    if 'tasks' in status:
        ...
        for task in status['tasks']:
            print(f"    {task['task_id']}: {task['status']} ({task.get('duration', 0):.1f}秒)")
