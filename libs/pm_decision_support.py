...
class PMDecisionSupport(BaseManager):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # インデックス作成
            conn.execute('CREATE INDEX IF NOT EXISTS idx_decisions_project ON decision_recommendations(project_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_metrics_project ON project_metrics(project_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_patterns_type ON decision_patterns(pattern_name)')
    ...
    def generate_decision_recommendations(self, project_id: str) -> List[DecisionRecommendation]:
        """意思決定推奨事項の生成"""
        try:
    ...
    def _generate_recommendation_for_type(self, project_id: str, decision_type: DecisionType, 
                                        analysis: Dict[str, Any]) -> Optional[DecisionRecommendation]:
        """特定タイプの意思決定推奨生成"""
        try:
            ...
            # 推奨事項の生成
            recommendation_text = self._generate_recommendation_text(decision_type, metrics, trends)
            ...
            # 影響度の推定
            estimated_impact = self._estimate_decision_impact(decision_type, metrics, trends)
    ...
    def _needs_decision(self, decision_type: DecisionType, metrics: Dict[str, Any], 
        ...
        for metric_name in thresholds.keys():
            ...
            if trend.get("direction") == "decreasing" and trend.get("significance", 0) > 0.5:
                return True
        
        return False
    
    def _generate_recommendation_text(self, decision_type: DecisionType, 
                                    metrics: Dict[str, Any], trends: Dict[str, Any]) -> str:
        """推奨事項テキストの生成"""
        templates = {
            DecisionType.RESOURCE_ALLOCATION: [
    ...
    def _generate_reasoning(self, decision_type: DecisionType, metrics: Dict[str, Any], 
                          trends: Dict[str, Any], risks: List[str]) -> str:
        """推論理由の生成"""
        reasoning_parts = []
        ...
        for metric_name, value in metrics.items():
            if metric_name in self.decision_thresholds.get(decision_type, {}):
                ...
                if metric_name in ["defect_rate", "rework_rate"]:
                    if value > threshold:
                        reasoning_parts.append(f"{metric_name}が{value:.1f}%でしきい値{threshold:.1f}%を超過")
                else:
                    if value < threshold:
                        reasoning_parts.append(f"{metric_name}が{value:.1f}%でしきい値{threshold:.1f}%を下回る")
    ...
    def _generate_alternatives(self, decision_type: DecisionType, metrics: Dict[str, Any]) -> List[str]:
        """代替案の生成"""
        alternatives_map = {
            DecisionType.RESOURCE_ALLOCATION: [
                "外部リソースの活用",
    ...
    def _analyze_decision_risks(self, decision_type: DecisionType, metrics: Dict[str, Any]) -> List[str]:
        """意思決定のリスク分析"""
        risks_map = {
            DecisionType.RESOURCE_ALLOCATION: [
                "リソース変更による一時的な生産性低下",
    ...
    def _analyze_decision_benefits(self, decision_type: DecisionType, metrics: Dict[str, Any]) -> List[str]:
        """意思決定の利益分析"""
        benefits_map = {
            DecisionType.RESOURCE_ALLOCATION: [
                "リソース効率性の向上",
    ...
    def _estimate_decision_impact(self, decision_type: DecisionType, 
                                metrics: Dict[str, Any], trends: Dict[str, Any]) -> float:
        """意思決定の影響度推定"""
        # 基本影響度
        base_impact = {
    ...
    def _determine_urgency(self, decision_type: DecisionType, 
                         metrics: Dict[str, Any], trends: Dict[str, Any]) -> DecisionUrgency:
        """緊急度の判定"""
        overall_score = self._calculate_overall_score_from_dict(metrics)
    ...
    def _calculate_confidence(self, decision_type: DecisionType, 
                            metrics: Dict[str, Any], trends: Dict[str, Any]) -> ConfidenceLevel:
        """信頼度の計算"""
        # データの完全性チェック
        ...
        data_completeness = len(available_metrics) / len(required_metrics) if required_metrics else 1.0
    ...
    def _calculate_improvements(self, decision_type: DecisionType, 
                             metrics: Dict[str, Any], trends: Dict[str, Any]) -> Dict[str, float]:
        """改善予測値の計算"""
        improvements = {}
        ...
        if decision_type == DecisionType.RESOURCE_ALLOCATION:
            ...
            improvements["timeline_improvement"] = max(10, 30 - (current_efficiency * 50))
            improvements["completion_improvement"] = max(5, 20 - (current_efficiency * 40))
        ...
        # デフォルト値を設定
        for key in ["efficiency_improvement", "timeline_improvement", "completion_improvement",
                   "quality_improvement", "defect_reduction", "rework_reduction", "risk_reduction"]:
    ...
    def _analyze_trends(self, project_id: str, historical_metrics: List[ProjectMetrics]) -> Dict[str, Any]:
        """トレンド分析"""
        trends = {}
        ...
        # 各メトリクスのトレンドを計算
        metrics_names = [
            "completion_rate", "quality_score", "timeline_adherence",
            "resource_efficiency", "team_satisfaction", "defect_rate", "rework_rate"
        ]
        ...
        for metric_name in metrics_names:
            ...
            if len(values) >= 2:
                ...
                significance = min(1.0, change_magnitude / values[0] if values[0] != 0 else 0)
                ...
                trends[metric_name] = {
                    ...
                    "change_rate": (values[0] - values[-1]) / values[-1] if values[-1] != 0 else 0
                }
        
        return trends
    
    def _identify_risks(self, metrics: Optional[ProjectMetrics], trends: Dict[str, Any]) -> List[str]:
        """リスクの特定"""
        risks = []
    ...
    def _identify_opportunities(self, metrics: Optional[ProjectMetrics], trends: Dict[str, Any]) -> List[str]:
        """機会の特定"""
        opportunities = []
    ...
    def record_decision_outcome(self, decision_id: str, actual_impact: float, 
                              success_rating: int, lessons_learned: str = ""):
        """意思決定の結果を記録"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                # project_idを取得
...
if __name__ == "__main__":
    ...
    for decision_type, stat in stats.items():
        print(f"    {decision_type}: {stat['count']}件 (平均評価: {stat['avg_rating']:.1f}/5)")
