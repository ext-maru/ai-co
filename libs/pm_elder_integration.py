...
try:
    from libs.elder_council_summoner import (
        ElderCouncilSummoner, CouncilTrigger, UrgencyLevel, 
        TriggerCategory, SystemEvolutionMetrics
    )
...
class PMElderIntegration:
    ...
    def assess_project_complexity(self, project_data: Dict[str, Any]) -> ProjectComplexity:
        """プロジェクト複雑度評価"""
        try:
            ...
            # Critical indicators
            critical_matches = sum(1 for indicator in self.complexity_thresholds['critical_indicators']
                                 if any(word in prompt for word in indicator.split()))
            complexity_score += critical_matches * 10
            
            # Complex indicators  
            complex_matches = sum(1 for indicator in self.complexity_thresholds['complex_indicators']
                                if any(word in prompt for word in indicator.split()))
            complexity_score += complex_matches * 5
            
            # Moderate indicators
            moderate_matches = sum(1 for indicator in self.complexity_thresholds['moderate_indicators']
                                 if any(word in prompt for word in indicator.split()))
    ...
    def request_project_approval(self, project_data: Dict[str, Any]) -> Tuple[bool, str]:
        """プロジェクト承認要求"""
        try:
            project_id = project_data.get('project_id') or str(uuid.uuid4())
            complexity = self.assess_project_complexity(project_data)
            
            logger.info(f"Project {project_id} complexity assessed as: {complexity.value}")
            ...
            if complexity == ProjectComplexity.SIMPLE:
                ...
                approval_request = self._create_approval_request(project_id, project_data, complexity)
    ...
    def _create_approval_request(self, project_id: str, project_data: Dict[str, Any], 
                               complexity: ProjectComplexity) -> ProjectElderApproval:
        """承認要求作成"""
...
**推定リスク**: {self._assess_project_risk(project_data)}
    ...
    def _summon_elder_council_for_project(self, project_id: str, project_data: Dict[str, Any]):
        ...
        try:
            ...
            # 4賢者からの意見収集
            trigger.four_sages_input = self._collect_four_sages_input_for_project(project_data)
    ...
    def _collect_four_sages_input_for_project(self, project_data: Dict[str, Any]) -> Dict[str, Any]:
        """プロジェクト用4賢者意見収集"""
        return {
            'knowledge_sage': {
    ...
    def _notify_elders_project_start(self, project_id: str, project_data: Dict[str, Any], 
                                   complexity: ProjectComplexity):
        """Elderへのプロジェクト開始通知"""
        try:
            message = f"""
...
いつでもお申し付けください。
    ...
    def _send_approval_request_notification(self, approval: ProjectElderApproval, 
...
- Council召集: `/elder council {approval.approval_id}`
    ...
    def escalate_quality_issue(self, project_id: str, quality_issues: List[str], 
...
戦略的指導を要請いたします。
    ...
    def _summon_elder_council_for_quality_issue(self, consultation: ElderConsultationRequest):
        ...
        try:
            ...
            logger.info(f"Elder Council summoned for quality issue {consultation.request_id}")
    ...
    def report_project_completion(self, project_id: str, project_result: Dict[str, Any]):
        """プロジェクト完了のElder報告"""
        try:
            complexity = self.project_approvals.get(project_id, {}).get('complexity', ProjectComplexity.SIMPLE)
...
正常に完了いたしました。ご指導ありがとうございました。
    ...
    def get_pending_approvals(self) -> List[ProjectElderApproval]:
        """保留中の承認要求一覧"""
        return [approval for approval in self.project_approvals.values() 
        ...
        try:
            for approval in self.project_approvals.values():
                if approval.approval_id == approval_id:
                    ...
                    logger.info(f"Project {approval.project_id} rejected by Elders: {reason}")
                    return True
