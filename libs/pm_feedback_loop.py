...
class PMFeedbackLoop(BaseManager):
    ...
    def process_task_result(self, task_id: str, task_data: Dict[str, Any]) -> Dict[str, Any]:
        """タスク結果を処理してPM評価を実行"""
        try:
            logger.info(f"🎯 PMフィードバック処理開始: {task_id}")
            
            # 品質評価実行
            evaluation_result = self.quality_evaluator.evaluate_task_quality(task_id, task_data)
    ...
    def _handle_retry_required_task(self, task_id: str, task_data: Dict[str, Any], evaluation_result: Dict[str, Any]):
        """再試行が必要なタスクの処理"""
        try:
            attempt_count = task_data.get('attempt_count', 1)
            ...
            retry_task_data['pm_feedback'] = evaluation_result.get('feedback_message', '')
            ...
            # 再試行通知
            if self.slack:
                self._send_retry_notification(task_id, evaluation_result, improvement_suggestions)
    ...
    def _generate_improvement_suggestions(self, evaluation_result: Dict[str, Any]) -> List[str]:
        """改善提案を生成"""
        suggestions = []
    ...
    def _send_approval_notification(self, task_id: str, evaluation_result: Dict[str, Any]):
        """承認通知"""
        try:
            if not self.slack:
                return
    ...
    def _send_retry_notification(self, task_id: str, evaluation_result: Dict[str, Any], suggestions: List[str]):
        """再試行通知"""
        try:
            if not self.slack:
                return
    ...
    def _send_rejection_notification(self, task_id: str, evaluation_result: Dict[str, Any]):
        """却下通知"""
        try:
            if not self.slack:
                return
