...
class PMQualityEvaluator(BaseManager):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # インデックス作成
            conn.execute('CREATE INDEX IF NOT EXISTS idx_task_id ON quality_evaluations(task_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_pm_approved ON quality_evaluations(pm_approved)')
    
    def evaluate_task_quality(self, task_id: str, task_data: Dict[str, Any]) -> Dict[str, Any]:
        """タスクの品質を総合評価"""
        try:
            ...
            # 1. テスト成功率評価
            evaluation_results['test_success_rate'] = self._evaluate_test_success(task_data)
            
            # 2. コード品質評価
            evaluation_results['code_quality_score'] = self._evaluate_code_quality(task_data)
            
            # 3. 要件適合度評価
            evaluation_results['requirement_compliance'] = self._evaluate_requirement_compliance(task_data)
            
            # 4. エラー率評価
            evaluation_results['error_rate'] = self._evaluate_error_rate(task_data)
            
            # 5. パフォーマンス評価
            evaluation_results['performance_score'] = self._evaluate_performance(task_data)
            ...
            logger.info(f"✅ 品質評価完了: {task_id} - スコア: {overall_score:.1f}% - {'承認' if pm_approved else '要改善'}")
        ...
        try:
            ...
            for py_file in python_files:
                ...
                if file_path.parts[0] in ['workers', 'libs']:
                    test_file = PROJECT_ROOT / 'tests' / 'unit' / f"test_{file_path.name}"
    ...
    def _should_retry(self, task_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT COUNT(*) FROM quality_evaluations WHERE task_id = ?",
                (task_id,)
            )
    ...
    def _record_evaluation(self, task_id: str, evaluation_results: Dict[str, Any], task_data: Dict[str, Any]):
        """評価結果をデータベースに記録"""
        with sqlite3.connect(self.db_path) as conn:
            # 試行回数取得
            cursor = conn.execute(
