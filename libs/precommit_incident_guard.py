...
class PreCommitIncidentGuard:
    ...
    WARNING_PATTERNS = {
        ...
        "deprecated_usage": [r"\.iteritems\(", r"urllib2\.", r"md5\.", r"cPickle\."],
        "todo_markers": [r"#\s*TODO", r"#\s*FIXME", r"#\s*HACK", r"#\s*XXX"],
    }
    ...
    def __init__(self):
        ...
        self.stats = {"commits_checked": 0, "commits_blocked": 0, "threats_detected": 0}
    ...
    def get_file_changes(self, file_path: str) -> Optional[CodeChange]:
        ...
        try:
            ...
            change.security_issues = self._analyze_security_issues(
    ...
    def assess_commit_risk(self, changes: List[CodeChange]) -> CommitRiskAssessment:
        ...
        critical_files = sum(
            1 for change in changes if self._is_critical_file(change.file_path)
        )
        all_security_issues = []
        all_risk_indicators = []
        ...
        if all_risk_indicators:
            warnings.extend(
                [f"⚠️ リスク指標: {indicator}" for indicator in set(all_risk_indicators)]
            )
    ...
    def format_assessment_report(
        self, assessment: CommitRiskAssessment, changes: List[CodeChange]
    ) -> str:
        """評価レポートをフォーマット"""
        report = []
        report.append("🏰" + "=" * 60)
