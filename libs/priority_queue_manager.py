    ...
    def mark_task_rate_limited(self, task_id: str):
        ...
        if task_id in self.processing_tasks:
            ...
            if task.can_retry():
                ...
                # リトライ遅延
                delay_seconds = (2 ** task.retry_count) * self.config['rate_limit_handling']['retry_delay_multiplier']
                ...
                self.logger.info(f"レート制限によりリトライキュー追加: {task_id} (試行 {task.retry_count}/{task.max_retries})")
            else:
                # リトライ上限に達した場合
                self._handle_task_failure(task)
