...
class ProjectDesignManager(BaseManager):
    ...
    def add_requirement(self, project_id: str, type: str, 
    ...
    def create_design(self, project_id: str, design_type: str, 
    ...
    def create_development_task(self, project_id: str, design_id: Optional[str],
                               name: str, description: str, 
                               assigned_worker: str = 'task_worker') -> str:
        """開発タスク作成"""
    ...
    def update_task_status(self, dev_task_id: str, status: str, 
    ...
    def record_test_result(self, project_id: str, dev_task_id: Optional[str],
                          test_type: str, status: str, details: Dict[str, Any]):
        """テスト結果記録"""
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (test_id, project_id, dev_task_id, test_type, status, json.dumps(details)))
            conn.commit()
    ...
    def update_phase_status(self, project_id: str, phase: str, status: str, notes: str = ""):
        """フェーズステータス更新"""
        with sqlite3.connect(self.db_path) as conn:
    ...
    def add_project_file(self, project_id: str, file_path: str, 
                        file_type: str, phase: str):
        """プロジェクトファイル登録"""
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
