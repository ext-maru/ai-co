...
class ProjectTemplate:
    ...
    def add_phase(self, phase_name: str, tasks: List[str], estimated_days: int = 7) -> 'ProjectTemplate':
        """フェーズを追加"""
        self.phases.append({
            'name': phase_name,
            'tasks': tasks,
    ...
    def add_checklist(self, phase_name: str, checklist_items: List[str]) -> 'ProjectTemplate':
        """チェックリストを追加"""
        self.checklists[phase_name] = checklist_items
        return self

    def add_elder_consultation(self, phase_name: str, sage_type: str, consultation_prompt: str) -> 'ProjectTemplate':
        """エルダー相談を追加"""
    ...
    def add_status_trigger(self, trigger_condition: str, action: str) -> 'ProjectTemplate':
        """状態トリガーを追加"""
...
class ProjectStatusManager:
    ...
    def create_project_status(self, project_id: str, project_name: str,
                            template_name: str = None, context_data: Dict = None) -> bool:
        """プロジェクト状態を作成"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        try:
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (project_id, project_name, template_name,
    ...
    def update_project_phase(self, project_id: str, phase_name: str,
                           phase_index: int, context_data: Dict = None) -> bool:
        """プロジェクトフェーズを更新"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        cursor.execute("""
            ...
            WHERE project_id = ?
        """, (phase_name, phase_index,
              json.dumps(context_data) if context_data else None, project_id))
    ...
    def log_continuity(self, project_id: str, session_id: str, action: str,
                      context_snapshot: Dict, previous_actions: List[str] = None,
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (project_id, session_id, action, json.dumps(context_snapshot),
...
class ProjectTemplateSystem:
    ...
    def _init_default_templates(self):
        ...
        web_template = ProjectTemplate(
            ...
            "Phase 4: 最適化・デプロイ",
            ["パフォーマンス最適化", "セキュリティ強化", "デプロイ準備", "本番デプロイ"],
            10
        ).add_checklist(
            "Phase 1: 要件定義・設計",
            ["要件書レビュー完了", "技術選定理由書作成", "アーキテクチャ図作成", "データベース設計書作成"]
        ).add_checklist(
            "Phase 2: 基盤実装",
            ["認証システム実装・テスト", "データベース作成・マイグレーション", "API基盤実装", "フロントエンド基盤実装"]
        ...
        ai_template = ProjectTemplate(
            ...
            "Phase 2: モデル開発",
            ["ベースライン実装", "モデル選定", "特徴量エンジニアリング", "モデル学習"],
            14
        ).add_phase(
            "Phase 3: 評価・改善",
            ["モデル評価", "ハイパーパラメータ調整", "モデル改善", "バリデーション"],
            10
        ).add_phase(
            "Phase 4: 統合・デプロイ",
        ...
        # 緊急修正プロジェクトテンプレート
        hotfix_template = ProjectTemplate(
            "hotfix",
            "緊急修正プロジェクト用テンプレート"
        ).add_phase(
            "Phase 1: 緊急調査",
    ...
    def create_project_from_template(self, project_name: str, template_name: str,
        ...
        self.status_manager.log_continuity(
            project_id, self.current_session_id, "project_created",
            {
    ...
    def _get_current_phase_tasks(self, status: Dict, template: ProjectTemplate) -> List[str]:
        """現在のフェーズのタスクを取得"""
    ...
    def _get_current_checklist(self, status: Dict, template: ProjectTemplate) -> List[str]:
        """現在のフェーズのチェックリストを取得"""
    ...
    def _get_elder_consultations(self, status: Dict, template: ProjectTemplate) -> List[Dict]:
        """現在のフェーズのエルダー相談を取得"""
        if not template or not status.get('current_phase'):
            return []
    ...
    def advance_phase(self, project_id: str, context_data: Dict = None) -> bool:
        ...
        # 継続性ログを記録
        self.status_manager.log_continuity(
            project_id, self.current_session_id, "phase_advanced",
            {
    ...
    def _complete_project(self, project_id: str, context_data: Dict = None) -> bool:
        ...
        # 継続性ログを記録
        self.status_manager.log_continuity(
            project_id, self.current_session_id, "project_completed",
...
class ProjectTemplateCLI:
    ...
    def create_project(self, project_name: str, template_name: str) -> str:
        """プロジェクト作成"""
        try:
            project_id = self.system.create_project_from_template(project_name, template_name)
    ...
    def list_projects(self):
        ...
        print("📋 プロジェクト一覧")
        print("=" * 80)
        print(f"{'ID':<20} {'名前':<30} {'テンプレート':<15} {'フェーズ':<20} {'状態':<10}")
        print("-" * 80)

        for project in projects:
            print(f"{project['project_id']:<20} {project['project_name']:<30} "
...
def main():
    ...
    parser = argparse.ArgumentParser(description='🏛️ エルダーズギルド プロジェクトテンプレートシステム')
