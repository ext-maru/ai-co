...
class ProjectMetadata:
    ...
    # 関連プロジェクト
    similar_projects: List[Tuple[str, float]] = field(default_factory=list)  # (project_id, similarity)
...
class ProjectAnalyzer:
    ...
    async def _analyze_dependencies(self, project_path: Path) -> List[ProjectDependency]:
        """依存関係分析"""
        dependencies = []
    ...
    async def _analyze_git_metrics(self, project_path: Path) -> Optional[GitMetrics]:
        ...
        try:
            ...
            for commit in commits:
                if '|' in commit:
                    ...
                    if len(parts) >= 3:
                        ...
                        try:
                            commit_date = datetime.fromisoformat(parts[2].replace(' +0900', ''))
            ...
            active_branches = len(branch_result.stdout.strip().split('\n')) if branch_result.returncode == 0 else 1
    ...
    async def _create_feature_vector(
        ...
        tech_vector = [1 if tech in basic_info["tech_stack"] else 0 for tech in all_techs]
    ...
    def _detect_tech_stack(self, project_path: Path) -> List[TechStack]:
        ...
        # ファイル存在チェック
        if (project_path / "requirements.txt").exists() or any(project_path.rglob("*.py")):
    ...
    def _calculate_complexity_score(self, lines: int, classes: int, functions: int) -> float:
        """複雑度スコア計算"""
        if lines == 0:
            return 0.0
...
class DocumentationGenerator:
    ...
    async def generate_documentation(
        self, 
        metadata: ProjectMetadata,
        similar_projects: List[ProjectMetadata] = None
    ...
    async def _generate_setup_guide(self, metadata: ProjectMetadata) -> str:
        ...
        if TechStack.PYTHON in metadata.tech_stack:
            guide += "- Python 3.8以降\n"
        if TechStack.JAVASCRIPT in metadata.tech_stack or TechStack.NEXTJS in metadata.tech_stack:
            guide += "- Node.js 16以降\n"
    ...
    async def _generate_usage_examples(self, metadata: ProjectMetadata) -> str:
        ...
        if metadata.project_type == ProjectType.LIBRARY:
            ...
            examples += "# APIリクエスト例\n"
            examples += "response = requests.get('http://localhost:8000/api/endpoint')\n"
...
class ProjectWebPortal:
    ...
    async def scan_projects(self, root_path: Path = None) -> List[ProjectMetadata]:
        ...
        for item in root_path.iterdir():
            if item.is_dir() and item.name not in ["venv", "__pycache__", ".git", "node_modules"]:
    ...
    async def generate_project_documentation(self, project_id: str) -> Optional[ProjectDocumentation]:
        """プロジェクト資料生成"""
        # プロジェクトメタデータ取得
    ...
    async def find_similar_projects(self, project_id: str, limit: int = 5) -> List[Dict[str, Any]]:
        """類似プロジェクト検索"""
        metadata = await self._get_project_metadata(project_id)
