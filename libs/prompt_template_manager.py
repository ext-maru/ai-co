...
from core import BaseManager, get_config, EMOJI
...
class PromptTemplateManager(BaseManager):
    ...
    def __init__(self):
        ...
        self.template_dir = Path(self.config.get('prompt.template_dir', 'config/prompts'))
...
Please proceed with the task without asking for permissions.''',
                    'variables': ['task_type', 'task_id', 'user_prompt', 'rag_context', 'additional_instructions'],
...
{{ additional_instructions }}''',
                    'variables': ['task_id', 'file_list', 'project_structure', 'additional_instructions'],
...
{{ additional_instructions }}''',
                    'variables': ['conversation_id', 'turn_number', 'conversation_history', 'user_input', 'context', 'additional_instructions'],
...
{{ additional_instructions }}''',
                    'variables': ['channel', 'user', 'message', 'channel_context', 'additional_instructions'],
    ...
    def create_template(self, worker_type: str, template_name: str, 
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                ''', (worker_type, template_name)).fetchone()
                ...
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (
                    worker_type, template_name, new_version, template_content,
    ...
    def get_template(self, worker_type: str, template_name: str = 'default',
    ...
    def generate_prompt(self, worker_type: str, template_name: str = 'default',
        ...
        try:
            ...
            self._record_prompt_history(
                worker_type, template_name, generated_prompt,
                context_vars, context_vars.get('task_id')
            )
    ...
    def _record_prompt_history(self, worker_type: str, template_name: str,
                              generated_prompt: str, variables: Dict[str, Any],
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                ''', (
                    worker_type, template_name, generated_prompt,
                    json.dumps(variables), task_id
                ))
        except Exception as e:
            self.logger.warning(f"Failed to record prompt history: {e}")
    
    def update_template(self, worker_type: str, template_name: str,
                       new_content: str, variables: List[str] = None,
                       description: str = None) -> bool:
        """テンプレートを更新（新しいバージョンを作成）"""
        return self.create_template(
            worker_type, template_name, new_content,
            variables, description
        )
    
    def rollback_template(self, worker_type: str, template_name: str,
    ...
    def get_template_history(self, worker_type: str, template_name: str,
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in results:
                    ...
                    if history_item.get('variables_used'):
                        history_item['variables_used'] = json.loads(history_item['variables_used'])
    ...
    if manager.initialize():
        ...
        for template in templates:
            if template['is_active']:
                print(f"- {template['worker_type']}/{template['template_name']} "
