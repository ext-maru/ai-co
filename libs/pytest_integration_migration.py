...
class TestContainerManager:
    ...
    def __init__(self):
        if not TESTCONTAINERS_AVAILABLE:
            raise ImportError("testcontainers is not available. Install with: pip install testcontainers")
    ...
    def create_postgres_container(self, 
                                 ...
                                 password: str = "test_pass") -> PostgresContainer:
        """PostgreSQLコンテナ作成"""
...
class PytestAPITester:
    ...
    async def _execute_api_step(self, session: aiohttp.ClientSession, step: Dict) -> aiohttp.ClientResponse:
        """個別APIステップ実行"""
    ...
    def _run_assertions(self, response: aiohttp.ClientResponse, assertions: List[Dict]):
        ...
        for assertion in assertions:
            if assertion['type'] == 'status_code':
                assert response.status == assertion['expected'], \
                ...
                assert assertion['expected'] in response.body, \
...
class TestElderSystemIntegration:
    ...
    @pytest.mark.asyncio
    async def test_elder_data_workflow(self, test_database, test_redis, test_data_factory):
        """エルダーデータワークフロー統合テスト"""
        # テストデータ準備
