...
class PytestIntegrationRunner:
    ...
    async def start_service(self, service_name: str, config: Dict[str, Any]) -> ServiceInfo:
    ...
    async def health_check(self, service: ServiceInfo, timeout: int = 30) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            while time.time() - start_time < timeout:
                try:
                    async with session.get(f"{service.url}{service.health_endpoint}") as resp:
                        if resp.status == 200:
                            return True
                except Exception:
                    pass
...
def assert_service_healthy(service: ServiceInfo):
    """サービスヘルスアサーション"""
    assert service.status == ServiceStatus.RUNNING, f"Service {service.name} is not running"
