...
class RabbitMQMonitor:
    ...
    def __init__(self):
        ...
        self.critical_queues = [
            ...
            "dialog_task_queue", "user_input_queue"
        ]
    ...
    def _monitor_loop(self):
        ...
        while self.monitoring:
            try:
                ...
                # 異常検知時の処理
                if self.connection_status.consecutive_failures >= self.failure_threshold:
            ...
            for queue_name in self.critical_queues:
                try:
                    ...
                    if queue_name in self.queue_info:
                        ...
                        # メッセージ蓄積の警告
                        if queue_info.message_count > 10 and queue_info.consumer_count == 0:
                            self._trigger_alerts("queue_backlog", 
                                f"キュー{queue_name}にメッセージが蓄積中: {queue_info.message_count}件")
                        
                        # コンシューマー減少の警告
                        if queue_info.consumer_count < prev_info.consumer_count:
                            self._trigger_alerts("consumer_lost", 
    ...
    def _update_stats(self):
        """統計情報更新"""
        if (self.stats["last_downtime"] and 
            not self.connection_status.is_connected):
            # ダウンタイム計算
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            print(f"🔍 監視状況: 接続={status['connection_status']['is_connected']}, "
