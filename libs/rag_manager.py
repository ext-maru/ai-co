...
{current_prompt}"""
    ...
    def search_similar_tasks(self, query, limit=5):
        """類似タスクを検索"""
        try:
            # データベースから関連タスクを取得
            ...
            logger.error(f"類似タスク検索エラー: {e}")
            return []
    
    def _format_similar_tasks(self, tasks):
        """タスクリストをフォーマット"""
        if not tasks:
        ...
        formatted = []
        for task in tasks:
            formatted.append(f"""Task ID: {task.get('task_id', 'unknown')}
プロンプト: {task.get('prompt', 'N/A')}
応答: {task.get('response', 'N/A')[:200]}...
要約: {task.get('summary', 'N/A')}""")
