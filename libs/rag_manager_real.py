...
from core.elders_legacy import DomainBoundary, EldersAILegacy, enforce_boundary
...
class RAGManager(EldersAILegacy):
        ...
        try:
            ...
            if operation == "add_document":
                ...
                return await self._generate_answer(
                    request.get("query"), request.get("context", [])
                )
        ...
        try:
            ...
            if strategy == "keyword":
                ...
                for row in cursor.fetchall():
                    ...
                    results.append(
                        RetrievalResult(doc, similarity, "content_similarity")
                    )

            else:  # hybrid
                # ハイブリッド検索
                keyword_results = await self._search_documents(
                    query, "keyword", limit // 2
                )
                similarity_results = await self._search_documents(
                    query, "similarity", limit // 2
                )
            ...
            return {"status": "error", "message": str(e), "query": query, "results": []}
    ...
    async def _generate_answer(self, query: str, context: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if context:
                answer = (
                    f"Based on the available information: {' '.join(context[:500])}"
                )
                confidence = 0.8
            else:
...
async def create_rag_manager(
    db_path: str = "data/rag_knowledge.db", config: Optional[Dict[str, Any]] = None
) -> RAGManager:
    """
    RAG管理システム作成
...
if __name__ == "__main__":
    ...
    async def test_rag_manager():
        ...
        # 統計取得テスト
        result = await rag.process_request({"operation": "get_stats"})
        print("Stats:", result)
