...
class ReportAnalyzer:
                ...
                if isinstance(phase_reqs, dict) and 'features' in phase_reqs:
                    ...
                    for feature in features:
                        ...
                        for deliverable in deliverables:
                            if isinstance(deliverable, str) and feature.lower() in deliverable.lower():
                                feature_met = True
                                break
                    ...
                    completeness['requirements_met'][phase] = {
                        'total': len(features),
                        'met': met_count,
                        'percentage': (met_count / len(features) * 100) if features else 0
                    }
        
        # 全体の完了率
        if completeness['requirements_met']:
            total_reqs = sum(r['total'] for r in completeness['requirements_met'].values())
            total_met = sum(r['met'] for r in completeness['requirements_met'].values())
            completeness['completion_percentage'] = (total_met / total_reqs * 100) if total_reqs else 0
    ...
    def _extract_success_indicators(self, report: Dict[str, Any]) -> List[str]:
        ...
        # ポジティブなキーワード
        positive_keywords = ['成功', '完了', '実装', '改善', '向上', 'success', 'completed', 'improved']
        summary = report.get('summary', '')
    ...
    def _assess_impact(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for deliverable in deliverables:
            if isinstance(deliverable, str):
                # ファイルパスやコンポーネント名を抽出
                if '/' in deliverable:
                    component = deliverable.split('/')[-2] if len(deliverable.split('/')) > 1 else 'system'
                    if component not in impact['affected_components']:
                        impact['affected_components'].append(component)
                
                # 利益を特定
                benefit_keywords = ['改善', '向上', '追加', '実装', 'improved', 'added', 'fixed']
    ...
    def compare_reports(self, report1: Dict[str, Any], 
        ...
        if perf1['execution_time'] and perf2['execution_time']:
            comparison['performance_comparison']['execution_time'] = {
                ...
                'faster_by': abs(perf1['execution_time']['total_hours'] - 
