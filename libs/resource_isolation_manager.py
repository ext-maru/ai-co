...
class ResourceIsolationManager:
    ...
    def __init__(self):
        ...
        # 監視スレッド開始
        self.monitoring_thread = threading.Thread(target=self._monitor_resources_loop, daemon=True)
    ...
    def create_isolation_context(self, security_level: SecurityLevel,
                               custom_quota: Optional[ResourceQuota] = None,
                               custom_config: Optional[Dict] = None,
                               session_duration_hours: int = 8) -> IsolationContext:
        """隔離コンテキスト作成"""
        ...
        self.logger.info(f"🏗️ Isolation context created: {context_id} ({security_level.value})")
        return context
    
    def _prepare_filesystem_mounts(self, security_level: SecurityLevel, context_id: str) -> List[Dict[str, str]]:
        """ファイルシステムマウント準備"""
        ...
        if security_level == SecurityLevel.SANDBOX:
            # 読み取り専用の最小システム
            mounts.extend([
                {"source": "/usr", "target": "/usr", "type": "bind", "options": "ro"},
                {"source": "/lib", "target": "/lib", "type": "bind", "options": "ro"},
                {"source": "/bin", "target": "/bin", "type": "bind", "options": "ro"}
            ])
        elif security_level == SecurityLevel.RESTRICTED:
            # 制限付きシステムアクセス
            mounts.extend([
                {"source": "/tmp", "target": "/tmp", "type": "tmpfs", "options": "size=100m"},
                {"source": "/var/tmp", "target": "/var/tmp", "type": "tmpfs", "options": "size=50m"}
            ])
        
        return mounts
    
    def _prepare_network_config(self, security_level: SecurityLevel, context_id: str) -> Dict[str, Any]:
        """ネットワーク設定準備"""
    ...
    def _apply_resource_limits(self, context: IsolationContext):
        ...
        try:
            ...
            self.logger.info(f"✅ Resource limits applied for context: {context.context_id}")
            
        except Exception as e:
            self.logger.error(f"❌ Failed to apply resource limits for {context.context_id}: {e}")
            raise
    ...
    def _apply_cpu_limits(self, context: IsolationContext):
        ...
        if context.isolation_type in [IsolationType.NAMESPACE, IsolationType.CONTAINER]:
            ...
            try:
                ...
                self.logger.debug(f"📊 CPU limit set: {context.resource_quota.cpu_percent*100}% for {context.context_id}")
                
            except Exception as e:
                self.logger.warning(f"⚠️ Failed to set CPU cgroup for {context.context_id}: {e}")
    ...
    def _apply_memory_limits(self, context: IsolationContext):
        ...
        if context.isolation_type in [IsolationType.NAMESPACE, IsolationType.CONTAINER]:
            try:
                ...
                self.logger.debug(f"💾 Memory limit set: {context.resource_quota.memory_mb}MB for {context.context_id}")
                
            except Exception as e:
                self.logger.warning(f"⚠️ Failed to set memory limit for {context.context_id}: {e}")
    ...
    def _apply_process_limits(self, context: IsolationContext):
        ...
        try:
            ...
            self.logger.warning(f"⚠️ Failed to set process limits for {context.context_id}: {e}")
    ...
    def _apply_network_limits(self, context: IsolationContext):
        ...
        if context.network_config.get("bandwidth_limit"):
            try:
                ...
                self.logger.warning(f"⚠️ Failed to set network limits for {context.context_id}: {e}")
    ...
    def check_resource_violations(self, context_id: str) -> List[ResourceAlert]:
        ...
        if usage.cpu_percent > quota.cpu_percent:
            alerts.append(ResourceAlert(
                ...
                alert_level="critical" if usage.cpu_percent > quota.cpu_percent * 1.5 else "warning",
        ...
        if usage.memory_mb > quota.memory_mb:
            alerts.append(ResourceAlert(
                ...
                alert_level="critical" if usage.memory_mb > quota.memory_mb * 1.2 else "warning",
    ...
    def terminate_context(self, context_id: str, reason: str = "Manual termination"):
        ...
        with self.context_lock:
            if context_id not in self.active_contexts:
                self.logger.warning(f"⚠️ Context not found for termination: {context_id}")
                return
    ...
    def _cleanup_resource_limits(self, context: IsolationContext):
        ...
        try:
            # cgroup削除
            cgroup_name = f"ai_company_{context.context_id}"
            subprocess.run(f"cgdelete -g cpu,memory:{cgroup_name}", shell=True, check=False)
            
            self.logger.debug(f"🧹 Resource limits cleaned up for {context.context_id}")
            
        except Exception as e:
            self.logger.warning(f"⚠️ Failed to cleanup resource limits for {context.context_id}: {e}")
    ...
    def _cleanup_filesystem(self, context: IsolationContext):
        ...
        try:
            # コンテキスト専用ディレクトリ削除
            context_workspace = Path(f"/home/aicompany/workspace/{context.security_level.value}/{context.context_id}")
            ...
            self.logger.warning(f"⚠️ Failed to cleanup filesystem for {context.context_id}: {e}")
    ...
    def _cleanup_network(self, context: IsolationContext):
        ...
        try:
            ...
            self.logger.debug(f"🌐 Network configuration cleaned up for {context.context_id}")
            
        except Exception as e:
            self.logger.warning(f"⚠️ Failed to cleanup network for {context.context_id}: {e}")
        ...
        while self.monitoring_active:
            try:
                ...
                for context_id in current_contexts:
                    ...
                    with self.context_lock:
                        context = self.active_contexts.get(context_id)
                        if context and context.expires_at and datetime.now() > context.expires_at:
...
class ResourceMonitor:
    ...
    def get_system_load(self) -> Dict[str, float]:
        ...
        return {
            ...
            "disk_io_percent": psutil.disk_io_counters().read_bytes + psutil.disk_io_counters().write_bytes,
            "network_io_percent": psutil.net_io_counters().bytes_sent + psutil.net_io_counters().bytes_recv
        }
...
class AlertManager:
    ...
    def handle_alert(self, alert: ResourceAlert):
        ...
        self.logger.warning(f"{emoji} Resource Alert [{alert.alert_level.upper()}]: {alert.message}")
