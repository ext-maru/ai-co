...
class RetryOrchestrator(BaseManager):
    ...
    def should_retry(self, task_info: Dict, error_info: Dict, fix_result: Dict) -> Dict:
        ...
        # リトライ履歴を確認
        retry_info = self.retry_history.get(task_id, {
            'count': 0,
            'last_attempt': None,
    ...
    def orchestrate_retry(self, task_info: Dict, error_info: Dict, fix_result: Dict) -> Dict:
        """リトライを実行"""
        result = {
    ...
    def _apply_category_strategy(self, category: str, task_info: Dict,
                                error_info: Dict, fix_result: Dict,
                                retry_info: Dict) -> Dict:
        """カテゴリ別の戦略を適用"""
    ...
    def _rebuild_task(self, original_task: Dict, error_info: Dict, fix_result: Dict) -> Dict:
        """リトライ用にタスクを再構築"""
    ...
    def _check_task_result(self, task_id: str, queue_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            # ここでは簡易的にファイルベースで実装
            queue_result_file = PROJECT_ROOT / 'data' / 'queue_results' / queue_name / f'{task_id}.json'
            if queue_result_file.exists():
                with open(queue_result_file, 'r') as f:
                    return json.load(f)

            # ワーカーステータスファイルから確認
            worker_status_file = PROJECT_ROOT / 'data' / 'worker_status' / f'{task_id}_status.json'
...
"""
    ...
    def get_retry_statistics(self) -> Dict:
        ...
        for task_id, info in self.retry_history.items():
            status = info['status'].value if isinstance(info['status'], RetryStatus) else info['status']
        ...
        max_retries_exceeded = status_counts.get(RetryStatus.MAX_RETRIES_EXCEEDED.value, 0)
    ...
    def cleanup_old_entries(self, hours: int = 24):
        ...
        for task_id, info in self.retry_history.items():
            if info['last_attempt'] and info['last_attempt'] < cutoff_time:
                if info['status'] in [RetryStatus.SUCCESS, RetryStatus.FAILED,
