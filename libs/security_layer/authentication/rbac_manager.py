...
class ElderZanRBACManager:
    ...
    def _assign_default_permissions_sync(self):
        ...
        if "elder_council" in self.roles:
            self.roles["elder_council"].permissions = [
                self.permissions["admin_system"]
            ]
    ...
    async def check_permission(self, 
        ...
        try:
            ...
            admin_permission = next(
                (p for p in role.permissions if p.name == "admin_system"), 
                None
            )
            if admin_permission and admin_permission.matches(action, resource):
                self.stats['permission_grants'] += 1
                logger.debug(f"Admin permission granted: {user_id} -> {action}:{resource}")
                return True
            ...
            for permission in role.permissions:
                if permission.matches(action, resource):
                    # 制限チェック
                    if await self._check_restrictions(user_role, action, resource, context):
                        self.stats['permission_grants'] += 1
                        logger.debug(f"Permission granted: {user_id} -> {action}:{resource}")
                        return True
        ...
        try:
            ...
            for restriction in restrictions:
                if restriction == "read_only" and action not in ["read"]:
                    ...
                    # 監査ログ記録
                    logger.info(f"Audit required: {user_role.user_id} -> {action}:{resource}")
                    ...
                    # 承認必須チェック
                    if not await self._check_approval_requirement(user_role, action, resource):
                        return False
                elif restriction == "multi_approval":
                    # 複数承認チェック
                    if not await self._check_multi_approval(user_role, action, resource):
                        return False
    ...
    async def _check_approval_requirement(self, 
                                        user_role: UserRole,
                                        action: str,
                                        resource: str) -> bool:
        """承認要件チェック"""
    ...
    async def _check_multi_approval(self, 
                                  user_role: UserRole,
                                  action: str,
                                  resource: str) -> bool:
        """複数承認チェック"""
    ...
    async def create_session_permission(self, 
                                      session_id: str,
                                      user_id: str,
                                      permissions: List[str],
                                      expires_in_minutes: int = 60) -> bool:
        """
        セッション権限作成
    ...
    async def check_session_permission(self, 
                                     session_id: str,
                                     user_id: str,
                                     permission: str) -> bool:
        """
    ...
    async def cleanup_expired(self):
        ...
        try:
            ...
            # 期限切れセッション権限削除
            expired_sessions = [
                session_id for session_id, session_perm in self.session_permissions.items()
            ...
            logger.info(f"Cleanup completed: {len(expired_users)} users, {len(expired_sessions)} sessions")
