...
class AES256EncryptionEngine:
    ...
    def _generate_key(self, 
                     key_id: str,
                     key_type: KeyType,
                     algorithm: str = "AES-256-GCM") -> EncryptionKey:
        """
        暗号化鍵生成
        ...
        try:
            if encrypted_result.algorithm == EncryptionMode.AES_256_GCM.value:
                ...
                if encrypted_result.metadata:
                    aad = json.dumps(encrypted_result.metadata, sort_keys=True).encode('utf-8')
                    decryptor.authenticate_additional_data(aad)
                
                plaintext = decryptor.update(encrypted_result.ciphertext) + decryptor.finalize()
    ...
    async def _rotate_key(self, key_id: str):
        ...
        try:
            with self.key_lock:
                ...
                if old_key:
                    # 新しい鍵生成
                    new_key = self._generate_key(key_id, old_key.key_type, old_key.algorithm)
                    self.keys[key_id] = new_key
                    
                    # 古い鍵をバックアップ（実際の実装では安全な保存が必要）
                    backup_key_id = f"{key_id}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
