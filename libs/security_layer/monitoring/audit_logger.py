...
class ComplianceAuditLogger:
    ...
    async def _generate_compliance_tags(self, event: AuditEvent) -> List[ComplianceStandard]:
        """コンプライアンスタグ生成"""
        try:
        ...
        try:
            ...
            with self.buffer_lock:
                for event in self.event_buffer:
                    if self._matches_criteria(event, start_time, end_time, user_id, event_type, risk_level):
    ...
    def _matches_criteria(self, 
                         ...
                         user_id: Optional[str] = None,
                         event_type: Optional[AuditEventType] = None,
                         risk_level: Optional[RiskLevel] = None) -> bool:
        """検索条件マッチング"""
    ...
    def _generate_recommendations(self, events: List[AuditEvent]) -> List[str]:
        ...
        # 認証失敗が多い場合
        auth_failures = [e for e in events if e.event_type == AuditEventType.AUTHENTICATION and e.result == 'failed']
