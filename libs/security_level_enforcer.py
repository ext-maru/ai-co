...
class SecurityLevelEnforcer:
    ...
    def _initialize_encryption(self):
        ...
        for level in SecurityLevel:
            ...
            password = os.getenv(
                f"SECURITY_KEY_{level.value.upper()}",
                f"ai_company_default_{level.value}",
            ).encode()
    ...
    def create_security_context(
        ...
        # データアクセスレベル決定
        data_access_levels = self.data_access_matrix.get(
            security_level, [DataClassification.PUBLIC]
        )
        ...
        self.logger.info(
            f"🆔 Security context created: {session_id} for {user_id} at {security_level.value}"
        )
        return context
        ...
        try:
            ...
            if not self._validate_session(request.context):
                return self._create_denied_result(
                    request, "Invalid or expired session", 0.9
                )
            ...
            if not self._check_permission(request):
                ...
                return self._create_denied_result(
                    request, "Insufficient permissions", 0.8
                )
            ...
            if not self._check_data_access(request):
                self.security_metrics["denied_access_attempts"] += 1
                return self._create_denied_result(
                    request, "Data classification access denied", 0.8
                )
            ...
            if not self._validate_cross_level_access(request):
                self.security_metrics["denied_access_attempts"] += 1
                return self._create_denied_result(
                    request, "Cross-level access denied", 0.9
                )
    ...
    def _validate_session(self, context: SecurityContext) -> bool:
        ...
        with self.session_lock:
            ...
            if (
                stored_context.user_id != context.user_id
                or stored_context.security_level != context.security_level
            ):
                return False
    ...
    def _validate_cross_level_access(self, request: AccessRequest) -> bool:
        ...
        # アクセスマトリックスチェック
        allowed_permissions = self.access_matrix.get(
            request.context.security_level, {}
        ).get(resource_security_level, [PermissionLevel.NONE])
    ...
    def _get_access_restrictions(self, request: AccessRequest) -> List[str]:
        ...
        if request.context.security_level == SecurityLevel.SANDBOX:
            restrictions.extend(
                ["no_network_access", "read_only_filesystem", "limited_cpu"]
            )
        ...
        if request.data_classification == DataClassification.TOP_SECRET:
            restrictions.extend(
                ["dual_authorization_required", "audit_trail_mandatory"]
            )

        return restrictions

    def _create_denied_result(
        self, request: AccessRequest, reason: str, risk_score: float
    ) -> AccessResult:
        """アクセス拒否結果作成"""
    ...
    def _record_potential_violation(self, request: AccessRequest, risk_score: float):
        ...
        self.logger.warning(
            f"⚠️ Potential security violation recorded: {violation.violation_id}"
        )
    ...
    def revoke_session(self, session_id: str, reason: str = "Manual revocation"):
        ...
        with self.session_lock:
            if session_id in self.active_sessions:
                ...
                self.logger.info(
                    f"🚫 Session revoked: {session_id} for {context.user_id} - {reason}"
                )
...
class AnomalyDetector:
    ...
    def detect_anomaly(self, request: AccessRequest) -> float:
        ...
        # 権限エスカレーション検知
        if (
            request.required_permission == PermissionLevel.ADMIN
            and request.context.security_level != SecurityLevel.TRUSTED
        ):
            anomaly_score += 0.5
        ...
        if request.data_classification in [
            DataClassification.RESTRICTED,
            DataClassification.TOP_SECRET,
        ] and request.context.security_level in [
            SecurityLevel.SANDBOX,
            SecurityLevel.RESTRICTED,
        ]:
            anomaly_score += 0.6
