...
class OptimizedSelfEvolutionManager:
    ...
    def auto_place_file_chunked(
        self, source_content, suggested_filename=None, task_id=None, chunk_size=1000
    ):
        """
        チャンク処理によるファイル配置
        大きなファイルでも段階的に処理
        ...
        try:
            ...
            # 配置先決定（最初のチャンクで判断）
            target_relative_dir = self._determine_placement_simple(
                filename, source_content[:chunk_size]
            )
