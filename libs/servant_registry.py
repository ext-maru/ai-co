...
class ServantRegistry:
        ...
        try:
            if self.registry_file.exists():
                ...
                for servant_id, servant_data in data.items():
                    ...
                    servant_data['last_activity'] = datetime.fromisoformat(servant_data['last_activity'])
                    servant_data['registration_date'] = datetime.fromisoformat(servant_data['registration_date'])
    ...
    async def _save_registry(self):
        ...
        try:
            ...
            for servant_id, servant_info in self.servants.items():
                ...
                servant_dict['registration_date'] = servant_info.registration_date.isoformat()
    ...
    async def unregister_servant(self, servant_id: str) -> bool:
        ...
        if servant_info.current_tasks:
            logger.error(f"Cannot unregister {servant_id}: has {len(servant_info.current_tasks)} active tasks")
            return False
    ...
    async def assign_task(self, servant_id: str, task_id: str) -> bool:
        ...
        if servant_info.status != "active":
            logger.error(f"Servant {servant_id} is not active (status: {servant_info.status})")
            return False
        
        # Check capacity
        if len(servant_info.current_tasks) >= servant_info.max_concurrent_tasks:
            logger.error(f"Servant {servant_id} is at capacity ({len(servant_info.current_tasks)}/{servant_info.max_concurrent_tasks})")
            return False
        ...
        logger.info(f"⚔️ Assigned task {task_id} to {servant_info.servant_type.title()} {servant_id}")
        return True
    
    async def complete_task(self, servant_id: str, task_id: str, success: bool = True) -> bool:
        ...
        if success:
            ...
            servant_info.success_rate = (servant_info.success_rate * (total_tasks - 1) + 1.0) / total_tasks
        else:
            # Decrease success rate
            servant_info.success_rate = (servant_info.success_rate * (total_tasks - 1)) / total_tasks
        
        await self._save_registry()
        
        status_emoji = "✅" if success else "❌"
        logger.info(f"{status_emoji} Task {task_id} completed by {servant_info.servant_type.title()} {servant_id}")
        return True
    
    async def list_servants(self, servant_type: Optional[str] = None, status: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for servant_id, servant_info in self.servants.items():
            ...
            servant_dict = {
                ...
                "success_rate": round(servant_info.success_rate * 100, 1),  # Convert to percentage
    ...
    async def get_servant_status(self, servant_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "status": servant_info.status,
            "specialties": servant_info.specialties,
            "current_task": servant_info.current_tasks[-1] if servant_info.current_tasks else None,
            ...
            "capacity_utilization": round((len(servant_info.current_tasks) / servant_info.max_concurrent_tasks) * 100, 1)
        }
    ...
    async def get_servant_capacity(self, servant_id: str) -> Optional[Dict[str, int]]:
        ...
        return {
            "current_tasks": len(servant_info.current_tasks),
            "max_concurrent_tasks": servant_info.max_concurrent_tasks,
            "available_capacity": servant_info.max_concurrent_tasks - len(servant_info.current_tasks),
            "utilization_percentage": round((len(servant_info.current_tasks) / servant_info.max_concurrent_tasks) * 100, 1)
        }
    
    async def find_available_servants(self, servant_type: Optional[str] = None, specialty: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        available_servants.sort(key=lambda x: (-x["available_capacity"], -x["success_rate"]))
    ...
    async def get_registry_stats(self) -> Dict[str, Any]:
        ...
        if success_rates:
            stats["average_success_rate"] = round(sum(success_rates) / len(success_rates) * 100, 1)
    ...
    async def maintenance_mode(self, servant_id: str, enable: bool = True) -> bool:
        ...
        if enable:
            # Check if servant has active tasks
            if servant_info.current_tasks:
                logger.error(f"Cannot enable maintenance mode for {servant_id}: has {len(servant_info.current_tasks)} active tasks")
                return False
