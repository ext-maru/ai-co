...
class SessionMetadata:
    ...
    def update_sage_interaction(self, sage_type: SageType):
        ...
        self.sage_interactions_count[sage_key] = (
            self.sage_interactions_count.get(sage_key, 0) + 1
        )
...
class SessionContext:
    ...
    @classmethod
    def create_new(
        cls, user_id: str, project_path: str, session_id: Optional[str] = None
    ) -> "SessionContext":
        """新規セッションコンテキスト作成"""
        now = datetime.now()
    ...
    def get_sage_interaction_summary(self) -> Dict[str, Any]:
        ...
        summary["average_confidence"] = sum(
            i.confidence_score for i in self.sage_interactions
        ) / len(self.sage_interactions)
        summary["average_processing_time"] = sum(
            i.processing_time for i in self.sage_interactions
        ) / len(self.sage_interactions)
