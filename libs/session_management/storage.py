...
from .models import SessionContext, SessionMetadata, SageInteraction, ContextSnapshot, SessionStatus, SageType
...
class SQLiteAdapter:
    ...
    def _init_db(self):
        ...
        # インデックス作成
        conn.execute('CREATE INDEX IF NOT EXISTS idx_session_user ON session_metadata(user_id)')
        conn.execute('CREATE INDEX IF NOT EXISTS idx_session_updated ON session_metadata(updated_at)')
        conn.execute('CREATE INDEX IF NOT EXISTS idx_interactions_session ON sage_interactions(session_id)')
        conn.execute('CREATE INDEX IF NOT EXISTS idx_interactions_sage ON sage_interactions(sage_type)')
        conn.execute('CREATE INDEX IF NOT EXISTS idx_interactions_time ON sage_interactions(timestamp)')
        ...
        try:
            ...
            def _load():
                ...
                return SessionMetadata(
                    ...
                    last_sage_consultation=datetime.fromisoformat(row['last_sage_consultation']) if row['last_sage_consultation'] else None,
    ...
    async def save_interactions(self, session_id: str, interactions: List[SageInteraction]) -> bool:
        """4賢者相互作用保存"""
        try:
            loop = asyncio.get_event_loop()
            ...
            def _save():
                ...
                # 既存データ削除
                conn.execute('DELETE FROM sage_interactions WHERE session_id = ?', (session_id,))
...
class JSONAdapter:
    ...
    def _calculate_checksum(self, data: Dict[str, Any]) -> str:
        ...
        clean_data = {k: v for k, v in data.items() if k not in ['saved_at', 'checksum']}
...
class VectorAdapter:
    ...
    async def save_vector(self, session_id: str, vector: List[float], metadata: Optional[Dict[str, Any]] = None) -> bool:
        """ベクトル保存"""
        try:
            loop = asyncio.get_event_loop()
    ...
    async def join_transaction(self, transaction_id: str, participant: str):
        ...
        async with self._lock:
            if transaction_id in self._active_transactions:
                self._active_transactions[transaction_id]['participants'].append(participant)
    ...
    async def commit_transaction(self, transaction_id: str):
        ...
        async with self._lock:
            if transaction_id in self._active_transactions:
                ...
                self._active_transactions[transaction_id]['committed_at'] = datetime.now()
    ...
    async def rollback_transaction(self, transaction_id: str):
        ...
        async with self._lock:
            if transaction_id in self._active_transactions:
                ...
                self._active_transactions[transaction_id]['rolled_back_at'] = datetime.now()
...
class HybridStorage:
    ...
    async def save_session(self, context: SessionContext) -> bool:
        ...
        try:
            ...
            await self.sqlite_adapter.save_interactions(context.metadata.session_id, context.sage_interactions)
            ...
            context_data = {
                ...
                'temp_data': context.temp_data
            }
            await self.json_adapter.save_context_data(context.metadata.session_id, context_data)
    ...
    async def load_session(self, session_id: str) -> Optional[SessionContext]:
        ...
        try:
            ...
            context = SessionContext(
                ...
                temp_data=context_data.get('temp_data', {})
            )
    ...
    async def search_similar_sessions(self, query_text: str, user_id: str, top_k: int = 5) -> List[SessionContext]:
        """類似セッション検索"""
        try:
            ...
            # ベクトル検索
            similar_results = await self.vector_adapter.search_similar(query_vector, top_k)
