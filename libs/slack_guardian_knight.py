...
class SlackGuardianKnight(IncidentKnight):
                            ...
                            if "missing_scope" in log_content:
                                issues.append(Issue(
                                    ...
                                    metadata={
                                        ...
                                        'required_scopes': ['channels:read', 'groups:read', 'mpim:read', 'im:read', 'channels:history']
                                    }
                                ))
                                break
    ...
    async def _check_infrastructure(self) -> List[Issue]:
        ...
        try:
            import subprocess
            result = subprocess.run(['sudo', 'systemctl', 'status', 'rabbitmq-server'], 
                                  capture_output=True, text=True, timeout=5)
            
            if "dist_port_already_used" in result.stdout or "dist_port_already_used" in result.stderr:
                issues.append(Issue(
            ...
            if env_file.exists():
                ...
                for config_file in duplicate_configs:
                    ...
                    if config_path.exists():
                        ...
                        self.logger.info(f"📦 設定ファイルバックアップ: {config_file} -> {backup_path.name}")
