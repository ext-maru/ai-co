...
class SyntaxRepairKnight(IncidentKnight):
    ...
    async def _fix_file_syntax(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if content != original_content:
                ...
                try:
                    ast.parse(content)
                    self.logger.info(f"✅ Fixed syntax: {file_path.relative_to(PROJECT_ROOT)}")
                    return True
                except SyntaxError as e:
                    # 修復失敗時は元に戻す
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(original_content)
                    self.logger.warning(f"⚠️ Syntax fix failed: {file_path.relative_to(PROJECT_ROOT)} - {e}")
                    return False
            else:
                self.logger.info(f"ℹ️ No fixes needed: {file_path.relative_to(PROJECT_ROOT)}")
                return True
    ...
    async def _fix_basic_indentation(self, content: str) -> str:
        ...
        for line in lines:
            ...
            elif any(stripped.startswith(kw) for kw in ['return', 'pass', 'break', 'continue']):
