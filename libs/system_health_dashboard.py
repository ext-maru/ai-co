...
class SystemHealthDashboard:
    ...
    def start_health_monitoring(self):
        ...
        # 予防的メンテナンススレッド
        maintenance_thread = threading.Thread(target=self._preventive_maintenance_loop, daemon=True)
    ...
    def _perform_health_check(self):
        ...
        if self.worker_recovery:
            ...
            if unhealthy_workers >= self.monitoring_rules["worker_failure_threshold"]:
                self._create_alert("warning", "worker", f"{unhealthy_workers}個のワーカーが異常状態です")
    ...
    def _check_disk_usage(self):
        ...
        try:
            ...
            if usage_percent > 90:
                self._create_alert("critical", "disk", f"ディスク使用量が危険レベル: {usage_percent:.1f}%")
            elif usage_percent > 80:
                self._create_alert("warning", "disk", f"ディスク使用量が高い: {usage_percent:.1f}%")
    ...
    def _check_memory_usage(self):
        ...
        try:
            ...
            if memory.percent > 90:
                self._create_alert("critical", "memory", f"メモリ使用量が危険レベル: {memory.percent}%")
    ...
    def _calculate_system_score(self):
        ...
        recent_incidents = [
            ...
            if alert.type == "critical" and 
            alert.timestamp > datetime.now() - timedelta(hours=24)
        ]
        base_score -= len(recent_incidents) * 5
        
        # 稼働時間による加点
        uptime_hours = (datetime.now() - self.system_stats["uptime_start"]).total_seconds() / 3600
        if uptime_hours > 24:
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        active_alerts = [
            ...
            for alert in self.active_alerts.values() if not alert.resolved
        ]
        ...
        return {
            ...
            "components": {
                "rabbitmq": self.rabbitmq_monitor.get_status_report() if self.rabbitmq_monitor else None,
                "workers": self.worker_recovery.get_system_status() if self.worker_recovery else None
            }
        }
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            print(f"🩺 ヘルススコア: {data['system_score']:.1f}, "
