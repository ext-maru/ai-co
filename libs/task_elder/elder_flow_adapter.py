...
class TaskElderFlowAdapter:
    ...
    async def execute_with_elder_flow(
        self, task_breakdown_list: List, plan_name: str
    ) -> Dict:
        ...
        for i, task in enumerate(task_breakdown_list, 1):
            ...
            # Elder Flow実行
            flow_result = await self._execute_single_task_with_elder_flow(
                task, flow_request
            )
        ...
        if results["quality_scores"]:
            results["average_quality_score"] = sum(results["quality_scores"]) / len(
    ...
    async def _execute_single_task_with_elder_flow(
        self, task_breakdown, flow_request: ElderFlowTaskRequest
    ) -> ElderFlowResult:
        """単一タスクをElder Flowで実行"""
        ...
        try:
            ...
            quality_score = self._calculate_quality_score(
                task_breakdown, sage_consultations
            )
    ...
    def _calculate_quality_score(
        self, task_breakdown, sage_consultations: Dict
    ) -> float:
        """品質スコアを計算"""
        base_score = 75.0
        ...
        if sage_consultations:
            knowledge_confidence = sage_consultations.get("knowledge_sage", {}).get(
                "confidence", 0.5
            )
            task_optimization = sage_consultations.get("task_sage", {}).get(
                "optimization_score", 0.5
            )
            risk_level = 1.0 - sage_consultations.get("incident_sage", {}).get(
                "risk_score", 0.5
            )
            context_score = sage_consultations.get("rag_sage", {}).get(
                "context_score", 0.5
            )

            sage_bonus = (
                knowledge_confidence + task_optimization + risk_level + context_score
            ) * 5
            base_score += sage_bonus

        # 優先度による調整
        priority_bonus = {"high": 10, "medium": 5, "low": 0}.get(
            task_breakdown.priority, 0
        )
    ...
    def get_integration_status(self) -> Dict:
        ...
        successful = sum(
            1 for h in self.integration_history if h["results"]["overall_success"]
        )
    ...
    async def get_integration_report(self) -> Dict:
        ...
        recent_integrations = (
            self.integration_history[-5:]
            if len(self.integration_history) >= 5
            else self.integration_history
        )
