...
class GitHubProjectsClient:
    ...
    def __init__(
        self, token: Optional[str] = None, owner: str = "aicompany", repo: str = "ai_co"
    ):
        ...
        try:
            async with self.session.post(self.graphql_url, json=payload) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"GraphQL request failed: {response.status} - {error_text}"
                    )
    ...
    async def update_project_item(
        ...
        priority: Optional[ProjectItemPriority] = None,
    ) -> Optional[ProjectItem]:
        """プロジェクトアイテムを更新"""
        if self.dry_run:
        ...
        # フィールドの更新
        if status:
            await self._update_item_fields(
                project_id, item_id, status, priority or ProjectItemPriority.MEDIUM
            )
    ...
    async def sync_plan_to_project(
        self, project_id: str, plan_name: str, tasks: List[Dict]
    ) -> Dict:
        """計画書をプロジェクトに同期"""
        if self.dry_run:
        ...
        for task in tasks:
            ...
            priority = priority_map.get(
                task.get("priority", "medium"), ProjectItemPriority.MEDIUM
            )
    ...
    async def get_project_overview(self, project_id: str) -> Dict:
        ...
        if self.dry_run:
            return {
                "project_id": project_id,
                "total_items": 10,
                "status_counts": {"Todo": 3, "In Progress": 4, "Done": 2, "Blocked": 1},
                "priority_counts": {"Low": 2, "Medium": 5, "High": 2, "Urgent": 1},
