...
from task_elder.project_board_manager import (
    BoardConfig,
    BoardTemplate,
    ProjectBoardManager,
)
    ...
    async def sync_plan_changes(
        ...
        try:
            ...
            if any(
                change in changes
                for change in [
                    ChangeType.UPDATED_PLAN,
                    ChangeType.TASK_ADDED,
                    ChangeType.TASK_UPDATED,
                    ChangeType.TASK_DELETED,
                ]
            ):
                # 既存プロジェクトボードの更新
                result = await self.board_manager.sync_plan_to_board(
                    plan_file, project_id
                )
...
:
    ...
    async def get_sync_status(self) -> Dict:
        """同期状況を取得"""
        recent_events = (
            self.sync_events[-10:] if len(self.sync_events) >= 10 else self.sync_events
        )
...
"""メイン実行関数"""
    ...
    print(
        f"   ✅ 成功率: {status['stats']['successful_syncs']}/{status['stats']['total_syncs']}"
    )
