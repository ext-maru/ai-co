...
class ProgressVisualization:
    ...
    async def collect_project_metrics(self, project_id: str) -> ProgressMetrics:
        ...
        try:
            ...
            estimated_completion = self._estimate_completion_date(
                status_breakdown.get("todo", 0) + status_breakdown.get("in_progress", 0),
                velocity
            )
    ...
    def _calculate_velocity(self, project_id: str) -> float:
        ...
        recent_metrics = [
            ...
            if m.get("project_id") == project_id and 
            datetime.fromisoformat(m.get("timestamp", "2000-01-01")) > datetime.now() - timedelta(days=7)
        ]
        ...
        days_diff = (datetime.fromisoformat(recent_metrics[-1]["timestamp"]) - 
                    datetime.fromisoformat(recent_metrics[0]["timestamp"])).days
    ...
    def _estimate_completion_date(self, remaining_tasks: int, velocity: float) -> Optional[str]:
        """完了予定日を推定"""
    ...
    def _determine_risk_level(self, health_score: float, metrics: ProgressMetrics) -> str:
        """リスクレベルを判定"""
        if health_score >= 80:
            return "低"
    ...
    def _generate_recommendations(self, metrics: ProgressMetrics, issues: List[str]) -> List[str]:
        """推奨事項を生成"""
        recommendations = []
    ...
    async def generate_progress_chart(self, project_id: str) -> str:
        ...
        try:
            ...
            completion_rates = [m["metrics"]["completion_rate"] for m in project_history]
            
            # チャートを作成
            fig, ax = plt.subplots(figsize=self.viz_config.figure_size)
            
            ax.plot(dates, completion_rates, marker='o', linewidth=2, markersize=6)
            ax.set_title(f"プロジェクト進捗推移 (ID: {project_id})", fontsize=self.viz_config.font_size + 2)
    ...
    async def generate_status_distribution(self, project_id: str) -> str:
        ...
        try:
            ...
            ax.set_title(f"タスクステータス分布 (ID: {project_id})", 
    ...
    async def _generate_html_report(self, report_data: Dict) -> str:
        ...
        if report_data["health"]["issues"]:
            issues_html = "<ul>" + "".join(f"<li>{issue}</li>" for issue in report_data["health"]["issues"]) + "</ul>"
            issues_section = f'<div class="issues"><h3>⚠️ 問題点</h3>{issues_html}</div>'
        
        # 推奨事項セクション
        recommendations_section = ""
        if report_data["health"]["recommendations"]:
            recommendations_html = "<ul>" + "".join(f"<li>{rec}</li>" for rec in report_data["health"]["recommendations"]) + "</ul>"
...
{charts_section}
        ...
        issues_section = ""
        if report_data["health"]["issues"]:
            issues_list = "\n".join(f"- {issue}" for issue in report_data["health"]["issues"])
            issues_section = f"### ⚠️ 問題点\n\n{issues_list}"
        
        # 推奨事項セクション
        recommendations_section = ""
        if report_data["health"]["recommendations"]:
            recommendations_list = "\n".join(f"- {rec}" for rec in report_data["health"]["recommendations"])
    ...
    async def get_visualization_summary(self) -> Dict:
        ...
        return {
            "stats": self.stats,
            "tracked_projects": len(set(m.get("project_id") for m in self.metrics_history)),
