...
from task_elder.github_projects_client import GitHubProjectsClient, ProjectBoard, ProjectItem, ProjectItemStatus, ProjectItemPriority
from task_elder.task_breakdown_engine import TaskBreakdownEngine, TaskBreakdown
...
class ProjectBoardManager:
    ...
    async def create_board_from_plan(self, plan_file: str, board_title: str, 
                                     board_config: Optional[BoardConfig] = None) -> Optional[ProjectBoard]:
        """計画書からプロジェクトボードを作成"""
        if not board_config:
            board_config = BoardConfig()
    ...
    async def sync_plan_to_board(self, plan_file: str, project_id: Optional[str] = None) -> Dict:
        """計画書をプロジェクトボードに同期"""
        ...
        try:
            ...
            # GitHub Projectsに同期
            async with self.github_client as client:
                sync_result = await client.sync_plan_to_project(project_id, plan_file, project_tasks)
        ...
        try:
            async with self.github_client as client:
                ...
                return {
                    ...
                    "health_score": self._calculate_health_score(status_counts, total_items)
                }
    ...
    async def auto_sync_all_boards(self) -> Dict:
        ...
        for key, mapping in self.sync_mappings.items():
            try:
                ...
                result = await self.sync_plan_to_board(mapping.plan_file, mapping.project_id)
    ...
    async def create_board_mapping(self, plan_file: str, project_id: str, 
                                   board_config: Optional[BoardConfig] = None) -> bool:
        """ボードマッピングを作成"""
        if not board_config:
            board_config = BoardConfig()
