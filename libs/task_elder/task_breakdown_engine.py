...
class TaskBreakdownEngine:
    ...
    def __init__(self):
        ...
        self.category_keywords = {
            "implementation": ["実装", "開発", "作成", "構築", "implementation", "develop", "create"],
    ...
    async def extract_tasks_from_plan(self, plan_file_path: str) -> List[TaskBreakdown]:
        ...
        try:
            ...
            for i, raw_task in enumerate(raw_tasks):
                task = self._create_task_breakdown(
                    raw_task, 
                    i + 1, 
                    plan_path.name
                )
    ...
    def _create_task_breakdown(self, raw_task: str, task_number: int, plan_name: str) -> TaskBreakdown:
    ...
    def get_extraction_stats(self) -> Dict:
        ...
        return {
            "stats": self.stats,
            "success_rate": (self.stats["successful_extractions"] / max(1, self.stats["total_extractions"])) * 100,
            "average_tasks_per_plan": self.stats["total_tasks"] / max(1, self.stats["successful_extractions"]),
    ...
    async def optimize_task_breakdown(self, tasks: List[TaskBreakdown]) -> List[TaskBreakdown]:
        """タスク分解を最適化"""
        optimized_tasks = []
    ...
    async def _fix_task_issues(self, task: TaskBreakdown, issues: List[str]) -> TaskBreakdown:
        """タスクの問題を修正"""
