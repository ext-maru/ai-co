...
class TaskSender:
    ...
    def send_to_queue(
        self, queue_name: str, message: Dict, priority: Optional[int] = None
    ) -> bool:
        """指定されたキューにメッセージを送信"""
        try:
            # メッセージにタイムスタンプを追加
            ...
            self.logger.info(
                f"Message sent to {queue_name}: task_id={message.get('task_id', 'unknown')}"
            )
            return True
    ...
    def send_task(
        ...
        # 優先度マッピング
        priority_map = {"critical": 10, "high": 7, "normal": 5, "low": 3}
    ...
    def send_error_analysis_request(
        self, error_text: str, context: Dict, response_queue: Optional[str] = None
    ) -> bool:
        """エラー分析リクエストを送信"""
        analysis_request = {
...
if __name__ == "__main__":
    ...
    success = sender.send_to_pm(
        files_created=["test_fibonacci.py"], task_id=task_id, git_commit=True
    )
