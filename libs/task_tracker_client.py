...
class TaskTrackerClient(BaseManager):
    ...
    def create_task(self, task_id: str, title: str, description: str = "", 
                   priority: int = 3, assignee: str = "pm") -> str:
    ...
    def create_pm_task(self, task_data: Dict) -> str:
        """PMワーカーのタスクを作成"""
        task_id = task_data.get('task_id', f"unknown_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    def update_pm_task_status(self, task_id: str, status: str, result_data: Dict = None):
        """PMワーカーのタスクステータスを更新"""
        # ステータスマッピング
        status_map = {
