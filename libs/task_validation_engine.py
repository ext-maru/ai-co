...
class TaskValidationEngine:
    ...
    def __init__(self):
        self.validation_log_path = Path("/home/aicompany/ai_co/logs/task_validation.log")
        ...
        self.servant_capabilities = {
            "knight": {
                "allowed_tasks": ["testing", "quality_assurance", "security", "guard_duty", "coverage"],
                "restricted_tasks": ["system_shutdown", "data_deletion", "configuration_change"],
                "elder_requirements": ["task_sage"]  # Minimum elder consultation required
            },
            "dwarf": {
                "allowed_tasks": ["building", "optimization", "infrastructure", "deployment", "mining"],
                "restricted_tasks": ["security_bypass", "unauthorized_access"],
                "elder_requirements": ["task_sage", "incident_sage"]
            },
            "wizard": {
                "allowed_tasks": ["analysis", "automation", "monitoring", "debugging", "spellcasting"],
                "restricted_tasks": ["destructive_magic", "unauthorized_automation"],
                "elder_requirements": ["knowledge_sage", "task_sage"]
            },
            "elf": {
                "allowed_tasks": ["monitoring", "logging", "alerting", "surveillance", "watching"],
        ...
        self.high_risk_keywords = [
            "delete", "remove", "destroy", "shutdown", "disable", "bypass", 
            "override", "escalate", "privilege", "admin", "root", "sudo",
            "configuration", "settings", "credentials", "password", "secret"
        ]
    ...
    async def validate_task(
        ...
        try:
            ...
            basic_validation = await self._basic_validation(servant_type, servant_id, task_description)
            ...
            approval_token = await self._generate_approval_token(
                servant_type, servant_id, task_description, consultation_result.consulted_elders
            )
            
            # Step 5: Log validation
            await self._log_validation(
                servant_type, servant_id, task_description, "APPROVED", 
                consultation_result.consulted_elders, approval_token
            )
    ...
    async def _basic_validation(self, servant_type: str, servant_id: str, task_description: str) -> ValidationResult:
        ...
        if servant_type not in self.servant_capabilities:
            return ValidationResult(
                is_valid=False,
                reason=f"Unknown servant type: {servant_type}"
            )
        ...
        if not task_description or len(task_description.strip()) < 10:
            return ValidationResult(
                is_valid=False,
                reason="Task description too short or empty"
            )
        ...
        if not has_matching_capability:
            logger.warning(f"Task may not align with {servant_type} capabilities: {task_description[:50]}...")
    ...
    async def _assess_risk_level(self, task_description: str) -> str:
        ...
        high_risk_count = sum(1 for keyword in self.high_risk_keywords if keyword in task_lower)
    ...
    async def _consult_elders(self, servant_type: str, task_description: str, risk_level: str, priority: str) -> ValidationResult:
    ...
    async def _simulate_elder_consultation(
        self, elders: List[str], servant_type: str, task_description: str, risk_level: str, priority: str
    ) -> List[Dict[str, Any]]:
        ...
        for elder in elders:
            ...
            approval = await self._elder_decision_logic(elder, servant_type, task_description, risk_level, priority)
            results.append(approval)
        
        return results
    
    async def _elder_decision_logic(
        self, elder: str, servant_type: str, task_description: str, risk_level: str, priority: str
    ) -> Dict[str, Any]:
        ...
        if elder == "knowledge_sage":
            ...
            knowledge_keywords = ["learn", "document", "analyze", "research", "study", "understand"]
        ...
        # Incident Sage consultation
            ...
            security_keywords = ["security", "monitor", "protect", "guard", "watch", "alert"]
        ...
        # Task Sage consultation
            ...
            efficiency_keywords = ["optimize", "improve", "enhance", "coordinate", "automate"]
        ...
        # RAG Sage consultation
            ...
            search_keywords = ["search", "find", "discover", "retrieve", "analyze", "information"]
    ...
    async def _generate_approval_token(
        self, servant_type: str, servant_id: str, task_description: str, consulted_elders: List[str]
    ) -> str:
    ...
    async def verify_approval_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            token_timestamp = datetime.strptime(token_data["timestamp"], "%Y%m%d_%H%M%S")
    ...
    async def _log_validation(
        ...
        result: str, consulted_elders: List[str], approval_token: Optional[str] = None
    ):
    ...
    async def get_validation_history(self, servant_id: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    async def get_validation_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_validations"] > 0:
            stats["approval_rate"] = round((stats["approved"] / stats["total_validations"]) * 100, 1)
