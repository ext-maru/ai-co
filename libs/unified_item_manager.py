    ...
    def sync_from_legacy_systems(self) -> List[ItemInfo]:
        ...
        if self.weapon_sharing:
            try:
                ...
                logger.info(f"📦 Synced {len(weapon_data)} weapons from WeaponSharingSystem")
            except Exception as e:
        ...
        if self.knight_brigade:
            try:
                ...
                for eq_id, equipment in equipment_data.items():
                    ...
                    unified_items.append(item)
                logger.info(f"⚔️ Synced {len(equipment_data)} equipment from KnightBrigade")
            except Exception as e:
        ...
        if self.dwarf_workshop:
            try:
                ...
                for mat_id, material in inventory_data.items():
                    ...
                    unified_items.append(item)
                logger.info(f"🔨 Synced {len(inventory_data)} materials from DwarfWorkshop")
            except Exception as e:
...
class UnifiedItemManager:
        ...
        try:
            if Path(self.data_file).exists():
                ...
                for item_id, item_data in data.get('items', {}).items():
                    item_data['created_at'] = datetime.fromisoformat(item_data['created_at']) if item_data.get('created_at') else None
                    item_data['updated_at'] = datetime.fromisoformat(item_data['updated_at']) if item_data.get('updated_at') else None
                    item_data['allocated_at'] = datetime.fromisoformat(item_data['allocated_at']) if item_data.get('allocated_at') else None
                    
                    self.items[item_id] = ItemInfo(**item_data)
                
                # 割り当て記録復元
                for alloc_id, alloc_data in data.get('allocations', {}).items():
                    alloc_data['allocated_at'] = datetime.fromisoformat(alloc_data['allocated_at'])
                    if alloc_data.get('expected_return'):
                        alloc_data['expected_return'] = datetime.fromisoformat(alloc_data['expected_return'])
                    if alloc_data.get('returned_at'):
                        alloc_data['returned_at'] = datetime.fromisoformat(alloc_data['returned_at'])
                    
                    self.allocations[alloc_id] = AllocationRecord(**alloc_data)
                
                self._rebuild_indexes()
                logger.info(f"📂 Loaded {len(self.items)} items and {len(self.allocations)} allocations")
        except Exception as e:
    ...
    def _rebuild_indexes(self):
        ...
        self.stats['active_allocations'] = len([a for a in self.allocations.values() if a.status == 'active'])
    ...
    def _perform_initial_sync(self):
        ...
        try:
            ...
            logger.info(f"🔄 Initial sync completed: {synced_count} new items integrated")
        except Exception as e:
    ...
    def allocate_item(self, item_id: str, allocated_to: str, 
        ...
        try:
            with self._lock:
                ...
                if item.allocated_to and item.allocated_to != allocated_to:
                    logger.warning(f"⚠️ Item {item_id} already allocated to {item.allocated_to}")
                    return None
    ...
    def get_allocation_history(self, item_id: Optional[str] = None) -> List[AllocationRecord]:
        """割り当て履歴の取得"""
        if item_id:
            return [alloc for alloc in self.allocations.values() if alloc.item_id == item_id]
