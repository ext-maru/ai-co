...
class UnitProgressTracker:
    ...
    def analyze_logs(self, start_date: datetime, end_date: datetime) -> List[UnitActivity]:
        """ログファイルから活動を分析"""
        activities = []
        ...
        for log_file, unit_type in log_files:
            ...
            if log_path.exists():
                activities.extend(self._parse_log_file(log_path, unit_type, start_date, end_date))
    ...
    def _parse_log_file(self, log_path: Path, unit_type: str, start_date: datetime, end_date: datetime) -> List[UnitActivity]:
        """ログファイルを解析"""
        activities = []
            ...
            with open(log_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if unit_type == "knights" or unit_type == "all":
                        activities.extend(self._detect_knights_activity(line, timestamp))
                    
                    if unit_type == "wizards" or unit_type == "all":
                        activities.extend(self._detect_wizards_activity(line, timestamp))
    ...
    def _detect_knights_activity(self, line: str, timestamp: datetime) -> List[UnitActivity]:
        """騎士団の活動を検出"""
        activities = []
        ...
        if match:
            ...
            activities.append(UnitActivity(
                timestamp=timestamp,
                unit_type="knights",
        ...
        # Detect auto actions
        match = re.search(self.log_patterns["knights"]["auto_action"], line, re.IGNORECASE)
        if match:
    ...
    def _detect_wizards_activity(self, line: str, timestamp: datetime) -> List[UnitActivity]:
        """ウィザーズの活動を検出"""
        activities = []
        
        # Detect knowledge gaps
        match = re.search(self.log_patterns["wizards"]["knowledge_gap"], line, re.IGNORECASE)
        if match:
            gap = match.group(1)
            activities.append(UnitActivity(
                timestamp=timestamp,
        ...
        # Detect enrichment
        match = re.search(self.log_patterns["wizards"]["enrichment"], line, re.IGNORECASE)
        if match:
    ...
    def _detect_dwarf_activity(self, line: str, timestamp: datetime) -> List[UnitActivity]:
        """ドワーフ工房の活動を検出"""
        activities = []
        
        # Detect optimization opportunities
        match = re.search(self.log_patterns["dwarf"]["optimization"], line, re.IGNORECASE)
        if match:
    ...
    def _detect_auto_activities(self, start_date: datetime, end_date: datetime) -> List[UnitActivity]:
        """自動生成された活動を検出"""
        activities = []
    ...
    def generate_daily_report(self, date: datetime) -> DailyProgress:
        ...
        summary = {
            ...
            "overall_health": self._calculate_overall_health(knights_stats, wizards_stats, dwarf_stats)
        }
    ...
    def _aggregate_unit_stats(self, activities: List[UnitActivity], unit_type: str) -> Dict[str, Any]:
        """ユニット別の統計を集計"""
    ...
    def _calculate_overall_health(self, knights: Dict, wizards: Dict, dwarf: Dict) -> float:
        """全体的な健康度を計算"""
        ...
        return (knights_score * knights_weight + 
                wizards_score * wizards_weight + 
                dwarf_score * dwarf_weight) * 100
...
def main():
    ...
    print(f"📁 Report saved to: knowledge_base/unit_progress/daily_progress_{report.date}.md")
