...
from libs.utt_system.data_models import (
    UTTDataManager, UTTTask, UTTTaskDependency, UTTTaskLog, UTTSageConsultation,
    IronWillCriteria, TaskStatus, TaskPriority, TaskCategory, SageType,
    Base
)
...
class UTTCRUDManager(EldersServiceLegacy):
    ...
    async def _log_operation(self, task_id: str, action: str, details: Dict[str, Any],
        ...
        try:
            ...
            await self.data_manager.process_request({
                "operation": "log_sage_consultation",  # 仮の方法
                "data": log_data
            })
        except Exception as e:
            self.logger.warning(f"Failed to log operation: {e}")

    async def _validate_task_data(self, task_data: Dict[str, Any]) -> Dict[str, List[str]]:
        """タスクデータ検証"""
        errors = {}
    ...
    async def _consult_sages(self, task_id: str, operation: str, 
        ...
        if operation in ["create", "update"] and task_data.get("priority") == TaskPriority.CRITICAL.value:
            task_consultation = {
    ...
    async def _trigger_elder_flow(self, task_id: str, operation: str, 
    ...
    async def create_task(self, task_data: Dict[str, Any], 
                         ...
                         enable_elder_flow: Optional[bool] = None) -> CRUDResult:
        """
        タスク作成
        ...
        try:
            ...
            if enable_elder_flow or self.config["enable_elder_flow"]:
                elder_flow_execution = await self._trigger_elder_flow(task_id, "create", task_data)
            ...
            return CRUDResult(
                success=False,
                error={
                    "type": "internal_error" if not isinstance(e, SQLAlchemyError) else "database_error",
                    "message": str(e)
                }
            )
    ...
    async def read_task(self, task_id: str, 
                       ...
                       include_history: bool = False) -> CRUDResult:
        """
        タスク読み取り
        ...
        try:
            ...
            return CRUDResult(
                ...
                metadata={
                    ...
                    "history": history_data["data"] if history_data and history_data["success"] else None,
                    "response_time": response_time
                }
            )
    ...
    async def update_task(self, task_id: str, update_data: Dict[str, Any],
                         ...
                         enforce_iron_will: Optional[bool] = None) -> CRUDResult:
        """
        タスク更新
        ...
        try:
            ...
            # Iron Will検証（有効な場合）
            if enforce_iron_will or (enforce_iron_will is None and self.config["enforce_iron_will"]):
            ...
            is_complex_update = (
                len(changes) > 3 or 
                "iron_will_criteria" in update_data or
                update_data.get("priority") == TaskPriority.CRITICAL.value
            )
            
            # Elder Flow起動（複雑な更新の場合）
            elder_flow_execution = None
            if is_complex_update and (enable_elder_flow or self.config["enable_elder_flow"]):
            ...
            result_data = updated_task.data if updated_task.success else {"task_id": task_id}
    ...
    async def delete_task(self, task_id: str,
                         ...
                         user_context: Optional[UserContext] = None) -> CRUDResult:
        """
        タスク削除
        ...
        try:
            ...
            use_soft_delete = soft_delete if soft_delete is not None else self.config["enable_soft_delete"]
            ...
            if use_soft_delete:
                ...
                if delete_result.success:
                    if delete_result.data:
                        ...
                        delete_result.data["deleted_at"] = delete_result.data.get("deleted_at")
                    else:
                        delete_result.data = {
    ...
    async def list_tasks(self, criteria: Optional[SearchCriteria] = None,
                        filters: Optional[Dict[str, Any]] = None,
                        sort_by: Optional[str] = None,
                        sort_order: str = "asc",
                        page: int = 1,
                        per_page: Optional[int] = None) -> CRUDResult:
        """
    ...
    async def search_tasks(self, query: str,
                          search_fields: Optional[List[str]] = None) -> CRUDResult:
        """
        タスク検索
    ...
    async def bulk_create_tasks(self, tasks_data: List[Dict[str, Any]],
                               user_context: Optional[UserContext] = None) -> CRUDResult:
        """
        一括タスク作成
    ...
    async def bulk_update_tasks(self, task_ids: List[str], update_data: Dict[str, Any],
                               user_context: Optional[UserContext] = None) -> CRUDResult:
        """
        一括タスク更新
    ...
    async def test_crud_basic():
        ...
        try:
            ...
            if create_result.success:
                ...
                # タスク更新テスト
                update_result = await manager.update_task(task_id, {
