...
from sqlalchemy import (
    ...
    Boolean, Float, create_engine, Index, func
)
...
class IronWillCriteria:
    ...
    def meets_iron_will_standard(self) -> bool:
        """Iron Will基準達成判定"""
        return (
            self.root_cause_resolution >= 95.0 and
            self.dependency_completeness >= 100.0 and
            self.test_coverage >= 95.0 and
            self.security_score >= 90.0 and
            self.performance_score >= 85.0 and
            self.maintainability_score >= 80.0
        )
...
class UTTTask(Base):
    ...
    # リレーション
    dependencies = relationship("UTTTaskDependency", foreign_keys="UTTTaskDependency.task_id", back_populates="task")
    dependents = relationship("UTTTaskDependency", foreign_keys="UTTTaskDependency.depends_on_task_id", back_populates="depends_on_task")
    logs = relationship("UTTTaskLog", back_populates="task", order_by="UTTTaskLog.created_at")
...
class UTTTaskDependency(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    task_id = Column(UUID(as_uuid=True), ForeignKey('utt_tasks.id'), nullable=False, index=True)
    depends_on_task_id = Column(UUID(as_uuid=True), ForeignKey('utt_tasks.id'), nullable=False, index=True)
    dependency_type = Column(String(20), nullable=False, default="blocks")  # blocks, relates_to, child_of
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    
    # リレーション
    task = relationship("UTTTask", foreign_keys=[task_id], back_populates="dependencies")
    depends_on_task = relationship("UTTTask", foreign_keys=[depends_on_task_id], back_populates="dependents")
...
class UTTTaskLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    task_id = Column(UUID(as_uuid=True), ForeignKey('utt_tasks.id'), nullable=False, index=True)
    log_type = Column(String(30), nullable=False, index=True)  # status_change, sage_consultation, elder_flow, etc.
...
class UTTSageConsultation(Base):
    ...
    consultation_type = Column(String(30), nullable=False)  # advice, analysis, approval, etc.
...
class UTTDataManager(EldersServiceLegacy):
    ...
    async def _update_task(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in ["title", "description", "status", "priority", "category", 
                     "assigned_sage", "assigned_servant", "elder_flow_id"]:
            if field in data:
                setattr(task, field, data[field])
        
        # 完了時の統計更新
        if data.get("status") == TaskStatus.COMPLETED.value and task.status != TaskStatus.COMPLETED.value:
    ...
    async def _add_dependency(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """タスク依存関係追加"""
        task = self.session.query(UTTTask).filter_by(task_id=data["task_id"]).first()
        depends_on_task = self.session.query(UTTTask).filter_by(task_id=data["depends_on_task_id"]).first()
    ...
    async def _update_iron_will_score(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 統計更新
        avg_score = self.session.query(UTTTask).filter(UTTTask.iron_will_score > 0).with_entities(
            func.avg(UTTTask.iron_will_score)
        ).scalar() or 0.0
