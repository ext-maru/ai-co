...
class SharedWeaponInventory:
    ...
    def add_weapon(self, weapon_id: str, weapon_type: WeaponType, 
                   specs: Dict[str, Any], owner: str = "workshop"):
        """武具を追加"""
        with self._lock:
            weapon_data = {
                'weapon_id': weapon_id,
            ...
            logger.info(f"➕ Added weapon {weapon_id} ({weapon_type.value}) to inventory")
    ...
    def get_available_weapons(self, weapon_type: Optional[WeaponType] = None) -> List[Dict[str, Any]]:
        """利用可能な武具を取得"""
        with self._lock:
            available = []
            ...
            if weapon_type:
                ...
                weapon_list = [w for weapons in self.inventory.values() for w in weapons]
            
            for weapon in weapon_list:
                if weapon['status'] == 'available' and not weapon['maintenance_required']:
...
class WeaponSharingCoordinator:
    ...
    def start(self):
        ...
        self._delivery_tracker_thread = threading.Thread(
            target=self._track_deliveries_loop,
            daemon=True
        )
    ...
    def submit_request(self, request: WeaponRequest) -> str:
        ...
        logger.info(f"📥 Received weapon request {request.request_id} from {request.requester_id}")
    ...
    def _process_single_request(self, request: WeaponRequest):
        ...
        if len(available_weapons) >= request.quantity:
            # 即座に配送可能
            self._fulfill_request_immediately(request, available_weapons[:request.quantity])
        else:
            # クラフトが必要
            self._schedule_crafting(request)
    
    def _fulfill_request_immediately(self, request: WeaponRequest, weapons: List[Dict[str, Any]]):
        """即座にリクエストを満たす"""
        weapon_specs = []
        
        for weapon in weapons:
            if self.inventory.allocate_weapon(weapon['weapon_id'], request.requester_id):
                weapon_specs.append({
        ...
        logger.info(f"📦 Immediately delivered {len(weapon_specs)} weapons for request {request.request_id}")
    ...
    def _schedule_crafting(self, request: WeaponRequest):
        ...
        logger.info(f"🔨 Scheduled crafting for request {request.request_id}, ETA: {estimated_delivery}")
    ...
    def _complete_crafting(self, delivery: WeaponDelivery):
        """クラフト完了処理"""
        # 武具を作成（簡略化）
        request = next((r for r in self.request_history if r.request_id == delivery.request_id), None)
        if not request:
            return
    ...
    def _complete_delivery(self, delivery: WeaponDelivery):
        ...
        # メトリクス更新
        delivery_time = (delivery.actual_delivery - delivery.estimated_delivery).total_seconds()
        self.delivery_metrics['average_delivery_time'] = (
            (self.delivery_metrics['average_delivery_time'] * 
             (self.delivery_metrics['fulfilled_requests'] - 1) + delivery_time) /
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.delivery_metrics,
            ...
            'active_deliveries': len([d for d in self.active_deliveries.values() 
