...
class WorkerHealthMonitor:
    ...
    async def check_worker_health(self, worker_id: str) -> WorkerState:
        ...
        if previous_status != worker_state.status:
            await self._emit_health_change(worker_id, previous_status, worker_state.status)
    ...
    def _kill_worker_process(self, worker_id: str):
        ...
        try:
            ...
            result = subprocess.run(['pkill', '-f', worker_id], 
    ...
    def _restart_worker(self, worker_id: str) -> bool:
        ...
        try:
            ...
            if 'task' in worker_id.lower():
                script_path = Path("/home/aicompany/ai_co/workers/enhanced_task_worker.py")
            elif 'pm' in worker_id.lower():
                script_path = Path("/home/aicompany/ai_co/workers/intelligent_pm_worker_simple.py")
            elif 'result' in worker_id.lower():
                script_path = Path("/home/aicompany/ai_co/workers/async_result_worker_simple.py")
            else:
    ...
    async def _emit_health_change(self, worker_id: str, old_status: HealthStatus, new_status: HealthStatus):
...
class AutoRecoveryEngine:
    ...
    async def attempt_recovery(self, worker_state: WorkerState) -> RecoveryResult:
        ...
        if worker_state.last_recovery:
            ...
            time_since_last = (datetime.now() - worker_state.last_recovery).total_seconds()
        ...
        for strategy in self.enabled_strategies:
            ...
            await self._emit_recovery_attempt(worker_state.worker_id, RecoveryAction.RESTART)
    ...
    async def _execute_recovery_action(self, strategy: RecoveryStrategy, worker_state: WorkerState) -> bool:
        """
...
class RecoveryStrategyManager:
    ...
    def _register_default_strategies(self):
        ...
        self.register_strategy(
            ...
            self._default_restart_strategy,
            priority=1
        )
        self.register_strategy(
            RecoveryStrategy.RESET,
            self._default_reset_strategy,
            priority=2
        )
        self.register_strategy(
            RecoveryStrategy.RECREATE,
            self._default_recreate_strategy,
            priority=3
        )
    ...
    def register_strategy(self, strategy: RecoveryStrategy, handler: Callable, priority: int = 0):
        """
    ...
    def select_strategy(self, worker_state: WorkerState) -> Optional[StrategyInfo]:
        ...
        if worker_state.failure_reason and "memory" in worker_state.failure_reason.lower():
            return self.strategies.get(RecoveryStrategy.RESET)
        
        # Default to restart
        return self.strategies.get(RecoveryStrategy.RESTART)
    
    async def execute_strategy(self, strategy: RecoveryStrategy, worker_state: WorkerState) -> bool:
        """
