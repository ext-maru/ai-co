...
class WorkerLoadBalancer:
    ...
    def scan_workers(self) -> List[WorkerProcess]:
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_info', 'status', 'create_time']):
            try:
                ...
                if any(keyword in info['name'].lower() for keyword in ['python', 'python3']):
                    if any(pattern in cmdline for pattern in ['worker', 'claude', 'ai_co', 'dashboard']):
    ...
    def identify_optimization_targets(self, workers: List[WorkerProcess]) -> Dict[str, List[WorkerProcess]]:
        """最適化対象を特定"""
        targets = {
            "high_cpu": [],
            "high_memory": [],
        ...
        for worker in workers:
            ...
            # 重複プロセス
            similar_workers = [w for w in workers if w.name == worker.name and w.pid != worker.pid]
            if len(similar_workers) > 2:  # 同名プロセスが3個以上
                targets["redundant"].append(worker)
        
        # 終了候補（アイドル + 重複）
        targets["candidates_for_termination"] = list(set(targets["idle_long"] + targets["redundant"]))
    ...
    def terminate_worker(self, worker: WorkerProcess, force: bool = False) -> bool:
        ...
        try:
            ...
            cmdline = ' '.join(proc.cmdline()) if proc.cmdline() else ''
            if not force and any(critical in cmdline for critical in ['claude', 'dashboard_server']):
                logger.info(f"Skipping critical process {worker.name} (PID: {worker.pid})")
                return False
    ...
    def optimize_worker_distribution(self) -> Dict[str, any]:
        ...
        if metrics.total_workers > optimal_count:
            workers_to_terminate = targets["candidates_for_termination"][:metrics.total_workers - optimal_count]
            ...
            for worker in workers_to_terminate:
                if self.terminate_worker(worker):
                    ...
                    optimization_result["optimization_actions"].append(f"Terminated {worker.name} (PID: {worker.pid})")
        ...
        while self.running:
            try:
                ...
                if metrics.total_workers > self.max_workers or metrics.system_load > 85:
                    logger.info(f"Auto-optimization triggered: {metrics.total_workers} workers, {metrics.system_load}% system load")
    ...
    def _generate_recommendations(self, metrics: LoadMetrics, targets: Dict[str, List[WorkerProcess]]) -> List[str]:
        """最適化推奨事項を生成"""
        recommendations = []
        
        if metrics.total_workers > self.max_workers:
            recommendations.append(f"{metrics.total_workers - self.max_workers}個のワーカーを削減推奨")
...
def main():
    ...
    if status['optimization_targets']['termination_candidates'] > 0:
        ...
        print(f"✅ 最適化完了:")
        print(f"   ワーカー数: {result['before']['worker_count']} → {result['after']['worker_count']}")
        print(f"   システム負荷: {result['before']['system_load']:.1f}% → {result['after']['system_load']:.1f}%")
        print(f"   終了したワーカー: {len(result['terminated_workers'])}個")
        
        if result['terminated_workers']:
            print(f"\n🗑️ 終了したワーカー:")
            for worker in result['terminated_workers']:
                print(f"   • {worker['name']} (PID: {worker['pid']}) - {worker['reason']}")
    
    # レポート保存
    balancer.export_metrics_report("/home/aicompany/ai_co/ai_todo/worker_load_balancer_report.json")
    print(f"\n📄 詳細レポートを保存しました")
