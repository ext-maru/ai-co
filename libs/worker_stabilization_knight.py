...
from libs.incident_knights_framework import IncidentKnight, Issue, Diagnosis, Resolution, IssueCategory, IssueSeverity, KnightType
...
class WorkerStabilizationKnight(IncidentKnight):
    ...
    def __init__(self, knight_id: str = "worker_stabilization_001", specialty: str = "Worker system stability"):
    ...
    async def _check_worker_processes(self) -> List[Issue]:
        ...
        try:
            # psutilで実行中プロセスを確認
            running_processes = [p.info for p in psutil.process_iter(['pid', 'name', 'cmdline'])]
            ...
            for worker in self.target_workers:
                ...
                if not worker_running:
                    issues.append(Issue(
                        ...
                        severity=IssueSeverity.CRITICAL if worker['priority'] == 'critical' else IssueSeverity.HIGH,
                    ...
                    self.worker_issues.append(WorkerIssue(
                        worker_name=worker['name'],
                    ...
                    if config_path.suffix == '.json':
                        ...
                        if missing_keys:
                            issues.append(Issue(
                                ...
                                affected_component=str(config_path),
                                detected_at=datetime.now(),
                                metadata={"missing_keys": missing_keys, "auto_fixable": True}
                            ))
            ...
            if worker_path.exists():
                try:
                    ...
                    for line in import_lines:
                        try:
                            # 基本的なインポートチェック（簡易版）
                            if 'pika' in line and not self._check_module_available('pika'):
                                missing_modules.append('pika')
                            elif 'anthropic' in line and not self._check_module_available('anthropic'):
                                missing_modules.append('anthropic')
                            elif 'psutil' in line and not self._check_module_available('psutil'):
                ...
                try:
                    ...
                    for pattern, error_type in error_patterns:
                        if pattern.lower() in log_content.lower():
                            ...
                            if recent_pattern in log_content or len(log_content.split(pattern)) > 2:
                                issues.append(Issue(
        ...
        try:
            ...
            if memory.percent > 85:
                issues.append(Issue(
                    ...
                    affected_component="system_resources",
                    detected_at=datetime.now(),
                    metadata={"memory_percent": memory.percent, "auto_fixable": True}
                ))
