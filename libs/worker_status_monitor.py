...
class WorkerStatusMonitor:
    ...
    def update_worker_task_status(self, worker_id: str, status: str, task_info: Dict = None):
        """Worker のタスク処理状況を更新"""
    ...
    def record_worker_error(self, worker_id: str, error_info: Dict):
        ...
        logger.warning(f"Error recorded for {worker_id}: {error_info.get('error_type')}")
    ...
    def generate_dashboard_data(self) -> Dict:
        ...
        workers_summary = {
            ...
            'by_type': {wtype: len(workers) for wtype, workers in workers_by_type.items()}
        }
        ...
        for worker_type, workers in workers_by_type.items():
            if workers:
                avg_processing_time = sum(w['performance_metrics']['avg_processing_time'] for w in workers) / len(workers)
                avg_success_rate = sum(w['performance_metrics']['success_rate'] for w in workers) / len(workers)
    ...
    def auto_register_worker(self, worker_instance):
        ...
        try:
            worker_info = {
                'worker_id': getattr(worker_instance, 'worker_id', f'worker_{int(time.time())}'),
