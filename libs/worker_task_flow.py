...
class TaskFlowTracker:
    ...
    def track_task_completion(self, completion_info: Dict) -> bool:
        ...
        try:
            ...
            for stage in reversed(flow.workflow_stages):
                if (stage.worker_type == completion_info['worker_type'] and 
                    stage.status in ['processing', 'error']):
                    current_stage = stage
                    break
            ...
            current_stage.completed_at = completion_info.get('completed_at', datetime.now())
    ...
    def track_task_error(self, error_info: Dict) -> bool:
        ...
        try:
            ...
            # 現在のステージにエラー情報を設定
            for stage in reversed(flow.workflow_stages):
                if (stage.worker_type == error_info['worker_type'] and 
                    stage.status == 'processing'):
                    stage.status = 'error'
            ...
            logger.warning(f"Task error tracked: {task_id} - {error_info.get('error_type')}")
            return True
    ...
    def calculate_flow_metrics(self, task_id: str) -> Dict:
        ...
        try:
            ...
            # 基本メトリクス
            total_processing_time = sum(stage.get('processing_time', 0) for stage in stages)
            ...
            if total_stages > 1:
                ...
                variance = sum((t - avg_time) ** 2 for t in stage_times) / len(stage_times)
                efficiency = max(0, 1 - (variance / (avg_time ** 2)) if avg_time > 0 else 0)
            else:
                efficiency = 1.0
            ...
            for i, stage in enumerate(stages):
                stage_breakdown.append({
                    ...
                    'percentage_of_total': (stage.get('processing_time', 0) / total_processing_time * 100) if total_processing_time > 0 else 0
                })
            ...
            return {
                ...
                'bottleneck_stage': bottleneck_stage.get('worker_type') if bottleneck_stage else None,
    ...
    def get_worker_performance_in_flows(self, worker_id: str) -> Dict:
        ...
        try:
            ...
            all_flows = list(self.active_flows.values()) + list(self.completed_flows.values())
            ...
            # 最速・最遅タスク
            fastest_task = min(worker_tasks, key=lambda t: t['processing_time']) if worker_tasks else None
            slowest_task = max(worker_tasks, key=lambda t: t['processing_time']) if worker_tasks else None
    ...
    def analyze_flow_bottlenecks(self, task_id: str) -> Dict:
        ...
        try:
            ...
            if bottleneck_severity > 0.5:
                suggestions.append(f"{primary_bottleneck.get('worker_type')} の処理時間が全体の{bottleneck_severity*100:.1f}%を占めています")
    ...
    def sync_with_task_tracker(self, task_tracker_data: Dict) -> bool:
        ...
        try:
            ...
            stage = WorkflowStage(
                worker_type=task_info['worker_type'],
                worker_id=task_info['worker_id'],
                status='processing' if task_tracker_data.get('status') == 'in_progress' else 'pending',
            ...
            flow = TaskFlow(
                ...
                status='in_progress' if task_tracker_data.get('status') == 'in_progress' else 'pending',
                workflow_stages=[stage],
                source='task_tracker'
            )
    ...
    def _save_flow_to_db(self, flow: TaskFlow):
        ...
        try:
            ...
            """, (
                ...
                json.dumps([asdict(stage) for stage in flow.workflow_stages], default=str)
            ))
    ...
    def _save_error_to_db(self, task_id: str, error_record: Dict):
        ...
        try:
            ...
            """, (
                ...
                error_record['error_at'].isoformat() if isinstance(error_record['error_at'], datetime) else error_record['error_at'],
    ...
    def get_combined_dashboard_data(self):
        ...
        try:
            ...
            if 'workers_summary' in worker_data:
                ...
                worker_status = {
                    'workers_summary': {'total_workers': len(self.status_monitor.workers_status)},
                    'queue_status': {},
                    'system_health': {},
                    'performance_metrics': {}
                }
