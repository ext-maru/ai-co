...
class WorkflowController(BaseManager):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # インデックス作成
            conn.execute('CREATE INDEX IF NOT EXISTS idx_phase_states_project ON phase_states(project_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_workflow_events_project ON workflow_events(project_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_phase_deps_project ON phase_dependencies(project_id)')
    ...
    def initialize_project_workflow(self, project_id: str, custom_phases: Optional[List[str]] = None) -> bool:
        """プロジェクトワークフローの初期化"""
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                # 既存の状態をクリア
                conn.execute('DELETE FROM phase_states WHERE project_id = ?', (project_id,))
                
                # 各フェーズを初期化
                for i, phase in enumerate(phases):
                    status = PhaseStatus.READY.value if i == 0 else PhaseStatus.PENDING.value
    ...
    def check_phase_dependencies(self, project_id: str, target_phase: str) -> Tuple[bool, List[str]]:
        """フェーズ実行前提条件チェック"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                # 依存フェーズを取得
                ...
                logger.info(f"📋 フェーズ依存関係チェック ({target_phase}): {'✅ 実行可能' if can_execute else '❌ 依存未解決'}")
    ...
    def start_phase(self, project_id: str, phase: str, force: bool = False) -> bool:
        ...
        try:
            # 依存関係チェック
            if not force:
                can_execute, unmet_deps = self.check_phase_dependencies(project_id, phase)
                if not can_execute:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute('''
                    ...
                    WHERE project_id = ? AND phase = ?
                ''', (PhaseStatus.IN_PROGRESS.value, datetime.now(), datetime.now(), project_id, phase))
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute('''
                    ...
                    WHERE project_id = ? AND phase = ?
                ''', (PhaseStatus.COMPLETED.value, datetime.now(), datetime.now(), project_id, phase))
            
            # イベント記録
            self._record_workflow_event(project_id, WorkflowEvent.PHASE_COMPLETED, phase)
    ...
    def _generate_phase_tasks(self, project_id: str, phase: str):
        ...
        try:
            ...
            for i, template in enumerate(templates):
                ...
                subtasks = self.task_splitter.split_into_subtasks(task_id, task_description)
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor:
                    ...
                    # 条件評価
                    if self._evaluate_condition(project_id, condition_expr, completed_phase):
                        # 遅延がある場合は遅延実行
                        if delay_minutes > 0:
                            self._schedule_delayed_advance(project_id, to_phase, delay_minutes)
                        else:
                            # 即座に次フェーズ開始
    ...
    def auto_advance_phase(self, project_id: str, target_phase: str) -> bool:
        ...
        try:
            ...
            if quality_passed:
                ...
                if success:
                    # 自動進行イベント記録
                    self._record_workflow_event(project_id, WorkflowEvent.AUTO_ADVANCE, target_phase, {
    ...
    def _run_quality_gate_check(self, project_id: str, phase: str) -> bool:
        ...
        try:
            ...
            logger.info(f"📊 品質ゲート結果: {quality_score:.1f}% - {'✅ 通過' if passed else '❌ 不合格'}")
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute('''
                    ...
                    WHERE project_id = ? AND phase = ?
                ''', (PhaseStatus.BLOCKED.value, reason, datetime.now(), project_id, phase))
    ...
    def _evaluate_condition(self, project_id: str, condition_expr: str, phase: str) -> bool:
        """条件式評価"""
        try:
            # 基本的な条件評価
    ...
    def _schedule_delayed_advance(self, project_id: str, phase: str, delay_minutes: int):
        """遅延自動進行をスケジュール"""
    ...
    def _record_workflow_event(self, project_id: str, event_type: WorkflowEvent, 
                              phase: Optional[str] = None, event_data: Optional[Dict] = None):
        """ワークフローイベント記録"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute('''
    ...
    def get_project_workflow_status(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 全体進捗
                completed_phases = sum(1 for p in phases if p['status'] == PhaseStatus.COMPLETED.value)
                overall_progress = (completed_phases / len(phases)) * 100 if phases else 0
    ...
    def get_workflow_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # プロジェクト数
                cursor = conn.execute('SELECT COUNT(DISTINCT project_id) FROM phase_states')
...
if __name__ == "__main__":
    ...
    can_execute, unmet_deps = controller.check_phase_dependencies(test_project_id, "development")
    ...
    for phase in status.get('phases', []):
        print(f"  {phase['phase']}: {phase['status']} ({phase['progress_percentage']:.1f}%)")
