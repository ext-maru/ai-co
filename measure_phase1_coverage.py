...
def main():
    ...
    if successful_tests:
        avg_module_coverage = sum(r['coverage_percent'] for r in successful_tests) / len(successful_tests)
        print(f"Average Module Coverage: {avg_module_coverage:.1f}%")
    
    if overall_percent:
        print(f"Overall Project Coverage: {overall_percent:.1f}%")
        print(f"Coverage Improvement: {overall_percent - 2.21:.1f}% (from baseline 2.21%)")
