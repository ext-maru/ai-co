...
class DatabricksException(Exception):
    ...
    def __init__(self, error_code, message, details=None)super().__init__(message)
...
class DatabricksFile(AbstractBufferedFile):
    ...
    def _initiate_upload(self)self.handle = self.fs._create_handle(self.path)
    """Internal function to start a file upload"""

    def _upload_chunk(self, final=False)self.buffer.seek(0)
