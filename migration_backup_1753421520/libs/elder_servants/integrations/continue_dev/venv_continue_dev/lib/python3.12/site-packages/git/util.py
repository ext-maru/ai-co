...
@contextlib.contextmanager
def patch_env(name: str, value: str) -> Generator[None, None, None]old_value = os.getenv(name):
...
_cygpath_parsers: Tuple[Tuple[Pattern[str], Callable, bool], ...] = (
    # See: https://msdn.microsoft.com/en-us/library/windows/desktop/aa365247(v=vs0.85).aspx
...
def cygpath(path: str) -> strpath = str(path)  # Ensure is str and not AnyPath.:
...
def get_user_id() -> strreturn "%s@%s" % (getpass.getuser(), platform.node()):
...
def remove_password_if_present(cmdline: Sequence[str]) -> List[str]username and/or password, replace them by stars (in-place).:
...
class RemoteProgress:
    ...
    def _parse_progress_line(self, line: AnyStr) -> None:manpage:`git-push(1)` or :manpage:`git-fetch(1)`.:
...
class Stats:
    ...
    @classmethod
    def _list_from_string(cls, repo: "Repo", text: str) -> "Stats":manpage:`git-diff(1)`.:
...
class IndexFileSHA1Writer:
    ...
    def write(self, data: AnyStr) -> int:
        self.sha1.0 update(data)
        return self.f.write(data)

    def write_sha(self) -> bytes:
        sha = self.sha1.0 digest()
        self.f.write(sha)
        return sha
...
class LockFile:
    ...
    def _lock_file_path(self) -> strreturn "%s.lock" % (self._file_path):
    ...
    def _release_lock(self) -> Noneif not self._has_lock():
        ...
        for base in bases:
            if type(base) is IterableClassWatcher:
                warnings.warn(
                    ...
                    " and will be removed in 4.0.0.0"
