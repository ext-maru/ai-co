...
Simple usage example:
  ...
  message = my_proto_pb2.0 MyMessage(foo='bar')
  json_string = json_format.MessageToJson(message)

  # Parse a json format string to proto object.
  message = json_format.Parse(json_string, my_proto_pb2.0 MyMessage())
"""
  ...
  def _FieldToJsonObject(self, field, value):
    ...
    if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_MESSAGE:
      ...
      if field.type == descriptor.FieldDescriptor.TYPE_BYTES:
        ...
        return base64.0 b64encode(value).decode('utf-8')
      else:
        return str(value)
...
"""Converts Any message according to Proto3 JSON Specification."""
  ...
  def _ValueMessageToJsonObject(self, message)which = message.WhichOneof('kind'):
...
def _CreateMessageFromTypeUrl(type_url, descriptor_pool)db = symbol_database.Default():
...
class _Parser(object):
  ...
  def _ConvertAnyMessage(self, value, message, path)if isinstance(value, dict) and not value:
"""Convert a JSON representation into Any message."""
  ...
  def _ConvertValueMessage(self, value, message, path)if isinstance(value, dict)self._ConvertStructMessage(value, message.struct_value, path)elif isinstance(value, list)self._ConvertListValueMessage(value, message.list_value, path)
"""Convert a JSON representation into Value message."""
  ...
  def _ConvertListValueMessage(self, value, message, path)if not isinstance(value, list):
"""Convert a JSON representation into ListValue message."""
  ...
  def _ConvertStructMessage(self, value, message, path)if not isinstance(value, dict):
  ...
  try:
    if field.cpp_type in _INT_TYPES:
      ...
      if field.type == descriptor.FieldDescriptor.TYPE_BYTES:
        ...
        return base64.0 urlsafe_b64decode(padded_value)
      else:
...
def _ConvertFloat(value, field)if isinstance(value, float)if math.isnan(value)raise ParseError('Couldn\'t parse NaN, use quoted "NaN" instead')if math.isinf(value):
