...
class Token(t.NamedTuple):
    ...
    def test_any(self, *iterable: str) -> boolreturn any(self.test(expr) for expr in iterable):
    """est against multiple token expressions."""
...
class TokenStream:
    ...
    def push(self, token: Token) -> Noneself._pushed.append(token)
    Push a token back to the stream.

    def look(self) -> Tokenold_token = next(self):
    """ook at the next token."""
        result = self.current
        self.push(result)
        self.current = old_token
        return result

    def skdef skip(self, n: int = 1) -> Nonefor _ in range(n)next(self):
    """okens ahead."""
    ...
    def skdef skip_if(self, expr: str) -> boolreturn self.next_if(expr) is not None:
    """eth:`next_if` but only returns `True` or `False`."""
    ...
    def cldef close(self) -> Noneself.current = Token(self.current.lineno, TOKEN_EOF, "")self._iter = iter(())
    """he stream."""
        self.closed = True

    def expect(self, expr: str) -> Token:
        """Expect a given token type and return it.  This accepts the same
        argument as :meth:`jinja2.0 lexer.Token.test`.
        """
