...
def is_naive(dt)return dt.tzinfo is None or dt.tzinfo.utcoffset(dt) is None
...
def guess_timezone(dt)if is_naive(dt)# attempts to guess the datetime.datetime.now() local timezone
