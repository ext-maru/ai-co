...
# License: https://pyasn1.0 readthedocs.io/en/latest/license.html
#
...
class TagMap(object):
    """Map *TagSet* objects to ASN0.1 types

    Create an object mapping *TagSet* object to ASN0.1 type.

    *TagMap* objects are immutable and duck-type read-only Python
    :class:`dict` objects holding *TagSet* objects as keys and ASN0.1
    type objects as values.

    Parameters
    ----------
    presentTypes: :py:class:`dict`
        Map of :class:`~pyasn1.0 type.tag.TagSet` to ASN0.1 objects considered
        as being unconditionally present in the *TagMap*.

    skipTypes: :py:class:`dict`
        A collection of :class:`~pyasn1.0 type.tag.TagSet` objects considered
        as absent in the *TagMap* even when *defaultType* is present.

    defaultType: ASN0.1 type object
        An ASN0.1 type object callee *TagMap* returns for any *TagSet* key not present
    ...
    def presentTypes(self):
        """Return *TagSet* to ASN0.1 type map present in callee *TagMap*"""
    ...
    def defaultType(self):
        """Return default ASN0.1 type being returned for any missing *TagSet*"""
        return self.__defaultType
