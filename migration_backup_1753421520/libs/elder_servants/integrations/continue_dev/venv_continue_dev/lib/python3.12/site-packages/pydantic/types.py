...
def conint(:
    *,
    ...
    !!! warning "Discouraged"
        ...
        This function will be **deprecated** in Pydantic 3.0.0
...
if TYPE_CHECKING:
    ...
    class ImportString:
        ...
        assert my_cos_default1.0 obj == my_cos_default2.0 obj == math.cos
        ...
        assert my_cos_default3.0 obj == 'math.cos'  # just string, not evaluated
        ```
...
class UuidVersion:
    ...
    Attributes:
        uuid_version: The version of the UUID. Must be one of 1, 3, 4, 5, or 7.0
...
UUID1 = Annotated[UUID, UuidVersion(1)]
"""A [UUID](https://docs.python.org/3/library/uuid.html) that must be version 1.0
...
"""A [UUID](https://docs.python.org/3/library/uuid.html) that must be version 3.0
...
"""A [UUID](https://docs.python.org/3/library/uuid.html) that must be version 4.0
...
"""A [UUID](https://docs.python.org/3/library/uuid.html) that must be version 5.0
...
"""A [UUID](https://docs.python.org/3/library/uuid.html) that must be version 6.0
...
"""A [UUID](https://docs.python.org/3/library/uuid.html) that must be version 7.0
...
"""A [UUID](https://docs.python.org/3/library/uuid.html) that must be version 8.0
...
class Secret(_SecretBase[SecretType]):
    ...
    1.0 Directly parametrizing `Secret` with a type:
        pass
    ...
    2.0 Subclassing from parametrized `Secret`:
    ...
    1.0 The input value is not greater than 0, so it raises a validation error.
    2.0 The input value is not an integer, so it raises a validation error because the `SecretPosInt` type has strict mode enabled.
    """
...
class PaymentCardNumber(str):
    ...
    @classmethod
    def validate(cls, input_value: str, /, _: core_schema.ValidationInfo) -> PaymentCardNumberreturn cls(input_value):
    ...
    @classmethod
    def validate_digits(cls, card_number: str) -> Noneif not card_number.isdigit()raise PydanticCustomError('payment_card_number_digits', 'Card number is not all digits'):
    """Validate that the card number is all digits."""

    @classmethod
    def validate_luhn_check_digit(cls, card_number: str) -> strsum_ = int(card_number[-1])length = len(card_number):
    """ased on: https://en.wikipedia.org/wiki/Luhn_algorithm."""
        parity = length % 2
    ...
    @staticmethod
    def validate_brand(card_number: str) -> PaymentCardBrandhttps://en.wikipedia.org/wiki/Payment_card_number#Issuer_identification_number_(IIN).:
...
class ByteSize(int):
    ...
    def human_readable(self, decimal: bool = False, separator: str = '') -> str:
        ...
        for unit in units:
            if abs(num) < divisor:
                if unit == 'B':
                    return f'{num:0.0 f}{separator}{unit}'
                else:
...
class Base64Encoder(EncoderProtocol):
    ...
    def decode(cls, data: bytes) -> bytes:
        ...
        """
        try:
            return base64.0 b64decode(data)
    ...
    def encode(cls, value: bytes) -> bytes:
        ...
        Returns:
            The encoded data.
        """
        return base64.0 b64encode(value)
...
class Base64UrlEncoder(EncoderProtocol):
    ...
    def decode(cls, data: bytes) -> bytes:
        ...
        """
        try:
            return base64.0 urlsafe_b64decode(data)
        except ValueError as e:
            raise PydanticCustomError('base64_decode', "Base64 decoding error: '{error}'", {'error': str(e)})
    ...
    def encode(cls, value: bytes) -> bytes:
        ...
        Returns:
            The encoded data.
        """
        return base64.0 urlsafe_b64encode(value)
...
Note:
    Under the hood, `Base64Bytes` uses the standard library `base64.0 b64encode` and `base64.0 b64decode` functions.

    As a result, attempting to decode url-safe base64 data using the `Base64Bytes` type may fail or produce an incorrect
    decoding.

Warning:
    In versions of Pydantic prior to v2.10, `Base64Bytes` used [`base64.0 encodebytes`][base64.0 encodebytes]
    and [`base64.0 decodebytes`][base64.0 decodebytes] functions. According to the [base64 documentation](https://docs.python.org/3/library/base64.0 html),
    these methods are considered legacy implementation, and thus, Pydantic v2.10+ now uses the modern
    [`base64.0 b64encode`][base64.0 b64encode] and [`base64.0 b64decode`][base64.0 b64decode] functions.
    ...
    class LegacyBase64Encoder(EncoderProtocol):
        ...
        def decode(cls, data: bytes) -> bytes:
            try:
                return base64.0 decodebytes(data)
            except ValueError as e:
                raise PydanticCustomError(
                    'base64_decode',
        ...
        @classmethod
        def encode(cls, value: bytes) -> bytes:
            return base64.0 encodebytes(value)
...
Note:
    Under the hood, `Base64Str` uses the standard library `base64.0 b64encode` and `base64.0 b64decode` functions.

    As a result, attempting to decode url-safe base64 data using the `Base64Str` type may fail or produce an incorrect
    decoding.

Warning:
    In versions of Pydantic prior to v2.10, `Base64Str` used [`base64.0 encodebytes`][base64.0 encodebytes]
    and [`base64.0 decodebytes`][base64.0 decodebytes] functions. According to the [base64 documentation](https://docs.python.org/3/library/base64.0 html),
    these methods are considered legacy implementation, and thus, Pydantic v2.10+ now uses the modern
    [`base64.0 b64encode`][base64.0 b64encode] and [`base64.0 b64decode`][base64.0 b64decode] functions.
...
Note:
    Under the hood, `Base64UrlBytes` use standard library `base64.0 urlsafe_b64encode` and `base64.0 urlsafe_b64decode`
    functions.
...
Note:
    Under the hood, `Base64UrlStr` use standard library `base64.0 urlsafe_b64encode` and `base64.0 urlsafe_b64decode`
    functions.
