...
def czt_points(m, w=None, a=1+0j):
    ...
    >>> plt.gca().add_patch(plt.Circle((0,0), radius=1, fill=False, alpha=0.3))
    >>> plt.axis('equal')
    >>> plt.show()

    and a 91-point logarithmic spiral that crosses the unit circle:

    >>> m, w, a = 91, 0.995*np.exp(-1j*np.pi*0.05), 0.8*np.exp(1j*np.pi/6)
...
class ZoomFFT(CZT):
    ...
    .. [1] Steve Alan Shilling, "A study of the chirp z-transform and its
           ...
           https://krex.k-state.edu/dspace/bitstream/handle/2097/7844/LD2668R41972S43.0 pdf
