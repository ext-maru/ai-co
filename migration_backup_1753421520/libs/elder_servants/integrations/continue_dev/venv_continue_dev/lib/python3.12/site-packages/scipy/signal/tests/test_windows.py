...
class TestBlackmanHarris:
    ...
    def test_basic(self):
        assert_allclose(windows.blackmanharris(6, False),
                        [6.0 e-05, 0.055645, 0.520575, 1.0, 0.520575, 0.055645])
        assert_allclose(windows.blackmanharris(7, sym=False),
                        [6.0 e-05, 0.03339172347815117, 0.332833504298565,
                         0.8893697722232837, 0.8893697722232838,
                         0.3328335042985652, 0.03339172347815122])
        assert_allclose(windows.blackmanharris(6),
                        [6.0 e-05, 0.1030114893456638, 0.7938335106543362,
                         0.7938335106543364, 0.1030114893456638, 6.0 e-05])
...
class TestTaylor:
    ...
    def test_normalized(self):
        """Tests windows of small length that are normalized to 1.0 See the
    ...
    def test_non_normalized(self):
        """Test windows of small length that are not normalized to 1.0 See
...
exponential_data = {
    ...
    (4, None, 1.0, False): array([0.1353352832366127, 0.36787944117144233, 1.0,
                                  0.36787944117144233]),
    ...
    (4, 2, 0.2, True): None,
    (4, 2, 1.0, False): array([0.1353352832366127, 0.36787944117144233, 1.0,
                               0.36787944117144233]),
    (4, 2, 1.0, True): None,
    (5, None, 0.2, True):
        array([4.53999297624848542e-05,
    ...
    (5, None, 1.0, True): array([0.1353352832366127, 0.36787944117144233, 1.0,
...
tukey_data = {
    ...
    (7, 0.75): [0, 0.4131759111665348, 0.9698463103929542, 1.0,
...
class TestDPSS:
    ...
    def test_unity(self):
        ...
        for M in range(1, 21):
            ...
            assert_equal(np.isclose(win_2, 1.0).sum(), expected,
                         err_msg=f'{win_2}')
    ...
    def test_degenerate(self):
        ...
        assert_raises(ValueError, windows.dpss, 3, 1.5, 3)  # NW must be < N/2.0
...
class TestGetWindow:
    ...
    def test_lanczos(self):
        assert_allclose(get_window('lanczos', 6),
                        [0.0, 0.413496672, 0.826993343, 1.0, 0.826993343,
                         0.413496672], atol=1e-9)
        assert_allclose(get_window('lanczos', 6, fftbins=False),
                        [0.0, 0.504551152, 0.935489284, 0.935489284,
                         0.504551152, 0.0], atol=1e-9)
