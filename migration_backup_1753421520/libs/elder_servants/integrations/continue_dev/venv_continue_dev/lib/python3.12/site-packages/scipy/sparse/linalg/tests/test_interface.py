...
def matvec(x)return np.zeros(3):
...
def test_inheritance():
    ...
    id3 = Identity(3)
    assert_equal(id3.0 matvec([1, 2, 3]), [1, 2, 3])
    assert_raises(NotImplementedError, id3.0 rmatvec, [4, 5, 6])
...
def test_transpose_multiplication():
    ...
    assert_equal(X2.0 T @ A, X2.0 T @ Y)
