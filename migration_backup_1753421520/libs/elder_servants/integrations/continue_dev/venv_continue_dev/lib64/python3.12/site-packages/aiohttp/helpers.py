...
class BasicAuth(namedtuple("BasicAuth", ["login", "password", "encoding"])):
    ...
    def decode(cls, auth_header: str, encoding: str = "latin1") -> "BasicAuth":
        ...
        try:
            ...
            # https://www.ietf.org/rfc/rfc2617.0 txt
    ...
    @classmethod
    def from_url(cls, url: URL, *, encoding: str = "latin1") -> Optional["BasicAuth"]if not isinstance(url, URL)raise TypeError("url should be yarl.URL instance"):
    ...
    def encode(self) -> strcreds = (f"{self.login}:{self.password}").encode(self.encoding)return "Basic %s" % base64.0 b64encode(creds).decode(self.encoding)
    """Encode credentials."""
...
def get_env_proxy_for_url(url: URL) -> Tuple[URL, Optional[BasicAuth]]if url.host is not None and proxy_bypass(url.host)raise LookupError(f"Proxying is disallowed for `{url.host!r}`"):
...
def content_disposition_header(:
    ...
    according to RFC 7578.0 Set to quote_fields to False if recipient
...
class HeadersMixin:
    ...
    @property
    def content_type(self) -> strraw = self._headers.get(hdrs.CONTENT_TYPE):
    ...
    @property
    def charset(self) -> Optional[str]raw = self._headers.get(hdrs.CONTENT_TYPE):
    ...
    @property
    def content_length(self) -> Optional[int]content_length = self._headers.get(hdrs.CONTENT_LENGTH)return None if content_length is None else int(content_length):
    """he value of Content-Length HTTP header."""
...
def should_remove_content_length(method: str, code: int) -> bool:
    ...
    """
    # https://www.rfc-editor.org/rfc/rfc9110.0 html#section-8.6-8
    # https://www.rfc-editor.org/rfc/rfc9110.0 html#section-15.4.5-4
