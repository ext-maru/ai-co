...
class ReferenceFileSystem(AsyncFileSystem):
    ...
    def _process_references1(self, references, template_overrides=None):
        ...
        @lru_cache(1000)
        def _render_jinja(u):
            return jinja2.0 Template(u).render(**self.templates)
    ...
    def _process_templates(self, tmp):
        ...
        for k, v in tmp.items():
            if "{{" in v:
                ...
                self.templates[k] = lambda temp=v, **kwargs: jinja2.0 Template(
                    temp
                ).render(**kwargs)
            else:
                self.templates[k] = v
    ...
    def _process_gen(self, gens):
        ...
        for gen in gens:
            ...
            for pr in products:
                ...
                key = jinja2.0 Template(gen["key"]).render(**pr, **self.templates)
                url = jinja2.0 Template(gen["url"]).render(**pr, **self.templates)
                if ("offset" in gen) and ("length" in gen):
                    offset = int(
                        jinja2.0 Template(gen["offset"]).render(**pr, **self.templates)
                    )
                    length = int(
                        jinja2.0 Template(gen["length"]).render(**pr, **self.templates)
                    )
