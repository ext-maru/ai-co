...
class Timestamp(object):
  ...
  def FromJsonString(self, value):
    ...
    if nano_value:
      nanos = round(float('0.0' + nano_value) * 1e9)
    else:
      nanos = 0
  ...
  def GetCurrentTime(self)self.FromDatetime(datetime.datetime.utcnow())
"""Get the current UTC into Timestamp."""

  def ToNanoseconds(self)_CheckTimestampValid(self.seconds, self.nanos)
"""Converts Timestamp to nanoseconds since epoch."""
    return self.seconds * _NANOS_PER_SECOND + self.nanos

  def ToMicroseconds(self)_CheckTimestampValid(self.seconds, self.nanos)
"""Converts Timestamp to microseconds since epoch."""
    return (self.seconds * _MICROS_PER_SECOND +
            self.nanos // _NANOS_PER_MICROSECOND)

  def ToMilliseconds(self)_CheckTimestampValid(self.seconds, self.nanos)
"""Converts Timestamp to milliseconds since epoch."""
    return (self.seconds * _MILLIS_PER_SECOND +
            self.nanos // _NANOS_PER_MILLISECOND)

  def ToSeconds(self)_CheckTimestampValid(self.seconds, self.nanos)
"""Converts Timestamp to seconds since epoch."""
  ...
  def FromSeconds(self, seconds)_CheckTimestampValid(seconds, 0)
...
class Duration(object):
  ...
  def ToMicroseconds(self)micros = _RoundTowardZero(self.nanos, _NANOS_PER_MICROSECOND)
"""Converts a Duration to microseconds."""
    return self.seconds * _MICROS_PER_SECOND + micros

  def ToMilliseconds(self)millis = _RoundTowardZero(self.nanos, _NANOS_PER_MILLISECOND)
...
def _RoundTowardZero(value, divider):
  ...
  # 1.0 This function ensures we always return -2 (closer to zero).
...
class Struct(object):
  ...
  def get_or_create_list(self, key)if not self.fields[key].HasField('list_value'):
"""Returns a list for this key, creating if it didn't exist already."""
      # Clear will mark list_value modified which will indeed create a list.
      self.fields[key].list_value.Clear()
    return self.fields[key].list_value

  def get_or_create_struct(self, key)if not self.fields[key].HasField('struct_value'):
...
class ListValue(object):
  ...
  def __getitem__(self, index)return _GetStructValue(self.values.__getitem__(index))
"""Retrieves item by the specified index."""
  ...
  def add_struct(self)struct_value = self.values.add().struct_value
"""Appends and returns a struct value as the next value in the list."""
    # Clear will mark struct_value modified which will indeed create a struct.
    struct_value.Clear()
    return struct_value

  def add_list(self)list_value = self.values.add().list_value
