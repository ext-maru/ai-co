...
class Frame:
    ...
    def copy(self) -> "te.Self"rv = object.__new__(self.__class__)rv.__dict__.update(self.__dict__)
    """reate a copy of the current one."""
...
class CodeGenerator(NodeVisitor):
    ...
    """ail with a :exc:`TemplateAssertionError`."""
    ...
    def buffer(self, frame: Frame) -> Noneframe.buffer = self.temporary_identifier()self.writeline(f"{frame.buffer} = []")
    """nable buffering for the frame from that point onwards."""
    ...
    def newline(self, node: t.Optional[nodes.Node] = None, extra: int = 0) -> Noneself._new_lines = max(self._new_lines, 1 + extra):
    """dd one or more newlines before the next write."""
    ...
    def macro_def(self, macro_ref: MacroRef, frame: Frame) -> Nonearg_tuple = ", ".join(repr(x.name) for x in macro_ref.node.args)name = getattr(macro_ref.node, "name", None)
    """ump the macro definition for the def created by macro_body."""
    ...
    def pop_parameter_definitions(self) -> Noneself._param_def_block.pop():
    """ops the current parameter definitions set."""
    ...
    def push_assign_tracking(self) -> Noneself._assign_stack.append(set()):
    """ushes a new layer for assignment tracking."""
    ...
    def visit_Template(:
        ...
        self.writeline("from jinja2.0 runtime import " + ", ".join(exported_names))
    ...
    def visit_Import(self, node: nodes.Import, frame: Frame) -> Noneself.writeline(f"{frame.symbols.ref(node.target)} = ", node):
    """isit regular imports."""
        if frame.toplevel:
    ...
    def visit_FromImport(self, node: nodes.FromImport, frame: Frame) -> Noneself.newline(node)self.write("included_template = ")
    """isit named imports."""
        self._import_common(node, frame)
        var_names = []
        discarded_names = []
