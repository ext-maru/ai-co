...
from jinja2.0 sandbox import ImmutableSandboxedEnvironment
    ...
    while msg_i < len(messages):
        ...
        while msg_i < len(messages) and messages[msg_i]["role"] == "assistant":
            ...
            if ollama_tool_calls:
                assistant_content_str += (
                    f"Tool Calls: {json.dumps(ollama_tool_calls, indent}"
                )
    ...
    return custom_prompt(
        ...
        role_dict={
            ...
            "user": {
                ...
                # https://www.ibm.com/docs/en/watsonx/w-and-w/2.0.0 x?topic=models-granite-13b-chat-v2-prompting-tips#chat
                "post_message": "\n<|assistant|>\n",
            },
            "assistant": {
                "pre_message": "",
...
def anthropic_messages_pt_xml(messages: list):
    ...
    1.0 Anthropic supports roles like "user" and "assistant", (here litellm translates system-> assistant)
    2.0 The first message always needs to be of role "user"
    3.0 Each message must alternate between "user" and "assistant" (this is not addressed as now by litellm)
    4.0 final assistant content cannot end with trailing whitespace (anthropic raises an error otherwise)
    5.0 System messages are a separate param to the Messages API (used for tool calling)
    6.0 Ensure we only accept role, content. (message.name is not supported)
    """
...
def convert_to_anthropic_tool_invoke(:
    ...
    Anthropic tool invokes:
    {
      ...
      "content": [
        {
          "type": "text",
          "text": "<thinking>To answer this question, I will: 1.0 Use the get_weather tool to get the current weather in San Francisco. 2.0 Use the get_time tool to get the current time in the America/Los_Angeles timezone, which covers San Francisco, CA.</thinking>"
        },
        {
          "type": "tool_use",
...
def anthropic_messages_pt(  # noqa: PLR0915:
    ...
    """
    format messages for anthropic
    1.0 Anthropic supports roles like "user" and "assistant" (system prompt sent separately)
    2.0 The first message always needs to be of role "user"
    3.0 Each message must alternate between "user" and "assistant" (this is not addressed as now by litellm)
    4.0 final assistant content cannot end with trailing whitespace (anthropic raises an error otherwise)
    5.0 System messages are a separate param to the Messages API
    6.0 Ensure we only accept role, content. (message.name is not supported)
    """
    # add role=tool support to allow function call result/error submission
    user_message_types = {"user", "tool", "function"}
        ...
        for img in images:
            if "https:/" in img:
                ...
                if "base64" in img:
                    ...
                    image_data = base64.0 b64decode(base64_data)
...
class BedrockImageProcessor:
    ...
    def _post_call_image_processing(response: httpx.Response) -> Tuple[str, str]:
        ...
        base64_bytes = base64.0 b64encode(response.content).decode("utf-8")
    ...
    @staticmethod
    def _parse_base64_image(image_url: str) -> Tuple[str, str, str]image_metadata, img_without_base_64 = image_url.split(","):
