...
async def _check_user_exists(user_id: str)prisma_client = await _get_prisma_client_or_raise_exception()
...
async def _check_team_exists(team_id: str)prisma_client = await _get_prisma_client_or_raise_exception()
...
async def _extract_group_member_ids(group: SCIMGroup) -> List[str]prisma_client = await _get_prisma_client_or_raise_exception():
    """xtract valid member IDs from SCIMGroup, verifying users exist."""
    member_ids = []
...
asdef _get_team_members_display(member_ids: List[str]) -> List[SCIMMember]prisma_client = await _get_prisma_client_or_raise_exception():
    ...
    return members
def _handle_team_membership_changes(user_id: str, existing_teams: List[str], new_teams: List[str]) -> Noneexisting_teams_set = set(existing_teams):
    """ms)
...
def _handle_name_update(path: str, op_type: str, value: Any, scim_metadata: Dict[str, Any]) -> None:
    ...
    if path == "name.givenname":
        ...
        if op_type == "remove":
            ...
            scim_metadata["familyName"] = sdef _handle_group_operations(op_type: str, value: Any, teams_set: Set[str]) -> Optional[Set[str]]group_values = _extract_group_values(value):
    """(value)
