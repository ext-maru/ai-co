...
class Scheduler:
    ...
    async def peek(self, id: str, model_name: str, health_deployments: list) -> boolqueue = await self.get_queue(model_name=model_name):
    """eturn if the id is at the top of the queue. Don't pop the value from heap."""
        if not queue:
            raise Exception(
                "Incorrectly setup. Queue is invalid. Queue={}".format(queue)
            )
