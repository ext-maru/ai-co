...
def test_gnp_generators_negative_edge_probability(generator, directed)G = generator(10, -1.1, directed=directed)
...
@pytest.mark.parametrize("generator", _gnp_generators)
@pytest.mark.parametrize("directed", (True, False))
def test_gnp_generators_basic(generator, directed)G = generator(10, 0.1, directed=directed)
    ...
    for v in range(n):
        for w in range(n):
            if v == w:
                ...
                assert abs(edge_counts[v][w] / float(runs) - p) <= 0.3
...
def test_gnp_random_graph_aliases(generator, seed, directed, expected_num_edges)G = generator(100, 0.25, seed=seed, directed=directed)
