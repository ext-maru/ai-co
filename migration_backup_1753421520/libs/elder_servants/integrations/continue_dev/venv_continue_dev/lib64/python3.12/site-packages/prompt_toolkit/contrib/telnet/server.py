...
class TelnetConnection:
    ...
    def __init__(:
        ...
        def data_received(data: bytes) -> Noneself.vt100_input.send_bytes(data):
    """elnetProtocolParser 'data_received' callback"""

        def size_received(rows: int, columns: int) -> Noneself.size = Size(rows=rows, columns=columns):
    """elnetProtocolParser 'size_received' callback"""
    ...
    def feed(self, data: bytes) -> NoneHandler for incoming data. (Called by TelnetServer.):
...
class TelnetServer:
    ...
    def start(self) -> NoneDeprecated: Use `.run()` instead.
    """
    ...
    async def stop(self) -> NoneDeprecated: Use `.run()` instead.
    """
