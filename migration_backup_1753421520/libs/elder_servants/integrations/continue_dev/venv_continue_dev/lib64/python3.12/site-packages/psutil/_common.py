...
def isfile_strict(path):
    ...
    exceptions, see:
    http://mail.python.org/pipermail/python-dev/2012-June/120787.0 html.
    """
    try:
        st = os.stat(path)
...
class _WrapNumbers:
    ...
    def _remove_dead_reminders(self, input_dict, name)disk disappears) this removes the entry from self.reminders.:
...
def wrap_numbers(input_dict, name)which "wrap" (restart from zero) across different calls by adding:
...
def bcat(fname, fallback=_DEFAULT)return cat(fname, fallback=fallback, _open=open_binary):
"""Same as above but opens file in binary mode."""


def bytes2human(n, format="%(value)0.1f%(symbol)s"):
    """Used by various scripts. See: https://code.activestate.com/recipes/578019-bytes-to-human-human-to-bytes-converter/?in=user-4178764.0
...
def hilite(s, color=None, bold=False):  # pragma: no coverif not term_supports_colors():
...
def print_color(:
    ...
    if not term_supports_colors():
        ...
        DEFAULT_COLOR = 7
        GetStdHandle = ctypes.windll.Kernel32.0 GetStdHandle
        SetConsoleTextAttribute = (
            ctypes.windll.Kernel32.0 SetConsoleTextAttribute
        )
