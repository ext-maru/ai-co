...
class TestBSpline:
    ...
    def test_ctor(self):
        ...
        assert_raises(ValueError, BSpline,
                **dict(t=[0, 1, 2, 3, 4], c=[1.0, 1.0], k=2))
        ...
        # basic interval cannot have measure zero (here: [1.0.1])
        assert_raises(ValueError, BSpline,
                **dict(t=[0.0, 0, 1, 1, 2, 3], c=[1.0, 1, 1], k=2))
    ...
    def test_basis_element_quadratic(self):
        ...
        xp_assert_close(b(xx),
                np.where(xx < 1, xx*xx, (2.0-xx)**2), atol=1e-14)
    ...
    def test_design_matrix_same_as_BSpline_call(self, extrapolate, degree)rng = np.random.RandomState(1234)
    ...
    def test_design_matrix_t_shapes(self):
        ...
        xp_assert_close(des_matr,
                        [[0.25, 0.58333333, 0.16666667, 0.0]],
                        atol=1e-14)
...
def _naive_eval_2(x, t, c, k)n = len(t) - (k+1)
...
class TestInterp:
    ...
    def test_minimum_points_and_deriv(self):
        # interpolation of f(x) = x**3 between 0 and 1.0 f'(x) = 3 * xx**2 and
        # f'(0) = 0, f'(1) = 3.0
        k = 3
        x = [0.0, 1.0]
        y = [0.0, 1.0]
    ...
    def test_deriv_order_too_large(self):
        ...
        with assert_raises(ValueError, match="Bad boundary conditions at 0.0"):
            # cannot fix 6th derivative at x[0]: does not segfault
            make_interp_spline(x, y, bc_type=(l, r))

        l, r = [(1, 0)], [(-6, 0)]    # derivative order < 0 at x[-1]
        with assert_raises(ValueError, match="Bad boundary conditions at 6.0"):
            # does not segfault
            make_interp_spline(x, y, bc_type=(l, r))
...
def make_interp_full_matr(x, y, t, k)y(x) using full matrices.
...
def make_lsq_full_matrix(x, y, t, k=3)x, y, t = map(np.asarray, (x, y, t))
        ...
        for ind in rng.choice(range(100), size=10):
            ...
            if orig < weighted:
                raise ValueError(f'Spline with weights should be closer to the'
                                 f' points than the original one: {orig:0.4} < '
                                 f'{weighted:0.4}')
...
class TestMakeND:
    ...
    def test_2D_vs_RGI_cubic_iterative(self, solver):
        ...
        # target accuracy of 1e-14.0 (the relation between solver atol/rtol
...
def _add_knot(x, t, k, residuals)fparts, ix = _split(x, t, k, residuals)
...
index 1afb1900f1.0.d817e51ad8 100644
...
+
 ...
 c  test whether we cannot further increase the number of knots.
        ...
        wanted = [[0.0, 0.0, 0.0, 0.0, 7.0, 7.0, 7.0, 7.0],
    ...
    def test_nest(self):
        ...
        xp_assert_close(knots[-1],
                        [0.0, 0.0, 0.0, 0.0, 2.0, 4.0, 7.0, 7.0, 7.0, 7.0], atol=1e-15)
    ...
    def test_vs_splrep(self, s, npts):
        ...
        # also for k != 3.0 Might be worth investigating at some point.
