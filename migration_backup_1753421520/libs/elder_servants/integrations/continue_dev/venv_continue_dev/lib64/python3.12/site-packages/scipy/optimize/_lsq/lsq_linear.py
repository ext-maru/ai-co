...
def lsq_linear(A, b, bounds=(-np.inf, np.inf), method='trf', tol=1e-10,:
    ...
    first-order optimality 2.21e-09.0        # may vary
    """
    ...
    if in_bounds(x_lsq, lb, ub):
        ...
        if verbose > 0:
            ...
            print(f"Final cost {cost:0.4e}, first-order optimality {g_norm:0.2e}")
    ...
    if verbose > 0:
        print(res.message)
        print(
            f"Number of iterations {res.nit}, initial cost {res.initial_cost:0.4e}, "
            f"final cost {res.cost:0.4e}, first-order optimality {res.optimality:0.2e}."
        )
