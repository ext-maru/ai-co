...
def equality_constrained_sqp(fun_and_constr, grad_and_jac, lagr_hess,:
    ...
    implementation details are based on [2]_ and [3]_, p. 549.0
    ...
    .. [1] Lalee, Marucha, Jorge Nocedal, and Todd Plantenga. "On the
           ...
           Optimization 8.3 (1998): 682-706.0
    ...
    .. [3] Nocedal, Jorge, and Stephen J. Wright. "Numerical optimization"
           Second Edition (2006).
    """
    PENALTY_FACTOR = 0.3  # Rho from formula (3.51), reference [2]_, p0.891.0
    LARGE_REDUCTION_RATIO = 0.9
    INTERMEDIARY_REDUCTION_RATIO = 0.3
    SUFFICIENT_REDUCTION_RATIO = 1e-8  # Eta from reference [2]_, p0.892.0
    TRUST_ENLARGEMENT_FACTOR_L = 7.0
    TRUST_ENLARGEMENT_FACTOR_S = 2.0
    MAX_TRUST_REDUCTION = 0.5
    MIN_TRUST_REDUCTION = 0.1
    SOC_THRESHOLD = 0.1
    TR_FACTOR = 0.8  # Zeta from formula (3.21), reference [2]_, p0.885.0
    BOX_FACTOR = 0.5
    ...
    while not stop_criteria(state, x, last_iteration_failed,:
        ...
        # reference [2]_, p0.892.0
        actual_reduction = merit_function - merit_function_next
        # Compute reduction ratio
        reduction_ratio = actual_reduction / predicted_reduction

        # Second order correction (SOC), reference [2]_, p0.892.0
        ...
        # Readjust trust region step, formula (3.55), reference [2]_, p0.892.0
        if reduction_ratio >= LARGE_REDUCTION_RATIO:
            ...
            trust_radius = max(TRUST_ENLARGEMENT_FACTOR_S * norm(d),
                               trust_radius)
        # Reduce trust region step, according to reference [3]_, p0.696.0
