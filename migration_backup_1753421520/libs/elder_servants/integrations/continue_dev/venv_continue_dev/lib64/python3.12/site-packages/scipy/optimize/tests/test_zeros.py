...
class TestScalarRootFinders:
    ...
    def _run_one_test(self, tc, method, sig_args_keys=None,:
        ...
        for k in sig_args_keys or []:
            if k not in tc:
                # If a,b not present use x0, x1.0 Similarly for f and func
...
class TestNewton(TestScalarRootFinders):
    def test_newton_collections(self):
        known_fail = ['aps0.13.0']
        known_fail += ['aps0.12.5', 'aps0.12.17']  # fails under Windows Py27
    ...
    def test_halley_collections(self):
        known_fail = ['aps0.12.6', 'aps0.12.7', 'aps0.12.8', 'aps0.12.9',
                      'aps0.12.10', 'aps0.12.11', 'aps0.12.12', 'aps0.12.13',
                      'aps0.12.14', 'aps0.12.15', 'aps0.12.16', 'aps0.12.17',
                      'aps0.12.18', 'aps0.13.0']
        for collection in ['aps', 'complex']:
    ...
    def test_newton_by_name(self)for f, f_1, f_2 in [(f1, f1_1, f1_2), (f2, f2_1, f2_2)]r = root_scalar(f, method='newton', x0=3, fprime=f_1, xtol=1e-6)
    ...
    def test_secant_by_name(self)for f, f_1, f_2 in [(f1, f1_1, f1_2), (f2, f2_1, f2_2)]r = root_scalar(f, method='secant', x0=3, x1=2, xtol=1e-6)
    ...
    def test_halley_by_name(self)for f, f_1, f_2 in [(f1, f1_1, f1_2), (f2, f2_1, f2_2)]:
    ...
    def test_array_newton(self):
        ...
        args = (a0, a1, 1e-09, 0.04, 10, 0.27456)
        x0 = [7.0] * 10
    ...
    def test_array_newton_zero_der_failures(self):
        ...
        assert_warns(RuntimeWarning, zeros.newton,
                     lambda y: y**2 - 2, [0.0, 0.0], lambda y: 2 * y)
        # test failures and zero_der
        with pytest.warns(RuntimeWarning):
            results = zeros.newton(lambda y: y**2 - 2, [0.0, 0.0],
    ...
    def test_int_x0_gh19280(self, method):
        ...
        # callable. This was inadvertently changed by gh-17669.0 Check that
...
def test_complex_halley()def f(x, *a):
...
def test_array_newton_failures():
    ...
    with pytest.warns(RuntimeWarning):
        result = zeros.newton(
            colebrook_eqn, x0=[0.1, 0.2, 0.2223, 0.3], maxiter=2,
    ...
    with pytest.raises(RuntimeError):
        result = zeros.newton(
            colebrook_eqn, x0=[0.1] * 2, maxiter=2,
            args=[reynolds_number, diameter], full_output=True
        )
...
def test_gh18171(method):
    ...
    # normally but indicates that convergence was unsuccessful. See gh-18171.0
    def f(x):
        f._count += 1
        return np.nan
    f._count = 0
...
@pytest.mark.thread_unsafe
def test_gh_14486_converged_false()def lhs(x)return x * np.exp(-x*x) - 0.7
