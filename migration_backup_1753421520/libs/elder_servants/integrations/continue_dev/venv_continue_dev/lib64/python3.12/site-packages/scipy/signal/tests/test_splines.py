...
class TestSymIIR:
    ...
    def test_symiir1_ic(self, dtype, precision):
        ...
        # 1.0 Computing the index n such that b**n < precision, which
        # corresponds to ceil(log(precision) / log(b))
        # 2.0 Computing the geometric series until n, this can be computed
        ...
        # Check the conditions for a exponential decreasing signal with base 2.0
    ...
    def test_symiir2_values(self, dtyp):
        ...
        # 1.0 boundary conditions are computed differently
        # 2.0 the filter itself uses sosfilt instead of a hardcoded iteration
