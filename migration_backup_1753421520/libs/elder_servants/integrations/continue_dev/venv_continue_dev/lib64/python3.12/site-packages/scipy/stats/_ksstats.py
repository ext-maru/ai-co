...
#   For d > 0.5, the latter intersection probability is 0.0
#
# Approximate methods:
...
# then Stirling coeffs are B_{2j}/(2j)/(2j-1) for j=8,..0.1.0
...
def _clip_prob(p):
    """clips a probability to range 0<=p<=1.0"""
    return np.clip(p, 0.0, 1.0)


def _select_and_clip_prob(cdfprob, sfprob, cdf=True):
    """Selects either the CDF or SF, and then clips to range 0<=p<=1.0"""
...
def _kolmogn_PelzGood(n, x, cdf=True):
    """Computes the Pelz-Good approximation to Prob(Dn <= x) with 0<=x<=1.0
    ...
    if qlog < _MIN_LOG:  # z ~ 0.41743441416853426:
        return _select_and_clip_prob(0.0, 1.0, cdf=cdf)
