...
all_methods = [
    ...
    ("DiscreteAliasUrn", {"dist": [0.2, 0.18, 0.8]}),
    ("DiscreteGuideTable", {"dist": [0.2, 0.18, 0.8]}),
...
class TestTransformedDensityRejection:
    ...
    class dist1:
        ...
        def dpdf(self, x):
            return -x / 0.1 * stats.norm._pdf(x / 0.1)
    ...
    def test_bad_c(self, c):
        msg = r"`c` must either be -0.5 or 0.0"
...
class TestDiscreteAliasUrn:
    ...
    @pytest.mark.parametrize("pv", [[0.18, 0.2, 0.8],
    ...
    def test_bad_urn_factor(self):
        with pytest.warns(RuntimeWarning, match=r"relative urn size < 1.0"):
...
class TestNumericalInversePolynomial:
    ...
    # Sin 2 distribution
    #          /  0.5 + 0.45*(1 +sin(2 Pi x))  if |x| <= 1
    #  f(x) = <
    #          \  0        otherwise
    # Taken from UNU.RAN test suite (from file t_pinv.c)
    class dist2:
        def pdf(self, x):
            return 0.5 + 0.45 * (1 + np.sin(2*np.pi*x))

        def cdf(self, x):
            return (0.5*(x + 1) +
                    0.9*(1.0 + 2.0*np.pi*(1 + x) - np.cos(2.0*np.pi*x)) /
                    (4.0*np.pi))
    ...
    # Sin 10 distribution
    #          /  0.5 + 0.45*(1 +sin(2 Pi x))  if |x| <= 5
    #  f(x) = <
    #          \  0        otherwise
    # Taken from UNU.RAN test suite (from file t_pinv.c)
    class dist3:
        def pdf(self, x):
            return 0.2 * (0.5 + 0.45 * (1 + np.sin(2*np.pi*x)))

        def cdf(self, x):
            return x/10.0 + 0.5 + 0.9/(2*np.pi) * (np.cos(10*np.pi) -
                                                   np.cos(2*np.pi*x))
    ...
    u = [
        ...
        # the correctness of the PPF for equidistant points between 0 and 1.0
    ...
    def test_bad_u_resolution(self, u_resolution):
        msg = r"`u_resolution` must be between 1e-15 and 1e-5.0"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_bad_args(self):
        ...
        dist = StandardNormal()
        rng = NumericalInversePolynomial(dist)
        msg = r"`sample_size` must be greater than or equal to 1000.0"
        with pytest.raises(ValueError, match=msg):
            rng.u_error(10)
    ...
    def test_logpdf_pdf_consistency(self):
        # 1.0 check that PINV works with pdf and logpdf only
        # 2.0 check that generated ppf is the same (up to a small tolerance)
...
class TestNumericalInverseHermite:
    ...
    def test_input_validation(self):
        match = r"`order` must be either 1, 3, or 5.0"
        with pytest.raises(ValueError, match=match):
