    ...
    def run_precommit_check(self) -> PreCommitResult:
        ...
        try:
            ...
            if self.config.enable_import_check and self.incident_predictor:
                prediction_result = self.incident_predictor.predict_development_risks(
                    staged_files
                )
                ...
                critical_imports = [
                    i
                    for i in prediction_result["import_issues"]
                    if i.severity == "critical"
                ]
                if critical_imports:
                    issues.extend(
                        [
                            f"Critical import error: {i.suggested_fix}"
                            for i in critical_imports[:3]
                        ]
                    )
            ...
            if self.config.enable_quick_tests:
                ...
                if not test_result["success"]:
                    issues.append(
                        f"Quick tests failed ({test_result['tests_run']} tests)"
                    )
                    risk_score += 0.3
            ...
            if self.config.enable_test_prediction and self.incident_predictor:
                ...
                if test_files and hasattr(self.incident_predictor, "test_predictor"):
                    test_predictions = (
                        self.incident_predictor.test_predictor.predict_test_results(
                            test_files
                        )
                    )
                    ...
                    failing_predictions = [
                        ...
                        if p.predicted_result in ["fail", "error"]
                    ]
                    if failing_predictions:
                        issues.append(
                            f"{len(failing_predictions)} tests predicted to fail"
                        )
                        risk_score += 0.2
