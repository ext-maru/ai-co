...
def test_rabbitmq_connection(self):
    ...
    try:
        ...
        # Check queue status
        queues_info = []
        for queue_name in ['ai_tasks', 'ai_pm', 'ai_results', 'task_queue', 'result_queue']:
            try:
...
def check_rabbitmq_process():
    ...
    try:
        result = subprocess.run(['pgrep', '-f', 'beam.smp.*rabbit'], 
...
def monitor_connections(duration_seconds=300, interval=10):
    ...
    while time.time() - start_time < duration_seconds:
        ...
        if not conn_ok:
            failure_count += 1
            log_connection_status("❌ CONNECTION FAILED", 
    ...
    # Summary
    success_rate = ((check_count - failure_count) / check_count) * 100 if check_count > 0 else 0
