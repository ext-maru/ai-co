...
class AlertConditionEvaluator:
    ...
    def evaluate_condition(
        self, condition: str, current_metrics: Dict[str, Any]
    ) -> bool:
        """条件を評価"""
        try:
            # 条件文字列を解析して評価
            ...
            if " for " in condition:
                ...
                return self._evaluate_duration_condition(
                    base_condition, duration, current_metrics
                )
            else:
                # 単純な条件
    ...
    def _evaluate_simple_condition(
        ...
        if " or " in condition:
            ...
            return any(
                self._evaluate_simple_condition(c.strip(), metrics) for c in conditions
            )
    ...
    def _evaluate_duration_condition(
        self, condition: str, duration: int, metrics: Dict[str, Any]
    ) -> bool:
        """持続時間を含む条件を評価"""
...
class AlertManager:
        ...
        for suppression_rule in self.config.get("suppression_rules", []):
            if suppression_rule["name"] == "maintenance_window":
                ...
                if (
                    ...
                    and rule.level.value in suppression_rule["suppress"]
                ):
                    return True
    ...
    async def _send_notifications(self, alert: Alert):
        """通知を送信"""
        notification_config = self.config["alert_levels"][alert.level.value][
            "notification"
        ]
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_alerts": [
                ...
                for alert in sorted(
                    self.alert_history[-10:], key=lambda a: a.timestamp, reverse=True
                )
            ],
        }
...
async def main():
    ...
    while True:
        try:
            ...
            output_path = Path(
                "/home/aicompany/ai_co/monitoring/alerts/current_alerts.json"
            )
