...
class SageReportCollector:
    ...
    async def collect_incident_sage_report(self, start_time: datetime, end_time: datetime) -> SageReport:
    ...
    async def collect_knowledge_sage_report(self, start_time: datetime, end_time: datetime) -> SageReport:
        ...
        try:
            ...
            charts = [
                ...
                self._create_category_distribution_chart(metrics['knowledge_categories']),
    ...
    async def collect_task_sage_report(self, start_time: datetime, end_time: datetime) -> SageReport:
        """Task Sageのレポートを収集"""
        try:
            metrics = {
                "total_tasks": 8924,
    ...
    async def collect_rag_sage_report(self, start_time: datetime, end_time: datetime) -> SageReport:
        """RAG Sageのレポートを収集"""
        try:
            metrics = {
                "total_queries": 24567,
    ...
    def _create_resolution_time_chart(self, metrics: Dict) -> Dict:
        ...
        # サンプルデータ生成
        hours = list(range(24))
        times = [metrics['avg_resolution_time'] + np.random.normal(0, 20) for _ in hours]
    ...
    def _create_worker_utilization_chart(self, metrics: Dict) -> Dict:
        ...
        # サンプルデータ生成
        hours = list(range(24))
        utilization = [metrics['worker_utilization'] + np.random.normal(0, 10) for _ in hours]
        ...
        fig.add_hline(y=80, line_dash="dash", line_color="red", 
                      annotation_text="警告閾値")
        fig.update_layout(
            title="ワーカー使用率の推移（24時間）",
            xaxis_title="時間",
            yaxis_title="使用率 (%)"
        )
        return {"type": "area", "data": fig.to_json()}
    ...
    def _create_query_volume_chart(self, metrics: Dict) -> Dict:
        ...
        # サンプルデータ生成
        hours = list(range(24))
        volume = [metrics['total_queries'] / 24 + np.random.normal(0, 100) for _ in hours]
...
class IntegratedReportGenerator:
    ...
    async def generate_report(self, period: str = "daily") -> IntegratedReport:
        ...
        # 統合推奨事項を生成
        recommendations = self._generate_recommendations(sage_reports, system_health, trends)
    ...
    def _generate_executive_summary(self, sage_reports: List[SageReport]) -> Dict[str, Any]:
        """エグゼクティブサマリーを生成"""
        # 各Sageのメトリクスから主要指標を抽出
        incident_metrics = next((r.metrics for r in sage_reports if r.sage_name == "Incident Sage"), {})
        knowledge_metrics = next((r.metrics for r in sage_reports if r.sage_name == "Knowledge Sage"), {})
        task_metrics = next((r.metrics for r in sage_reports if r.sage_name == "Task Sage"), {})
        rag_metrics = next((r.metrics for r in sage_reports if r.sage_name == "RAG Sage"), {})
        
        # システム健全性スコアを計算
        health_score = self._calculate_health_score({
            "incident_resolution_rate": incident_metrics.get('resolved_incidents', 0) / max(incident_metrics.get('total_incidents', 1), 1) * 100,
    ...
    def _generate_recommendations(self, sage_reports: List[SageReport], 
                                system_health: Dict, trends: Dict) -> List[Dict[str, Any]]:
        """統合推奨事項を生成"""
        recommendations = []
    ...
    async def save_report(self, report: IntegratedReport, output_dir: str = "/home/aicompany/ai_co/monitoring/reports/generated"):
        """レポートを保存"""
