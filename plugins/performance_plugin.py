...
class PerformanceMonitorPlugin(WorkerPlugin):
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        """プラグイン初期化"""
        self.db_path = Path(config.get('db_path', '/home/aicompany/ai_co/db/performance.db'))
    ...
    def on_task_complete(self, task: Dict[str, Any], result: Any):
        ...
        if task_id in self.active_tasks:
            ...
            # リソース使用量の集計
            cpu_avg = sum(task_info['cpu_samples']) / len(task_info['cpu_samples']) if task_info['cpu_samples'] else 0
            memory_avg = sum(task_info['memory_samples']) / len(task_info['memory_samples']) if task_info['memory_samples'] else 0
            memory_peak = max(task_info['memory_samples']) if task_info['memory_samples'] else 0
            ...
            self._record_metrics(
                ...
                status='completed',
                metadata=json.dumps({
                    'files_created': len(result.get('files_created', [])) if isinstance(result, dict) else 0
                })
            )
