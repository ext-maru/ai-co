...
class PostgreSQLUnificationMigrator:
    ...
    def __init__(self):
        self.pg_url = os.getenv('GRIMOIRE_DATABASE_URL', 'postgresql://aicompany@localhost:5432/ai_company_grimoire')
    ...
    async def create_unified_schemas(self):
        ...
        # タスクインデックス
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_unified_tasks_status ON unified_tasks(status)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_unified_tasks_sage ON unified_tasks(assigned_sage)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_unified_tasks_priority ON unified_tasks(priority, created_at)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_unified_tasks_metadata ON unified_tasks USING gin(metadata)")
        
        # 会話インデックス  
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_conversations_timestamp ON unified_conversations(timestamp)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_conversations_session ON unified_conversations(session_id)")
        ...
        try:
            ...
            for task in tasks:
                try:
                    ...
                    # 基本フィールドマッピング
                    task_id = task_dict.get('task_id') or task_dict.get('id', f"task_{datetime.now().timestamp()}")
                    title = task_dict.get('title') or task_dict.get('description', 'Untitled Task')[:100]
                    description = task_dict.get('description') or task_dict.get('content', '')
                    status = task_dict.get('status', 'pending')
                    priority = task_dict.get('priority', 'medium')
                    
                    # 日時変換
                    created_at = task_dict.get('created_at') or task_dict.get('timestamp', datetime.now())
                    ...
                    """, task_id, title, description, status, priority, 
                         created_at, json.dumps(metadata), 'knowledge_sage')
        ...
        try:
            ...
            for conv in conversations:
                try:
                    ...
                    # フィールドマッピング
                    conversation_id = conv_dict.get('conversation_id') or conv_dict.get('id', f"conv_{datetime.now().timestamp()}")
                    user_message = conv_dict.get('user_message') or conv_dict.get('message', '')
                    ai_response = conv_dict.get('ai_response') or conv_dict.get('response', '')
                    
                    # 日時変換
                    timestamp = conv_dict.get('timestamp') or conv_dict.get('created_at', datetime.now())
                    ...
                    """, conversation_id, user_message, ai_response, 
                         timestamp, json.dumps(context), 'knowledge_sage')
                    
                    self.migration_stats['conversations_migrated'] += 1
                    
                except Exception as e:
                    logger.error(f"会話移行エラー: {e}")
                    self.migration_stats['errors'].append(f"Conversation migration: {str(e)}")
            ...
            self.migration_stats['errors'].append(f"Conversation migration failed: {str(e)}")
    ...
    def generate_migration_report(self, verification_result):
        ...
        report = {
            ...
            'success_rate': (
                (self.migration_stats['tasks_migrated'] + self.migration_stats['conversations_migrated']) /
                max(1, self.migration_stats['tasks_migrated'] + self.migration_stats['conversations_migrated']) * 100
            ) if (self.migration_stats['tasks_migrated'] + self.migration_stats['conversations_migrated']) > 0 else 0,
