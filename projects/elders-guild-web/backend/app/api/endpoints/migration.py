...
from ...services.migration_service import migration_service, feature_toggle_service, data_sync_service
...
async def sync_elder_council_data(
    current_user: User = Depends(require_elder),
    db: Session = Depends(get_db)
):
...
async def migrate_endpoint(
    ...
    if action == "migrate":
        ...
        raise HTTPException(
            ...
            detail="Action must be 'migrate' or 'proxy'"
        )
