    ...
    try:
        ...
        if affected_systems:
            incidents = [
                ...
                if any(system in i.affected_systems for system in affected_systems)
            ]
        ...
        await websocket_manager.send_sage_message(
            SageMessage(
                ...
                content={
                    ...
                    "total": total_count,
                    "active_critical": len([
                        i for i in incident_store.values() 
...
async def update_incident(incident_id: str, update: IncidentUpdate):
    ...
    try:
        ...
        if (old_severity != incident.severity and 
            severity_order.get(incident.severity, 0) > severity_order.get(old_severity, 0)):
            await websocket_manager.send_sage_message(
                SageMessage(
                    message_id=str(uuid4()),
                    sage_type="incident",
...
async def get_affected_systems():
    ...
    try:
        ...
        for system in systems:
            system_incidents = [
                i for i in incident_store.values() 
                if system in i.affected_systems
            ]
            system_stats[system] = {
...
async def get_incident_stats():
    ...
    try:
        ...
        stats = {
            ...
            "total_affected_systems": len(set(
                system for incident in incident_store.values() 
                for system in incident.affected_systems
            )),
            "resolution_rate": (
                len([i for i in incident_store.values() if i.status in ["resolved", "closed"]]) 
