...
from app.schemas.sages import SearchQuery
from app.schemas.sages import SearchResponse
from app.schemas.sages import SearchResult
            ...
            if score > 0:
                results.append(
                    SearchResult(
                        ...
                        highlighted_content=highlight_content(task.description or task.title, query.query),
                    )
                )
...
async def get_search_analytics():
    ...
    try:
        analytics = {
            ...
            "knowledge_categories": list(set(article.category for article in knowledge_store.values())),
            "task_projects": list(set(task.project for task in task_store.values() if task.project)),
            "incident_systems": list(
