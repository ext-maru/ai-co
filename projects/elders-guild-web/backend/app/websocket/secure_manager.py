...
from ..models.auth import User, UserRole, PermissionType
...
class SecureWebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, user: User, connection_type: str = "general") -> str:
        ...
        elif connection_type in self.sage_connections and user.role in [UserRole.SAGE, UserRole.ELDER, UserRole.GRAND_ELDER]:
    ...
    async def _handle_elder_council_message(self, connection_id: str, message: dict):
        ...
        if connection_id not in self.council_connections:
            await self.send_personal_message({
                "type": "error",
                "message": "Insufficient permissions for Elder Council"
            }, connection_id)
            return
    ...
    def get_connection_stats(self) -> dict:
        ...
        return {
            ...
            "sage_connections": {
                sage_type: len(connections) 
                for sage_type, connections in self.sage_connections.items()
            },
