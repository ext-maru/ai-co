...
class DocumentType(str, Enum):
    """書類タイプ"""
    # 共通
    APPLICATION_FORM = "application_form"  # 申込書一式
...
def get_document_categories(contract_type: ContractType) -> List[DocumentCategory]:
    ...
    if contract_type == ContractType.INDIVIDUAL:
        ...
        corporate_docs = [
            ...
            if not req.document_type.value.startswith("rep_")
        ]
        representative_docs = [
            req for req in requirements 
            if req.document_type.value.startswith("rep_")
        ]
