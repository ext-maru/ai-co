...
from app.models.upload import Upload, ContractUpload, UploadStatus
from app.models.contract_type import (
    ContractType, DocumentType, DOCUMENT_REQUIREMENTS,
    get_document_categories
)
from app.schemas.contract_upload import (
    ContractUploadCreate, ContractUploadResponse, ContractUploadDetail,
    DocumentUploadStatus, FileUploadResponse, DocumentValidationResult
)
...
class ContractUploadService:
    ...
    async def get_contract_upload_detail(
        ...
        """契約書類アップロード詳細取得"""
        query = self.db.query(ContractUpload).filter(
            ContractUpload.id == contract_upload_id
        )
        ...
        # アップロード済み書類取得
        uploaded_docs = self.db.query(Upload).filter(
        ...
        # 不足書類
        missing_documents = [
            ds.display_name for ds in required_docs 
            if not ds.uploaded
        ]
    ...
    async def upload_document(
        ...
        """書類アップロード"""
        # 契約アップロード確認
        contract_upload = self.db.query(ContractUpload).filter(
            ContractUpload.id == contract_upload_id,
        ...
        validation = await self._validate_file(
            file_data, filename, content_type, requirement
        )
        if not validation.is_valid:
            raise ValueError(f"ファイル検証エラー: {', '.join(validation.errors)}")
        
        # 既存ファイル数確認
        existing_count = self.db.query(Upload).filter(
            Upload.contract_upload_id == contract_upload_id,
    ...
    async def _validate_file(
        ...
        if file_ext not in requirement.allowed_formats:
            errors.append(
                f"許可されていないファイル形式です。"
                f"許可: {', '.join(requirement.allowed_formats)}"
            )
        ...
        if size_mb > requirement.max_size_mb:
            errors.append(
                f"ファイルサイズが大きすぎます。"
                f"最大: {requirement.max_size_mb}MB"
            )
    ...
    async def submit_contract_upload(
        ...
        if detail.completion_rate < 100:
            raise ValueError(
                f"必要書類が不足しています。"
                f"不足: {', '.join(detail.missing_documents)}"
            )
        
        if detail.expired_documents:
            raise ValueError(
                f"期限切れの書類があります。"
                f"期限切れ: {', '.join(detail.expired_documents)}"
            )
