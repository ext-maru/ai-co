...
<body>
            ...
            "elders": {
                ...
                "crisis_sage": {
                    ...
                    "alert_level": (
                        ...
                        else "yellow" if psutil.virtual_memory().percent < 90 else "red"
                    ),
    ...
    def serve_api_analytics_run(self):
        ...
        try:
            ...
            response = {
                ...
                "reports": {"json": str(results["json_report"]), "html": str(results["html_report"])},
            ...
            self._send_json_response({"status": "error", "message": f"分析中にエラーが発生しました: {str(e)}"})
    ...
    def serve_api_analytics_latest(self):
        ...
        try:
            ...
            if not reports_dir.exists():
                self._send_json_response({"status": "no_data", "message": "まだ分析レポートがありません"})
                return
            ...
            self._send_json_response({"status": "error", "message": f"レポート取得中にエラーが発生しました: {str(e)}"})
    ...
    def serve_api_analytics_predictions(self):
        ...
        try:
            ...
            if not reports_dir.exists():
                self._send_json_response({"status": "no_data", "predictions": {}})
                return
            ...
            response = {
                ...
                "visualizations": {
                    ...
                    "insights_count": {
                        r["type"]: len(r.get("insights", [])) for r in report_data.get("detailed_results", [])
                    },
                },
            }
            ...
            self._send_json_response(
                {"status": "error", "message": f"予測データ取得中にエラーが発生しました: {str(e)}"}
            )
