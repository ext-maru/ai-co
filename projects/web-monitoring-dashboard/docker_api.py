...
from libs.shared_enums import ProjectType
...
def list_containers():
    ...
    try:
        ...
        return jsonify(
            {"containers": container_list, "count": len(container_list), "timestamp": datetime.now().isoformat()}
        )
...
def create_container():
    ...
    try:
        ...
        if "template" in data:
            ...
            template_config = template_manager.get_template(project_type=project_type, security_level=security_level)
...
def remove_container(container_id):
    ...
    try:
        ...
        return jsonify(
            {"id": container_id, "removed": True, "message": f"Container {container_id} removed successfully"}
        )
...
def get_container_logs(container_id):
    ...
    try:
        ...
        # Get logs
        logs = container.logs(tail=tail if tail != "all" else "all", since=since, timestamps=timestamps, decode=True)
...
def list_images():
    ...
    try:
        ...
        return jsonify({"images": image_list, "count": len(image_list), "timestamp": datetime.now().isoformat()})
...
def list_templates():
    ...
    try:
        ...
        return jsonify(
            {"templates": template_list, "count": len(template_list), "timestamp": datetime.now().isoformat()}
        )
