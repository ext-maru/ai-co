...
class UserManager:
    ...
    def create_user(self, email: str, name: str, password: str, roles: List[str]) -> Dict[str, Any]:
        ...
        return {"id": user_id, "email": email, "name": name, "roles": roles, "permissions": permissions}
...
class RBACManager:
    ...
    def __init__(self):
        ...
        self.roles = {
            ...
            "viewer": {"name": "Viewer", "description": "Read-only access", "permissions": ["read"], "inherits": []},
            "user": {
                "name": "Standard User",
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not rbac_manager.check_permission(user_roles, resource, action):
                audit_logger.log_event(
                    ...
                    {"resource": resource, "action": action, "roles": user_roles},
                )
...
def create_user():
    ...
    try:
        ...
        audit_logger.log_event(
            "user_created", g.current_user["user_id"], {"new_user_id": user["id"], "email": email, "roles": roles}
        )
