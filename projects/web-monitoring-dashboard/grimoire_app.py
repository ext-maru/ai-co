...
from libs.grimoire_database import EvolutionType
from libs.grimoire_database import GrimoireDatabase
from libs.grimoire_database import MagicSchool
from libs.grimoire_database import SpellType
from libs.grimoire_spell_evolution import EvolutionEngine
from libs.grimoire_spell_evolution import EvolutionStrategy
from libs.grimoire_vector_search import GrimoireVectorSearch
from libs.grimoire_vector_search import SearchQuery
    ...
    def _setup_routes(self):
        ...
        def api_search():
            ...
            try:
                ...
                search_query = SearchQuery(
                    ...
                    magic_schools=(
                        [MagicSchool(school) for school in data.get("schools", [])] if data.get("schools") else None
                    ),
                ...
                return jsonify(
                    {"success": True, "results": search_results, "total": len(search_results), "query": data["query"]}
                )
        ...
        def api_get_spell(spell_id):
            ...
            try:
                ...
                # 関連呪文取得
                related_spells = loop.run_until_complete(self.search_engine.find_related_spells(spell_id, limit=5))

                # 昇華履歴取得
                evolution_history = loop.run_until_complete(self.database.get_evolution_history(spell_id))
        ...
        def api_create_spell():
            ...
            try:
                ...
                # 自動タグ提案
                suggested_tags = loop.run_until_complete(self.search_engine.suggest_tags(data["content"]))
                ...
                # 呪文作成とインデックス化
                spell_id = loop.run_until_complete(self.search_engine.index_spell(str(uuid.uuid4()), spell_data))

                loop.close()

                return jsonify({"success": True, "spell_id": spell_id, "suggested_tags": suggested_tags})
        ...
        def api_create_evolution_plan():
            ...
            try:
                ...
                plan = loop.run_until_complete(
                    self.evolution_engine.create_evolution_plan(data["spell_ids"], evolution_type, strategy)
                )
...
if __name__ == "__main__":
    ...
    # ログ設定
    logging.basicConfig(
        level=logging.DEBUG if args.debug else logging.INFO, format="%(asctime)s [%(name)s] %(levelname)s: %(message)s"
    )
