...
class MasterConsoleController:
    ...
    def _get_services_status(self) -> Dict[str, Any]:
        ...
        for service_name, config in self.system_services.items():
            try:
                ...
                # 安全なフォールバック
                services_status[service_name] = {"status": "unknown", "health": 0, "error": str(e)}
    ...
    def _check_service_health(self, service_name: str, port: int) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(f"http://localhost:{port}/health", timeout=2)  # 短いタイムアウト

            if response.status_code == 200:
                return {"status": "healthy", "health": 100, "response_time": response.elapsed.total_seconds() * 1000}
            else:
                return {"status": "degraded", "health": 50, "response_time": response.elapsed.total_seconds() * 1000}

        except requests.exceptions.RequestException:
            # 📋 タスク賢者パターン: サービス停止時の適切な状態表示
            return {"status": "stopped", "health": 0, "response_time": 0}
    ...
    def _get_ai_recommendations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # サービス停止チェック
            stopped_services = [name for name, status in services_status.items() if status.get("status") == "stopped"]
    ...
    def _calculate_overall_health(self, dashboard_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            health_scores = [service.get("health", 0) for service in services.values()]
...
def dashboard():
    ...
    try:
        ...
        timestamp_formatted = datetime.fromisoformat(
            dashboard_data["timestamp"].replace("Z", "+00:00")
            if "Z" in dashboard_data["timestamp"]
            else dashboard_data["timestamp"]
        ).strftime("%Y-%m-%d %H:%M:%S")

        active_services = sum(
            1 for service in dashboard_data.get("services", {}).values() if service.get("status") == "healthy"
        )
...
def health_check():
    """ヘルスチェック"""
    return jsonify(
        {"status": "healthy", "service": "master_console", "timestamp": datetime.now().isoformat(), "version": "1.0.0"}
    )
