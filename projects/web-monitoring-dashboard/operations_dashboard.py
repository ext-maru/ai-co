...
class OperationsMonitor:
    ...
    def __init__(self):
        ...
        # 履歴データ
        self.historical_data = {"daily_stats": [], "weekly_trends": [], "incident_history": []}
    ...
    def _check_alerts(self, system_metrics: Dict[str, Any], app_metrics: Dict[str, Any]) -> List[Dict[str, Any]]:
        """アラートチェック"""
        alerts = []
    ...
    def _save_metrics_to_db(self, system_metrics: Dict[str, Any], app_metrics: Dict[str, Any]):
        """メトリクスをデータベースに保存"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
    ...
    def _calculate_health_score(self, system_metrics: Dict[str, Any], app_metrics: Dict[str, Any]) -> float:
        """ヘルススコア計算"""
        score = 100.0
    ...
    def get_historical_data(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            system_history = [
                {"timestamp": row[0], "cpu_usage": row[1], "memory_usage": row[2], "disk_usage": row[3]}
            ...
            return {"system_history": system_history, "application_history": app_history, "period_hours": hours}
...
def health_check():
    """ヘルスチェック"""
    return jsonify({"status": "healthy", "service": "operations_dashboard", "timestamp": datetime.now().isoformat()})
