...
class A2AAnomalyAnalyzer:
    ...
    def _extract_features(self, communications: List[tuple]) -> np.ndarray:
        ...
        for comm in communications:
            (comm_id, timestamp, source, target, msg_type, 
             priority, payload_size, response_time, status, error_msg, metadata) = comm
    ...
    def _analyze_agent_patterns(self, anomalies: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "communication_flows": Counter([f"{a['source_agent']} -> {a['target_agent']}" 
                                          for a in anomalies if a["source_agent"] and a["target_agent"]]).most_common(5)
        }
    ...
    def _analyze_error_patterns(self, anomalies: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate": len([s for s in error_statuses if s == "error"]) / len(error_statuses) if error_statuses else 0
        }
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if agent.get("problematic_sources"):
            ...
            recommendations.append(f"🤖 エージェント '{top_source[0]}' が異常の{top_source[1]}件に関与しています。監視を強化してください。")
        ...
        if not recommendations:
            recommendations.append("✅ 異常パターンは検出されましたが、重大な問題は見つかりませんでした。継続的な監視を推奨します。")
