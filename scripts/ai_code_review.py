...
class AICodeReviewer:
    ...
    def get_changed_files(self) -> List[str]:
        ...
        try:
            result = subprocess.run(
                ["git", "diff", "--name-only", f"{self.base_branch}...{self.head_branch}"],
                capture_output=True,
                text=True
            )
    ...
    def get_file_diff(self, file_path: str) -> str:
        ...
        try:
            result = subprocess.run(
                ["git", "diff", f"{self.base_branch}...{self.head_branch}", "--", file_path],
                capture_output=True,
                text=True
            )
            ...
            for analysis in analyses:
                if analysis['issues']:
                    ...
                    for issue in analysis['issues']:
                        ...
                        summary += f"- {icon} Line {issue['line']}: {issue['message']}\n"
                    summary += "\n"
        else:
