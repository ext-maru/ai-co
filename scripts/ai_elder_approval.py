...
def create_approval_request(request_type, title, description, details=None, urgency="normal"):
    """承認申請を作成"""
    request_data = {
...
def main():
    ...
    new_parser.add_argument('--type', '-t', required=True,
                          choices=['system_change', 'eternal_knowledge', 'hierarchy_change', 
                                  'rule_change', 'emergency', 'feature', 'other'],
                          help='申請種別')
    new_parser.add_argument('--title', required=True, help='申請タイトル')
    new_parser.add_argument('--description', '-d', required=True, help='申請内容の説明')
    new_parser.add_argument('--urgency', '-u', default='normal',
                          choices=['critical', 'high', 'normal', 'low'],
                          help='緊急度')
    new_parser.add_argument('--details', '-D', nargs='*', 
                          help='追加詳細 (key=value形式)')
    ...
    quick_parser.add_argument('pattern', 
