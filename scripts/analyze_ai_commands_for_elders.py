...
class AICommandAnalyzer:
    ...
    def analyze_commands(self):
        ...
        # Get all AI commands
        ai_commands = sorted(
            [f.name for f in self.scripts_dir.glob("ai-*") if f.is_file()]
        )
    ...
    def elder_consultation(self, analysis):
        ...
        consultations = {
            ...
            "task_oracle": {
                "name": "タスク賢者",
                "concerns": ["タスク実行の最適な順序が不明確", "依存関係が明示されていない", "ワークフローが断片化"],
                "recommendations": ["ワークフロー別のコマンドグループ化", "依存関係の明示的な管理", "タスクチェーン機能の実装"],
            },
            "incident_sage": {
                "name": "インシデント賢者",
                "concerns": ["エラーハンドリングが統一されていない", "権限管理が不明確", "障害時の影響範囲が予測困難"],
                "recommendations": ["統一エラーハンドリングフレームワーク", "権限レベルの明確化", "依存関係マップの作成"],
            },
            "rag_sage": {
                "name": "RAG賢者",
                "concerns": ["コマンド検索が困難", "関連コマンドの発見が偶発的", "使用例が不足"],
                "recommendations": ["自然言語検索機能", "コマンド推薦システム", "豊富な使用例の追加"],
            },
        }
            ...
            "migration_phases": [
                {
                    ...
                    "name": "分析と計画",
                    "duration": "3日",
                    "tasks": ["全コマンドの機能マッピング", "重複機能の特定", "依存関係の分析"],
                },
                {
                    "phase": 2,
                    "name": "基盤整備",
                    "duration": "1週間",
                    "tasks": ["新コマンド体系の実装", "エイリアスシステムの構築", "ヘルプシステムの統一"],
                },
                {
                    "phase": 3,
                    "name": "段階的移行",
                    "duration": "2週間",
                    "tasks": ["旧コマンドから新コマンドへのマッピング", "移行スクリプトの作成", "ユーザーへの通知"],
                },
            ],
            "benefits": ["学習曲線を50%削減", "コマンド発見効率を3倍に向上", "エラー率を40%削減", "開発効率を30%向上"],
        }
    ...
    def generate_report(self):
        ...
        json_path = (
            report_dir
            / f"ai_command_reorganization_{self.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
...
"""
        ...
        md_path = (
            json_path.parent
            / f"AI_COMMAND_REORGANIZATION_REPORT_{self.timestamp.strftime('%Y%m%d')}.md"
        )
...
def main():
    ...
    print(f"   - {json_path}")
    print(
        f"   - {json_path.parent / f'AI_COMMAND_REORGANIZATION_REPORT_{analyzer.timestamp.strftime('%Y%m%d')}.md'}"
    )
