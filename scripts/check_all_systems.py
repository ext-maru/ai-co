...
def check_system():
    ...
    # Git設定
    try:
        result = subprocess.run(['git', 'config', 'user.name'], capture_output=True, cwd='/home/aicompany/ai_co')
    ...
    # データベース
    db_file = Path("/home/aicompany/ai_co/task_history.db")
    results["features"]["SQLite DB"] = db_file.exists()
    
    # 設定ファイル
    configs = ["slack.conf", "system.conf", "scaling.conf"]
    config_ok = all(Path(f"/home/aicompany/ai_co/config/{conf}").exists() for conf in configs)
    results["features"]["設定ファイル"] = config_ok
    
    # 出力ディレクトリ
    output_dir = Path("/home/aicompany/ai_co/output")
    ...
    for pattern in ["workers/*_worker_*.py", "libs/*_manager_*.py", "scripts/evolution_*.py"]:
        evolved_files.extend(Path("/home/aicompany/ai_co").glob(pattern))
    ...
    try:
        ...
        print(f"📁 生成ファイル: {len(list(Path('/home/aicompany/ai_co/output').rglob('*.py')))}個")
