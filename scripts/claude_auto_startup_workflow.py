...
class ClaudeAutoStartupWorkflow:
    ...
    async def run_full_workflow(self):
        ...
        try:
            ...
            self.console.print(
                Panel(f"❌ ワークフロー失敗: {e}", title="⚠️ Error", border_style="bright_red")
            )
            raise
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        summary += (
                            f"- アクティブトリガー: {system_status.get('total_triggers', 0)}件\n"
                        )
    ...
    def _create_legacy_knowledge_summary(self):
        ...
        # エルダー評議会の最新状況
        elder_requests = list(
            (self.knowledge_base_path / "elder_council_requests").glob("*.md")
        )
        if elder_requests:
            legacy_summary += "### 🏛️ エルダー評議会最新状況\n\n"
            # 最新の3件を追加
            for req_file in sorted(
                elder_requests, key=lambda x: x.stat().st_mtime, reverse=True
            )[:3]:
        ...
        with Progress(
            ...
            try:
                ...
                greeting_file = (
                    ...
                    / "elder_greetings"
                    / f"claude_startup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
                )
...
"""
    ...
    def _verify_claude_cli_setup(self):
        ...
        try:
            ...
            # Claude CLI設定ファイル確認
            claude_config_path = (
                Path(self.ai_company_home) / ".anthropic" / "claude_cli.toml"
            )
