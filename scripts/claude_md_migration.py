...
class CLAUDEMDMigrator:
    ...
    def extract_tags(self, title: str, content: str):
        ...
        keywords = [
            ...
            'PostgreSQL', 'pgvector', 'CorePostgres',
            'グランドエルダー', 'クロードエルダー'
        ]
    ...
    async def create_embeddings_with_context(self, title: str, content: str, category: str):
        """コンテキスト付きembedding生成"""
    ...
    try:
        ...
        for path in claude_md_paths:
            if os.path.exists(path):
                ...
                for i, section in enumerate(sections):
                    if len(section['content'].strip()) > 50:  # 短すぎるセクションはスキップ
        ...
        for query in test_queries:
            ...
            if results:
                ...
                print(f"  '{query}' → {top_result['section_title'][:40]}... (類似度: {top_result['similarity']:.3f})")
