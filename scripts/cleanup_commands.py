...
class CommandCleaner:
    def __init__(self, dry_run=True):
        ...
        # 保持すべきコア機能
        self.core_commands = [
            "ai", "ai-start", "ai-stop", "ai-status", "ai-send", "ai-logs",
            "ai-tasks", "ai-workers", "ai-config", "ai-help", "ai-version",
            "ai-rag-search", "ai-git", "ai-monitor", "ai-venv"
        ]
    ...
    def create_backup(self, file_path):
        ...
        if not self.dry_run:
            ...
            try:
                ...
                self.cleanup_summary["errors"].append(f"Backup failed for {file_path}: {e}")
                return False
        return True
    ...
    def clean_backup_files(self):
        ...
        bak_files = []
        for dir_path in [self.project_root / "scripts", 
        ...
        for bak_file in bak_files:
            ...
            if not self.dry_run:
                ...
                try:
                    ...
                    self.cleanup_summary["errors"].append(f"Failed to remove {bak_file}: {e}")
        ...
        for pycache_dir in pycache_dirs:
            ...
            if not self.dry_run:
                try:
                    ...
                    self.cleanup_summary["errors"].append(f"Failed to remove {pycache_dir}: {e}")
    ...
    def check_duplicate_commands(self):
        ...
        for cmd, description in duplicates.items():
            cmd_exists = any([
                ...
                (self.project_root / "commands" / f"{cmd.replace('-', '_')}.py").exists()
            ])
