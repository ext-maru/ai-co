...
"""
        ...
        for category, modules in categories.items():
            if modules:
                ...
                for module, data in sorted(modules):
                    ...
                    emoji = "✅" if coverage_percent >= 80 else "⚠️" if coverage_percent >= 60 else "❌"
                    report += f"| {emoji} {module} | {coverage_percent:.1f}% | {data['covered_lines']} | {data['missing_lines']} |\n"
        
        # 改善が必要なモジュール
        low_coverage = [(m, d) for m, d in analysis['module_coverage'].items() if d['coverage'] < 80]
        if low_coverage:
            report += "\n## ⚠️ 改善が必要なモジュール\n\n"
    ...
    def main(self):
        ...
        # レポート保存
        report_file = self.report_dir / f"coverage_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
