    ...
    def generate_coverage_report(self) -> Dict[str, any]:
        ...
        for lib_name in target_libs:
            ...
            if lib_file.exists():
                ...
                # 個別レポート
                status = "✅" if result["coverage_percentage"] >= 90 else "🟡" if result["coverage_percentage"] >= 70 else "❌"
    ...
    def suggest_test_improvements(self, results: Dict[str, any]) -> List[str]:
        ...
        for result in results["results"]:
            if result["coverage_percentage"] < 90:
                ...
                # 重要そうな関数を優先
                important_functions = [
                    f for f in result["untested_functions"] 
                    if any(keyword in f.lower() for keyword in ['create', 'update', 'delete', 'process', 'analyze', 'calculate'])
                ]
