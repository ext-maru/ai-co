...
class IncidentKnightsDeployer:
    ...
    async def _execute_mass_repair(self, scout_knight: CommandGuardianKnight, repair_knight: AutoRepairKnight):
        """大規模自動修復の実行"""
        ...
        self.deployment_log.append({
            ...
            'success_rate': repaired_count / total_issues if total_issues > 0 else 0,
            'timestamp': datetime.now().isoformat()
        })
        
        logger.info(f"🎯 修復結果: {repaired_count}/{total_issues} 成功 ({repaired_count/total_issues*100:.1f}%)")
    ...
    def _generate_summary(self):
        """サマリーの生成"""
        mass_repair = next((log for log in self.deployment_log if log['step'] == 'mass_repair'), {})
...
"""
        ...
        markdown_file = PROJECT_ROOT / "knowledge_base" / "incident_knights_deployment_report.md"
