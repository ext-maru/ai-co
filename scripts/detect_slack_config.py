...
def find_slack_config():
    ...
    for pattern, file_type in config_patterns:
        for config_file in Path(pattern.parent).glob(pattern.name):
            if config_file.name == 'slack.conf' or config_file.name == 'slack_config.json':
                    ...
                    with open(config_file, 'r') as f:
                        for line in f:
                            if '=' in line and not line.startswith('#'):
                                key, value = line.strip().split('=', 1)
                                value = value.strip('"\'')
                                check_key_value(key, value, found_config, config_file.name)
        ...
        for source_file in PROJECT_ROOT.rglob(pattern):
            ...
            try:
                with open(source_file, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    # Webhook URLパターン
                    webhook_match = re.search(r'https://hooks\.slack\.com/services/[^\s"\']+', content)
