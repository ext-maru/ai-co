...
class CommandExecutorDiagnostic:
    ...
    def check_logs(self):
        ...
        if self.log_file.exists():
            ...
            print(f"   最終更新: {mtime.strftime('%Y-%m-%d %H:%M:%S')} ({age.total_seconds():.0f}秒前)")
            ...
            with open(self.log_file, 'r') as f:
                ...
                if lines:
                    ...
                    # エラー確認
                    error_lines = [l for l in lines[-20:] if 'ERROR' in l or 'error' in l]
                    if error_lines:
...
"""
            ...
            for i in range(15):
                ...
                if check_result.get('status') != 'pending':
                    print(f"✅ テスト実行完了！ Exit Code: {check_result.get('exit_code', 'N/A')}")
