...
def diagnose_permissions():
    ...
    tests = [
        ...
        {
            "name": "メッセージ送信",
            "method": lambda: client.chat_postMessage(
                channel=channel_id, text="権限テスト", dry_run=True
            ),
            "scope": "chat:write",
        },
        {
    ...
    if app_token and app_token.startswith("xapp-"):
        ...
        print(
            f"✅ Socket Mode: {'有効' if config.get_bool_env('SLACK_SOCKET_MODE_ENABLED') else '無効'}"
        )
    else:
        print("❌ App Token: 未設定")
