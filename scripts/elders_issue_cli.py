...
"""
    ...
    async def auto_assign(self, args):
        ...
        if args.issue:
            ...
            # 全ての未アサインIssue
            unassigned = [i for i in self.manager.repo.get_issues(state='open') 
                         if not any(l.name.endswith('-workshop') or l.name.endswith('-knights') 
                                   or l.name.endswith('-wizards') or l.name.endswith('-forest') 
                                   for l in i.labels)]
