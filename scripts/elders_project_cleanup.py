...
def delete_expired_projects():
    ...
    for project in schedule['projects']:
        ...
        if deletion_date <= today:
            # 削除実行
            project_path = os.path.join(DELETION_DIR, f"{project['deletion_date']}_{project['name']}")
...
def record_deletion(project):
    ...
    record_file = os.path.join(kb_dir, f"{datetime.now().strftime('%Y%m%d')}_{project['name']}.json")
