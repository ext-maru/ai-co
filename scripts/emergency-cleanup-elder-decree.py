...
class EmergencyElderCleanup:
    ...
    def __init__(self):
        ...
        self.backup_dir = (
            self.root / "cleanup_backup" / datetime.now().strftime("%Y%m%d_%H%M%S")
        )
        self.moved_files = []
...
<<<<<<< HEAD
        ...
        for file_path in self.root.glob("*"):
            if file_path.is_file():
                ...
                if any(
                    keyword in name
                    for keyword in ["report", "analysis", "summary", "audit"]
                ):
                    violations["reports"].append(file_path)

                # ドキュメント系
                elif name.endswith(".md") and name not in ["README.md", "CLAUDE.md"]:
                ...
                # 一時ファイル
                    ...
                    keyword in name for keyword in ["temp", "tmp", "debug", "manual"]
                ):
