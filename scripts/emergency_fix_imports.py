    ...
    for file_path in test_files:
        try:
            ...
            if "PROJECT_ROOT = Path(__file__)" in content:
                ...
                has_path_import = any(
                    "from pathlib import Path" in line or "import pathlib" in line
                    for line in lines
                )

                # sys import が存在するか確認
                has_sys_import = any("import sys" in line for line in lines)
                ...
                if project_root_line > 0:
                    ...
                    path_import_before = any(
                        (
                            "from pathlib import Path" in lines[i]
                            or "import pathlib" in lines[i]
                        )
...
def fix_base_test_imports():
    ...
    if os.path.exists(base_test_file):
        try:
            ...
            if (
                "class ManagerTestCase" not in content
                and "class WorkerTestCase" in content
            ):
