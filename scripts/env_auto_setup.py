...
""".format(os.popen('date').read().strip())
        ...
        categories = {
            '基本設定': [
                'WORKER_DEV_MODE', 'INCIDENT_KNIGHTS_ENABLED', 
                'AUTO_FIX_ENABLED', 'SLACK_NOTIFICATIONS'
            ],
            'RabbitMQ設定': [
            ...
            'エルダーズギルド設定': [
                'ELDERS_GUILD_MODE', 'FOUR_SAGES_ENABLED', 
                'KNIGHTS_AUTO_DEPLOY'
            ],
            'API設定': [
            ...
            'パフォーマンス設定': [
                ...
                'BATCH_SIZE', 'CACHE_TTL'
            ],
            '開発設定': [
                'DEBUG', 'TESTING', 'PROFILE', 'COVERAGE'
            ]
        }
