...
class FourSagesQualityIntegration:
    ...
    async def consult_sages_for_quality(
        self, learning_request: QualityLearningRequest
    ) -> Dict[str, Any]:
        """
        ...
        try:
            self.logger.info(
                f"Consulting 4 Sages for quality analysis: {learning_request.test_file}"
            )
            ...
            coordination_result = self.base_sages.coordinate_learning_session(
                base_request
            )
            ...
            if "knowledge_sage" in individual_responses:
                sage_insights[
                    "knowledge_sage"
                ] = await self._process_knowledge_sage_quality_response(
            ...
            if "task_sage" in individual_responses:
                sage_insights[
                    "task_sage"
                ] = await self._process_task_sage_quality_response(
            ...
            if "incident_sage" in individual_responses:
                sage_insights[
                    "incident_sage"
                ] = await self._process_incident_sage_quality_response(
            ...
            if "rag_sage" in individual_responses:
                sage_insights[
                    "rag_sage"
                ] = await self._process_rag_sage_quality_response(
                    individual_responses["rag_sage"], learning_request
                )

            # Form quality-specific consensus
            quality_consensus = await self._form_quality_consensus(
                sage_insights, learning_request
            )
            ...
            session_summary = {
                ...
                "expected_quality_improvement": quality_consensus.get(
            ...
            self.logger.info(
                f"4 Sages quality consultation completed: {quality_consensus.get('consensus_reached', False)}"
            )
            return session_summary

        except Exception as e:
            self.logger.error(f"4 Sages quality consultation failed: {e}")
            return {"session_id": "error", "error": str(e), "consensus_reached": False}

    async def _process_knowledge_sage_quality_response(
        self, response: Dict[str, Any], request: QualityLearningRequest
    ) -> SageQualityInsight:
        """Process Knowledge Sage response for quality insights"""

        # Extract base response
        base_recommendation = response.get(
            "recommendation", "Store test patterns for learning"
        )
        ...
        historical_analysis = await self._perform_historical_quality_analysis(
            request.test_file
        )
        ...
        if request.quality_metrics.get("pattern_compliance", 0) < 0.8:
            knowledge_recommendations.append(
                "Store and learn from high-quality test patterns"
            )

        if len(request.current_issues) > 3:
            knowledge_recommendations.append(
                "Build knowledge base of common quality issues and solutions"
            )

        knowledge_recommendations.append(
            "Maintain historical quality trends for this test file"
        )
        ...
        quality_impact = self._calculate_knowledge_quality_impact(
            request.quality_metrics, quality_patterns
        )
    ...
    async def _process_task_sage_quality_response(
        self, response: Dict[str, Any], request: QualityLearningRequest
    ) -> SageQualityInsight:
        """Process Task Sage response for quality insights"""

        base_recommendation = response.get(
            "recommendation", "Optimize quality improvement workflow"
        )
        ...
        workflow_optimization = await self._suggest_quality_workflow_optimization(
            request
        )
        ...
        quality_impact = self._calculate_task_quality_impact(
            request.quality_metrics, quality_objectives
        )
    ...
    async def _process_incident_sage_quality_response(
        self, response: Dict[str, Any], request: QualityLearningRequest
    ) -> SageQualityInsight:
        """Process Incident Sage response for quality insights"""

        base_recommendation = response.get(
            "recommendation", "Monitor for quality degradation risks"
        )
        ...
        prevention_strategies = await self._develop_quality_prevention_strategies(
            request
        )
        ...
        quality_impact = self._calculate_incident_quality_impact(
            request.quality_metrics, quality_risks
        )
    ...
    async def _process_rag_sage_quality_response(
        self, response: Dict[str, Any], request: QualityLearningRequest
    ) -> SageQualityInsight:
        """Process RAG Sage response for quality insights"""

        base_recommendation = response.get(
            "recommendation", "Search for similar high-quality test patterns"
        )
        ...
        quality_impact = self._calculate_rag_quality_impact(
            request.quality_metrics, similar_patterns
        )
    ...
    async def _form_quality_consensus(
        self,
        sage_insights: Dict[str, SageQualityInsight],
        request: QualityLearningRequest,
    ) -> Dict[str, Any]:
        """Form consensus from all sage quality insights"""

        if not sage_insights:
            return {"consensus_reached": False, "reason": "No sage insights available"}
        ...
        if consensus_reached:
            final_recommendation = self._generate_consolidated_quality_recommendation(
                sage_insights
            )
        else:
            final_recommendation = (
                "Sages recommend further analysis - no strong consensus"
            )
    ...
    def _generate_consolidated_quality_recommendation(
        self, sage_insights: Dict[str, SageQualityInsight]
    ) -> str:
        ...
        pattern_mentions = sum(
            1
            for insight in sage_insights.values()
        ...
        workflow_mentions = sum(
            1
            for insight in sage_insights.values()
            if "workflow" in insight.recommendation.lower()
            or "optimize" in insight.recommendation.lower()
        )
    ...
    def _identify_quality_focus_areas(
        self,
        sage_insights: Dict[str, SageQualityInsight],
        request: QualityLearningRequest,
    ) -> List[str]:
        ...
        for sage_name, insight in sage_insights.items():
            if insight.quality_impact > 0.1:  # Significant impact
                ...
                if (
                    sage_focus == "pattern_learning"
                    and "Pattern Compliance" not in focus_areas
                ):
                    focus_areas.append("Pattern Learning")
                elif (
                    sage_focus == "objective_tracking"
                    and "Quality Objectives" not in focus_areas
                ):
                    focus_areas.append("Quality Objectives")
                elif (
                    sage_focus == "issue_prevention"
                    and "Risk Prevention" not in focus_areas
                ):
                    focus_areas.append("Risk Prevention")
                elif (
                    sage_focus == "pattern_discovery"
                    and "Best Practices" not in focus_areas
                ):
                    focus_areas.append("Best Practices")

        return focus_areas[:5]  # Top 5 focus areas

    def _calculate_sage_agreement(
        self, sage_insights: Dict[str, SageQualityInsight]
    ) -> float:
        ...
        impact_variance = sum(
            (i - sum(quality_impacts) / len(quality_impacts)) ** 2
    ...
    async def _identify_quality_patterns(
        self, request: QualityLearningRequest
    ) -> List[Dict]:
    ...
    async def _perform_historical_quality_analysis(
        self, test_file: str
    ) -> Dict[str, Any]:
    ...
    async def _analyze_quality_objectives(
        self, request: QualityLearningRequest
    ) -> List[Dict]:
    ...
    async def _suggest_quality_workflow_optimization(
        self, request: QualityLearningRequest
    ) -> List[str]:
    ...
    async def _assess_quality_risks(
        self, request: QualityLearningRequest
    ) -> List[Dict]:
    ...
    async def _develop_quality_prevention_strategies(
        self, request: QualityLearningRequest
    ) -> List[str]:
    ...
    async def _find_similar_quality_patterns(
        self, request: QualityLearningRequest
    ) -> List[Dict]:
    ...
    async def _discover_quality_best_practices(
        self, request: QualityLearningRequest
    ) -> List[Dict]:
    ...
    def _calculate_knowledge_quality_impact(
        self, metrics: Dict[str, float], patterns: List[Dict]
    ) -> float:
    ...
    def _calculate_task_quality_impact(
        self, metrics: Dict[str, float], objectives: List[Dict]
    ) -> float:
    ...
    def _calculate_incident_quality_impact(
        self, metrics: Dict[str, float], risks: List[Dict]
    ) -> float:
        ...
        risk_impact = sum(
            0.03 if risk["severity"] == "high" else 0.02 for risk in risks
        )

        return min(base_impact + risk_impact, 0.18)  # Cap at 18% improvement

    def _calculate_rag_quality_impact(
        self, metrics: Dict[str, float], patterns: List[Dict]
    ) -> float:
        ...
        similarity_impact = sum(
            0.02 for pattern in patterns if pattern["similarity"] > 0.8
        )
    ...
    async def get_quality_insights_summary(
        ...
        try:
            ...
            return {
                ...
                "session_statistics": {
                    ...
                    "consensus_rate": consensus_sessions / total_sessions
                    if total_sessions > 0
                    else 0,
            ...
            return {"error": str(e), "summary_timestamp": datetime.now().isoformat()}

    def _assess_integration_health(
        self, sage_participation: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if sage_participation:
            avg_confidence = sum(
                data["avg_confidence"] for data in sage_participation.values()
            ) / len(sage_participation)
            avg_impact = sum(
                data["avg_quality_impact"] for data in sage_participation.values()
            ) / len(sage_participation)
        else:
            avg_confidence = 0.0
            avg_impact = 0.0
        ...
        return {
            ...
            "recommendations": self._get_integration_health_recommendations(
                health_status
            ),
        }
...
async def consult_sages_for_test_quality(
    test_file: str,
    quality_metrics: Dict[str, float],
