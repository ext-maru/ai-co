...
class FourSagesMCPCLI:
    ...
    async def task_sage_management(
        self, task_description: str, priority: str = "normal"
    ):
        """タスク賢者管理"""
        if not await self.initialize():
            return
    ...
    async def incident_sage_monitoring(
        self, incident_description: str, severity: str = "normal"
    ):
        """インシデント賢者監視"""
        if not await self.initialize():
            return
    ...
    async def show_status(self):
        ...
        print(
            f"🔗 MCP接続: {'✅ 接続済み' if integration_status['mcp_connected'] else '❌ 未接続'}"
        )
        print(
            f"🧙‍♂️ 賢者統合: {'✅ 完了' if integration_status['sages_integrated'] else '❌ 未完了'}"
        )
    ...
    async def run_cli():
        ...
        try:
            if args.status:
                ...
                print(
                    "   python3 scripts/four_sages_mcp_cli.py --sage knowledge --search '4賢者'"
                )
                print(
                    "   python3 scripts/four_sages_mcp_cli.py --collaborative-analysis 'システム統合'"
                )
