    ...
    def get_issue_with_comments(self, issue_number: int) -> Dict:
        ...
        try:
            ...
            issue_result = subprocess.run([
                ...
                '--json', 'title,body,number,state,labels,assignees,author,createdAt'
    ...
    def display_issue_dashboard(self, issue_number: int):
        ...
        if issue['comments']:
            ...
            for i, comment in enumerate(issue['comments'], 1):
                ...
                # 進捗キーワードをハイライト
                if any(keyword in comment['body'] for keyword in ['完了', '進行中', '開始', '%']):
                    print("📈 進捗情報あり")
        else:
            print("\n💬 コメントなし")
    ...
    def get_master_issue_progress(self, master_issue_number: int) -> Dict:
            ...
            try:
                ...
                if sub_issue_num != master_issue_number:  # Master Issue自体を除外
                    ...
                    if sub_issue:
                        ...
                        elif '進行中' in all_text or 'in-progress' in str(sub_issue.get('labels', [])):
        ...
        if progress_data['total_sub_issues'] > 0:
            progress_data['progress_percentage'] = (
                progress_data['completed_sub_issues'] / progress_data['total_sub_issues'] * 100
            )
