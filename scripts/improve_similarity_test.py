    ...
    try:
        ...
        for i, text in enumerate(detailed_texts):
            ...
            """,
                ...
                text,
                str(embedding),
                json.dumps({"type": "detailed", "sage_type": ["knowledge", "task", "incident", "rag"][i] if i < 4 else "general"})
            )
        ...
        print(f"クエリ: '{query}'")
        for row in results:
            print(f"  {row['title']} (類似度: {row['similarity']:.4f}): {row['content']}")
        ...
        def preprocess_text(text):
            # キーワードを強調
            keywords = ["4賢者", "エルダーズギルド", "ナレッジ賢者", "タスク賢者", "インシデント賢者", "RAG賢者"]
        ...
        for i, row in enumerate(all_results):
            print(f"{i+1}. {row['title']} (タイプ: {row['doc_type']}) - 類似度: {row['similarity']:.4f}")
