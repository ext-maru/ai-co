...
class KnightsGitHubAction:
    ...
    def _check_syntax_errors(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = subprocess.run(
                ['pylint', '--errors-only', '--output-format=json', str(self.project_root)],
                capture_output=True,
                text=True
            )
                ...
                for error in pylint_errors:
                    if error['type'] == 'error':
                        errors.append({
                            ...
                            'auto_fixable': self._is_syntax_fixable(error['message']),
                            'fix_action': 'syntax_fix'
                        })
        except subprocess.CalledProcessError as e:
            print(f"Warning: pylint not found or failed. Installing required dependencies...")
    ...
    def _check_import_errors(self) -> List[Dict[str, Any]]:
        ...
        for py_file in py_files:
            try:
                ...
                if 'datetime' in content and 'import datetime' not in content and 'from datetime' not in content:
                    errors.append({
                        'file': str(py_file),
                        'line': 0,
    ...
    def _check_security_issues(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"Warning: bandit not found or failed. Installing required dependencies...")
            print(f"Error details: {e}")
    ...
    def _check_test_failures(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = subprocess.run(
                ['pytest', 'tests/unit/', '--tb=short', '--json-report', '--json-report-file=test_report.json'],
                capture_output=True,
                text=True
            )
            ...
            print(f"Warning: pytest not found or failed. Installing required dependencies...")
            print(f"Error details: {e}")
    ...
    def _run_formatters(self, files: List[str]) -> None:
        ...
        try:
            ...
            print(f"Warning: black not found. Make sure it's installed via requirements.txt")
            print(f"Error details: {e}")
        ...
        try:
            ...
            print(f"Warning: isort not found. Make sure it's installed via requirements.txt")
            print(f"Error details: {e}")
        except FileNotFoundError:
...
def main():
    ...
    fix_parser.add_argument('--fix-type', choices=['all', 'syntax', 'imports', 'tests', 'security'], default='all')
