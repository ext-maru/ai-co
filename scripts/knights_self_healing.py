...
class IncidentKnightsSelfHealing:
        ...
        try:
            ...
            for import_name in imports:
                if not self._can_import(import_name):
                    self._add_issue("import_error", str(file_path), f"Cannot import {import_name}")
                    if self.auto_fix:
    ...
    async def _fix_syntax_error(self, file_path: Path, content: str, error: SyntaxError):
        """構文エラーを修正"""
    ...
    async def _fix_import_error(self, module_name: str):
        ...
        if module_name in self.package_mapping:
            ...
            try:
                ...
                self._add_fixed("import_error", module_name, f"パッケージをインストール: {package_name}")
            ...
            for part in parts[:-1]:
                ...
                if not init_file.exists():
                    with open(init_file, "w") as f:
                        f.write(f'"""{part} package - Auto-generated by Incident Knights"""\n')
...
'''
    ...
    async def _check_file_attributes(self, file_path: Path):
        ...
        if file_path.name.startswith("ai-") or file_path.parent.name == "scripts":
            ...
            if not first_line.startswith("#!"):
                self._add_issue("missing_shebang", str(file_path), "実行可能ファイルにシェバングがありません")
                if self.auto_fix:
                    await self._fix_shebang(file_path)

            # 実行権限チェック
            if not os.access(file_path, os.X_OK):
                self._add_issue("missing_execute_permission", str(file_path), "実行権限がありません")
                if self.auto_fix:
                    file_path.chmod(file_path.stat().st_mode | 0o111)
                    self._add_fixed("missing_execute_permission", str(file_path), "実行権限を付与")
    ...
    async def _check_environment_variables(self):
        """環境変数をチェック"""
        required_vars = ["WORKER_DEV_MODE", "INCIDENT_KNIGHTS_ENABLED", "AUTO_FIX_ENABLED"]
    ...
    async def _check_dependencies(self):
        ...
        if not requirements_file.exists():
            self._add_issue("missing_requirements", "requirements.txt", "依存関係ファイルがありません")
            if self.auto_fix:
                # 基本的な依存関係を作成
                basic_deps = [
                    "pytest>=7.0.0",
                ...
                self._add_fixed("missing_requirements", "requirements.txt", "依存関係ファイルを作成")
    ...
    def _add_fixed(self, issue_type: str, location: str, fix_description: str):
        ...
        self.issues_fixed.append(
            {"type": issue_type, "location": location, "fix": fix_description, "timestamp": datetime.now().isoformat()}
        )
    ...
    def _generate_report(self) -> Dict[str, any]:
        ...
        report = {
            ...
            "fix_rate": len(self.issues_fixed) / len(self.issues_found) if self.issues_found else 0,
        ...
        report_file = report_dir / f"scan_report_{self.start_time.strftime('%Y%m%d_%H%M%S')}.json"
...
async def main():
    ...
    # 自己修復システムを初期化
    healing = IncidentKnightsSelfHealing(auto_fix=args.auto_fix, batch_mode=args.batch_mode)
