...
class BaselineMetrics:
    ...
    def analyze_complexity(self, filepath):
        ...
        try:
            ...
            complexity_indicators = {
                "if_statements": len([1 for line in content.split('\n') if 'if ' in line]),
                "for_loops": len([1 for line in content.split('\n') if 'for ' in line]),
                "while_loops": len([1 for line in content.split('\n') if 'while ' in line]),
                "try_blocks": len([1 for line in content.split('\n') if 'try:' in line]),
    ...
    def analyze_dependencies(self, filepath):
        ...
        try:
            ...
            # 標準ライブラリとサードパーティを分離
            stdlib_modules = {'os', 'sys', 'json', 'time', 'datetime', 'pathlib', 'logging', 
                            'asyncio', 'typing', 'collections', 'functools', 're', 'uuid'}
            
            stdlib_imports = [imp for imp in imports if imp.split('.')[0] in stdlib_modules]
            third_party_imports = [imp for imp in imports if imp.split('.')[0] not in stdlib_modules 
            ...
            return {
                ...
                "unique_dependencies": len(set(imp.split('.')[0] for imp in third_party_imports))
            }
        except Exception as e:
            return {"error": str(e)}
    ...
    def collect_all_metrics(self):
        ...
        # サマリー統計
        total_lines = sum(m["lines"]["total_lines"] for m in self.metrics.values() if "total_lines" in m.get("lines", {}))
        total_code_lines = sum(m["lines"]["code_lines"] for m in self.metrics.values() if "code_lines" in m.get("lines", {}))
        total_classes = sum(m["structure"]["classes"] for m in self.metrics.values() if "classes" in m.get("structure", {}))
        total_functions = sum(m["structure"]["functions"] for m in self.metrics.values() if "functions" in m.get("structure", {}))
        total_tests = sum(m["tests"]["test_functions"] for m in self.metrics.values() if "test_functions" in m.get("tests", {}))
        ...
        self.metrics["summary"] = {
            ...
            "test_coverage_estimate": f"{(total_tests / (total_classes + total_functions) * 100):.1f}%" if (total_classes + total_functions) > 0 else "0%"
        }
    ...
    def print_summary(self):
        ...
        for filepath, metrics in self.metrics.items():
            ...
            print(f"   行数: {lines.get('total_lines', 0)} (コード: {lines.get('code_lines', 0)})")
            print(f"   構造: クラス {structure.get('classes', 0)}, 関数 {structure.get('functions', 0)}, メソッド {structure.get('methods', 0)}")
            print(f"   テスト: {'✅' if tests.get('test_file_exists') else '❌'} {tests.get('test_functions', 0)} 個")
