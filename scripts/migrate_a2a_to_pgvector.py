...
class A2APgVectorMigration:
        ...
        for i in range(0, len(communications), self.batch_size):
            ...
            if insert_data:
                try:
                    ...
                    logger.info(f"Migrated {len(insert_data)} communications (total: {self.stats['migrated_communications']})")
    ...
    def generate_migration_report(self) -> Dict[str, Any]:
        ...
        if self.stats["errors"]:
            report["recommendations"].append(f"{len(self.stats['errors'])}個のエラーが発生しました。ログを確認してください")
...
def main():
    ...
    try:
        ...
        print("\n📊 Migration Summary")
        print("-" * 40)
        print(f"Communications migrated: {report['statistics']['migrated_communications']}/{report['statistics']['total_communications']}")
        print(f"Anomaly patterns migrated: {report['statistics']['migrated_anomalies']}/{report['statistics']['total_anomalies']}")
