...
class WorkerMigrationManager:
    ...
    def _load_migration_state(self) -> Dict:
        ...
        return {
            'phase': 'not_started',
            'workers': {name: {'migrated': False, 'last_attempt': None} 
    ...
    def start_async_worker(self, worker_name: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Starting async worker: {worker_name}", command=' '.join(cmd))
            ...
            if proc.poll() is None:
                self.logger.info(f"Async worker {worker_name} started successfully", pid=proc.pid)
                return True
            else:
                stdout, stderr = proc.communicate()
                self.logger.error(f"Async worker {worker_name} failed to start",
                                stdout=stdout.decode()[:500],
                                stderr=stderr.decode()[:500])
                return False
                
        except Exception as e:
            self.logger.error(f"Failed to start async worker {worker_name}", error=str(e))
            return False
    
    def migrate_worker(self, worker_name: str, traffic_percentage: int = 100) -> bool:
        """単一ワーカーの移行"""
        self.logger.info(f"Migrating worker: {worker_name}", traffic_percentage=traffic_percentage)
        ...
        try:
            if traffic_percentage == 100:
                ...
                self.logger.info(f"Partial migration: {traffic_percentage}% traffic to new worker")
    ...
    def migrate_all(self, traffic_percentage: int = 100) -> bool:
        """全ワーカーの移行（優先度順）"""
        self.logger.info("Starting full migration", traffic_percentage=traffic_percentage)
        ...
        workers_by_priority = sorted(
            self.legacy_workers.items(),
            key=lambda x: x[1]['priority']
        )
        
        success_count = 0
        for worker_name, config in workers_by_priority:
            self.logger.info(f"Migrating {worker_name} (priority: {config['priority']})")
            ...
            if self.migrate_worker(worker_name, traffic_percentage):
                ...
                self.logger.error(f"Migration failed for {worker_name}, stopping full migration")
                break
        ...
        if success_count == len(self.legacy_workers):
            ...
            self.logger.error(f"Migration partially failed: {success_count}/{len(self.legacy_workers)} workers")
            result = False
...
def main():
    ...
    if args.action == 'status':
        ...
        if not args.force:
            confirm = input(f"Migrate {args.worker} with {args.traffic}% traffic? (y/N): ")
            if confirm.lower() != 'y':
        ...
        if not args.force:
            confirm = input(f"Migrate all workers with {args.traffic}% traffic? (y/N): ")
            if confirm.lower() != 'y':
                print("Migration cancelled")
                sys.exit(0)
