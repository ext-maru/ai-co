...
class MonitoringDashboard:
        ...
        try:
            ...
            for worker_name, processes in worker_processes.items():
                worker_stats[worker_name] = {
                    ...
                    'avg_cpu_percent': sum(p['cpu_percent'] for p in processes) / len(processes) if processes else 0,
                    'avg_memory_mb': sum(p['memory_mb'] for p in processes) / len(processes) if processes else 0,
                    'processes': processes
                }
    ...
    def check_alerts(self, system_metrics: Dict, worker_metrics: Dict, queue_metrics: Dict):
        """アラートチェック"""
        current_time = datetime.utcnow()
        new_alerts = []
        
        # CPU使用率チェック
        if system_metrics.get('cpu', {}).get('percent', 0) > self.alert_thresholds['cpu_percent']:
            new_alerts.append({
                'type': 'cpu_high',
                'severity': 'warning',
        ...
        # メモリ使用率チェック
        if system_metrics.get('memory', {}).get('percent', 0) > self.alert_thresholds['memory_percent']:
            new_alerts.append({
                'type': 'memory_high',
                'severity': 'warning',
        ...
        # ディスク使用率チェック
        if system_metrics.get('disk', {}).get('percent', 0) > self.alert_thresholds['disk_percent']:
            new_alerts.append({
                'type': 'disk_high',
                'severity': 'critical',
        ...
        for worker in expected_workers:
            if worker not in worker_metrics or worker_metrics[worker]['process_count'] == 0:
                new_alerts.append({
                    'type': 'worker_down',
                    'severity': 'critical',
    ...
    def update_metrics_history(self, system_metrics: Dict, worker_metrics: Dict):
        ...
        # システムメトリクス追加
        self.metrics_history['cpu'].append(system_metrics.get('cpu', {}).get('percent', 0))
        self.metrics_history['memory'].append(system_metrics.get('memory', {}).get('percent', 0))
        self.metrics_history['disk'].append(system_metrics.get('disk', {}).get('percent', 0))
        
        # ワーカー数追加
        total_workers = sum(stats.get('process_count', 0) for stats in worker_metrics.values())
    ...
    def generate_dashboard_data(self) -> Dict[str, Any]:
        ...
        dashboard_data = {
            ...
            'alerts': {
                'active': [a for a in self.alerts if a['timestamp'] > (datetime.utcnow() - timedelta(hours=1)).isoformat()],
                'recent': new_alerts,
                'total_count': len(self.alerts)
            },
            'history': {
                'last_24h': {
                    'timestamps': self.metrics_history['timestamps'][-144:],  # 10分間隔で24時間
            ...
            'summary': {
                'total_workers': sum(stats.get('process_count', 0) for stats in worker_metrics.values()),
                'active_alerts': len([a for a in self.alerts if a['timestamp'] > (datetime.utcnow() - timedelta(hours=1)).isoformat()]),
                'system_health': 'healthy' if not new_alerts else 'warning' if any(a['severity'] == 'warning' for a in new_alerts) else 'critical'
            }
        }
    ...
    def print_dashboard(self, data: Dict[str, Any]):
        ...
        for worker_name, stats in data.get('workers', {}).items():
            ...
            print(f"  {status} {worker_name}: {stats['process_count']} processes, "
        ...
        for queue_name, stats in data.get('queues', {}).items():
            ...
            print(f"  {status} {queue_name}: {stats['messages']} messages, "
