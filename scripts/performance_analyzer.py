...
class PerformanceAnalyzer:
    ...
    def get_process_info(
        ...
        # (CPU数: {cpu_count})\")\n \n # Claude プロセス多重起動チェック\n claude_processes = [p for p in top_cpu + top_memory if
        # 'claude' in p.name.lower()]\n if len(claude_processes) > 3:\n bottlenecks.append(f\"Claude プロセス多重起動:
        # {len(claude_processes)}個\")\n \n self.logger.info(f\"🔍 ボトルネック検出: {len(bottlenecks)}個\")\n return
        ...
        # recommendations.append(\"CPU負荷軽減: 不要なプロセス停止、非同期処理の活用\")\n \n high_cpu_elders = [p for p in elder_processes if
        # p.cpu_percent > 10]\n if high_cpu_elders:\n recommendations.append(f\"エルダーズプロセス最適化:
        # {len(high_cpu_elders)}個のプロセスがCPU負荷高\")\n \n # メモリ最適化\n if metrics.memory_percent > 75:\n
        # recommendations.append(\"メモリ使用量削減: キャッシュクリア、メモリリーク調査\")\n \n high_mem_elders = [p for p in elder_processes if
        # p.memory_mb > 100]\n if high_mem_elders:\n recommendations.append(f\"エルダーズメモリ最適化:
        # {len(high_mem_elders)}個のプロセスがメモリ消費大\")\n \n # プロセス最適化\n sleeping_processes = [p for p in elder_processes if
        # p.status == 'sleeping' and p.cpu_percent < 0.1]\n if len(sleeping_processes) > 5:\n
        # recommendations.append(f\"休眠プロセス整理: {len(sleeping_processes)}個の低活動プロセス\")\n \n # Claude多重起動対策\n claude_count
        # = len([p for p in elder_processes if 'claude' in p.name.lower()])\n if claude_count > 2:\n
        ...
        # recommendations.append(\"データベース最適化: インデックス再構築、古いデータアーカイブ\")\n \n # 監視システム\n recommendations.append(\"継続監視設定:
        # Performance Analyzer の定期実行とアラート設定\")\n \n self.logger.info(f\"💡 推奨事項: {len(recommendations)}項目生成\")\n return
        # recommendations\n \n def calculate_health_score(self, metrics: SystemMetrics, bottlenecks: List[str]) ->
        ...
        # self.detect_bottlenecks(metrics, top_cpu, top_memory)\n \n # 推奨事項生成\n recommendations =
        # self.generate_recommendations(metrics, bottlenecks, elder_processes)\n \n # ヘルススコア計算\n health_score =
        # self.calculate_health_score(metrics, bottlenecks)\n \n analysis = PerformanceAnalysis(\n
        ...
        # f\"performance_analysis_{timestamp}.json\"\n \n # JSON形式で保存\n with open(report_file, 'w') as f:\n
        # json.dump(asdict(analysis), f, indent=2, ensure_ascii=False)\n \n self.logger.info(f\"📊 分析レポート保存:
        ...
        # action=\"store_true\", help=\"総合パフォーマンス分析実行\")\n parser.add_argument(\"--metrics\", action=\"store_true\",
