...
class PostgreSQLMCPIntegrationTest:
    ...
    async def test_mcp_interface_simulation(self):
        ...
        try:
            ...
            class PostgreSQLMCPInterface:
                ...
                async def search_with_context(self, query: str, context: str = None) -> List[Dict]:
                    """コンテキスト付き検索"""
            ...
            assert best_match['similarity'] > 0.1, f"類似度不足: {best_match['similarity']}"
    ...
    async def test_fallback_mechanism(self):
        ...
        try:
                ...
                async def search(self, query: str, use_fallback=False) -> List[Dict]:
                    ...
                    if not use_fallback:
                        try:
                            ...
                            # 結果が不十分な場合はフォールバック
                            if not results or max(r['similarity'] for r in results) < 0.3:
            ...
            # フォールバック強制テスト
            fallback_results = await hybrid_system.search("4賢者について", use_fallback=True)
    ...
    async def test_data_integrity(self):
        ...
        try:
            ...
            self.test_results.append({
                'test': test_name,
                'status': 'PASS',
                'message': f'データ整合性OK - 総件数: {integrity_check["total_count"]}, 平均長: {quality_check["avg_length"]:.0f}'
            })
            print(f"✅ {test_name} - 成功")
    ...
    async def test_performance_benchmark(self):
        ...
        try:
            ...
            avg_similarity = sum(float(r['similarity']) for r in all_results) / len(all_results)
