...
def run_integration_tests():
    ...
    integration_test = PROJECT_ROOT / "tests/integration/test_elder_worker_integration.py"
    
    if integration_test.exists():
        # カバレッジ計測付きで実行
        cov = coverage.Coverage(source=[str(PROJECT_ROOT / "workers"), str(PROJECT_ROOT / "libs")])
        cov.start()
...
def run_security_tests():
    ...
    from libs.unified_auth_provider import create_demo_auth_system, AuthRequest, ElderRole
...
def generate_test_report(unit_results, integration_result, security_results):
    ...
    # JSON形式で保存
    report_file = RESULTS_DIR / f"elder_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    print(f"Integration Test: {'PASSED' if integration_result.get('passed', False) else 'FAILED'}")
    ...
    # 全体の成功/失敗判定
    all_passed = (
        unit_passed == len(unit_results) and
        integration_result.get("passed", False) and
        security_passed == len(security_results)
    )
