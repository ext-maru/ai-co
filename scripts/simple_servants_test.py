...
""",
        ...
        if is_valid:
            ...
            if success:
                doc_content = response.data.get("documentation", "")
                has_content = len(doc_content) > 100
                print(f"  Documentation generated: {'✅' if has_content else '❌'}")
                print(f"  Quality score: {response.metrics.get('quality_score', 'N/A')}")
        ...
        return is_valid and (response.status == "success" if is_valid else False)
