...
# オーケストレータでヘルスチェック
python3 -m libs.quality.quality_pipeline_orchestrator health || echo -e "${YELLOW}⚠️ Health check requires python-a2a server implementation${NC}"

echo ""
echo -e "${GREEN}🎉 Quality Pipeline System Ready!${NC}"
echo "Run quality check: python3 -m libs.quality.quality_pipeline_orchestrator run --path /path/to/project"
