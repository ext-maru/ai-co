...
class SystemRecoveryOrchestrator:
    ...
    async def _execute_api_integration_phase(self):
        ...
        try:
            ...
            self.recovery_session["phases"]["api_integration"]["issues"] = len(api_issues)
    ...
    async def _execute_worker_stabilization_phase(self):
        ...
        self.recovery_session["phases"]["worker_stabilization"]["status"] = "in_progress"
        
        try:
            # ワーカー問題の検出
            worker_issues = await self.worker_knight.patrol()
            self.recovery_session["phases"]["worker_stabilization"]["issues"] = len(worker_issues)
            self.recovery_session["total_issues"] += len(worker_issues)
            
            logger.info(f"🔍 ワーカー問題検出: {len(worker_issues)}件")
            
            # 重要度順に修復実行
            sorted_issues = sorted(worker_issues, 
            ...
            self.recovery_session["phases"]["worker_stabilization"]["fixed"] = fixed_count
            self.recovery_session["phases"]["worker_stabilization"]["status"] = "completed"
            self.recovery_session["total_fixed"] += fixed_count
    ...
    async def _execute_system_verification_phase(self):
        ...
        try:
            ...
            self.recovery_session["phases"]["system_verification"]["tests"] = total_tests
            self.recovery_session["phases"]["system_verification"]["passed"] = passed_tests
            self.recovery_session["phases"]["system_verification"]["status"] = "completed"
    ...
    async def _generate_final_report(self) -> Dict[str, Any]:
        ...
        if self.recovery_session["total_issues"] > 0:
            self.recovery_session["success_rate"] = self.recovery_session["total_fixed"] / self.recovery_session["total_issues"]
        ...
        final_report = {
            ...
            "summary": {
                ...
                "overall_status": "success" if self.recovery_session["success_rate"] > 0.7 else "partial"
            },
        ...
        # 報告ファイル保存
        report_file = self.reports_dir / f"{self.recovery_session['session_id']}_final_report.json"
    ...
    async def _generate_error_report(self, error_message: str) -> Dict[str, Any]:
        ...
        # エラー報告ファイル保存
        error_file = self.reports_dir / f"{self.recovery_session['session_id']}_error_report.json"
    ...
    async def _generate_recommendations(self) -> List[str]:
        ...
        if api_phase["status"] == "completed" and api_phase["fixed"] < api_phase["issues"]:
            recommendations.append("APIキーの手動設定確認が必要です")
        
        # ワーカー安定化の結果に基づく推奨
        worker_phase = self.recovery_session["phases"]["worker_stabilization"]
        if worker_phase["status"] == "completed" and worker_phase["fixed"] < worker_phase["issues"]:
