...
from libs.weapon_sharing_system import (
    initialize_weapon_sharing, shared_inventory, 
    weapon_coordinator, WeaponType, WeaponRequest, RequestPriority
)
...
def test_weapon_sharing():
    ...
    for weapon_type, count in initial_weapons:
        result = crafting_engine.provide_weapon_to_knights("inventory", weapon_type, count)
    ...
    for weapon_id, weapon_data in knight_squad.equipment.items():
        print(f"     • {weapon_id}: {weapon_data['weapon_type']} "
