    ...
    def _validate_module(self, module_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module, inspect.isclass):
                ...
                if any(base.__name__ in ['BaseWorker', 'BaseManager'] for base in obj.__bases__):
            ...
            if base.__name__ in required_methods:
                for method_name in required_methods[base.__name__]:
                    if not hasattr(cls, method_name):
                        results['errors'].append(f"{cls.__name__}: 必須メソッド {method_name} が未実装")
                    elif method_name not in cls.__dict__:
                        results['warnings'].append(f"{cls.__name__}: {method_name} が継承元のデフォルト実装を使用")
