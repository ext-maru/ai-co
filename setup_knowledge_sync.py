...
class KnowledgeSyncSystem:
    ...
    def __init__(self):
        ...
        # プロジェクトナレッジテンプレート
        self.template_path = PROJECT_ROOT / "projects" / "sample-project" / "PROJECT_KNOWLEDGE.md"
        ...
        if projects_dir.exists():
            for item in projects_dir.iterdir():
                if item.is_dir() and not item.name.startswith('.'):
                    # .gitやキャッシュディレクトリを除外
                    if (item / ".git").exists() or (item / "package.json").exists() or (item / "setup.py").exists():
...
"""
    ...
    def analyze_project(self, project_path: Path) -> Dict[str, Any]:
        ...
        if not info["main_tech"]:
            info["main_tech"] = info["tech_stack"][0] if info["tech_stack"] else "[主要技術]"
    ...
    async def analyze_knowledge_patterns(self, knowledge_file: Path) -> List[Dict[str, Any]]:
        """知識パターンの分析"""
        ...
        try:
            ...
            if "WebSocket" in content:
                patterns.append({
                    "pattern": "WebSocket通信パターン",
                    "usage_count": 4  # 仮の値
                })

            if "エラーハンドリング" in content:
                patterns.append({
                    "pattern": "エラーハンドリングパターン",
                    "usage_count": 5
                })
    ...
    async def generate_sync_report(self, sync_results: Dict[str, Any]):
        """同期レポートの生成"""
        report_path = PROJECT_ROOT / "knowledge_base" / "sync_reports" / f"sync_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
