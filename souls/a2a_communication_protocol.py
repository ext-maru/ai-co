...
from souls.base_soul import SoulIdentity, SoulRequest, SoulResponse, ElderType
...
class MessageRouter:
    ...
    def register_soul(self, node: SoulNode):
        ...
        with self.routing_lock:
            ...
            elif node.identity.elder_type in [ElderType.GRAND_ELDER, ElderType.CLAUDE_ELDER, ElderType.ANCIENT_ELDER]:
                self.groups["elders"].add(node.soul_id)
        
        self.logger.info(f"🔗 Soul node registered: {node.soul_id} ({node.identity.elder_type.value})")
    ...
    def find_best_soul_for_capability(self, capability: str, exclude: Set[str] = None) -> Optional[str]:
        """能力に基づく最適魂の探索"""
    ...
    def get_routing_statistics(self) -> Dict[str, Any]:
        ...
        with self.routing_lock:
            ...
            return {
                ...
                "active_souls": len([n for n in self.routing_table.values() if n.status == "active"]),
...
class A2ACommunicationProtocol:
    ...
    async def start_protocol(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"🌟 A2A Protocol started for {self.soul_identity.soul_id} on {self.host}:{self.port}")
            return True
    ...
    async def _process_received_message(self, message_data: str):
        ...
        try:
            ...
            if handler:
                ...
                self.logger.warning(f"⚠️ No handler for message type: {message.message_type}")
    ...
    async def _send_multicast_message(self, message: A2AMessage) -> bool:
        ...
        self.logger.info(f"📡 Multicast sent to {success_count}/{len(group_members)} souls in group {message.target_group}")
        return success_count > 0
            ...
            try:
                ...
                heartbeat_message = A2AMessage(
                    ...
                    payload={
                        ...
                        "load": self.router.routing_table[self.soul_identity.soul_id].current_load,
                        "status": "active"
                    }
                )
    ...
    async def _handle_request(self, message: A2AMessage):
        """リクエストハンドラー"""
        self.logger.info(f"📨 Received request from {message.sender_soul_id}: {message.payload.get('type', 'unknown')}")
    ...
    async def _handle_broadcast(self, message: A2AMessage):
        """ブロードキャストハンドラー"""
        self.logger.info(f"📡 Broadcast from {message.sender_soul_id}: {message.payload.get('announcement', '')}")
    
    async def _handle_error(self, message: A2AMessage):
        """エラーハンドラー"""
        self.logger.error(f"💥 Error from {message.sender_soul_id}: {message.payload.get('error', 'Unknown error')}")
    ...
    async def send_request(self, target_soul_id: str, request_type: str, payload: Dict[str, Any], requires_response: bool = True) -> Optional[A2AMessage]:
        """リクエスト送信"""
        message = A2AMessage(
            message_id=str(uuid.uuid4()),
    ...
    async def send_to_group(self, group_name: str, message_type: MessageType, payload: Dict[str, Any]):
        """グループメッセージ送信"""
        message = A2AMessage(
            message_id=str(uuid.uuid4()),
            message_type=message_type,
...
async def create_a2a_protocol(soul_identity: SoulIdentity, port: int = 0) -> A2ACommunicationProtocol:
    """A2A通信プロトコルの作成"""
...
async def test_a2a_communication():
    """A2A通信のテスト"""
    from souls.base_soul import create_soul_identity, SoulCapability
    ...
    identity2 = create_soul_identity(
        "Test Soul 2", 
        ElderType.SAGE,
        [SoulCapability.WISDOM, SoulCapability.ANALYSIS]
    )
    ...
    response = await protocol1.send_request(
        identity2.soul_id,
        "test_request",
        {"message": "Hello from Claude Elder!"}
    )
