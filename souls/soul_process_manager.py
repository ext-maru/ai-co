...
class SoulProcessManager:
    ...
    async def start_manager(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"🏛️ Soul Process Manager started (max_processes: {self.max_processes})")
            return True
    ...
    def register_soul(
        ...
        with self.manager_lock:
            ...
            if not self._can_create_soul(soul_identity.elder_type):
                raise ValueError(f"Cannot create more souls of type {soul_identity.elder_type.value}")
            ...
            self.logger.info(f"🌟 Soul registered: {soul_identity.soul_name} ({soul_identity.soul_id})")
    ...
    async def start_soul(self, soul_id: str) -> bool:
        ...
        with self.manager_lock:
            ...
            try:
                ...
                a2a_protocol = await create_a2a_protocol(soul_identity, process_info.a2a_port)
                self.a2a_protocols[soul_id] = a2a_protocol
                
                # 魂プロセス起動
                process = mp.Process(
                    target=self._soul_process_wrapper,
                    args=(soul_identity, process_info.configuration, process_info.a2a_port),
                ...
                self.logger.info(f"✨ Soul started: {process_info.soul_name} (PID: {process.pid})")
                return True
    ...
    async def restart_soul(self, soul_id: str) -> bool:
        ...
        with self.manager_lock:
            ...
            if success:
                ...
                self.logger.info(f"🔄 Soul restarted: {process_info.soul_name} (restart #{process_info.restart_count})")
    ...
    async def start_all_souls(self) -> Dict[str, bool]:
        ...
        # Elder Tree階層順で起動
        for elder_type in [ElderType.GRAND_ELDER, ElderType.CLAUDE_ELDER, ElderType.ANCIENT_ELDER, 
                          ElderType.SAGE, ElderType.KNIGHT, ElderType.SERVANT]:
            souls_to_start = [
                soul_id for soul_id, info in self.processes.items()
                if info.elder_type == elder_type and info.state != SoulProcessState.RUNNING
            ]
    ...
    async def stop_all_souls(self) -> Dict[str, bool]:
        ...
        # 逆階層順で停止（下位から停止）
        for elder_type in [ElderType.SERVANT, ElderType.KNIGHT, ElderType.SAGE,
                          ElderType.ANCIENT_ELDER, ElderType.CLAUDE_ELDER, ElderType.GRAND_ELDER]:
            souls_to_stop = [
                soul_id for soul_id, info in self.processes.items()
                if info.elder_type == elder_type and info.state == SoulProcessState.RUNNING
            ]
    ...
    def get_all_souls_status(self) -> Dict[str, Dict[str, Any]]:
        """全魂ステータス取得"""
        return {
            soul_id: self.get_soul_status(soul_id)
            for soul_id in self.processes.keys()
        }
    
    def get_manager_statistics(self) -> Dict[str, Any]:
        """管理システム統計取得"""
        with self.manager_lock:
            running_count = len([p for p in self.processes.values() if p.state == SoulProcessState.RUNNING])
            crashed_count = len([p for p in self.processes.values() if p.state == SoulProcessState.CRASHED])
            
            elder_type_counts = {}
            for elder_type in ElderType:
                count = len([p for p in self.processes.values() if p.elder_type == elder_type])
                        ...
                        if process_info.state == SoulProcessState.RUNNING:
                            ...
                            if soul_id in self.running_processes:
                                ...
                                if not process.is_alive():
                                    # プロセス死亡検出
                                    self.logger.warning(f"💀 Process death detected: {soul_id}")
                                    process_info.state = SoulProcessState.CRASHED
                                    process_info.crash_count += 1
                                    self.stats["total_crashes"] += 1
                        ...
                        if health_response:
                            ...
                            if soul_id in self.processes:
                                ...
                                if process_info.state == SoulProcessState.RUNNING:
                                    ...
                                    self.logger.warning(f"⚠️ Soul unresponsive: {soul_id}")
                                    
                    except Exception as e:
                        self.logger.warning(f"⚠️ Health check failed for {soul_id}: {e}")
    ...
    async def _statistics_loop(self):
        ...
        while self.is_running.value:
            try:
                # システムリソース統計更新
                for soul_id, process_info in self.processes.items():
                    if process_info.process_id and process_info.state == SoulProcessState.RUNNING:
                        try:
                            psutil_process = psutil.Process(process_info.process_id)
    ...
    def _can_create_soul(self, elder_type: ElderType) -> bool:
        ...
        current_count = len([p for p in self.processes.values() if p.elder_type == elder_type])
        return current_count < hierarchy_info["max_instances"]
    
    def _allocate_a2a_port(self) -> int:
        """A2Aポート割り当て"""
        # 簡単な実装：9000番台から順次割り当て
        used_ports = {info.a2a_port for info in self.processes.values() if info.a2a_port}
    ...
    def _create_soul_identity_from_info(self, process_info: SoulProcessInfo) -> SoulIdentity:
        """プロセス情報から魂アイデンティティを作成"""
        capabilities = [SoulCapability(cap) for cap in process_info.capabilities if cap in [c.value for c in SoulCapability]]
    ...
    @staticmethod
    def _soul_process_wrapper(soul_identity: SoulIdentity, configuration: Dict[str, Any], a2a_port: int):
        """魂プロセスラッパー（プロセス内実行）"""
        try:
            # 新しいプロセスでのロガー設定
            process_logger = get_logger(f"soul_wrapper_{soul_identity.soul_id}")
            
            process_logger.info(f"👑 Soul process starting: {soul_identity.soul_name} (PID: {os.getpid()})")
...
def create_standard_elder_tree() -> List[SoulIdentity]:
    ...
    # Grand Elder Maru
    souls.append(create_soul_identity(
        "Grand Elder Maru",
        ElderType.GRAND_ELDER,
        [SoulCapability.LEADERSHIP, SoulCapability.WISDOM, SoulCapability.PROBLEM_SOLVING]
    ))
    ...
    souls.append(create_soul_identity(
        ...
        [SoulCapability.LEADERSHIP, SoulCapability.COMMUNICATION, SoulCapability.ANALYSIS]
    ))
    ...
    servant_configs = [
        ...
        ("Quality Inspector", [SoulCapability.QUALITY_ASSURANCE, SoulCapability.ANALYSIS]),
