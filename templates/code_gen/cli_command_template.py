...
'''
    ...
    def _generate_subcommand(self, subcmd: Dict[str, Any], is_async: bool = False) -> str:
        """サブコマンドを生成"""
        ...
        decorator = "cli.command()" if not is_async else "cli.command()\n@click.coroutine"
...
def run(ctx, '''
        ...
        param_names.extend([opt.get("name", "opt").replace("-", "_") for opt in options])
        content += ", ".join(param_names)
        ...
        "subcommands": [
            {
                ...
                "options": [
                    {"name": "environment", "short": "e", "type": "str", 
                     "default": "production", "help": "Target environment"},
                    {"name": "force", "short": "f", "is_flag": True,
            ...
            },
            {
                ...
                "options": [
                    {"name": "format", "short": "f", "type": "str",
                     "default": "table", "help": "Output format (table/json)"}
                ]
            }
        ],
        "options": [
            {"name": "config", "short": "c", "type": "path",
