        ...
        if relationships:
            ...
            for rel in relationships:
                ...
                if rel_type == "one-to-many":
                    content += f"    {target.lower()}s = relationship('{target}', " \
                        "back_populates='{model_name.lower()}')\n"
                elif rel_type == "many-to-one":
                    content += f"    {target.lower()} = relationship('{target}', " \
...
'''
        ...
        for field_name, field_type in fields.items():
            if field_type == "string":
                ...
                content += (
                    f"        sa.Column('{field_name}', sa.Text(), nullable=True),\n"
                )
            elif field_type == "integer":
                content += f"        sa.Column('{field_name}', sa.Integer(), nullable=False),\n"
            elif field_type == "float":
                content += (
                    f"        sa.Column('{field_name}', sa.Float(), nullable=False),\n"
                )
            elif field_type == "boolean":
...
'''
    ...
    def generate(self, params: Dict[str, Any]) -> Dict[str, str]:
        ...
        return {
            ...
            (
                (
                    f"f"f"migrations/{datetime.now().strftime('%Y%m%d_%H%M%S')}_create_{model_name." \
                        "lower()}.py": self.generate_migration(""
                )
            )
                params
            ),
...
if __name__ == "__main__":
    ...
    params = {
        ...
        "relationships": [{"type": "many-to-one", "target": "Category"}],
