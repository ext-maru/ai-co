...
class ASTAnalyzer:
    ...
    def _analyze_class(self, node: ast.ClassDef) -> ClassInfo:
        ...
        is_abstract = (
            "ABC" in bases
            or "abc.ABC" in bases
            or any("abstract" in d.lower() for d in decorators)
        )
...
class PatternLibrary:
    ...
    def suggest_patterns(self, function_info: FunctionInfo) -> List[Tuple[str, str]]:
        ...
        if function_info.calls_made:
            if (
                "publish" in function_info.calls_made
                or "basic_publish" in function_info.calls_made
            ):
...
class TestTemplateEngine:
    ...
    def generate_function_tests(
        self, function_info: FunctionInfo, module_path: Path
    ) -> str:
    ...
    def _generate_init_test(self, class_info: ClassInfo) -> str:
        ...
        return pattern.format(
            ...
            assertions="\n".join(assertions)
            if assertions
            else "        assert instance is not None",
        )

    def _generate_method_tests(
        self, method: FunctionInfo, class_info: ClassInfo
    ) -> List[str]:
    ...
    def _apply_pattern(
        ...
        # Handle exceptions
        exception = (
            list(function_info.raises)[0] if function_info.raises else "Exception"
        )
...
class CoverageGapAnalyzer:
    ...
    def prioritize_modules(
        self, gaps: List[Tuple[Path, float]]
...
class AutoTestGenerator:
    ...
    def generate_tests_for_module(self, module_path: Path) -> Dict[str, str]:
        ...
        for class_info in module_info.classes:
            ...
            test_code = self.template_engine.generate_test_class(
        ...
        if module_info.functions:
            ...
            for func in module_info.functions:
                func_tests = self.template_engine.generate_function_tests(
                    func, module_path
                )
                if func_tests:
    ...
    def learn_from_existing_tests(self) -> Dict[str, Any]:
        ...
        for test_file in high_coverage_tests:
            if test_file.exists():
                ...
                if "@patch" in content:
                    patterns_found["mocking"].append(
                        "Uses @patch decorator extensively"
                    )
    ...
    def generate_for_coverage_gaps(
        self, max_modules: int = 3
        ...
        for i, (module_path, coverage, priority) in enumerate(
    ...
    def save_generated_tests(
        self, tests: Dict[str, Dict[str, str]], output_dir: Path
    ) -> List[Path]:
