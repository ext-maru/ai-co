...
class CoverageParser:
    ...
    def parse_coverage_json(self, coverage_file: Path = None) -> Dict[str, CoverageInfo]:
...
class GapAnalyzer:
    ...
    def _calculate_priority(self, file_path: str, coverage_info: CoverageInfo) -> ModulePriority:
    ...
    def get_top_priority_modules(self, gaps: List[ModulePriority], count: int = 5) -> List[ModulePriority]:
        """Get top priority modules for test generation"""
        return gaps[:count]
        
    def get_modules_by_importance(self, gaps: List[ModulePriority], importance: str) -> List[ModulePriority]:
        """Get modules filtered by importance level"""
        return [gap for gap in gaps if gap.importance_level == importance]
        
    def analyze_missing_coverage_areas(self, module_priority: ModulePriority) -> Dict[str, List[str]]:
        ...
        for func in functions:
            ...
            if func_lines.intersection(missing_lines_set):
                ...
                if func['name'].startswith('handle_') or 'error' in func['name'].lower():
    ...
    def generate_gap_report(self, gaps: List[ModulePriority]) -> str:
        ...
        for i, gap in enumerate(high_priority[:10], 1):
            ...
            if missing_areas['functions']:
                report.append(f"- **Missing Functions**: {', '.join(missing_areas['functions'][:5])}")
    ...
    def _generate_module_type_summary(self, gaps: List[ModulePriority]) -> str:
        ...
        for module_type, type_gaps in by_type.items():
            avg_coverage = sum(g.coverage_info.coverage_percentage for g in type_gaps) / len(type_gaps)
            summary.append(f"- **{module_type}**: {len(type_gaps)} modules, avg coverage: {avg_coverage:.1f}%")
...
def main():
    ...
    for i, module in enumerate(top_modules, 1):
        print(f"{i}. {module.file_path.name} - Score: {module.priority_score}, Coverage: {module.coverage_info.coverage_percentage:.1f}%")
