...
class ProvenPatternLibrary:
    ...
    def get_patterns_for_method(self, method_info: Dict[str, Any]) -> List[TestPattern]:
        ...
        if method_info.get('has_external_calls', False):
            if any(call in ['publish', 'connect', 'execute'] for call in method_info.get('calls', [])):
...
class EnhancedTestGenerator:
    ...
    def _analyze_module(self, module_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'has_connections': any(imp in ['pika', 'redis', 'aioredis'] for imp in imports),
    ...
    def _analyze_class_detailed(self, node: ast.ClassDef, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'has_monitoring': any('metrics' in m['name'] or 'monitor' in m['name'] for m in methods)
        }
        
    def _analyze_method_detailed(self, node: ast.FunctionDef, content: str) -> Dict[str, Any]:
        ...
        external_indicators = ['connect', 'publish', 'send', 'get', 'post', 'execute', 'query']
    ...
    def _generate_enhanced_header(self, module_path: Path, analysis: Dict[str, Any]) -> str:
    ...
    def _generate_enhanced_class_test(self, class_info: Dict[str, Any], module_path: Path) -> str:
        ...
        init_test = self._apply_pattern('class_initialization', {
            'class_name': class_name,
            'primary_attribute': class_info['primary_attributes'][0] if class_info['primary_attributes'] else 'connection'
        })
        ...
        for method_info in class_info['methods']:
            if method_info['name'].startswith('_') and method_info['name'] != '__init__':
                continue
                
            method_tests = self._generate_method_tests_with_patterns(method_info, class_name)
    ...
    def _generate_method_tests_with_patterns(self, method_info: Dict[str, Any], class_name: str) -> List[str]:
    ...
    def _build_pattern_context(self, method_info: Dict[str, Any], class_name: str, pattern: TestPattern) -> Dict[str, str]:
