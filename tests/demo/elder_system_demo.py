...
async def demo_elder_hierarchy():
    ...
    if grand_elder:
        print(f"✅ Grand Elder can access Sage functions: {auth_system.check_elder_permission(grand_elder, ElderRole.SAGE)}")
        print(f"✅ Grand Elder can access Servant functions: {auth_system.check_elder_permission(grand_elder, ElderRole.SERVANT)}")
    
    # Servant → 制限された権限
    servant = auth_system.users.get('servant1')
    if servant:
        print(f"❌ Servant cannot access Sage functions: {not auth_system.check_elder_permission(servant, ElderRole.SAGE)}")
        print(f"❌ Servant cannot access Elder functions: {not auth_system.check_elder_permission(servant, ElderRole.CLAUDE_ELDER)}")
    
    # Sage専門分野
    print("\nTesting Sage specializations:")
    task_sage = auth_system.users.get('task_sage')
    if task_sage:
        print(f"✅ Task Sage can access Task functions: {auth_system.check_sage_permission(task_sage, SageType.TASK)}")
        print(f"❌ Task Sage cannot access Knowledge functions: {not auth_system.check_sage_permission(task_sage, SageType.KNOWLEDGE)}")
