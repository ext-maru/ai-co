...
class TestCoverageAnalyzer:
    ...
    def find_python_files(self, directory: str) -> List[Path]:
        ...
        for file_path in dir_path.rglob("*.py"):
            ...
            if not any(x in str(file_path) for x in ["test_", "_test.py", "/test", "venv", "__pycache__"]):
    ...
    def calculate_module_coverage(self) -> Dict[str, Dict]:
        ...
        for module in self.critical_modules:
            ...
            stats = {
                ...
                "coverage_estimate": len(module_tests) / len(python_files) * 100 if python_files else 0
            }
    ...
    def find_untested_critical_components(self) -> List[Dict]:
        ...
        # Critical workers
        critical_workers = [
            "pm_worker", "enhanced_task_worker", "result_worker", 
            "error_intelligence_worker", "slack_monitor_worker"
        ]
        ...
        # Critical libs
        critical_libs = [
            "queue_manager", "task_sender", "error_intelligence_manager",
            "slack_notifier", "worker_health_monitor", "elder_council_summoner"
        ]
    ...
    def generate_report(self) -> str:
        ...
        sorted_modules = sorted(module_stats.items(), 
                              key=lambda x: x[1]["coverage_estimate"])
        ...
        low_coverage_modules = [m for m, s in module_stats.items() 
                              if s['coverage_estimate'] < 30]
...
def main():
    ...
    summary = {
        ...
        "summary": {
            ...
            "modules_with_low_coverage": [m for m, s in module_stats.items() 
                                        if s['coverage_estimate'] < 30],
