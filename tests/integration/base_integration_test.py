...
class BaseIntegrationTest:
    ...
    def create_mock_sage(self, sage_type: str) -> AsyncMock:
        ...
        mock_sage.process_request = AsyncMock(return_value={
            "status": "success",
            "sage": sage_type,
    ...
    def create_mock_elder_flow(self) -> MagicMock:
        """モックElder Flow作成"""
        mock_flow = MagicMock()
        mock_flow.execute = AsyncMock(return_value={
            "status": "success",
            "phases_completed": 5,
            "quality_score": 95.5,
            "execution_time": 0.5
...
class ElderFlowIntegrationTest(BaseIntegrationTest):
    ...
    async def simulate_elder_flow_execution(
        ...
        if phases is None:
            phases = ["sage_council", "servant_execution", "quality_gate", "report", "git_automation"]
