...
from libs.elder_servants.coordination.four_organizations_coordinator import (
    ...
    TaskComplexity, CoordinationPattern, get_coordinator
)
from libs.elder_servants.base.elder_servant_base import (
    ServantDomain, ServantCapability, ServantRequest, ServantResponse
)
...
class TestFourOrganizationsCoordination:
    ...
    async def _setup_test_servants(self, coordinator):
        ...
        # 各組織のテストサーバントを登録
        # ドワーフ工房
        registry.register(TestDwarfServant, "test_dwarf_crafter", ServantDomain.DWARF_WORKSHOP)
        registry.register(TestDwarfServant, "test_dwarf_builder", ServantDomain.DWARF_WORKSHOP)
        
        # RAGウィザーズ
        registry.register(TestWizardServant, "test_wizard_scout", ServantDomain.RAG_WIZARDS)
        registry.register(TestWizardServant, "test_wizard_analyst", ServantDomain.RAG_WIZARDS)
        ...
        # インシデント騎士団
        registry.register(TestKnightServant, "test_knight_guardian", ServantDomain.INCIDENT_KNIGHTS)
        registry.register(TestKnightServant, "test_knight_defender", ServantDomain.INCIDENT_KNIGHTS)
    ...
    async def test_hierarchical_coordination(self, coordinator):
        ...
        verification_count = sum(1 for org in verification_orgs if org in result.organization_results)
    ...
    async def test_organization_specialization_routing(self, coordinator):
        ...
        specialization_tests = [
            ...
            (ServantDomain.RAG_WIZARDS, "research_analysis", "Research and analysis task"),
...
async def run_coordination_integration_test():
        ...
        try:
            ...
            if "simple" in test_id:
                ...
                task = CoordinationTask(
                    ...
                    pattern=CoordinationPattern.SEQUENTIAL,
                    required_organizations=[ServantDomain.RAG_WIZARDS, ServantDomain.DWARF_WORKSHOP],
                    optional_organizations=[]
                )
                result = await coordinator.coordinate_task(task)
            ...
            print(f"   {status}: 品質={result.quality_metrics.get('average_quality', 0):.1f}%, 効率={result.coordination_efficiency:.1%}")
    ...
    print(f"\n📊 4組織間協調テスト結果")
    print("=" * 50)
    print(f"✅ 成功率: {success_rate:.1%} ({test_results['successful_tests']}/{test_results['total_tests']})")
