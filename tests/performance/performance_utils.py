...
class PerformanceCollector:
    ...
    def add_metric(self, operation: str, duration: float, metadata: Optional[Dict] = None):
        """メトリクスを追加"""
        with self._lock:
...
class WorkerBenchmark:
    ...
    def benchmark_task_worker(self, task_count: int, concurrent_workers: int, timeout: int) -> Dict[str, float]:
...
class RabbitMQBenchmark:
    ...
    def benchmark_connection_pool(self, connection_count: int, operations_per_connection: int) -> Dict[str, float]:
        ...
        return {
            ...
            "avg_message_send_time": statistics.mean(message_send_times) if message_send_times else 0,
            "connection_success_rate": successful_connections / connection_count if connection_count > 0 else 0
        }
...
class MemoryLeakDetector:
    ...
    def detect_memory_leak(self, func: Callable, iterations: int, threshold_mb: float) -> Dict[str, Any]:
        """メモリリーク検出"""
