...
def test_node_similarity_calculation():
    ...
    if similarity_high > similarity_low:
        ...
        print(f"  ❌ Expected high > low, got {similarity_high:.3f} vs {similarity_low:.3f}")
...
def test_embedding_similarity_search():
    ...
    candidate_embeddings = [
        ...
        SemanticEmbedding("machine learning algorithms", [0.15, 0.85, 0.25, 0.65, 0.15])
    ]
    
    similar = graph.find_similar_embeddings(query_embedding, candidate_embeddings, top_k=2)
...
async def test_concept_relations_discovery():
    ...
    if relations:
        ...
        if (hasattr(first_relation, 'confidence') and 
            0 <= first_relation.confidence <= 1):
...
def test_knowledge_clustering():
    ...
    if isinstance(clusters, list) and len(clusters) <= 2:
        ...
        print(f"  ❌ Wrong cluster count: {len(clusters) if isinstance(clusters, list) else 'not list'}")
...
def test_knowledge_evolution_tracking():
    ...
    evolution_events = [
        {"timestamp": datetime.now() - timedelta(days=7), "type": "concept_added", "concept": "GPT"},
        {"timestamp": datetime.now() - timedelta(days=5), "type": "relation_discovered", "relation": "GPT->NLP"},
        {"timestamp": datetime.now() - timedelta(days=2), "type": "concept_updated", "concept": "GPT"},
        {"timestamp": datetime.now(), "type": "cluster_formed", "cluster": "transformer_models"}
    ]
    ...
    expected_fields = ["evolution_rate", "concept_growth", "relation_growth", "total_events"]
...
async def test_knowledge_inference():
    ...
    ac_relation = None
    if inferred:
        ac_relation = next((r for r in inferred if r.concept_a == "A" and r.concept_b == "C"), None)
...
def test_anomaly_detection_in_knowledge():
    ...
    knowledge_statements = [
        {"concept_a": "machine learning", "concept_b": "algorithms", "confidence": 0.95},
        {"concept_a": "deep learning", "concept_b": "neural networks", "confidence": 0.92},
        {"concept_a": "database", "concept_b": "storage", "confidence": 0.88},
        {"concept_a": "machine learning", "concept_b": "cooking recipes", "confidence": 0.15},  # 異常
        {"concept_a": "neural networks", "concept_b": "car maintenance", "confidence": 0.05}   # 異常
    ]
...
def test_multilingual_knowledge_integration():
    ...
    for field in required_fields:
        if field in unified_concept:
            print(f"  ✅ {field}: {unified_concept[field] if field != 'translations' else len(unified_concept[field])}言語")
            tests_passed += 1
        else:
...
def test_graph_statistics():
    ...
    expected_metrics = [
        "total_nodes", "total_edges", "average_degree",
        "clustering_coefficient", "graph_density", "connected_components"
    ]
    
    for metric in expected_metrics:
        if metric in stats and isinstance(stats[metric], (int, float)) and stats[metric] >= 0:
...
def test_knowledge_quality_assessment():
    ...
    expected_metrics = ["overall_quality_score", "consistency_score", "completeness_score", "accuracy_score"]
...
async def test_complete_discovery_workflow():
    ...
    discovery_result = await graph.run_discovery_cycle(input_documents, discovery_config)
    ...
    if hasattr(discovery_result, 'confidence') and 0 <= discovery_result.confidence <= 1:
