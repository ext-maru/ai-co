...
def test_prediction_model_training():
    ...
    training_data = [
        {"features": [80, 90, 95], "incident_occurred": True, "incident_type": "memory_leak"},
        {"features": [40, 50, 45], "incident_occurred": False, "incident_type": None},
        {"features": [85, 92, 98], "incident_occurred": True, "incident_type": "cpu_spike"},
        {"features": [30, 35, 32], "incident_occurred": False, "incident_type": None},
        {"features": [90, 95, 99], "incident_occurred": True, "incident_type": "memory_leak"},
    ...
    if model.model_type in ["threshold_based", "random_forest", "neural_network", "svm"]:
...
async def test_incident_prediction():
    ...
    if len(predictions) > 0:
        ...
        if isinstance(prediction, IncidentForecast):
            print(f"  ✅ 予測内容: {prediction.incident_type} (信頼度: {prediction.confidence:.3f})")
            tests_passed += 1
        else:
...
def test_risk_prioritization():
    ...
    if prioritized[0].risk_level == "critical" and prioritized[0].impact == "severe":
        ...
        print(f"  ❌ Wrong top priority: {prioritized[0].risk_level} + {prioritized[0].impact}")
...
async def test_preventive_actions():
    ...
    threat_pattern = ThreatPattern("memory_leak_pattern", "high", ["memory_growth", "gc_pressure"])
    ...
    if actions:
        ...
        valid_action_types = ["scale_up", "restart_service", "clear_cache", "optimize_config", "throttle_requests"]
...
def test_metrics_and_health():
    ...
    expected_metrics = ["overall_accuracy", "precision_by_type", "recall_by_type", "false_positive_rate"]
...
async def test_full_prediction_cycle():
    ...
    expected_fields = ["predictions", "risks_assessed", "actions_recommended", "alerts_generated"]
    for field in expected_fields:
