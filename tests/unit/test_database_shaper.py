    ...
    async def test_design_complex_schema(self):
        ...
        data_requirements = {
            "data_objects": [
                ...
                {"name": "Order", "attributes": [{"name": "id", "type": "integer"}, {"name": "user_id", "type": "integer"}]},
                {"name": "Product", "attributes": [{"name": "id", "type": "integer"}, {"name": "name", "type": "string"}]}
            ]
        }
...
class TestPerformanceOptimization:
    ...
    async def test_performance_bottleneck_detection(self):
        ...
        query_patterns = [
            {"type": "SELECT", "execution_time_ms": 5000}  # 遅いクエリ
        ]
...
class TestORMModelGeneration:
    ...
    async def test_generate_sqlalchemy_models(self):
        ...
        database_schema = {
            ...
            "relationships": [
                {"type": "one_to_many", "from": "users", "to": "orders"}
            ]
        }
...
class TestQualityValidation:
    ...
    async def test_database_design_quality_validation(self):
        ...
        high_quality_result = {
            "database_schema": {"tables": []},
            "table_count": 5,
            "design_quality": {"normalization": 90, "consistency": 85, "performance": 88},
            "strategies_count": 8,
            "index_count": 10,
            "implementation_complexity": "low"
        }
        
        quality_score = await shaper._validate_database_design_quality(high_quality_result)
        ...
        quality_score = await shaper._validate_database_design_quality(low_quality_result)
...
class TestErrorHandling:
    ...
    async def test_exception_handling(self):
        ...
        with patch.object(shaper, '_design_database_schema', side_effect=Exception("Test exception")):
            task = {
...
class TestHelperClasses:
    ...
    def test_performance_optimizer(self):
        """パフォーマンス最適化器テスト"""
        from libs.elder_servants.dwarf_workshop.database_shaper import PerformanceOptimizer
    ...
    def test_migration_generator(self):
        """マイグレーション生成器テスト"""
        from libs.elder_servants.dwarf_workshop.database_shaper import MigrationGenerator
