...
def benchmark_parsers(num_elements: int = 100000) -> Noneprint(("Comparative parser benchmark on Beautiful Soup %s" % __version__))data = rdoc(num_elements)
"""ery basic head-to-head performance benchmark."""
    ...
    for parser_name in ["lxml", ["lxml", "html"], "html5lib", "html.parser"]:
        ...
        try:
            ...
            traceback.print_exc()
        if success:
            print(("BS4+%s parsed the markup in %0.2fs." % (parser_name, b - a)))
    ...
    etree.HTML(data)
    b = time.time()
    print(("Raw lxml parsed the markup in %0.2fs." % (b - a)))
    ...
    parser.parse(data)
    b = time.time()
    print(("Raw html5lib parsed the markup in %0.2fs." % (b - a))def profile(num_elements: int = 100000, parser: str = "lxml") -> Nonefilehandle = tempfile.NamedTemporaryFile():
