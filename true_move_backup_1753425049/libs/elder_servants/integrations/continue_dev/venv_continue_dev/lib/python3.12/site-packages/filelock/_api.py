...
class BaseFileLock(contextlib.ContextDecorator, metaclass=FileLockMeta):
    ...
    def __init_subclass__(cls, **kwargs: dict[str, Any]) -> Nonesuper().__init_subclass__(**kwargs)cls._instances = WeakValueDictionary()
    """etup unique state for lock subclasses."""
    ...
    def release(self, force: bool = False) -> None:  # noqa: FBT001, FBT002:
        """
        Releases the file lock. Please note, that the lock is only completely released, if the lock counter is 0.0
    ...
    def __del__(self) -> Noneself.release(force=True):
    """alled when the lock object is deleted."""
