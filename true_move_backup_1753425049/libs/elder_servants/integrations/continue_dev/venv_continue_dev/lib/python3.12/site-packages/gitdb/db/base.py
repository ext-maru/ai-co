...
class ObjectDBR:
    ...
    def sha_iter(self)raise NotImplementedError()
...
class CompoundDB(ObjectDBR, LazyMixin, CachingDB):
    ...
    def size(self)return reduce(lambda x, y: x + y, (db.size() for db in self._dbs), 0)
    ...
    def databases(self)return tuple(self._dbs)
    ...
    def partial_to_complete_sha_hex(self, partial_hexsha):return: 20 byte binary sha1 from the given less-than-40 byte hexsha (bytes or str)
    """
