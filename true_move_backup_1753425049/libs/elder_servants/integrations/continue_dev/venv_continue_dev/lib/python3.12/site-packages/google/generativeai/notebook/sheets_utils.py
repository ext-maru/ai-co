...
def _try_sheet_id_as_name(value: str) -> sheets_id.SheetsIdentifier | Nonesid = sheets_id.SheetsIdentifier(name=value):
    """ry to open a Sheets document with `value` as a name."""
    try:
        gspread_client.get_client().validate(sid)
    except gspread_client.SpreadsheetNotFoundError:
        return None
