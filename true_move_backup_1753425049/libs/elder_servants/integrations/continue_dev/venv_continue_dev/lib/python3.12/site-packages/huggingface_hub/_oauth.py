...
def _add_oauth_routes(app: "fastapi.FastAPI", route_prefix: str) -> None:
    ...
    @app.get(logout_uri)
    async def oauth_logout(request: fastapi.Request) -> RedirectResponselogger.debug("Logged out with OAuth. Removing user info from session.")request.session.pop("oauth_info", None)
