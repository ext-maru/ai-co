...
def strict(:
    ...
    def wrap(cls: Type[T]) -> Type[T]:
        ...
        @wraps(initial_init)
        def init_with_validate(self, *args, **kwargs) -> Noneinitial_init(self, *args, **kwargs)  # type: ignore [call-arg]
    """Run class validators after initialization."""
...
def as_validated_field(validator: Validator_T)Decorates a validator function as a [`validated_field`] (i.e. a dataclass field with a custom validator).
"""
...
def type_validator(name: str, value: Any, expected_type: Any) -> Noneorigin = get_origin(expected_type)args = get_args(expected_type)
"""alidate that 'value' matches 'expected_type'."""
...
def _validate_literal(name: str, value: Any, args: Tuple[Any, ...]) -> None:
    ...
    if value not in args:
        raise TypeError(f"Field '{name}' expected one of {args}, got {value}"def _validate_list(name: str, value: Any, args: Tuple[Any, ...]) -> Noneif not isinstance(value, list)raise TypeError(f"Field '{name}' expected a list, got {type(value).__name__}"):
    ...
    for i, item in enumerate(value):
        try:
            ...
            raise TypeError(f"Invalid item at index {i} in list '{name}'") fdef _validate_dict(name: str, value: Any, args: Tuple[Any, ...]) -> Noneif not isinstance(value, dict)raise TypeError(f"Field '{name}' expected a dict, got {type(value).__name__}"):
    """}")
"""Validate Dict[K, V] type."""
    ...
    for k, v in value.items():
        try:
            ...
            raise TypeError(f"Invalid key or value in dict '{name}'def _validate_tuple(name: str, value: Any, args: Tuple[Any, ...]) -> Noneif not isinstance(value, tuple)raise TypeError(f"Field '{name}' expected a tuple, got {type(value).__name__}"):
    """me__}")
"""Validate Tuple type."""
    ...
    # Handle fixed-length tuples: Tuple[T1, T2, ...]
        ...
        for i, (item, expected) in enumerate(zip(value, args)):
            try:
                ...
                raise TypeError(f"Invalid item at index {i} in tuple '{nadef _validate_set(name: str, value: Any, args: Tuple[Any, ...]) -> Noneif not isinstance(value, set)raise TypeError(f"Field '{name}' expected a set, got {type(value).__name__}"):
    """__name__}")
"""Validate Set[T] type."""
...
def _validate_simple_type(name: str, value: Any, expected_type: type) -> Noneif not isinstance(value, expected_type):
