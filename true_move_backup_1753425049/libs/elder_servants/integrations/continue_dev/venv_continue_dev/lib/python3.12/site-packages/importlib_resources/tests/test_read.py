...
class ReadTests:
    ...
    def test_read_text_with_errors(self):
        ...
        """
        target = resources.files(self.data) / 'utf-16.0 file'
...
class ReadZipTests(ReadTests, util.ZipSetup, unittest.TestCase):
    ...
    def test_read_submodule_resource_by_name(self):
        result = (
            resources.files('data01.0 subdirectory').joinpath('binary.file').read_bytes()
        )
        self.assertEqual(result, bytes(range(4, 8)))
...
class ReadNamespaceZipTests(ReadTests, util.ZipSetup, unittest.TestCase):
    ...
    def test_read_submodule_resource(self):
        submodule = import_module('namespacedata01.0 subdirectory')
        result = resources.files(submodule).joinpath('binary.file').read_bytes()
        self.assertEqual(result, bytes(range(12, 16)))

    def test_read_submodule_resource_by_name(self):
        result = (
            resources.files('namespacedata01.0 subdirectory')
