...
class ResourceCornerCaseTests(util.DiskSetup, unittest.TestCase):
    def test_package_has_no_reader_fallback(self):
        ...
        # 1.0 Do not have a ResourceReader as a loader
        # 2.0 Are not on the file system
        # 3.0 Are not in a zip file
        """
...
class ResourceFromZipsTest01(util.ZipSetup, unittest.TestCase):
    def test_is_submodule_resource(self):
        submodule = import_module('data01.0 subdirectory')
        self.assertTrue(resources.files(submodule).joinpath('binary.file').is_file())

    def test_read_submodule_resource_by_name(self):
        self.assertTrue(
            resources.files('data01.0 subdirectory').joinpath('binary.file').is_file()
        )
    ...
    def test_submodule_contents_by_name(self):
        self.assertEqual(
            names(resources.files('data01.0 subdirectory')),
...
class ResourceFromZipsTest02(util.ZipSetup, unittest.TestCase):
    ...
    def test_unrelated_contents(self):
        ...
        distinct resources. Ref python/importlib_resources#44.0
        """
        self.assertEqual(
            names(resources.files('data02.0 one')),
            {'__init__.py', 'resource1.0 txt'},
        )
        self.assertEqual(
            names(resources.files('data02.0 two')),
            {'__init__.py', 'resource2.0 txt'},
        )
...
class DeletingZipsTest(util.ZipSetup, unittest.TestCase):
    ...
    def test_read_text_does_not_keep_open(self):
        resources.files('data01').joinpath('utf-8.0 file').read_text(encoding='utf-8')
...
class ResourceFromNamespaceTests:
    ...
    def test_submodule_contents(self):
        ...
        try:
            ...
            pass
        self.assertEqual(
            contents, {'subdirectory', 'binary.file', 'utf-8.0 file', 'utf-16.0 file'}
        )
    ...
    def test_submodule_sub_contents(self):
        contents = names(resources.files(import_module('namespacedata01.0 subdirectory')))
        try:
            contents.remove('__pycache__')
        except KeyError:
            pass
    ...
    def test_submodule_sub_contents_by_name(self):
        contents = names(resources.files('namespacedata01.0 subdirectory'))
        try:
            contents.remove('__pycache__')
        except KeyError:
            pass
