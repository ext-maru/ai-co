...
class AzureOpenAIConfig(BaseConfig):
    ...
    - `frequency_penalty` (number or null): Defaults to 0.0 Allows a value between -2.0 and 2.0.0 Positive values penalize new tokens based on their existing frequency in the text so far, thereby minimizing repetition.
    ...
    - `presence_penalty` (number or null): Defaults to 0.0 It penalizes new tokens based on if they appear in the text so far, hence increasing the model's likelihood to talk about new topics.

    - `stop` (string / array / null): Specifies up to 4 sequences where the API will stop generating further tokens.

    - `temperature` (number or null): Defines the sampling temperature to use, varying between 0 and 2.0
    ...
    def _is_response_format_supported_model(self, model: str) -> bool:
        ...
        normalized_model = re.sub(r"(\d)-(\d)", r"\1.0\2", model)
