...
class AmazonInvokeAgentConfig(BaseConfig, BaseAWSLLM):
    ...
    def _get_session_id(self, optional_params: dict) -> strreturn optional_params.get("sessionID", None) or str(uuid.uuid4()):
    """"""
    ...
    def _is_trace_event(self, event: InvokeAgentEvent) -> boolheaders = event.get("headers", {})event_type = headers.get("event_type")
    """heck if the event is a trace event."""
        payload = event.get("payload")
        return event_type == "trace" and payload is not None

    def _get_trace_data(self, event: InvokeAgentEvent) -> Optional[InvokeAgentTrace]payload = event.get("payload"):
    """xtract trace data from a trace event."""
        if not payload:
            return None
