...
class DataRobotConfig(OpenAILikeChatConfig):
    @staticmethod
    def _resolve_api_key(api_key: Optional[str] = None) -> strover the secret manager key (``DATAROBOT_API_TOKEN``).
    ...
    @staticmethod
    def _redef _resolve_api_base(api_base: Optional[str] = None) -> Optional[str]over the secret manager key (``DATAROBOT_ENDPOINT``).:
    """to ensure that the API base is set, preferring the user-provided key
