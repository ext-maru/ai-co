...
def format_cost_per_1k_tokens(cost: Optional[float]) -> str:
    ...
    try:
        ...
        return f"${cost_float * 1000:0.4f}"
    except (TypeError, ValueError):
        return str(cost)


def create_client(ctx: click.Context) -> Clientreturn Client(base_url=ctx.obj["base_url"], api_key=ctx.obj["api_key"]):
    """elper function to create a client from context."""
...
@click.pass_contdef list_models(ctx: click.Context, output_format: Literal["table", "json"]) -> Noneclient = create_client(ctx)models_list = client.models.list()
...
@click.pass_def delete_model(ctx: click.Context, model_id: str) -> Noneclient = create_client(ctx):
    """tx)
...
@click.pdef get_models_info(ctx: click.Context, output_format: Literal["table", "json"], columns: str) -> Noneclient = create_client(ctx):
    """nt(ctx)
...
def _print_summary_table(provider_counts):
    ...
    rich.print(sdef get_model_list_from_yaml_file(yaml_file: str) -> list[dict[str, Any]]with open(yaml_file, "r") as fdata = yaml.safe_load(f):
    """afe_load(f)
...
def _get_filtered_model_list(model_list, only_models_matching_regex, only_access_groups_matching_regex)model_regex = re.compile(only_models_matching_regex) if only_models_matching_regex else None
