...
class TeamMemberPermissionChecks:
    ...
    async def can_team_member_execute_key_management_endpoint(
        ...
        # 1.0 Don't execute these checks if the user role is proxy admin
        if user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN.value:
            return

        # 2.0 Check if the operation is being done on a team key
        if existing_key_row.team_id is None:
            return

        # 3.0 Get Team Object from DB
        ...
        # 4.0 Extract `Member` object from `team_table`
        key_assigned_user_in_team = _get_user_in_team(
            team_table=team_table, user_id=user_api_key_dict.user_id
        )

        # 5.0 Check if the team member has permissions for the endpoint
