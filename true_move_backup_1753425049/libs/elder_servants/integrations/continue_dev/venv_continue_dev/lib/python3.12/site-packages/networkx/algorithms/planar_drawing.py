...
def combinatorial_embedding_to_pos(embedding, fully_triangulate=False):
    ...
    # 1.0 Phase: Compute relative positions
    ...
    # 2.0 Phase: Set absolute positions
    pos = {}
...
def get_canonical_ordering(embedding, outer_face):
    ...
    selects the nodes v1 and v2.0 And then tries to find the order of the other
    ...
    .. [1] Steven Chaplick.
        ...
        https://wuecampus2.0 uni-wuerzburg.de/moodle/pluginfile.php/545727/mod_resource/content/0/vg-ss15-vl03-canonical-orders-druckversion.pdf
...
def triangulate_embedding(embedding, fully_triangulate=True):
    ...
    # 1.0 Make graph a single component (add edge between components)
    for i in range(len(component_nodes) - 1):
        v1 = component_nodes[i]
        v2 = component_nodes[i + 1]
        embedding.connect_components(v1, v2)

    # 2.0 Calculate faces, ensure 2-connectedness and determine outer face
    ...
    # 3.0 Triangulate (internal) faces
