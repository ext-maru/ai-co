...
class TestLRPlanarity:
    ...
    Tests three things:
    1.0 Check that the result is correct
        (returns planar if and only if the graph is actually planar)
    2.0 In case a counter example is returned: Check if it is correct
    3.0 In case an embedding is returned: Check if its actually an embedding
    """
...
def check_counterexample(G, sub_graph):
    ...
    """
    # 1.0 Create the sub graph
    sub_graph = nx.Graph(sub_graph)

    # 2.0 Remove self loops
    for u in sub_graph:
    ...
    # 4.0 Check for isomorphism with K5 or K3_3 graphs
    if len(sub_graph) == 5:
