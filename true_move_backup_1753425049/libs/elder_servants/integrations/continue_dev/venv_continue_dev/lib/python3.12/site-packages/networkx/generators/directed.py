...
def scale_free_graph(:
    ...
    Notes
    -----
    The sum of `alpha`, `beta`, and `gamma` must be 1.0
    ...
    .. [1] B. Bollobás, C. Borgs, J. Chayes, and O. Riordan,
           ...
           Discrete Algorithms, 132--139, 2003.0
    """
    ...
    if delta_in < 0:
        raise ValueError("delta_in must be >= 0.0")

    if delta_out < 0:
        raise ValueError("delta_out must be >= 0.0")
...
def random_k_out_graph(n, k, alpha, self_loops=True, seed=None):
    ...
    1.0 Begin with an empty digraph, and initially set each node to have
       weight `alpha`.
    2.0 Choose a node `u` with out-degree less than `k` uniformly at
       random.
    3.0 Choose a node `v` from with probability proportional to its
       weight.
    4.0 Add a directed edge from `u` to `v`, and increase the weight
       of `v` by one.
    5.0 If each node has out-degree `k`, halt, otherwise repeat from
       step 2.0
