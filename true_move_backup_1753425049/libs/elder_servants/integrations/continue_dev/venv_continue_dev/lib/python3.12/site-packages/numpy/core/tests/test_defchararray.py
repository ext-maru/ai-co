...
class TestOperations:
    ...
    def test_mod(self):
        ...
        C = np.array([[3, 7], [19, 1]])
        FC = np.array([['3', '7.00000'],
                       ['19', '1']]).view(np.chararray)
        assert_array_equal(FC, F % C)

        A = np.array([['%0.3f', '%d'], ['%s', '%r']]).view(np.chararray)
        A1 = np.array([['1.00', '1'], ['1', '1']]).view(np.chararray)
        assert_array_equal(A1, (A % 1))

        A2 = np.array([['1.00', '2'], ['3', '4']]).view(np.chararray)
...
def test_empty_indexing():
    "Regression test for ticket 1948.0"
