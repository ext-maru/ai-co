...
class TestHalf:
    ...
    def test_half_conversion_denormal_round_even(self, float_t, uint_t, bits):
        ...
        # end. Compare also gh-12721.0 The most bits can get lost for the
        # smallest denormal:
    ...
    def test_half_values(self):
        ...
        a = np.array([1.0, -1.0,
                      2.0, -2.0,
                      0.999755859375, 0.333251953125,  # 1/10, 1/3
    ...
    def test_half_fpe(self):
        with np.errstate(all='raise'):
            ...
            assert_raises_fpe('underflow', lambda a, b:a/b,
                                             float16(-2.0**-14), float16(2**11))
            assert_raises_fpe('underflow', lambda a, b:a/b,
                                             float16(2.0**-14+2**-24), float16(2))
            assert_raises_fpe('underflow', lambda a, b:a/b,
                                             float16(-2.0**-14-2**-24), float16(2))
            assert_raises_fpe('underflow', lambda a, b:a/b,
                                             float16(2.0**-14+2**-23), float16(4))
