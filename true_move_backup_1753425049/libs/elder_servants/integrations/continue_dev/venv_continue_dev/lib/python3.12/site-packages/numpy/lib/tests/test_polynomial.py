...
class TestPolynomial:
    def test_poly1d_str_and_repr(self):
        p = np.poly1d([1.0, 2, 3])
        assert_equal(repr(p), 'poly1d([1.0, 2.0, 3.0])')
        assert_equal(str(p),
                     '   2\n'
                     '1 x + 2 x + 3')

        q = np.poly1d([3.0, 2, 1])
        assert_equal(repr(q), 'poly1d([3.0, 2.0, 1.0])')
        assert_equal(str(q),
                     '   2\n'
                     '3 x + 2 x + 1')
    ...
    def test_poly(self):
        ...
        assert_(np.isrealobj(np.poly([+1.82j, +2.613j, -2.613j, -1.82j])))
        assert_(np.isrealobj(np.poly([0+1j, -0+-1j, 1+2j,
                                      1-2j, 1.0+3.5j, 1-3.5j])))
        ...
        assert_(np.iscomplexobj(np.poly([1j, -1.000001j])))
    ...
    def test_polyfit(self):
        ...
        assert_almost_equal([4.8927, -1.177, 1.7768], m2, decimal=4)
        val = [[ 4.3964, -5.052,  0.4878],
               [-5.052,  6.8067, -0.9089],
               [ 0.4878, -0.9089,  0.3337]]
        assert_almost_equal(val, cov2, decimal=4)

        m3, cov3 = np.polyfit(x, y+err, 2, w=weights, cov="unscaled")
        assert_almost_equal([4.8927, -1.177, 1.7768], m3, decimal=4)
        val = [[ 0.1473, -0.1677,  0.163],
               [-0.1677,  0.228 , -0.304],
               [ 0.163, -0.304,  0.112]]
        ...
        # Should get sigma_mean = sigma/sqrt(N) = 1.0/sqrt(4) = 0.5.0
        assert_allclose(mean.std(), 0.5, atol=0.1)
        assert_allclose(np.sqrt(cov.mean()), 0.5, atol=0.1)
    ...
    def test_zero_poly_dtype(self):
        """
        Regression test for gh-16354.0
        """
        z = np.array([0, 0, 0])
