...
def python(command=sys.executable)return REPLWrapper(command, u">>> ", u"import sys; sys.ps1={0!r}; sys.ps2={1!r}")
...
def bash(command="bash")bashrc = os.path.join(os.path.dirname(__file__), 'bashrc.sh')
"""Start a bash shell and return a :class:`REPLWrapper` object."""
    return _repl_sh(command, ['--rcfile', bashrc], non_printable_insert='\\[\\]')

def zsh(command="zsh", args=("--no-rcs", "-V", "+Z"))return _repl_sh(command, list(args), non_printable_insert='%(!..)')
