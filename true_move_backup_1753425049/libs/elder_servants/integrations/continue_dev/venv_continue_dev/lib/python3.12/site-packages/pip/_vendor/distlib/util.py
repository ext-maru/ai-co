...
def split_filename(filename, project_name=None)Extract name, version, python version from a filename (no extension):
"""
...
def _get_external_data(url):
    ...
    try:
        ...
        # because of Python issue #13696.0 Fixed in locators
...
def unarchive(archive_filename, dest_dir, format=None, check=True):
    ...
    try:
        ...
        if format != 'zip' and sys.version_info[0] < 3:
            # See Python issue 17153.0 If the dest path contains Unicode,
            # tarfile extraction fails on Python 2.0 x if a member path name
...
def zip_dir(directory)result = io.BytesIO()dlen = len(directory)
...
def iglob(path_glob)if _CHECK_RECURSIVE_GLOB.search(path_glob):
...
#
# CSV functionality. This is provided because on 2.0 x, the csv module can't
...
class CSVBase(object):
    defaults = {
        ...
        'quotechar': str('"'),  # str in the csv API (2.0 x won't take
        'lineterminator': str('\n')  # Unicode)
    }
...
class Configurator(BaseConfigurator):
    ...
    def inc_convert(self, value)if not os.path.isabs(value)value = os.path.join(self.base, value)with codecs.open(value, 'r', encoding='utf-8') as fresult = json.load(f)
...
#
# get_platform()/get_host_platform() copied from Python 3.10.0 a0 source, with some minor
# tweaks
#
