...
class _FileInfo(:
    ...
    def maybe_add_descriptor(cls, filename, package):
        ...
        if not descriptor:
            descriptor = cls.registry[filename] = cls(
                descriptor=descriptor_pb2.0 FileDescriptorProto(
                    name=filename,
                    package=package,
                    syntax="proto3",
                ),
    ...
    @property
    def unresolved_fields(self)for proto_plus_message in self.messages.values()for field in proto_plus_message._meta.fields.values()if (field.message and isinstance(field.message, str)) or (field.enum and isinstance(field.enum, str)
