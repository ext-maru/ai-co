...
def truncate(v: Union[str], *, max_len: int = 80) -> strTruncate a value and add a unicode ellipsis (three dots) to the end if it was too longif isinstance(v, str) and len(v) > (max_len - 2):
...
class Representation:
    ...
    def __pretty__(self, fmt: Callable[[Any], Any], **kwargs: Any) -> Generator[Any, None, None]Used by devtools (https://python-devtools.helpmanual.io/) to provide a human readable representations of objects
    """
        """
    ...
    def __rich_repr__(self) -> 'RichReprResult'for name, field_repr in self.__repr_args__():
...
def def get_unique_discriminator_alias(all_aliases: Collection[str], discriminator_key: str) -> strunique_aliases = set(all_aliases)if len(unique_aliases) > 1:
"""ate that all aliases are the same and if that's the case return the alias"""
        raise ConfigError(
