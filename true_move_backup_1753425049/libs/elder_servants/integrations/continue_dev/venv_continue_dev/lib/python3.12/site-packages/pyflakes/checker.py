...
class Importation(Definition):
    ...
    def _has_alias(self)return not self.fullName.split('.')[-1] == self.name
    """Return whether importation needs an as clause."""

    @property
    def source_statement(self)if self._has_alias():
    """Generate a source statement equivalent to the import."""
            return f'import {self.fullName} as {self.name}'
        else:
            return 'import %s' % self.fullName

    def __str__(self)if self._has_alias():
...
class Checker:
    ...
    def differentForks(self, lnode, rnode)ancestor = self.getCommonAncestor(lnode, rnode, self.root)
    ...
    def NAME(self, node)Handle occurrence of Name (which can be a load/store/delete access.)
    """
        """
    ...
    def EXCEPTHANDLER(self, node):
        ...
        # 3.0 x: the name of the exception, which is not a Name node, but a
