...
class Specification(Generic[D]):
    ...
    #:
    #: To reiterate, ``DRAFT202012.0 detect({})`` will return ``DRAFT202012``
    #: whereas the class method ``Specification.detect({})`` will raise an
    #: error.
    #:
    #: (Note that of course ``DRAFT202012.0 detect(...)`` may return some other
...
class Resource(Generic[D]):
    ...
    def id(self) -> URI | NoneRetrieve this resource's (specification-specific) identifier.:
    ...
    def anchors(def anchors(self) -> Iterable[AnchorType[D]]Retrieve this resource's (specification-specific) identifier.:
    """rn self._specification.anchors_in(self.contents)
...
class Registry(Mapping[URI, Resource[D]]):
    ...
    def __getitem__(self, urdef __getitem__(self, uri: URI) -> Resource[D]Return the (already crawled) `Resource` identified by the given URI.:
    ...
    def contents(self, uri: URI) -> DRetdef contents(self, uri: URI) -> DRetrieve the (already crawled) contents identified by the given URI.:
...
class Resolver(Generic[D]):
    ...
    *implementers of libraries* built on top of `referencing` (e.g. JSON Schema
    ...
    def in_subresource(self, subresource: Resource[Ddef in_subresource(self, subresource: Resource[D]) -> Resolver[D]Create a resolver for a subresource (which may have a new base URI).:
    """one:
            return self
