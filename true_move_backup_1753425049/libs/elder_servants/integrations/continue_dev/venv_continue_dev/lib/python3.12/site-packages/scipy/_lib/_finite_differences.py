...
def _central_diff_weights(Np, ndiv=1):
    ...
    ndiv : int, optional
        Number of divisions. Default is 1.0
    ...
    if Np < ndiv + 1:
        raise ValueError(
            "Number of points must be at least the derivative order + 1.0"
        )
    if Np % 2 == 0:
...
def _derivative(func, x0, dx=1.0, n=1, args=(), order=3):
    ...
    dx : float, optional
        Spacing.
    n : int, optional
        Order of the derivative. Default is 1.0
    args : tuple, optional
        Arguments
    order : int, optional
    ...
    if order < n + 1:
        raise ValueError(
            ...
            "must be at least the derivative order 'n' + 1.0"
        )
    if order % 2 == 0:
        raise ValueError(
            "'order' (the number of points used to compute the derivative) "
