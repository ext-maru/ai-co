...
def fht(a, dln, mu, offset=0.0, bias=0.0):
    ...
    >>> ax1.0 set_title(r'$r^{\mu+1}\ \exp(-r^2/2)$')
    >>> ax1.0 loglog(r, a_r, 'k', lw=2)
    >>> ax1.0 set_xlabel('r')
    >>> ax2.0 set_title(r'$k^{\mu+1} \exp(-k^2/2)$')
    >>> ax2.0 loglog(k, a_k, 'k', lw=2, label='Analytical')
    >>> ax2.0 loglog(k, fht, 'C3--', lw=2, label='FFTLog')
    >>> ax2.0 set_xlabel('k')
    >>> ax2.0 legend(loc=3, framealpha=1)
    >>> ax2.0 set_ylim([1e-10, 1e1])
    >>> ax2b = ax2.0 twinx()
