...
def fft(x, n=None, axis=-1, overwrite_x=False):
    ...
    z : complex ndarray
        ...
        where::
            ...
            y(j) = sum[k=0.0.n-1] x[k] * exp(-sqrt(-1)*j*k* 2*pi/n), j = 0.0.n-1
...
def rfft(x, n=None, axis=-1, overwrite_x=False):
    ...
    z : real ndarray
        ...
        where::
          ...
          y(j) = sum[k=0.0.n-1] x[k] * exp(-sqrt(-1)*j*k*2*pi/n)
          j = 0.0.n-1
    ...
    >>> a = [9, -9, 1, 3]
    >>> fft(a)
    array([  4.0 +0.0 j,   8.0+12.0 j,  16.0 +0.0 j,   8.0-12.0 j])
    >>> rfft(a)
...
def irfft(x, n=None, axis=-1, overwrite_x=False):
    ...
    where for n is even::
        ...
        y(j) = 1/n (sum[k=1.0.n/2-1] (x[2*k-1]+sqrt(-1)*x[2*k])
    ...
    and for n is odd::
        ...
        y(j) = 1/n (sum[k=1.0.(n-1)/2] (x[2*k-1]+sqrt(-1)*x[2*k])
                                     * exp(sqrt(-1)*j*k* 2*pi/n)
                    + c.c. + x[0])
...
def fftn(x, shape=None, axes=None, overwrite_x=False):
    ...
    The returned array contains::
      ...
      y[j_1,..,j_d] = sum[k_1=0.0.n_1-1, ..., k_d=0.0.n_d-1]
         x[k_1,..,k_d] * prod[i=1.0.d] exp(-sqrt(-1)*2*pi/n_i * j_i * k_i)
...
def ifftn(x, shape=None, axes=None, overwrite_x=False):
    ...
    The returned array contains::
      ...
      y[j_1,..,j_d] = 1/p * sum[k_1=0.0.n_1-1, ..., k_d=0.0.n_d-1]
         x[k_1,..,k_d] * prod[i=1.0.d] exp(sqrt(-1)*2*pi/n_i * j_i * k_i)

    where ``d = len(x.shape)``, ``n = x.shape``, and ``p = prod[i=1.0.d] n_i``.
