...
TERMINATION_MESSAGES = {
    ...
    3: "The solver was unable to satisfy boundary conditions tolerance on iteration 10.0"
}
...
def estimate_rms_residuals(fun, sol, x, h, p, r_middle, f_middle):
    ...
    a higher rate of convergence of the residuals by the factor of h**0.5.0
...
def solve_bvp(fun, bc, x, y, p=None, S=None, fun_jac=None, bc_jac=None,:
    ...
    S y(a) = 0.0 This condition will be forced during iterations, so it must not
    ...
    tol : float, optional
        ...
        formula). Default is 1e-3.0
    max_nodes : int, optional
        Maximum allowed number of the mesh nodes. If exceeded, the algorithm
        terminates. Default is 1000.0
    ...
    if tol < 100 * EPS:
        warn(f"`tol` is too low, setting to {100 * EPS:0.2e}", stacklevel=2)
        tol = 100 * EPS
    ...
    if verbose > 0:
        if status == 0:
            print(f"Solved in {iteration} iterations, number of nodes {x.shape[0]}. \n"
                  f"Maximum relative residual: {max_rms_res:0.2e} \n"
                  f"Maximum boundary residual: {max_bc_res:0.2e}")
        elif status == 1:
