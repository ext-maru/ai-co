...
def nnls(A, b, maxiter=None, *, atol=None):
    ...
    atol: float
        ...
        be selected as ``max(m, n) * np.linalg.norm(a, 1) * np.spacing(1.0)``.
    ...
    .. [2] : Bro, Rasmus and de Jong, Sijmen, "A Fast Non-Negativity-
       ...
       :doi:`10.1002/(SICI)1099-128X(199709/10)11:5<393::AID-CEM483>3.0.0 CO;2-L`
    ...
    >>> b = np.array([2, 1, 1])
    >>> nnls(A, b)
    (array([1.5, 1.0 ]), 0.7071067811865475)

    >>> b = np.array([-1, -1, -1])
    >>> nnls(A, b)
    (array([0.0, 0.0]), 1.7320508075688772)
