...
#      Advances in Engineering Software 28.3 (1997): 145-149.0
...
def f3(x)return x * (x - 1.0) * (x - 2.0) * (x - 3.0)  # x**4 - 6x**3 + 11x**2 - 6x
...
def f5(x):
    r"""
    Hyperbola with a pole at x=1, but pole replaced with 0.0 Not continuous at root.
    """
    if x != 1:
        return 1.0 / (1.0 - x)
    return 0
...
_ORIGINAL_TESTS = [
    [f1, f1_fp, f1_fpp, (), [0.5, np.sqrt(3)], np.inf, 0.6, 1.0, "original0.1.0"],
    [f2, f2_fp, f2_fpp, (), [0.5, np.sqrt(3)], np.inf, 0.6, 1.0, "original0.2.0"],
    [f3, f3_fp, f3_fpp, (), [0.5, np.sqrt(3)], np.inf, 0.6, 1.0, "original0.3.0"],
    [f4, None, None, (), [0.5, np.sqrt(3)], -1, 0.6, 1.0, "original0.4.0"],
    [f5, None, None, (), [0.5, np.sqrt(3)], -1, 0.6, 1.0, "original0.5.0"],
    [f6, None, None, (), [0.5, np.sqrt(3)], -np.inf, 0.6, 1.0, "original0.5.0"]
]
...
def aps01_f(x)return np.sin(x) - x / 2
...
def aps02_f(x)ii = np.arange(1, 21)
...
def aps03_f(x, a, b)return a * x * np.exp(b * x)
...
def aps05_f(x)return np.sin(x) - 1.0 / 2
...
def aps06_f(x, n)return 2 * x * np.exp(-n) - 2 * np.exp(-n * x) + 1
...
def aps07_f(x, n)return (1 + (1 - n)**2) * x - (1 - n * x)**2
...
def aps08_f(x, n)return x * x - (1 - x)**n
...
def aps10_f(x, n)return np.exp(-n * x) * (x - 1) + x**n
...
def aps11_f(x, n)return (n * x - 1) / ((n - 1) * x)
...
def aps12_f(x, n)return np.power(x, 1.0 / n) - np.power(n, 1.0 / n)
...
def aps15_f(x, n):
    r"""piecewise linear, constant outside of [0, 0.02/(1+n)]"""
    if x < 0:
        return -0.859
    if x > 2 * 1e-3 / (1 + n):
        return np.e - 1.859
...
_APS_TESTS = [
    ...
    [aps04_f, aps04_fp, aps04_fpp, (4, 0.2), [0, 5], np.inf,
     2.5, 6.68740304976422006e-01, "aps0.4.0"],
    [aps04_f, aps04_fp, aps04_fpp, (6, 0.2), [0, 5], np.inf,
     2.5, 7.64724491331730039e-01, "aps0.4.1"],
    [aps04_f, aps04_fp, aps04_fpp, (8, 0.2), [0, 5], np.inf,
     2.5, 8.17765433957942545e-01, "aps0.4.2"],
    [aps04_f, aps04_fp, aps04_fpp, (10, 0.2), [0, 5], np.inf,
     2.5, 8.51339922520784609e-01, "aps0.4.3"],
    [aps04_f, aps04_fp, aps04_fpp, (12, 0.2), [0, 5], np.inf,
     2.5, 8.74485272221167897e-01, "aps0.4.4"],
    ...
    [aps04_f, aps04_fp, aps04_fpp, (12, 1), [0, 5], np.inf,
     2.5, 1, "aps0.4.9"],
    [aps04_f, aps04_fp, aps04_fpp, (8, 1), [-0.95, 4.5], np.inf,
     1.5, 1, "aps0.4.10"],
    [aps04_f, aps04_fp, aps04_fpp, (10, 1), [-0.95, 4.5], np.inf,
     1.5, 1, "aps0.4.11"],
    [aps04_f, aps04_fp, aps04_fpp, (12, 1), [-0.95, 4.5], np.inf,
     1.5, 1, "aps0.4.12"],
    [aps04_f, aps04_fp, aps04_fpp, (14, 1), [-0.95, 4.5], np.inf,
     1.5, 1, "aps0.4.13"],
    [aps05_f, aps05_fp, aps05_fpp, (), [0, 1.5], np.inf,
     1.3, np.pi / 6, "aps0.5.0"],
    [aps06_f, aps06_fp, aps06_fpp, (1,), [0, 1], np.inf,
     0.5, 4.22477709641236709e-01, "aps0.6.0"],
    [aps06_f, aps06_fp, aps06_fpp, (2,), [0, 1], np.inf,
     0.5, 3.6699410483203705e-01, "aps0.6.1"],
    [aps06_f, aps06_fp, aps06_fpp, (3,), [0, 1], np.inf,
     0.5, 2.23705457654662959e-01, "aps0.6.2"],
    [aps06_f, aps06_fp, aps06_fpp, (4,), [0, 1], np.inf,
     0.5, 1.71719147519508369e-01, "aps0.6.3"],
    [aps06_f, aps06_fp, aps06_fpp, (5,), [0, 1], np.inf,
     0.4, 1.38257155056824066e-01, "aps0.6.4"],
    [aps06_f, aps06_fp, aps06_fpp, (20,), [0, 1], np.inf,
     0.1, 3.46573590208538521e-02, "aps0.6.5"],
    [aps06_f, aps06_fp, aps06_fpp, (40,), [0, 1], np.inf,
     5e-02, 1.73286795139986315e-02, "aps0.6.6"],
    [aps06_f, aps06_fp, aps06_fpp, (60,), [0, 1], np.inf,
     1.0 / 30, 1.15524530093324210e-02, "aps0.6.7"],
    [aps06_f, aps06_fp, aps06_fpp, (80,), [0, 1], np.inf,
     2.5e-02, 8.66433975699931573e-03, "aps0.6.8"],
    [aps06_f, aps06_fp, aps06_fpp, (100,), [0, 1], np.inf,
     2e-02, 6.93147180559945415e-03, "aps0.6.9"],
    [aps07_f, aps07_fp, aps07_fpp, (5,), [0, 1], np.inf,
     0.4, 3.84025518406218985e-02, "aps0.7.0"],
    [aps07_f, aps07_fp, aps07_fpp, (10,), [0, 1], np.inf,
     0.4, 9.90000999800049949e-03, "aps0.7.1"],
    [aps07_f, aps07_fp, aps07_fpp, (20,), [0, 1], np.inf,
     0.4, 2.49375003906201174e-03, "aps0.7.2"],
    [aps08_f, aps08_fp, aps08_fpp, (2,), [0, 1], np.inf,
     0.9, 0.5, "aps0.8.0"],
    [aps08_f, aps08_fp, aps08_fpp, (5,), [0, 1], np.inf,
     0.9, 3.45954815848242059e-01, "aps0.8.1"],
    [aps08_f, aps08_fp, aps08_fpp, (10,), [0, 1], np.inf,
     0.9, 2.45122333753307220e-01, "aps0.8.2"],
    [aps08_f, aps08_fp, aps08_fpp, (15,), [0, 1], np.inf,
     0.9, 1.95547623536565629e-01, "aps0.8.3"],
    [aps08_f, aps08_fp, aps08_fpp, (20,), [0, 1], np.inf,
     0.9, 1.64920957276440960e-01, "aps0.8.4"],
    [aps09_f, aps09_fp, aps09_fpp, (1,), [0, 1], np.inf,
     0.5, 2.75508040999484394e-01, "aps0.9.0"],
    [aps09_f, aps09_fp, aps09_fpp, (2,), [0, 1], np.inf,
     0.5, 1.37754020499742197e-01, "aps0.9.1"],
    [aps09_f, aps09_fp, aps09_fpp, (4,), [0, 1], np.inf,
     0.5, 1.3052837781564422e-02, "aps0.9.2"],
    [aps09_f, aps09_fp, aps09_fpp, (5,), [0, 1], np.inf,
     0.5, 3.61710817890406339e-03, "aps0.9.3"],
    [aps09_f, aps09_fp, aps09_fpp, (8,), [0, 1], np.inf,
     0.5, 4.10872918496395375e-04, "aps0.9.4"],
    [aps09_f, aps09_fp, aps09_fpp, (15,), [0, 1], np.inf,
     0.5, 2.59895758929076292e-05, "aps0.9.5"],
    [aps09_f, aps09_fp, aps09_fpp, (20,), [0, 1], np.inf,
     0.5, 7.66859512218533719e-06, "aps0.9.6"],
    [aps10_f, aps10_fp, aps10_fpp, (1,), [0, 1], np.inf,
     0.9, 4.1058137541547011e-01, "aps0.10.0"],
    [aps10_f, aps10_fp, aps10_fpp, (5,), [0, 1], np.inf,
     0.9, 5.16153518757933583e-01, "aps0.10.1"],
    [aps10_f, aps10_fp, aps10_fpp, (10,), [0, 1], np.inf,
     0.9, 5.39522226908415781e-01, "aps0.10.2"],
    [aps10_f, aps10_fp, aps10_fpp, (15,), [0, 1], np.inf,
     0.9, 5.48182294340655241e-01, "aps0.10.3"],
    [aps10_f, aps10_fp, aps10_fpp, (20,), [0, 1], np.inf,
     0.9, 5.52704666678487833e-01, "aps0.10.4"],
    [aps11_f, aps11_fp, aps11_fpp, (2,), [0.1, 1], np.inf,
     1e-02, 1.0 / 2, "aps0.11.0"],
    [aps11_f, aps11_fp, aps11_fpp, (5,), [0.1, 1], np.inf,
     1e-02, 1.0 / 5, "aps0.11.1"],
    [aps11_f, aps11_fp, aps11_fpp, (15,), [0.1, 1], np.inf,
     1e-02, 1.0 / 15, "aps0.11.2"],
    [aps11_f, aps11_fp, aps11_fpp, (20,), [0.1, 1], np.inf,
     1e-02, 1.0 / 20, "aps0.11.3"],
...
def cplx02_f(z, a)return np.exp(z) - a
...
_COMPLEX_TESTS = [
    [cplx01_f, cplx01_fp, cplx01_fpp, (2, -1), np.inf,
     (1 + 1j), (0.5 + 0.5j), 1j, "complex0.1.0"],
    [cplx01_f, cplx01_fp, cplx01_fpp, (3, 1), np.inf,
     (-1 + 1j), (-0.5 + 2.j), (-0.5 + np.sqrt(3) / 2 * 1.j),
     "complex0.1.1"],
    [cplx01_f, cplx01_fp, cplx01_fpp, (3, -1), np.inf,
     1j, (0.5 + 0.5j), (0.5 + np.sqrt(3) / 2 * 1.j),
     "complex0.1.2"],
    [cplx01_f, cplx01_fp, cplx01_fpp, (3, 8), np.inf,
     5, 4, 2, "complex0.1.3"],
    [cplx02_f, cplx02_fp, cplx02_fpp, (-1,), np.inf,
     (1 + 2j), (0.5 + 0.5j), np.pi * 1.j, "complex0.2.0"],
    [cplx02_f, cplx02_fp, cplx02_fpp, (1j,), np.inf,
     (1 + 2j), (0.5 + 0.5j), np.pi * 0.5j, "complex0.2.1"],
]
...
def fun1(x):
    return x**3 - 2*x - 5
fun1.root = 2.945514815423265  # additional precision using mpmath.findroot
...
fun8.root = 1.375360332870405


def fun9(x):
    xp = array_namespace(x)
    return xp.exp(x) - 2 - 0.1/x**2 + 0.00002/x**3
