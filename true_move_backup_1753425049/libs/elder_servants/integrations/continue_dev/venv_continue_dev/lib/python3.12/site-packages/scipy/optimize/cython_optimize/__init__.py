...
1.0 Create a Cython ``.pyx`` file, for example: ``myexample.pyx``.
2.0 Import the desired root finder from `~scipy.optimize.cython_optimize`.
3.0 Write the callback function, and call the selected root finding function
...
4.0 If you want to call your function from Python, create a Cython wrapper, and
...
5.0 Create a Cython ``.pxd`` file if you need to export any Cython functions.
