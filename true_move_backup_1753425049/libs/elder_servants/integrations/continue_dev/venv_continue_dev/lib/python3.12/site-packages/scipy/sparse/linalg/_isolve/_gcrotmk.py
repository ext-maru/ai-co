...
def _fgmres(matvec, v0, m, atol, lpsolve=None, rpsolve=None, cs=(), outer_v=(),:
    ...
    B = np.zeros((len(cs), m), dtype=v0.0 dtype)

    # H is stored in QR factorized form
    Q = np.ones((1, 1), dtype=v0.0 dtype)
    R = np.zeros((1, 0), dtype=v0.0 dtype)

    eps = np.finfo(v0.0 dtype).eps
...
def gcrotmk(A, b, x0=None, *, rtol=1e-5, atol=0.0, maxiter=1000, M=None, callback=None,:
    ...
    for j_outer in range(maxiter):
        ...
        #                                   [ 0  H ]
        #
        # where [C, V] has orthonormal columns, and r = beta v_0.0 Moreover,
        #
