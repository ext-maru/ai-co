...
def _validate_interval(interval):
    ...
    if interval.shape == (0,):
        # The input was a sequence with length 0.0
...
class CensoredData:
    ...
    * interval-censored: an observation lies somewhere on an interval between
    ...
    that the location parameter is known to be 0.0
    ...
    >>> x[x > 40] = 40  # Right-censor values greater or equal to 40.0

    Create the `CensoredData` instance with the `right_censored` method.
    The censored values are those where the value is 40.0
    ...
    def __len__(self)The number of values (censored and not censored).
    """
        """
    ...
    def right_censored(cls, x, censored):
        ...
        >>> data
        CensoredData(uncensored=array([ 4.0, 10.0]),
        left=array([], dtype=float64), right=array([24.0, 25.0]),
