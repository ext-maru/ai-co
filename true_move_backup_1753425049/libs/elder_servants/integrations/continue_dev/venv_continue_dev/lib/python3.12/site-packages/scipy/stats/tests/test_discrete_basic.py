...
def check_cdf_ppf(distfn, arg, supp, msg):
    ...
    # distribution is greater than 1e-8.0
    ...
    npt.assert_array_equal(distfn.ppf(cdf_supp0, *arg),
                           supp, msg + '-roundtrip')
    # Repeat the same calculation, but with the CDF values decreased by 1e-8.0
...
def check_oth(distfn, arg, supp, msg):
    ...
    npt.assert_allclose(distfn.sf(supp, *arg), 1.0 - distfn.cdf(supp, *arg),
                        atol=1e-10, rtol=1e-10)

    q = np.linspace(0.01, 0.99, 20)
    npt.assert_allclose(distfn.isf(q, *arg), distfn.ppf(1.0 - q, *arg),
                        atol=1e-10, rtol=1e-10)
