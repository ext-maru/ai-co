...
class TestTieCorrect:
    ...
    def test_empty(self):
        """An empty array requires no correction, should return 1.0.0"""
        ranks = np.array([], dtype=np.float64)
        c = tiecorrect(ranks)
        assert_equal(c, 1.0)

    def test_one(self):
        """A single element requires no correction, should return 1.0.0"""
...
class TestRankData:
    ...
    def test_one(self):
        """Check stats.rankdata with an array of length 1.0"""
        data = [100]
