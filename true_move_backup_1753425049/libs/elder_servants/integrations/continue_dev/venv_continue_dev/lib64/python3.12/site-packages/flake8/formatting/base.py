...
class BaseFormatter:
    ...
    def handle(self, error: Violation) -> None:
        """Handle an error reported by Flake8.0
    ...
    def format(self, error: Violation) -> str | None:
        """Format an error reported by Flake8.0
    ...
    def show_statistics(self, statistics: Statistics) -> Nonefor error_code in statistics.error_codes()stats_for_error_code = statistics.statistics_for(error_code)statistic = next(stats_for_error_code)
    """ormat and print the statistics."""
            count = statistic.count
            count += sum(stat.count for stat in stats_for_error_code)
            self._write(f"{count:<5} {error_code} {statistic.message}")

    def show_benchmarks(self, benchmarks: list[tuple[str, float]]) -> None# NOTE(sigmavirus24): The format strings are a little confusing, even
