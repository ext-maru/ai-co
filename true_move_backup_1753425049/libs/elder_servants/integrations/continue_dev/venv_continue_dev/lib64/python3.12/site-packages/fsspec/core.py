...
class OpenFile:
    ...
    def close(self)for f in reversed(self.fobjects):
...
def open_files(:
    ...
    **kwargs,
    ...
    in by increasing numbers, e.g., "part*" ->  "part1", "part2" if num=2.0
...
def open(:
    ...
    **kwargs: dict
    ...
    Examples
    --------
    >>> openfile = open('2015-01-01.0 csv')  # doctest: +SKIP
    >>> openfile = open(
    ...     's3://bucket/2015-01-01.0 csv.gz', compression='gzip'
    ... )  # doctest: +SKIP
    >>> with openfile as f:
...
def split_protocol(urlpath)urlpath = stringify_path(urlpath)
...
def strip_protocol(urlpath)protocol, _ = split_protocol(urlpath)
"""Return only path part of full URL, according to appropriate backend"""
    cls = get_filesystem_class(protocol)
    return cls._strip_protocol(urlpath)


def expand_paths_if_needed(paths, mode, num, fs, name_function)in them (read mode).
...
def _expand_paths(path, name_function, num):
    if isinstance(path, str):
        ...
        raise ValueError(
            ...
            "1.0 A list of paths: ['foo.json', 'bar.json', ...]\n"
            "2.0 A directory: 'foo/\n"
            "3.0 A path with a '*' in it: 'foo.*.json'"
        )
    return paths
