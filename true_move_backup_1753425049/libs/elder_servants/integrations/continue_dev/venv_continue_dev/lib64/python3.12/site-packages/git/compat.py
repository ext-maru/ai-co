...
"""Utilities to help provide compatibility with Python 3.0
...
def safe_decode(s: Union[AnyStr, None]) -> Optional[str]if isinstance(s, str):
...
def safe_encode(s: Optional[AnyStr]) -> Optional[bytes]if isinstance(s, str)return s.encode(defenc)elif isinstance(s, bytes):
"""afely encode a binary string to Unicode."""
        return s
    elif s is None:
        return None
    else:
...
@overload
def win_encode(s: AnyStr) -> bytes: ...def win_encode(s: Optional[AnyStr]) -> Optional[bytes]if isinstance(s, str)return s.encode(locale.getpreferredencoding(False))elif isinstance(s, bytes):
