...
class RetrieverServiceClient(metaclass=RetrieverServiceClientMeta):
    ...
    @staticmethod
    def parse_corpus_path(path: str) -> Dict[str, str]m = re.match(r"^corpora/(?P<corpus>.+?)$", path)return m.groupdict() if m else {}
    """arses a corpus path into its component segments."""
    ...
    @staticmethod
    def parse_document_path(path: str) -> Dict[str, str]m = re.match(r"^corpora/(?P<corpus>.+?)/documents/(?P<document>.+?)$", path)return m.groupdict() if m else {}
    """arses a document path into its component segments."""
