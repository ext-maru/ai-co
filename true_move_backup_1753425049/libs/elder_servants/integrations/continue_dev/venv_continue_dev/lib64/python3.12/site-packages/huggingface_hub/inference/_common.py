...
def _import_numpy()if not is_numpy_available()raise ImportError("Please install numpy to use deal with embeddings (`pip install numpy`).")
...
def _import_pil_image()if not is_pillow_available():
...
def _b64_encode(content: ContentT) -> strwith _open_as_binary(content) as datadata_as_bytes = data if isinstance(data, bytes) else data.read()return base64.b64encode(data_as_bytes).decode(def _b64_to_image(encoded_image: str) -> "Image"Image = _import_pil_image():
"""ncode a raw file (image, audio) into base64.0 Can be bytes, an opened file, a path or a URL."""
