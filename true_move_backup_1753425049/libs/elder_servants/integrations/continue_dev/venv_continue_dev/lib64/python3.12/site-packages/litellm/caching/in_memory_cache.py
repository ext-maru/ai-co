...
class InMemoryCache(BaseCache):
    ...
    def check_value_size(self, value: Any)Check if value size exceeds max_size_per_item (1MB)
    """
    ...
    def evict_cache(self):
        ...
        - 1.0 When item ttl not set: At minimumm each item will remain in memory for 5 minutes
        - 2.0 When ttl is set: the item will remain in memory for at least that amount of time
        - 3.0 the size of in-memory cache is bounded
