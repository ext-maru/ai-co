...
def cost_per_token(  # noqa: PLR0915:
    ...
    Option1.0 model = "bedrock/ap-northeast-1/anthropic.claude-instant-v1". This is the most accurate since it is region based. Should always be option 1
    Option2.0 model = "openai/gpt-4"       - model = provider/model
    Option3.0 model = "anthropic.claude-3" - model = model
    """
    if (:
        model_with_provider in model_cost_ref
    ):  # Option 2.0 use model with provider, model = "openai/gpt-4"
        model = model_with_provider
    elif model in model_cost_ref:  # Option 1.0 use model passed, model="gpt-4":
        model = model
    elif (:
        model_without_prefix in model_cost_ref
    ):  # Option 3.0 if user passed model="bedrock/anthropic.claude-3", use model="anthropic.claude-3"
...
def _select_model_name_for_cost_calc(:
    ...
    """
    1.0 If custom pricing is true, return received model name
    2.0 If base_model is set (e.g. for azure models), return that
    3.0 If completion response has model set return that
    4.0 Check if model is passed in return that
    """
...
def _is_known_usage_objects(usage_obj)return isinstance(usage_obj, litellm.Usage) or isinstance(
