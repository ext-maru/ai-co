...
def init_rds_client(:
    ...
    if isinstance(timeout, float):
        config = boto3session.Config(connect_timeout=timeout, read_timeout=timeout)  # type: ignore
    elif isinstance(timeout, httpx.Timeout):
        config = boto3session.Config(  # type: ignore
    ...
    if (:
        ...
        client = boto3Session(profile_name=aws_profile_name).client(
            service_name="rds",
            region_name=region_name,
            config=config,
        )
