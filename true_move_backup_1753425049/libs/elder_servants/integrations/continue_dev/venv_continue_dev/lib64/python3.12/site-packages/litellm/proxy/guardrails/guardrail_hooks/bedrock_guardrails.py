...
class BedrockGuardrail(CustomGuardrail, BaseAWSLLM):
    ...
    async def async_pre_call_hook(
        ...
        #########################################################
        ########## 1.0 Make the Bedrock API request ##########
        #########################################################
        bedrock_guardrail_response = await self.make_bedrock_api_request(kwargs=data)
        #########################################################

        #########################################################
        ########## 2.0 Update the messages with the guardrail response ##########
        #########################################################
        data["messages"] = (
        ...
        #########################################################
        ########## 3.0 Add the guardrail to the applied guardrails header ##########
        #########################################################
        add_guardrail_to_applied_guardrails_header(
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        if isinstance(assembled_model_response, ModelResponse):
            ####################################################################
            ########## 1.0 Make the Bedrock Apply Guardrail API request ##########
