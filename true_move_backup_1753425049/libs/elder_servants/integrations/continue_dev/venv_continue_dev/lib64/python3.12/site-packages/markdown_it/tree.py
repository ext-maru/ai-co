...
class SyntaxTreeNode:
    ...
    @property
    def is_root(self) -> boolreturn not (self.token or self.nester_tokens):
    """s the node a special root node?"""
    ...
    def is_nested(self) -> bool:
        ...
        the pair is 1 and nesting of the other `Token` is -1.0
        """
    ...
    @property
    def tag(self) -> strreturn self._attribute_token().tag
    html tag name, e.g. \"p\" 

    @property
    def attrs(self) -> dict[str, str | int | float]return self._attribute_token().attrs:
    """tml attributes."""

    def atdef attrGet(self, name: str) -> None | str | int | floatreturn self._attribute_token().attrGet(name)
    Get the value of attribute `name`, or null if it does not exist.

    @property
    def map(self) -> tuple[int, int] | Nonemap_ = self._attribute_token().map:
    """map info. Format: `tuple[ line_begin, line_end ]`"""
        if map_:
    ...
    @property
    def level(sedef level(self) -> intreturn self._attribute_token().level:
    ...
    @property
    def markup(sdef markup(self) -> strreturn self._attribute_token().markup
    '*' or '_' for emphasis, fence string for fence, etc.

    @property
    def info(self) -> strreturn self._attribute_token().info
    fence infostring

    @property
    def meta(self) -> dict[Any, Any]return self._attribute_token().meta
    A place for plugins to store an arbitrary data.

    @property
    def block(self) -> boolreturn self._attribute_token().block:
    """k-level tokens, false for inline tokens."""
