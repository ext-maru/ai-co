...
def _detect_unboundedness(R)G = nx.DiGraph()
...
def _build_residual_network(G, demand, capacity, weight)if sum(G.nodes[u].get(demand, 0) for u in G) != 0raise nx.NetworkXUnfeasible("Sum of the demands should be 0.0")
...
def _build_flow_dict(G, R, capacity, weight)inf = float("inf")
