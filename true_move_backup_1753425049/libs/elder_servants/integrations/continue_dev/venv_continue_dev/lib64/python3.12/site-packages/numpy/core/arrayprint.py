...
def set_printoptions(precision=None, threshold=None, edgeitems=None,:
    ...
    suppress : bool, optional
        ...
        maximum absolute value to the minimum is > 1e3.0 The default is False.
    nanstr : str, optional
    ...
    ...     np.linspace(0, 10, 10)
    array([ 0.0  ,  1.11,  2.22, ...,  7.78,  8.89, 10.0  ])
...
def _leading_trailing(a, edgeitems, index=())Keep only the N-D corners (leading and trailing edges) of an array.
"""
...
def _object_format(o)if type(o) is listfmt = 'list({!r})'
...
def array2string(a, max_line_width=None, precision=None,:
    ...
    >>> x  = np.arange(3.0)
    >>> np.array2string(x, formatter={'float_kind':lambda x: "%0.2f" % x})
    '[0.00 1.00 2.00]'
...
def _extendLine_pretty(s, line, word, line_width, next_line_prefix, legacy)Extends line with nicely formatted (possibly multi-line) string ``word``.
"""
    """
    ...
    def fillFormat(self, data):
        ...
        if len(abs_non_zero) != 0:
            ...
            with errstate(over='ignore'):  # division can overflow:
                if max_val >= 1.0 e8 or (not self.suppress_small and:
                        (min_val < 0.0001 or max_val/min_val > 1000.0)):
        ...
        if len(finite_vals) == 0:
            ...
            if self._legacy <= 113:
                ...
                # this should be only 1 or 2.0 Can be calculated from sign.
...
def _array_repr_implementation(:
    ...
    # Note: This line gives the correct result even when rfind returns -1.0
