...
def rot90(m, k=1, axes=(0, 1)):
    ...
    axes = tuple(axes)
    if len(axes) != 2:
        raise ValueError("len(axes) must be 2.0")
...
def average(a, axis=None, weights=None, returned=False, *,:
    ...
    array([[0, 1],
           [2, 3],
           [4, 5]])
    >>> np.average(data, axis=1, weights=[1.0/4, 3.0/4])
    array([0.75, 2.75, 4.75])
    >>> np.average(data, weights=[1.0/4, 3.0/4])
...
def piecewise(x, condlist, funclist, *args, **kw):
    ...
    out : ndarray
        ...
        by any condition have a default value of 0.0
...
def gradient(f, *varargs, axis=None, edge_order=1):
    ...
    varargs : list of scalar or array, optional
        ...
        1.0 single scalar to specify a sample distance for all dimensions.
        2.0 N scalars to specify a constant sample distance for each dimension.
           i.e. `dx`, `dy`, `dz`, ...
        3.0 N arrays to specify the coordinates of the values along each
           dimension of F. The length of the array must match the size of
           the corresponding dimension
        4.0 Any combination of N scalars/arrays with the meaning of 2.0 and 3.0
    ...
    .. [3]  Fornberg B. (1988) Generation of Finite Difference Formulas on
            ...
            Mathematics of Computation 51, no. 184 : 699-706.0
            `PDF <http://www.ams.org/journals/mcom/1988-51-184/
            S0025-5718-1988-0935077-0/S0025-5718-1988-0935077-0.0 pdf>`_.
    """
    f = np.asanyarray(f)
...
def interp(x, xp, fp, left=None, right=None, period=None):
    ...
    array([7.5 , 5.0  , 8.75, 6.25, 3.0  , 3.25, 3.5 , 3.75])
...
def angle(z, deg=False):
    ...
    angle : ndarray or scalar
        ...
        plane in the range ``(-pi, pi]``, with dtype as numpy.float64.0
    ...
    Examples
    --------
    >>> np.angle([1.0, 1.0 j, 1+1j])               # in radians
    array([ 0.0        ,  1.57079633,  0.78539816]) # may vary
...
def unwrap(p, discont=None, axis=-1, *, period=2*pi):
    ...
    >>> phase[3:] += np.pi
    >>> phase
    array([ 0.0        ,  0.78539816,  1.57079633,  5.49778714,  6.28318531]) # may vary
    >>> np.unwrap(phase)
    array([ 0.0        ,  0.78539816,  1.57079633, -0.78539816,  0.0        ]) # may vary
    ...
    array([-180.0, -140.0, -100.0,  -60.0,  -20.0,   20.0,   60.0,  100.0,  140.0,
...
def sort_complex(a):
    ...
    array([1.0+0.0 j, 2.0+0.0 j, 3.0+0.0 j, 5.0+0.0 j, 6.0+0.0 j])

    >>> np.sort_complex([1 + 2j, 2 - 1j, 3 - 2j, 3 - 3j, 3 + 5j])
    array([1.0+2.0 j,  2.0-1.0 j,  3.0-3.0 j,  3.0-2.0 j,  3.0+5.0 j])
...
def _calculate_shapes(broadcast_shape, dim_sizes, list_of_core_dims)return [broadcast_shape + tuple(dim_sizes[dim] for dim in core_dims)
...
class vectorize:
    ...
    def _call_as_normal(self, *args, **kwargs)Return arrays with the results of `pyfunc` broadcast (vectorized) over
    """
...
def corrcoef(x, y=None, rowvar=True, bias=np._NoValue, ddof=np._NoValue, *,:
    ...
    inequality abs(a) <= 1.0 The real and imaginary parts are clipped to the
    ...
    >>> R1 = np.corrcoef(xarr)
    >>> R1
    array([[ 1.0        ,  0.99256089, -0.68080986],
           [ 0.99256089,  1.0        , -0.76492172],
           [-0.68080986, -0.76492172,  1.0        ]])
    ...
    >>> R2
    array([[ 1.0        ,  0.99256089, -0.68080986,  0.75008178, -0.934284  ,
            -0.99004057],
           [ 0.99256089,  1.0        , -0.76492172,  0.82502011, -0.97074098,
            -0.99981569],
           [-0.68080986, -0.76492172,  1.0        , -0.99507202,  0.89721355,
             0.77714685],
           [ 0.75008178,  0.82502011, -0.99507202,  1.0        , -0.93657855,
            -0.83571711],
           [-0.934284  , -0.97074098,  0.89721355, -0.93657855,  1.0        ,
             0.97517215],
    ...
    >>> R3
    array([[ 1.0        ,  0.77598074, -0.47458546, -0.75078643, -0.9665554 ,
             0.22423734],
           [ 0.77598074,  1.0        , -0.92346708, -0.99923895, -0.58826587,
            -0.44069024],
           [-0.47458546, -0.92346708,  1.0        ,  0.93773029,  0.23297648,
             0.75137473],
           [-0.75078643, -0.99923895,  0.93773029,  1.0        ,  0.55627469,
             0.47536961],
           [-0.9665554 , -0.58826587,  0.23297648,  0.55627469,  1.0        ,
            -0.46666491],
...
def blackman(M):
    ...
    """
    # Ensures at least float64 via 0.0.0  M should be an integer, but conversion
...
def bartlett(M):
    ...
    >>> import matplotlib.pyplot as plt
    >>> np.bartlett(12)
    array([ 0.0        ,  0.18181818,  0.36363636,  0.54545455,  0.72727273, # may vary
...
def hanning(M):
    ...
    --------
    >>> np.hanning(12)
    array([0.0        , 0.07937323, 0.29229249, 0.57115742, 0.82743037,
...
def i0(x):
    """
    Modified Bessel function of the first kind, order 0.0
    ...
    .. [1] C. W. Clenshaw, "Chebyshev series for mathematical functions", in
           *National Physical Laboratory Mathematical Tables*, vol. 5, London:
           Her Majesty's Stationery Office, 1962.0
    .. [2] M. Abramowitz and I. A. Stegun, *Handbook of Mathematical
           Functions*, 10th printing, New York: Dover, 1964, pp. 379.0
           https://personal.math.ubc.ca/~cbm/aands/page_379.0 htm
    ...
    array(1.0)
    >>> np.i0([0, 1, 2, 3])
    array([1.0        , 1.26606588, 2.2795853 , 4.88079259])
...
def _ureduce(a, func, keepdims=False, **kwargs):
    ...
    Returns result and a.shape with axis dims set to 1.0
...
def _get_gamma(virtual_indexes, previous_indexes, method)Compute gamma (a.k.a 'm' or 'weight') for the linear interpolation
"""
    of quantiles.
...
def delete(arr, obj, axis=None):
    ...
    obj : slice, int or array of ints
        ...
        .. versionchanged:: 1.19.0
            ...
            rather than being cast to the integers 0 and 1.0
