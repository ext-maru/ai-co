...
class TestAverage:
    ...
    def test_weights(self):
        ...
        # assert_equal(average(y1, weights=w1), 9.0/2.0)
...
class TestGradient:
    ...
    def test_second_order_accurate(self):
        ...
        assert_(np.all(num_error < 0.3) == True)
...
class TestVectorize:
    ...
    def test_string_ticket_1892(self):
        # Test vectorization over strings: issue 1892.0
...
class TestCorrCoef:
    ...
    B = np.array(
        [[0.10377691, 0.5417086, 0.49807457],
         ...
         [0.9314666, 0.66800209, 0.3538394]])
    res1 = np.array(
        [[1.0, 0.9379533, -0.4931983],
         [0.9379533, 1.0, 0.30007991],
         [-0.4931983, 0.30007991, 1.0]])
    res2 = np.array(
        [[1.0, 0.9379533, -0.4931983, 0.30151751, 0.66318558, 0.51532523],
         [0.9379533, 1.0, 0.30007991, -0.4781421, 0.88157256, 0.78052386],
         [-0.4931983, 0.30007991, 1.0, -0.96717111, 0.71483595, 0.83053601],
         [0.30151751, -0.4781421, -0.96717111, 1.0, -0.51366032, -0.66173113],
    ...
    def test_xy(self):
        x = np.array([[1, 2, 3]])
        y = np.array([[1j, 2j, 3j]])
        assert_allclose(np.corrcoef(x, y), np.array([[1.0, -1.j], [1.j, 1.0]]))
...
class TestCov:
    ...
    def test_xy(self):
        x = np.array([[1, 2, 3]])
        y = np.array([[1j, 2j, 3j]])
        assert_allclose(cov(x, y), np.array([[1.0, -1.j], [1.j, 1.0]]))
    ...
    def test_1D_rowvar(self):
        ...
        y = np.array([0.780, 0.3107, 0.2111, 0.334, 0.8501])
...
class Test_I0:
    ...
    def test_simple(self):
        assert_almost_equal(
            i0(0.5),
            np.array(1.634833707413234))

        # need at least one test above 8, as the implementation is piecewise
        A = np.array([0.49842636, 0.6969809, 0.22011976, 0.155549, 10.0])
        expected = np.array([1.6307822, 1.12518299, 1.1214991, 1.0006049, 2815.71662847])
        assert_almost_equal(i0(A), expected)
        assert_almost_equal(i0(-A), expected)

        B = np.array([[0.827002, 0.99959078],
                      [0.89694769, 0.39298162],
                      [0.37954418, 0.5206293],
                      [0.36465447, 0.72446427],
                      [0.48164949, 0.50324519]])
        assert_almost_equal(
            i0(B),
            np.array([[1.17843223, 1.26583466],
                      [1.21147086, 1.3898290],
                      [1.3633899, 1.0067775],
                      [1.3352052, 1.13557954],
                      [1.5884290, 1.6432317]]))
...
class TestKaiser:
    ...
    def test_simple(self):
        ...
        assert_almost_equal(kaiser(5, 1.0),
                            np.array([0.78984831, 0.94503323, 1.0,
                                      0.94503323, 0.78984831]))
        assert_almost_equal(kaiser(5, 1.56789),
                            np.array([0.58285404, 0.88409679, 1.0,
...
class TestPiecewise:
    ...
    def test_multidimensional_extrafunc(self):
        ...
        assert_array_equal(y, np.array([[-1.0, -1.0, -1.0],
                                        [3.0, 3.0, 1.0]]))
...
class TestInterp:
    ...
    def test_non_finite_any_nan(self, sc)assert_equal(np.interp(0.5, [np.nan,      1], sc([     0,     10])), sc(np.nan))
    ...
    def test_non_finite_inf(self, sc)assert_equal(np.interp(0.5, [-np.inf, +np.inf], sc([      0,      10])), sc(np.nan))
    ...
    def test_non_finite_half_inf_xf(self, sc)assert_equal(np.interp(0.5, [-np.inf,       1], sc([-np.inf,      10])), sc(np.nan))
    ...
    def test_non_finite_half_inf_x(self, sc)assert_equal(np.interp(0.5, [-np.inf, -np.inf], sc([0, 10])), sc(10))
    ...
    def test_non_finite_half_inf_f(self, sc)assert_equal(np.interp(0.5, [0, 1], sc([      0, -np.inf])), sc(-np.inf))
    ...
    def test_complex_interp(self):
        ...
        x = np.linspace(0, 1, 5)
        y = np.linspace(0, 1, 5) + (1 + np.linspace(0, 1, 5))*1.j
        x0 = 0.3
        y0 = x0 + (1+x0)*1.j
        ...
        xp = [190, -190, 350, -350]
        fp = [5+1.j, 10+2j, 3+3j, 4+4j]
        y = [7.5+1.5j, 5.0+1.j, 8.75+1.75j, 6.25+1.25j, 3.0+3j, 3.25+3.25j,
             3.5+3.5j, 3.75+3.75j]
...
class TestQuantile:
    ...
    def test_max_ulp(self):
        ...
        # The default linear method would result in 0 + 0.2 * (0.45/2) = 0.18.0
        # 0.18 is not exactly representable and the formula leads to a 1 ULP
        # different result. Ensure it is this exact within 1 ULP, see gh-20331.0
    ...
    def test_quantile_identification_equation(self, method, alpha):
        ...
        # level alpha), see
        # https://doi.org/10.48550/arXiv0.912.902        
    ...
    def test_quantile_add_and_multiply_constant(self, method, alpha):
        # Test that
        #  1.0 quantile(c + x) = c + quantile(x)
        #  2.0 quantile(c * x) = c * quantile(x)
        #  3.0 quantile(-x) = -quantile(x, 1 - alpha)
...
class TestMedian:
    ...
    def test_basic(self):
        ...
        a = np.array([0.444502, 0.463301, 0.141249, 0.606775])
        assert_almost_equal((a[1] + a[3]) / 2.0, np.median(a))
        a = np.array([0.463301, 0.444502, 0.141249])
        assert_equal(a[0], np.median(a))
        a = np.array([0.444502, 0.141249, 0.463301])
    ...
    @pytest.mark.parametrize('arr',
                             ([1.0, 2.0, 3.0], [1.0, np.nan, 3.0], np.nan, 0.0))
    def test_subclass2(self, arr)class MySubclass(np.ndarray):
