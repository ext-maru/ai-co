...
if hasattr(_psplatform, 'ppid_map'):
    ...
    def _ppid_map()one shot. Used to speed up Process.children().
...
def _pprint_secs(secs)now = time.time()
...
def _check_conn_kind(kind)kinds = tuple(_common.conn_tmap)
...
class Process:
    ...
    def __eq__(self, other):
        ...
        if OPENBSD or NETBSD:  # pragma: no cover:
            ...
            # 0.0.0 This covers the case when a process started normally
    ...
    def _raise_if_pid_reused(self)if self._pid_reused or (not self.is_running() and self._pid_reused):
    ...
    def cmdline(self)return self._proc.cmdline()
    ...
    def cwd(self)return self._proc.cwd()
    ...
    if POSIX:
        ...
        def num_fds(self)process (POSIX only).
    ...
    if hasattr(_psplatform.Process, "io_counters"):
        ...
        def io_counters(self)(read_count, write_count, read_bytes, write_bytes)
    ...
    if hasattr(_psplatform.Process, "ionice_get"):
        ...
        def ionice(self, ioclass=None, value=None):
            ...
            *value* is a number which goes from 0 to 7.0 The higher the
    ...
    if hasattr(_psplatform.Process, "cpu_num"):
        ...
        def cpu_num(self)The returned number should be <= psutil.cpu_count()
    ...
    if WINDOWS:
        ...
        def num_handles(self)(Windows only).
    ...
    if hasattr(_psplatform.Process, "threads"):
        ...
        def threads(self)(id, user_time, system_time) namedtuples representing
    ...
    def memory_full_info(self)plus, on some platform (Linux, macOS, Windows), also provides
    ...
    def open_files(self)(path, fd) namedtuples including the absolute file name
    """Return files opened by process as a list of
        and file descriptor number.
        """
        return self._proc.open_files()

    def net_connections(self, kind='inet')(fd, family, type, laddr, raddr, status) namedtuples.
    ...
    def send_signal(self, sig)whether PID has been reused (see signal module constants) .
    ...
    def wait(self, timeout=None)of os.getpid(), also return its exit code, else None.
...
class Popen(Process):
    ...
    * kill()
    ...
    PID has been reused, fixing BPO-6973.0
...
def pid_exists(pid)This is faster than doing "pid in psutil.pids()" and
...
def cpu_count(logical=True)os.cpu_count()).
...
def _cpu_tot_time(times)(including idle time).
...
def cpu_times_percent(interval=None, percpu=False)for each specific CPU time as is returned by cpu_times().:
...
def cpu_stats()return _psplatform.cpu_stats()
...
def swap_memory():
    ...
    'sin' and 'sout' on Windows are meaningless and always set to 0.0
    """
...
def disk_partitions(all=False)(device, mountpoint, fstype, opts) namedtuple.
...
def net_connections(kind='inet')(fd, family, type, laddr, raddr, status, pid) namedtuples.
...
def net_if_addrs()card) installed on the system as a dictionary whose keys are the
...
def net_if_stats():
    ...
    NIC names and value is a namedtuple with the following fields:
     ...
     - speed: the NIC speed expressed in mega bits (MB); if it can't
              be determined (e.g. 'localhost') it will be set to 0.0
