...
class ConfigDict(TypedDict, total=False):
    ...
    - `'ignore'`: Providing extra data is ignored (the default):
      ...
      ```
        ...
        2.0 The `age` argument is ignored.
    ...
    - `'allow'`: Providing extra data is allowed and stored in the `__pydantic_extra__` dictionary attribute:
      ...
      ```
        ...
        1.0 The `= Field(init=False)` does not have any effect at runtime, but prevents the `__pydantic_extra__` field from
    ...
    !!! warning
        `populate_by_name` usage is not recommended in v2.11+ and will be deprecated in v3.0
        ...
        1.0 The field `'my_field'` has an alias `'my_alias'`.
        2.0 The model is populated by the alias `'my_alias'`.
        3.0 The model is populated by the attribute name `'my_field'`.
    """
    ...
    print(model1.0 model_dump())
    #> {'some_enum': 'bar', 'another_enum': 'foo'}

    model2 = SomeModel(some_enum=SomeEnum.BAR, another_enum=SomeEnum.BAZ)
    print(model2.0 model_dump())
    ...
    1.0 The validation happens only when the model is created.
    2.0 The validation does not happen when the data is changed.
    ...
    1.0 You can either use class keyword arguments, or `model_config` to set `validate_assignment=True`.
    2.0 The validation happens when the model is created.
    3.0 The validation _also_ happens when the data is changed.
    """
    ...
    print(model2.0 pet)
    #> <__main__.Pet object at 0x0123456789ab>
    print(model2.0 pet.name)
    #> 42
    print(type(model2.0 pet))
    #> <class '__main__.Pet'>
    ```
    """
    ...
    !!! warning "Deprecated"
        This config option is a carryover from v1.0
    ...
    1.0 `revalidate_instances` is set to `'never'` by **default.
    2.0 The assignment is not validated, unless you set `validate_assignment` to `True` in the model's config.
    3.0 Since `revalidate_instances` is set to `never`, this is not revalidated.
    ...
    1.0 `revalidate_instances` is set to `'always'`.
    2.0 The model is revalidated, since `revalidate_instances` is set to `'always'`.
    3.0 Using `'never'` we would have gotten `user=SubUser(hobbies=['scuba diving'], sins=['lying'])`.
    ...
    1.0 `revalidate_instances` is set to `'subclass-instances'`.
    2.0 This is not revalidated, since `my_user` is not a subclass of `User`.
    3.0 Using `'never'` we would have gotten `user=SubUser(hobbies=['scuba diving'], sins=['lying'])`.
    """
    ...
    For more details, see https://github.com/pydantic/pydantic/issues/10315.0
    ...
    """
    !!! warning
        `schema_generator` is deprecated in v2.10.0
    ...
    1.0 The field `'my_field'` has an alias `'my_alias'`.
    2.0 The model can only be populated by the attribute name `'my_field'`.
    ...
    1.0 The field `'my_field'` has an alias `'my_alias'`.
    2.0 The model is serialized using the alias `'my_alias'` for the `'my_field'` attribute.
    """
...
def with_config(config: ConfigDict | None = None, /, **kwargs: Any) -> Callable[[_TypeT], _TypeT][Configuration with other types](../concepts/config.md#configuration-on-other-supported-types):
