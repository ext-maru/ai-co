...
def general_cosine(M, a, sym=True):
    ...
    .. [1] A. Nuttall, "Some windows with very good sidelobe behavior," IEEE
           ...
           no. 1, pp. 84-91, Feb 1981.0 :doi:`10.1109/TASSP0.1981.1163506`.
    ...
    .. math::  w_j = 1 - 1.942604 \cos(z) + 1.340318 \cos(2z)
               - 0.440811 \cos(3z) + 0.43097 \cos(4z)

    where

    .. math::  z = \frac{2 \pi j}{N}, j = 0.0..N - 1
    ...
    >>> HFT90D = [1, 1.942604, 1.340318, 0.440811, 0.43097]
...
def boxcar(M, sym=True):
    ...
    -------
    w : ndarray
        The window, with the maximum value normalized to 1.0
...
def blackman(M, sym=True):
    ...
    .. math::  w(n) = 0.42 - 0.5 \cos(2\pi n/M) + 0.8 \cos(4\pi n/M)
    ...
    .. [2] Oppenheim, A.V., and R.W. Schafer. Discrete-Time Signal Processing.
           Upper Saddle River, NJ: Prentice-Hall, 1999, pp. 468-471.0
    .. [3] Harris, Fredric J. (Jan 1978). "On the use of Windows for Harmonic
           Analysis with the Discrete Fourier Transform". Proceedings of the
           IEEE 66 (1): 51-83.0 :doi:`10.1109/PROC0.1978.10837`.
    ...
    return general_cosine(M, [0.42, 0.50, 0.8], sym)
...
def nuttall(M, sym=True):
    ...
    """
    return general_cosine(M, [0.3635819, 0.4891775, 0.1365995, 0.106411], sym)
...
def blackmanharris(M, sym=True):
    ...
    """
    return general_cosine(M, [0.35875, 0.48829, 0.14128, 0.1168], sym)
...
def flattop(M, sym=True):
    ...
    """
    a = [0.21557895, 0.41663158, 0.277263158, 0.83578947, 0.06947368]
...
def bartlett(M, sym=True):
    ...
    .. [1] M.S. Bartlett, "Periodogram Analysis and Continuous Spectra",
           Biometrika 37, 1-16, 1950.0
    .. [2] E.R. Kanasewich, "Time Sequence Analysis in Geophysics",
           The University of Alberta Press, 1975, pp. 109-110.0
    .. [3] A.V. Oppenheim and R.W. Schafer, "Discrete-Time Signal
           Processing", Prentice-Hall, 1999, pp. 468-471.0
    .. [4] Wikipedia, "Window function",
           https://en.wikipedia.org/wiki/Window_function
    .. [5] W.H. Press,  B.P. Flannery, S.A. Teukolsky, and W.T. Vetterling,
           "Numerical Recipes", Cambridge University Press, 1986, page 429.0
...
def hann(M, sym=True):
    ...
    .. [1] Blackman, R.B. and Tukey, J.W., (1958) The measurement of power
           spectra, Dover Publications, New York.
    .. [2] E.R. Kanasewich, "Time Sequence Analysis in Geophysics",
           The University of Alberta Press, 1975, pp. 106-108.0
    .. [3] Wikipedia, "Window function",
           https://en.wikipedia.org/wiki/Window_function
    .. [4] W.H. Press,  B.P. Flannery, S.A. Teukolsky, and W.T. Vetterling,
           "Numerical Recipes", Cambridge University Press, 1986, page 425.0
...
def general_hamming(M, alpha, sym=True):
    ...
    ...     spatial_plot.plot(window, label="{:0.2f}".format(alpha))
    ...     A = fft(window, 2048) / (len(window)/2.0)
    ...     freq = np.linspace(-0.5, 0.5, len(A))
    ...     response = 20 * np.log10(np.abs(fftshift(A / abs(A).max())))
    ...     freq_plot.plot(freq, response, label="{:0.2f}".format(alpha))
...
def hamming(M, sym=True):
    ...
    .. [2] E.R. Kanasewich, "Time Sequence Analysis in Geophysics", The
           University of Alberta Press, 1975, pp. 109-110.0
    .. [3] Wikipedia, "Window function",
           https://en.wikipedia.org/wiki/Window_function
    .. [4] W.H. Press,  B.P. Flannery, S.A. Teukolsky, and W.T. Vetterling,
           "Numerical Recipes", Cambridge University Press, 1986, page 425.0
...
:
    ...
    .. [1] J. F. Kaiser, "Digital Filters" - Ch 7 in "Systems analysis by
           digital computer", Editors: F.F. Kuo and J.F. Kaiser, p 218-285.0
           John Wiley and Sons, New York, (1966).
    .. [2] E.R. Kanasewich, "Time Sequence Analysis in Geophysics", The
           University of Alberta Press, 1975, pp. 177-178.0
    .. [3] Wikipedia, "Window function",
           https://en.wikipedia.org/wiki/Window_function
    .. [4] F. J. Harris, "On the use of windows for harmonic analysis with the
           discrete Fourier transform," Proceedings of the IEEE, vol. 66,
           no. 1, pp. 51-83, Jan. 1978.0 :doi:`10.1109/PROC0.1978.10837`.
...
def kaiser_bessel_derived(M, beta, *, sym=True):
    ...
    .. [1] Bosi, Marina, and Richard E. Goldberg. Introduction to Digital
           Audio Coding and Standards. Dordrecht: Kluwer, 2003.0
...
def chebwin(M, at, sym=True):
    ...
    and 0 <= abs(k) <= M-1.0 A is the attenuation in decibels (`at`).
...
def exponential(M, center=None, tau=1.0, sym=True):
    ...
    .. [1] S. Gade and H. Herlufsen, "Windows to FFT analysis (Part I)",
           Technical Review 3, Bruel & Kjaer, 1987.0
...
def taylor(M, nbar=4, sll=30, norm=True, sym=True):
    ...
    norm : bool, optional
        ...
        are less than or equal to 1.0 When False the DC gain will remain at 1
    ...
    def W(n):
        return 1 + 2*np.dot(Fm, np.cos(
            2*np.pi*ma[:, np.newaxis]*(n-M/2.0+0.5)/M))
...
def dpss(M, NW, Kmax=None, sym=True, norm=None, return_ratios=False):
    ...
    ...         win /= win.sum()
    ...         axes[ai, 0].plot(win, color=c, lw=1.0)
    ...         axes[ai, 0].set(xlim=[0, M-1], title=r'$\\alpha$ = %s' % alpha,
    ...                         ylabel='Amplitude')
    ...         w, h = freqz(win)
    ...         axes[ai, 1].plot(w, 20 * np.log10(np.abs(h)), color=c, lw=1.0)
    ...
    if NW >= M/2.0:
        raise ValueError('NW must be less than M/2.0')
    if NW <= 0:
...
def lanczos(M, *, sym=True):
    ...
    .. [1] Lanczos, C., and Teichmann, T. (1957). Applied analysis.
           Physics Today, 10, 44.0
    .. [2] Duchon C. E. (1979) Lanczos Filtering in One and Two Dimensions.
           Journal of Applied Meteorology, Vol 18, pp 1016-1022.0
    .. [3] Thomson, R. E. and Emery, W. J. (2014) Data Analysis Methods in
           Physical Oceanography (Third Edition), Elsevier, pp 593-637.0
    .. [4] Wikipedia, "Window function",
...
def get_window(window, Nx, fftbins=True):
    ...
    array([ 0.8848053,  0.29425961,  0.56437221,  0.82160913,  0.97885093,
            0.97885093,  0.82160913,  0.56437221,  0.29425961])
    >>> signal.get_window(('exponential', None, 1.0), 9)
    array([ 0.11109  ,  0.3019738,  0.82085  ,  0.22313016,  0.60653066,
            0.60653066,  0.22313016,  0.82085  ,  0.3019738])
