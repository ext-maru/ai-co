...
class LinearOperator:
    ...
    def _rmatvec(self, x)if type(self)._adjoint == LinearOperator._adjoint:
    ...
    def _rmatmat(self, X)if type(self)._adjoint == LinearOperator._adjointreturn np.hstack([self.rmatvec(col.reshape(-1, 1)) for col in X.T])
    ...
    def _adjoint(self)return _AdjointLinearOperator(self)
    """Default implementation of _adjoint; defers to rmatvec."""

    def _transpose(self)return _TransposedLinearOperator(self)
