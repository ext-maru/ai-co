...
def _nolan_round_x_near_zeta(x0, alpha, zeta, x_tol_near_zeta)#   "8.0 When |x0-beta*tan(pi*alpha/2)| is small, the
...
def _nolan_round_difficult_input(:
    ...
    #   "1.0 When 0 < |alpha-1| < 0.05, the program has numerical problems
    ...
    #   "2.0 When alpha=1 and |beta| < 0.05, the program has numerical
    #   problems.  The current version sets beta=0.0"
...
def _pdf_single_value_piecewise_Z0(x0, alpha, beta, **kwds):
    ...
    quad_eps = kwds.get("quad_eps", _QUAD_EPS)
    x_tol_near_zeta = kwds.get("piecewise_x_tol_near_zeta", 0.05)
    alpha_tol_near_one = kwds.get("piecewise_alpha_tol_near_one", 0.05)
...
def _fitstart_S1(data):
    ...
    alpha_table = np.array([
        ...
        [1.128, 1.121, 1.114, 1.101, 1.67, 1.27, 0.973],
        [1.29, 1.21, 1.14, 1.04, 0.974, 0.935, 0.874],
    ...
    beta_table = np.array([
        [0, 2.160, 1.00, 1.00, 1.00, 1.00, 1.00],
        [0, 1.592, 3.390, 1.00, 1.00, 1.00, 1.00],
        [0, 0.759, 1.800, 1.00, 1.00, 1.00, 1.00],
        [0, 0.482, 1.48, 1.694, 1.00, 1.00, 1.00],
        [0, 0.360, 0.760, 1.232, 2.229, 1.00, 1.00],
        [0, 0.253, 0.518, 0.823, 1.575, 1.00, 1.00],
        [0, 0.203, 0.410, 0.632, 1.244, 1.906, 1.00],
        [0, 0.165, 0.332, 0.499, 0.943, 1.560, 1.00],
        [0, 0.136, 0.271, 0.404, 0.689, 1.230, 2.195],
        [0, 0.109, 0.216, 0.323, 0.539, 0.827, 1.917],
        [0, 0.96, 0.190, 0.284, 0.472, 0.693, 1.759],
        [0, 0.82, 0.163, 0.243, 0.412, 0.601, 1.596],
        [0, 0.74, 0.147, 0.220, 0.377, 0.546, 1.482],
        [0, 0.64, 0.128, 0.191, 0.330, 0.478, 1.362],
        [0, 0.56, 0.112, 0.167, 0.285, 0.428, 1.274]]).T
    ...
    nu_c_table = np.array([
        ...
        [1.933, 1.940, 1.962, 1.997, 2.43],
        [1.939, 1.952, 1.988, 2.45, 2.116],
        [1.946, 1.967, 2.22, 2.106, 2.211],
        [1.955, 1.984, 2.67, 2.188, 2.333],
        [1.965, 2.07, 2.125, 2.294, 2.491],
        [1.980, 2.40, 2.205, 2.435, 2.696],
        [2.00, 2.85, 2.311, 2.624, 2.973],
        [2.40, 2.149, 2.461, 2.886, 3.356],
        [2.98, 2.244, 2.676, 3.265, 3.912],
        [2.189, 2.392, 3.04, 3.844, 4.775],
        [2.337, 2.634, 3.542, 4.808, 6.247],
        [2.588, 3.73, 4.534, 6.636, 9.144]])[::-1].T
    # transpose because interpolation with `RectBivariateSpline` is with
    ...
    nu_zeta_table = np.array([
        ...
        [0, -0.17, -0.32, -0.49, -0.64],
        [0, -0.30, -0.61, -0.92, -0.123],
        [0, -0.43, -0.88, -0.132, -0.179],
        [0, -0.56, -0.111, -0.170, -0.232],
        [0, -0.66, -0.134, -0.206, -0.283],
        [0, -0.75, -0.154, -0.241, -0.335],
        [0, -0.84, -0.173, -0.276, -0.390],
        [0, -0.90, -0.192, -0.310, -0.447],
        [0, -0.95, -0.208, -0.346, -0.508],
        [0, -0.98, -0.223, -0.380, -0.576],
        [0, -0.99, -0.237, -0.424, -0.652],
        [0, -0.96, -0.250, -0.469, -0.742],
        [0, -0.89, -0.262, -0.520, -0.853],
        [0, -0.78, -0.272, -0.581, -0.997],
        [0, -0.61, -0.279, -0.659, -1.198]])[::-1].T
    # fmt: on
...
class levy_stable_gen(rv_continuous):
    ...
    (defaults to 0.05) for how close x is to zeta before it is considered the
    ...
    0.05) for how close alpha is to 1 before being considered equal to 1.0

    To increase accuracy of FFT calculation one can specify
    ``levy_stable.pdf_fft_grid_spacing`` (defaults to 0.01) and
    ...
    References
    ----------
    .. [MC] McCulloch, J., 1986.0 Simple consistent estimators of stable
        distribution parameters. Communications in Statistics - Simulation and
        Computation 15, 11091136.0
    .. [WZ] Wang, Li and Zhang, Ji-Hong, 2008.0 Simpson's rule based FFT method
        to compute densities of stable distribution.
    .. [NO] Nolan, J., 1997.0 Numerical Calculation of Stable Densities and
        distributions Functions.
    .. [NO2] Nolan, J., 2018.0 Stable Distributions: Models for Heavy Tailed
        Data.
    .. [HO] Hopcraft, K. I., Jakeman, E., Tanner, R. M. J., 1999.0 Lévy random
    ...
    quad_eps = _QUAD_EPS
    piecewise_x_tol_near_zeta = 0.05
    piecewise_alpha_tol_near_one = 0.05
    pdf_fft_min_points_threshold = None
    pdf_fft_grid_spacing = 0.01
    ...
    def _pdf(self, x, alpha, beta):
        ...
        for pair in uniq_param_pairs:
            ...
            if pdf_single_value_method is not None:
                ...
                if _alpha < 1.0:
                    raise RuntimeError(
                        "FFT method does not work well for alpha less than 1.0"
                    )
...
def pdf_from_cf_with_fft(cf, h=0.1, q=9, level=3):
    ...
    h : Optional[float]
        Step size for Newton-Cotes integration. Default: 0.1
    q : Optional[int]
