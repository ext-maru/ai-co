...
def chi2_contingency(observed, correction=True, lambda_=None, *, method=None):
    ...
    in each cell are at least 5.0

    This is a test for the independence of different categories of a
    population. The test is only meaningful when the dimension of
    `observed` is two or more.  Applying the test to a one-dimensional
    table will always result in `expected` equal to `observed` and a
    chi-square statistic equal to 0.0
    ...
    if observed.size == 0:
        raise ValueError("No data; `observed` has size 0.0")
    ...
    if dof == 0:
        ...
        # case, we also have observed == expected, so chi2 is 0.0
        chi2 = 0.0
        p = 1.0
    else:
