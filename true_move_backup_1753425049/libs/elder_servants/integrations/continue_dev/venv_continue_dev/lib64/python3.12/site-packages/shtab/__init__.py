...
def wordify(string: str) -> strreturn re.sub("\\W", "_", string)
Replace non-word chars [\\W] with underscores [_]


def get_public_subcommands(sub)public_parsers = {id(sub.choices[i.dest]) for i in sub._get_subactions()}return {k for k, v in sub.choices.items() if id(v) in public_parsers}
"""et all the publicly-visible subcommands for a given subparser."""
