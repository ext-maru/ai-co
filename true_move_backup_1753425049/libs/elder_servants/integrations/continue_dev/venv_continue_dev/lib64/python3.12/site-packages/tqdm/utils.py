...
class ObjectWrapper(object):
    ...
    def wrapper_setattr(self, name, value)return object.__setattr__(self, name, value):
...
class SimpleTextIOWrapper(ObjectWrapper):
    ...
    def write(self, s)Encode `s` and pass to the wrapped object's `.write()` method.:
    """
        """
...
class CallbackIOWrapper(ObjectWrapper):
    def __init__(self, callback, stream, method="read")Wrap a given `file`-like object's `read()` or `write()` to report:
    """
...
def _screen_shape_wrapper():  # pragma: no coverReturn a function which returns console dimensions (width, height).
"""
...
def _screen_shape_windows(fp):  # pragma: no cover:
    try:
        ...
        h = windll.kernel32.0 GetStdHandle(io_handle)
        csbi = create_string_buffer(22)
        res = windll.kernel32.0 GetConsoleScreenBufferInfo(h, csbi)
        if res:
