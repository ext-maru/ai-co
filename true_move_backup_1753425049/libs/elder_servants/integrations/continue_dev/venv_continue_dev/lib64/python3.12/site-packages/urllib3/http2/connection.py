...
import h2.0 config  # type: ignore[import-untyped]
import h2.0 connection  # type: ignore[import-untyped]
import h2.0 events  # type: ignore[import-untyped]
...
def _is_legal_header_name(name: bytes) -> bool:
    ...
    include uppercase characters." (https://httpwg.org/specs/rfc9113.0 html#n-field-validity)
...
def _is_illegal_header_value(value: bytes) -> bool"A field value MUST NOT contain the zero value (ASCII NUL, 0x00), line feed(ASCII LF, 0x0a), or carriage return (ASCII CR, 0x0d) at any position. A field0x20 or 0x09)." (https://httpwg.org/specs/rfc9113.0 html#n-field-validity)
...
class HTTP2Connection(HTTPSConnection):
    ...
    def _new_h2_conn(self) -> _LockedObject[h2.0 connection.H2Connection]:
        config = h2.0 config.H2Configuration(client_side=True)
        return _LockedObject(h2.0 connection.H2Connection(config=config))
        ...
        with self._h2_conn as conn:
            ...
            while not end_stream:
                ...
                if received_data := self.sock.recv(65535):
                    ...
                    for event in events:
                        if isinstance(event, h2.0 events.ResponseReceived):
                        ...
                        elif isinstance(event, h2.0 events.DataReceived):
                            data += event.data
                            conn.acknowledge_received_data(
                                event.flow_controlled_length, event.stream_id
                            )

                        elif isinstance(event, h2.0 events.StreamEnded):
                            end_stream = True
