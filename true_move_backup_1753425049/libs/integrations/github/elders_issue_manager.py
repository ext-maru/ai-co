...
class EldersIssueManager:
    ...
    async def create_sub_issues_from_epic(self, epic_issue: Issue) -> List[Issue]:
        ...
        for item in checklist_items:
            ...
            if sub_issue:
                ...
                await self._update_epic_checklist(
                    epic_issue, item["text"], sub_issue.number
                )
    ...
    async def update_progress_chart(self, master_issue: Issue):
        ...
        logger.info(
            f"Updated progress for Issue #{master_issue.number}: {progress_data['percentage']}%"
        )
    ...
    async def handle_pr_merge(self, pr: PullRequest):
        ...
        for issue_number in related_issues:
            try:
                ...
                logger.info(
                    f"Auto-closed Issue #{issue_number} due to PR #{pr.number} merge"
                )
    ...
    def _parse_checklist(self, body: str) -> List[Dict[str, Any]]:
        ...
        return [{"checked": match[0] == "x", "text": match[1]} for match in matches]
    ...
    async def _create_sub_issue(
        self, epic_issue: Issue, item: Dict[str, Any]
    ) -> Optional[Issue]:
        """Sub Issueを作成"""
        try:
            ...
            sub_issue = self.repo.create_issue(title=title, body=body, labels=labels)
    ...
    async def _update_epic_checklist(
        self, epic_issue: Issue, item_text: str, sub_issue_number: int
    ):
        """Epic Issueのチェックリストを更新"""
    ...
    async def _gather_sage_insights(self) -> Dict[str, Any]:
        ...
        insights["knowledge"] = await self.knowledge_sage.process_request(
            {"type": "get_insights", "topic": "issue_management"}
        )
...
async def main():
    ...
    parser.add_argument(
        "command", choices=["sync", "report", "auto-assign", "create-subs"]
    )
