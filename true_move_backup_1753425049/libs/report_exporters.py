...
class PDFExporter:
    ...
    def export(
        self, html_content: str, data: Dict[str, Any], report_id: str, output_dir: Path
    ) -> Path:
        """PDFエクスポート"""
    ...
    def _export_fallback(
        self, html_content: str, data: Dict[str, Any], file_path: Path
    ) -> Path:
...
class MarkdownExporter:
    ...
    def _generate_markdown(self, data: Dict[str, Any]) -> str:
        ...
        if "period" in data:
            lines.append(
                f"**期間:** {data['period'].get('start', '')} - {data['period'].get('end', '')}"
            )
            lines.append("")
...
if __name__ == "__main__":
    ...
    test_data = {
        ...
        "period": {"start": "2025-07-08", "end": "2025-07-09"},
        "health_score": 95.5,
        "health_change": 2.3,
        "sages": [
            {
        ...
        "tasks": {"completed": 45, "in_progress": 12, "completion_rate": 78.9},
    }
