...
from libs.session_management.models import (
    SageInteraction,
    SessionContext,
    SessionMetadata,
)
...
class ElderZanSecurityLayer:
    ...
    async def decrypt_data(
        self, encrypted_data: Dict[str, Any], context: Optional[SecurityContext] = None
    ...
    async def secure_storage_operation(
        self,
        operation: str,
        ...
        try:
            ...
            if self.encryption_enabled:
                encrypted_context = await self.encrypt_data(
                    session_context.to_dict(), security_context
                )
                # 暗号化データでセッションを更新
            ...
            if operation == "save":
                ...
                result = await self.hybrid_storage.load_session(
                    session_context.metadata.session_id
                )
            else:
