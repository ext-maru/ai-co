...
class CommandAuditor:
    ...
    def check_command_usage(self, cmd_name):
        ...
        # 非推奨の可能性を判定
        if (
            len(usage_info["referenced_in_code"]) == 0
            and usage_info["log_mentions"] == 0
        ):
                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            ...
                            for other_cmd in other_cmds:
                                if (
                                    other_cmd != cmd_name
                                    and other_cmd in self.results["commands"]
                                ):
    ...
    def generate_recommendations(self):
        ...
        if (
            "ai-code" in self.results["commands"]
            and "ai-send" in self.results["commands"]
        ):
            recommendations.append(
                {
    ...
    """監査を実行"""
        ...
        self.results["summary"] = {
            ...
            "implemented_commands": len(
                [
                    c
                    for c, i in self.results["commands"].items()
                    if i["implementation_type"]
                ]
            ),
            "likely_deprecated": len(
                [
                    c
                    for c, i in self.results["commands"].items()
            ...
            "commands_with_issues": len(
                [c for c, i in self.results["commands"].items() if i["issues"]]
            ),
        }
