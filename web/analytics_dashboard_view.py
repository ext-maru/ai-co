...
class AnalyticsDashboardView:
    ...
    def _format_metrics(self, analysis_type, metrics):
        ...
        if analysis_type == 'commit_pattern':
            ...
            formatted.append({
                'label': '承認率',
                'value': f"{metrics.get('approval_rate', 0):.1f}",
                'unit': '%'
            })
            formatted.append({
                'label': '平均実行時間',
                'value': f"{metrics.get('avg_execution_time', 0):.1f}",
                'unit': '秒'
            })
            ...
            for sage_name, stats in sage_perf.items():
                formatted.append({
                    'label': f'{sage_name}承認率',
                    'value': f"{stats.get('approval_rate', 0) * 100:.1f}",
                    'unit': '%'
                })
            ...
            formatted.append({
                'label': 'ヘルススコア',
                'value': f"{metrics.get('current_health_score', 0):.0f}",
                'unit': '点'
            })
            formatted.append({
                'label': 'エラー率',
