...
class FastAuthenticationEngine:
    ...
    def _cleanup_expired_cache(self):
        ...
        # トークンキャッシュクリーンアップ
        expired_tokens = [
            token for token, (_, cached_time) in self.token_cache.items()
        ...
        # ユーザーキャッシュクリーンアップ
        expired_users = [
            user_id for user_id, (_, cached_time) in self.user_cache.items()
        ...
        # 権限キャッシュクリーンアップ
        expired_permissions = [
            user_id for user_id, (_, cached_time) in self.permission_cache.items()
            if (current_time - cached_time) > self.cache_ttl['permission']
        ]
        for user_id in expired_permissions:
            del self.permission_cache[user_id]
        
        logger.info(f"キャッシュクリーンアップ完了: tokens={len(expired_tokens)}, users={len(expired_users)}, permissions={len(expired_permissions)}")
    ...
    def generate_test_token(self, user_id: str, expires_in_minutes: int = 60) -> str:
        ...
        payload = {
            'user_id': user_id,
            'iat': time.time(),
            'exp': time.time() + (expires_in_minutes * 60)
        }
...
def run_benchmark():
    ...
    try:
        ...
        for _ in range(iterations):
            start_time = time.time()
            user_data = fast_auth_engine.authenticate_fast(test_token)
            response_time = (time.time() - start_time) * 1000
