...
from web.auth_manager import AuthManager, LoginRequired, RoleRequired, AuthenticationError, AuthorizationError
...
auth_manager = AuthManager(
    db_path=str(config.PROJECT_ROOT / 'data' / 'auth.db'),
...
def login():
    ...
    if form.validate_on_submit():
        try:
            user, session_obj = auth_manager.authenticate(
                form.username.data,
                form.password.data
            )
...
def api_clear_sessions():
    ...
    try:
        ...
        return jsonify({
            'message': f'{cleaned}個の期限切れセッションをクリアしました',
            'cleaned': cleaned
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
