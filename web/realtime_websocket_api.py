        ...
        while self.is_monitoring:
            for monitor_id, monitor in self.monitors.items():
                if monitor["status"] == "active":
                    ...
                    if result:
                        # 変更があればブロードキャスト
                        self.websocket.broadcast(f"monitor:{monitor['type']}", {
...
class CollaborationSessionManager:
    ...
    def create_session(self, session_type: str, participants: List[str], objective: str) -> str:
        ...
        # セッション作成通知
        self.websocket.broadcast(f"session:{session_id}", {
...
class LiveNotificationSystem:
    ...
    def send_notification(self, user_id: str, notification_type: str, 
        ...
        # ユーザーチャンネルへ通知
        self.websocket.broadcast(f"user:{user_id}", {
            "event": "notification",
    ...
    def broadcast_announcement(self, title: str, message: str, channels: List[str] = None):
        ...
        if channels:
            for channel in channels:
                self.websocket.broadcast(channel, {
                    "event": "announcement",
                    "announcement": announcement
                })
        else:
            # 全接続にブロードキャスト
            self.websocket.broadcast("global", {
                "event": "announcement",
                "announcement": announcement
            })
