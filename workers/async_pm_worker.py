...
class TaskPhaseManager:
    ...
    async def execute_task_phases(
        self, task_context: TaskContext, phases_to_run: List[str]
    ) -> Dict[str, Any]:
        """タスクフェーズの並列実行"""
        async with self.task_semaphore:
            ...
            try:
                ...
                for phase in phases_to_run:
                    ...
                    # フェーズ実行
                    phase_result = await self._execute_single_phase(task_context, phase)
    ...
    async def _execute_single_phase(
        self, context: TaskContext, phase: str
    ) -> Dict[str, Any]:
        """単一フェーズの実行"""
        self.logger.info(
            "Executing phase",
    ...
    async def _phase_development(self, context: TaskContext) -> Dict[str, Any]:
        ...
        # 並列ファイル作成
        tasks = [
            self._create_file_async(context, filename) for filename in files_to_create
        ]
...
class AsyncPMWorker(AsyncBaseWorker):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.cache_manager = CacheManager(
            redis_client=self.redis_client, default_ttl=config.get("cache_ttl", 3600)
        )
    ...
    async def process_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # タスクコンテキスト作成
        context = TaskContext(
            task_id=task_id, created_at=datetime.utcnow(), status="processing"
        )
    ...
    async def _handle_project_mode(
        self, context: TaskContext, message: Dict[str, Any]
    ) -> Dict[str, Any]:
        """プロジェクトモードの処理"""
        phases = ["requirements", "design", "development", "testing", "deployment"]

        # 並列フェーズ実行
        phase_results = await self.phase_manager.execute_task_phases(context, phases)
    ...
    async def _record_statistics(self):
        ...
        # Redisに保存
        await self.cache_manager.set("pm_worker_stats", stats, ttl=86400)
...
async def main():
    ...
    async def _consult_elder_guidance(
        self, context: TaskContext, message: Dict[str, Any]
    ):
        """エルダーガイダンスの取得"""
        try:
            ...
            # Task Sageに最適化相談
            task_guidance = await self._consult_task_sage_for_optimization(
                context, message
            )
            self.pm_optimization_state["task_sage_guidance"] = task_guidance

            # Knowledge Sageにパターン分析依頼
            knowledge_patterns = await self._get_knowledge_sage_patterns(
                context, message
            )
            self.pm_optimization_state["knowledge_sage_patterns"] = knowledge_patterns

            # Incident Sageに監視設定
            incident_monitoring = await self._setup_incident_sage_monitoring(
                context, message
            )
    ...
    async def _consult_task_sage_for_optimization(
        ...
        try:
            ...
            # 4賢者システムに学習セッション要求
            session_result = self.four_sages_integration.coordinate_learning_session(
                learning_request
            )

            # Task Sageの具体的な推奨事項を抽出
            task_recommendations = self._extract_task_sage_recommendations(
                session_result
            )
    ...
    async def _get_knowledge_sage_patterns(
        self, context: TaskContext, message: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Knowledge Sageに分析依頼
            pattern_result = self.four_sages_integration.facilitate_cross_sage_learning(
                pattern_request
            )
            ...
            return {
                ...
                "best_practices": ["イテレーション管理", "ステークホルダー管理", "リスク管理", "品質保証"],
            }
    ...
    async def _setup_incident_sage_monitoring(
        self, context: TaskContext, message: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            monitoring_config = {
                "task_id": context.task_id,
                "monitoring_points": ["フェーズ実行時間", "エラー発生率", "リソース使用状況", "タスク完了率"],
                "alert_thresholds": {
            ...
            # Incident Sageに監視設定
            monitoring_result = await self._request_incident_sage_monitoring(
                monitoring_config
            )
    ...
    async def _get_rag_sage_project_analysis(
        self, context: TaskContext, message: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _report_to_knowledge_sage(
        self, context: TaskContext, result: Dict[str, Any]
    ):
        """Knowledge Sageへの結果報告"""
        try:
            report_data = {
            ...
            await self.four_sages_integration.report_to_claude_elder(
                "knowledge_sage", "project_completion_report", report_data
            )
    ...
    async def _escalate_critical_issue(
        self, task_id: str, error: str, context: TaskContext
    ):
        ...
        try:
            ...
            await self.four_sages_integration.escalate_to_grand_elder(
                "pm_critical_failure", "high", escalation_data
            )
            ...
            stats = {
                ...
                "elder_consultations": {
                    "total_consultations": len(
                        self.pm_optimization_state.get("elder_recommendations", [])
                    ),
                    "last_consultation": self.pm_optimization_state.get(
                        "last_elder_consultation"
                    ),
    ...
    def _extract_task_sage_recommendations(
        self, session_result: Dict[str, Any]
    ) -> List[str]:
        """Task Sage推奨事項抽出"""
        # 簡略化実装
        return ["タスクの並列化を検討", "リソース使用量を最適化", "エラーハンドリングを強化", "フェーズ間のデータ共有を改善"]

    async def _request_incident_sage_monitoring(
    ...
    async def _request_rag_sage_analysis(
    ...
    async def _record_best_practice(self, context: TaskContext, result: Dict[str, Any]):
        ...
        # Knowledge Sageに記録
        await self.four_sages_integration.report_to_claude_elder(
            "knowledge_sage", "best_practice_record", best_practice
        )

    async def _request_improvement_suggestions(
        self, context: TaskContext, result: Dict[str, Any]
    ):
        """改善提案要求"""
        improvement_request = {
        ...
        # Task Sageに改善提案要求
        await self.four_sages_integration.report_to_claude_elder(
            "task_sage", "improvement_request", improvement_request
        )

    async def _send_elder_council_report(
        self, context: TaskContext, result: Dict[str, Any]
    ):
        """エルダー評議会への報告"""
        council_report = {
    ...
    def _extract_success_factors(self, result: Dict[str, Any]) -> List[str]:
        """成功要因抽出"""
        return ["適切なフェーズ管理", "効率的なリソース使用", "適切なエラーハンドリング"]

    def _extract_issues(self, result: Dict[str, Any]) -> List[str]:
        """問題抽出"""
        return ["実行時間が長い", "エラー発生率が高い", "リソース使用量が多い"]
